...
TL;DR: EvidFuse是一个免训练的多智能体框架，用于数据驱动报告的文本-图表交织生成，解决了现有LLM系统中图表-文本不一致和证据冻结的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的系统采用分阶段管道（文本优先-图表第二或图表优先-文本第二），导致图表-文本不一致和"证据冻结"问题，即中间证据空间固定后，模型无法随着叙述发展检索或构建新的视觉证据，导致分析浅显且预定义。

Method: EvidFuse通过两个协作组件实现：1) 数据增强分析智能体，配备EDA衍生知识和原始表格访问能力；2) 实时证据构建写入器，规划大纲并起草报告，同时间歇性地发出细粒度分析请求。这种设计允许在叙述需要时精确构建和整合视觉证据。

Result: 实验表明，EvidFuse在LLM作为评判者和人类评估中，在图表质量、图表-文本对齐和报告级有用性方面均获得最高排名。

Conclusion: EvidFuse通过解耦可视化分析和长文本起草，实现了写作时的文本-图表交织生成，能够按需扩展证据空间，直接约束后续主张，从而生成更深入、更一致的数据驱动报告。
