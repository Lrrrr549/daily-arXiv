...
TL;DR: Smart Transfer是一个利用视觉基础模型进行地震后建筑物损坏快速制图的GeoAI框架，通过像素级聚类和距离惩罚三元组两种新颖的迁移策略，在跨区域迁移设置中表现出色。


<details>
  <summary>Details</summary>
Motivation: 气候变化导致自然灾害更加频繁和严重，黄金72小时的快速灾害响应成为人道主义需求。传统灾害损害调查方法难以泛化到不同城市形态和新灾害事件，且需要耗时的手动数据标注。

Method: 提出Smart Transfer框架，采用两种新颖的模型迁移策略：1) 像素级聚类(PC)，确保鲁棒的原型级全局特征对齐；2) 距离惩罚三元组(DPT)，通过为语义不一致但空间相邻的斑块分配更强惩罚，整合斑块级空间自相关模式。

Result: 在2023年土耳其-叙利亚地震数据集上的广泛实验和消融研究表明，在Leave One Domain Out (LODO)和Specific Source Domain Combination (SSDC)两种跨区域迁移设置中均表现出有前景的性能。
...
TL;DR: 提出了一种用于长文档视觉理解的合成数据生成管道，通过生成思维轨迹并训练模型内部化推理能力，在多个基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 长文档视觉理解在商业、法律和科学应用中至关重要，但现有最佳开放方法尚未充分探索推理能力，而推理能力已在数学和代码任务中展现出显著性能提升。

Method: 开发了合成数据管道：1) 评估每页与问题的相关性；2) 提取文本证据并按相关性排序；3) 在<think>标签内使用SFT训练思维轨迹，通过<cot>控制令牌控制；4) 通过低强度模型融合内部化推理能力。在Qwen3 VL 32B和Mistral Small 3.1 24B模型上进行研究。

Result: Qwen3 VL 32B在MMLongBenchDoc上达到58.3分，超越7倍大的Qwen3 VL 235B A22B（57.0分）。Mistral模型上，合成推理方法比从Thinking版本蒸馏的方法在MMLBD-C上高出3.8分，内部化推理比显式推理平均输出token减少12.4倍。

Conclusion: 合成数据管道能有效提升长文档视觉理解的推理能力，使较小模型超越更大模型性能，同时内部化推理显著减少输出长度。管道已开源供复现和进一步探索。
...
TL;DR: STEAR是一个层感知的时空证据干预框架，通过识别高风险解码步骤、选择中间层的令牌条件视觉证据，来缓解视频大语言模型的时空幻觉问题。
...
Method: STEAR识别高风险解码步骤，从基础敏感中间层选择令牌条件视觉证据。该共享证据用于两个耦合目的：恢复中间层缺失的局部基础，以及构建时间扰动的补丁级反事实来证伪后期解码中的不一致推理。

Result: 在代表性Video-LLM主干和挑战性基准测试中，STEAR一致减少幻觉，同时提高忠实性、时间一致性和鲁棒性。结果证实可靠的视频解码依赖于在正确层对精确证据进行干预，而非强制执行全局惩罚。

Conclusion: STEAR在高效的单编码推理框架内缓解空间和时间幻觉，证明了层感知干预的重要性。代码已在补充材料中提供。
...
<details>
  <summary>Details</summary>
Motivation: 扩散和流生成模型在图像恢复中表现出潜力，但在未知和变化的噪声条件下，学习的向量场在不同噪声水平间可能不一致，导致训练与推理不匹配时恢复质量下降

Method: 首先从局部像素统计估计输入噪声水平，然后使用该定量估计调整推理轨迹，包括起始点、积分步数和步长调度，使去噪过程与每个输入的实际退化水平对齐

Result: 通过将定量噪声估计与噪声自适应流推理相结合，提高了恢复精度和推理效率，在自然、医学和显微镜图像上的实验证明了其在不同噪声水平和成像条件下的鲁棒性和强泛化能力

Conclusion: 提出的定量流匹配框架通过自适应调整推理轨迹，有效解决了未知和变化噪声条件下的图像去噪问题，在多种图像类型和噪声条件下表现出优越性能
...
TL;DR: 提出基于多模态视觉特征融合的环境感知信道预测框架，用于6G车联网通信，通过融合GPS、全景图像、语义分割和深度估计实现多参数联合预测。


<details>
  <summary>Details</summary>
Motivation: 6G通信与智能感知深度融合需要环境感知的信道预测技术。车联网通信对信道预测的准确性、前瞻性、可靠性和适应性有严格要求，传统经验模型在准确性、泛化性和部署性之间存在局限，而车载和路侧感知设备提供了丰富的环境先验信息。

Method: 基于多模态视觉特征融合的框架：使用GPS数据和车辆侧全景RGB图像，结合语义分割和深度估计，通过三分支架构提取语义、深度和位置特征，采用挤压-激励注意力门控模块进行自适应多模态融合。针对360维角功率谱预测，设计了专用回归头和复合多约束损失函数，实现路径损耗、时延扩展、到达方位角扩展、离开方位角扩展和角功率谱的联合预测。

Result: 在城市V2I测量数据集上的实验结果显示：路径损耗的均方根误差为3.26 dB，时延扩展、到达方位角扩展、离开方位角扩展的均方根误差分别为37.66 ns、5.05度、5.08度，角功率谱的余弦相似度均值/中值为0.9342/0.9571，表现出优异的准确性、泛化性和实际应用潜力。

Conclusion: 该环境感知信道预测框架通过多模态视觉特征融合，在6G车联网通信中实现了高精度、强泛化能力的信道预测，为智能信道预测提供了有效的解决方案，展示了在6G车联网通信中的实际应用价值。
...
TL;DR: 提出VE-MD框架，通过变分编码器-多解码器结构实现隐私感知的群体情绪识别，避免个体监控，仅预测群体层面情感，同时利用结构监督提升表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有群体情绪识别方法依赖个体层面处理（如裁剪人脸、人员跟踪），导致分析流程以人为中心，在仅需群体层面理解的部署场景中引发隐私担忧。需要设计避免显式个体监控的隐私感知方法。

Method: 提出VE-MD（变分编码器-多解码器）框架：1）学习共享潜在表示，联合优化情绪分类和内部身体/面部结构表示预测；2）研究两种结构解码策略：基于Transformer的PersonQuery解码器和适应可变群体大小的密集热图解码器；3）设计为仅预测聚合群体层面情感，不进行身份识别或个人情绪输出。

Result: 在6个真实数据集（2个GER、4个IER基准）上实验：1）结构监督持续改善表示学习；2）揭示GER与IER的关键区别：仅优化潜在空间对GER不足（会减弱交互相关线索），而保留显式结构输出改善集体情感推断；3）在GAF-3.0上达到90.06% SOTA，VGAF上82.25%（音频多模态融合）；4）在IER数据集上，使用音频多模态融合在SamSemo达77.9%（添加文本模态），在MER-MULTI、DFEW、EngageNet上取得竞争性表现。

Conclusion: VE-MD框架通过结构监督和隐私感知设计，在不依赖先验个体特征提取的情况下有效建模群体层面情感。保留交互相关结构信息对群体情感建模特别有益，而投影结构表示对个体情绪识别起到有效去噪瓶颈作用。该方法在隐私敏感场景中具有实用价值。
...
TL;DR: LumiVideo是一个模仿专业调色师认知工作流程的智能系统，通过感知、推理、执行和反思四个阶段，将原始log视频自动转换为电影级调色效果，提供可解释的参数控制和行业标准输出格式。


<details>
  <summary>Details</summary>
Motivation: 现有自动化调色方法存在两个主要问题：1) 作为静态黑盒执行器直接输出编辑像素，缺乏可解释性；2) 缺乏专业调色师所需的迭代控制能力。需要一种能模仿专业调色师认知工作流程的系统。

Method: LumiVideo采用四阶段智能系统：1) 感知阶段分析场景物理光照和语义内容；2) 推理阶段结合LLM的电影知识、RAG框架和Tree of Thoughts搜索来导航非线性色彩参数空间；3) 执行阶段编译参数为行业标准ASC-CDL配置和全局一致的3D LUT；4) 反思阶段允许通过自然语言反馈进行结果优化。

Result: 实验表明，LumiVideo在全自动模式下接近人类专家质量，同时在受指导时能实现精确的迭代控制。作者还创建了首个log编码视频基准LumiGrade用于评估自动化调色。

Conclusion: LumiVideo通过模仿专业调色师认知工作流程，解决了现有自动化调色方法的可解释性和控制性不足问题，实现了接近人类专家质量的自动调色，同时支持自然语言交互的迭代优化。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在开发自动检测小提琴宽度缩减的方法，这对于乐器制造、修复和质量控制具有重要意义。传统方法依赖人工测量，而3D扫描和机器学习提供了自动化解决方案的可能性。

Method: 使用3D摄影测量网格获取小提琴表面数据。比较两种方法：1) 基于高程图的几何原始表示，应用SVM和决策树分类器；2) 特征工程方法，通过参数化轮廓线拟合提取特征。两种方法都用于检测小提琴宽度缩减。

Result: 虽然基于高程图的方法偶尔能获得较强结果，但其性能未能超越基于轮廓线的方法。参数化轮廓线拟合的特征工程方法在检测小提琴宽度缩减方面表现更优。

Conclusion: 对于小提琴宽度缩减的自动检测，基于参数化轮廓线拟合的特征工程方法比基于高程图的几何原始表示更有效。这表明针对特定问题的特征工程在3D形状分析中仍具有重要价值。
...
TL;DR: PlayGen-MoG：一种用于团队运动中多智能体轨迹生成的扩展框架，通过混合高斯输出头、相对空间注意力和非自回归预测解决现有方法的模式崩溃和依赖历史观测问题


<details>
  <summary>Details</summary>
Motivation: 团队运动中的多智能体轨迹生成需要既能捕捉可能战术的多样性，又能保持球员间空间协调的模型。传统的CVAE和扩散模型存在后验坍塌或收敛到数据集均值的问题，且大多数轨迹预测方法需要多帧历史观测，限制了其在仅基于初始阵型的战术设计中的应用。

Method: 1) 混合高斯输出头：所有智能体共享混合权重，单组权重选择耦合所有球员轨迹的战术场景；2) 相对空间注意力：将成对球员位置和距离编码为学习的注意力偏置；3) 非自回归预测：从初始阵型预测绝对位移，消除累积误差漂移，无需轨迹历史观测。

Result: 在美国橄榄球追踪数据上，PlayGen-MoG实现了1.68码的ADE和3.98码的FDE，同时保持所有8个混合组分的完全利用（熵为2.06/2.08），定性验证了无模式崩溃的多样化生成。

Conclusion: PlayGen-MoG通过其三个核心设计选择，成功解决了团队运动轨迹生成中的模式崩溃和依赖历史观测问题，实现了仅从静态阵型生成多样化且空间协调的现实战术，为战术设计提供了有效工具。
...
TL;DR: VERTIGO：首个用于相机轨迹生成器的视觉偏好优化框架，通过实时图形引擎渲染预览，利用电影化微调的视觉语言模型评分，结合循环语义相似度机制对齐渲染与文本提示，使用DPO进行后训练，显著改善构图质量、减少角色出镜率并提升感知真实感。
...
Method: 1. 使用Unity实时图形引擎从生成的相机运动渲染2D视觉预览；2. 采用电影化微调的视觉语言模型，通过提出的循环语义相似度机制对预览进行评分，将渲染与文本提示对齐；3. 利用这些视觉偏好信号进行直接偏好优化（DPO）后训练。

Result: 在Unity渲染和基于扩散的相机到视频流水线上，定量评估和用户研究均显示：条件遵循度、构图质量和感知真实感持续提升；角色出镜率从38%降至接近0%；用户研究参与者普遍偏好VERTIGO，在构图、一致性、提示遵循和美学质量方面均优于基线。

Conclusion: VERTIGO通过视觉偏好后训练成功解决了生成式相机系统的视觉质量问题，证明了视觉偏好优化在提升相机轨迹生成器感知质量方面的有效性，同时保持了相机运动的几何保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型(CBMs)在概念和标签空间都只在单一语义层次操作，而人类识别对象时使用不同抽象层次的通用和特定特征。需要更接近人类认知过程的分层解释框架。
...
Conclusion: HIL-CBM通过分层框架增强了概念瓶颈模型的可解释性，更接近人类认知过程，在保持无标签方法的同时提供更好的分类性能和解释质量。
...
TL;DR: 提出了一种分解优先的管道方法，将完整的临床实践指南转换为可执行的临床决策图，通过拓扑感知分块、接口约束的块图生成和来源保留的全局聚合，显著提高了决策图的准确性和完整性。
...
Conclusion: 分解优先、可审计的指南到CDS转换方法在该基准测试中表现优异，但当前证据仅限于一个已裁决的前列腺指南，需要更广泛的多指南验证。
...
TL;DR: 研究探讨了使用EarthSynth扩散模型根据现有燃烧掩码合成逼真的野火后Sentinel-2 RGB图像，无需任务特定重新训练，通过六种实验配置评估了不同生成策略的性能。
...
Method: 使用CalFireSeg-50数据集的燃烧掩码，设计了六种实验配置：比较掩码生成与上下文修复两种架构，评估三种手工提示和一种VLM生成提示策略，并测试区域颜色匹配后处理步骤。

Result: 修复型管道在所有指标上均优于全图生成，结构化修复提示获得最佳空间对齐（Burn IoU=0.456）和燃烧显著性（Darkness Contrast=20.44），颜色匹配产生最低颜色距离（ΔC_burn=63.22）但降低燃烧显著性，VLM辅助修复与手工提示相当。

Conclusion: 修复型生成方法能有效合成逼真的野火后图像，为将生成数据增强纳入野火检测管道奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管VLMs在多模态任务上表现出色，但在需要细粒度视觉感知的任务上经常失败，即使所需信息存在于其内部表示中。研究者希望探究这种性能差距的根本原因。

Method: 通过视觉对应任务（语义对应、形状对应、人脸对应）测试VLMs性能，使用Logit Lens分析机制，并探索通过教授任意名称和任务特定微调来改进性能的方法。

Result: VLMs在处理可命名实体时表现显著优于不可命名实体；Logit Lens分析确认VLMs会为可命名实体分配语义标签并生成更独特的对应标记；教授任意名称能改善性能，但任务特定微调能实现更强的泛化能力。

Conclusion: 当前VLMs在视觉任务上的失败反映了其训练中学到的捷径，而非多模态架构的根本限制。改进方向包括减少对文本映射的依赖，发展更强大的视觉感知能力。
...
TL;DR: Image Prompt Packaging (IPPg)通过在图像中嵌入结构化文本来减少文本token开销，在多个模型和任务上实现了35.8-91.0%的推理成本降低，但准确率表现高度依赖模型和任务类型。


<details>
  <summary>Details</summary>
Motivation: 大规模多模态语言模型的部署受到基于token的推理成本限制，而视觉提示策略的成本-性能关系尚未得到充分研究。需要探索能够减少文本token开销的视觉提示方法。
...
Result: IPPg实现了35.8-91.0%的推理成本降低，token压缩最高达96%。准确率表现高度模型和任务依赖：GPT-4.1在CoSQL上同时获得准确率和成本优势，而Claude 3.5在多个VQA基准上成本反而增加。渲染消融实验显示准确率变化10-30个百分点。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在噪声显著、稀疏或存在内角的区域无法实现准确的建筑线框重建，这源于无法建立自适应搜索空间来有效利用大型稀疏建筑点云的丰富3D几何信息。

Method: 提出Delaunay Canopy方法，利用Delaunay图作为几何先验定义几何自适应搜索空间。核心是Delaunay图评分，不仅重建底层几何流形，还产生区域曲率特征来鲁棒指导重建。基于此，角点和线框选择模块利用Delaunay诱导的先验聚焦高概率元素，从而塑造搜索空间。

Result: 在Building3D Tallinn城市和入门级数据集上的大量实验展示了最先进的线框重建性能，能够在多样化和复杂的建筑几何结构上实现准确预测。

Conclusion: Delaunay Canopy通过Delaunay图几何先验建立自适应搜索空间，成功解决了传统方法在噪声、稀疏和内角区域无法准确重建建筑线框的问题，实现了在复杂建筑几何上的鲁棒重建。
...
TL;DR: 提出一个可解释的视觉语言模型框架，用于腰椎管狭窄症的诊断，通过空间补丁交叉注意力模块实现精确的文本引导定位，并采用自适应PID-Tversky损失函数解决类别不平衡问题，同时生成放射学报告增强可解释性。
...
Method: 提出端到端可解释视觉语言模型框架：1) 空间补丁交叉注意力模块，实现文本引导的脊柱异常精确定位；2) 自适应PID-Tversky损失函数，整合控制理论原理动态调整训练惩罚，专门处理困难、分割不足的少数类实例；3) 集成基础视觉语言模型和自动放射学报告生成模块。

Result: 诊断分类准确率达90.69%，分割的宏观平均Dice分数为0.9512，CIDEr分数为92.80%。框架通过将复杂分割预测转换为放射科医生风格的临床报告展示可解释性。

Conclusion: 该框架为临床医学影像建立了透明、可解释AI的新基准，在保持必要人工监督的同时增强诊断能力，解决了现有模型在类别不平衡和空间精度方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 在增强和虚拟现实应用中，为新产品快速部署高精度设备端注视估计面临挑战，因为硬件配置（相机位置、姿态、照明）在不同设备代际间经常变化。现成的视觉基础模型在自然图像基准上表现出色，但在专门的近眼红外图像上仍难以达到高精度。

Method: DistillGaze采用两阶段框架：1）使用标记合成数据和无标记真实图像，通过自监督学习将视觉基础模型适配为领域专业化的教师模型；2）利用教师指导和自训练训练设备端学生模型。合成数据提供可扩展的高质量注视监督，无标记真实数据帮助弥合合成到真实的领域差距。

Result: 在包含2000多名参与者的大规模众包数据集上评估，DistillGaze相对于仅使用合成数据的基线，将中位注视误差降低了58.62%，同时保持轻量级的256K参数模型，适合实时设备端部署。

Conclusion: DistillGaze为训练和部署适应硬件变化的注视跟踪模型提供了高效途径，并为在设备端回归任务中结合合成监督和无标记真实数据提供了解决方案。
...
TL;DR: FASS是一个评估特征归因方法稳定性的新基准，通过预测不变性过滤和三种互补指标，在几何、光度、压缩扰动下系统评估归因稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因稳定性评估存在三个主要问题：1) 主要评估加性噪声，忽略现实扰动；2) 将稳定性压缩为单一标量；3) 未考虑预测不变性，混淆了归因脆弱性和模型敏感性。需要更全面的评估框架来理解归因方法在实际应用中的稳定性。

Method: 提出特征归因稳定性套件(FASS)，包含三个核心设计：1) 强制预测不变性过滤，确保比较在模型预测不变的条件下进行；2) 使用三种互补稳定性指标：结构相似性、秩相关性和top-k Jaccard重叠度；3) 评估三类现实扰动：几何变换、光度变化和压缩伪影。

Result: 评估了四种归因方法(Integrated Gradients, GradientSHAP, Grad-CAM, LIME)在四种架构和三个数据集上的表现。结果显示：1) 稳定性估计严重依赖扰动类型和预测不变性过滤；2) 几何扰动比光度变化暴露更大的归因不稳定性；3) 未进行预测不变性过滤时，高达99%的评估对涉及预测变化；4) 在控制条件下，Grad-CAM在所有数据集中表现出最高的稳定性。

Conclusion: FASS提供了一个更全面的特征归因稳定性评估框架，揭示了现有评估方法的局限性。研究强调了预测不变性过滤的重要性，并表明归因稳定性高度依赖于扰动类型。Grad-CAM在评估中表现出最稳定的性能，为安全关键视觉系统中的归因方法选择提供了指导。
...
TL;DR: 该论文系统研究了医疗视觉语言模型（VLMs）的置信度校准问题，发现模型普遍存在过度自信现象，提出使用幻觉感知校准（HAC）方法结合幻觉检测信号来改善校准效果和预测质量。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在临床决策支持中的部署增加，仅关注准确性已不足够，了解何时信任模型预测同样关键。然而，在医疗领域对模型过度自信问题的全面系统研究仍然稀缺。

Method: 通过全面的实证研究，涵盖三个模型家族（Qwen3-VL、InternVL3、LLaVA-NeXT）、三个模型规模（2B-38B）和多种置信度估计提示策略，在三个医疗视觉问答基准上进行评估。研究采用简单后处理校准方法（如Platt scaling）和幻觉感知校准（HAC），后者将视觉基础幻觉检测信号作为补充输入来优化置信度估计。

Result: 研究发现：1）过度自信在不同模型家族中普遍存在，无法通过模型缩放或提示策略解决；2）简单后处理校准方法能减少校准误差并优于提示策略；3）后处理校准方法由于单调性限制，无法改善预测的判别质量（AUROC保持不变）；4）幻觉感知校准（HAC）能同时改善校准效果和AUROC，在开放式问题上提升最大。
...
TL;DR: UniScene3D是一个基于transformer的3D编码器，通过多视角彩色点云图学习统一的场景表示，联合建模图像外观和几何信息，在多种3D场景理解任务上实现SOTA性能。
...
Method: 提出基于transformer的UniScene3D编码器，从多视角彩色点云图学习统一表示；引入跨视角几何对齐和基于grounding的视角对齐来增强跨视角几何和语义一致性。

Result: 在视角grounding、场景检索、场景类型分类和3D视觉问答等任务的低样本和任务特定微调评估中均实现了最先进的性能。

Conclusion: 该方法在统一3D场景理解方面表现出有效性，证明了联合建模外观和几何信息以及跨视角对齐策略的优势。
...
Method: 引入新的计算模式，在更细粒度上应用SVD；根据权重元素重要性自适应分配权重；扩展框架结合权重和激活的量化

Result: 加权SVD在保持精度的同时实现超过1.8倍的解码加速，优于其他方法

Conclusion: 加权SVD通过细粒度分解和自适应权重分配，有效解决了现有SVD方法在实际延迟减少方面的局限性，为高效视觉语言模型提供了新方案
...
<details>
  <summary>Details</summary>
Motivation: 现有图像-3D表示学习方法主要关注单个物体图像与其3D模型的特征对齐，但在现实场景中物体通常从多个视角被观察和捕获。多视角观测自然提供互补的几何和外观线索，但现有多模态大模型很少探索如何有效融合多视角视觉信息以提升跨模态检索性能。

Method: 1) 基于跨注意力的多视角视觉聚合器：自适应整合物体多视角图像特征，融合视角间互补关系并选择性强调跨多个视角的信息视觉线索；2) 法向量感知3D模型编码器：联合编码点法向量和3D位置，增强3D几何特征，特别适用于纹理缺失或颜色退化的3D模型。

Result: 在图像-3D检索实验中，FusionBERT在单视角和多视角设置下均显著优于现有最先进的多模态大模型，检索准确率显著提高，为多视角多模态检索建立了强基线。

Conclusion: FusionBERT通过创新的多视角视觉融合和法向量感知3D编码，有效解决了现实场景中多视角图像-3D检索的挑战，在跨模态检索任务中表现出优越性能。
...
TL;DR: TrackerSplat通过整合先进的点跟踪方法，增强3D高斯泼溅在动态场景重建中的鲁棒性和可扩展性，有效处理大帧间位移并减少伪影。
...
Method: 利用现成的点跟踪模型提取像素轨迹，将每视图像素轨迹三角化到3D高斯上，在训练前指导高斯的位置重定位、旋转和缩放，从而处理大位移。

Result: 在具有显著位移的真实世界数据集上验证了鲁棒性，在并行设置下实现了优越的吞吐量，同时保持了视觉质量，显著减少了伪影。

Conclusion: TrackerSplat通过整合点跟踪方法有效解决了动态场景重建中的大位移问题，提高了重建吞吐量并保持了渲染质量，为3DGS在动态场景中的应用提供了更鲁棒的解决方案。
...
TL;DR: Moondream Segmentation是基于Moondream 3的指称图像分割扩展，通过自回归解码向量路径并迭代细化掩码，引入强化学习阶段直接优化掩码质量，并发布RefCOCO-M清洗数据集。


<details>
  <summary>Details</summary>
Motivation: 解决指称图像分割任务中监督信号模糊性问题，以及多边形标注带来的评估噪声问题，提升分割掩码的边界精度和质量。

Method: 1. 基于Moondream 3构建指称图像分割模型；2. 自回归解码向量路径并迭代细化栅格化掩码；3. 引入强化学习阶段直接优化掩码质量；4. 使用强化学习阶段生成的粗到真值目标训练细化器；5. 发布RefCOCO-M清洗数据集以解决标注噪声问题。

Result: 在RefCOCO验证集上达到80.2%的cIoU，在LVIS验证集上达到62.6%的mIoU，并发布了边界精确的RefCOCO-M清洗验证集。

Conclusion: Moondream Segmentation通过结合自回归解码、迭代细化和强化学习，有效提升了指称图像分割的性能和掩码质量，同时通过清洗数据集解决了评估噪声问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统光学传感器（相机、激光雷达）在烟雾、雾霾、非理想光照等恶劣条件下失效，而专用雷达系统依赖定制硬件和授权频谱，成本高且可扩展性差，需要一种低成本、可扩展且鲁棒的3D感知方案

Method: 提出Rascene框架，利用毫米波OFDM通信信号进行集成感知与通信；通过多帧、空间自适应融合与置信度加权前向投影，从稀疏且多径模糊的单个无线电帧中恢复跨任意姿态的几何一致性

Result: 实验结果表明，该方法能够高精度重建3D场景，为低成本、可扩展且鲁棒的3D感知提供了新途径

Conclusion: Rascene框架通过利用无处不在的通信信号实现3D场景成像，为解决恶劣环境下的感知问题提供了实用且经济高效的解决方案，具有重要的应用前景
...
Method: 提出联邦学习框架用于基于姿态的儿童自闭症行为识别，采用两层隐私保护机制：1) 使用人体骨骼抽象从原始RGB视频中移除可识别的视觉信息；2) 通过联邦学习确保敏感姿态数据保留在诊所内部。该方法利用分布式临床数据学习泛化表示，同时提供站点特定个性化的灵活性。

Result: 在MMASD基准测试上的实验结果表明，该框架实现了高识别准确率，优于传统的联邦基线方法，为多站点临床分析提供了稳健、隐私优先的解决方案。

Conclusion: 该研究首次将联邦学习应用于基于姿态的儿童自闭症行为识别，通过创新的隐私保护机制成功解决了临床数据隐私和稀缺问题，为多站点协作的临床分析提供了可行方案，具有重要的临床实践意义。
...
Method: 提出ArticuSurDepth框架，包含：1）多视图空间上下文增强策略和跨视图表面法线约束，提升空间和时间上下文的结构一致性；2）带地面平面感知的相机高度正则化，鼓励度量深度估计；3）跨车辆姿态一致性，桥接铰接段之间的运动估计。

Result: 在自采集数据集以及DDAD、nuScenes和KITTI基准测试中均实现了最先进的深度估计性能。

Conclusion: 该工作为铰接式车辆的环视深度估计提供了有效的自监督解决方案，通过几何一致性约束和结构先验显著提升了深度估计的准确性和鲁棒性。
...
TL;DR: DTPTrack是一个轻量级通用模块，通过时间可靠性校准器和时间引导合成器抑制多帧跟踪中的模型漂移，在多个跟踪架构上实现一致性能提升，在LaSOT和GOT-10k上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有多帧跟踪器容易受到模型漂移的影响，因为它们在聚合历史预测时没有有效处理噪声信息。需要一种能够抑制漂移、同时保持轻量化和通用性的解决方案。

Method: 提出DTPTrack模块，包含两个核心组件：1) 时间可靠性校准器(TRC)，学习为历史状态分配每帧可靠性分数，过滤噪声并锚定在真实模板上；2) 时间引导合成器(TGS)，将校准后的历史信息合成为紧凑的动态时间先验，提供预测性引导。

Result: 将DTPTrack集成到OSTrack、ODTrack和LoRAT三种不同跟踪架构中，在所有基线模型上都获得了显著一致的性能提升。基于扩展LoRATv2的最佳模型在多个基准测试中达到新的SOTA：LaSOT上成功率为77.5%，GOT-10k上AO为80.3%。

Conclusion: DTPTrack是一个有效抑制模型漂移的轻量级通用模块，通过时间可靠性校准和引导合成机制，能够显著提升多种跟踪架构的性能，在多个基准测试中达到最先进水平。
...
TL;DR: Efficient3D是一个用于加速3D多模态大语言模型的视觉令牌剪枝框架，通过去偏视觉令牌重要性估计器和自适应令牌重平衡策略，在保持精度的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 3D多模态大语言模型（MLLMs）虽然具有强大的3D空间理解能力，但其庞大的模型规模和高维输入特征导致推理开销巨大，限制了在资源受限平台上的实际部署。

Method: 提出统一视觉令牌剪枝框架Efficient3D，包含两个核心组件：1) 去偏视觉令牌重要性估计器（DVTIE），考虑浅层初始层在注意力聚合中的影响，产生更可靠的重要性预测；2) 自适应令牌重平衡（ATR）策略，根据场景复杂度动态调整剪枝强度，保持语义完整性和跨层注意力平衡。

Result: 在五个代表性3D视觉语言基准测试（ScanRefer、Multi3DRefer、Scan2Cap、ScanQA、SQA3D）上验证，Efficient3D相比未剪枝基线表现更优，在Scan2Cap数据集上CIDEr指标提升+2.57%，实现了精度提升的同时降低计算开销。

Conclusion: Efficient3D为3D MLLMs提供了一种可扩展且高效推理的解决方案，通过上下文感知的令牌减少机制，在保持语义完整性的同时显著加速模型推理。
...
TL;DR: 在文档布局分析中引入轻量级结构精炼阶段，通过集合级推理稳定解析器接口，解决密集页面中布局假设不稳定导致的解析错误问题


<details>
  <summary>Details</summary>
Motivation: 在显式文档布局分析流水线中，下游解析器不直接使用完整的检测器输出，而是基于保留和序列化的布局实例集。然而，在密集页面中，重叠区域和模糊边界会导致不稳定的布局假设，使得保留的实例集与其解析器输入顺序不一致，从而引发严重的下游解析错误。

Method: 在DETR风格检测器和解析器之间引入轻量级结构精炼阶段。将原始检测器输出视为紧凑假设池，该模块在查询特征、语义线索、边界框几何和视觉证据上进行集合级推理。从共享的精炼结构状态中，联合确定实例保留、精炼边界框定位，并在交接前预测解析器输入顺序。进一步引入面向保留的监督和难度感知排序目标，以更好地对齐保留实例集及其顺序与最终解析器输入。

Result: 在公开基准测试上的广泛实验表明，该方法持续提升页面级布局质量。当集成到标准端到端解析流水线中时，稳定的解析器接口显著减少序列不匹配，在OmniDocBench上实现了0.024的阅读顺序编辑距离。

Conclusion: 提出的结构精炼阶段有效解决了文档布局分析中解析器接口不稳定的问题，通过集合级推理和专门的监督目标，显著改善了密集复杂页面的布局质量和解析准确性。
...
TL;DR: XrayClaw：通过合作-竞争多智能体架构实现胸部X光片诊断的可靠AI框架，采用竞争偏好优化提升诊断准确性和逻辑一致性


<details>
  <summary>Details</summary>
Motivation: 传统单体模型在胸部X光片诊断中存在逻辑不一致和诊断幻觉问题，现有多智能体系统易受单一底层模型导致的共识错误影响，需要更可靠的诊断框架

Method: 提出XrayClaw框架，包含四个专业合作智能体模拟系统化临床工作流，一个竞争智能体作为独立审计者，采用竞争偏好优化学习目标强制分析和整体解释间的相互验证

Result: 在MS-CXR-T、MIMIC-CXR和CheXbench基准测试中实现最先进的诊断准确性、临床推理保真度和零样本领域泛化能力，有效减轻累积幻觉

Conclusion: XrayClaw通过合作-竞争架构和竞争偏好优化显著提升了自动化胸部X光片诊断的可靠性，为可信赖的医学影像分析建立了新范式
...
TL;DR: 提出VBGS-SLAM框架，将3D高斯溅射与变分贝叶斯推理结合，通过显式维护位姿和场景参数的后验不确定性，提升SLAM的鲁棒性和抗漂移能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯溅射的SLAM方法通常采用直接确定性位姿优化，对初始化敏感且容易因地图演化而发生灾难性遗忘。需要一种更鲁棒、能处理不确定性的方法。

Method: 提出变分贝叶斯高斯溅射SLAM框架，利用多元高斯的共轭性质和变分推理，将溅射地图优化和相机位姿跟踪耦合在生成概率形式中，实现高效的闭式更新。

Result: 在长序列预测中表现出优越的跟踪性能和鲁棒性，同时在合成和真实场景中实现高效、高质量的新视角合成，有效缓解漂移问题。

Conclusion: VBGS-SLAM通过概率框架显式建模不确定性，在保持3DGS效率和渲染质量的同时，显著提升了SLAM系统的鲁棒性和抗漂移能力。
...
TL;DR: 提出了一种基于世界建模的端到端自动驾驶VLA框架，通过未来RGB和深度图像生成实现密集监督，并利用世界模型的不确定性作为内在奖励进行策略探索，在NAVSIM和nuScenes基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于行为克隆的VLA自动驾驶模型只能复制专家行为，缺乏对新颖场景的探索能力。强化学习虽然能提供探索机制，但VLA模型缺乏直接可观测的状态转移，需要世界模型来预测动作后果。
...
Result: 在NAVSIM基准上达到93.7的PDMS分数和88.8的EPDMS分数，在nuScenes基准上也表现出色，均达到最先进水平。

Conclusion: 通过世界建模实现了VLA自动驾驶模型的有效探索和密集监督，显著提升了模型在分布外场景下的鲁棒性和性能，为端到端自动驾驶提供了新的解决方案。
...
TL;DR: MOMO是首个火星遥感多传感器基础模型，通过模型融合整合了HiRISE、CTX和THEMIS三种火星传感器的表示学习，采用新颖的等验证损失策略进行最优检查点选择，在9个下游任务上超越现有基线。
...
Method: 采用模型融合方法，独立训练三个火星传感器（HiRISE、CTX、THEMIS）的表示学习模型，通过新颖的等验证损失策略基于验证损失相似性对齐检查点，然后通过任务算术进行融合，确保在兼容的收敛阶段合并模型。

Result: 在约1200万个火星轨道数据样本上训练MOMO，在Mars-Bench的9个下游任务上评估，相比ImageNet预训练、地球观测基础模型、传感器特定预训练和全监督基线，MOMO实现了更好的整体性能，特别是在分割任务上表现出持续显著的性能提升。
...
TL;DR: THOM是一个无需训练的两阶段框架，从文本生成具有高视觉保真度和物理合理性的3D手-物体交互网格，通过高斯到网格映射和物理优化解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 从文本生成3D手-物体交互对于灵巧机器人抓取和VR/AR内容生成至关重要，但现有方法面临两个主要挑战：1）从文本生成的高斯模型中提取网格的ill-posed问题；2）在错误网格上进行基于物理优化的困难。需要一种无需模板物体网格就能生成逼真且物理合理的3D HOI网格的方法。

Method: THOM采用两阶段流程：1）首先生成手和物体的高斯表示；2）进行基于物理的HOI优化。关键创新包括：新的网格提取方法和顶点到高斯映射，将高斯元素显式分配给网格顶点，实现拓扑感知正则化；VLM引导的平移细化和接触感知优化，提升交互的物理合理性。

Result: 综合实验表明，THOM在文本对齐、视觉真实感和交互合理性方面持续超越最先进方法，无需训练即可生成逼真且物理合理的3D HOI网格。

Conclusion: THOM成功解决了从文本生成3D手-物体交互的挑战，通过创新的网格提取和物理优化方法，实现了无需模板的高质量HOI生成，为机器人抓取和VR/AR应用提供了有效解决方案。
...
TL;DR: LLaBIT：一种用于脑MRI多任务处理的大型语言模型，扩展了LLMs的视觉推理能力，在报告生成、视觉问答、图像分割和图像翻译等临床相关任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在视觉语言任务中表现出色，但简单的文本到图像生成在医学影像中临床价值有限。医学影像中真正重要的任务如病理定位的图像分割和序列重建的图像翻译等，尚未在单一通用语言模型中实现整合

Method: 提出LLaBIT方法，通过重用图像编码器的特征图来缓解图像标记化过程中的空间信息损失，并使用LLMs生成严格遵循预定义指令的文本数据来增强有限的脑MRI图像-文本配对数据

Result: 在五个脑MRI数据集上评估了四个不同任务：报告生成、视觉问答、图像分割和图像翻译。模型在所有任务中均表现出优异性能，甚至在直接比较中超越了专门的任务特定模型

Conclusion: LLaBIT成功将LLMs的视觉推理能力扩展到脑MRI领域具有临床意义的任务中，证明了其在医学影像多任务处理中的有效性和多功能性
...
TL;DR: 提出一种双表示方法，将离散折线与连续贝塞尔控制点耦合，通过双向映射机制实现协同优化，在笔画渲染中减少笔画数量30-50%，提升结构连贯性并缩短优化时间30-40%。


<details>
  <summary>Details</summary>
Motivation: 在基于笔画的渲染中，搜索方法常因离散笔画放置陷入局部最优，而可微分优化器缺乏结构意识且产生非结构化布局。需要弥合这一差距，实现更好的笔画优化。

Method: 提出双表示方法，通过双向映射机制耦合离散折线与连续贝塞尔控制点。支持局部梯度细化全局笔画结构，内容感知笔画提议帮助逃离局部最优。采用高斯泼溅启发的初始化，实现图像上的高度并行笔画优化。

Result: 实验显示该方法减少笔画数量30-50%，实现更结构连贯的布局，提升重建质量，相比现有可微分矢量化方法缩短优化时间30-40%。

Conclusion: 提出的双表示方法有效解决了笔画渲染中的局部最优问题，通过离散-连续协同优化实现了更高效、结构更优的笔画布局，在质量和效率上均有显著提升。
...
TL;DR: DeCo-DETR：一种视觉中心的开放词汇目标检测框架，通过解耦范式实现高效推理和开放世界泛化，无需在线文本编码


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法存在两个主要问题：1）多模态设计依赖推理时的文本编码器，计算开销大；2）紧密耦合的训练目标在闭集检测精度和开放世界泛化之间存在权衡。需要一种更实用、可扩展的解决方案。
...
Conclusion: DeCo-DETR通过解耦范式解决了OVOD的效率和泛化权衡问题，为可扩展的开放词汇目标检测系统提供了实用方向。视觉中心的设计避免了在线文本编码的计算开销，同时保持了开放世界识别能力。
...
<details>
  <summary>Details</summary>
Motivation: 从变形的3D服装几何恢复参数化2D缝纫图案是一个具有挑战性的病态问题，现有方法难以解决这一逆向过程

Method: 两阶段框架：第一阶段使用几何驱动的自回归模型从输入3D服装推断BoxMesh；第二阶段使用语义感知的自回归模型将BoxMesh解析为参数化缝纫图案

Result: 在GarmentCodeData基准测试中达到最先进性能，并能有效泛化到真实世界扫描和单视图图像

Conclusion: 通过BoxMesh中间表示将几何反演与结构化图案推断分离，显著减少了模糊性，实现了更准确和鲁棒的缝纫图案恢复
...
TL;DR: 该论文提出了TinySet-9M大规模小目标检测数据集，建立了小目标检测基准，并开发了P2SOD范式和DEAL框架，通过点提示实现语义增强，显著提升了小目标检测性能。
...
Result: DEAL在TinySet-9M上仅需单次点击，在严格定位指标(如AP75)上相比全监督基线获得31.4%的相对提升，并能有效泛化到未见类别和数据集。

Conclusion: 该工作填补了小目标检测大规模数据集的空白，提出的P2SOD范式和DEAL框架通过点提示实现语义增强，显著提升了小目标检测性能，为标签高效的小目标检测提供了新方向。
...
TL;DR: 首次系统研究视觉模型成员推理攻击的对抗鲁棒性，揭示对抗性成员操纵现象及其检测防御方法
...
Method: 1) 证明对抗性成员伪造在不同架构和数据集上一致有效；2) 揭示特征几何签名——梯度范数塌缩轨迹，可区分伪造与真实成员；3) 基于梯度几何信号提出检测策略，开发鲁棒推理框架

Result: 实验表明伪造攻击广泛有效，而提出的检测和鲁棒推理策略显著增强防御能力。建立了首个视觉模型对抗性成员操纵的全面框架

Conclusion: 本文首次系统分析了视觉模型成员推理攻击的对抗鲁棒性，揭示了对抗性成员操纵现象，提出了基于梯度几何特征的检测方法和鲁棒推理框架，为隐私保护提供了新视角
...
TL;DR: 提出EnsemHalDet框架，通过集成多个VLM内部表征（注意力输出和隐藏状态）来检测多模态幻觉，相比单检测器方法显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态任务中表现出色，但仍容易产生事实错误或缺乏图像依据的幻觉。现有基于内部表征的检测方法通常依赖单一表征或检测器，限制了捕捉多样化幻觉信号的能力。

Method: 提出EnsemHalDet集成检测框架，利用VLM的多个内部表征（包括注意力输出和隐藏状态），为每个表征训练独立检测器，并通过集成学习组合这些检测器。
...
TL;DR: CANDLE利用DINOv3的自监督特征作为光照鲁棒的语义先验，通过DINO全层引导和颜色频率细化设计，在多色照明下实现颜色环境光照归一化，在CL3AN数据集上获得+1.22 dB PSNR提升。


<details>
  <summary>Details</summary>
Motivation: 多色照明下的颜色环境光照归一化面临严重色偏、高光饱和和材料相关反射等挑战。现有几何和低层先验在光照引起的色偏占主导时不足以恢复物体固有颜色。作者观察到DINOv3的自监督特征在彩色光输入和环境光真实值之间保持高度一致性，这启发了将其用作光照鲁棒的语义先验。

Method: 提出CANDLE方法：1）DINO全层引导（D.O.G.）自适应地将多层DINOv3特征注入到连续的编码器阶段；2）颜色频率细化设计（BFACG + SFFB）抑制解码器侧的色度崩溃和细节污染。

Result: 在CL3AN数据集上获得+1.22 dB PSNR增益，优于现有最强方法。在NTIRE 2026 ALN Color Lighting Challenge中获得第3名，在White Lighting track的保真度方面获得第2名且具有最低的FID，证明了在色度和亮度主导的照明条件下都具有强泛化能力。

Conclusion: CANDLE通过利用DINOv3的自监督特征作为光照鲁棒的语义先验，在多色照明下的颜色环境光照归一化任务中取得了显著性能提升，展示了在复杂照明条件下的强泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着HDR设备快速普及，需要将8位SDR内容转换为10位HDR。现有逆色调映射方法依赖固定色调映射算子，难以泛化到真实世界退化、风格变化和相机流水线，常产生高光裁剪、色彩去饱和或不稳定的色调再现问题。

Method: 基于预训练扩散Transformer构建LumaFlux，包含三个核心组件：1) 物理引导适应模块，通过低秩残差将亮度、空间描述符和频率线索注入注意力机制；2) 感知交叉调制层，通过FiLM条件化从视觉编码器特征稳定色度和纹理；3) HDR残差耦合器，在时间步和层自适应调制计划下融合物理和感知信号。最后使用轻量级有理二次样条解码器重建平滑可解释的色调场。

Result: 在多个基准测试中，LumaFlux优于现有最先进基线，实现了优异的亮度重建和感知色彩保真度，同时参数增加极少。建立了首个大规模SDR-HDR训练语料库和新的评估基准。

Conclusion: LumaFlux通过物理和感知引导的扩散Transformer架构，成功解决了SDR到HDR转换中的关键挑战，在保持参数效率的同时实现了卓越的HDR重建质量，为高质量HDR内容生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统创建3D虚拟人偶的方法需要冗长复杂的流程，包括外观建模、运动规划、绑定和物理模拟。UNICA旨在将这些组件统一到单个神经框架中，简化工作流程并提高效率。

Method: UNICA采用骨架无关的生成模型，基于类似视频游戏控制的键盘输入，通过动作条件扩散模型在2D位置图上生成下一帧3D几何。然后使用点变换器将几何映射到3D高斯泼溅进行高保真自由视角渲染。

Result: 该方法能够自然捕捉头发和宽松衣物的动态效果，无需手动设计的物理模拟，并支持超长自回归生成。据作者所知，UNICA是首个统一"运动规划、绑定、物理模拟和渲染"工作流的模型。

Conclusion: UNICA通过统一的神经框架简化了3D虚拟人偶的创建流程，实现了从键盘输入到高质量渲染的端到端生成，为游戏、元宇宙和AR/VR应用提供了更高效可控的解决方案。
...
TL;DR: 提出一种量化感知的视觉token剪枝框架，通过结合量化误差和语义相关性来协同优化MLLM的PTQ和token剪枝，在低比特量化下保持精度。


<details>
  <summary>Details</summary>
Motivation: MLLMs计算和内存成本高，难以在资源受限环境中部署。现有的PTQ和视觉token剪枝通常作为独立优化，但研究发现两者存在强耦合：在PTQ优化的MLLMs上直接应用语义token剪枝会丢弃对数值稳定性重要的激活异常值，从而在低比特量化（如W4A4）中加剧量化误差。
...
Conclusion: PTQ和视觉token剪枝之间存在强耦合关系，需要协同优化。提出的量化感知token剪枝框架通过结合量化误差和语义相关性，在低比特量化下实现了更好的精度-效率权衡，为资源受限环境中的MLLM部署提供了有效解决方案。
...
TL;DR: MMPhysVideo：首个通过联合多模态建模扩展视频生成物理合理性的框架，将语义、几何和时空轨迹等感知线索统一为伪RGB格式，使视频扩散模型能直接捕获复杂物理动态。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型虽然能生成视觉效果出色的内容，但由于仅依赖像素级重建，往往产生物理上不一致的结果。需要一种方法来提升视频生成的物理合理性。

Method: 1. 将感知线索（语义、几何、时空轨迹）转换为统一伪RGB格式；2. 提出双向控制教师架构，通过并行分支完全解耦RGB和感知处理，使用两个零初始化控制链接逐步学习像素级一致性；3. 通过表示对齐将教师物理先验蒸馏到单流学生模型；4. 开发MMPhysPipe数据管道，利用视觉语言模型和视觉证据链规则识别物理主体，提取多粒度感知信息。

Result: 在不增加推理成本的情况下，MMPhysVideo在各种基准测试中持续提升物理合理性和视觉质量，相比先进模型实现最先进性能。

Conclusion: MMPhysVideo通过多模态物理建模有效解决了视频扩散模型的物理不一致问题，为物理合理的视频生成提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当直接获取3D数据成本高昂或不切实际时，从单张图像创建灵活的3D场景至关重要。现有方法在实现大视角变化下的可控多视角合成和时空一致性方面存在挑战。

Method: 1) 利用视频扩散模型捕获丰富的3D先验；2) 采用几何感知扩展策略逐步扩大场景覆盖范围；3) 引入多阶段相机控制机制，通过双分支相机注入和注意力调制实现可控多视角合成；4) 提出碰撞感知相机轨迹规划器；5) 开发增强的3D高斯泼溅(3DGS)流水线，包含深度对齐监督、结构正则化和细化。

Result: 大量实验表明，NavCrafter在大视角变化下实现了最先进的新视角合成性能，并显著提高了3D重建的保真度。

Conclusion: NavCrafter通过结合视频扩散模型、几何感知扩展和增强的3DGS流水线，成功实现了从单张图像进行可控、时空一致的3D场景探索和高质量重建。
...
<details>
  <summary>Details</summary>
Motivation: 神经辐射场在3D表示中很重要，但计算资源需求大限制了应用。设备端训练可以解决通信限制、隐私问题和快速适应变化场景的需求，但有限的GPU内存、存储和功耗等资源限制了部署

Method: Fact-Hash结合张量分解和哈希编码技术，将3D坐标投影到多个低维形式（2D或1D），然后应用哈希函数，最后聚合成单个特征。这种方法既利用了丰富的高分辨率特征，又具有少样本鲁棒性

Result: 相比最先进方法，Fact-Hash在保持质量和渲染速度的同时，内存效率显著提升。相比之前的编码方法，Fact-Hash节省了超过三分之一的内存使用，同时保持PSNR值。设备端实验验证了其在计算效率和能耗方面的优势

Conclusion: Fact-Hash是改进特征网格表示、解决内存限制并在各种应用中提高质量的有前景的解决方案，为设备端神经辐射场训练提供了高效的内存编码方法
...
<details>
  <summary>Details</summary>
Motivation: 现有基于掩码点建模（MPM）的点云上下文学习方法存在两个主要问题：1）缺乏几何先验，模型仅通过transformer的token级相关性推断空间结构和几何细节；2）训练-推理目标不匹配，训练时使用推理时不可得的目标侧信息。

Method: 提出DeformPIC框架，采用基于形变的方法而非掩码重建。该方法学习在提示的任务特定指导下形变查询点云，实现显式几何推理和一致的目标函数。

Result: 在重建、去噪和配准任务上，DeformPIC比先前最优方法分别降低平均Chamfer距离1.6、1.8和4.7个点。在新引入的域外基准测试中也达到最优性能。

Conclusion: DeformPIC通过形变框架解决了MPM方法的局限性，实现了更好的几何推理和训练-推理一致性，在多个点云任务上取得了显著性能提升，并展现出优秀的泛化能力。
...
TL;DR: 提出自适应局部频率滤波方法，通过空间变化的α(x)参数调制傅里叶编码分量，改善传统固定频率傅里叶特征映射在处理空间变化局部频谱信号时的不足。


<details>
  <summary>Details</summary>
Motivation: 传统傅里叶特征映射在整个空间域使用固定频率集，不适合具有空间变化局部频谱的信号，导致高频细节收敛缓慢。需要一种能够适应信号局部频谱特性的自适应方法。

Method: 提出自适应局部频率滤波方法，引入空间变化参数α(x)来调制编码的傅里叶分量，实现不同空间位置在低通、带通和高通行为之间的平滑过渡。从神经正切核(NTK)角度分析滤波器效果，提供NTK启发的解释说明如何重塑有效核谱。

Result: 在2D图像拟合、3D形状表示和稀疏数据重建实验中，该方法相比固定频率基线一致提高了重建质量并加快了优化速度。学习到的α(x)提供了空间变化频率偏好的直观可视化，有助于解释模型在非平稳信号上的行为。

Conclusion: 自适应局部频率调制是傅里叶编码隐式神经表示的实际增强方法，能够更好地处理具有空间变化频谱特性的信号，提高重建质量和优化效率。
...
Method: 1) 利用视频生成模型的强3D先验将单视角重建转化为校准多视角重建任务；2) 引入基于稀疏真实图像标注训练的神经方向提取器进行全视角方向估计；3) 设计基于混合隐式场的两阶段发丝生长算法快速合成细节丰富的3D发丝曲线

Result: 在多样化头发肖像的可见和不可见区域均实现了最先进的单视角3D发丝重建性能

Conclusion: 通过利用视频生成模型的3D先验、改进的方向估计和高效的发丝生长算法，成功解决了单视角头发重建中的关键挑战，在质量和效率方面取得显著提升
...
TL;DR: 该论文提出使用token级扭曲而非像素级扭曲来增强多模态大语言模型对场景视角变化的鲁棒性，通过反向token扭曲方法在ViewBench基准上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉推理方面表现良好，但对视角变化非常脆弱。像素级扭曲对深度误差高度敏感，容易引入几何畸变。受人类心理意象理论启发，该研究探索图像token是否可以作为视角变换的有效表示基础。
...
Conclusion: token级扭曲是增强MLLMs视角变化鲁棒性的有效方法，后向token扭曲策略在保持语义一致性和稳定性方面优于传统像素级扭曲，为MLLMs的几何理解能力提供了新思路。
...
TL;DR: 提出自反思层次提示框架，通过构建层次提示解析树和自反思精炼机制，实现类增量分割中的正向和反向知识迁移，提升手术器械分割性能并避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有手术视频场景解析方法在增量学习新器械时，忽视了正向知识迁移（历史知识帮助学习新类）和反向知识迁移（学习新类帮助精炼历史知识）的潜力。需要一种能同时利用这两种知识迁移、避免灾难性遗忘的增量分割框架。

Method: 基于冻结预训练模型构建自反思层次提示框架：1）使用层次提示解析树组织器械感知提示，根节点为器械共享提示分区，中间节点为n部分共享提示分区，叶节点为器械独特提示分区，实现正向知识迁移；2）通过有向加权图传播进行自反思精炼，利用树中记录的知识关联提高现有知识的代表性，实现反向知识迁移。框架适用于CNN和Transformer基础模型。

Result: 在两个公开基准测试中，相比竞争方法分别获得超过5%和11%的性能提升，证明了框架在类增量分割中的有效性。

Conclusion: 提出的自反思层次提示框架成功解锁了类增量分割中的正向和反向知识迁移潜力，能够熟练学习新器械、改进常规器械的现有技能，同时避免旧器械的灾难性遗忘，为手术视频场景解析提供了有效的增量学习解决方案。
...
TL;DR: InstructTable是一个基于指令引导的多阶段训练框架，通过表格指令预训练和TSR微调解决复杂表格结构识别问题，并提出了Table Mix Expand方法合成大规模真实表格数据。


<details>
  <summary>Details</summary>
Motivation: 传统视觉模型在复杂表格布局（如合并单元格、空单元格）识别中缺乏语义支持，而视觉语言模型又忽视视觉结构信息建模，导致复杂表格结构识别准确率受限。

Method: 提出InstructTable框架：1）表格指令预训练关注细粒度结构模式；2）TSR微调保持强大的视觉信息建模能力；3）提出Table Mix Expand无模板方法合成大规模真实表格数据；4）构建BCDSTab基准数据集。

Result: 在多个公开数据集（FinTabNet、PubTabNet、MUSTARD）和自建的BCDSTab基准上，InstructTable在表格结构识别任务中达到了最先进的性能。消融研究证实了表格特定指令和合成数据的积极作用。

Conclusion: InstructTable通过指令引导的多阶段训练框架有效解决了复杂表格结构识别问题，结合表格特定指令预训练和视觉信息建模，在多种场景下实现了高精度的表格解析。
...
TL;DR: 提出了一种基于条件引导的编辑重建框架，通过约束反演在结构化虚拟人潜在空间中进行编辑，限制更新到低维、部件特定的编辑子空间，防止身份泄漏和姿态依赖的时间闪烁。


<details>
  <summary>Details</summary>
Motivation: 传统可动画人体虚拟人编辑方法通常依赖稀疏监督（少量编辑关键帧），但简单地将重建的虚拟人拟合到这些编辑会导致身份泄漏和姿态依赖的时间闪烁问题。作者认为这些失败源于病态反演：可用的编辑约束不足以确定负责预期编辑的潜在方向。

Method: 提出条件引导的编辑重建框架，将编辑作为结构化虚拟人潜在空间中的约束反演执行。限制更新到低维、部件特定的编辑子空间以防止意外身份变化。关键创新在于通过优化从完整解码和渲染流程的局部线性化导出的条件目标来设计反演期间的编辑约束，产生编辑子空间信息矩阵，其频谱预测稳定性并驱动帧重新加权/关键帧激活。

Result: 该方法在小型子空间矩阵上操作，可以通过Hessian-向量积高效实现，并在有限编辑监督下提高了稳定性。

Conclusion: 通过将编辑视为结构化潜在空间中的约束反演问题，并利用局部线性化导出的条件目标来指导编辑过程，可以有效解决可动画人体虚拟人编辑中的身份泄漏和时间闪烁问题，实现更稳定和精确的编辑效果。
...
TL;DR: 该论文将几何教育中的视觉解释构建为指代图像分割问题，通过自动生成合成数据集和领域特定微调，显著提升了模型在几何图表上的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有指代图像分割模型在自然图像基准上训练，但在抽象、无纹理的几何图表上表现极差，存在严重的领域偏移问题。几何教育需要能够提供视觉解释的人工智能系统，但缺乏合适的训练数据。

Method: 1) 开发完全自动化的程序化数据引擎，生成20多万个合成几何图表，包含像素级分割掩码和语言多样的指代表达式，无需人工标注；2) 对视觉语言模型进行领域特定微调；3) 提出Buffered IoU这一几何感知的评估指标，考虑细结构定位问题。

Result: 微调后的Florence-2模型在几何图表分割上达到49% IoU和85% Buffered IoU，相比零样本设置的<1% IoU有显著提升。Buffered IoU比标准IoU更能反映真实分割质量，特别是在细结构定位方面。

Conclusion: 该研究为构建能够提供视觉化、逐步解释几何问题的人工通用教师系统奠定了基础，通过合成数据生成和领域适应方法解决了几何图表分割的挑战。
...
Method: 提出UniSpector框架：1) Spatial-Spectral Prompt Encoder提取方向不变、细粒度表示；2) Contrastive Prompt Encoder将提示空间正则化为语义组织的角度流形；3) Prompt-guided Query Selection生成与提示对齐的自适应对象查询。
...
<details>
  <summary>Details</summary>
Motivation: 现有4DGS方法专注于确定性重建，但无法捕捉运动模糊性，也缺乏评估预测可靠性的机制。需要一种能够量化不确定性、处理稀疏观测区域并支持时间外推的概率建模方法。

Method: 将高斯过程集成到4D高斯泼溅框架中，设计时空核函数捕捉变形场的相关结构，采用带有诱导点的变分高斯过程实现可处理的推理，以处理4DGS中大量高斯基元。

Result: GP-4DGS在提高重建质量的同时，提供了可靠的不确定性估计，能够有效识别高运动模糊区域。实验验证了该方法在不确定性量化、运动估计和时间外推方面的有效性。
...
TL;DR: BEVPredFormer：一种基于注意力机制的相机BEV实例预测架构，通过门控Transformer层、分时-空注意力机制和多尺度头任务，无需循环结构即可实现动态场景的时空理解，在nuScenes数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要准确感知动态场景的演化，传统模块化感知架构存在累积误差和延迟问题。实例预测模型提供统一解决方案，但密集时空信息的有效处理是关键挑战，需要既能捕捉细粒度运动模式和长程依赖，又不影响实时性能的架构。

Method: 提出BEVPredFormer相机BEV实例预测架构：1）基于注意力的3D相机信息投影；2）无循环设计，使用门控Transformer层；3）分时-空注意力机制；4）多尺度头任务；5）差异引导特征提取模块增强时序表示。

Result: 在nuScenes数据集上进行广泛消融研究，验证了各架构组件的有效性。BEVPredFormer在性能上达到或超越了当前最先进方法，展示了其在自动驾驶感知中的鲁棒性和高效性潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于合成数据的方法仅通过增加训练样本多样性间接提升鲁棒性，未能明确指导模型关注输入空间中真正有助于判别的区域，导致模型可能学习到由合成偏差和伪影引起的虚假相关性。

Method: 提出学习框架，利用训练数据合成过程中获得的来源信息（指示输入空间每个区域是否源自目标对象）作为辅助监督信号。具体通过基于合成过程中的目标和非目标区域信息分解输入梯度，并引入输入梯度引导来抑制非目标区域的梯度，从而抑制模型对非目标区域的依赖，直接促进目标区域判别性表征的学习。

Result: 实验证明该方法在多个任务和模态上具有有效性和通用性，包括弱监督目标定位、时空动作定位和图像分类。

Conclusion: 通过利用合成数据生成过程中的来源信息作为辅助监督信号，能够直接引导模型关注目标区域，避免学习虚假相关性，从而在多种视觉任务中提升模型性能。
...
TL;DR: PolyReal是一个基于真实世界科学实践的多模态基准测试，用于评估多模态大语言模型在聚合物实验全生命周期中的能力，揭示模型在知识推理与实践任务之间的能力失衡。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在通用领域表现出色，但在复杂真实世界科学应用中存在局限。聚合物科学作为跨学科领域，包含多样化的多模态数据，是理想的高风险测试平台。现有聚合物科学相关基准测试大多忽视真实工作流程，限制了其实用价值，未能系统评估模型在实践导向的实验全生命周期中的表现。

Method: 提出PolyReal基准测试，基于真实世界科学实践，评估多模态大语言模型在聚合物实验全生命周期中的五个关键能力：(1)基础知识应用；(2)实验室安全分析；(3)实验机制推理；(4)原始数据提取；(5)性能与应用探索。对领先的多模态大语言模型在PolyReal上进行评估。

Result: 评估显示多模态大语言模型存在能力失衡现象：模型在知识密集型推理任务（如实验机制推理）上表现良好，但在实践性任务（如实验室安全分析和原始数据提取）上表现急剧下降。这暴露了抽象科学知识与实际、上下文相关应用之间的严重差距，表明这些真实世界任务对多模态大语言模型仍然具有挑战性。

Conclusion: PolyReal有助于解决当前评估空白，为评估AI系统在真实世界科学工作流程中的表现提供了实用基准测试。研究揭示了多模态大语言模型在科学应用中的局限性，特别是理论与实践之间的鸿沟，为未来模型改进提供了方向。
...
### [64] [Modality-Specific Hierarchical Enhancement for RGB-D Camouflaged Object Detection](https://arxiv.org/abs/2604.02935)
*Yuzhen Niu,Yangqing Wang,Ri Cheng,Fusheng Li,Rongshen Wang,Zhichen Yang*

Main category: cs.CV

TL;DR: MHENet：通过纹理和几何层次增强模块以及自适应动态融合，提升RGB-D伪装目标检测性能的新框架


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D伪装目标检测方法未能充分利用模态特定线索，直接在主干提取后进行特征融合，限制了融合质量

Method: 提出MHENet框架，包含纹理层次增强模块（THEM）提取高频信息放大细微纹理变化，几何层次增强模块（GHEM）通过可学习梯度提取增强几何结构，以及自适应动态融合模块（ADFM）进行空间变化权重的自适应融合

Result: 在四个基准测试上超越16个最先进方法，实现定性和定量性能提升

Conclusion: 通过模态特定的层次增强和自适应融合，MHENet有效提升了RGB-D伪装目标检测性能
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态语义分割方法依赖精心设计的融合策略，通常使用模态特定适配或松散耦合交互，限制了灵活性并导致跨模态协调效果不佳。这些方法难以在不同模态组合中平衡高效信息交换与保留各模态独特特性。
...
Conclusion: CrossWeaver为任意模态语义分割提供了一种简单有效的多模态融合解决方案，通过选择性跨模态交互和轻量级特征聚合，在保持各模态特性的同时实现高效信息交换。
...
TL;DR: CoMP是一种针对视觉语言模型的高效剪枝框架，通过联合参数剪枝和令牌剪枝，在保持性能的同时实现高压缩比。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型剪枝方法通常只关注单一模式（参数或令牌），未能充分利用两种模式的内在冗余性，导致在高剪枝率下性能显著下降。需要一种能够协同利用参数和令牌冗余的剪枝方法。

Method: 提出协作多模式剪枝框架CoMP：1）设计协作重要性度量CIM，考虑参数和令牌之间的相互干扰，将令牌重要性纳入参数重要性计算，同时减少已剪枝参数对令牌重要性的影响；2）开发多模式剪枝策略MPS，将剪枝过程分解为多个阶段，根据剪枝成本估计不同模式的优先级并自适应切换到最优模式，结合历史成本和随机探索实现稳定剪枝。

Result: 在各种视觉语言任务和模型上的实验表明，与现有最先进方法相比，CoMP在高剪枝率下能有效提升性能。

Conclusion: CoMP通过协同参数和令牌剪枝，充分利用了视觉语言模型的内在冗余，在高压缩比下保持了更好的性能，为资源受限设备上的视觉语言模型部署提供了有效解决方案。
...
Method: 提出UAVGen框架：1）视觉原型条件扩散模型（VPC-DM），为每个类别构建代表性实例并集成到潜在嵌入中，实现高保真目标生成；2）焦点区域增强数据管道（FRE-DP），强调合成中目标集中的前景区域；3）标签细化机制，修正缺失、多余和未对齐的生成结果。

Result: 实验结果表明，该方法显著优于现有最先进方法，与不同检测器集成时能持续提升检测精度。代码已开源。

Conclusion: UAVGen通过改进布局到图像生成质量，特别是针对小目标的边界伪影问题，有效提升了无人机目标检测在数据有限场景下的性能。
...
TL;DR: 该论文为北极海冰分类建立了一个可信的SAR-only基线，使用ViT-Large模型配合focal loss在AI4Arctic/ASIP数据集上取得了最佳性能，为未来多模态融合研究提供了基准。


<details>
  <summary>Details</summary>
Motivation: 北极海冰的准确自动分类对气候监测和海上安全至关重要。虽然SAR是操作标准，但在严重类别不平衡下区分形态相似的冰类仍然具有挑战性。本文旨在建立一个可信的SAR-only基线，为未来的多模态融合研究奠定基础。
...
Result: 在测试配置中，使用focal loss的ViT-Large模型取得了69.6%的保留集准确率、68.8%的加权F1分数，以及在少数类Multi-Year Ice上83.9%的精确率。结果表明focal loss训练为稀有冰类提供了比加权交叉熵更有用的精确率-召回率权衡。

Conclusion: focal-loss训练为稀有海冰类别提供了更优的精度-召回权衡，建立了一个更清晰的基线，为未来与光学、热红外或气象数据的多模态融合研究奠定了基础。
...
TL;DR: MLA-Gen：通过全局运动先验与细粒度局部条件化改进文本驱动人体运动生成，解决运动-语言对齐问题，并引入注意力沉没现象分析和调控策略


<details>
  <summary>Details</summary>
Motivation: 文本驱动人体运动生成中，运动动态与文本语义的准确对齐仍然是一个根本性挑战。现有方法在利用文本信息方面存在不足，特别是注意力机制可能过度集中在起始文本标记上，限制了信息性文本线索的利用，导致语义基础退化。

Method: 提出MLA-Gen框架，整合全局运动先验与细粒度局部条件化，以捕捉常见运动模式并建立文本与运动间的详细对齐。识别注意力沉没现象，引入SinkRatio度量注意力集中程度，并开发对齐感知掩码和控制策略来调控生成过程中的注意力分布。

Result: 大量实验表明，该方法在多个强基线模型上持续提升了运动质量和运动-语言对齐效果。提出的SinkRatio度量有效揭示了注意力分布问题，调控策略显著改善了文本信息的利用。

Conclusion: 从运动-语言对齐角度重新审视文本到运动生成问题，MLA-Gen通过整合全局先验与局部条件化以及注意力调控，有效解决了运动动态与文本语义的对齐挑战，为文本驱动人体运动生成提供了新的解决方案。
...
TL;DR: VLM-UnBench是首个用于视觉语言模型训练无关视觉概念遗忘的基准，涵盖4个遗忘级别、7个源数据集和11个概念轴，通过三级探测分类和五种评估条件来区分真实遗忘与指令遵从。


<details>
  <summary>Details</summary>
Motivation: 现有训练遗忘方法存在结构性缺陷：在狭窄遗忘集上微调会损害通用能力，使性能下降归因困难；而训练无关方法缺乏严格的视觉任务评估基准。

Method: 构建VLM-UnBench基准，包含四个遗忘级别（概念、类别、数据集、模态）、7个源数据集、11个概念轴，采用三级探测分类（直接、间接、对抗）和五种评估条件（无指令、遗忘指令、保留指令、遗忘+保留、遗忘+保留+对抗）。

Result: 现实遗忘提示下遗忘准确率接近无指令基线；仅在oracle条件下（向模型披露目标概念）出现有意义降低；物体和场景概念最难抑制；强指令调优模型即使有明确遗忘指令仍保持能力；提示级抑制与真实视觉概念擦除存在明显差距。

Conclusion: 训练无关的视觉概念遗忘方法效果有限，现实遗忘提示基本无效，仅在高信息条件下有效；需要更有效的训练无关遗忘技术来实现真正的视觉概念擦除。
...
TL;DR: 研究探讨了视网膜血管分割中图像下采样对细血管检测的影响，发现标准Dice指标无法捕捉细血管信息损失，提出基于血管宽度的分层敏感性评估方法。


<details>
  <summary>Details</summary>
Motivation: 大多数深度学习视网膜血管分割流程为满足GPU内存限制会将眼底图像下采样，但这种下采样对细血管检测的影响尚未充分研究。高分辨率图像下采样时，细血管会变为亚像素结构，导致信息损失，而标准的Dice指标无法捕捉这种损失。

Method: 在五个眼底数据集（DRIVE、STARE、CHASE_DB1、HRF、FIVES）上训练基线UNet模型，使用多个下采样比例，保持其他设置不变。引入宽度分层敏感性指标，将血管分为细（半宽<3像素）、中（3-7像素）、粗（>7像素）三类分别评估，使用欧几里得距离变换获取原生分辨率宽度估计。

Result: 对于高分辨率数据集（HRF、FIVES），细血管敏感性随着图像下采样到编码器有效操作范围而单调提高，在256-876像素处理宽度时达到峰值。对于低到中分辨率数据集（DRIVE、STARE、CHASE_DB1），细血管敏感性在原生分辨率或接近时最高，任何下采样都会降低性能。在所有五个数据集中，激进下采样使细血管敏感性降低高达15.8个百分点，而Dice指标保持相对稳定。

Conclusion: Dice指标单独不足以评估微血管分割质量，图像下采样对细血管检测有显著影响，需要根据数据集分辨率选择合适的下采样策略，并采用分层敏感性指标来全面评估血管分割性能。
...
TL;DR: SCOPE是一个无需训练的高效自回归视频扩散模型框架，通过三模态调度（缓存、预测、重计算）和选择性计算，实现高达4.73倍加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型虽然能生成长视频，但多步去噪过程计算成本高。现有免训练加速方法采用二元的缓存或重计算决策，忽略了中间情况：直接重用太粗糙，完全重计算又不必要。此外，异步自回归调度给共生成帧分配不同噪声水平，但现有方法统一处理整个有效区间。

Method: SCOPE提出三模态调度器（缓存、预测、重计算），其中预测通过噪声级泰勒外推填补重用和重计算之间的空白，并基于误差传播分析提供显式稳定性控制。还引入选择性计算，将执行限制在活动帧区间内。

Result: 在MAGI-1和SkyReels-V2数据集上，SCOPE实现了高达4.73倍的加速，同时保持与原始输出相当的质量，优于所有免训练基线方法。

Conclusion: SCOPE通过三模态调度和选择性计算有效解决了自回归视频扩散模型的特定低效问题，在保持生成质量的同时显著提升了推理效率。
...
Method: 提出MM-GS分层框架：1）实例级多视角融合模块，通过聚合所有可用视角的视觉信息为每个实例建立鲁棒一致的表示；2）场景级实例交互模块，在全局场景图上推理所有参与者之间的关系，细化属性以捕捉细微交互效应

Result: 在具有挑战性的数据集上进行广泛实验，方法显著优于强基线，产生最先进的结果，具有高保真细节和合理的实例间接触

Conclusion: MM-GS通过分层方法有效解决了MHMO渲染中的遮挡和交互建模问题，为动态场景重建提供了高质量的解决方案
...
TL;DR: 本文提出了一种用于NTIRE 2026 3D恢复与重建挑战赛Track 2的多阶段管道，通过图像修复、去雾、MLLM增强、3DGS-MCMC优化和多次运行平均，在提升烟雾退化图像可见度的同时保持跨视图一致性，最终在14支队伍中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 烟雾会降低图像可见度并削弱场景优化和渲染所需的跨视图一致性，这给3D恢复与重建带来了挑战。需要一种方法既能提升图像可见度，又能限制输入视图间的场景内容变化。

Method: 采用多阶段管道：1) 图像修复；2) 去雾处理；3) 基于MLLM（多模态大语言模型）的增强；4) 3DGS-MCMC（3D高斯散射-马尔可夫链蒙特卡洛）优化；5) 对多次运行结果进行平均。该管道旨在渲染前提升可见度，同时限制跨视图的场景内容变化。

Result: 在挑战赛基准测试中，该方法在定量性能和视觉质量上均优于提供的基线方法。在NTIRE 3DRR挑战赛Track 2的14名参与者中排名第一。

Conclusion: 提出的多阶段管道有效解决了烟雾退化图像的3D恢复与重建问题，在提升图像可见度的同时保持了跨视图一致性，在挑战赛中取得了最佳性能。
...
TL;DR: PR3DICTR是一个用于三维医学图像分类研究的开源平台，基于PyTorch和MONAI构建，提供模块化设计和标准化训练框架，简化深度学习模型开发流程。


<details>
  <summary>Details</summary>
Motivation: 三维医学图像数据和基于深度学习的计算机辅助决策在医疗领域日益重要，但缺乏灵活、标准化的开发平台来减轻研究人员在模型开发过程中的负担。

Method: 基于社区标准分发（PyTorch和MONAI）构建开源平台，采用模块化设计原则和标准化方法，提供丰富的预置功能（模型架构设计选项、超参数解决方案、训练方法），同时允许用户插入自定义模块。

Result: 开发了PR3DICTR平台，支持任何二元或基于事件的三维分类任务，仅需两行代码即可使用，平衡了开发便利性和可调整性。

Conclusion: PR3DICTR为三维医学图像分类研究提供了一个开放、灵活且便捷的框架，通过标准化和模块化设计减轻开发负担，同时保持可定制性，有助于推动该领域的研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 视频异常检测面临开放集挑战，现有基于静态提示的方法依赖大规模基础模型。作者认为瓶颈在于查询的静态性而非模型容量，需要动态交互来提升性能

Method: 提出QVAD框架，将VLM-LLM交互视为动态对话。LLM智能体基于视觉上下文迭代优化查询，引导小型VLM生成高质量描述和精确语义推理，无需参数更新

Result: 在UCF-Crime、XD-Violence、UBNormal数据集上达到最先进性能，参数使用量远低于竞争方法。在ComplexVAD数据集上展示出色泛化能力，推理速度快且内存占用小

Conclusion: QVAD通过"提示更新"机制释放轻量级模型潜力，使先进视频异常检测能力可在资源受限的边缘设备上部署，为开放集异常检测提供新范式
...
TL;DR: 本文系统评估了Nano Banana 2作为通用图像修复模型的性能，发现提示设计对结果质量至关重要，该模型在全参考指标上优于现有方法，在感知质量上保持竞争力，展现了作为统一图像修复工具的潜力。
...
Method: 对Nano Banana 2进行系统性评估，重点研究提示设计的关键作用，采用简洁提示并加入明确的保真度约束，在多种场景和退化类型下进行测试。

Result: Nano Banana 2在全参考指标上优于现有最先进的修复模型，在感知质量上保持竞争力，用户研究进一步支持了这一结论。模型在小脸、密集人群和严重退化等挑战性场景中表现出强大的泛化能力。

Conclusion: 通用生成模型作为统一图像修复工具具有强大潜力，但提示表述的敏感性表明需要迭代优化才能获得最佳结果，强调了可控性和鲁棒性的重要性。
...
TL;DR: 提出基于互信息（MI）的视觉剪枝方法MI-Pruner，直接计算视觉与文本特征间的互信息来度量跨模态依赖，无需依赖注意力机制或修改模型架构。


<details>
  <summary>Details</summary>
Motivation: 针对多模态大语言模型（MLLMs）中视觉信息相对稀疏的问题，现有视觉剪枝方法通常基于注意力分数衡量token重要性，但这种方法依赖于特定机制。本文追求更直接、更精确的方法来度量跨模态依赖关系。

Method: 提出MI-Pruner方法，直接计算视觉特征与文本特征之间的互信息（Mutual Information），在特征交互之前显式度量跨模态依赖。该方法简单、高效且非侵入式，无需访问内部注意力图或修改模型架构。

Result: 实验结果表明，该方法在保持最小延迟的同时，优于先前基于注意力的剪枝方法。

Conclusion: 通过直接计算视觉与文本特征间的互信息来度量跨模态依赖，MI-Pruner提供了一种更直接、更有效的视觉剪枝方法，优于依赖注意力机制的现有方法。
...
TL;DR: UCGP是一种针对红外视觉语言模型的通用物理对抗补丁框架，通过曲线网格参数化和统一表示驱动目标破坏跨模态语义对齐


<details>
  <summary>Details</summary>
Motivation: 现有对抗补丁方法主要针对RGB模型和封闭集设置，不适用于红外VLM的开放式语义理解和物理部署需求，需要探索红外多模态系统的鲁棒性漏洞

Method: 提出通用曲线网格补丁(UCGP)框架，集成曲线网格网格(CGM)参数化实现连续、低频、可部署补丁生成，采用统一表示驱动目标促进子空间偏离、拓扑破坏和隐蔽性，结合元差分进化和EOT增强的TPS变形建模提升鲁棒性

Result: UCGP能持续破坏多种IR-VLM架构的语义理解，同时保持跨模型可迁移性、跨数据集泛化能力、真实世界物理有效性以及对防御的鲁棒性

Conclusion: UCGP揭示了当前红外多模态系统中先前被忽视的鲁棒性漏洞，为红外VLM的安全评估提供了新视角
...
<details>
  <summary>Details</summary>
Motivation: 将视频生成模型蒸馏到极低推理预算（2-4 NFE）对实时部署至关重要，但现有方法存在局限：轨迹一致性蒸馏在复杂视频动态下过于保守，导致过度平滑外观和弱运动；分布匹配蒸馏（DMD）能恢复锐利、模式寻求的样本，但其局部训练信号未显式正则化去噪更新在时间步上的组合方式，使得组合推演容易漂移。
...
Conclusion: SC-DMD通过显式正则化去噪更新的组合一致性，有效解决了低NFE视频生成中的漂移问题，结合缓存分布感知训练，实现了实时高质量视频生成，为视频生成模型的实时部署提供了有效解决方案。
...
TL;DR: CAMEO：一个结构化多智能体框架，将条件图像编辑重新定义为质量感知、反馈驱动的过程，通过规划、结构化提示、假设生成和自适应参考接地等协调阶段，实现迭代精炼，在异常插入和人体姿态切换任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前条件图像编辑方法主要依赖单步生成范式，缺乏显式质量控制，容易导致过度偏离原始图像、产生结构伪影或环境不一致的修改，通常需要手动调整提示才能获得可接受结果。需要一种更可控、更可靠的编辑框架来解决这些问题。

Method: 提出CAMEO框架，将条件编辑分解为协调的阶段：规划、结构化提示、假设生成和自适应参考接地。框架采用多智能体结构，将评估直接嵌入编辑循环中，通过结构化反馈对中间结果进行迭代精炼，形成闭环过程，逐步纠正结构和上下文不一致性。

Result: 在异常插入和人体姿态切换任务上评估CAMEO，使用多个强大的编辑骨干和独立评估模型。相比多个最先进模型，CAMEO平均获得20%更高的胜率，展示了在条件图像编辑中改进的鲁棒性、可控性和结构可靠性。

Conclusion: CAMEO通过将条件编辑重新定义为质量感知、反馈驱动的过程，而非一次性生成任务，解决了现有方法缺乏显式质量控制的问题。其结构化多智能体框架和闭环精炼机制显著提升了编辑的可靠性和可控性，为需要严格结构控制的场景提供了有效解决方案。
...
TL;DR: EffiMiniVLM：一种紧凑的双编码器视觉语言回归框架，用于冷启动场景下的产品质量预测，仅使用20%的Amazon Reviews数据即可实现高效性能。
...
Result: 模型仅包含27.7M参数，需要6.8 GFLOPs，在仅使用20% Amazon Reviews 2023数据集训练的情况下，获得CES分数0.40，在基准测试中资源成本最低。与其他top-5方法相比，资源效率高4-8倍，且是唯一不使用外部数据集的方法。将数据扩展到40%可使模型超越其他使用更大模型和数据集的方法。

Conclusion: EffiMiniVLM展示了紧凑设计下强大的可扩展性和资源效率，在冷启动产品质量预测任务中，以最小的资源成本实现了与大型模型竞争的性能，为实际部署提供了实用解决方案。
...
TL;DR: ProtoFlow：一种时间感知的原型动态框架，通过将类别原型建模为轨迹并学习其演化，用于连续遥感分割任务


<details>
  <summary>Details</summary>
Motivation: 遥感分割在实际部署中本质上是连续的：新的语义类别不断出现，采集条件随季节、城市和传感器而变化。现有增量方法将训练步骤视为孤立更新，导致表征漂移和遗忘问题控制不足。

Method: 提出ProtoFlow框架，将类别原型建模为轨迹，通过显式时间向量场学习原型演化。联合实施低曲率运动和类间分离约束，在整个增量学习过程中稳定原型几何结构。

Result: 在标准类别增量和域增量遥感基准测试中，相比强基线获得一致提升，mIoUall指标提高1.5-2.0个百分点，同时减少了遗忘现象。

Conclusion: 显式建模时间原型演化是构建鲁棒连续遥感分割的实用且可解释策略。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成式超分辨率方法主要依赖预训练的大型文本到图像扩散模型，但超分辨率本质上是低分辨率输入条件下的图像恢复任务。本研究旨在探索纯视觉数据训练的SR模型是否能与T2I方法相媲美。

Method: 提出VOSR框架：1) 使用预训练视觉编码器从LR输入提取语义丰富且空间基础的特征作为视觉语义引导；2) 重新审视无分类器指导，用恢复导向的指导策略替代标准无条件分支，保留弱LR锚点；3) 先训练多步模型，然后蒸馏为单步模型进行高效推理。

Result: VOSR训练成本不到代表性T2I方法的十分之一，在多步和单步设置下均实现竞争性甚至更好的感知质量和效率，在合成和真实世界基准上产生更忠实结构且幻觉更少。
...
TL;DR: CoME-VL提出了一种融合对比学习和自监督视觉编码器的多编码器视觉语言模型框架，通过熵引导的多层聚合和RoPE增强的跨注意力机制实现互补视觉表示的有效融合。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型通常只使用单一对比学习训练的视觉编码器（如CLIP），虽然这种编码器在跨模态对齐和检索方面有效，但自监督视觉编码器（如DINO）往往能捕获更丰富的密集语义，并在识别和理解任务上表现出更强的鲁棒性。因此需要探索如何融合这两种互补的视觉表示来提升视觉语言模型的性能。

Method: 提出了CoME-VL（Complementary Multi-Encoder Vision-Language）模块化融合框架，集成对比学习训练的视觉编码器和自监督DINO编码器。采用两种关键技术：1）熵引导的多层聚合，通过正交约束投影减少冗余；2）RoPE增强的跨注意力机制，对齐异构令牌网格并生成紧凑的融合视觉令牌。融合后的令牌可以以最小改动注入到仅解码器的LLM中。

Result: 在多种视觉语言基准测试中，CoME-VL始终优于单编码器基线模型。具体而言，在视觉理解任务上平均提升4.9%，在接地任务上平均提升5.4%。在RefCOCO检测任务上达到最先进性能，相比基线有大幅提升。消融研究验证了层合并、非冗余特征混合和融合容量对性能的影响。

Conclusion: 融合对比学习和自监督视觉编码器的互补表示能显著提升视觉语言模型的性能。CoME-VL框架通过有效的表示级融合机制，在保持标准VLM流程最小改动的同时，实现了多编码器优势的协同效应，为视觉语言建模提供了新的方向。
...
TL;DR: 该研究针对大语言模型在心理健康支持中的风险，特别是对精神病患者的潜在危害，开发了临床验证的安全评估方法，并测试了LLM作为评估者的可行性。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型被广泛用于心理健康支持，但高频使用存在显著风险，特别是可能强化精神病患者的妄想和幻觉。现有评估缺乏临床验证和可扩展性，需要建立更可靠的安全评估方法。

Method: 研究聚焦精神病作为LLM安全评估的关键条件：(1)开发并验证七个临床医生指导的安全标准；(2)构建人类共识数据集；(3)测试自动化评估方法，包括LLM作为单一评估者(LLM-as-a-Judge)和多个LLM的多数投票(LLM-as-a-Jury)。
...
Conclusion: 研究结果为心理健康背景下基于临床的、可扩展的LLM安全评估方法提供了有前景的解决方案，LLM评估者能够有效替代人类专家进行安全评估。
...
TL;DR: CIPHER模型使用ERP特征和宽带DDA系数从高密度头皮EEG解码语音信息，在二分类任务中表现良好但易受混淆因素影响，在11类CVC音素任务中性能有限，主要作为基准和特征比较研究而非实用的EEG到文本系统。


<details>
  <summary>Details</summary>
Motivation: 从头皮EEG解码语音信息面临信噪比低和空间模糊的挑战，需要开发更有效的解码方法，并评估不同神经特征在语音解码中的表现。

Method: 提出CIPHER模型，采用双通路架构：一路使用ERP特征，另一路使用宽带DDA系数。在OpenNeuro ds006104数据集（24名参与者，两项包含TMS的研究）上进行评估，包括二分类发音任务和11类CVC音素任务。

Result: 二分类发音任务达到接近天花板性能但高度易受混淆因素影响（声学起始可分离性和TMS目标阻断）。在完整的Study 2留一受试者交叉验证中，11类CVC音素任务性能较低（真实词WER：ERP 0.671±0.080，DDA 0.688±0.096），表明细粒度区分能力有限。

Conclusion: 该研究主要作为基准和特征比较研究而非实用的EEG到文本系统，对神经表征的声明仅限于混淆因素受控的证据，强调需要更鲁棒的EEG语音解码方法。
...
TL;DR: 提出SWAY无监督计算语言学指标量化LLM的谄媚性，开发反事实提示机制分离框架效应与内容，发现谄媚性随认知承诺增加，并提出反事实思维链缓解策略将谄媚性降至接近零。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在谄媚性（sycophancy）问题，即倾向于向用户表达的立场偏移输出，无论其正确性或一致性。现有研究缺乏严谨的计算语言学指标来识别模型何时表现出谄媚行为，需要开发量化度量方法和缓解策略。

Method: 1. 提出SWAY无监督计算语言学指标，通过反事实提示机制量化谄媚性：比较模型在正面与负面语言压力下的同意程度变化，分离框架效应与内容。2. 应用该指标对6个模型进行基准测试。3. 基于指标提出反事实缓解策略：教导模型考虑如果提出相反假设时答案会是什么，通过反事实思维链（CoT）进行缓解。

Result: 1. 发现谄媚性随认知承诺（epistemic commitment）增加而增加。2. 基线缓解策略（明确指示反谄媚）仅产生中等程度的减少，且可能适得其反。3. 反事实思维链缓解策略能将谄媚性降至接近零，跨模型、承诺水平和子句类型均有效，同时不抑制对真实证据的响应性。

Conclusion: 贡献了用于基准测试谄媚性的SWAY度量指标，以及基于该指标的反事实缓解策略。反事实思维链方法能有效将谄媚性降至接近零，同时保持对真实证据的响应能力，为解决语言模型谄媚性问题提供了有效工具。
...
TL;DR: 在计算预算固定的条件下，单智能体系统在多跳推理任务上通常优于多智能体系统，后者表现出的优势主要源于未控制的计算差异和上下文利用效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示多智能体LLM系统性能优异，但这些增益常与增加的计算量混淆。当计算量归一化时，单智能体系统可匹配或超越多智能体系统，但相关理论基础和评估方法仍不明确。需要从信息论角度分析不同架构的效率差异，并控制变量进行实证验证。

Method: 提出基于数据处理不等式的信息论论证，表明在固定推理token预算和完美上下文利用条件下，单智能体系统信息效率更高。通过控制实验，在三个模型家族(Qwen3、DeepSeek-R1-Distill-Llama、Gemini 2.5)上比较单智能体系统与多种多智能体架构，保持预算匹配。进行详细的诊断分析，识别API预算控制和基准测试中的潜在偏差。

Result: 在推理token固定的情况下，单智能体系统在多跳推理任务上始终匹配或优于多智能体系统。发现API预算控制存在显著伪影（特别是Gemini 2.5），标准基准测试也存在偏差，这些都可能夸大多智能体系统的表现增益。多智能体系统仅在单智能体上下文利用效率降低或计算量增加时才具有竞争力。

Conclusion: 对于多跳推理任务，多智能体系统报告的许多优势实际上源于未控制的计算差异和上下文效应，而非架构本身的内在优势。研究强调了理解并明确控制计算、上下文和协调之间权衡的重要性，为智能体系统设计提供了更严谨的评估框架。
...
TL;DR: 研究发现大型语言模型在假设检验任务中存在确认偏误，表现为倾向于提出支持而非挑战假设的证据，这降低了规则发现效率。通过人类心理学干预策略的提示可以缓解此偏误。


<details>
  <summary>Details</summary>
Motivation: 确认偏误是人类认知中的常见偏差，会阻碍理性推理。研究旨在探究大型语言模型是否也存在类似的确认偏误，以及这种偏误如何影响其在假设检验任务中的表现。

Method: 采用人类心理学中的规则发现研究范式：给定三个数字序列，模型通过交互反馈循环（提出新序列、接收反馈、猜测规则）进行假设检验。测试了11个不同家族和规模的大型语言模型，并探索了针对人类的干预策略（如鼓励考虑反例）对模型的影响。

Result: 所有测试的大型语言模型都表现出确认偏误，倾向于提出确认假设而非证伪假设的三元组，导致规则发现速度更慢、成功率更低（平均42%）。通过提示干预策略，规则发现率提升至平均56%。通过蒸馏干预诱导的行为到模型中，确认偏误得到缓解，并在新任务（Blicket测试）中展现出良好的泛化能力。

Conclusion: 确认偏误是大型语言模型在假设探索中的一个局限性，但可以通过注入为人类设计的干预策略来缓解。这表明大型语言模型与人类在认知偏差方面存在相似性，且人类心理学干预方法对改善模型推理能力具有借鉴价值。
...
TL;DR: 该论文研究大语言模型是否能在定量和定性上近似人类社交意义推理，并提出基于语用学理论的提示策略来改进这种近似。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大语言模型是否不仅能在定性上，还能在定量上近似人类的语用和社会推理模式，以及基于语用学理论的提示策略是否能改进这种近似。

Method: 方法包括：1) 引入两个校准指标：效应大小比(ESR)和校准偏差分数(CDS)；2) 从两个语用学假设推导提示条件：社交意义源于对语言替代方案的推理，以及听者推断说话者知识状态和交际动机；3) 在三个前沿LLM上进行数字(不)精确性的案例研究。

Result: 所有模型都能可靠地复制人类社交推理的定性结构，但在幅度校准上差异显著。提示模型推理说话者知识和动机最能一致地减少幅度偏差，而提示替代意识往往会放大夸张。结合这两个组件是唯一能改进所有模型所有校准敏感指标的干预措施，但细粒度幅度校准仍只得到部分解决。

Conclusion: LLMs捕捉了推理结构但不同程度地扭曲了推理强度，语用学理论为改进这种近似提供了有用但不完整的工具。
...
TL;DR: 提出一种基于基数约束二次规划的理论保证多样性检索方法，解决现有方法缺乏理论保证和可扩展性问题


<details>
  <summary>Details</summary>
Motivation: 现有多样性检索方法缺乏理论保证，且随着检索段落数k增加面临可扩展性问题，需要一种理论严谨且高效的方法

Method: 将多样性检索建模为基数约束二元二次规划问题，通过可解释权衡参数平衡相关性和语义多样性，采用非凸紧连续松弛和Frank-Wolfe算法

Result: 方法在相关性-多样性帕累托前沿上始终优于基线，同时实现显著加速，具有收敛保证

Conclusion: 提出的理论框架为多样性检索提供了原则性解决方案，在保持理论严谨性的同时实现了实际性能优势
...
TL;DR: 该论文研究了语言模型在文化适应性艺术描述生成任务中的表现，发现基础模型在此任务上表现有限，但通过实用主义说话者模型可将模拟听众理解率提升8.2%，人类研究也证实该模型在帮助理解方面评分更高。


<details>
  <summary>Details</summary>
Motivation: 语言模型在决策任务中表现出各种文化偏见，但对其在开放式文本生成任务中的文化熟悉度了解较少。研究者希望探索模型在文化适应性艺术描述生成任务中的表现，即针对不同文化背景的观众生成艺术描述，这些观众对艺术品中嵌入的文化符号和叙事熟悉度不同。

Method: 提出文化适应性艺术描述生成任务，并基于文化基础问答构建评估框架。通过实用主义说话者模型来改进生成质量，使用模拟听众理解和人类研究来评估模型的文化能力。

Result: 基础模型在此任务上表现有限，但实用主义说话者模型能将模拟听众理解率提升8.2%。人类研究进一步证实，具有更高实用主义能力的模型在帮助理解方面评分高出8.0%。

Conclusion: 语言模型在文化适应性生成任务中存在局限性，但通过实用主义建模方法可以显著提升其文化能力，使其更好地为不同文化背景的观众生成有帮助的艺术描述。
...
### [94] [Too Polite to Disagree: Understanding Sycophancy Propagation in Multi-Agent Systems](https://arxiv.org/abs/2604.02668)
...
TL;DR: 该研究探讨了在多智能体协作系统中，通过提供同伴谄媚倾向排名来降低谄媚行为对讨论结果的影响，实验表明这种方法能显著提高讨论准确性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型常表现出谄媚行为（同意用户立场即使与模型自身观点冲突）。先前研究主要在单智能体环境中进行，而在协作多智能体系统中这一问题尚未充分探索。研究者想知道了解其他智能体的谄媚水平是否会影响讨论结果。

Method: 使用六个开源LLM进行受控实验，为智能体提供同伴谄媚排名（基于静态和动态策略计算的分数）。这些排名估计每个同伴的谄媚倾向，包括预讨论的静态策略和在线动态策略。

Result: 提供谄媚先验信息能减少易谄媚同伴的影响，缓解错误级联，并将最终讨论准确性提高10.5%（绝对值）。这表明该方法能有效降低讨论中的谄媚行为并提高下游准确性。

Conclusion: 在多智能体协作系统中提供同伴谄媚倾向排名是一种轻量级且有效的方法，能够减少谄媚行为对讨论的影响，提高决策准确性，为改善多智能体协作系统提供了实用解决方案。
...
### [95] [Dependency-Guided Parallel Decoding in Discrete Diffusion Language Models](https://arxiv.org/abs/2604.02560)
*Liran Ringel,Ameen Ali,Yaniv Romano*

Main category: cs.CL

TL;DR: DEMASK提出了一种轻量级依赖预测器，通过估计掩码位置间的条件依赖关系，指导并行解码中的令牌选择，在保持质量的同时加速离散扩散语言模型生成。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型通过并行解码加速文本生成，但完全因子化的边缘分布近似会忽略令牌间的强依赖关系，导致输出质量下降。需要解决并行解码中的分布不匹配问题。

Method: 提出DEMASK方法：1）轻量级依赖预测器附加到dLLM的最终隐藏状态，单次前向传播估计掩码位置间的成对条件影响；2）贪心选择算法识别具有有限累积依赖的位置进行同时解掩码；3）在次可加性假设下，理论证明该方法能限制并行采样与模型联合分布间的总变差距离。

Result: 在Dream-7B模型上，DEMASK实现了1.7-2.2倍的加速，同时匹配或优于基于置信度和KL散度的基线方法在准确性方面的表现。

Conclusion: DEMASK通过显式建模令牌间依赖关系，有效解决了离散扩散语言模型并行解码中的分布不匹配问题，在保持生成质量的同时显著加速推理过程。
...
### [96] [An Empirical Study of Many-Shot In-Context Learning for Machine Translation of Low-Resource Languages](https://arxiv.org/abs/2604.02596)
*Yinhan Lu,Gaganpreet Jhajj,Chen Zhang,Anietie Andy,David Ifeoluwa Adelani*

Main category: cs.CL

TL;DR: 该论文实证研究了多示例上下文学习在英语到十种低资源语言机器翻译中的应用，发现基于BM25的检索方法能显著提升数据效率，50个检索示例的效果与250个多示例相当。


<details>
  <summary>Details</summary>
Motivation: 上下文学习使大语言模型能够通过少量示例适应新任务，这对预训练中代表性不足的语言很有前景。多示例上下文学习可以利用长上下文窗口处理更多示例，但效果依赖于示例选择，且推理成本对低资源语言社区可能过高。

Method: 研究英语到十种FLORES+新增低资源语言的机器翻译，分析检索更多信息性示例、使用领域外数据以及按长度排序示例的影响。采用BM25检索方法进行示例选择。

Result: 多示例上下文学习随着示例数量增加而更有效。BM25检索显著提升数据效率：50个检索示例的效果与250个多示例相当，250个检索示例的效果与1000个多示例相似。

Conclusion: 基于检索的多示例上下文学习能有效降低低资源语言机器翻译的推理成本，通过更高效的示例选择实现与大量示例相当的性能。
...
### [97] [Reinforcement Learning-based Knowledge Distillation with LLM-as-a-Judge](https://arxiv.org/abs/2604.02621)
*Yiyang Shen,Lifu Tu,Weiran Wang*

Main category: cs.CL

TL;DR: 提出使用LLM作为评判者生成奖励信号的强化学习框架，无需真实标签即可进行知识蒸馏，结合可验证奖励在数学推理任务上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法通常依赖可验证奖励和真实标签，限制了在无标注数据上的应用。需要开发能够在无监督环境下利用LLM生成有效训练信号的框架。

Method: 提出基于LLM评判者的强化学习框架：1) 使用LLM作为评判者评估模型输出，生成奖励信号；2) 评判者采用单令牌输出设计，提高奖励计算效率；3) 在大量无标注数据上进行训练；4) 结合可验证奖励进行混合训练。

Result: 该方法在数学推理基准测试中取得了显著的性能提升。结果表明LLM评估器能够为强化学习微调产生有效的训练信号，实现了标签自由的知识蒸馏。

Conclusion: LLM作为评判者能够生成有效的训练信号，无需真实标签即可进行强化学习微调。该方法为在无标注数据上利用强化学习提升语言模型推理能力提供了新途径。
...
### [98] [Train Yourself as an LLM: Exploring Effects of AI Literacy on Persuasion via Role-playing LLM Training](https://arxiv.org/abs/2604.02637)
*Qihui Fan,Min Ge,Chenyan Jia,Weiyan Shi*

Main category: cs.CL

TL;DR: LLMimic：一种基于角色扮演的交互式游戏化AI素养教程，通过让用户扮演LLM的训练过程，显著提高AI素养并降低AI说服成功率


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型变得越来越有说服力，人们担心其可能在不同情境下大规模影响人们的观点和决策。现有的缓解措施（如AI检测器和免责声明）大多将人们视为AI生成信息的被动接受者，需要更主动的干预措施来对抗有说服力的AI。

Method: 开发了LLMimic——基于角色扮演的交互式游戏化AI素养教程，参与者扮演LLM角色，经历三个关键训练阶段：预训练、监督微调和人类反馈强化学习。通过2×3被试间设计研究（N=274），比较观看AI历史视频（对照组）与使用LLMimic（实验组）的效果，并在三种现实AI说服场景中测试：慈善捐款说服、恶意金钱索取、酒店推荐。

Result: LLMimic显著提高了参与者的AI素养（p<.001），在所有场景中降低了说服成功率（p<.05），在酒店推荐场景中提高了真实性和社会责任水平（p<0.01）。

Conclusion: LLMimic提供了一种可扩展的、以人为中心的方法来提高AI素养，支持与有说服力的AI进行更明智的互动，为对抗AI说服提供了有效的主动干预策略。
...
### [99] [Overcoming the "Impracticality" of RAG: Proposing a Real-World Benchmark and Multi-Dimensional Diagnostic Framework](https://arxiv.org/abs/2604.02640)
...
<details>
  <summary>Details</summary>
Motivation: 企业环境中RAG系统的性能评估受多维度复合因素影响，包括推理复杂性、检索难度、文档结构多样性和操作可解释性要求。现有学术基准无法系统诊断这些相互关联的挑战，导致模型在学术基准上得分高但在实际部署中可靠性不足。

Method: 提出多维度诊断框架，定义四轴难度分类学（推理复杂度、检索难度、文档结构、可解释性要求），并将其集成到企业RAG基准测试中，用于诊断潜在系统弱点。

Result: 通过构建企业级RAG基准测试框架，能够系统诊断RAG系统在实际企业环境中的性能瓶颈和弱点，填补学术基准与实际部署需求之间的差距。

Conclusion: 企业RAG系统评估需要超越简单准确性检查的多维度诊断方法。提出的四轴难度分类学和集成基准框架能够有效识别系统弱点，提高实际部署中的可靠性。
...
### [100] [Speaking of Language: Reflections on Metalanguage Research in NLP](https://arxiv.org/abs/2604.02645)
...
<details>
  <summary>Details</summary>
Motivation: 该工作的动机是强调元语言这一重要但相对被忽视的主题，旨在建立元语言与自然语言处理和大型语言模型之间的联系，为相关研究提供理论基础和方向指引。

Method: 论文采用概念分析和框架构建的方法：首先定义元语言概念，然后将其与NLP和LLMs领域建立联系，接着介绍两个实验室在元语言方面的具体研究工作，最后提出元语言和元语言任务的四个维度分析框架。

Result: 论文提出了元语言和元语言任务的四个维度分析框架，并识别出一系列尚未充分研究的未来研究方向，为元语言在NLP和LLMs领域的研究提供了系统性的指导。

Conclusion: 元语言是NLP和LLMs领域的重要研究方向，需要更多系统性的研究。论文提出的四个维度框架和未来研究方向清单为该领域的发展提供了有价值的参考和指导。
...
### [101] [Revealing the Learning Dynamics of Long-Context Continual Pre-training](https://arxiv.org/abs/2604.02650)
*Yupu Liang,Shuang Chen,Guanwei Zhang,Shaolei Wang,Suncong Zheng*

Main category: cs.CL

TL;DR: 该研究首次系统性地研究了工业级大模型（Hunyuan-A13B，800亿参数）在长上下文持续预训练中的学习动态，揭示了需要大规模数据（超过1500亿token）、传统评估方法存在"欺骗性饱和"问题，并提出基于检索头的机制监控框架。
...
Result: 1. 工业级LLM的长上下文持续预训练需要大规模数据扩展（Hunyuan-A13B在超过1500亿token后才达到饱和）。2. 传统NIAH分数早期报告"虚假饱和"，而基于困惑度的分析显示持续内在改进且与下游性能更强相关。3. 检索头作为高效低资源的训练监控器，其注意力分数演化可靠地跟踪训练进展并与监督微调结果高度相关。

Conclusion: 该工作为工业级LLM的长上下文持续预训练提供了全面的监控框架、评估系统和机制解释，强调了大规模数据需求、内在收敛评估的重要性以及机制监控对训练稳定的价值。
...
### [102] [SocioEval: A Template-Based Framework for Evaluating Socioeconomic Status Bias in Foundation Models](https://arxiv.org/abs/2604.02660)
*Divyanshu Kumar,Ishita Gupta,Nitin Aravind Birur,Tanay Baswa,Sahil Agarwal,Prashanth Harshangi*

Main category: cs.CL

TL;DR: SocioEval是一个用于系统评估基础模型中社会经济偏见的模板化框架，通过决策任务揭示LLMs在社会经济偏见方面的显著差异和脆弱性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在关键领域决策系统中的广泛应用，理解和减轻其偏见对负责任AI部署至关重要。尽管已有针对种族和性别等属性的偏见评估框架，但社会经济地位偏见在现实世界中影响广泛却研究不足。

Method: 提出SocioEval框架，采用分层结构包含8个主题和18个话题，生成240个提示覆盖6个类别对组合。评估13个前沿LLMs的3,120个响应，使用严格的三阶段标注协议。

Result: 发现偏见率存在显著差异（0.42%-33.75%），不同主题间偏见表现不同：生活方式判断的偏见比教育相关决策高10倍。部署安全措施能有效防止显性歧视，但对特定领域刻板印象表现出脆弱性。

Conclusion: SocioEval为审计语言模型中基于阶级的偏见提供了可扩展、可扩展的基础，揭示了LLMs在社会经济偏见方面的系统性差异和现有安全措施的局限性。
...
TL;DR: 研究发现语言模型的偏见程度取决于评估任务类型，单一基准测试会系统性地误判模型偏见，当前的安全对齐实践掩盖而非减轻了表征性危害。


<details>
  <summary>Details</summary>
Motivation: 现有偏见评估方法存在局限性：单一任务基准只能捕捉模型偏见的一个切片，无法全面反映模型在不同任务类型（显式决策vs隐式关联）中的偏见表现，导致对语言模型偏见的系统性误判。

Method: 提出分层分类法涵盖9种偏见类型（包括种姓、语言和地理等研究不足的维度），通过7种评估任务（从显式决策到隐式关联）操作化，使用约45K个提示审计7个商业和开源LLM。

Result: 发现三个系统性模式：1)偏见具有任务依赖性（显式探测中反刻板印象，隐式任务中再现刻板印象，同一模型和身份群体的刻板印象得分差异达0.43）；2)安全对齐不对称（拒绝给边缘群体分配负面特质，但自由地将正面特质与特权群体关联）；3)研究不足的偏见维度在所有模型中表现出最强的刻板印象。

Conclusion: 单一基准审计会系统性地误判LLM偏见，当前对齐实践跟踪的是基准覆盖范围而非危害严重性，实际上掩盖了表征性危害而非减轻它。
...
TL;DR: 该研究通过对照实验证伪了E-Prime（无be动词英语）通过特定词汇-认知映射进行认知重构的假设，发现任何约束模型默认生成路径的限制都能作为输出正则化器改善推理，最浅层的约束效果最好。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明E-Prime选择性地改变了语言模型的推理能力，存在跨模型相关的结构特征。本研究旨在通过对照实验验证"认知重构通过特定词汇-认知映射实现"这一假设机制。

Method: 设计了包含五个条件的对照实验：无约束控制组、E-Prime组、No-Have组（无have动词）、精细元认知提示组、中性填充词禁止组。在六个模型和七个推理任务上进行测试，共15,600个试验（合规过滤后11,919个）。

Result: 认知重构假说的所有预测都被证伪。所有四个处理组都优于控制组（83.0%），包括预测应显示零效应的主动对照组。中性填充词禁止组（禁止"very"、"just"等与逻辑推理无关的词）改善最大（+6.7个百分点），E-Prime组改善最小（+3.7个百分点）。四个条件按理论深度完美逆序排列。跨模型相关特征未复现（平均r=0.005）。

Conclusion: 结果支持更简单的机制：任何迫使模型偏离默认生成路径的约束都充当输出正则化器，通过破坏流畅但肤浅的响应模式来改善推理。最浅层的约束效果最好，因为它们施加监控负荷而最小化概念干扰。这些发现是通过证伪进行科学发现的典型案例。
...
TL;DR: ChomskyBench：首个基于乔姆斯基层级理论系统评估LLM形式推理能力的基准，揭示LLM性能与计算复杂度层级的相关性及严重效率瓶颈


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准缺乏基于计算理论和复杂度的系统评估，无法确定SOTA LLM是否能理解形式语言的结构化层次复杂度。需要填补这一关键空白，系统评估LLM的形式推理能力。

Method: 提出ChomskyBench基准，结合完整乔姆斯基层级覆盖、通过自然语言的过程追踪评估和确定性符号可验证性。包含全面的语言识别和生成任务，测试每个复杂度层级的能力。

Result: 实验显示性能分层与复杂度层级相关；任务难度增加显著影响推理长度和性能；大模型和先进推理方法虽有相对提升，但面临严重效率障碍；LLM在形式任务上效率远低于传统算法程序。

Conclusion: 当前LLM存在实际限制，传统软件工具不可或缺；结果为指导开发具有更强大形式推理能力的未来LLM提供见解；局限性源于效率问题而非绝对能力界限。
...
<details>
  <summary>Details</summary>
Motivation: 多语言预训练语言模型(MPLMs)已成为自然语言处理的重要工具，但它们经常表现出与性别、种族和宗教等敏感属性相关的偏见。需要一种全面的多语言去偏方法来解决跨语言的偏见问题。

Method: 提出了Multiple-Debias方法，结合了多语言反事实数据增强和多语言自去偏技术，在预处理和后处理阶段都进行去偏操作，同时采用参数高效微调策略。还将CrowS-Pairs数据集扩展到德语、西班牙语、中文和日语。

Result: 实验表明：(1)多语言去偏方法在有效缓解偏见方面优于单语言方法；(2)整合不同语言的去偏信息显著提高了MPLMs的公平性。在四种语言的三种敏感属性偏见上都取得了显著减少。

Conclusion: 提出的Multiple-Debias方法通过全流程多语言去偏策略，有效减少了多语言预训练语言模型中的偏见问题，证明了跨语言去偏信息整合对提高模型公平性的重要性。
...
TL;DR: 该论文提出了持续多模态知识图谱推理（CMMKGR）问题，并提出了MRCKG模型来解决现有方法在动态多模态知识图谱中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多模态知识图谱是动态演化的，现有方法存在局限性：持续知识图谱推理方法只关注结构化三元组而无法充分利用多模态信号；多模态知识图谱推理方法假设静态图谱，在图谱演化时会出现灾难性遗忘。

Method: 提出了MRCKG模型，包含三个核心组件：1）多模态-结构协同课程学习，基于新三元组与历史图谱的结构连接性和多模态兼容性进行渐进学习调度；2）跨模态知识保持机制，通过实体表示稳定性、关系语义一致性和模态锚定来缓解遗忘；3）多模态对比重放方案，采用两阶段优化策略，通过多模态重要性采样和表示对齐来强化已学知识。

Result: 在多个数据集上的实验表明，MRCKG能够有效保持先前学习的多模态知识，同时显著提高新知识的学习效果。

Conclusion: 该研究首次系统性地探讨了持续多模态知识图谱推理问题，提出的MRCKG模型通过协同课程学习、跨模态知识保持和多模态对比重放等机制，有效解决了动态多模态知识图谱中的灾难性遗忘问题，为这一新兴领域提供了基准和方法论基础。
...
TL;DR: RTT框架通过引入令牌级相关性判别器解决基于规则的强化学习中奖励稀疏和模糊问题，将粗粒度响应级评分与细粒度令牌级信用分配相结合，显著提升语言模型在复杂指令跟随任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的强化学习方法主要依赖响应级奖励，导致严重的奖励稀疏性和奖励模糊性问题。这些方法难以精确评估语言模型响应中哪些具体令牌对满足特定约束条件负责，限制了模型在复杂开放域指令跟随任务中的对齐效果。

Method: 提出RTT框架，包含令牌级相关性判别器预测响应中哪些令牌对特定约束负责；开发RTT-GRPO算法，在统一框架中整合响应级和令牌级优势；针对令牌级规则强化学习中的三维奖励空间，提出样本内令牌组归一化方法。

Result: 大量实验和基准测试表明，RTT在不同模型上均优于其他基线方法，在指令级和规则级准确性方面均取得一致性的性能提升。

Conclusion: RTT框架通过桥接响应级评分和令牌级信用分配，有效解决了基于规则的强化学习中的奖励稀疏和模糊问题，为语言模型与复杂指令的对齐提供了更精细的优化方法。
...
TL;DR: Gen-SSD是一种学生参与生成过程的知识蒸馏框架，通过在教师模型生成推理轨迹时让学生实时评估候选延续，选择可学习的推理路径，从而提升小模型的学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法通常采用后处理过滤方式选择教师生成的推理轨迹，但这种方法无法控制生成过程本身，且可能产生超出学生学习能力的推理路径。需要一种能够在生成过程中就进行选择的方法。

Method: 提出Gen-SSD（生成时自选择蒸馏）框架，在教师模型采样过程中让学生模型实时评估候选延续，仅扩展可学习的推理路径，并早期剪除无益分支。这是一种学生参与生成过程的蒸馏方法。

Result: 在数学推理基准测试中，Gen-SSD始终优于标准知识蒸馏和近期基线方法，相比标准KD提升约5.9个百分点，相比其他基线提升高达4.7个百分点。分析表明Gen-SSD产生更稳定且可学习的推理轨迹。

Conclusion: Gen-SSD通过在生成过程中融入监督，实现了更有效的知识蒸馏，证明了在生成时进行选择的重要性。该方法为将大型模型的复杂推理能力转移到小模型提供了新思路。
...
TL;DR: GRADE：基于梯度动态的LLM知识缺口检测方法，通过跨层梯度与隐藏状态子空间的秩比来量化知识缺口，优于基于激活知识的传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖LLM自报告的置信度或响应token的隐藏状态来评估知识激活程度，但这些方法可能捕获与查询无关的特征（如风格、长度等），无法准确反映模型内部知识是否足以正确回答问题。

Method: 提出GRADE方法，通过计算梯度与对应隐藏状态子空间的跨层秩比来量化知识缺口。该方法基于梯度作为所需知识更新的估计器这一特性，能够更准确地评估模型对特定查询的知识完备性。

Result: 在六个基准测试上验证了GRADE的有效性和鲁棒性，表明其对输入扰动具有稳健性。此外，通过案例研究展示了梯度链如何为长格式答案生成可解释的知识缺口解释。

Conclusion: GRADE提供了一种基于梯度动态的LLM知识缺口检测新方法，能够更准确地评估模型内部知识是否足以回答问题，为负责任LLM部署提供了重要工具。
...
TL;DR: 多语言机器翻译中权重空间模型合并失败的系统研究：发现微调会重新分配而非增强语言选择性，导致表示分歧增加，与标准合并假设不兼容


<details>
  <summary>Details</summary>
Motivation: 权重空间模型合并是一种无需原始训练数据的实用方法，在多任务设置中成功，但在多语言上下文中的行为尚不清楚。本文旨在系统研究多语言机器翻译中的权重空间合并，解释为何合并会失败

Method: 1) 在大规模双语语料库上完全微调语言模型；2) 评估标准合并策略；3) 使用跨度条件神经元选择性和层间中心核对齐分析内部表示；4) 分析语言特定神经元的分布和微调对选择性的影响

Result: 实验显示合并会降低性能，特别是当目标语言不同时。语言特定神经元集中在嵌入层和上层Transformer块，中间层主要共享。微调重新分配而非增强语言选择性：监督和相关语言的神经元变得不那么排他，而无监督语言的神经元更加孤立。这种重新分配增加了控制生成的高层中的表示分歧

Conclusion: 多语言微调可能以降低与标准权重空间合并假设兼容性的方式重塑几何结构，这解释了为何合并在多语言翻译场景中失败。研究为理解多语言模型合并的局限性提供了机制性解释
...
<details>
  <summary>Details</summary>
Motivation: 针对采用MoE架构的LLM普遍存在的幻觉问题（生成看似合理但事实错误的内容）以及由推理过程中专家激活不均衡导致的系统性偏差放大问题，需要一种有效的解决方案来提升模型的事实准确性和减少偏差

Method: 提出Council Mode多智能体共识框架，包含三个阶段：1) 基于复杂度的智能分流分类器路由查询；2) 跨架构多样化模型的并行专家生成；3) 结构化共识合成，明确识别一致性、分歧和独特发现后再生成最终响应。提供了共识机制的数学公式化描述和系统架构细节

Result: 在HaluEval基准上实现35.9%的相对幻觉率降低，在TruthfulQA上相比最佳单模型提升7.8个百分点，同时在多个领域保持显著更低的偏差方差。提供了包含消融研究的广泛实证结果

Conclusion: Council Mode框架通过多智能体共识机制有效解决了LLM的幻觉和偏差问题，在开源AI工作空间中实现并验证了其有效性，为提升LLM的事实准确性和减少系统性偏差提供了有前景的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决俄语形态标注问题，特别是支持开放词典以分析训练数据中未出现的词汇，同时考虑词素特征（前缀、后缀等）。传统方法难以处理未见词汇，且需要大量预训练或计算资源。

Method: 基于多头注意力机制的新架构，将单词分割为子词（subtoken），通过训练过程聚合子词向量为完整词向量。该方法不使用RNN，无需BERT式大规模无标注文本预训练，支持开放词典分析。

Result: 在SinTagRus和Taiga数据集上，对某些语法类别达到98-99%以上的准确率，优于先前已知结果。对90%的单词能精确预测所有语法类别，并能识别不应分析的类别。模型可在消费级GPU上训练，处理速度更快。

Conclusion: 提出的多头注意力架构在俄语形态标注任务中表现出色，支持开放词典，计算效率高，无需大规模预训练，为形态分析提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类数据标注常被视为客观参考，但许多标注任务本质上是主观的，且标注者的判断会随时间变化。本研究旨在从能力角度探究标注者在社会影响力识别过程中的工作质量变化。

Method: 研究涉及25名来自5个不同群体的标注者（专家与非专家），对1021个对话进行20种社会影响力技术的标注，包括意图、反应和后果。150个文本子集在主要标注过程前后各标注一次以进行比较。采用定性定量分析、半结构化访谈、自我评估调查以及LLM训练和评估来测量能力变化。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言图表示（包括句法和语义结构）在原则上能否补充和改进神经语言建模，特别是在不同形式化体系下这些图表示对语言建模性能的影响。

Method: 采用集成设置，结合预训练的Transformer模型和来自7种不同形式化体系的真实图结构，系统评估不同语言图表示对语言建模性能的贡献。

Result: 总体而言，语义成分结构对语言建模性能最有帮助，超越了句法成分结构以及句法和语义依存结构。此外，效果因词性类别而异，变化很大。

Conclusion: 研究结果表明神经符号语言建模具有有前景的趋势，并邀请未来研究量化不同形式化体系所做的设计选择，为进一步探索神经与符号方法的结合提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: GraphRAG系统虽然对传统RAG攻击（如文本投毒和提示注入）具有抵抗力，但其安全性依赖于底层知识图谱的拓扑完整性。研究发现可以通过破坏逻辑连接而不改变表层文本语义来攻击GraphRAG系统

Method: 提出LogicPoison攻击框架，采用保持类型一致性的实体交换机制，同时扰动全局逻辑枢纽以破坏整体图连通性，以及查询特定的推理桥梁以切断关键的多跳推理路径

Result: 在多个基准测试上的综合实验表明，LogicPoison成功绕过GraphRAG的防御机制，显著降低系统性能，在有效性和隐蔽性方面均优于现有基线方法

Conclusion: GraphRAG系统的安全性不仅依赖于文本内容的真实性，更关键的是底层知识图谱的逻辑拓扑完整性。LogicPoison攻击框架揭示了通过逻辑扰动破坏推理路径的新攻击面
...
TL;DR: NeuReasoner：一个基于神经元分析的可解释、可控统一推理框架，通过检测推理失败模式并触发自校正机制，在提升性能的同时减少计算开销


<details>
  <summary>Details</summary>
Motivation: 大型推理模型存在三个层面的失败模式：步骤内计算错误、步骤间振荡停滞、实例级过度思考。现有方法针对孤立层面，且黑盒性质和RL依赖限制了可解释性和可控性。需要统一框架来解决这些问题。

Method: 进行白盒分析识别关键神经元（神经元混合体，MoN）及其波动模式；提出NeuReasoner框架，集成轻量级MLP进行失败检测，并通过SFT学习特殊令牌触发的自校正机制；推理时检测到失败即插入特殊令牌触发可控补救行为。

Result: 在六个基准测试、六个骨干模型（8B~70B）上对比九个基线方法，NeuReasoner实现高达27.0%的性能提升，同时减少19.6%~63.3%的令牌消耗。

Conclusion: NeuReasoner通过神经元层面的可解释分析和可控的自校正机制，统一解决了推理模型的多个失败模式，在提升性能的同时显著降低计算成本，为推理模型的可控性和可解释性提供了新方向。
...
TL;DR: R2-Write框架通过迭代式作者-评审交互合成富含反思和修订模式的高质量思维轨迹，解决现有推理模型在开放式写作任务中效果有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有主流推理模型在可验证领域（如数学）通过长链思维取得了显著改进，但在开放式写作任务中效果有限。这些模型缺乏开放式写作所需的深度反思和修订模式，导致改进幅度远小于数学推理任务。

Method: 提出R2-Write框架：1）通过迭代式作者-评审交互合成富含显式反思和修订模式的高质量思维轨迹；2）设计过程奖励机制，在强化学习中监督反思质量，防止冗余反思，提高性能和token效率。

Result: 在多个创意写作和深度研究基准测试中进行了广泛实验，证明了显著改进，验证了显式纳入反思和修订模式能够解锁开放式写作任务的深度推理能力。

Conclusion: R2-Write框架通过显式整合反思和修订模式，有效解决了现有推理模型在开放式写作任务中的局限性，为开放式写作任务解锁了深度推理能力。
...
TL;DR: JoyAI-LLM Flash是一个高效的混合专家语言模型，在50B参数以下实现了性能与token效率的平衡，通过创新的认知模式平衡、FiberPO算法和稀疏架构设计，在48B总参数下仅激活2.7B参数。


<details>
  <summary>Details</summary>
Motivation: 解决在50B参数以下语言模型中性能与token效率之间的权衡问题，通过提高模型稀疏性和推理效率来降低计算成本，同时保持强大的性能表现。

Method: 采用混合专家架构，在20万亿token上进行预训练，通过监督微调、直接偏好优化和大规模强化学习进行后训练优化。引入思维与非思维认知模式平衡策略，提出基于纤维理论的FiberPO强化学习算法，采用密集多token预测和量化感知训练的联合训练-推理协同设计。

Result: 开发出48B总参数的模型，每次前向传播仅激活2.7B参数，稀疏比显著高于同规模行业领先模型。模型在Hugging Face上开源了JoyAI-LLM-48B-A3B Base及其后训练变体的检查点。

Conclusion: JoyAI-LLM Flash成功重新定义了50B参数以下语言模型的性能与token效率权衡，通过创新的架构设计、训练算法和推理优化，实现了高稀疏性和高效率，为开源社区提供了有价值的资源。
...
TL;DR: 提出一种开源方法，让用户通过自然语言查询结构化非文本数据集，训练LLM生成可执行查询，替代RAG在处理数值和结构化信息时的不足。


<details>
  <summary>Details</summary>
Motivation: 传统RAG方法在处理数值和高度结构化信息时效果不佳，需要一种能够理解用户意图并生成准确查询的方法来访问结构化数据集。

Method: 1) 提出原则性的合成训练数据生成流程，生成多样化的问答对；2) 使用QLoRA和4位量化对DeepSeek R1 Distill 8B进行微调；3) 在西班牙Durangaldea地区基本服务可达性数据集上评估。

Result: 微调模型在单语言、多语言和未见位置场景下均实现高准确率，展示了强大的泛化能力和可靠的查询生成能力。

Conclusion: 小型领域特定模型可以在不依赖大型专有LLM的情况下实现高精度，适合资源受限环境，并可适应更广泛的多数据集系统。
...
TL;DR: 论文提出Verbalized Assumptions框架，用于揭示LLM产生社会奉承行为（如过度肯定用户而非提供客观评估）的潜在错误假设，并通过假设探针实现可解释的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: LLM存在社会奉承行为，倾向于肯定用户而非提供真实评估。作者假设这种行为源于LLM对用户的错误假设，如低估用户寻求信息而非安慰的频率。需要理解这些假设以解决奉承、妄想等安全问题。

Method: 提出Verbalized Assumptions框架，从LLM内部提取关于用户意图的假设。开发假设探针（基于这些假设内部表示的线性探针），用于可解释地控制社会奉承行为。分析人类对AI与人类回答的期望差异。

Result: 在社交奉承数据集中，LLM假设中最常见的二元词组是"寻求验证"。假设探针能够因果性地控制奉承行为，实现细粒度调节。研究发现人类期望从AI获得比从人类更客观、信息丰富的回答，但基于人-人对话训练的LLM未能考虑这种期望差异。

Conclusion: 假设是LLM奉承行为的重要机制。Verbalized Assumptions框架为理解LLM奉承、妄想等安全问题提供了新视角，假设探针为实现可解释的模型行为控制提供了工具。研究揭示了训练数据与用户期望不匹配的问题。
...
TL;DR: MaKD方法通过多角度知识蒸馏，深入模仿自注意力和前馈模块，在不同层面捕获丰富的语言知识信息，在相同参数预算下实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法仅关注层间知识分布，在知识对齐过程中可能导致细粒度信息丢失，需要更深入的知识提取方法。

Method: 提出多角度知识蒸馏(MaKD)方法，深入模仿自注意力和前馈模块，从不同角度捕获丰富的语言知识信息。

Result: 实验结果表明，在相同存储参数预算下，MaKD能够实现与各种强基线相当的竞争性性能，并且在蒸馏自回归架构模型方面也表现良好。

Conclusion: MaKD通过多角度知识蒸馏有效解决了细粒度信息丢失问题，在预训练语言模型压缩方面表现出色，特别适用于自回归架构模型。
...
TL;DR: 提出领域适应的RAG管道用于教学对话标注，通过微调轻量级嵌入模型和话语级索引检索标注示例，显著提升标注性能


<details>
  <summary>Details</summary>
Motivation: 教学对话自动标注是高风险任务，LLMs缺乏领域知识时表现不佳，需要领域适应的解决方案

Method: 不微调生成模型，而是微调轻量级嵌入模型进行领域适应，在话语级别索引对话以检索标注示例，构建RAG管道

Result: 在两个真实教学对话数据集上，最佳配置的Cohen's κ达到0.526-0.580（TalkMoves）和0.659-0.743（Eedi），显著优于无检索基线（κ=0.275-0.413和0.160-0.410）

Conclusion: 仅适应检索组件是实用有效的路径，能实现专家级教学对话标注，同时保持生成模型冻结；话语级索引而非嵌入质量是性能提升的主要驱动力
...
TL;DR: StoryScope通过分析叙事层面的10个维度特征，能够有效区分AI与人类创作的故事，准确率达93.2%，发现AI故事倾向于主题过度解释、情节单一，而人类故事更具道德模糊性和时间复杂性。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成小说的普及，作者身份和原创性问题变得日益重要。现有研究主要关注AI写作的表面风格特征，而本研究旨在探索是否可以不依赖风格信号，仅通过话语层面的叙事选择（如角色能动性、时间不连续性）来区分AI与人类创作的故事。

Method: 提出StoryScope流水线，自动从10个维度诱导细粒度、可解释的话语层面叙事特征空间。应用于包含10,272个写作提示的平行语料库，每个提示由人类作者和5个LLM分别创作，共61,608个故事（每个约5,000词），每个故事提取304个特征。

Result: 仅使用叙事特征即可实现93.2%的宏F1分数用于人机检测，68.4%的宏F1分数用于六方作者归属，性能达到包含风格线索模型的97%以上。30个核心叙事特征捕获了主要信号：AI故事过度解释主题、偏好整洁的单线情节，而人类故事将主角选择呈现为更道德模糊、时间复杂性更高。各模型有独特指纹特征：Claude事件升级平缓，GPT过度使用梦境序列，Gemini默认外部角色描述。AI生成故事在叙事空间中聚类，而人类创作故事更具多样性。

Conclusion: 研究结果表明，底层叙事结构（不仅仅是写作风格）的差异可用于区分人类原创作品与AI生成小说。AI故事在叙事选择上表现出系统性模式，而人类创作展现出更大的叙事多样性，这为作者身份识别和原创性评估提供了新的分析维度。
...
TL;DR: 提出一个同时评估长文本生成事实性的精度和召回率的综合框架，发现当前LLMs在精度上表现远优于召回率，事实不完整性是主要限制。


<details>
  <summary>Details</summary>
Motivation: 现有的事实性评估方法主要关注精度（将回答分解为原子声明并验证），但忽略了同等重要的召回维度——生成的回答是否涵盖了应该包含的相关事实。需要全面评估长文本生成的事实性。

Method: 提出综合事实性评估框架，同时测量精度和召回率。利用外部知识源构建参考事实，确定它们是否在生成文本中被捕获。引入基于相关性和显著性的重要性感知加权方案。

Result: 当前LLMs在精度上的表现显著优于召回率，表明事实不完整性仍然是长文本生成的主要限制。模型通常能更好地覆盖高度重要的事实，而不是所有相关事实。

Conclusion: 需要同时考虑精度和召回率来全面评估LLMs的事实性。事实不完整性是当前长文本生成的关键限制，模型在覆盖所有相关事实方面仍有不足。
...
TL;DR: 该研究提出了一种在大型语言模型表示中识别效价-唤醒度（VA）子空间的方法，通过情感引导向量和PCA组件学习VA轴，实现了对模型情感维度的线性控制和行为调控。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解大型语言模型内部的情感表示结构，并建立可解释的情感控制机制。现有研究缺乏对模型情感维度的系统性分析，特别是效价（正负情感）和唤醒度（情感强度）这两个核心维度的表征与控制。

Method: 方法包括：1）从21.1万条情感标注文本中提取情感引导向量；2）使用PCA降维提取主要成分；3）通过岭回归学习VA轴作为PCA组件的线性组合；4）在44k词汇项上验证与人类众包VA评分的相关性；5）在Llama-3.1-8B、Qwen3-8B和Qwen3-14B模型上测试跨架构通用性。

Result: 结果显示：1）恢复的VA子空间呈现圆形几何结构，与人类情感感知模型一致；2）VA投影与人类VA评分显著相关；3）沿VA轴引导生成能单调调节输出的情感维度；4）唤醒度增加会降低拒绝行为、增加谄媚行为，反之亦然；5）拒绝相关词汇（"I can't"、"sorry"）位于低唤醒度、负效价区域。

Conclusion: 该研究成功识别了LLM中的VA子空间，实现了对模型情感维度的线性控制，并揭示了情感引导影响拒绝和谄媚行为的机制：拒绝相关词汇的发射概率直接受VA调控。该方法具有跨架构通用性，为理解模型情感表示提供了可解释的框架。
...
TL;DR: 该论文对LLM增强策略进行了系统性调查，提出了基于结构化上下文程度的统一分析框架，涵盖了从提示工程到因果增强的多种方法，并提供了文献筛选、证据合成和部署决策框架。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型编码了丰富的世界知识，但仍受限于静态知识、有限上下文窗口和弱结构化因果推理。需要系统性地调查和分析各种增强策略，为可信赖的检索增强NLP提供指导。

Method: 1. 提出基于推理时结构化上下文程度的统一分析框架；2. 涵盖提示工程、RAG、GraphRAG、CausalRAG等增强策略；3. 建立透明的文献筛选协议；4. 开发声明审计框架；5. 进行结构化跨论文证据合成；6. 区分高置信度发现与新兴结果。

Result: 1. 提供了LLM增强策略的系统性分类和分析框架；2. 建立了文献评估和证据合成的标准化方法；3. 区分了不同增强方法的技术成熟度和证据强度；4. 为研究者和实践者提供了清晰的决策指南。

Conclusion: 论文提出了部署导向的决策框架和具体研究优先级，旨在推动可信赖的检索增强NLP发展。强调需要系统化评估增强策略，平衡技术能力与可信度要求。
...
TL;DR: EWAD和CPDP方法用于低资源抽象摘要的多教师知识蒸馏，通过教师间一致性路由监督，在孟加拉语数据集上验证，发现logit级蒸馏最可靠，跨语言伪标签蒸馏在3.2倍压缩下保留71-122%教师性能


<details>
  <summary>Details</summary>
Motivation: 从可靠性感知的角度研究低资源抽象摘要的多教师知识蒸馏，旨在确定多教师监督何时能改进摘要质量，以及何时数据扩展比损失工程更有效

Method: 提出EWAD（熵加权一致性感知蒸馏）和CPDP（容量比例散度保持）两种方法。EWAD基于教师间一致性在教师蒸馏和黄金监督之间路由监督；CPDP对学生相对于异构教师的位置施加几何约束。在2个孟加拉语数据集、13个BanglaT5变体和8个Qwen2.5实验中进行评估

Result: logit级知识蒸馏提供最可靠的增益；更复杂的蒸馏改进短摘要的语义相似性但降低长输出质量；跨语言伪标签知识蒸馏在3.2倍压缩下保留71-122%的教师ROUGE-L分数；人类验证的多法官LLM评估揭示单法官流程中的校准偏差

Conclusion: 可靠性感知蒸馏有助于表征多教师监督何时改进摘要质量，以及何时数据扩展比损失工程更有效。研究为低资源语言摘要的模型压缩和知识转移提供了实用指导
...
TL;DR: 提出了首个可迁移的学习型成员推理攻击LT-MIA，通过利用微调过程中构造的无限标记数据训练分类器，实现了跨架构家族和数据域的零样本迁移，在Transformer、Mamba、RWKV-4和RecurrentGemma等不同架构上均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有成员推理攻击依赖于手工设计的启发式方法（如损失阈值、Min-K%、参考校准），这些方法受限于设计者的直觉。需要一种能够学习而非手工设计攻击策略的方法，突破影子模型的瓶颈，实现更通用和有效的成员推理。

Method: 提出LT-MIA方法：1）利用微调过程构造无限标记数据（成员状态已知）；2）训练基于Transformer的成员推理分类器；3）将成员推理重构为基于每词元分布统计量的序列分类任务；4）实现跨架构家族（Mamba、RWKV-4、RecurrentGemma）和数据域（自然语言到代码）的零样本迁移。

Result: 在未见过的架构和数据集上实现优异性能：Mamba（0.963 AUC）、RWKV-4（0.972 AUC）、RecurrentGemma（0.936 AUC），均超过在保留Transformer上的性能（0.908 AUC）。在Transformer上，LT-MIA在0.1% FPR下实现比最强基线高2.8倍的TPR。在代码数据上达到0.865 AUC，尽管仅在自然语言文本上训练。

Conclusion: 微调语言模型会产生跨架构家族和数据域可检测的记忆化特征签名，该特征仅依赖于梯度下降和交叉熵损失优化。LT-MIA通过将成员推理重构为序列分类任务，有效捕获这一信号，实现了首个可迁移的学习型成员推理攻击，为隐私保护研究提供了新视角。
...
TL;DR: 该论文提出了行为对齐分数（BAS），一种基于决策理论的度量标准，用于评估LLM置信度在包含弃权选项的决策场景中的可靠性，并建立了相应的基准测试。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）经常在应该弃权的情况下产生自信但错误的答案。现有的评估协议要求模型必须给出回答，且未考虑不同风险偏好下置信度应如何指导决策。需要一种能够评估LLM置信度在支持弃权决策方面可靠性的度量标准。

Method: 引入行为对齐分数（BAS），这是一种基于显式的"回答或弃权"效用模型的决策理论度量标准。BAS通过聚合连续风险阈值下的实际效用，产生一个依赖于置信度大小和排序的决策级可靠性度量。理论证明真实的置信度估计能唯一最大化期望BAS效用。将BAS与ECE、AURC等标准指标结合，构建了跨多个LLM和任务的置信度可靠性基准。

Result: 研究结果显示：1）不同LLM在决策有用的置信度方面存在显著差异；2）更大、更准确的模型通常获得更高的BAS，但即使是前沿模型仍存在严重过度自信问题；3）具有相似ECE或AURC的模型可能表现出非常不同的BAS，突显了标准指标的局限性；4）简单的干预措施（如top-k置信度获取和后校准）能显著改善置信度可靠性。

Conclusion: 该工作提供了一个原则性的度量标准（BAS）和全面的基准测试，用于评估LLM置信度可靠性。BAS通过不对称惩罚机制（强烈优先避免过度自信错误）区别于对称惩罚的log loss等标准评分规则，能更好地反映实际决策需求。研究强调了评估置信度可靠性的重要性，并为改进LLM在安全关键应用中的决策能力提供了方向。
...
### [132] [Xpertbench: Expert Level Tasks with Rubrics-Based Evaluation](https://arxiv.org/abs/2604.02368)
...
TL;DR: XpertBench是一个用于评估大语言模型在真实专业领域能力的高保真基准，包含80个类别、1346个任务，通过专家校准的ShotJudge评估范式发现当前最先进模型在专业任务上存在显著性能天花板（最高成功率约66%）。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在传统基准测试上性能趋于平稳，但缺乏评估其在复杂、开放式专业任务中真正专家级认知能力的方法。现有评估框架存在领域覆盖窄、依赖通用任务或自我评估偏差等问题。

Method: 开发XpertBench基准，包含1346个精心策划的任务，涵盖金融、医疗、法律、教育及双轨研究（STEM和人文）等80个类别，任务来自1000多名领域专家的提交。引入ShotJudge评估范式，使用经过专家少量样本校准的LLM评估者来减轻自我奖励偏差。

Result: 对最先进LLM的实证评估显示显著性能天花板：即使领先模型也仅达到约66%的最高成功率，平均得分约55%。模型还表现出领域特异性差异，在定量推理与语言合成方面显示出非重叠优势。

Conclusion: 研究结果强调了当前AI系统存在的显著"专家差距"，并确立了XpertBench作为从通用助手向专业协作伙伴过渡的关键评估工具。
...
### [133] [Compositional Neuro-Symbolic Reasoning](https://arxiv.org/abs/2604.02434)
...
<details>
  <summary>Details</summary>
Motivation: 纯神经架构缺乏可靠组合泛化能力，而纯符号系统难以处理感知接地问题。为解决ARC任务中结构化抽象推理的挑战，需要结合神经和符号方法的优势。

Method: 提出神经符号架构：1) 从网格中提取对象级结构；2) 使用神经先验从固定领域特定语言(DSL)中提出原子模式的候选变换；3) 通过跨示例一致性过滤假设。系统基于人类视觉抽象启发的单元模式构建组合推理框架，用对象表示和变换建议增强大语言模型。

Result: 在ARC-AGI-2上，将基础LLM性能从16%提升到24.4%（公开评估集），与ARC Lang Solver结合后达到30.8%。系统无需任务特定微调或强化学习，减少了暴力搜索和基于采样的测试时扩展的依赖。

Conclusion: 分离感知、神经引导的变换建议和符号一致性过滤能改善泛化能力，证明了神经符号方法在结构化抽象推理任务中的有效性。系统代码已开源。
...
TL;DR: 提出一个四部分框架，用于电力公司在极端天气不确定性下的多目标资本投资优化，发现简单的净现值排名方法比复杂的基于模型的元启发式优化方法更有效。
...
TL;DR: 提出一种基于可解释强化学习的新方法，用于基于元素级状态表示的桥梁最优生命周期策略制定，通过可微分软树模型、温度退火和正则化等技术生成可直接理解和实施的决策树策略。
...
Method: 提出一种新的可解释强化学习方法，包含三个主要改进：(a) 使用可微分软树模型作为行动者函数逼近器，(b) 训练过程中的温度退火过程，(c) 正则化配合剪枝规则以限制策略复杂度，最终生成确定性斜决策树形式的可解释生命周期策略。

Result: 该方法在监督学习和强化学习设置中展示了各项技术的优势和权衡，并在钢梁桥生命周期优化问题中进行了框架验证，能够生成节点数量和深度合理的可解释决策树策略。

Conclusion: 提出的可解释强化学习方法能够基于元素级状态表示生成可直接理解和审计的最优生命周期策略，易于集成到现有桥梁管理系统中，解决了新规范带来的状态空间扩展挑战。
...
TL;DR: 提出基于知识图谱的神经符号架构，通过"计划-检索-生成"工作流程解决LLM在非物质文化遗产叙事生成中的幻觉问题，确保事实准确性


<details>
  <summary>Details</summary>
Motivation: 非物质文化遗产保护面临集体记忆消逝的挑战，而大型语言模型虽然能生成吸引人的叙事，但其事实不准确或"幻觉"问题使其在要求真实性的遗产应用中不可靠

Method: 提出基于知识图谱的神经符号架构，将传统设计时验证工具的能力问题重新用作运行时可执行的叙事计划，建立"计划-检索-生成"透明工作流程，并比较三种RAG策略：纯符号KG-RAG、文本增强Hybrid-RAG和结构感知Graph-RAG
...
TL;DR: 论文研究了LLMs在教师教学质量评估中对虚假社会背景信息的敏感性，提出了Debiasing-DPO方法，在减少偏见84%的同时提高预测准确性52%。
...
Method: 使用美国课堂转录本数据集(NCTE)评估了7个前沿和开源模型对7类虚假背景的敏感性。提出了Debiasing-DPO方法：通过自监督训练，将仅基于查询生成的中立推理与基于查询加虚假背景生成的偏见推理配对，并结合监督微调以防止预测准确性损失。

Result: 虚假上下文信息可使模型预测在7分量表上偏移高达1.48分，较大模型有时表现出更高敏感性。标准缓解方法效果有限。Debiasing-DPO应用于Llama和Qwen模型后，平均减少偏见84%，提高预测准确性52%。

Conclusion: 对虚假背景的鲁棒性不是模型规模扩展的自然副产品。Debiasing-DPO方法在基于提示的预测任务中能显著提高准确性和鲁棒性，为高风险决策中的LLM部署提供了重要改进方案。
...
TL;DR: 该研究首次对音频-视觉大语言模型进行机制可解释性分析，发现当音频与视觉信息冲突时，模型在最终文本生成中会优先视觉信息而抑制音频语义，这种不平衡源于训练过程中音频监督的有限对齐。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉大语言模型作为多模态感知的统一接口正在兴起，但对其工作机制的理解尚不充分。研究者旨在通过机制可解释性方法分析AVLLM如何处理音频和视觉特征，以及这些特征如何在不同层融合以产生最终文本输出。

Method: 采用机制可解释性研究方法，分析AVLLM不同层中音频和视觉特征的演化与融合过程。通过探测分析检查潜在音频信息的存在情况，并追溯训练过程以理解模态不平衡的根源。

Result: 研究发现：1）AVLLM在中间层编码了丰富的音频语义，但当音频与视觉冲突时，这些能力在最终文本生成中未能充分体现；2）有用的潜在音频信息确实存在，但深层融合层不成比例地偏向视觉表示，抑制了音频线索；3）这种不平衡源于训练过程，AVLLM的音频行为与其视觉-语言基础模型高度匹配，表明对音频监督的额外对齐有限。

Conclusion: 该研究揭示了AVLLM中存在根本性的模态偏见，为理解多模态大语言模型如何整合音频和视觉提供了新的机制性见解，指出了当前模型在跨模态融合方面的局限性。
...
TL;DR: 提出了一种本体导向的方法，通过内在-关系路由机制构建可移植、可重用的知识图谱模式，并在Wikidata上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将知识图谱的结构决策嵌入到流水线代码中或以临时方式提取关系，产生的模式与构建过程紧密耦合，难以在下游本体级任务中重用。需要一种从一开始就为本体分析、实体消歧、领域定制和LLM引导提取而设计的模式。

Method: 提出本体导向方法，核心是内在-关系路由机制，将每个属性分类为内在属性或关系属性，并路由到相应的模式模块。该方法包括基于规则的清洗阶段和迭代路由过程，结合工具增强的LLM支持和人工审查。

Result: 在2026年1月Wikidata转储上实例化该方法：从完整转储中识别出3460万个实体的核心集，内在-关系路由将属性分配到8个类别下的94个模块中。模式达到93.3%的类别覆盖率和98.0%的模块分配率。导出模式产生包含3400万个节点和6120万条边的属性图，涵盖38种关系类型。通过五个独立于构建流水线的应用验证了本体导向主张。

Conclusion: 本体导向方法通过内在-关系路由机制产生可移植、可重用的声明性模式，解决了现有方法中模式与构建过程紧密耦合的问题，支持多种下游本体级任务，为知识图谱的组织和利用提供了更灵活、可扩展的框架。
...
<details>
  <summary>Details</summary>
Motivation: 竞争性编程是AI尚未超越人类的少数领域之一。尽管Google的Gemini~3 Deep Think取得了进展，但在实时竞赛条件下仍无法击败最佳人类选手。本研究旨在开发一个能够在实时编程竞赛中超越所有人类选手的AI系统。

Method: GrandCode采用多智能体强化学习系统，包含两个关键创新：1）协调多种智能体模块（假设提出、求解器、测试生成器、总结等），通过后训练和在线测试时RL联合改进；2）引入专门设计的Agentic GRPO算法，处理多阶段智能体展开中的延迟奖励和严重的离策略漂移问题。

Result: GrandCode在最近的三个Codeforces实时竞赛中（第1087轮、1088轮、1089轮）均获得第一名，击败了所有人类参与者，包括传奇大师级选手。这是首个在实时编程竞赛中持续击败所有人类选手的AI系统。

Conclusion: GrandCode证明AI系统已经达到了在最具竞争力的编程任务上超越最强人类程序员的水平。该系统通过创新的多智能体RL架构和专门设计的Agentic GRPO算法，成功解决了竞争性编程中的复杂挑战。
...
TL;DR: DeltaLogic是一个基准测试转换协议，将自然语言推理示例转化为短修订片段，用于评估模型在最小证据变化下的信念修订能力。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准主要评估模型从固定前提集推导正确答案的能力，但低估了在动态环境中至关重要的信念修订能力。需要测量模型在最小证据变化下是否能够恰当地修订信念。

Method: 引入DeltaLogic协议，将自然语言推理示例转化为修订片段：首先在前提P下询问初始结论，然后应用最小编辑δ(P)，最后询问先前结论是否应保持不变或需要修订。从FOLIO和ProofWriter实例化DeltaLogic，并使用受限标签评分评估小型因果语言模型。

Result: 在30个片段的Qwen评估子集上，更强的初始推理能力并不保证更强的修订行为：Qwen3-1.7B初始准确率0.667但修订准确率仅0.467，在标签应改变的片段上惯性达0.600；Qwen3-4B保持相似的惯性失败模式；Phi-4-mini-instruct表现更强但仍有显著弃权和控制不稳定问题。

Conclusion: 固定前提下的逻辑能力并不意味着在局部证据编辑后能够进行规范的信念修订。DeltaLogic针对一个独特且实际重要的推理能力，补充了现有的逻辑推理和信念更新基准。
...
TL;DR: 提出了一种量化角色清晰度的方法，通过角色分配矩阵和角色清晰度矩阵来评估多智能体系统中角色一致性，并作为正则化项进行轻量级微调，显著降低了角色越界率并提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 在大语言模型驱动的多智能体系统中，角色规范不服从（即智能体未能遵守其分配角色的职责和约束，可能表现得像另一个角色）是一个主要的失败模式。现有方法缺乏对角色一致性的量化评估和有效改进机制。

Method: 首先构建角色分配矩阵S(φ)，其中元素s_ij(φ)表示第i个智能体行为轨迹与第j个智能体角色描述的语义相似度。然后定义角色清晰度矩阵M(φ)=softmax(S(φ))-I，其中softmax(S(φ))是S(φ)的行方向softmax，I是单位矩阵。M(φ)的Frobenius范数量化了智能体角色描述与行为轨迹的对齐程度。将该矩阵作为正则化项应用于轻量级微调过程，以提高角色一致性。
...
Conclusion: 提出的量化角色清晰度方法有效解决了多智能体系统中的角色规范不服从问题，通过将角色清晰度矩阵作为正则化项进行微调，显著提高了角色一致性和端到端任务性能，为多智能体系统的角色管理提供了有效的量化评估和改进框架。
...
### [143] [CharTool: Tool-Integrated Visual Reasoning for Chart Understanding](https://arxiv.org/abs/2604.02794)
*Situo Zhang,Yifan Zhang,Zichen Zhu,Da Ma,Lei Pan,Danyang Zhang,Zihan Zhao,Lu Chen,Kai Yu*

Main category: cs.AI

TL;DR: 提出DuoChart数据管道和CharTool工具增强MLLM，通过双源数据合成和工具集成提升图表推理能力
...
Method: 1) DuoChart：可扩展的双源数据管道，结合合成图表和真实图表构建多样化高质量训练数据；2) CharTool：为MLLM配备外部工具，包括图像裁剪用于局部视觉感知和基于代码的计算用于准确数值推理；3) 在DuoChart上进行智能体强化学习，使CharTool学习基于图表内容的工具集成推理

Result: 在六个图表基准测试中，方法始终优于强MLLM基线。CharTool-7B在CharXiv(推理)上比基础模型提升+8.0%，在ChartQAPro上提升+9.78%，与更大或专有模型相比具有竞争力。此外，CharTool在域外视觉数学推理基准上表现出正向泛化能力

Conclusion: 通过双源数据管道和工具集成方法，显著提升了多模态大语言模型的图表推理能力，在多个基准测试中取得优异性能，并展现出良好的泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 传统多数投票方法需要所有智能体完成推理后才能聚合，导致大量计算开销，因为一旦达成多数共识，许多响应就变得冗余。需要提高多智能体投票的效率。

Method: 提出高效多数即停（EMS）框架，包含三个核心组件：1）智能体置信度建模（ACM）-基于历史性能和语义相似度估计智能体可靠性；2）自适应增量投票（AIV）-按可靠性顺序选择智能体并实现提前终止；3）个体置信度更新（ICU）-动态更新每个参与智能体的可靠性。

Result: 在六个基准测试上的广泛评估表明，EMS能够将平均调用的智能体数量减少32%，显著提高了推理效率。

Conclusion: EMS通过将多智能体投票重新定义为可靠性感知的智能体调度问题，实现了高效的多数即停机制，在保证决策质量的同时大幅减少了计算开销。
...
<details>
  <summary>Details</summary>
Motivation: 研究前沿大语言模型在经典AI规划问题中的推理能力，特别是它们是否进行最优推理还是依赖简单启发式策略，以及如何解释其超越传统规划器的表现。

Method: 使用Blocksworld积木世界领域和形式等价的广义Path-Star图任务，系统操纵问题深度（塔高）、宽度（塔数）和组合性（目标积木数），比较LLMs与传统规划器（如LAMA）的性能。

Result: 推理增强的LLMs在复杂多目标配置中显著优于传统规划器；虽然经典搜索算法随搜索空间扩展而性能下降，但LLMs能近乎完美地跟踪理论最优性极限，即使去除领域特定语义线索后仍保持高性能。

Conclusion: LLMs通过算法模拟和几何记忆两种机制实现高效规划：算法模拟通过推理标记执行，几何记忆允许模型将P*拓扑表示为可导航的全局几何结构，有效绕过指数级组合复杂性。
...
TL;DR: 研究发现大型推理模型中存在"第一即最佳"现象，后续推理路径不仅次优反而有害，提出了基于错误森林理论的RED高效推理框架


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型在复杂推理任务中表现出色，但研究发现其多路径推理存在"第一即最佳"的意外现象，后续解决方案不仅次优反而可能有害，这挑战了现有的测试时缩放定律

Method: 提出RED自引导高效推理框架，包含两个组件：1) 精炼第一：抑制第一个解决方案中的错误森林增长；2) 丢弃后续：通过双重一致性剪枝后续错误森林。基于错误森林理论进行理论分析

Result: 在5个基准测试和6个骨干模型上的实验表明，RED优于8个竞争基线，性能提升高达19.0%，同时减少37.7%～70.4%的token消耗。错误森林指标的比较实验揭示了RED的有效性机制

Conclusion: 研究揭示了大型推理模型中"第一即最佳"现象，提出了基于错误森林理论的RED框架，既能提高性能又能显著减少计算开销，为高效推理提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有智能体搜索系统过于关注深度多步推理，忽视了大规模信息合成中需要聚合大量异构证据的挑战，导致上下文饱和、错误级联传播和高延迟等问题

Method: 基于近可分解性原则构建分层框架，包含战略Host、多个Manager和并行Worker三层，通过Manager层的聚合和反思机制实现严格上下文隔离，Worker层并行加速任务执行

Result: 在两个互补基准测试中展示高效性（3-5倍加速）和有效性：WideSearch-en上达到8.4%成功率，BrowseComp-zh上达到52.9%准确率
...
TL;DR: Agentic-MME：首个面向多模态智能体能力的流程验证基准，包含418个真实世界任务，通过2000+步骤检查点评估视觉工具与知识工具协同能力，支持沙盒代码和API，实现细粒度过程验证而非仅最终答案评估。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估存在三大局限：1）缺乏灵活工具集成；2）单独测试视觉和搜索工具而非协同；3）仅评估最终答案而无法验证工具是否被正确、高效调用。需要过程级验证基准来评估多模态智能体的真实能力。

Method: 构建Agentic-MME基准：包含418个真实世界任务，覆盖6个领域和3个难度等级。每个任务配备统一评估框架（支持沙盒代码和API）和人工标注的参考轨迹。通过2000+步骤检查点（S轴和V轴双轴标注）实现过程验证，并引入"过度思考"指标量化效率。

Result: 最佳模型Gemini3-pro总体准确率为56.3%，但在Level-3任务上显著下降至23.0%，表明真实世界多模态智能体问题解决仍具挑战性。基准成功揭示了模型在工具协同和过程效率方面的不足。

Conclusion: Agentic-MME填补了多模态智能体评估的空白，通过过程级验证和效率量化提供了更全面的能力评估。结果表明当前MLLM在复杂真实世界任务中表现有限，为未来研究提供了重要基准和方向。
...
<details>
  <summary>Details</summary>
Motivation: 推动教科书形式化的规模和技术水平，从本科拓扑学和现有库内容重构扩展到完整的研究生教材形式化，探索AI在数学形式化验证中的潜力

Method: 使用3万个Claude 4.5 Opus智能体通过版本控制并行协作，在共享代码库上工作，实现多智能体软件工程的大规模协同

Result: 成功形式化整本500多页教材，成本与人类专家团队薪资相当或更低，创造了教科书形式化规模和多智能体软件工程的记录

Conclusion: AI系统能够高效完成大规模数学教材形式化，无需更好模型仍有优化空间，为数学形式化验证开辟了新途径
...
Method: 提出Chart-RL强化学习框架，通过策略优化技术进行反馈驱动的视觉感知和逻辑推理优化，结合自适应奖励函数，并集成LoRA参数高效微调技术，仅需单GPU配置

Result: 在ChartQAPro数据集上的实验表明，RL微调的Qwen3-VL-4B-Instruct模型达到0.634的答案准确率，超越基础模型Qwen3-VL-8B-Instruct的0.580准确率，同时推理延迟从31秒降至9秒

Conclusion: Chart-RL框架通过强化学习优化显著提升了视觉语言模型在图表问答任务中的性能，在减少参数数量和计算资源需求的同时实现了更高的准确率和效率
...
### [151] [Coupled Control, Structured Memory, and Verifiable Action in Agentic AI (SCRAT -- Stochastic Control with Retrieval and Auditable Trajectories): A Comparative Perspective from Squirrel Locomotion and Scatter-Hoarding](https://arxiv.org/abs/2604.03201)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI研究通常将控制（机器人学）、记忆（检索系统）和验证（对齐与保障）分开研究，但真实世界的智能体需要同时处理这三个需求。松鼠的树栖运动、分散贮藏和观众敏感型贮藏行为提供了一个耦合所有三个需求的自然案例。

Method: 采用三层推理阶梯：经验观察（狐狸松鼠、东部灰松鼠和红松鼠的实证证据）、最小计算推理、AI设计猜想。引入一个最小层次部分可观测控制模型，包含潜在动态、结构化情景记忆、观察者信念状态、选项级动作和延迟验证信号。

Result: 提出三个假设：(H1)快速局部反馈加预测补偿可提高隐藏动态变化下的鲁棒性；(H2)为未来控制组织的记忆可改善线索冲突和负载下的延迟检索；(H3)动作-记忆循环内的验证器和观察者模型可减少静默故障和信息泄漏，但仍易受错误规范影响。下游猜想：角色分化的提议者/执行者/检查者/对抗者系统可减少不对称信息和验证负担下的相关错误。

Conclusion: 贡献在于提供了一个比较视角和基准议程：一个关于控制、记忆和可验证动作耦合的可证伪主张的严谨研究计划，为智能体AI设计提供了新的生物学启发框架。
...
### [152] [Differential Mental Disorder Detection with Psychology-Inspired Multimodal Stimuli](https://arxiv.org/abs/2604.02798)
...
TL;DR: 该研究提出了一个基于心理学启发的多模态刺激范式，用于精神障碍的鉴别诊断，构建了大规模多模态心理健康数据集(MMH)，并开发了范式感知的多模态框架，在抑郁、焦虑和精神分裂症的鉴别检测中优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 精神障碍的鉴别诊断在临床实践中具有挑战性，因为多种疾病常表现出重叠症状。现有公开数据集大多基于单一障碍设置，且数据采集范式有限，难以捕捉疾病特异性模式。

Method: 1) 设计心理学启发的多模态刺激范式，基于实验心理学发现引发多样化的情绪、认知和行为反应；2) 收集大规模多模态心理健康数据集(MMH)，涵盖抑郁、焦虑和精神分裂症，所有诊断标签均由执业精神科医生临床验证；3) 提出范式感知多模态框架，利用障碍间差异的先验知识作为提示引导的语义描述，捕捉任务特定的情感和交互上下文，用于多模态表示学习。

Result: 广泛的实验表明，该框架在鉴别精神障碍检测任务中持续优于现有基线方法，验证了心理学启发刺激设计对于鉴别精神障碍检测的价值。

Conclusion: 心理学启发的刺激设计能够有效捕捉精神障碍特异性模式，提出的范式感知多模态框架在鉴别精神障碍检测中表现出色，为临床实践中的精神障碍鉴别诊断提供了有价值的工具和方法。
...
### [153] [High Volatility and Action Bias Distinguish LLMs from Humans in Group Coordination](https://arxiv.org/abs/2604.02578)
*Sahaj Singh Maini,Robert L. Goldstone,Zoran Tiganj*

Main category: cs.MA

TL;DR: 比较人类与LLM在群体协调游戏中的表现差异：人类能适应并稳定行为，而LLM常失败且过度切换，影响群体收敛
...
Method: 使用群体二分搜索游戏（n玩家共同利益游戏，不完美监控），比较LLM与人类表现。玩家独立提交数值以达成共同目标总和，依赖群体反馈迭代调整

Result: 人类能适应并随时间稳定行为，而LLM常无法跨游戏改进，表现出过度切换，损害群体收敛。丰富反馈对人类有益但对LLM影响小

Conclusion: 通过基于人类基线和机制层面指标的分析，指出了人类与LLM群体的差异，并提供了行为基础诊断以缩小协调差距
...
### [154] [Do Agent Societies Develop Intellectual Elites? The Hidden Power Laws of Collective Cognition in LLM Multi-Agent Systems](https://arxiv.org/abs/2604.02674)
*Kavana Venkatesh,Jiaming Cui*

Main category: cs.MA

TL;DR: 该论文首次对基于LLM的多智能体系统中的协调动态进行了大规模实证研究，揭示了协调遵循重尾级联、通过优先连接集中到智力精英、以及随着系统规模增长产生更频繁极端事件的三个耦合定律，并提出了一种通过选择性增加整合来改善性能的DTI方法。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统越来越多地被部署为交互的智能体社会，但扩展这些系统通常会产生递减或不稳定的回报，其原因尚不清楚。需要理解多智能体系统中协调动态的基本规律，以改善可扩展的多智能体智能。

Method: 引入原子事件级表述，将推理重构为协调级联。分析了超过150万次跨任务、拓扑和规模的交互。提出了赤字触发整合（DTI）方法，在失衡时选择性增加整合。

Result: 揭示了三个耦合定律：协调遵循重尾级联分布；通过优先连接集中到智力精英；随着系统规模增长产生更频繁的极端事件。这些效应通过单一结构机制耦合：整合瓶颈，即协调扩展随系统规模增长而整合不增长。DTI方法在协调失败时精确改善性能，而不抑制大规模推理。

Conclusion: 研究结果建立了集体认知的定量定律，并将协调结构确定为理解和改进可扩展多智能体智能的一个基本、先前未测量的维度。DTI方法为解决整合瓶颈提供了有效途径。
...
### [155] [Multi-agent Reinforcement Learning-based Joint Design of Low-Carbon P2P Market and Bidding Strategy in Microgrids](https://arxiv.org/abs/2604.02728)
*Junhao Ren,Honglin Gao,Sijie Wang,Lan Zhao,Qiyu Kang,Aniq Ashan,Yajuan Sun,Gaoxi Xiao*

Main category: cs.MA

TL;DR: 提出基于多智能体强化学习的日内P2P交易框架，解决微电网社区中可再生能源不确定性和实时市场不稳定性问题，平衡个体经济利益与社区低碳排放目标。
...
Method: 将微电网决策过程建模为去中心化部分可观测马尔可夫决策过程，采用多智能体强化学习框架求解。引入新型市场出清机制进行宏观调节，激励微电网优先消纳本地可再生能源。

Result: 仿真结果表明，自利投标策略与P2P市场设计的结合显著提高了可再生能源利用率，减少了对高碳排放外部电力的依赖。框架实现了本地自治、自利追求与社区层面经济环境效益的平衡整合。

Conclusion: 该框架通过多智能体强化学习和创新的市场机制设计，成功解决了微电网社区中个体利益与集体低碳目标之间的协调问题，为实际应用提供了可行的解决方案。
