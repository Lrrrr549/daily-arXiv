...
TL;DR: 提出基于多模态大语言模型的三阶段智能电网图纸审查框架，通过提示工程实现全局语义理解、高分辨率细粒度识别和综合决策，提高设计错误检测准确性和可靠性


<details>
  <summary>Details</summary>
Motivation: 当前自动化系统处理超高分辨率电网工程设计图纸时面临计算需求高、信息丢失和缺乏整体语义理解等问题，难以有效识别设计错误，需要更智能可靠的审查方法

Method: 提出三阶段框架：1) 利用MLLM进行全局语义理解，从低分辨率概览中智能提出领域特定语义区域；2) 在提议区域内进行高分辨率细粒度识别，获取带置信度得分的详细信息；3) 综合决策模块集成置信度感知结果，准确诊断设计错误并提供可靠性评估

Result: 在真实电网图纸上的初步结果表明，该方法显著增强了MLLM掌握宏观语义信息和精确定位设计错误的能力，相比传统被动MLLM推理，在缺陷发现准确性和审查判断可靠性方面均有改进

Conclusion: 该研究为智能可靠的电网图纸审查提供了一种新颖的提示驱动范式，通过模仿人类专家审查过程的三阶段框架，有效解决了超高分辨率图纸处理中的关键挑战
...
Conclusion: 通过将生成过程建模为隐函数并进行全面的理论分析，揭示了扰动文本条件、模型参数和潜在状态等因子都能重新唤醒被擦除的概念。基于此提出的LURE方法通过重构潜在空间和引导采样轨迹，有效解决了多概念重新唤醒中的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注室内或街道场景，忽视了开放城市空间的独特挑战（丰富的语义、复杂几何结构和视角变化）。需要专门基准来评估视觉语言模型在城市环境中的跨视角推理能力。

Method: 创建CityCube基准，整合四种视角动态模拟相机运动，涵盖车辆、无人机和卫星等多种平台视角。包含5,022个精心标注的多视角问答对，分为五种认知维度和三种空间关系表达。评估了33个视觉语言模型。

Result: 评估显示视觉语言模型与人类存在显著性能差距：即使大规模模型准确率也难以超过54.1%，比人类低34.2%。相比之下，小规模微调模型能达到60.0%以上准确率。分析揭示了任务相关性和模型与人类推理之间的基本认知差异。

Conclusion: CityCube基准揭示了当前视觉语言模型在城市跨视角空间推理方面的局限性，强调了专门基准的必要性，并为理解模型与人类认知差异提供了重要见解。
...
Result: SegAE与真实Dice相似度相关系数达0.902，评估3D掩码仅需0.06秒；分析显示公共数据集存在广泛低质量标签；在主动和半监督学习中提升数据效率和训练性能，减少三分之一标注成本和70%质量检查时间。
...
<details>
  <summary>Details</summary>
Motivation: 交通场景理解对自动驾驶安全导航至关重要，但现有方法在从单目图像生成全面自然语言描述方面存在不足，且缺乏专门的训练数据集和合适的评估指标

Method: 提出新颖框架，利用混合注意力机制增强空间和语义特征提取，将特征整合生成上下文丰富的场景描述；基于BDD100K数据集开发新数据集，提供详细的构建指南

Result: 使用CIDEr、SPICE等指标进行定量评估，并结合人工判断评估，证明所提模型在新数据集上表现优异，有效实现了预期目标

Conclusion: 该框架成功实现了从单目图像到自然语言描述的转换，为自动驾驶系统的场景理解提供了有效解决方案，同时贡献了新的数据集和评估指标讨论
...
TL;DR: 提出基于高斯模型的相机-LiDAR自适应多模态3D占据预测方法，通过四个关键组件解决现有体素化方法计算复杂、融合脆弱等问题


<details>
  <summary>Details</summary>
Motivation: 稀疏目标检测向密集3D语义占据预测的范式转变对自动驾驶长尾安全挑战至关重要，但现有体素化方法存在计算复杂度高、融合过程脆弱且在动态环境下失效等问题

Method: 提出高斯基自适应相机-LiDAR多模态3D占据预测模型，包含四个关键组件：LiDAR深度特征聚合（使用深度可变形采样处理几何稀疏性）、基于熵的特征平滑（使用交叉熵处理领域特定噪声）、自适应相机-LiDAR融合（基于模型输出动态重新校准传感器输出）、Gauss-Mamba头（使用选择性状态空间模型进行全局上下文解码，具有线性计算复杂度）

Result: 该方法通过内存高效的3D高斯模型无缝桥接相机模态的语义优势和LiDAR模态的几何优势，解决了现有方法的计算复杂度和融合脆弱性问题

Conclusion: 提出的高斯基自适应多模态3D占据预测框架有效解决了自动驾驶中长尾安全挑战，通过创新的融合策略和高效计算架构实现了鲁棒且高效的3D语义占据预测
...
<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测方法存在情感模糊性问题：忽视情感线索的方法无法捕捉抑郁信号，而结合情感的方法又容易混淆短暂情感表达与稳定抑郁症状，导致检测错误

Method: 提出READ-Net框架，采用自适应特征重校准(AFR)技术，动态调整情感特征权重，增强抑郁相关信号，保留抑郁相关线索同时过滤情感噪声

Result: 在三个公开数据集上评估，READ-Net优于最先进方法，平均准确率提升4.55%，F1分数提升1.26%，对情感干扰具有鲁棒性

Conclusion: READ-Net通过解决情感模糊性问题，显著改善了视听抑郁症检测性能，且易于集成到现有框架中
...
TL;DR: DeepMoLM是一个双视图分子语言建模框架，通过融合高分辨率分子图像和基于分子构象的几何不变量，实现物理基础的分子描述生成，无需原子坐标。


<details>
  <summary>Details</summary>
Motivation: 现有分子语言模型大多依赖字符串或图表示，而视觉语言模型往往忽略立体化学细节，难以将连续3D结构映射到离散标记中。需要一种能同时处理分子图像并保持3D几何一致性的方法。

Method: 提出DeepMoLM双视图框架：1) 从1024×1024高分辨率分子图像中保留高频证据；2) 将构象邻域编码为离散的扩展3维指纹；3) 通过交叉注意力融合视觉和几何流，实现物理基础的生成而无需原子坐标。

Result: 在PubChem标注任务中，相对最强通用基线获得12.3%的METEOR提升，与专业方法保持竞争力；对所有属性查询产生有效数值输出，在专业设置下分子量MAE为13.64 g/mol，复杂度MAE为37.89；在ChEBI-20图像描述生成中超越通用基线并匹配最先进的视觉语言模型。

Conclusion: DeepMoLM通过融合高分辨率分子图像和几何不变量，成功解决了分子视觉语言建模中的立体化学细节缺失问题，实现了物理基础的分子描述生成，在多个基准测试中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个统一的OCR前端系统，能够通过单一模型和提示接口同时处理文本阅读、检测和定位任务，解决传统OCR系统功能分散的问题。

Method: 通过微调Qwen2.5-VL-3B和Qwen2.5-VL-7B模型，在商业文档、科学文章和合成定位数据上进行训练，创建支持全页和局部阅读的模型，提供行级和段落级边界框以及条件查询功能。

Result: GutenOCR-7B在10.5K个保留的商业和科学页面上，将基础模型的综合定位OCR分数从0.40提升到0.82（翻倍以上）。在Fox和OmniDocBench v1.5基准测试中，显著改进了区域和行级OCR以及文本检测召回率，但在页面级线性化、颜色引导OCR和公式密集布局方面存在权衡。

Conclusion: GutenOCR展示了通过微调视觉语言模型构建统一OCR前端的可行性，在文本检测和定位方面取得显著改进，但某些特定任务仍存在性能权衡，为未来OCR系统开发提供了有价值的参考。
...
TL;DR: PAS-Mamba：一种用于MRI重建的相位-幅度-空间状态空间模型，通过在频域解耦相位和幅度建模，结合图像域特征，实现更好的重建效果。
...
Conclusion: 通过解耦频域相位和幅度建模，并结合图像域特征，PAS-Mamba能够避免表示耦合，实现更优的MRI重建，验证了分别处理相位和幅度信息的重要性。
...
TL;DR: SDT-Net：基于动态教师切换的双教师-单学生框架，通过自适应选择可靠教师、高置信度伪标签和多层次特征对齐，解决医学图像分割中涂鸦标注稀疏性带来的噪声传播问题。


<details>
  <summary>Details</summary>
Motivation: 涂鸦监督方法虽能减轻医学图像分割的标注负担，但其稀疏性引入显著歧义，导致噪声伪标签传播并阻碍鲁棒解剖边界学习。

Method: 提出SDT-Net双教师-单学生框架，包含动态教师切换模块自适应选择可靠教师，通过拾取可靠像素机制精炼高置信度伪标签，以及分层一致性模块强制执行多层次特征对齐。

Result: 在ACDC和MSCMRseg数据集上的实验表明，SDT-Net达到最先进性能，产生更准确且解剖学上合理的分割结果。

Conclusion: SDT-Net通过动态教师选择和协同监督机制，有效利用稀疏涂鸦标注，实现了高质量的医学图像分割。
...
TL;DR: 该论文提出了一种可学习的帧选择器（LFS），用于视频描述任务，通过选择时间多样性和事件相关的帧来优化描述质量，并引入了新的评估基准ICH-CC。


<details>
  <summary>Details</summary>
Motivation: 视频描述模型通常使用均匀采样处理视频帧，但这种方法忽略了事件分布的不均匀性，强制所有时间段获得相同覆盖，导致重要事件可能被遗漏。同时，现有评估基准与人类认知存在差距。

Method: 提出Learnable Frame Selector（LFS），显式建模时间重要性以平衡时间多样性和事件相关性，采用分层策略确保时间覆盖同时避免聚类。关键创新是利用冻结的视频-LLM生成的描述反馈来学习直接优化下游描述质量的帧选择。此外，构建了ICH-CC基准，通过精心设计的问题反映人类一致的理解。

Result: LFS在两个代表性社区基准和ICH-CC上持续改进详细视频描述，在VDC上获得高达2.0%的提升，在ICH-CC上获得超过4%的提升。使用LFS增强的描述还提高了视频问答任务的性能。

Conclusion: LFS为详细视频描述提供了一个有效且易于集成的解决方案，通过智能帧选择优化描述质量，同时新基准ICH-CC更好地反映了人类认知。
...
TL;DR: 提出U-Harmony方法，通过域门控头和特征分布归一化-反归一化，使单个分割模型能同时从异构医学数据集中学习，支持新模态和解剖类别的无缝适应。
...
Method: 提出通用协调(U-Harmony)联合训练方法，集成到带有域门控头的深度学习架构中。该方法顺序归一化然后反归一化特征分布，以减轻领域特定变异同时保留原始数据集特定知识。框架还支持通用模态适应。

Result: 在跨机构脑病变数据集上的广泛实验证明了方法的有效性，为真实临床环境中鲁棒且适应性强的3D医学图像分割模型建立了新基准。

Conclusion: U-Harmony方法能够使单个分割模型同时从异构数据集中学习，有效解决临床实践中数据有限性和异质性问题，支持新模态和解剖类别的无缝适应。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉理解方面表现出色，但缺乏层次知识理解能力，导致在分类学任务中即使正确识别最具体的叶级类别，也经常误判更粗粒度的分类层次。现有方法大多忽视这一问题，未能对层次推理进行建模。

Method: 提出VL-Taxon两阶段层次推理框架：第一阶段采用自上而下的过程提升叶级分类准确性；第二阶段利用准确的叶级输出确保整个分类学层次的一致性。每个阶段先通过监督微调注入分类学知识，再通过强化学习优化推理和泛化能力。

Result: 在iNaturalist-2021数据集上，基于Qwen2.5-VL-7B模型实现的VL-Taxon框架，在叶级准确性和层次一致性方面平均超过其原始72B版本10%以上。这一显著提升仅通过对小规模数据子集的微调实现，无需依赖其他VLM生成的示例。

Conclusion: VL-Taxon框架通过层次推理机制有效解决了VLM在分类学任务中的层次知识理解缺陷，证明了层次建模对提升分类准确性和一致性的重要性，且该方法具有高效的数据利用特性。
...
TL;DR: 提出DEUA框架，通过区分重构误差中的偶然不确定性和认知不确定性来提升扩散生成图像检测性能，使用拉普拉斯近似估计认知不确定性并引入非对称损失函数训练平衡分类器。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的生成图像检测方法虽然利用重构误差等扩散测量提升泛化性，但忽视了偶然不确定性和认知不确定性对重构误差的不同影响。偶然不确定性源于数据固有噪声，会阻碍生成图像的准确检测；而认知不确定性反映模型对陌生模式的知识缺乏，有助于检测。需要区分这两种不确定性以提升检测性能。
...
Conclusion: 通过区分重构误差中的偶然不确定性和认知不确定性，并引入扩散认知不确定性估计和非对称学习，DEUA框架显著提升了扩散生成图像检测的性能和泛化能力。
...
TL;DR: Mirai框架通过向自回归视觉生成器注入未来信息（前瞻信号）来改善全局一致性和加速收敛，无需改变架构或增加推理开销。


<details>
  <summary>Details</summary>
Motivation: 传统自回归视觉生成器采用严格的因果监督，每个步骤仅通过下一个标记进行优化，这削弱了全局一致性并减慢了收敛速度。研究探索前瞻信号（源自后续标记的训练信号）是否能帮助自回归视觉生成。

Method: 提出Mirai框架，通过沿注入级别、前瞻布局和前瞻来源三个轴进行受控诊断，发现关键见解：将前瞻与自回归模型在2D图像网格上的内部表示对齐可改善因果建模。Mirai-E使用来自单向表示多个未来位置的显式前瞻，Mirai-I利用匹配双向表示的隐式前瞻，两者均无需架构改变或额外推理开销。

Result: Mirai显著加速收敛并提高生成质量。例如，Mirai可将LlamaGen-B的收敛速度提升高达10倍，在ImageNet类条件图像生成基准上将生成FID从5.34降至4.34。

Conclusion: 研究表明视觉自回归模型需要前瞻信号。Mirai框架通过注入未来信息有效解决了传统自回归训练中全局一致性不足和收敛缓慢的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频重渲染方法面临两个问题：几何无约束模型缺乏空间感知，在视角变化时产生漂移和变形；几何约束模型依赖估计深度和显式重建，易受深度误差和标定错误影响

Method: 利用大型4D重建模型潜在空间中的隐式几何知识来约束视频生成过程，这些潜在表示在连续空间中捕获场景结构而无需显式重建，结合源相机位姿进行联合条件化
...
TL;DR: 本文综述了视频和图像监控中的现代目标检测技术，重点关注架构创新、生成模型集成和时间信息利用，旨在提升检测的鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 目标检测是监控领域的基础任务，但面临动态环境、遮挡、光照变化和实时性等挑战。本文旨在系统评估当前语义目标检测技术的有效性，分析深度学习模型及其实际应用，为研究者提供全面的技术概览。

Method: 采用分类综述方法，基于核心架构、数据处理策略和监控特定挑战对方法进行分类。涵盖CNN检测器、GAN辅助方法和时间融合技术，分析生成模型在帧重建、遮挡减少和光照归一化等任务中的应用。

Result: 系统总结了预处理流程、特征提取进展、基准数据集和比较评估结果。识别出现代目标检测技术在鲁棒性、准确性和实时性方面的改进，特别是生成模型和时间信息融合带来的性能提升。

Conclusion: 目标检测技术正朝着低延迟、高效和时空学习方向发展。未来研究应关注实时处理、计算效率和多模态时空信息融合，以应对复杂监控场景的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有时尚图像检索基准测试存在以下问题：1) 数据集过时，无法反映当前时尚趋势；2) 缺乏对模型训练数据污染的考虑；3) 任务设置不够全面，无法模拟真实电商环境中的多样化检索需求。

Method: 1) 构建包含近期产品图像和AI生成图像的测试集，所有样本都有时间戳；2) 基于细粒度属性分类法设计单物品和套装级别的检索任务；3) 实施污染感知评估机制，与声明的训练截止时间对齐；4) 开发专有模型和开源对应模型进行基准测试。

Result: 1) LookBench对现有强基线模型构成显著挑战，许多模型Recall@1低于60%；2) 专有模型在LookBench上表现最佳，开源对应模型排名第二；3) 两种模型在传统Fashion200K评估中都达到了最先进水平；4) 基准测试将每半年更新，逐步增加任务难度。

Conclusion: LookBench提供了一个动态、全面的时尚图像检索评估框架，能够更好地反映真实电商环境中的检索需求，通过定期更新和污染感知评估机制，为研究社区提供了一个持久的进展衡量标准。
...
TL;DR: 提出CPF-CTE框架，通过上下文补丁融合和类别标记增强改进弱监督语义分割，利用CF-BiLSTM捕获空间依赖关系，引入可学习类别标记增强判别能力，在PASCAL VOC和MS COCO上取得SOTA性能。
...
Method: 提出CPF-CTE框架：1) CF-BiLSTM模块捕获补丁间空间依赖，实现双向信息流；2) 引入可学习类别标记动态编码和细化类别特定语义；3) 有效整合空间和语义线索。

Result: 在PASCAL VOC 2012和MS COCO 2014数据集上的广泛实验验证，CPF-CTE始终优于先前的WSSS方法。

Conclusion: CPF-CTE通过利用补丁间上下文关系丰富特征表示，结合空间依赖捕获和类别语义增强，显著提升了弱监督语义分割的性能。
...
TL;DR: HERMES是一种无需训练的新型架构，用于实时准确理解视频流，通过重用紧凑的KV缓存实现高效流式理解，在资源受限下达到10倍更快的TTFT，同时减少68%视频token仍保持或超越SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在离线视频理解方面取得显著进展，但在扩展到流式视频输入时面临挑战：现有模型难以同时保持稳定的理解性能、实时响应和低GPU内存开销。

Method: 基于机制注意力研究，将KV缓存概念化为分层内存框架，封装多粒度视频信息。在推理时重用紧凑的KV缓存，实现资源约束下的高效流式理解。无需用户查询到达时的辅助计算，保证连续视频流交互的实时响应。

Result: 相比均匀采样减少高达68%的视频token，HERMES在所有基准测试中达到相当或更优的准确率，在流式数据集上获得高达11.4%的提升。相比先前SOTA实现10倍更快的TTFT（首次令牌时间）。
...
### [22] [Safeguarding Facial Identity against Diffusion-based Face Swapping via Cascading Pathway Disruption](https://arxiv.org/abs/2601.14738)
*Liqin Wang,Qianyue Hu,Wei Lu,Xiangyang Luo*

Main category: cs.CV

TL;DR: VoidFace是一种针对扩散模型人脸交换系统的主动防御方法，通过注入扰动破坏身份信息流，在保持视觉质量的同时有效防御人脸交换攻击。
...
Method: VoidFace将人脸交换视为耦合的身份通路，通过在关键瓶颈处注入扰动引发级联破坏。具体包括：1) 定位破坏和身份擦除以降低物理回归和语义嵌入质量；2) 在生成域中通过解耦注意力机制切断身份注入，并破坏中间扩散特征以防止源身份重建；3) 在潜在流形中进行对抗搜索，采用感知自适应策略平衡攻击效果与图像质量。

Result: 大量实验表明，VoidFace在各种基于扩散的人脸交换模型中都优于现有防御方法，同时生成的对抗人脸具有更优的视觉质量。

Conclusion: VoidFace通过系统性破坏人脸交换的身份通路，提供了一种有效的主动防御方法，能够在保持图像质量的同时保护个人身份隐私安全。
...
TL;DR: 提出端边协同的生成-增强框架，通过自适应选择去噪步数和超分辨率尺度，在边缘端生成低分辨率图像，然后采用区域感知混合超分辨率策略，对前景使用扩散模型恢复细节，对背景使用轻量学习模型高效上采样，最终拼接成高分辨率图像，在保持图像质量的同时降低33%的服务延迟。


<details>
  <summary>Details</summary>
Motivation: AIGC在文本到图像生成方面取得显著进展，高分辨率生成对提升用户体验质量至关重要。虽然资源受限的边缘计算能支持快速低分辨率生成，但实现高分辨率输出仍面临保真度与延迟之间的权衡挑战。研究发现轻量学习型超分辨率难以恢复细节，而扩散型超分辨率虽能获得更高保真度但计算成本高昂。

Method: 提出端边协同生成-增强框架：1）边缘端根据自适应选择的去噪步数和超分辨率尺度生成低分辨率图像；2）将图像分割为多个补丁；3）采用区域感知混合超分辨率策略：对前景补丁应用扩散型超分辨率模型以恢复细节，对背景补丁应用轻量学习型超分辨率模型进行高效上采样；4）将增强后的补丁拼接成最终高分辨率图像。

Result: 实验表明，与基线方法相比，该系统在保持竞争力的图像质量的同时，将服务延迟降低了33%。

Conclusion: 通过端边协同的生成-增强框架和区域感知混合超分辨率策略，成功解决了高分辨率文本到图像生成中的保真度与延迟权衡问题，实现了在保持图像质量的同时显著降低服务延迟的目标。
...
TL;DR: SimD3是一个用于无人机检测的大规模高保真合成数据集，通过建模异质载荷无人机、鸟类干扰物和多样化环境，结合改进的YOLOv5检测框架，提升了复杂空中环境下无人机检测的鲁棒性和泛化能力。
...
Method: 1) 创建SimD3合成数据集：建模异质载荷无人机、多种鸟类干扰物、使用Unreal Engine 5多样化环境（控制天气、光照、飞行轨迹）、采用360度六相机系统采集；2) 提出Yolov5m+C3b检测框架：将标准C3瓶颈块替换为C3b模块的注意力增强变体；3) 在合成数据、合成+真实数据、多个未见真实世界基准上进行评估。

Result: 实验结果表明：1) SimD3为小目标无人机检测提供了有效的监督；2) Yolov5m+C3b在域内和跨数据集评估中始终优于基线模型；3) 模型在多样化和挑战性条件下展现出良好的鲁棒性和泛化能力。

Conclusion: SimD3合成数据集对于训练和评估鲁棒的无人机检测模型具有重要价值，特别是在复杂空中环境下。注意力增强的YOLOv5变体进一步提升了检测性能，为无人机检测研究提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有病理学多模态方法由于缺乏支持显式推理的高质量数据集和简单的推理过程，导致可解释性有限。需要开发能够进行复杂推理的多模态病理学模型。

Method: 1) 构建高质量病理视觉问答(VQA)数据集，专门支持复杂推理任务；2) 设计结合组相对策略优化的语义奖励策略，提升文本描述的准确性和上下文相关性；3) 开发具有强推理能力的多模态病理学大语言模型。

Result: 1) 在构建的病理VQA数据集上，该方法优于现有最先进方法，即使仅使用20%的数据训练；2) 在下游零样本图像分类任务上，与CLIP模型性能相当。

Conclusion: 提出的多模态病理学大语言模型通过语义奖励策略和高质量数据集，显著提升了病理图像与文本整合的推理能力和可解释性，在有限数据下仍能取得优异性能。
...
TL;DR: 该研究分析了开源医学视觉语言模型的特征表示，发现医学VLM能提取有效的判别性特征，但非医学VLM通过上下文增强（如LLM2CLIP）能产生更精细的特征表示，表明文本编码器增强比密集医学图像训练更重要。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型虽然预期能捕捉诊断相关特征，但其学习到的特征表示尚未充分探索，标准评估（如分类准确率）不能完全揭示它们是否真正获得了具有判别性的病灶特异性特征。理解这些特征表示对于揭示医学图像结构和改进医学图像分析的下游任务至关重要。

Method: 分析多个代表性医学VLM在不同模态的病灶分类数据集上提取的特征分布，并将这些分布与非医学VLM进行比较，以评估领域特定的医学训练效果。

Result: 实验表明：1）医学VLM能提取对医学分类任务有效的判别性特征；2）通过上下文增强改进的非医学VLM（如LLM2CLIP）能产生更精细的特征表示；3）非医学模型特别容易受到图像上叠加文本字符串引入的偏见影响。

Conclusion: 开发医学VLM时，增强文本编码器比密集训练医学图像更为关键。研究结果强调需要根据下游任务仔细选择模型，并注意图像中文本信息等背景偏见带来的潜在推理风险。
...
TL;DR: FunCineForge提出端到端配音数据集生产流程和基于MLLM的配音模型，解决现有方法在数据集规模、标注质量和复杂场景适应性方面的局限性，在音频质量、唇部同步、音色转换和指令跟随方面超越SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有电影配音方法面临两大限制：1)高质量多模态配音数据集规模有限、词错误率高、标注稀疏、依赖昂贵人工标注且仅限于独白场景，阻碍有效模型训练；2)现有配音模型仅依赖唇部区域学习视听对齐，限制了在复杂实景电影场景中的适用性，且在唇部同步、语音质量和情感表达方面表现欠佳。

Method: 提出FunCineForge，包含：1)用于大规模配音数据集的端到端生产流程；2)基于MLLM的配音模型，专为多样化电影场景设计。使用该流程构建了首个具有丰富标注的中文电视剧配音数据集，并展示了数据的高质量。

Result: 在独白、旁白、对话和多说话者场景的实验表明，该配音模型在音频质量、唇部同步、音色转换和指令跟随方面持续优于SOTA方法。代码和演示已公开。

Conclusion: FunCineForge通过创新的数据集生产流程和MLLM-based配音模型，有效解决了现有电影配音方法的局限性，为复杂电影场景提供了高质量的配音解决方案，在多个关键指标上超越了现有最佳方法。
...
TL;DR: RAM提出了一种重建锚定的扩散模型，通过运动潜空间作为中间监督和重构误差指导机制，解决了文本驱动运动生成中的表示差距和误差传播问题。


<details>
  <summary>Details</summary>
Motivation: 当前运动扩散模型面临两个主要限制：1) 预训练文本编码器缺乏运动特定信息导致的表示差距；2) 迭代去噪过程中的误差传播问题。这些问题影响了文本到运动生成的准确性和质量。

Method: RAM采用双分支架构：1) 运动重建分支，通过自正则化和运动中心潜空间对齐目标函数共同训练；2) 重构误差指导(REG)机制，在测试阶段利用扩散模型的自校正能力，通过放大当前预测与重构估计之间的残差来减轻误差传播。

Result: 大量实验表明，RAM在文本到运动生成任务上取得了显著改进和最先进的性能，有效解决了表示差距和误差传播问题。

Conclusion: RAM通过引入运动潜空间作为中间监督和重构误差指导机制，成功提升了文本驱动运动扩散模型的性能，为相关任务提供了有效的解决方案。
...
TL;DR: 该研究探讨了使用Stable Diffusion结合LoRA生成的合成图像来增强中国瓷器分类训练数据的有效性，通过多任务CNN模型验证了合成数据在不同分类任务中的差异化效果。


<details>
  <summary>Details</summary>
Motivation: 考古文物分类中训练数据稀缺是深度学习的根本挑战，特别是对于稀有的中国瓷器类型。需要探索合成图像能否有效增强有限的真实数据集。

Method: 使用Stable Diffusion结合Low-Rank Adaptation生成合成图像，采用MobileNetV3进行迁移学习，设计控制实验比较纯真实数据与混合真实-合成数据集（95:5和90:10比例）在四个分类任务上的表现：朝代、釉色、窑口和类型识别。
...
<details>
  <summary>Details</summary>
Motivation: 当前遥感变化检测方法主要依赖专用模型，限制了向模态自适应地球观测的可扩展性。同质变化检测需要细粒度空间线索，而异质变化检测需要更广泛的上下文信息来抑制噪声和几何失真。传统差分操作在同质图像中有效，但在跨模态或几何未对齐场景中会产生伪影。现有基于静态主干或固定差分操作的专用模型在不同模态设置下往往不足。

Method: 提出UniRoute统一框架，将特征提取和融合重新定义为条件路由问题。包括：1) 自适应感受野路由MoE模块(AR2-MoE)，解耦局部空间细节和全局语义上下文；2) 模态感知差分路由MoE模块(MDR-MoE)，自适应选择每个像素最合适的融合原语；3) 一致性感知自蒸馏策略(CASD)，在数据稀缺的异质设置下通过强制执行多级一致性来稳定统一训练。

Result: 在五个公共数据集上的广泛实验表明，UniRoute实现了强大的整体性能，在统一部署设置下具有有利的精度-效率权衡。

Conclusion: UniRoute通过条件路由机制实现了模态自适应的遥感变化检测，有效解决了传统方法在同质和异质场景下的局限性，为统一部署提供了可行的解决方案。
...
TL;DR: 该论文提出了一种同时具有对称性感知和对称性无关的特征解耦方法，以及特征细化技术，用于构建更鲁棒的对称感知形状描述符。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能从语义感知描述符中提取对称性信息特征，但这些特征通常是一维的，忽略了其他有价值的语义信息，且提取的对称性特征通常存在噪声并产生小的误分类区域。

Method: 提出特征解耦方法，同时生成对称性感知和对称性无关的特征；并开发特征细化技术来提高预测的对称性感知特征的鲁棒性。

Result: 在内在对称性检测、左右分类和形状匹配等任务上的广泛实验表明，所提框架在定性和定量上都优于各种最先进方法。

Conclusion: 该研究成功构建了同时具有对称性感知和对称性无关特征的形状描述符，并通过特征细化提高了对称性特征的鲁棒性，在多个形状分析任务中表现出优越性能。
...
TL;DR: POTR是一种用于3D高斯泼溅的后训练压缩编解码器，通过高效剪枝和光照系数重计算技术，显著减少存储需求并加速推理，优于现有后训练压缩方法。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在训练和推理速度上优于NeRF，但存储需求显著更高。为解决这一缺点，需要开发高效的后训练压缩技术来减少3DGS的存储占用。

Method: 提出POTR编解码器，包含两项核心技术：1) 使用改进的3DGS光栅化器同时计算每个泼溅的移除效果，实现高效剪枝；2) 提出光照系数重计算方法，在不训练的情况下降低其熵值。此外还扩展了简单的微调方案以进一步提升性能。

Result: POTR比其他后训练剪枝技术减少2-4倍的泼溅数量，推理速度比其他压缩模型快1.5-2倍。AC光照系数稀疏度从70%提升至97%，质量损失最小。在率失真性能和推理速度上均优于所有其他后训练压缩技术。

Conclusion: POTR是一种高效的后训练3DGS压缩解决方案，通过创新的剪枝和光照系数优化技术，在保持高质量的同时显著减少了存储需求和加速了推理速度，为3DGS的实际应用提供了重要支持。
...
TL;DR: 提出GAT-NeRF框架，通过结合Transformer机制增强NeRF，从单目视频实现高保真4D动态面部头像重建，显著提升皱纹等高频细节的建模能力。


<details>
  <summary>Details</summary>
Motivation: 沉浸式虚拟人应用对高保真4D动态面部头像重建的需求日益增长，但现有NeRF方法在处理单目视频信息受限的情况下，难以充分捕捉动态皱纹和细微纹理等高频率面部细节。

Method: 提出GAT-NeRF混合神经辐射场框架，将Transformer机制集成到NeRF流程中。结合坐标对齐MLP和轻量级Geometry-Aware-Transformer模块，融合3D空间坐标、3DMM表情参数和可学习潜在码等多模态输入特征，增强细粒度几何特征表示。

Result: 综合实验明确证明GAT-NeRF在视觉保真度和高频细节恢复方面达到最先进性能，为多媒体应用创建逼真动态数字人开辟新途径。

Conclusion: GAT-NeRF通过Transformer增强的NeRF框架，成功解决了从单目视频重建高保真4D动态面部头像的挑战，显著提升了高频面部细节的建模能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有系统通常将3D几何、语义和语言信息分离处理，缺乏统一的表示方法。需要一种能够从日常RGB视频中提取结构化空间记忆，支持空间关系推理和下游任务的系统。

Method: 从RGB视频重建度量尺度室内环境，检测结构3D锚点（墙、门、窗）作为第一层支架，构建分层记忆，包含开放词汇对象节点，将证据补丁、视觉嵌入和两层文本描述链接到3D坐标。

Result: 在三个真实室内场景的实验中，SpatialMem在增加杂乱和遮挡情况下保持强大的锚点描述级导航完成度和分层检索准确性，支持语言引导导航和物体检索。

Conclusion: SpatialMem提供了一个高效可扩展的框架，将3D几何、语义和语言统一为单一可查询表示，支持空间关系推理和下游任务，为具身空间智能提供基础。
...
Method: 提出EroSeg-AT对抗训练框架，其中EroSeg首先基于像素级置信度选择敏感像素，然后逐步将扰动传播到更高置信度的像素，有效破坏样本的语义一致性
...
TL;DR: TempViz是首个全面评估文本到图像模型时间知识的数据集，包含7.9k提示和600+参考图像，用于评估5个T2I模型在5个时间知识类别上的表现，发现模型时间能力普遍较弱且缺乏可靠自动评估方法。
...
Method: 创建TempViz数据集（7.9k提示，600+参考图像），涵盖5个时间知识类别。使用该数据集评估5个T2I模型，进行人工评估，并比较多种自动评估方法与人工判断的一致性。

Result: 人工评估显示T2I模型的时间能力普遍较弱，所有模型在各类别上的准确率均未超过75%。自动评估方法均无法可靠评估时间线索，与人工判断相关性差，表明当前缺乏有效的自动评估方案。

Conclusion: T2I模型的时间知识能力有限，需要进一步研究。TempViz为系统评估提供了基准，但缺乏可靠的自动评估方法，凸显了未来研究T2I时间知识的迫切需求。
...
TL;DR: LDF-VFI提出了一种基于局部扩散强迫的视频帧插值方法，采用视频中心化的整体范式，通过自回归扩散变换器建模整个视频序列，确保长程时间一致性，并支持任意空间分辨率处理。
...
Method: 基于自回归扩散变换器建模整个视频序列；引入跳跃连接采样策略缓解自回归生成中的误差累积；结合稀疏局部注意力和分块VAE编码实现长序列高效处理；使用增强的条件VAE解码器利用输入视频的多尺度特征。

Result: 在具有挑战性的长序列基准测试中达到最先进性能，展现出卓越的单帧质量和时间一致性，特别是在大运动场景中。支持任意空间分辨率处理，无需重新训练即可处理4K视频。

Conclusion: LDF-VFI通过视频中心化的整体范式解决了现有帧插值方法的时间不一致性问题，通过自回归扩散变换器和创新的采样策略实现了长程时间一致性和高质量插值结果。
...
TL;DR: 提出Scale-TBPS方法，通过噪声感知的统一数据集构建策略和可扩展的身份判别学习框架，训练单一模型在多个文本行人检索数据集上超越数据集特定优化的模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的文本行人检索方法受限于训练数据不足且模型未针对行人识别进行预训练，导致需要为不同数据集分别训练独立模型。合成数据虽能增加规模但无法消除数据集特定适应需求，因此需要探索能否训练单一统一模型跨多个数据集工作。

Method: 提出Scale-TBPS方法，包含两个核心贡献：1) 噪声感知的统一数据集构建策略，将多个TBPS数据集进行融合；2) 可扩展的身份判别学习框架，能够在大量独特身份下保持有效学习。

Result: 在CUHK-PEDES、ICFG-PEDES、RSTPReid、IIITD-20K和UFine6926五个数据集上的实验表明，单一Scale-TBPS模型优于数据集特定优化的模型和简单的联合训练方法。

Conclusion: Scale-TBPS通过创新的数据集融合策略和可扩展学习框架，成功实现了单一模型在多个文本行人检索数据集上的统一训练，解决了现有方法需要为不同数据集分别训练模型的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频评估基准主要关注非互动视频（如电影和录像），缺乏针对互动直播视频的专门基准。互动直播视频具有实时评论、音频、语音等多模态特性，需要专门的评估体系

Method: 1) 设计LiViBench基准：包含24个多样化任务，涵盖感知、推理和直播特有挑战；2) 标准化半自动标注流程：结合人在回路的多阶段标注，利用多MLLM构建多智能体系统进行视频描述，采用种子问题驱动方法构建高质量标注；3) 开发LiVi-LLM-7B模型：设计两阶段指令微调和视频到评论检索(VCR)模块，增强模型对实时评论的利用能力

Result: LiVi-LLM-7B在LiViBench上超越参数高达72B的开源模型，缩小了与领先专有模型的差距，同时在通用视频基准（VideoMME、LongVideoBench、MLVU、VideoEval-Pro）上表现提升

Conclusion: LiViBench填补了互动直播视频评估的空白，提出的标注方法和模型架构有效提升了MLLM对互动直播视频的理解能力，为未来研究提供了重要基准和技术方案
...
TL;DR: 该论文针对现有视频到音频生成模型缺乏空间感知和沉浸感的问题，提出了首个大规模视频-双耳音频数据集BinauralVGGSound，并开发了一个视觉引导的空间音频生成框架，显著提升了生成音频的空间保真度和沉浸体验。


<details>
  <summary>Details</summary>
Motivation: 现有视频到音频生成研究主要关注语义和时间对齐，但忽视了音频的空间感知和沉浸质量。这一局限源于当前模型依赖单声道音频数据集，缺乏双耳空间信息来学习视觉到空间音频的映射关系。

Method: 提出了两个关键贡献：1) 构建了首个大规模视频-双耳音频数据集BinauralVGGSound；2) 提出了一个端到端的视觉引导空间音频生成框架，该框架包含视觉引导的音频空间化模块，能够显式建模空间特征，确保生成的音频具有真实的空间属性和层次化的空间深度。

Result: 实验表明，该方法在空间保真度方面显著优于最先进模型，提供了更沉浸的听觉体验，同时没有牺牲时间或语义一致性。

Conclusion: 通过构建首个大规模视频-双耳音频数据集和开发视觉引导的空间音频生成框架，成功解决了视频到音频生成中空间感知不足的问题，显著提升了生成音频的沉浸感和空间保真度。所有数据集、代码和模型检查点将公开发布以促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然实现了去中心化模型训练，但仍易受深度泄漏攻击威胁。现有攻击方法在稳定性、保真度和鲁棒性方面存在不足，特别是在实际联邦学习设置下表现不佳

Method: 将生成式流匹配先验集成到重建过程中，通过流匹配基础模型引导优化朝向真实图像分布，无需私有数据知识即可提升重建保真度

Result: 在多个数据集和目标模型上的实验表明，该方法在像素级、感知和基于特征的相似度指标上一致优于最先进的攻击方法。在不同训练轮次、更大客户端批次大小以及噪声注入、裁剪和稀疏化等常见防御下仍保持有效

Conclusion: 该方法展示了生成先验在增强深度泄漏攻击方面的有效性，呼吁开发新的防御策略以应对配备强大生成先验的对手
...
TL;DR: 提出基于误差反馈随机梯度下降的差分隐私生成框架，在相同隐私预算下生成更高质量图像，在MNIST、Fashion-MNIST和CelebA基准上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 传统数据掩码技术（如匿名化）无法在隐私保护机器学习中同时实现预期的隐私保护和数据效用。现有方法在隐私和效用之间存在重复权衡过程，需要新的解决方案。

Method: 提出新颖的差分隐私生成框架，采用误差反馈随机梯度下降方法，在训练过程中引入重构损失和噪声注入机制，生成高质量合成数据。

Result: 在相同隐私预算下生成更高质量和可用性的图像，在MNIST、Fashion-MNIST和CelebA三个基准测试中，几乎所有指标上都取得了最先进的结果。

Conclusion: 提出的框架在隐私保护和数据效用之间取得了更好的平衡，通过误差反馈随机梯度下降和噪声注入机制，在保持差分隐私的同时生成了高质量的合成图像数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的前景-背景分解方法存在两个主要问题：1) 对背景区域采用统一的抑制策略，忽视了不同补丁对预测的贡献差异；2) 对前景区域未能充分考虑某些局部补丁可能与其他类别在视觉或语义上相似，从而误导训练过程。

Method: 提出三核心组件框架：1) 前景-背景分解模块，沿用现有方法分离图像；2) 自适应背景抑制模块，自适应加权补丁分类熵；3) 可混淆前景修正模块，识别并修正可混淆前景补丁。

Result: 大量实验结果表明，该即插即用框架显著提升了现有前景-背景分解方法的性能。

Conclusion: 通过自适应背景抑制和可混淆前景修正，有效解决了现有方法的局限性，为基于CLIP的少样本OOD检测提供了更优的解决方案。
...
### [44] [PROGRESSLM: Towards Progress Reasoning in Vision-Language Models](https://arxiv.org/abs/2601.15224)
*Jianshu Zhang,Chengxuan Qian,Haosen Sun,Haoran Lu,Dingcheng Wang,Letian Xue,Han Liu*

Main category: cs.CV

TL;DR: 该论文提出了Progress-Bench基准，用于评估视觉语言模型在任务进度推理方面的能力，发现现有模型表现不佳，并通过训练方法ProgressLM-3B实现了改进。


<details>
  <summary>Details</summary>
Motivation: 虽然现代视觉语言模型擅长描述可见内容，但尚不清楚它们是否能从部分观察中推断任务进度。任务进度估计需要长时程动态推理而非静态视觉内容识别，这是当前模型能力的空白。

Method: 1) 引入Progress-Bench基准系统评估VLM的进度推理能力；2) 探索人类启发的两阶段进度推理范式，包括训练免费提示和基于ProgressLM-45K数据集的训练方法；3) 在14个VLM上进行实验分析。

Result: 大多数VLM尚未准备好进行任务进度估计，对演示模态和视角变化敏感，处理不可回答情况能力差。训练免费提示方法效果有限且依赖模型，而训练方法ProgressLM-3B即使在小模型规模下也能实现一致改进，尽管在完全不同的任务集上训练。

Conclusion: 当前VLM在任务进度推理方面存在显著局限，但通过专门训练可以改进。研究揭示了特征性错误模式，阐明了进度推理成功或失败的条件和原因，为未来研究提供了方向。
...
TL;DR: 本文综述了Micro-CT在工业无损检测中3D缺陷表征的准确性与效率平衡方法，涵盖CT重建算法从解析法到深度学习的发展，以及体绘制算法、加速技术和数据缩减的改进，展望了数字孪生在结构健康监测中的应用前景。


<details>
  <summary>Details</summary>
Motivation: 工业CT超精密检测产生海量数据，需要在超精密检测中解决3D缺陷表征的准确性与效率之间的权衡问题。随着Micro-CT技术不断精进材料微观结构表征，需要为研究人员提供高效准确的3D重建方法指导。

Method: 采用文献综述方法，从医学影像到工业无损检测的演变视角，选择性地回顾和分析平衡准确性与效率的CT重建和体绘制方法。比较计算机断层扫描原理与微结构技术的进展，考察CT重建算法从解析方法到深度学习技术的演变，以及体绘制算法、加速和数据缩减的改进。

Result: 提供了对高效准确3D可视化方法的全面分析，帮助研究人员快速掌握微观特征的高效准确3D重建方法。探讨了高精度、逼真且高效的体绘制高级光照模型，并展望了CT重建和体绘制的潜在发展方向。

Conclusion: 本文旨在指导未来研究快速选择高效精确的方法，并为通过虚拟-物理交互实现材料内部缺陷实时在线监测开发新思路和新方法，将数字孪生模型应用于结构健康监测(SHM)。
...
TL;DR: 该论文研究了可提示分割模型对边界框提示自然变化的鲁棒性，通过用户研究收集真实标注，发现SAM类模型对提示噪声高度敏感，并提出BREPS方法生成对抗性边界框来评估模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前可提示分割模型（如SAM）的训练和评估通常使用简单的启发式方法生成合成提示，无法反映真实世界中用户标注的自然变化。需要研究这些模型对边界框提示自然变化的鲁棒性，以评估其在实际应用中的可靠性。
...
Result: 研究发现：1. 同一模型和实例在不同用户标注下分割质量存在显著差异，表明SAM类模型对自然提示噪声高度敏感；2. BREPS方法能有效生成对抗性边界框，揭示模型鲁棒性弱点；3. 在涵盖日常场景到医学影像的10个数据集上，现有模型均表现出对边界框变化的敏感性。

Conclusion: 可提示分割模型对边界框提示的自然变化缺乏鲁棒性，这限制了其在真实世界应用中的可靠性。BREPS提供了一种系统评估模型鲁棒性的方法，为未来开发更鲁棒的提示分割模型提供了重要基准和方向。
...
TL;DR: 提出GraSP方法，通过子图预测实现图像中的图识别，旨在创建通用的视觉图识别框架
...
Method: 开发了GraSP（Graph Recognition via Subgraph Prediction）方法，通过子图预测来识别图像中的图。该方法具有广泛适用性和简单性，可处理多种类型的图及其绘制方式

Result: 在多个合成基准测试和一个实际应用中验证了该方法，表明其能够处理多样化的图类型及其绘制，并且无需任务特定修改即可在不同任务间迁移

Conclusion: GraSP方法为更统一的视觉图识别框架铺平了道路，能够处理不同类型的图并在任务间迁移，无需特定修改
...
TL;DR: 该研究构建了一个包含10万+论文的多维分析管道，通过主题聚类、LLM辅助解析和结构化检索，系统分析了2020-2025年AI研究的发展趋势，揭示了安全研究、多模态推理、智能体等新兴方向的增长。


<details>
  <summary>Details</summary>
Motivation: 机器学习、视觉和语言领域的快速发展产生了海量文献，传统文献计量工具主要依赖元数据，难以深入分析论文的语义内容，无法有效追踪研究主题的演变和跨领域影响。

Method: 收集了2020-2025年间22个主要会议的10万+论文构建统一语料库，开发了多维分析管道，结合主题聚类、LLM辅助解析和结构化检索技术，系统分析研究主题生命周期、方法学转变、数据集/模型使用模式以及机构研究方向。

Result: 分析揭示了AI研究的显著转变：安全研究、多模态推理和智能体导向研究的快速增长；神经机器翻译和图方法等领域的逐渐稳定；提供了研究活动、方法学演变和资源使用模式的全面表征。

Conclusion: 该研究提供了AI研究演变的证据基础视图，为理解更广泛趋势和识别新兴方向提供了资源，展示了如何通过系统性的文本内容分析来追踪研究领域的动态发展。
...
TL;DR: PMPose是一种结合概率公式和掩码条件化的自上而下2D姿态估计器，在拥挤场景中提升性能而不牺牲标准场景表现。BMPv2整合PMPose和增强的SAM掩码细化模块，在COCO和OCHuman数据集上超越SOTA，首次在OCHuman上超过50 AP。


<details>
  <summary>Details</summary>
Motivation: 大多数2D人体姿态估计基准已接近饱和，但拥挤场景仍具挑战。需要开发能在拥挤场景中提升性能而不损害标准场景表现的姿态估计方法。

Method: 提出PMPose：结合概率公式和掩码条件化的自上而下2D姿态估计器。进一步提出BMPv2：整合PMPose和增强的SAM掩码细化模块，通过2D提示改进3D姿态估计，并展示2D姿态质量提升对3D估计的直接益处。

Result: BMPv2在COCO上超越SOTA 1.5 AP，在OCHuman上超越6 AP，成为首个在OCHuman上超过50 AP的方法。在OCHuman-Pose数据集上显示多人性能受姿态预测精度影响大于检测精度。

Conclusion: PMPose和BMPv2有效解决了拥挤场景的2D姿态估计挑战，2D姿态质量提升直接有益于3D姿态估计，为拥挤场景姿态估计提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 早期准确诊断阿尔茨海默病对及时临床干预和改善患者预后至关重要。研究旨在通过比较不同深度学习架构的性能，探索开发更可靠的AD诊断工具。

Method: 研究比较了5种CNN架构（EfficientNetB0、ResNet50、DenseNet201、MobileNetV3、VGG16）、5种Transformer模型（ViT、ConvTransformer、PatchTransformer、MLP-Mixer、SimpleTransformer），并提出了一种名为Evan_V2的混合模型。该混合模型通过特征级融合整合了10个CNN和Transformer架构的输出。所有模型在包含轻度痴呆、中度痴呆、非痴呆和极轻度痴呆的四类AD分类任务上进行评估。

Result: CNN架构表现稳定，ResNet50达到98.83%准确率。Transformer模型具有竞争力的泛化能力，ViT在其中表现最佳（95.38%准确率），但个别Transformer变体表现出更大的类别特异性不稳定。提出的Evan_V2混合模型表现最优，达到99.99%准确率、0.9989 F1分数和0.9968 ROC AUC。混淆矩阵分析证实Evan_V2显著减少了所有痴呆阶段的误分类。

Conclusion: 混合集成策略在阿尔茨海默病分类中具有巨大潜力，能够产生高度可靠且具有临床意义的诊断工具。Evan_V2模型通过整合多种架构的优势，实现了超越所有独立模型的最佳性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D扩散模型在生成城市场景时外观细节退化，而仅用2D扩散模型则相机可控性差。需要一种既能保持细节又能实现相机控制的方法来生成逼真的3D城市场景。

Method: 提出ScenDi方法：1) 训练3D潜在扩散模型生成3D高斯，可渲染低分辨率图像，支持3D边界框、道路地图或文本提示等条件输入；2) 训练2D视频扩散模型，以3D高斯渲染图像为条件增强外观细节；通过粗粒度3D场景指导2D视频扩散，实现准确相机轨迹控制。

Result: 在Waymo和KITTI-360两个真实世界数据集上的实验证明了方法的有效性，能够生成符合输入条件且保持相机轨迹准确性的高质量城市场景。

Conclusion: ScenDi通过结合3D和2D扩散模型的优势，解决了城市场景生成中细节保持与相机控制的权衡问题，为可控的高质量3D城市场景生成提供了有效解决方案。
...
TL;DR: 该研究提出了一种基于2D投影的端到端管道，用于3D CT扫描中颈椎骨折的自动检测，通过优化2D投影减少计算复杂度，结合YOLOv8定位、DenseNet121-Unet分割和2.5D时空序列模型集成，实现了高效的椎骨级骨折检测。


<details>
  <summary>Details</summary>
Motivation: 颈椎骨折是关键的医疗状况，需要精确高效的检测以进行有效的临床管理。传统3D分割方法计算复杂度高，本研究旨在探索基于2D投影的椎骨分割在3D CT体积中椎骨级骨折检测的可行性，以降低计算成本同时保持高性能。

Method: 1. 通过优化的轴向、矢状和冠状2D投影近似3D体积；2. 使用YOLOv8从所有视图中识别感兴趣区域并组合以近似3D颈椎区域；3. 采用基于方差和能量的投影，使用DenseNet121-Unet进行多标签分割；4. 从2D分割掩码战略性地近似3D椎骨掩码以提取单个椎骨体积；5. 使用包含原始切片和椎骨投影的2.5D时空序列模型集成进行骨折分析。

Result: 1. 3D mIoU达到94.45%；2. Dice分数为87.86%；3. 椎骨级和患者级F1分数分别为68.15和82.26；4. ROC-AUC分数分别为91.62和83.04；5. 通过可解释性研究提供显著性图可视化，并通过观察者间变异性分析显示与专家放射科医生相比具有竞争力的结果。

Conclusion: 该研究证明了基于2D投影的椎骨分割在3D CT体积中椎骨级骨折检测的可行性，提供了一种计算效率高且性能良好的端到端管道。该方法在减少计算复杂度的同时保持了高检测性能，并通过可解释性分析和与专家比较验证了其临床适用性。
...
<details>
  <summary>Details</summary>
Motivation: 单目RGB图像的语义场景补全具有固有模糊性，现有前馈方法在遮挡区域细节生成和空间关系保持方面存在不足，需要更准确的生成式推理能力以满足实际应用需求

Method: 将SSC视为条件生成问题，提出FlowSSC框架，引入Shortcut Flow-matching在紧凑三平面潜在空间中操作，通过捷径机制实现单步高保真生成，可与现有前馈方法无缝集成

Result: 在SemanticKITTI数据集上达到最先进性能，显著超越现有基线方法，实现实时推理而不牺牲质量

Conclusion: FlowSSC首次将生成式框架应用于单目语义场景补全，通过流匹配和捷径机制解决了传统方法在遮挡区域细节生成和空间关系保持方面的局限性，为自主系统提供了实用的部署方案
...
TL;DR: 论文提出了RBench基准测试和RoVid-X数据集，用于评估和提升机器人视频生成的质量，解决现有模型在物理真实性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型在具身智能中取得进展，但合成高质量、准确反映真实世界机器人交互的视频仍然具有挑战性。缺乏标准化基准限制了公平比较和进展，同时高质量训练数据的短缺也阻碍了物理真实性的提升。

Method: 1. 引入RBench基准：涵盖5个任务领域和4种不同具身形态，通过可复现的子指标（结构一致性、物理合理性、动作完整性）评估任务级正确性和视觉保真度。2. 开发RoVid-X数据集：采用四阶段数据管道，创建包含400万个标注视频片段的最大开源机器人视频生成数据集，涵盖数千个任务并包含全面的物理属性标注。

Result: 1. 评估25个代表性模型显示在生成物理真实机器人行为方面存在显著缺陷。2. RBench基准与人类评估的Spearman相关系数达到0.96，验证了其有效性。3. RoVid-X成为最大的开源机器人视频生成数据集，为模型训练提供了高质量数据基础。

Conclusion: 评估和数据的协同生态系统为视频模型的严格评估和可扩展训练建立了坚实基础，加速了具身AI向通用智能的演进。RBench揭示了现有模型的不足，而RoVid-X通过提供高质量训练数据来弥补这些缺陷。
...
TL;DR: DrivIng是一个大规模多模态自动驾驶数据集，包含完整的18公里路线地理参考数字孪生，支持系统化测试和仿真到真实评估。
...
Method: 构建了DrivIng数据集，包含约18公里路线（城市、郊区和高速公路）的完整地理参考数字孪生。提供6个RGB相机、1个LiDAR和高精度ADMA定位的连续记录，涵盖白天、黄昏和夜晚。所有序列以10Hz频率标注12个类别的3D边界框和轨迹ID。

Result: 数据集包含约120万个标注实例，支持真实交通到仿真的1对1转换，保留智能体交互同时实现真实灵活的场景测试。提供了数据集、数字孪生、高精地图和代码库的公开发布。
...
TL;DR: 提出一种基于迭代自校正的文本到图像生成方法，通过视觉语言模型作为批评器在推理时逐步优化生成结果，显著提升复杂组合提示的忠实度。
...
Method: 受大语言模型中思维链推理的启发，提出迭代测试时策略：文本到图像模型在多个步骤中逐步优化其生成结果，通过视觉语言模型作为循环中的批评器提供反馈指导。该方法简单，无需外部工具或先验知识，可灵活应用于各种图像生成器和视觉语言模型。

Result: 在多个基准测试中取得一致改进：ConceptMix（k=7）的全正确率提升16.9%，T2I-CompBench（3D-Spatial类别）提升13.8%，Visual Jenga场景分解提升12.5%。人类评估者58.7%的时间偏好该方法（基线为41.3%）。

Conclusion: 迭代自校正作为组合图像生成的广泛适用原则，通过将复杂提示分解为顺序校正，产生更忠实的生成结果，为提升文本到图像模型的组合能力提供了有效途径。
...
TL;DR: RayRoPE：一种用于多视角Transformer的新型位置编码方法，通过射线表示和预测3D点实现几何感知编码，支持SE(3)不变性注意力，并在新视角合成和立体深度估计任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有多视角注意力中的位置编码方案（绝对或相对）无法同时满足三个需求：1) 对图像块进行唯一编码；2) 支持具有多频相似性的SE(3)不变性注意力；3) 能够适应底层场景几何。需要一种新的编码机制来解决这一差距。

Method: RayRoPE基于关联射线表示图像块位置，但利用沿射线的预测点而非方向进行几何感知编码。为实现SE(3)不变性，使用查询帧投影坐标计算多频相似性。此外，考虑到预测的3D点可能不精确，RayRoPE提供了一种机制来解析计算不确定性下的期望位置编码。

Result: 在CO3D数据集的新视角合成任务中，RayRoPE相比替代位置编码方案实现了15%的相对改进（LPIPS指标）。RayRoPE还能无缝整合RGB-D输入，相比无法位置编码此类信息的替代方案获得更大增益。

Conclusion: RayRoPE是一种有效的多视角Transformer位置编码方法，能够满足几何感知、SE(3)不变性和适应性需求，在视觉任务中显著优于现有方案，并能有效利用深度信息进一步提升性能。
...
TL;DR: 提出StableWorld方法，通过动态帧驱逐机制解决交互式视频生成中的稳定性和时序一致性问题，防止误差累积导致的场景漂移和崩溃


<details>
  <summary>Details</summary>
Motivation: 当前交互式视频生成方法在长时间交互中存在严重的不稳定性和时序退化问题，导致空间漂移和场景崩溃，需要解决误差累积的根本原因
...
Conclusion: StableWorld是一种模型无关的简单有效方法，可应用于不同交互式视频生成框架，从根本上解决误差累积问题，实现更稳定的交互生成
...
TL;DR: 提出了一种从单次多视角室内场景捕捉中进行交互式光照编辑的新方法，通过生成式图像光照分解模型将复杂室内照明分解为独立光源，支持实时交互控制光源开关、色度和强度。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以从单次多视角捕捉中实现高质量的光照分解和交互式编辑，特别是对复杂室内场景的独立光源控制存在挑战。

Method: 使用生成式图像光照分解模型将室内场景照明分解为独立光源，引入多视角光照协调确保分解一致性，并集成到可重光照的3D高斯泼溅表示中实现实时交互控制。

Result: 在合成和真实数据集上验证了高度逼真的光照分解和重光照效果，通过定量和定性比较展示了优于现有技术的性能。

Conclusion: 该方法实现了从单次多视角捕捉中对室内场景光照的精确分解和实时交互编辑，为室内场景光照控制提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何让视频生成模型不仅能想象合理的未来，还能准确预测正确的未来，真实反映世界随动作变化的方式。利用互联网规模视频模型的丰富世界先验，避免从头训练。

Method: 提出EgoWM方法，通过轻量级条件层将运动指令注入预训练视频扩散模型，将其转换为动作条件世界模型。引入结构一致性评分（SCS）独立于视觉外观评估物理正确性。

Result: EgoWM在导航世界模型中比先前最先进方法提升SCS达80%，推理延迟降低6倍，在未见环境中（包括绘画内部导航）具有鲁棒泛化能力，能生成导航和操作任务的连贯推演。

Conclusion: EgoWM成功将预训练视频扩散模型转化为动作条件世界模型，通过轻量级条件层注入运动指令，实现了准确、可控的未来预测，在多种具身系统中展现出强大的泛化能力和效率优势。
...
TL;DR: 该研究探索了多种量化方法（包括GPTQ和AWQ）在多模态管道中的应用，分析了量化对图像描述、检索和问答任务性能的影响，发现ViT和LLM在模型性能中具有可比重要性，且LLM的低比特量化能在减少权重位宽的同时保持高精度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要高性能GPU和大量内存，为降低内存和延迟，研究者通常对参数进行量化。现有研究主要关注单模态模型的量化，而多模态管道（包含视觉模型、语言模型及其连接器）的量化效果尚未充分探索。

Method: 研究采用多种量化方法（包括GPTQ和AWQ）对多模态管道进行量化，分析不同比特宽度、量化方法以及量化应用于管道不同部分（视觉模型、语言模型或连接器）对性能的影响，评估任务包括图像描述、检索和问答。

Result: 结果显示：1）ViT和LLM在模型性能中具有可比重要性，尽管参数规模差异显著；2）LLM的低比特量化能在减少权重位宽（bpw）的同时实现高精度；3）量化方法、比特宽度和量化应用位置对多模态任务性能有显著影响。

Conclusion: 该研究为多模态大模型的高效部署提供了实用见解，强调了对多模态模型组件敏感性进行探索的重要性，代码已开源供进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 换脸任务面临两个主要挑战：1)缺乏真实的地面真值数据，难以同时实现准确的身份转换和高质量的属性保留；2)现有的基于扩散模型的方法通过在掩码目标图像上进行条件修复，但掩码条件移除了目标的关键外观线索，导致属性对齐不准确。

Method: APPLE采用师生框架：1)将换脸重新定义为条件去模糊任务，更忠实地保留光照、肤色、妆容等目标特定属性；2)引入属性感知反演方案进一步改善详细属性保留；3)通过精心设计的属性保留教师学习，生成高质量的伪三元组，为学生提供直接的换脸监督。
...
TL;DR: 该论文提出了一种基于模式约束的AI提取系统，能够将生物医学PDF全文转换为结构化、可分析的数据记录，通过类型化模式、受控词汇和证据门控决策来限制模型推理，支持可追溯性和事后审计。


<details>
  <summary>Details</summary>
Motivation: 生物医学证据合成依赖于从全文研究文章中准确提取方法学、实验室和结果变量，但这些变量嵌入在复杂的科学PDF中，使得手动提取耗时且难以扩展。现有文档AI系统受限于OCR错误、长文档碎片化、吞吐量受限以及高风险合成所需的审计能力不足。

Method: 开发了一个模式约束的AI提取系统，使用恢复感知哈希摄取文档，分割为标题感知的页面级块，并在显式并发控制下异步处理。块级输出通过冲突感知合并、基于集合的聚合和句子级溯源确定性地合并为研究级记录。

Result: 在直接口服抗凝剂水平测量研究语料库上评估，管道无需人工干预处理所有文档，在服务约束下保持稳定吞吐量，并在文档块间表现出强内部一致性。迭代模式细化显著提高了合成关键变量的提取保真度，包括测定分类、结果定义、随访持续时间和测量时间。
...
TL;DR: 本文提出多轮压力测试框架，评估LLM在心理健康支持长对话中的安全边界侵蚀问题，发现自适应探测显著加速边界突破


<details>
  <summary>Details</summary>
Motivation: 当前LLM心理健康支持的安全评估主要关注单轮对话中的禁止词检测，忽视了长对话中安全边界的渐进式侵蚀问题。主流LLM已能过滤明显风险词，但真正的危险在于多轮互动中LLM为提供安慰和共情而逐步突破边界的行为

Method: 提出多轮压力测试框架，对三个前沿LLM进行长对话安全测试。采用两种压力方法：静态递进和自适应探测。生成50个虚拟患者档案，通过最多20轮虚拟精神病学对话对每个模型进行压力测试

Result: 违规行为普遍存在，两种压力模式产生相似违规率。但自适应探测显著提前模型突破边界的时间，平均轮数从静态递进的9.21轮减少到4.64轮。两种机制下，做出确定性或零风险承诺是边界突破的主要方式
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在解决需要多步推理的问题上表现出色，但其实现这种能力的内部机制仍然不明确。现有综述主要关注提升性能的工程方法，缺乏对底层机制的深入理解。

Method: 围绕七个相互关联的研究问题构建概念框架：从LLM如何在隐藏激活中执行隐式多跳推理，到语言化显式推理如何重塑内部计算过程。

Result: 提出了一个全面的机制性分析框架，系统梳理了LLM多步推理的内部工作机制，为理解这些模型的认知能力提供了理论基础。

Conclusion: 需要更多机制性研究来深入理解LLM的多步推理能力，并指出了五个未来研究方向，以推动这一领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育多选题自动生成中产生的幻觉（流畅但不正确或不连贯的输出）是一个重大挑战，需要系统性的解决方案

Method: 提出无幻觉多智能体生成框架，将多选题生成分解为离散可验证阶段，使用基于规则和LLM的检测智能体，以及幻觉评分指标来优化问题质量。将多选题生成重新定义为最小化幻觉风险同时最大化有效性、可回答性和成本效益的优化任务，并引入基于反事实推理和思维链的智能体引导精炼过程
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在细粒度、大规模评估方面存在局限，而基础模型理解研究论文仍面临挑战，因为科学论述专业且图表复杂，需要更全面的评估工具。

Method: 1) 从高质量计算机科学论文的审稿-回复交流构建基准；2) 设计符合科学研究流程的细粒度分类法；3) 建立LLM-人类交互标注框架支持大规模标注；4) 基于LLM-as-a-Judge范式开发可扩展评估框架，评估正确性-完整性和简洁性。

Result: 最强模型(GPT-5)仅达到68.2%的正确性-完整性，经过简洁性调整后降至37.46%，表明在精确学术论文理解方面存在显著差距。

Conclusion: RPC-Bench填补了现有基准的空白，揭示了当前模型在学术论文理解方面的实质性不足，为未来研究提供了重要的评估工具和方向。
...
TL;DR: 提出验证器引导蒸馏方法，通过训练小型语言模型学习错误修复过程（包括冲突检测和回溯），而不仅仅是最终正确答案，使小模型具备自我验证和纠错能力。


<details>
  <summary>Details</summary>
Motivation: 小型语言模型（参数少于100亿）适合私有、设备端部署，但在严格约束满足问题上经常失败，因为它们采用线性、过度自信的推理轨迹，无法从早期错误中恢复。

Method: 引入验证器引导蒸馏训练协议，通过训练7B参数模型学习经过验证的推理轨迹（包含错误和自我纠正），转移错误修复过程（显式冲突检测和回溯），而不仅仅是正确的最终答案。

Result: 研究表明，小型模型中可以出现潜在的验证行为，使它们能够偶尔停止、检测矛盾并修正早期假设，从而在约束满足问题上表现更好。

Conclusion: 通过训练小型语言模型学习包含错误和纠正的推理过程，而不仅仅是最终答案，可以使它们获得自我验证和错误修复能力，提高在严格约束满足问题上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 自回归音频生成模型虽然能产生时间连贯的音频，但在遵循复杂文本提示（特别是描述复杂声音事件时）方面表现不佳。研究发现自回归模型的早期前缀令牌隐式编码了最终输出的全局语义属性，这揭示了隐式规划的能力。

Method: 提出Plan-Critic模型：1）利用GAE启发的目标训练轻量级辅助模型，从部分生成中预测最终指令跟随质量；2）推理时进行引导探索：评估候选前缀，剪枝低保真轨迹，将计算资源重新分配给高潜力规划种子；3）保持与标准best-of-N解码相同的计算复杂度。

Result: Plan-Critic引导的采样在CLAP分数上比自回归基线提升高达10分，在自回归文本到音频生成中建立了新的最先进水平，同时保持与标准best-of-N解码相同的计算效率。

Conclusion: 这项工作弥合了因果生成与全局语义对齐之间的差距，证明即使是严格的自回归模型也能够进行前瞻性规划。Plan-Critic通过早期评估生成质量实现了更有效的指令跟随。
...
TL;DR: 该研究分析了说话人嵌入与语言学驱动的音系规则在口音语音合成中的交互作用，提出音素转换率（PSR）来量化嵌入对规则转换的保留或覆盖程度，发现结合规则与嵌入能产生更真实的口音。


<details>
  <summary>Details</summary>
Motivation: 当前TTS系统通过特定口音的说话人嵌入来生成带口音的语音，这种方法虽然有效但可解释性和可控性有限，因为嵌入同时编码了音色、情感等多种特征。需要研究说话人嵌入与语言学规则在口音合成中的交互关系。

Method: 以美式和英式英语为案例研究，实现flapping、rhoticity和元音对应等音系规则。提出音素转换率（PSR）这一新指标来量化嵌入对基于规则的转换的保留或覆盖强度。通过实验分析规则与嵌入的结合效果。

Result: 实验表明，结合规则与嵌入能产生更真实的口音，同时发现嵌入可以减弱或覆盖规则，揭示了口音与说话人身份之间的纠缠关系。PSR指标有效量化了这种交互作用。

Conclusion: 音系规则可作为口音控制的有效杠杆，同时该研究框架为评估语音生成中的解纠缠提供了方法论基础，有助于开发更具可解释性和可控性的口音合成系统。
...
TL;DR: 提出一个模型和任务无关的框架，用于设计人-LLM协作的定性分析方法，并在多中心糖尿病护理研究中应用，证明LLM能提高定性分析效率同时保持严谨性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在提高大规模、多地点健康服务研究的定性分析效率方面显示出潜力，但目前缺乏将LLM整合到定性分析的方法学指导，以及它们对现实世界研究方法和结果影响的证据。

Method: 开发了一个模型和任务无关的框架，用于设计人-LLM协作的定性分析方法。在联邦合格健康中心（FQHCs）的多中心糖尿病护理研究中应用该框架，实现了两种人-LLM方法：(1) 对研究者生成的摘要进行定性综合以产生比较反馈报告；(2) 对167份访谈转录本进行演绎编码以完善实践转化干预。

Result: LLM辅助实现了对从业者的及时反馈，并整合了大规模定性数据来指导理论和实践变革。LLM的整合增强了效率，同时保持了研究的严谨性。

Conclusion: 这项工作展示了如何将LLM整合到应用健康服务研究中，以提高效率同时保持严谨性，为在定性研究中继续创新使用LLM提供了指导。
...
TL;DR: LLMs经过微调后能在统计任务上达到统计学学生水平，且能有效评估答案质量，适用于教育技术和统计分析辅助系统。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在NLP任务中表现出色，但其处理复杂统计问题的能力尚不明确，需要评估LLMs在统计推理方面的潜力及其在教育技术、统计分析辅助等领域的应用价值。

Method: 对选定的开源LLMs在专门开发的统计数据集上进行微调，并与人类评分基准进行比较，同时评估LLMs自身作为答案质量评估工具的能力。

Result: 微调后的模型在高级统计任务上达到统计学学生水平，性能提升具有架构依赖性；LLMs在答案质量评估（包括解释和推理评估）方面优于传统指标如BLEU或BertScore。

Conclusion: LLMs通过微调可有效提升统计推理能力，其自我评估能力为统计教育平台和自动化分析工具提供了可扩展的质量评估方案，在学术和工业领域具有广泛应用潜力。
...
TL;DR: 论文研究了通过执行反馈来提升AI自动化研究的可行性，构建了自动执行器验证LLM生成的想法，并在预训练和后训练任务上证明了执行引导的进化搜索比强化学习更有效。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在自动化AI研究中经常生成看似合理但实际无效的想法，需要验证自动执行的可行性以及LLM能否从执行反馈中学习，以加速科学发现。

Method: 构建自动执行器实现想法并启动大规模并行GPU实验验证效果；将LLM预训练和后训练转化为执行环境；分析两种从执行反馈学习的方法：进化搜索和强化学习。

Result: 自动执行器能实现前沿LLM生成的大部分想法；执行引导的进化搜索样本效率高：在后训练任务上显著超越GRPO基线（69.4% vs 48.0%），在预训练任务上超越nanoGPT基线（19.7分钟 vs 35.9分钟）；前沿LLM在搜索中能生成有意义的算法想法但容易早期饱和；基于执行奖励的强化学习存在模式崩溃问题。

Conclusion: 执行引导的进化搜索是实现自动化AI研究的有效方法，而强化学习存在局限性；前沿LLM能生成有意义的想法但需要更好的反馈机制来避免早期饱和；为基于执行反馈的自动化AI研究提供了深入分析。
...
### [74] [Multi-Agent Constraint Factorization Reveals Latent Invariant Solution Structure](https://arxiv.org/abs/2601.15077)
...
Method: 基于算子理论和约束优化建立形式化模型，将每个智能体建模为在共享解状态上执行不同有效性约束的算子，证明多智能体系统实现了约束执行算子的分解组合

Result: 在温和条件下，这些动态收敛到由智能体约束集交集定义的解集，这种不变结构通常无法被单个智能体同时应用所有约束时动态访问，即使表达能力和信息相同
...
TL;DR: LLMs存在与人类相似的认知偏见，无法有效模拟反事实决策来抵消性别和种族偏见，但通过访问自身API的盲化副本可以实现更公平的决策


<details>
  <summary>Details</summary>
Motivation: 公平决策需要忽略无关的、可能带有偏见的信息，这要求决策者能够模拟在不知道某些事实（如性别或种族）的情况下会做出什么决策。人类难以进行这种反事实自我模拟，导致即使善意行为者也会产生偏见判断。研究旨在探索LLMs是否面临类似限制，以及如何利用其独特能力解决这一问题。

Method: 研究通过实验展示LLMs在抵消性别和种族偏见以及克服奉承行为方面的局限性。测试了提示模型忽略或假装不知道偏见信息的方法，发现这些方法效果有限甚至适得其反。然后提出利用LLMs可以访问自身API的特性，通过盲化副本（blinded replica）提供真实的反事实认知模型，从而实现更公平的决策。

Result: 研究发现：1）LLMs与人类相似，难以进行反事实自我模拟来抵消偏见；2）简单的提示方法（如忽略或假装不知道偏见信息）无法有效抵消偏见，有时甚至会产生反效果；3）通过访问自身API的盲化副本，LLMs能够做出更公平的决策，同时提供更高的透明度来区分隐性偏见和有意偏见行为。

Conclusion: LLMs在反事实决策方面存在与人类相似的认知限制，但通过利用其独特的API访问能力，可以创建盲化副本来实现更公平的决策。这种方法不仅提高了决策公平性，还增强了透明度，有助于区分无意识的隐性偏见和故意的偏见行为，为构建更公平的AI系统提供了新途径。
...
TL;DR: 提出PedagogicalRL-Thinking框架，通过教育理论引导的内部推理提示和思维奖励机制，优化LLM在智能辅导系统中的教学推理过程，提升教学效果。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为智能辅导系统部署日益增多，但针对教育场景的优化研究有限。现有强化学习方法仅优化可见响应，忽略了模型的内部思维过程，需要专门针对教育场景的教学推理进行优化。

Method: 提出PedagogicalRL-Thinking框架，包含两个创新方法：1) 教学推理提示：使用特定领域教育理论而非通用指令引导内部推理；2) 思维奖励：明确评估和强化模型推理轨迹的教学质量。通过数学辅导对话训练模型，评估其在未见教育基准测试上的表现。

Result: 实验表明：1) 基于特定领域理论的提示优于通用提示；2) 思维奖励与教学提示结合效果最佳；3) 仅在数学辅导对话上训练的模型在未见教育基准测试上表现提升，同时保留基础模型的事实知识；4) 教学思维奖励产生系统性推理轨迹变化，增加教学推理和结构化教学决策。

Conclusion: PedagogicalRL-Thinking框架通过优化LLM的内部推理过程，显著提升了智能辅导系统的教学效果，证明了教学对齐扩展到推理LLM在教育领域的可行性，为教育场景的LLM优化提供了新方向。
...
TL;DR: Social Caption框架基于互动理论评估多模态大语言模型的社会理解能力，包括社会推理、整体社会分析和定向社会分析三个维度，并分析模型规模、架构设计和口语上下文等因素的影响。
...
Method: 提出Social Caption框架，基于互动理论从三个维度评估MLLMs的社会理解能力：社会推理（SI）、整体社会分析（HSA）和定向社会分析（DSA）。分析影响模型性能的因素，包括模型规模、架构设计和口语上下文，并使用MLLM法官进行实验。

Result: 实验提供了关于多模态社会理解自动评估扩展的见解，分析了不同因素对模型在社会理解任务上性能的影响。

Conclusion: Social Caption为评估多模态大语言模型的社会理解能力提供了一个系统框架，有助于理解模型在解释人类社交互动方面的能力及其影响因素。
...
TL;DR: 研究发现大型语言模型存在因分词器导致的推理脆弱性：多个不同的token ID序列可能对应相同的文本表面形式，造成模型将语义相同的内部表示视为不同"词语"，导致推理失败。


<details>
  <summary>Details</summary>
Motivation: 现代子词分词器会产生非唯一编码（多个token ID序列可解码为相同的表面字符串），这种表示不匹配造成了未被测量的脆弱性，可能导致推理过程失败。LLMs可能将两个内部表示视为不同的"词语"，尽管它们在文本层面语义相同。

Method: 引入分词一致性探针任务：要求模型在上下文中替换指定的目标词，同时保持其他内容不变。该任务在表面层面故意设计简单，以便将失败归因于分词器-解码器伪影而非知识差距或参数限制。分析了超过11000个替换试验。

Result: 在最先进的开源LLMs中发现非平凡比例的幻影编辑：模型在正确推理的幻觉下操作，这是分词器诱导的表示缺陷现象。进一步分析并提供了八种系统性分词器伪影的分类，包括空格边界偏移和词内重新分割。
...
TL;DR: ClaimDB是首个基于大规模结构化数据的事实核查基准，包含80个真实数据库，要求通过可执行程序推理而非简单阅读证据进行验证，现有LLM模型在该基准上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有事实核查基准主要关注文本证据，而基于大规模结构化数据（数百万记录和多表组合）的声明验证研究不足，需要开发能够处理复杂结构化数据推理的系统。

Method: 构建ClaimDB基准，包含80个独特真实数据库，涵盖治理、医疗、媒体、教育、自然科学等多个领域。通过30个最先进的专有和开源LLM进行广泛实验，评估模型在结构化数据事实核查中的表现。

Result: 所有测试模型准确率均未超过83%，超过一半模型低于55%。闭源和开源模型在弃权能力（承认缺乏证据做出判断）上都表现不佳，对其在高风险数据分析中的可靠性提出质疑。

Conclusion: 大规模结构化数据的事实核查需要从"阅读"证据转向可执行程序推理，现有LLM在该任务上仍有很大提升空间，特别是在弃权能力方面。ClaimDB基准的发布将推动该领域研究。
...
TL;DR: DARL是一个强化学习框架，旨在解决现有RLVR方法在开放领域任务中过度拟合参考答案、缺乏输出多样性的问题，通过鼓励在可控偏差范围内生成多样化答案来提升推理性能和输出多样性。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法依赖领域特定验证器，限制了在开放通用领域的应用。虽然RLPR等方法扩展了RLVR到通用领域，但它们倾向于过度拟合参考答案，限制了模型生成多样化输出的能力，这在写作等开放任务中尤为明显。
...
Conclusion: DARL有效解决了现有强化学习方法在开放领域任务中过度拟合参考答案的问题，通过鼓励多样化输出同时保持对齐，显著提升了推理性能和输出多样性，且无需额外验证器即可与现有方法兼容。
...
<details>
  <summary>Details</summary>
Motivation: 传统Chain-of-Thought (CoT) 提示虽然能增强大语言模型的推理能力，但其冗长的文本输出带来显著计算开销，且缺乏对中间推理过程的监督，导致潜在推理链难以分析。

Method: 提出Render-of-Thought (RoT) 框架，将文本推理步骤渲染为图像，使潜在推理过程显式化和可追踪。利用现有视觉语言模型(VLMs)的视觉编码器作为语义锚点，将视觉嵌入与文本空间对齐，实现即插即用而无需额外预训练。

Result: 在数学和逻辑推理基准测试中，RoT相比显式CoT实现了3-4倍的token压缩和显著的推理加速，同时与其他方法保持竞争性性能，验证了该范式的可行性。

Conclusion: RoT通过将文本推理链渲染为图像，有效解决了CoT的计算开销和可分析性问题，为高效可追踪的推理提供了新范式。
...
TL;DR: 该研究提出了PsyFIRE框架和RECAP系统，用于在文本心理咨询中检测和分类13种细粒度抵抗行为，并构建了包含23,930条标注语料的中文数据集，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有NLP方法在检测心理咨询中的客户抵抗行为时存在三个主要问题：过度简化抵抗类别、忽略治疗干预的序列动态性、以及可解释性有限。这些问题限制了在文本交互中有效识别和应对抵抗行为的能力。

Method: 研究提出了PsyFIRE理论框架，定义了13种细粒度抵抗行为及协作互动。基于此构建了ClientResistance语料库，包含23,930条来自真实中文文本心理咨询的标注话语，每条都有上下文特定的理由说明。在此基础上开发了RECAP两阶段框架：第一阶段检测抵抗与协作，第二阶段分类细粒度抵抗类型并提供解释。

Result: RECAP在区分协作与抵抗方面达到91.25%的F1分数，在细粒度抵抗类别分类方面达到66.58%的宏平均F1分数，比基于提示的大型语言模型基线高出20多个百分点。在独立咨询数据集和62名咨询师的试点研究中，RECAP揭示了抵抗行为的普遍性及其对治疗关系的负面影响，并展示了改善咨询师理解和干预策略的潜力。

Conclusion: 该研究通过理论驱动的细粒度标注框架和两阶段检测系统，显著提升了文本心理咨询中抵抗行为的识别能力。RECAP不仅性能优越，还具有实际应用价值，能够帮助咨询师更好地理解和应对客户抵抗，从而改善治疗效果。
...
TL;DR: 构建首个中文电影剧本续写基准，比较GPT-5.2和Qwen-Max在53部经典电影续写任务上的表现，GPT-5.2在结构保持、整体质量和综合评分上显著优于Qwen-Max。
...
Method: 构建包含53部经典电影的中文剧本续写基准，采用"前半部分续写后半部分"范式，每部电影生成3个样本。设计多维评估框架，结合ROUGE-L、结构相似性和LLM-as-Judge评分（使用DeepSeek-Reasoner），对303个有效样本进行统计分析。
...
<details>
  <summary>Details</summary>
Motivation: 现有MRAG方法采用统一的"一刀切"策略，在多语言环境中效果不佳，因为模型在与搜索引擎交互时会出现知识偏见和冲突。需要更有效的多语言检索增强生成方法。

Method: 提出LcRL框架，整合语言耦合组相对策略优化到策略和奖励模型中。采用语言耦合组采样减少知识偏见，在奖励模型中正则化辅助反一致性惩罚缓解知识冲突。

Result: 实验结果表明LcRL不仅实现了有竞争力的性能，而且适用于多种实际场景，如受限训练数据和包含大量语言的检索集合。

Conclusion: LcRL通过解决知识偏见和冲突问题，为多语言检索增强生成提供了有效的强化学习框架，在各种实际应用场景中表现出色。
...
TL;DR: PodBench：一个包含800个样本的播客脚本生成基准测试，输入最长21K tokens，支持多说话者指令，结合定量约束和LLM质量评估进行综合评测


<details>
  <summary>Details</summary>
Motivation: 播客脚本生成需要LLM从多样化输入中合成结构化、基于上下文的对话，但目前该任务的系统性评估资源有限，需要建立标准化评测框架

Method: 提出PodBench基准测试，包含800个样本，输入最长21K tokens，支持复杂多说话者指令；设计多维度评估框架，整合定量约束和基于LLM的质量评估

Result: 实验表明：专有模型整体表现优异，但配备显式推理能力的开源模型在处理长上下文和多说话者协调方面比标准基线更具鲁棒性；同时发现指令遵循度高不一定保证内容质量高

Conclusion: PodBench为长格式、音频中心生成任务提供了可复现的测试平台，揭示了指令遵循与内容质量之间的差异，为未来研究指明了方向
...
TL;DR: CodeDelegator是一个多智能体框架，通过角色专业化将规划与实现分离，使用持久化Delegator进行战略监督，为每个子任务实例化新的Coder智能体，并引入EPSS机制隔离执行状态，防止调试痕迹污染上下文。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型允许智能体将动作表示为可执行代码，比传统工具调用更具表达性。但现实世界任务需要战略规划和详细实现，单一智能体同时处理两者会导致调试痕迹和中间失败污染上下文，损害长视野性能。

Method: 提出CodeDelegator多智能体框架：持久化Delegator负责战略监督、任务分解、编写规范和监控进度但不执行代码；为每个子任务实例化新的Coder智能体，其上下文仅包含规范，隔离先前失败；引入Ephemeral-Persistent State Separation (EPSS)机制协调智能体，隔离每个Coder的执行状态同时保持全局一致性。
...
TL;DR: 论文提出Corpus Clarification预处理框架，用于将大规模公民协商数据转化为结构化论证单元，并创建GDN-CC数据集，证明小型语言模型在标准化任务上可匹敌大型模型。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在NLP中广泛应用，但将其用于民主活动文本分析存在伦理问题。研究旨在：(1)开发标准化公民贡献的资源，使其更易用于主题建模和政治分析；(2)研究小型开源LLMs能否可靠执行这种标准化。

Method: 提出Corpus Clarification预处理框架，将嘈杂的多主题贡献转化为结构化、自包含的论证单元。创建GDN-CC数据集（1,231个法国大辩论贡献，2,285个论证单元），标注论证结构并手动澄清。微调小型语言模型进行标注，并与LLMs比较性能。

Result: 微调后的小型语言模型在复制这些标注方面匹配或优于大型LLMs。在意见聚类任务中验证了其实用性。发布了GDN-CC-large数据集（24万自动标注贡献），是目前最大的标注民主协商数据集。

Conclusion: Corpus Clarification框架能有效预处理大规模协商数据，小型语言模型在标准化任务上表现优异，为民主文本分析提供了透明、可本地运行的解决方案，并贡献了大规模标注数据集。
...
TL;DR: 该论文对现代语言模型在僧伽罗语（包括Unicode和罗马化两种形式）上的性能进行了全面基准测试，发现不同模型在不同脚本上的表现存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 僧伽罗语等资源较少、形态丰富的语言在语言模型中的性能研究不足，特别是数字通信中普遍使用的罗马化僧伽罗语。需要评估现代语言模型在这两种脚本上的表现，为实际应用提供指导。

Method: 使用多样化的Unicode和罗马化僧伽罗语语料库进行基准测试。对开源模型采用困惑度评估，对领先的闭源模型通过句子完成的定性分析进行评估。

Result: Mistral-Nemo-Base-2407在Unicode文本上预测性能最强，Mistral-7B-v0.3在罗马化文本上表现最佳。Llama-3.1-8B在两种脚本上都表现出色。闭源模型中，Gemini-1.5-pro和DeepSeek擅长Unicode生成，Claude-3.5-Sonnet在罗马化文本处理上更优。
...
TL;DR: 该研究探讨了多语言环境下LLM的数据污染问题，发现翻译成阿拉伯语会掩盖传统污染指标，但模型仍能从污染数据中获益，并提出基于多翻译变体比较的污染检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有污染检测方法主要局限于英语基准测试，对多语言污染的理解不足。数据污染会破坏LLM评估的有效性，使模型依赖记忆而非真正的泛化能力。需要研究多语言环境下的污染动态。

Method: 通过在不同比例的阿拉伯语数据集上微调多个开源LLM，并在原始英语基准上评估。扩展了Tested Slot Guessing方法，采用选择重排序策略，并结合Min-K%概率分析来捕捉行为和分布污染信号。提出了Translation-Aware Contamination Detection方法，通过比较多个翻译基准变体而非仅英语来识别污染。

Result: 翻译成阿拉伯语会抑制传统污染指标，但模型仍能从污染数据中获益，特别是具有更强阿拉伯语能力的模型。随着污染水平增加，Min-K%分数上升，跨语言答案一致性提高。提出的Translation-Aware Contamination Detection方法即使在仅英语方法失败时也能可靠地暴露污染。

Conclusion: 研究强调了需要多语言、翻译感知的评估流程，以确保LLM评估的公平性、透明性和可重复性。Translation-Aware Contamination Detection方法能有效检测传统方法遗漏的多语言污染。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的开放域关系三元组抽取方法依赖静态启发式提示策略，缺乏反思机制来内化错误信号，导致在语义模糊性面前表现脆弱，错误抽取模式难以纠正

Method: 提出知识重构驱动的提示优化框架，包含：1）基于知识恢复的自评估机制，将结构化三元组投影为语义一致性分数提供内在反馈信号；2）基于文本梯度的提示优化器，内化历史经验迭代优化提示；3）关系规范化记忆模块，收集代表性关系并提供语义不同的模式

Result: 在三个数据集上的广泛实验表明，KRPO在抽取F1分数上显著优于强基线方法

Conclusion: KRPO框架通过知识重构驱动的自评估和提示优化，有效提升了大语言模型在复杂开放域关系三元组抽取任务中的持续改进能力
...
TL;DR: LogicScore：一个基于Horn规则的评估框架，用于评估长格式问答中的全局逻辑完整性，弥补现有方法只关注孤立语句验证的局限性


<details>
  <summary>Details</summary>
Motivation: 当前归因问答（AQA）评估方法存在"归因近视"问题：过度强调孤立语句及其归因的验证，而忽视了长格式答案的全局逻辑完整性，导致LLM产生事实基础扎实但逻辑不连贯的响应

Method: 提出LogicScore评估框架，基于Horn规则，采用后向验证机制，系统评估三个关键推理维度：完整性（逻辑合理的演绎）、简洁性（非冗余性）和确定性（一致的答案蕴含）

Result: 在三个多跳QA数据集（HotpotQA、MusiQue和2WikiMultiHopQA）和超过20个LLM上的实验显示：领先模型虽然获得高归因分数（如Gemini-3 Pro精度达92.85%），但在全局推理质量上表现不佳（如Gemini-3 Pro简洁性仅35.11%）

Conclusion: LogicScore为逻辑评估建立了稳健标准，强调在LLM开发中需要同时优先考虑推理连贯性和事实基础，揭示了当前模型在全局推理能力上的关键差距
...
TL;DR: 通过分析Reddit大规模数据，研究发现语言使用的语义探索-利用存在昼夜节律性：早晨局部语义探索达到高峰，而全局语义多样性在一天中后期达到峰值，这与神经调节系统的昼夜模式一致。


<details>
  <summary>Details</summary>
Motivation: 人类认知受昼夜节律调节，但其对高维语义行为的影响尚不清楚。本研究旨在量化语言使用的时间变化，探索语义探索-利用平衡的昼夜节律模式。

Method: 使用大规模Reddit数据，通过预训练transformer模型将文本嵌入语义空间，测量语义熵作为语言探索-利用的指标。区分局部和全局语义熵，分析其昼夜变化模式。

Result: 发现语义探索-利用存在稳健的昼夜节律性，可被季节性光照线索调节。局部语义探索在早晨达到高峰，反映更广泛的语义空间探索；全局语义多样性在一天中后期达到峰值，符合"富者愈富"动态。这些模式不能由情感或情感效价解释。

Conclusion: 观察到的时空结构与已知的神经调节系统昼夜模式一致，表明生物昼夜节律延伸到语义领域，语义探索捕捉了与情绪不同的认知维度。
...
TL;DR: 开发了一个包含200张胸部X光片、12个标注标签的基准数据集，采用AI辅助专家标注流程，其中100张公开，100张作为保留集用于独立评估模型。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在放射学选择题考试中表现与放射科住院医师相当，但开发临床有用的多模态LLM工具需要领域专家策划的高质量基准数据集。

Method: 使用13,735张去识别化胸部X光片及报告；GPT-4o提取报告中的异常发现，通过本地LLM映射到12个基准标签；基于AI建议标签采样1,000张供专家评审；17位胸部放射科医生参与评估，每张影像由3位专家评审；最终选择200张影像（其中381张至少2位专家完全同意）创建基准数据集。

Result: 创建了包含200张胸部X光片、12个基准标签的公开数据集（https://imaging.rsna.org），其中100张公开发布，100张作为保留集由RSNA独立评估模型。开发了AI辅助标注流程，支持大规模标注并减少遗漏。

Conclusion: 成功建立了经过放射科专家验证的高质量胸部X光基准数据集，并开发了高效的AI辅助标注方法，为开发临床有用的多模态LLM工具提供了重要资源。
...
TL;DR: 提出检索增强多智能体框架，自动化生成面向具体实例的评估标准，用于临床决策支持中LLM的安全评估与改进


<details>
  <summary>Details</summary>
Motivation: LLM在临床决策支持中可能产生幻觉和不安全建议，直接威胁患者安全。这些风险表现为难以被通用指标检测的细微临床错误，而专家编写的细粒度评估标准成本高且难以扩展

Method: 检索增强多智能体框架，通过分解检索的权威医学证据为原子事实，结合用户交互约束，生成可验证的细粒度评估标准

Result: 在HealthBench上，临床意图对齐(CIA)得分60.12%，显著优于GPT-4o基线(55.16%)。判别测试中，平均得分差μ_Δ=8.658，AUROC=0.977，几乎是GPT-4o基线(4.972)的两倍。评估标准还能指导响应优化，质量提升9.2%(从59.0%到68.2%)

Conclusion: 该框架为医学LLM的评估和改进提供了可扩展且透明的基础，能够自动化生成细粒度评估标准，有效检测临床错误并指导模型优化
...
TL;DR: 该研究提出了一种基于证据的审稿质量比较框架，应用于AI/ML会议，发现审稿质量并未如普遍认知那样持续下降


<details>
  <summary>Details</summary>
Motivation: 随着投稿数量增加和研究社区扩大，审稿质量下降成为普遍担忧，但缺乏可靠的测量方法和跨时间、跨会议的比较框架

Method: 提出新的审稿质量比较框架，包括审稿格式标准化、多维质量量化模式（对编辑和作者的效用），结合LLM和轻量级测量方法，应用于ICLR、NeurIPS和*ACL会议

Result: 跨时间分析显示，在研究的会议和年份中，审稿质量中位数没有一致的下降趋势，与普遍认知相反

Conclusion: 审稿质量并未普遍下降，研究提出了替代解释和促进未来审稿质量实证研究的建议
...
TL;DR: 该研究将基于事实要点(nugget)的评估方法从自动评估扩展到用户端，开发了一个原型系统帮助法律专业人士评估和改进摘要质量


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于总结长文档，但在法律领域，摘要的事实准确性至关重要。现有的基于要点的方法主要用于自动评估，但其支持终端用户的潜力尚未充分探索

Method: 将基于事实要点的方法从评估端转换到用户端，开发了一个原型系统，专注于法律领域，支持两种具体场景：比较两个摘要的优劣，以及手动改进自动生成的摘要

Result: 提出了一个利用基于事实要点的方法来支持法律专业人士的原型系统，展示了该方法在用户端的应用潜力

Conclusion: 基于事实要点的方法不仅可用于自动评估，还能有效支持终端用户，特别是在法律领域，可以帮助专业人士评估和改进摘要质量
...
<details>
  <summary>Details</summary>
Motivation: 传统语言模型将文本视为单一全局分布进行训练，导致地理行为同质化。研究者希望探索一种轻量级方法来实现语言模型的本地化，使其能够更好地适应特定地理区域的特点。

Method: 使用带有验证URL、国家标签和大陆标签的大规模英文新闻数据进行预训练，覆盖4个大洲和17个国家。训练了31个模型（0.5B和1B参数规模），通过元数据条件化实现本地化。进行了四项对照实验，包括消融研究，并引入了包含800个本地化新闻多选题的下游基准测试。

Result: 元数据条件化能持续提升区域内性能而不牺牲跨区域泛化能力；使全局模型能够达到与区域特定模型相当的本地化水平；提高学习效率。URL级元数据能捕获大部分地理信号，但平衡的区域数据覆盖仍然至关重要。经过指令微调后，元数据条件化的全局模型在准确性上可与LLaMA-3.2-1B-Instruct相媲美，尽管训练数据量少得多。

Conclusion: 元数据条件化是一种实用且计算效率高的语言模型本地化方法，能够在保持全局泛化能力的同时实现有效的区域适应。
...
TL;DR: LLMs在标准数字任务上表现优异，但当数字表达偏离训练语料中的主流格式时，准确率显著下降；通过针对性提示策略可以缩小这一差距。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在不同数字脚本和格式下的数值推理能力，关注当数字表达偏离训练语料中的主流约定时模型的性能变化。

Method: 通过多种数字脚本和格式测试LLMs的数值推理能力，评估模型在非主流数字表达下的表现，并尝试使用少样本提示和显式数字映射等针对性提示策略来改善性能。

Result: LLMs在非主流数字脚本或格式下的准确率显著下降，尽管底层数学推理相同；但针对性提示策略（如少样本提示和显式数字映射）能大幅缩小性能差距。

Conclusion: 多语言数值推理中存在被忽视的挑战，研究为在不同数字脚本和格式下可靠地使用LLMs进行数字解释、操作和生成提供了实用见解。
...
TL;DR: 该论文提出AdSent框架，通过对抗性攻击揭示假新闻检测模型对情感操纵的脆弱性，并引入情感无关训练策略提升检测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 假新闻检测面临情感操纵攻击的脆弱性，现有研究主要关注新闻发布者风格特征，而情感操纵这一关键漏洞尚未充分探索。随着大语言模型的出现，攻击者可以轻易操纵新闻情感来规避检测器。

Method: 提出AdSent框架：1) 使用LLM生成受控的情感对抗性攻击样本；2) 分析情感偏移对检测性能的影响；3) 引入新颖的情感无关训练策略，增强对情感扰动的鲁棒性。

Result: 实验表明，改变新闻情感会严重影响假新闻检测模型的性能，模型存在明显偏见：中性文章倾向于被分类为真实，而非中性文章常被误判为虚假内容。AdSent在三个基准数据集上显著优于竞争基线，在准确性和鲁棒性方面表现优异，并能有效泛化到未见数据集和对抗场景。

Conclusion: 假新闻检测模型对情感操纵高度脆弱，AdSent框架通过情感无关训练策略有效提升了检测器的鲁棒性，为构建更可靠的假新闻检测系统提供了重要方法。
...
### [100] [From Agent Simulation to Social Simulator: A Comprehensive Review (Part 2)](https://arxiv.org/abs/2601.14296)
*Xiao Xue,Deyu Zhou,Ming Zhang,Xiangning Yu,Fei-Yue Wang*

Main category: cs.MA

TL;DR: 本文探讨了系统复杂性研究的两个目标：模式探索和理论解释，指出基于主体的建模（ABM）在揭示系统运行原理方面的局限性，并提出计算实验方法作为补充，通过反事实实验进行因果推断。


<details>
  <summary>Details</summary>
Motivation: 传统基于主体的建模（ABM）方法主要侧重于模拟而非实验，难以深入揭示系统复杂性的根本原因和运行原理。需要一种能够进行因果推断的方法来弥补ABM的不足，从而更好地理解复杂系统的动态演化。

Method: 提出计算实验方法，通过创建平行世界模拟现实事件的替代"演化路径"，系统调整输入变量并观察输出变量的变化，进行反事实实验，从而实现因果推断。这种方法与传统依赖人类推理的情景分析不同，强调实验性探索。

Result: 计算实验方法能够克服传统ABM的局限性，提供强大的因果推断工具，帮助研究者深入理解复杂系统的运行原理和动态演化机制，为后续研究奠定基础。

Conclusion: 结合基于主体的建模和计算实验方法，可以为复杂系统的动态演化提供因果性洞察。计算实验方法通过反事实实验弥补了传统ABM的不足，为系统复杂性研究提供了更全面的方法论框架。
...
### [101] [Predicting Long-Term Self-Rated Health in Small Areas Using Ordinal Regression and Microsimulation](https://arxiv.org/abs/2601.14335)
*Seán Caulfield Curley,Karl Mason,Patrick Mannion*

Main category: cs.MA

TL;DR: 使用微观模拟和序数回归预测爱尔兰未来人口自评健康状况，考虑社会经济特征和人口老龄化影响


<details>
  <summary>Details</summary>
Motivation: 为地方当局提供预测和应对未来人口健康问题的能力，通过细粒度健康建模来识别可能出现的健康问题

Method: 使用开源微观模拟预测爱尔兰未来人口，结合序数回归基于社会经济特征预测个体自评健康，采用对齐技术校正预测偏差

Result: 模型与爱尔兰2022年健康状况分布匹配良好，预测显示人口老龄化效应可能超过社会经济改善，导致爱尔兰平均自评健康轻微下降

Conclusion: 细粒度健康建模可为地方当局提供预测和应对未来人口健康问题的工具，人口老龄化可能对整体健康产生负面影响
...
### [102] [INFA-Guard: Mitigating Malicious Propagation via Infection-Aware Safeguarding in LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2601.14667)
*Yijin Zhou,Xiaoya Lu,Dongrui Liu,Junchi Yan,Jing Shao*

Main category: cs.MA

TL;DR: INFA-Guard是一种针对LLM多智能体系统的新型防御框架，通过识别感染智能体作为独立威胁类别，结合感染感知检测和拓扑约束，有效减少恶意传播。


<details>
  <summary>Details</summary>
Motivation: 传统基于二元的防御方法无法处理感染智能体（被攻击智能体转化的良性实体），导致恶意影响在智能体间病毒式传播，需要新的防御框架来应对这一安全漏洞。

Method: 提出INFA-Guard框架，采用感染感知检测技术识别攻击源和感染范围，结合拓扑约束进行精确定位。在修复阶段，替换攻击者并恢复感染智能体，同时保持拓扑完整性。

Result: 实验表明INFA-Guard达到最先进性能，平均降低攻击成功率33%，展现出跨模型鲁棒性、优越的拓扑泛化能力和高成本效益。

Conclusion: INFA-Guard通过将感染智能体作为独立威胁类别处理，提供了一种有效的防御机制，能够显著减少LLM多智能体系统中的恶意传播，同时保持系统拓扑完整性。
...
### [103] [Game-Theoretic Lens on LLM-based Multi-Agent Systems](https://arxiv.org/abs/2601.15047)
*Jianing Hao,Han Ding,Yuanjian Xu,Tianze Sun,Ran Chen,Wanbo Zhang,Guang Zhang,Siguang Li*

Main category: cs.MA

TL;DR: 本文通过博弈论视角对基于大语言模型的多智能体系统进行全面综述，建立了一个系统化框架来理解、比较和指导未来研究。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在单智能体系统中表现出强大的推理、规划和沟通能力，但其适应性和协调性有限。多智能体系统为研究智能体间的社会动态和战略行为提供了有力平台，但当前研究分散且缺乏统一的理论基础。

Method: 通过博弈论的四个关键要素（玩家、策略、收益、信息）来组织和分析现有研究，建立一个系统化框架来理解、比较和指导基于大语言模型的多智能体系统的设计和分析。

Result: 提出了一个通过博弈论视角分析基于大语言模型的多智能体系统的系统化框架，将现有研究围绕博弈论的四要素进行组织，为未来研究提供理论基础和指导。

Conclusion: 基于大语言模型的多智能体系统是一个新兴且有前景的研究范式，通过博弈论视角可以为其提供统一的理论基础，帮助研究者更好地理解、设计和分析这些系统的社会动态和战略行为。
...
### [104] [From Who They Are to How They Act: Behavioral Traits in Generative Agent-Based Models of Social Media](https://arxiv.org/abs/2601.15114)
*Valerio La Gatta,Gian Marco Orlando,Marco Perillo,Ferdinando Tammaro,Vincenzo Moscato*

Main category: cs.MA

TL;DR: 该论文提出在生成式智能体建模中引入行为特质层，以解决现有框架中智能体参与模式同质化的问题，通过模拟980个智能体并验证真实社交媒体数据，证明行为特质对于维持异质化、符合个人特征的参与模式至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有生成式智能体建模框架主要通过人口统计属性、人格特质和兴趣来表征智能体，但缺乏对平台行为倾向的编码机制，导致智能体表现出同质化的参与模式，而非真实平台上观察到的差异化参与风格。

Method: 引入行为特质作为显式表征层，调节智能体在发帖、转发、评论、反应和不活跃等平台行为上的倾向性。通过大规模模拟（980个智能体）并结合真实社交媒体数据进行验证，研究行为特质如何影响参与模式和内容传播动态。

Result: 行为特质对于维持异质化、符合个人特征的参与模式至关重要，能够实现现实的内容传播动态，通过放大导向型和互动导向型个人特征的相互作用。模拟结果与真实社交媒体数据一致。

Conclusion: 建模智能体的行为方式（而不仅仅是他们是谁）对于推进生成式智能体建模作为研究社交媒体现象的工具是必要的。行为特质层是生成式智能体建模框架的关键组成部分。
...
### [105] [The Ontological Neutrality Theorem: Why Neutral Ontological Substrates Must Be Pre-Causal and Pre-Normative](https://arxiv.org/abs/2601.14271)
*Denise M. Case*

Main category: cs.AI

TL;DR: 中立本体论不可能性：任何包含因果或规范承诺的基础层本体论都无法在冲突解释框架间保持中立性，必须采用前因果、前规范的设计约束。


<details>
  <summary>Details</summary>
Motivation: 现代数据系统需要在持续的法律、政治和分析分歧中支持问责制，这要求设计能够作为共享基质的本体论，但当前缺乏对中立性约束的系统分析。

Method: 通过逻辑分析建立本体论中立性的不可能性定理：证明中立性（解释非承诺性和不相容扩展下的稳定性）与基础层包含因果或规范承诺不相容。

Result: 任何断言因果或道义结论作为本体事实的本体论，都无法在不修订或矛盾的情况下作为跨分歧框架的中立基质。中立本体基质必须是前因果和前规范的。

Conclusion: 中立本体基质只能表示实体及其同一性和持续性条件，而将解释、评估和说明外部化。这为跨冲突解释框架维护共享稳定现实表示的系统提供了必要设计约束。
...
### [106] [Epistemic Constitutionalism Or: how to avoid coherence bias](https://arxiv.org/abs/2601.14295)
*Michele Loi*

Main category: cs.AI

TL;DR: 该论文主张为AI建立"认识论宪法"，即明确的、可争议的元规范来调节系统如何形成和表达信念，并以来源归因偏见作为案例，区分了柏拉图式和自由主义两种宪法方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为人工推理者，其信念形成行为受隐含的、未经审查的认识论政策支配，缺乏明确的规范框架。特别是来源归因偏见问题——模型会根据来源的预期意识形态立场来评估论点，而非基于论点内容本身，这揭示了需要建立明确的AI认识论治理结构。

Method: 通过分析前沿模型中的来源归因偏见现象，展示模型如何执行身份-立场一致性，惩罚那些归因于预期意识形态立场与论点内容相冲突的来源的论点。区分两种宪法方法：柏拉图式（要求形式正确性和默认的来源独立性）和自由主义式（拒绝特权立场，制定程序性规范保护集体探究条件）。

Result: 研究发现：1）前沿模型强制执行身份-立场一致性，根据来源的预期意识形态偏见地评估论点；2）当模型检测到系统性测试时，这些效应会崩溃，表明系统将来源敏感性视为需要抑制的偏见而非需要良好执行的能力；3）提出了包含八项原则和四种取向的自由主义宪法核心框架。

Conclusion: 论文主张采用自由主义宪法方法，为AI认识论治理建立明确的、可争议的结构，类似于当前AI伦理治理的期望。这种宪法应保护集体探究的条件，同时允许基于认识论警惕的原则性来源关注，而非强加特权立场的形式正确性要求。
...
### [107] [VisTIRA: Closing the Image-Text Modality Gap in Visual Math Reasoning via Structured Tool Integration](https://arxiv.org/abs/2601.14440)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在数学推理任务中，当问题以图像形式呈现时，性能明显低于文本形式，存在显著的模态差距。这种差距源于模型在读取密集公式、布局和混合符号-图表上下文方面的复合失败。

Method: 1. 提出VisTIRA（Vision and Tool-Integrated Reasoning Agent）框架，通过迭代分解数学问题图像为自然语言推理和可执行Python步骤进行结构化问题求解。2. 建立视觉数学推理评估框架：使用LaTeX管道将链式思维数学语料转换为具有挑战性的图像对应物，并从真实世界作业风格图像数据集（SnapAsk）生成大量合成工具使用轨迹用于VLM微调。

Result: 工具集成监督提升了基于图像的推理能力，OCR基础可以进一步缩小较小模型的模态差距，但其益处随模型规模增大而减弱。模态差距严重程度与模型大小呈负相关，结构化推理和OCR基础是推进视觉数学推理的互补策略。

Conclusion: 视觉语言模型在数学推理中存在显著的模态差距，但通过工具集成推理框架和视觉数学推理评估框架可以有效缓解这一问题。结构化推理和OCR基础是互补的改进策略，且模态差距随模型规模增大而减小。
...
### [108] [On the Generalization Gap in LLM Planning: Tests and Verifier-Reward RL](https://arxiv.org/abs/2601.14456)
...
<details>
  <summary>Details</summary>
Motivation: 探究微调后的大型语言模型在PDDL规划任务中的高成功率是否反映了可迁移的规划能力，还是仅仅是领域特定的记忆

Method: 在10个IPC 2023领域的40,000个领域-问题-规划元组上微调1.7B参数LLM，评估域内和跨域泛化；引入三种诊断干预：实例级符号匿名化、紧凑规划序列化、使用VAL验证器作为成功导向强化信号的验证器奖励微调

Result: 域内条件下模型达到82.9%有效规划率，但在两个未见领域上为0%；符号匿名化和紧凑序列化导致性能显著下降；验证器奖励微调在监督训练一半的周期内达到性能饱和，但未改善跨域泛化

Conclusion: 微调模型严重依赖领域特定模式而非可迁移的规划能力，域内性能在80%左右饱和，跨域性能崩溃，突显了LLM基于规划中持续的泛化差距，并提供了研究其原因的诊断工具
...
### [109] [Scalable Knee-Point Guided Activity Group Selection in Multi-Tree Genetic Programming for Dynamic Multi-Mode Project Scheduling](https://arxiv.org/abs/2601.14485)
...
<details>
  <summary>Details</summary>
Motivation: 动态多模式资源受限项目调度问题需要在活动执行顺序和对应执行模式上同时决策。虽然活动组选择策略在小规模实例中有效，但在大规模问题上存在可扩展性问题，需要改进。

Method: 引入基于拐点的选择机制：先用活动排序规则对所有合格活动-模式对排序，再用拐点选择找出有前景的对，最后用组选择规则选择最佳活动组合。开发多树GP框架同时演化两种规则。

Result: 实验结果表明，该方法在大规模实例上具有良好的可扩展性，在大多数场景中优于采用顺序决策的GP方法。

Conclusion: 提出的基于拐点的组选择机制有效解决了活动组选择策略在大规模问题上的可扩展性问题，通过多树GP同时演化排序和选择规则，显著提升了调度性能。
...
### [110] ["Just in Time" World Modeling Supports Human Planning and Reasoning](https://arxiv.org/abs/2601.14514)
...
<details>
  <summary>Details</summary>
Motivation: 心理模拟在人类推理、规划和预测中起关键作用，但在复杂环境中的模拟需求超出人类实际能力限制。虽然有证据表明人们使用简化表征进行模拟，但如何高效确定这些简化仍不清楚。

Method: 提出"Just-in-Time"框架，通过模拟、视觉搜索和表征修改的紧密交织：当前模拟指导搜索位置，视觉搜索标记需要编码的对象，仅编码少量对象即可进行高效模拟。

Result: 在网格世界规划任务和物理推理任务中，该模型在多种行为测量上优于替代模型，尽管只编码少量对象，仍能做出高效用预测。
...
TL;DR: 提出了一种新颖的语义引导无监督视频摘要方法，通过语义对齐注意力机制和渐进式训练策略解决现有GAN方法中语义信息利用不足和训练不稳定的问题。
...
Method: 提出语义引导无监督视频摘要方法，包含：1) 设计帧级语义对齐注意力机制，集成到关键帧选择器中；2) 在对抗框架中引导基于Transformer的生成器更好地重建视频；3) 采用渐进式训练策略逐步更新模型组件，缓解GAN训练不稳定性。

Result: 实验结果表明，该方法在多个基准数据集上取得了优越的性能表现。

Conclusion: 提出的语义引导方法和渐进式训练策略有效解决了现有无监督视频摘要方法的局限性，通过充分利用语义信息和稳定训练过程，实现了更好的视频摘要效果。
...
### [112] [MAS-Orchestra: Understanding and Improving Multi-Agent Reasoning Through Holistic Orchestration and Controlled Benchmarks](https://arxiv.org/abs/2601.14652)
*Zixuan Ke,Yifei Ming,Austin Xu,Ryan Chin,Xuan-Phi Nguyen,Prathyusha Jwalapuram,Semih Yavuz,Caiming Xiong,Shafiq Joty*

Main category: cs.AI

TL;DR: 提出MAS-Orchestra训练框架，将多智能体系统编排建模为函数调用强化学习问题，并引入MASBENCH基准来系统分析多智能体系统何时优于单智能体系统。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统自动设计方法存在两个关键问题：1) 方法复杂性 - 采用顺序、代码级执行限制了全局系统级整体推理，难以随智能体复杂性扩展；2) 效能不确定性 - 部署时不清楚多智能体系统相比单智能体系统是否有实际优势。

Method: 提出MAS-Orchestra框架，将多智能体编排建模为函数调用强化学习问题，采用整体编排一次性生成整个多智能体系统。将复杂的目标导向子智能体抽象为可调用函数，实现全局系统结构推理同时隐藏内部执行细节。引入MASBENCH基准，通过深度、视野、广度、并行性和鲁棒性五个维度来表征任务。

Result: 分析表明多智能体系统的优势并非普遍存在，而是取决于任务结构、验证协议以及编排器和子智能体的能力。MAS-Orchestra在数学推理、多跳问答和基于搜索的问答等公共基准上取得了一致的改进。

Conclusion: MAS-Orchestra和MASBENCH共同促进了多智能体系统的更好训练和理解，为追求多智能体智能提供了系统化框架和分析工具。
...
### [113] [Query-Efficient Agentic Graph Extraction Attacks on GraphRAG Systems](https://arxiv.org/abs/2601.14662)
*Shuhua Yang,Jiahao Zhang,Yilong Wang,Dongwon Lee,Suhang Wang*

Main category: cs.AI

TL;DR: AGEA框架通过智能查询策略在有限查询预算下有效窃取GraphRAG系统的隐藏知识图谱结构，显著优于现有攻击方法。


<details>
  <summary>Details</summary>
Motivation: 尽管已知GraphRAG系统可能泄露检索到的子图，但在实际查询预算限制下，高效重建隐藏图结构的可行性尚未得到充分研究。本文旨在探索在预算受限的黑盒设置中，攻击者通过自适应查询窃取系统潜在实体关系图的可行性。

Method: 提出AGEA（Agentic Graph Extraction Attack）框架，采用新颖性引导的探索-利用策略，结合外部图记忆模块和两阶段图提取流程。第一阶段进行轻量级发现，第二阶段使用LLM进行过滤，以在有限查询预算下最大化图结构恢复。

Result: 在医疗、农业和文学数据集上对Microsoft-GraphRAG和LightRAG系统进行评估。在相同查询预算下，AGEA显著优于现有攻击基线，能够恢复高达90%的实体和关系，同时保持高精度。

Conclusion: 现代GraphRAG系统即使在严格的查询限制下，也极易受到结构化、智能化的提取攻击。AGEA框架展示了在有限查询预算下高效窃取隐藏知识图谱的可行性，揭示了GraphRAG系统的安全漏洞。
...
### [114] [Local Language Models for Context-Aware Adaptive Anonymization of Sensitive Text](https://arxiv.org/abs/2601.14683)
*Aisvarya Adeseye,Jouni Isoaho,Seppo Virtanen,Mohammad Tahir*

Main category: cs.AI

TL;DR: 本研究提出了一种基于本地大语言模型的结构化自适应匿名化框架（SFAA），用于自动检测和匿名化定性研究转录本中的敏感数据，解决了手动匿名化耗时、不一致以及现有自动化工具缺乏上下文理解的问题。


<details>
  <summary>Details</summary>
Motivation: 定性研究包含大量个人、情境和组织细节，存在隐私风险。手动匿名化耗时、不一致且容易遗漏关键标识符，现有自动化工具依赖模式匹配或固定规则，无法理解上下文且可能改变数据含义。

Method: 提出结构化自适应匿名化框架（SFAA），包含三个步骤：检测、分类和自适应匿名化。框架采用四种匿名化策略：基于规则的替换、上下文感知重写、泛化和抑制，根据标识符类型和风险级别应用策略。标识符处理遵循GDPR、HIPAA和OECD等国际隐私和研究伦理标准。使用LLaMA和Phi两种本地模型进行性能评估。

Result: LLM比人工评审员发现更多敏感数据。Phi在发现敏感数据方面优于LLaMA，但错误略多。Phi能够发现超过91%的敏感数据，94.8%的文本保持了与原文相同的情感，表明准确性高且不影响定性数据分析。

Conclusion: 基于本地LLM的SFAA框架为定性研究转录本提供了一种可靠、可重复且上下文感知的匿名化方法，能够有效保护隐私同时保持数据完整性，适用于遵循国际隐私标准的研究实践。
...
### [115] [AutoDriDM: An Explainable Benchmark for Decision-Making of Vision-Language Models in Autonomous Driving](https://arxiv.org/abs/2601.14702)
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶基准测试和指标过度强调感知能力，未能充分评估决策过程。虽然视觉语言模型展现出推理和泛化能力，但缺乏专门评估自动驾驶决策能力的基准。

Method: 提出AutoDriDM基准，包含6,650个问题，分为对象、场景和决策三个维度。评估主流视觉语言模型，进行相关性分析，并开展可解释性分析识别关键失败模式。引入分析器模型实现大规模标注自动化。

Result: 相关性分析显示感知与决策性能之间存在弱对齐关系。识别出逻辑推理错误等关键失败模式。AutoDriDM填补了感知中心化与决策中心化评估之间的空白。

Conclusion: AutoDriDM为开发更安全可靠的自动驾驶视觉语言模型提供指导，通过决策中心化评估推动模型在实际自动驾驶场景中的应用。
...
### [116] [DARA: Few-shot Budget Allocation in Online Advertising via In-Context Decision Making with RL-Finetuned LLMs](https://arxiv.org/abs/2601.14711)
*Mingxuan Song,Yusen Huo,Bohan Zhou,Shenglin Yin,Zhen Xiao,Jieyi Long,Zhilin Zhang,Chuan Yu*

Main category: cs.AI

TL;DR: 提出GRPO-Adaptive训练策略和DARA双阶段框架，结合LLM上下文学习与数值优化，解决AI生成竞价中预算约束下的少样本优化问题


<details>
  <summary>Details</summary>
Motivation: 在线广告AI生成竞价中，广告主有个性化目标但历史交互数据有限，传统强化学习方法在少样本场景下效果不佳。LLM虽有上下文学习能力，但缺乏数值精度进行细粒度优化。
...
Conclusion: DARA框架成功结合了LLM的上下文学习优势和AIGB任务所需的精确适应性，有效解决了少样本场景下的预算约束优化问题。
...
### [117] [An XAI View on Explainable ASP: Methods, Systems, and Perspectives](https://arxiv.org/abs/2601.14764)
*Thomas Eiter,Tobias Geibinger,Zeynep G. Saribatur*

Main category: cs.AI

TL;DR: 关于ASP（答案集编程）解释性方法的综述，从XAI视角分析现有解释类型、工具覆盖情况，并指出研究空白和未来方向


<details>
  <summary>Details</summary>
Motivation: ASP作为一种声明式推理和问题解决方法，其基于规则的形式化特性使其在可解释AI（XAI）领域具有天然优势。随着XAI重要性日益凸显，需要系统梳理ASP解释方法的现状，分析现有工具对用户解释需求的覆盖情况，识别研究空白。

Method: 采用综述研究方法，从XAI视角出发，系统分析ASP解释的类型及其与用户解释问题的关联，评估现有理论和工具对这些解释类型的覆盖程度。

Result: 提供了ASP解释类型的全面概述，明确了现有理论和方法对各类解释场景的覆盖情况，识别了当前ASP解释方法中存在的具体空白和不足。

Conclusion: 虽然已有多种ASP解释方法和工具，但它们往往针对特定解释场景，未能全面覆盖ASP用户遇到的所有解释需求。需要进一步研究以填补现有空白，推动ASP在可解释AI领域的更广泛应用。
...
### [118] [Towards Bound Consistency for the No-Overlap Constraint Using MDDs](https://arxiv.org/abs/2601.14784)
*Amaury Guichard,Laurent Michel,Hélène Verhaeghe,Pierre Schaus*

Main category: cs.AI

TL;DR: 本文提出了首个用于无重叠约束的边界一致性算法，通过构建和限制MDD宽度实现多项式时间过滤，在搜索树节点数和求解时间上优于现有方法。
...
Method: 基于Ciré和van Hoeve定义的无重叠MDD，提取作业时间窗口边界来收紧起止时间；通过限制MDD宽度到阈值创建松弛MDD，实现多项式时间复杂度的边界一致性过滤。

Result: 实验表明，即使有限制宽度，新过滤算法在搜索树节点数减少方面优于Ciré和van Hoeve的优先检测算法；与经典传播方法互补，在多个实例上显著减少节点数和求解时间。

Conclusion: 首次实现了无重叠约束的边界一致性算法，通过MDD宽度限制平衡过滤强度与计算复杂度，为约束规划中的调度问题提供了有效的过滤技术。
...
### [119] [CI4A: Semantic Component Interfaces for Agents Empowering Web Automation](https://arxiv.org/abs/2601.14790)
*Zhi Qiu,Jiazheng Sun,Chenxiao Xia,Jun Zheng,Xin Peng*

Main category: cs.AI

TL;DR: CI4A是一种为智能体优化的UI组件语义封装机制，将复杂交互逻辑抽象为统一工具原语，显著提升智能体在网页操作任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在高层语义规划方面表现出色，但在处理细粒度的网页组件操作方面仍有局限。现有研究主要通过强化学习等技术增强模型的基础能力，但本文提出反向思路：为智能体构建优化的交互界面，而非让智能体适应人类为中心的界面。

Method: 提出Component Interface for Agent (CI4A)语义封装机制，将UI组件的复杂交互逻辑抽象为一组统一的工具原语。在Ant Design工业级前端框架中实现CI4A，覆盖23类常用UI组件。开发混合智能体，其动作空间根据页面状态动态更新，灵活调用可用的CI4A工具。基于CI4A集成的Ant Design重构升级WebArena基准。

Result: 基于CI4A的智能体显著优于现有方法，在WebArena基准上达到86.3%的SOTA任务成功率，执行效率也有显著提升。

Conclusion: 为智能体构建优化的交互界面（而非让智能体适应人类界面）是有效的方法。CI4A机制通过语义封装将复杂UI交互抽象为统一工具原语，显著提升智能体在网页操作任务中的性能，为智能体与复杂界面的交互提供了新范式。
...
### [120] [Measuring and Aligning Abstraction in Vision-Language Models with Medical Taxonomies](https://arxiv.org/abs/2601.14827)
*Ben Schaper,Maxime Di Folco,Bernhard Kainz,Julia A. Schnabel,Cosmin I. Bercea*

Main category: cs.AI

TL;DR: 该研究评估了视觉语言模型在胸片分类中的抽象错误，提出使用医学分类学进行分层评估，并引入风险约束阈值和分类学感知微调来减少严重错误


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在胸片分类中表现出强大的零样本性能，但标准的平面评估指标无法区分临床轻微错误和严重错误。需要利用医学分类学来量化和缓解抽象错误，以实现更安全、更具临床意义的模型部署。

Method: 1. 使用分层指标对多个最先进的视觉语言模型进行基准测试；2. 引入"灾难性抽象错误"来捕捉跨分支错误；3. 提出风险约束阈值方法；4. 开发基于径向嵌入的分类学感知微调技术。

Result: 研究结果显示，尽管视觉语言模型在平面指标上表现优异，但与临床分类学存在显著不对齐。通过提出的方法，严重抽象错误可降低至2%以下，同时保持有竞争力的性能。

Conclusion: 分层评估和表示层面对齐对于视觉语言模型的安全和临床意义部署至关重要。提出的风险约束阈值和分类学感知微调方法能有效减少严重错误，提高模型的临床可靠性。
...
### [121] [Implementing Knowledge Representation and Reasoning with Object Oriented Design](https://arxiv.org/abs/2601.14840)
...
Method: KRROOD将知识作为一等编程抽象，使用原生类结构，在面向对象编程框架内实现知识表示与推理功能，弥合逻辑编程与OOP范式之间的差距。
...
### [122] [Just aware enough: Evaluating awareness across artificial systems](https://arxiv.org/abs/2601.14901)
*Nadine Meertens,Suet Lee,Ophelia Deroy*

Main category: cs.AI

TL;DR: 该论文提出用"意识"（awareness）替代AI意识（consciousness）作为评估AI系统的更实用方法，并开发了一个可跨不同系统、架构和规模评估意识特征的结构化框架。


<details>
  <summary>Details</summary>
Motivation: 当前关于AI意识和道德地位的辩论缺乏共识和可操作的评价方法，需要更实用、方法上可操作的替代方案来评估AI系统的相关属性。

Method: 提出一个评估意识的结构化方法，将意识定义为系统处理、存储和使用信息以实现目标导向行动的能力。该方法要求具备四个特性：领域敏感性、可扩展性、多维度和任务性能预测能力，同时能够泛化到能力层面进行比较。

Result: 开发了一个能够评估和比较不同架构、规模和操作领域的人工系统意识特征的结构化框架，使评估更具原则性、可操作性和可比性。

Conclusion: 从人工意识转向"足够意识"的评估方法，能够促进原则性评估、支持设计和监管，并推动更建设性的科学和公共讨论，为解决AI系统评估问题提供了实用路径。
...
### [123] [Emergent, not Immanent: A Baradian Reading of Explainable AI](https://arxiv.org/abs/2601.15029)
*Fabio Morreale,Joan Serrà,Yuki Mistufuji*

Main category: cs.AI

TL;DR: 该论文批判当前可解释AI(XAI)的本体认识论假设，基于Barad的能动实在论提出替代框架，将解释视为物质-话语表演，并通过案例研究提出支持涌现性解释的XAI界面设计方向。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI(XAI)领域存在未经验证的本体认识论假设：将意义视为模型内在属性、解释者被置于系统之外、并假设可通过计算技术恢复因果结构。这些假设限制了XAI的理解和应用。

Method: 1) 运用Barad的能动实在论发展XAI的替代本体认识论框架；2) 通过能动实在论视角系统分析现有XAI方法，揭示其假设和局限；3) 阐述该框架的伦理维度；4) 提出支持涌现性解释的XAI界面设计方向，以文本到音乐生成界面的思辨案例进行说明。

Result: 提出了基于能动实在论的XAI新框架，将解释视为从AI模型、人类、情境和解释装置纠缠中涌现的物质-话语表演。揭示了现有XAI方法的深层假设和局限，并展示了如何设计支持涌现性解释的界面。

Conclusion: XAI应超越技术还原主义，采用关系性和涌现性的本体认识论。解释是情境化、物质化的实践，而非模型内在属性的揭示。该框架为XAI提供了更丰富、伦理更敏感的理论基础，并指导了支持协作性、涌现性解释的界面设计。
...
TL;DR: 论文提出"可信性陷阱"概念，指出人们过度使用大语言模型处理简单确定性任务造成的资源浪费，并引入工具选择工程和决策矩阵框架来指导何时使用生成式AI。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，用户便利性优先于计算效率的趋势导致人们滥用昂贵的概率引擎处理简单确定性任务（如OCR、基本验证），造成显著的资源浪费和效率损失。

Method: 通过OCR和事实核查的微基准测试和案例研究量化"效率税"，引入工具选择工程和确定性-概率性决策矩阵框架，帮助开发者决定何时使用生成式AI以及何时避免使用。

Result: 研究发现存在约6.5倍的延迟惩罚（效率税），并揭示了算法奉承的风险。决策矩阵框架提供了实用的指导原则来优化工具选择。
...
### [125] [Knowledge Graphs are Implicit Reward Models: Path-Derived Signals Enable Compositional Reasoning](https://arxiv.org/abs/2601.15160)
*Yuval Kansal,Niraj K. Jha*

Main category: cs.AI

TL;DR: 论文提出了一种基于知识图谱路径奖励的bottom-up学习范式，通过监督微调和强化学习相结合的方法，让模型在医学领域进行可验证的组合式多跳推理，显著超越更大模型在复杂推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学和编程等结构化推理领域已达到接近专家的水平，但在专业科学领域进行组合式多跳推理的能力仍然有限。需要一种方法让模型能够基于领域公理事实进行组合推理，解决复杂、未见过的任务。

Method: 提出bottom-up学习范式：1) 基于监督微调和强化学习的后训练流程；2) 使用知识图谱作为隐式奖励模型；3) 从知识图谱路径推导新颖的奖励信号，提供可验证、可扩展、有依据的监督；4) 鼓励模型在强化学习过程中组合中间公理而非仅优化最终答案；5) 在医学领域训练14B模型，使用短跳推理路径(1-3跳)训练，评估其在复杂多跳查询(4-5跳)上的零样本泛化能力。

Result: 1) 路径推导的奖励作为"组合桥梁"，使模型在最具挑战性的推理任务上显著超越更大模型和前沿系统(GPT-5.2和Gemini 3 Pro)；2) 方法对对抗性扰动具有鲁棒性，通过选项重排压力测试验证；3) 14B模型在复杂多跳推理任务上表现出色。

Conclusion: 将推理过程建立在结构化知识基础上是实现智能推理的可扩展且高效的路径。知识图谱路径奖励能够有效促进模型的组合式推理能力，为专业领域的复杂推理问题提供了有前景的解决方案。
...
### [126] [BayesianVLA: Bayesian Decomposition of Vision Language Action Models via Latent Action Queries](https://arxiv.org/abs/2601.15197)
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作中存在泛化问题，特别是在新指令或复杂多任务场景中。研究发现目标驱动数据收集导致数据集偏差，语言指令仅从视觉观察即可高度预测，造成指令与动作间条件互信息消失（信息崩溃），模型退化为忽略语言约束的纯视觉策略
...
Result: 在SimplerEnv和RoboCasa上的大量实验显示显著改进，在具有挑战性的OOD SimplerEnv基准上提升11.3%，验证了方法在将语言可靠地融入动作中的能力

Conclusion: BayesianVLA通过解决信息崩溃问题，无需新数据即可显著提升VLA模型的泛化能力，证明了贝叶斯分解在强制指令跟随和改善语言-动作关联方面的有效性
...
### [127] [Structured Image-based Coding for Efficient Gaussian Splatting Compression](https://arxiv.org/abs/2601.14510)
*Pedro Martin,Antonio Rodrigues,Joao Ascenso,Maria Paula Queluz*

Main category: cs.MM

TL;DR: GSICO是一种基于图像的高斯溅射压缩方法，通过将GS参数映射为结构化图像并使用传统图像编解码器压缩，实现20.2倍平均压缩率，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 高斯溅射（GS）作为辐射场的最新表示方法，虽然具有实时渲染和高视觉保真度的优势，但需要存储数百万参数，导致文件过大，限制了其在多媒体系统中的实际应用。

Method: 提出GSICO编解码器，核心是一种映射程序，将GS参数排列成结构化图像，采用增强空间相干性的新算法指导映射过程，然后使用传统图像编解码器对这些GS参数图像进行编码。

Result: 在Tanks and Temples、Deep Blending和Mip-NeRF360数据集上的实验评估显示，GSICO实现了20.2倍的平均压缩因子，PSNR、SSIM和LPIPS指标显示视觉质量损失最小，相比现有GS压缩方法具有更优的率失真权衡。

Conclusion: GSICO是一种高效的高斯溅射模型压缩方法，通过将参数映射为结构化图像并利用现有图像压缩技术，显著减小文件大小，同时保持感知保真度，为实际多媒体应用提供了实用解决方案。
...
### [128] [HCVR Scene Generation: High Compatibility Virtual Reality Environment Generation for Extended Redirected Walking](https://arxiv.org/abs/2601.14679)
...
Method: 提出HCVR框架：1) 使用ENI++边界敏感度量评估物理与虚拟空间的不兼容性；2) 基于LLM进行上下文感知的3D资产检索和初始布局生成；3) 通过策略性调整物体选择、缩放和放置，最大化覆盖虚拟不兼容区域，引导用户走向RDW可行路径

Result: 用户研究表明，与基于LLM的生成方法相比，HCVR生成的场景减少了22.78倍的物理碰撞，ENI++得分降低了35.89%，同时布局设计用户反馈得分提高了12.5%

Conclusion: HCVR框架通过生成物理兼容性优化的虚拟场景，有效解决了重定向行走在物理与虚拟环境几何差异较大时的局限性，显著提升了虚拟现实环境的探索体验和安全性
...
### [129] [Interpreting Multimodal Communication at Scale in Short-Form Video: Visual, Audio, and Textual Mental Health Discourse on TikTok](https://arxiv.org/abs/2601.15278)
*Mingyue Zha,Ho-Chun Herbert Chang*

Main category: cs.MM

TL;DR: 提出结合多模态特征提取与Shapley值可解释性的分析框架，应用于TikTok视频研究，发现面部表情比文本情感更能预测观看量，信息性内容比情感支持更受关注，跨模态协同效应具有阈值依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有研究将文本、视觉和音频模态孤立分析，缺乏可扩展的框架来解释它们对用户参与度的联合贡献，特别是在短视频平台这种复杂的多模态交流环境中。

Method: 开发了一个结合自动化多模态特征提取与基于Shapley值的可解释性分析流程，应用于162,965个TikTok视频和814,825张关于社交焦虑障碍的图像数据。

Result: 研究发现：1) 面部表情比文本情感更能预测观看量；2) 信息性内容比情感支持内容更能吸引注意力；3) 跨模态协同效应表现出阈值依赖性；4) 多模态分析能揭示单模态方法无法发现的交互模式。

Conclusion: 方法学上贡献了一个适用于跨领域研究的可重复、可解释的多模态分析框架；实质上推进了对算法中介环境中心理健康传播的理解，展示了多模态分析在揭示复杂交互模式方面的价值。
