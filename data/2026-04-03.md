...
TL;DR: DOne框架通过解耦结构理解和元素渲染，解决视觉语言模型在Design-to-Code生成中的"整体瓶颈"问题，在复杂布局数据集上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在Design-to-Code生成中存在"整体瓶颈"，难以同时处理高层次结构层次和细粒度视觉细节，导致布局失真或使用通用占位符。
...
TL;DR: Look Twice (LoT)是一个无需训练、在推理时改进预训练多模态大语言模型利用多模态证据能力的框架，通过注意力模式识别相关视觉区域和文本元素，并用提示标记突出显示以引导模型重新关注相关证据。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在回答需要外部知识的图像问题时，难以识别最相关的视觉和文本证据，特别是在需要整合噪声或部分相关文本证据以及定位细粒度视觉信息的场景中。

Method: 利用模型注意力模式估计查询相关的视觉区域和检索到的文本元素，通过轻量级提示级标记突出显示这些证据，引导模型在生成答案时重新关注相关证据，无需额外训练或架构修改。

Result: 在多个基于知识的VQA基准测试中，相比零样本MLLMs取得了一致的改进；在纯视觉和幻觉导向的基准测试中，仅视觉证据突出显示也能提升模型性能。

Conclusion: LoT框架通过推理时证据突出显示有效提升了预训练MLLMs在多模态证据利用方面的能力，无需额外训练成本，在知识密集型视觉问答任务中表现出显著改进。
...
TL;DR: MedQwen是一个参数高效的医学视觉语言模型，通过频谱路由的专家混合架构和理论基础的缩放规则，在保持基础架构不变的情况下实现接近全微调的性能，同时大幅减少可训练参数并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在通用基准上表现优异，但在医学影像领域缺乏鲁棒性。医学数据的异质性监督导致跨数据集干扰和对数据制度的敏感性。在临床工作流中，数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。

Method: 提出MedQwen模型，采用参数高效的频谱路由专家混合架构。从预训练权重的非重叠奇异值分解段初始化每个专家，引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。使用理论基础的缩放规则将低秩更新与全秩、全微调的专家混合对齐。

Result: 在23个医学数据集上（涵盖视觉问答、报告生成、放射学分类和幻觉缓解），MedQwen实现了强大可靠的性能：在零样本分类上以339倍更少的可训练参数接近全微调性能，将顺序遗忘减少到约5%，而强基线模型性能下降20-50%。

Conclusion: MedQwen通过频谱路由的专家混合架构和理论基础的缩放规则，有效解决了医学视觉语言模型中的跨数据集干扰、数据制度敏感性和灾难性遗忘问题，为临床工作流中的持续学习提供了高效解决方案。
...
TL;DR: VideoZeroBench：用于长视频问答的分层基准测试，通过时空证据验证来严格评估视频多模态大语言模型的细粒度视觉理解和推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型的评估存在两个关键局限：(1) 虚高的分数掩盖了细粒度视觉理解和推理的缺陷；(2) 答案正确性评估通常不验证模型是否识别了支持其预测的精确时空证据。

Method: 提出VideoZeroBench分层基准测试，包含500个手动标注的问题，覆盖13个领域，每个问题都配有作为证据的时间区间和空间边界框。引入五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。
...
TL;DR: 基于视觉Transformer的模型在扩展的美国足球训练视频数据集中检测危险擒抱动作，通过不平衡感知训练将危险召回率提升8%以上


<details>
  <summary>Details</summary>
Motivation: 早期识别接触性运动中的危险动作能够及时干预并提高运动员安全性。美国足球中的危险擒抱动作检测对于预防伤害至关重要，但现有数据集规模有限且存在类别不平衡问题。

Method: 构建了包含733个单人擒抱假人视频片段的数据集，每个片段围绕首次接触点进行时间定位，并使用标准擒抱技术评估(SATT-3)的撞击区域组件进行标注。采用视觉Transformer模型结合不平衡感知训练方法进行危险动作检测。

Result: 在交叉验证中获得危险召回率0.67和危险F1分数0.59。相较于先前在小数据集上的基线(危险召回率0.58，危险F1分数0.56)，在更大数据集上将危险召回率提升了超过8个百分点。
...
Method: 1. 从蹦床动作捕捉数据生成合成蹦床姿态数据集(STP)；2. 开发流水线将噪声动作捕捉数据拟合到参数化人体模型；3. 生成多视角逼真图像；4. 使用STP数据微调ViTPose模型

Result: 1. 在2D姿态估计上取得最先进结果，缩小了常规姿态与极端姿态的性能差距；2. 在3D三角测量中，MPJPE减少12.5mm，相比预训练ViTPose模型提升19.6%

Conclusion: 合成数据微调能有效提升姿态估计模型在极端运动场景下的性能，为特定领域姿态估计提供了可行方案
...
### [7] [Moiré Video Authentication: A Physical Signature Against AI Video Generation](https://arxiv.org/abs/2604.01654)
...
<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实视频区分，需要一种可靠的物理认证方法来检测AI生成视频

Method: 利用莫尔效应：当相机拍摄紧凑双层光栅结构时产生的干涉条纹。推导出莫尔运动不变性，证明条纹相位与光栅图像位移通过光学几何线性耦合，与观看距离和光栅结构无关。验证器从视频中提取两种信号并测试其相关性

Result: 在真实拍摄视频和多个最先进AI生成器的生成视频上进行验证，发现真实视频与AI生成视频产生显著不同的相关性特征，表明该方法能有效区分两者

Conclusion: 确定性光学现象可以作为物理基础的、可验证的签名来对抗AI生成视频，为视频认证提供了鲁棒的方法
...
TL;DR: 提出一种基于自助法的注意力正则化方法，通过重采样输入特征生成注意力分数的基线分布，估计显著性水平和后验概率，从而去除噪声引起的虚假注意力，提高稀疏性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer(ViT)依赖注意力机制对输入特征进行加权，注意力分数常被用作决策过程的解释。然而，注意力分数几乎总是非零值，导致注意力图噪声大、扩散性强，限制了可解释性。需要量化注意力分数的不确定性度量，获得正则化的注意力分数。

Method: 将ViT的注意力分数置于统计框架中，利用统计学习技术引入注意力分数的自助法。通过重采样输入特征生成注意力分数的基线分布，然后使用该自助分布估计注意力分数的显著性和后验概率。这种方法称为"注意力正则化"。

Result: 在自然图像和医学图像中，注意力正则化方法能够直接去除噪声引起的虚假注意力，显著提高收缩性和稀疏性。通过模拟和真实世界数据集进行了定量评估，证明了方法的有效性。

Conclusion: 研究表明自助法可作为使用注意力分数作为ViT解释时的实用正则化工具。该方法提高了注意力图的解释性，减少了噪声影响，为注意力机制的可解释性提供了统计基础。
...
TL;DR: AffordTissue：用于胆囊切除术中预测工具-动作特定组织可操作区域的多模态框架，通过密集热图提供空间推理，提升手术自动化安全性
...
Method: 提出AffordTissue多模态框架，结合：1）时序视觉编码器捕捉多视角下的工具运动和组织动态；2）语言条件化实现不同器械-动作对的泛化；3）DiT风格解码器进行密集可操作性预测。建立了首个组织可操作性基准数据集，包含103个胆囊切除术的15,638个视频片段，涵盖6种工具-动作对。

Result: 实验显示相比视觉语言模型基线有显著改进（ASSD：20.6像素 vs 60.2像素）。任务特定架构在大规模基础模型上表现出色，能有效预测工具-动作特定的组织可操作性区域。

Conclusion: AffordTissue通过预测工具-动作特定的组织可操作性区域，为安全手术自动化提供显式空间推理，有望实现向适当组织区域的显式策略指导，并在器械偏离预测安全区域时提供早期安全停止机制。
...
TL;DR: GRAZE是一个无需训练的流水线，用于在美式足球训练视频中定位首次接触点（FPOC），通过Grounding DINO发现候选交互、运动感知时序推理和SAM2像素级验证，在真实训练视频中实现高精度接触起始定位。
...
Method: 提出GRAZE训练免费流水线：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时序推理进行细化；3) 使用SAM2作为显式像素级接触验证器，而非仅依赖检测置信度。候选发现与接触确认的分离使方法对杂乱场景和冲击附近不稳定接地具有鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的视频中产生有效输出，在77.5%的所有视频中将FPOC定位在±10帧内，在82.7%的所有视频中定位在±20帧内。结果显示无需任务特定训练即可在真实训练视频中实现帧级准确的接触起始定位。

Conclusion: GRAZE证明了在无约束训练视频中实现帧级准确接触起始定位的可行性，无需标记的擒抱接触示例或任务特定训练。分离候选发现与接触确认的方法对杂乱场景具有鲁棒性，为生物力学分析提供了可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的开放词汇3D场景理解方法存在两个关键局限：1) 非结构化、重叠的高斯分布导致空间模糊性，需要概率性特征注册；2) 基于物体级掩码的特征池化造成多级语义模糊性，稀释了细粒度细节。

Method: 提出基于稀疏体素光栅化的结构化、不相交几何表示框架，通过单目深度和法线先验进行正则化，建立稳定几何基础。实现确定性、置信感知的特征注册过程，并利用基础模型AM-RADIO的密集对齐特性解决多级语义模糊性，避免分层训练的计算开销。

Result: 在开放词汇3D物体检索和点云理解基准测试中达到最先进性能，特别是在细粒度查询任务上表现优异，而传统注册方法在这些任务上通常失败。

Conclusion: 稀疏体素光栅化作为结构化几何表示能有效解决3D高斯泼溅方法中的空间和语义模糊性问题，通过确定性特征注册和利用基础模型的密集对齐特性，显著提升了开放词汇3D场景理解的性能，尤其在细粒度任务上。
...
<details>
  <summary>Details</summary>
Motivation: 从第一人称视频理解和预测物体运动对具身感知和交互至关重要，但现有生成模型缺乏显式物理推理，难以处理遮挡、快速运动等问题，导致物理不一致的6DoF轨迹生成

Method: 提出EgoFlow流匹配框架，采用混合Mamba-Transformer-Perceiver架构联合建模时间动态、场景几何和语义意图，通过梯度引导推理过程强制执行可微物理约束（如碰撞避免和运动平滑性）

Result: 在HD-EPIC、EgoExo4D和HOT3D数据集上优于基于扩散和Transformer的基线方法，在准确性、泛化性和物理真实性方面表现更好，碰撞率降低达79%，对未见场景具有强泛化能力

Conclusion: 流匹配生成建模为可扩展且物理基础的第一人称运动理解提供了有前景的解决方案，EgoFlow无需后处理过滤或额外监督即可生成连贯可控的运动
...
<details>
  <summary>Details</summary>
Motivation: 无标记姿态估计技术使得在自然环境中使用标准视频捕捉详细人体运动成为可能，但姿态数据的高维度、噪声和时间复杂性给提取有意义的协调和行为变化模式带来了挑战
...
Conclusion: 该分析流程为从复杂姿态数据中提取有意义的行为模式提供了一个通用框架，可适应不同实验环境，支持对运动动态的全面分析
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模面临重建保真度与生成合理性之间的基本矛盾：前馈重建方法效率高且输入对齐好，但缺乏全局先验导致结构不完整；扩散生成方法能提供丰富几何细节，但多视角一致性差。

Method: 提出UniRecGen统一框架，将两种范式整合为协作系统。通过共享规范空间对齐坐标空间、3D表示和训练目标；采用解耦协作学习，保持训练稳定性并实现推理时的无缝协作。重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。
...
<details>
  <summary>Details</summary>
Motivation: 热成像在夜视中至关重要，但受鬼影效应（光子流中细节纹理丢失）困扰。现有HADAR技术仅适用于材料均匀的有限场景，而真实世界中材料非均匀性普遍存在，需要通用抗鬼影成像方法

Method: 提出TAG（thermal anti-ghosting）通用计算热成像框架，利用高光谱光子流进行非参数纹理恢复，解决材料非均匀性问题，实现抗鬼影成像

Result: TAG在多种场景中普遍优于HADAR，首次实现鬼影人脸表情恢复，展示了热3D拓扑对齐和情绪检测，揭示了材料非均匀性对HADAR效果边界的影响
...
<details>
  <summary>Details</summary>
Motivation: 无人机低空图像语义分割面临极端尺度变化、复杂物体边界和边缘设备计算资源有限等挑战。现有基于Transformer的分割方法计算开销大，而轻量级方法难以捕捉高分辨率航空场景的细粒度细节。

Method: 提出PBSeg框架，包含：1）原型交叉注意力（PBCA），利用特征冗余降低计算复杂度；2）高效多尺度特征提取模块，结合可变形卷积（DConv）和上下文感知调制（CAM），捕捉局部细节和全局语义。

Result: 在两个挑战性无人机数据集上评估：UAVid数据集达到71.86% mIoU，UDD6数据集达到80.92% mIoU，在保持计算效率的同时建立了有竞争力的性能。

Conclusion: PBSeg为无人机应用提供了一种高效的原型分割框架，通过创新的原型交叉注意力和多尺度特征提取，在计算效率和分割质量之间取得了良好平衡。
...
TL;DR: ReFlow：基于自校正流匹配的单目动态场景重建框架，无需外部运动指导


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部密集运动指导（如光流）来稳定动态区域重建，但会引入额外复杂性和误差传播。动态区域初始化不完整导致重建不稳定，需要更鲁棒的解决方案。

Method: 1. 完整规范空间构建模块：增强静态和动态区域初始化；2. 分离式动态场景建模模块：解耦静态和动态组件进行针对性运动监督；3. 自校正流匹配机制：包含全流匹配（对齐3D场景流与2D观测）和相机流匹配（强制静态对象多视角一致性）。

Result: 在多种场景下的广泛实验表明，ReFlow实现了优越的重建质量和鲁棒性，为单目4D重建建立了新的自校正范式。

Conclusion: ReFlow通过自校正流匹配机制，无需外部运动指导即可实现鲁棒准确的动态场景重建，解决了现有方法依赖外部指导的问题。
...
Result: 在广泛使用的数据集上，GAAL方法相比各种最先进的表格-图像融合基线和测试时表格缺失基线表现出优越性能

Conclusion: GAAL范式通过梯度对齐有效提供单模态辅助，提升整体融合性能，为解决多模态融合中的梯度冲突问题提供了新思路
...
TL;DR: 提出卫星图像无关训练框架SFT，通过无人机多视角序列重建3D场景并生成伪正射影像，实现无卫星数据下的跨视角地理定位
...
Method: 1) 使用3D高斯泼溅从多视角无人机图像重建密集3D场景；2) 通过PCA引导的正射投影将重建几何投影为伪正射影像；3) 轻量级几何引导修复获得纹理完整的无人机侧视图；4) 从生成的正射影像提取DINOv3特征，仅用无人机数据学习Fisher向量聚合模型，测试时复用该模型编码卫星图块进行跨视角检索。

Result: 在University-1652和SUES-200数据集上的实验表明，SFT框架显著优于卫星无关泛化基线，并缩小了与使用卫星图像训练方法的性能差距。

Conclusion: 提出的卫星无关训练框架通过3D场景重建和伪正射影像生成，实现了无需卫星数据的地理定位训练，为实际部署提供了可行方案。
...
TL;DR: SHOE是一个新的开放词汇人-物交互检测评估框架，通过语义相似度而非精确字符串匹配来评估预测，解决了传统mAP指标无法识别语义相似但词汇不同的预测的问题。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如mAP）将HOI类别视为离散分类标签，无法识别语义相似但词汇不同的预测（如"靠在沙发上"vs"坐在沙发上"），限制了开放词汇预测的评估能力。

Method: SHOE将每个HOI预测分解为动词和对象组件，使用多个大型语言模型的平均值估计它们的语义相似度，并将它们组合成相似度分数，实现超越精确字符串匹配的评估。

Result: SHOE得分与人类判断的一致性达到85.73%，优于现有的基于LLM和嵌入的基线方法，更接近人类对交互的理解。
...
TL;DR: ROSETTA提出了一种鲁棒的开集测试时适应方法，通过角度损失调节特征范数和特征范数损失抑制csOOD logits，在保持ID分类性能的同时提升OOD检测能力。


<details>
  <summary>Details</summary>
Motivation: 开集测试时适应中，协变量偏移的ID样本和OOD样本共存，熵最小化用于保持ID性能，熵最大化用于增强OOD检测，但两者存在内在冲突，导致csID分类和csOOD检测之间的权衡问题。

Method: 首先分析OSTTA中熵最大化的局限性，然后引入角度损失调节特征范数大小，以及特征范数损失抑制csOOD logits，形成ROSETTA方法。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强OOD检测同时保持高ID分类性能；Cityscapes验证了真实世界语义分割的有效性；HAC数据集证明了在不同开集TTA设置中的适用性。

Conclusion: ROSETTA通过角度损失和特征范数损失有效解决了开集测试时适应中熵最小化与最大化之间的冲突，在多种数据集和任务上展现出优越性能。
...
Method: 首先通过注册来自多个客户端的局部合并LiDAR点云构建共享几何骨架，初始化全局3DGS模型。联邦优化期间固定高斯位置以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度、球谐系数）。服务器使用可见性感知聚合更新，根据每个客户端观察每个高斯的频率加权其贡献。

Result: 实验表明F3DGS在实现与集中训练相当的重建质量的同时，支持跨智能体的分布式优化。收集了包含同步LiDAR、RGB和IMU测量的多序列室内数据集进行评估。

Conclusion: F3DGS为分布式机器人系统中的3D重建提供了一种有效的联邦学习方法，解决了部分可观测性挑战，同时保持几何一致性。数据集、开发工具包和源代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型对物理可实现对抗攻击的鲁棒性研究不足。现有攻击方法（语言扰动和2D视觉攻击）要么代表性不足，要么物理真实性有限。相比之下，对抗性3D纹理更具物理可行性和破坏性，但标准3D仿真器不提供从VLA目标函数到物体外观的可微分优化路径，难以进行端到端优化。

Method: 提出Tex3D框架：1) 前景-背景解耦(FBD)：通过双渲染器对齐实现可微分纹理优化，同时保留原始仿真环境；2) 轨迹感知对抗优化(TAAO)：优先处理行为关键帧，并使用基于顶点的参数化稳定优化过程。

Result: 在仿真和真实机器人实验中，Tex3D显著降低了VLA模型在多个操作任务中的性能，任务失败率高达96.7%。
...
TL;DR: 利用基础模型特征训练的稀疏自编码器产生单语义、空间定位的神经元，通过视觉语言提示生成可解释的性状描述，构建了包含8万个性状标注的Bioscan-Traits数据集


<details>
  <summary>Details</summary>
Motivation: 形态性状是生物与环境相互作用的重要指标，但传统提取方法依赖专家标注，效率低下且难以大规模应用，限制了生态学研究。主要瓶颈在于缺乏连接生物图像与性状标注的高质量数据集。

Method: 1. 在基础模型特征上训练稀疏自编码器，获得单语义、空间定位的神经元；2. 利用这些神经元定位显著区域；3. 通过视觉语言提示生成可解释的性状描述；4. 构建Bioscan-Traits数据集（19K昆虫图像，80K性状标注）；5. 通过消融实验评估设计敏感性。

Result: 1. 成功构建Bioscan-Traits数据集，包含80K性状标注，覆盖BIOSCAN-5M中的19K昆虫图像；2. 人类评估确认生成的形态描述具有生物学合理性；3. 消融实验系统评估了关键设计选择对性状描述质量的影响。

Conclusion: 通过模块化流水线而非昂贵的人工标注，该方法为大规模形态分析提供了可扩展的解决方案，将生物学意义注入基础模型，弥合了生态学相关性与机器学习实用性之间的差距。
...
TL;DR: LivingWorld是一个从单张图像生成具有环境动态的4D世界的交互式框架，通过渐进构建全局一致的运动场来解决场景扩展中的动态建模挑战。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法主要关注静态几何重建，而忽略了场景尺度的环境动态（如云、水、烟雾）。建模这些动态具有挑战性，因为运动需要在扩展场景中保持连贯性，同时支持低延迟的用户反馈。

Method: 1. 渐进构建全局一致的运动场；2. 引入几何感知对齐模块解决视角间的方向和尺度歧义；3. 使用紧凑的基于哈希的运动场表示，支持高效查询和稳定传播；4. 支持渲染时的双向运动传播，无需昂贵的视频细化。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测在密集标注数据集上表现良好，但3D标注成本高昂，现实场景中通常只有部分目标被标注，需要解决稀疏标注下的检测问题

Method: 1. 道路感知补丁增强(RAPA)：利用稀疏标注，将分割的目标补丁增强到道路区域，同时保持3D几何一致性；2. 原型过滤(PBF)：通过原型相似性和深度不确定性过滤预测，生成高质量伪标签，维护全局2D RoI特征原型，选择与学习原型特征一致且深度估计可靠的伪标签

Result: 大量实验证明了所提方法的有效性，源代码已公开

Conclusion: 通过结合几何保持增强和原型引导的伪标签生成，实现了稀疏监督下的鲁棒3D目标检测
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在处理高度动态运动或需要精细运动控制时效果不佳，主要原因是常用训练数据集中缺乏此类示例

Method: 提出DynaVid框架，利用计算机图形流水线渲染的合成光流数据训练；采用两阶段生成：运动生成器先合成运动，然后运动引导的视频生成器基于该运动生成视频帧

Result: 在剧烈人体运动生成和极端相机运动控制两个挑战性场景中，DynaVid显著提升了动态运动生成和相机运动控制的真实感和可控性

Conclusion: 通过合成光流数据训练，DynaVid能够从合成数据学习动态运动模式，同时从真实视频保持视觉真实感，解决了现有数据集在动态运动方面的局限性
...
<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间持续交互时面临动态环境分布漂移的挑战，现有方法依赖测试阶段预先获取的域ID，限制了在未知交互场景中的实用性，且模型容易过拟合特定环境的感知噪声，导致泛化能力不足和灾难性遗忘
...
Conclusion: 提出的框架能够实现鲁棒的连续环境适应，通过解耦表示消除感知不确定性并提高泛化能力，同时通过权重融合确保模型适应新分布的同时最大程度保留旧环境的判别能力
...
TL;DR: 该论文提出频率域适应(FDA)和谐波约束最优传输(HOT)框架，通过迁移低频光谱分量来增强rPPG模型对光照、相机特性等外观变化的鲁棒性，提升跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积描记术(rPPG)在面部视频中进行非接触式生理测量，但在实际部署中常因域偏移导致性能显著下降。现有深度学习方法虽然在单个数据集上表现良好，但容易过拟合到光照、相机特性、颜色响应等外观相关因素，这些因素在不同域间变化显著。

Method: 提出频率域适应(FDA)作为建模rPPG中外观变化的原理性策略，通过迁移编码域依赖外观特征的低频光谱分量，鼓励rPPG模型学习对外观变化的不变性，同时保留心脏诱导信号。进一步提出谐波约束最优传输(HOT)，利用心脏信号的谐波特性来指导原始表示和FDA转移表示之间的对齐，确保生理一致性。

Result: 广泛的跨数据集实验表明，提出的FDA和HOT框架有效增强了rPPG模型在不同数据集间的鲁棒性和泛化能力。

Conclusion: 通过频率域适应和谐波约束对齐，该研究解决了rPPG模型在实际部署中的域偏移问题，为构建更稳健、泛化能力更强的非接触式生理测量系统提供了有效方法。
...
TL;DR: 提出UnderEraser方法，通过外部知识蒸馏和内部上下文交叉注意力机制，在视频对象移除中同时消除目标对象及其引起的副作用（阴影、反射等），保持时空一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频对象移除方法难以消除对象引起的副作用（阴影、反射、光照变化），同时保持整体连贯性，这源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 提出双视角理解方法：1）外部：通过蒸馏方案将视觉基础模型中对象与其诱导效应的关系迁移到视频扩散模型；2）内部：提出帧间上下文交叉注意力机制，使每个去噪块基于目标区域周围未掩码的上下文信息。

Result: 实验表明该方法达到最先进性能，并建立了首个真实世界视频对象移除基准，促进未来研究和社区发展。

Conclusion: 外部和内部引导联合使模型能够理解目标对象、其诱导效应和全局背景上下文，实现清晰连贯的对象移除，代码、数据和模型已开源。
...
TL;DR: 提出一种通过群体检测实现端到端共享注意力估计的方法，同时完成群体检测和共享注意力估计，优于现有方法
...
Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的实验表明，SteerFlow相比现有方法始终获得更好的编辑质量，并能自然扩展到复杂多轮编辑范式而不累积漂移。
...
TL;DR: 提出DPMO方法将点标注转换为掩码标注，并开发RPS框架用于密集人群实例分割，在多个数据集上达到SOTA性能，同时设计掩码监督损失函数提升计数精度。


<details>
  <summary>Details</summary>
Motivation: 当前人群数据集主要使用点标注，而区域标注（如边界框）稀缺且不准确。直接应用SAM等大型基础模型在密集人群场景效果不佳，需要专门的方法从点标注生成实例分割掩码。

Method: 1. 提出Dense Point-to-Mask Optimization (DPMO)，结合SAM与最近邻排斥圆约束从点标注生成密集实例分割；2. 提出Reinforced Point Selection (RPS)框架，使用Group Relative Policy Optimization (GRPO)训练，从初始点预测采样中选择最佳预测点；3. 设计掩码监督的损失函数。
...
TL;DR: 提出几何感知的无人机地理定位框架，通过3D场景重建和鸟瞰图渲染统一粗粒度位置识别与细粒度姿态估计


<details>
  <summary>Details</summary>
Motivation: 现有方法将透视畸变视为外观噪声而非几何变换，采用解耦的位置检索和姿态估计流程，无法有效处理倾斜无人机图像与正交卫星地图之间的严重几何差异

Method: 使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景，渲染虚拟鸟瞰图作为几何中介进行跨视图检索，并引入卫星级注意力块高效处理多个位置假设

Result: 在精炼的University-1652基准和SUES-200数据集上显著优于现有方法，实现鲁棒的米级定位精度，在复杂城市环境中具有更好的泛化能力

Conclusion: 通过显式建模3D场景几何，将透视畸变转化为几何变换而非外观噪声，实现了端到端的无人机地理定位，为GNSS拒止环境下的无人机导航提供了有效解决方案
...
TL;DR: 该研究构建了US-365K超声图像-文本数据集，提出超声诊断分类体系(UDT)，并开发了Ultrasound-CLIP框架，通过语义感知对比学习提升超声图像理解能力。
...
Method: 1) 构建US-365K大规模超声图像-文本数据集（36.5万配对样本，52个解剖类别）；2) 建立超声诊断分类体系(UDT)，包含超声分层解剖分类和超声诊断属性框架；3) 提出Ultrasound-CLIP框架，引入语义软标签和语义损失进行样本细化区分；4) 构建基于UDAF文本表示的异构图模态，支持病变-属性关系的结构化推理。

Result: 在患者级数据划分下，该方法在分类和检索基准测试中达到最先进性能，同时在零样本学习、线性探测和微调任务中表现出强大的泛化能力。

Conclusion: 该研究通过构建大规模超声数据集和专门设计的超声诊断分类体系，开发了针对超声医学影像的语义感知对比学习框架，显著提升了超声图像的理解和诊断能力，为超声AI应用提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频模型主要用作生成渲染器，通过感知、几何或简单语义信号进行条件控制。自监督学习获得的高维特征作为视觉模型的通用接口，但其包含的风格、光照和语义信息相互纠缠，限制了生成能力。需要探索如何将这些特征作为预训练视频扩散模型的通用条件信号。

Method: 引入轻量级架构和训练策略，解耦外观特征与其他需要保留的特征（如几何、语义）。通过特征维度补偿低空间分辨率，提高从显式空间表示进行生成渲染的可控性。

Result: 能够实现视频域迁移（如风格化、重光照）和从3D生成视频等任务。通过解耦外观特征实现鲁棒控制，特征维度补偿空间分辨率提高了可控性。

Conclusion: 自监督学习特征可以作为预训练视频扩散模型的通用条件信号，通过适当的架构设计和训练策略解耦特征，能够实现视频域迁移和3D到视频生成等应用，提高生成渲染的可控性。
...
TL;DR: 本文介绍了RebusBench基准测试，用于评估大视觉语言模型在解决需要神经符号推理的谜语（rebus puzzle）方面的能力，发现当前模型在此类任务上表现严重不足（准确率低于10%）。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型在显式视觉识别方面表现出色，但在需要多步骤推理的神经符号任务中存在认知差距。谜语（rebus puzzle）需要模型从视觉线索中提取信息、检索语言先验知识（如习语），并进行抽象映射，这超出了单纯的像素空间理解。

Method: 作者构建了RebusBench基准测试，包含1,164个谜语，用于评估模型整合感知和知识的能力。评估了包括Qwen、InternVL和LLaVA在内的最先进模型，并测试了模型缩放和上下文学习的影响。

Result: 评估结果显示模型表现严重不足：精确匹配准确率低于10%，语义准确率低于20%。模型缩放和上下文学习均未带来显著改进，表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 大视觉语言模型在需要神经符号推理的任务中存在根本性缺陷，无法有效整合感知和知识。RebusBench为评估这种认知能力提供了一个有价值的基准，揭示了当前模型架构的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAM）主要关注2D外观或潜在表示建模，缺乏对物理世界具身系统至关重要的几何基础。需要开发几何感知的统一框架来提升自动驾驶的未来预测和规划能力。

Method: 采用模块化架构：大语言模型处理语言指令、多视角图像和动作；三个轻量级生成器分别生成深度、未来视频和动作。通过几何感知世界表示统一指导未来预测和规划。

Result: 在Navsim v1和v2基准测试中表现优异：Navsim v1达到89.2 PDMS，Navsim v2达到88.7 EPDMS，优于现有基于世界模型的方法，同时生成更高质量的未来视频和深度预测。

Conclusion: DriveDreamer-Policy通过集成几何感知的世界表示，实现了更连贯的未来想象和更明智的驾驶动作。显式深度学习为视频想象提供互补优势并提高规划鲁棒性，为自动驾驶系统提供了统一且可控的解决方案。
...
TL;DR: FSKD：基于知识蒸馏的LiDAR-to-RGBI框架，使用多模态教师融合LiDAR与RGBI数据，训练仅需RGBI输入的SegFormer学生模型，实现零样本高精度森林结构监测


<details>
  <summary>Details</summary>
Motivation: 机载LiDAR虽能提供高分辨率森林结构数据（如CHM、PAI、FHD），但成本高、采集频率低。需要开发仅依赖RGBI图像就能准确预测森林结构指标的方法，以实现可扩展的森林监测

Method: 提出FSKD知识蒸馏框架：多模态教师模型通过交叉注意力融合RGBI图像与LiDAR衍生的平面指标和垂直剖面；RGBI-only SegFormer学生模型学习复现这些输出。使用德国萨克森州384 km²森林数据（20 cm GSD）训练，在八个地理不同的测试区域评估

Result: 学生模型在零样本CHM预测上达到SOTA性能（MedAE 4.17 m，R²=0.51，IoU 0.87），MAE比HRCHM/DAC基线提升29-46%（5.81 m vs. 8.14-10.84 m），相关系数更强（0.713 vs. 0.166-0.652）。多模态融合比纯RGBI训练提升10-26%性能，能联合预测CHM、PAI、FHD

Conclusion: FSKD框架通过知识蒸馏实现了仅需RGBI输入的高精度森林结构监测，打破了严格同时采集的限制（支持冬LiDAR夏RGBI），为德国数字孪生和国家数字正射影像项目等提供了可扩展的20 cm分辨率操作化监测方案
...
<details>
  <summary>Details</summary>
Motivation: 江南园林作为重要数字资产在影视游戏和数字旅游中潜力巨大，但传统手动建模依赖专家经验、耗时费力，需要自动化解决方案。

Method: 提出GardenDesigner框架，编码江南园林美学原则，基于程序化建模构建智能体链：地形分布智能体应用水为中心的地形规则，道路生成智能体应用探索性路径规则，资产选择和布局优化智能体遵循美学文化约束选择和安排对象，引入GardenVerse知识库增强资产布置过程。

Result: 开发了Unity交互界面和工具，非专业用户可通过文本输入在一分钟内构建江南园林。实验和人工评估表明，GardenDesigner能生成多样且美观的江南园林。

Conclusion: GardenDesigner框架成功实现了江南园林的自动化生成，通过编码美学原则和智能体链方法，为数字内容创作提供了高效解决方案，显著降低了专业门槛和时间成本。
...
TL;DR: 提出一种利用轮式里程计实现时间一致性单目深度估计的框架，通过光流三角化估计相机位姿和稀疏深度，使用递归贝叶斯估计度量尺度，重缩放预训练深度估计基础模型的相对深度输出。
...
Method: 提出一致性感知单目深度估计框架：1) 利用连续帧间光流进行三角化，估计相机位姿和稀疏深度；2) 使用稀疏深度通过递归贝叶斯估计更新度量尺度；3) 将估计的尺度应用于预训练深度估计基础模型输出的相对深度，进行重缩放。

Result: 在KITTI、TartanAir、MS2和自建数据集上评估，展示了鲁棒且准确的深度估计性能，实现了时间一致的深度预测。

Conclusion: 通过结合轮式里程计和递归贝叶斯尺度估计，提出的框架能够实现时间一致的单目深度估计，解决了现有方法在连续帧间深度预测不一致的问题，适用于自动驾驶和移动机器人应用。
...
TL;DR: STRIVE是一个用于视频问答的时空强化学习框架，通过构建多个视频变体并进行联合归一化，解决了传统方法奖励方差低的问题，提升了策略更新的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于群体的策略优化方法在大型多模态模型中存在局限性，当回答具有相似正确性时，奖励方差较低，导致优势估计弱或不稳定。这限制了视频问答中强化学习的有效性。

Method: STRIVE框架包含三个核心组件：1) 构建输入视频的多个时空变体；2) 在文本生成和视觉变体上进行联合归一化；3) 引入重要性感知采样机制，优先选择与问题最相关的帧，同时保持时间覆盖。通过将群体比较从语言多样性扩展到结构化视觉扰动，丰富了奖励信号。

Result: 在六个具有挑战性的视频推理基准测试中（VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench、PerceptionTest）均取得了优于现有强化学习基线的性能提升，在多个大型多模态模型上表现一致。

Conclusion: 结构化时空探索是稳定多模态强化学习并提升视频推理性能的有效机制。STRIVE通过扩展群体比较到视觉变体，解决了奖励方差低的问题，促进了跨互补视觉视角的鲁棒推理。
...
TL;DR: SafeRoPE：针对MMDiT的轻量级安全生成框架，通过分析注意力机制发现不安全语义集中在可解释的低维子空间，通过扰动RoPE来抑制不安全内容生成


<details>
  <summary>Details</summary>
Motivation: 基于整流流变换器的文本到图像模型（如SD3、FLUX）虽然生成保真度高，但对不安全语义（特别是多标记交互触发）仍然脆弱。现有缓解方法主要依赖微调或注意力调制进行概念遗忘，但计算开销大且专为U-Net设计，难以直接适配变换器基础的扩散模型（如MMDiT）。

Method: 深入分析MMDiT注意力机制，发现不安全语义集中在头部级别的可解释低维子空间中，有限的安全关键头部负责不安全特征提取。提出SafeRoPE框架：1) 在安全关键头部内分解不安全嵌入构建头部级不安全子空间，通过投影计算潜在风险分数；2) 引入头部级RoPE扰动，在不降低良性内容或图像质量的情况下抑制不安全语义；3) 结合LRS和RoPE扰动对查询和键向量嵌入执行风险特定的头部级旋转。

Result: 广泛实验表明，SafeRoPE在平衡有效有害内容缓解和效用保持方面实现了最先进的性能，为MMDiT的安全生成提供了有效解决方案。

Conclusion: SafeRoPE是一个轻量级、细粒度的安全生成框架，通过扰动RoPE来精确抑制不安全输出，同时保持生成保真度，为变换器基础的扩散模型提供了有效的安全生成方法。
...
TL;DR: 该论文提出了一种随机标签桥接训练方法，使大型语言模型参数能够适应视觉基础任务，无需人工标注，并发现部分桥接训练通常更有效。


<details>
  <summary>Details</summary>
Motivation: 语言预训练模型和视觉预训练模型的异常参数比例差异显著，使得跨模态（语言和视觉）适应比跨域适应更具挑战性。先前研究多集中于跨域迁移，认为语言预训练模型不适合下游视觉任务，但本文挑战了这一假设。

Method: 提出随机标签桥接训练方法，作为模态适应学习器，无需人工标注即可帮助LLM参数适应视觉基础任务。该方法包含一个桥接训练阶段，并探索部分桥接训练策略。

Result: 研究发现添加桥接训练阶段可以有效对齐LLM参数与视觉任务，随机标签桥接训练简单而有效。更重要的是，部分桥接训练通常更有利，因为LLM中的某些层表现出强大的基础属性，即使未经视觉任务微调也保持有益。

Conclusion: 该研究为直接在视觉模型中利用语言预训练参数开辟了新途径，突出了部分桥接训练作为跨模态适应实用路径的潜力，挑战了语言预训练模型不适合视觉任务的假设。
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督域自适应方法在医学图像严重程度分类中表现不佳，主要因为严重程度类别具有自然顺序性，导致类别边界模糊，传统方法难以有效对齐源域和目标域

Method: 提出基于排序学习的域自适应方法：1）跨域排序：利用类别顺序信息对跨域样本对进行排序；2）连续分布对齐：对齐源域和目标域的秩分数分布，实现类别特定的分布对齐

Result: 在溃疡性结肠炎和糖尿病视网膜病变严重程度分类任务上的实验验证了方法的有效性，成功实现了类别特定秩分数分布的对齐

Conclusion: 该方法通过利用严重程度类别的顺序信息进行跨域排序和对齐，有效解决了医学图像严重程度分类中的域自适应问题，为具有有序标签的分类任务提供了新的解决方案
...
TL;DR: 提出一种纹理感知的Transformer模型，直接从网格面片的原始像素学习，结合几何描述符进行语义分割，在城市场景和文化遗产数据集上显著优于现有方法。
...
Conclusion: 提出的纹理感知Transformer能有效利用3D网格的纹理和几何信息进行语义分割，在城市场景和文化遗产数据集上表现出色，为纹理3D网格分析提供了新的解决方案。
...
TL;DR: PI-VQ提出了一种位置无关的向量量化自编码器，通过消除潜在编码中的位置信息来捕获全局语义特征，并引入匹配量化算法提升容量，支持直接图像插值和单次前向传播的图像合成。


<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（VQ-VAE、VQ-GAN）学习的离散表示具有位置依赖性：编码在空间上排列且上下文纠缠，需要自回归或扩散先验来建模依赖关系。本文探究位置信息对于空间对齐数据的离散表示是否必要。

Method: 提出置换不变向量量化自编码器（PI-VQ），约束潜在编码不携带位置信息。为应对置换不变表示的信息容量减少，引入基于最优二分图匹配的匹配量化算法，相比朴素最近邻量化提升3.5倍有效瓶颈容量。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得具有竞争力的精度、密度和覆盖度指标。学习到的编码具有组合结构，支持基于插值的采样，允许在单次前向传播中合成新图像。

Conclusion: 位置无关表示能够捕获全局语义特征，支持直接图像插值而无需学习先验。讨论了位置无关表示固有的权衡（可分离性和可解释性），并指出未来研究方向。
...
TL;DR: FaCT-GS是一个用于快速灵活CT重建的框架，通过优化体素化和光栅化流水线，显著提升了基于高斯泼溅的CT重建速度，比现有方法快4-13倍，并支持从现有体积快速拟合高斯作为先验。


<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅(GS)在图像渲染中表现出色并已应用于CT重建，但其优势尚不足以促使从成熟算法转向。现有GS方法在CT重建中仍存在显著局限性，特别是速度和可扩展性方面。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流水线来加速CT重建。改进的体素化技术能够快速将高斯拟合到现有体积，可作为先验用于热启动重建或作为压缩表示。

Result: 在标准512×512投影上比现有GS CT重建方法快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能够适应不同投影和输出体积尺寸。

Conclusion: FaCT-GS通过优化流水线显著提升了基于高斯泼溅的CT重建速度，解决了现有方法的主要限制，为快速灵活CT重建提供了实用框架，并支持从现有体积快速拟合高斯作为先验。
...
TL;DR: 现代视觉语言模型在基本几何变换下存在系统性脆弱性，缺乏空间不变性和等变性，导致物体识别性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示最先进的视觉语言模型在基础几何变换下的根本脆弱性。虽然这些模型在语义任务上表现出色，但在空间推理方面存在系统性缺陷，特别是在处理简单旋转、缩放和恒等变换时无法可靠确定物体身份。

Method: 研究方法包括：1）系统评估不同视觉领域（符号草图、自然照片、抽象艺术）；2）测试多种架构、模型容量和提示策略；3）分析语义内容稀疏性对性能的影响；4）考察模型在几何变换下的空间不变性和等变性表现。

Result: 研究结果显示：1）当语义内容变得稀疏时，性能急剧下降；2）这种脆弱性在不同架构、模型容量和提示策略中普遍存在；3）模型在基本几何变换下缺乏稳健的空间不变性和等变性；4）当前VLM在语义理解和空间推理之间存在系统性差距。

Conclusion: 结论指出当前视觉语言模型在语义理解和空间推理能力之间存在系统性差距，揭示了这些模型缺乏几何基础的问题。这强调了未来多模态系统需要更强的几何基础，以提升在基本空间变换下的鲁棒性。
...
Result: 在三个基准数据集上，该方法提升了片段级一致性和边界质量，在三种不同模型上均获得更高的F1和Edit分数。帧级准确率基本保持不变，表明精确分割可通过简单损失设计而非复杂架构实现。

Conclusion: 通过轻量级双损失框架，仅需最小架构修改即可显著提升时序动作分割质量，证明了简单损失设计在实现精确分割中的有效性，为实际部署提供了更实用的解决方案。
...
Conclusion: MAR-MAER通过结合度量感知嵌入正则化和概率潜在建模，有效解决了文本到图像生成中的质量一致性和模糊提示处理问题，在自动指标和人类评估中都表现出优越性能
...
TL;DR: HieraVid是一个分层剪枝框架，通过段级、帧级和层级三个层次逐步动态减少视频冗余，在仅保留30%令牌的情况下实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解方面表现出色，但输入视频令牌数量庞大导致计算负担重。现有方法主要在输入级别剪枝视频令牌，忽略了视频和大语言模型中固有的信息结构。

Method: 基于视频具有段-帧结构和LLM内部单向传播多模态信息的观察，提出三级分层剪枝：1) 段级：视频令牌先按时间分段并空间合并；2) 帧级：同一段内相似帧联合剪枝以保持多样性；3) 层级：随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准测试上进行实验，仅保留30%令牌时，HieraVid达到新的SOTA性能，同时保持LLaVA-Video-7B和LLaVA-OneVision-7B超过98%和99%的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将3D高斯场景中的可供性推理视为一次性预测问题，假设静态观察已提供足够证据。但在复杂3D场景中，许多失败案例源于固定观察下任务相关证据不完整，而非预测能力不足。
...
Conclusion: 将可供性推理重新定义为序列证据获取过程，通过智能体跨维度证据获取框架A3R，能有效解决复杂3D场景中证据不完整问题，提升推理准确性和效率。
...
Method: 提出基于图的空间分布优化方法(GS^2)：1) 基于证据下界(ELBO)的自适应致密化策略，自动控制致密化过程；2) 透明度感知的渐进剪枝策略，动态移除低透明度高斯点；3) 基于图的特征编码模块，通过特征引导的点位移调整空间分布。

Result: GS^2实现了紧凑的高斯表示，同时提供卓越的渲染质量。相比原始3DGS，仅使用约12.5%的高斯点就获得了更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法。

Conclusion: GS^2通过优化高斯点的空间分布，在显著减少内存占用的同时保持了高质量的渲染效果，解决了3DGS实际应用中的内存瓶颈问题，为紧凑高效的3D场景表示提供了有效解决方案。
...
TL;DR: 该论文发现现代文本到图像生成模型存在低成本的提示词越狱攻击漏洞，仅需自然语言提示即可绕过安全过滤器，攻击成功率高达74.47%


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型广泛应用于创意工具和在线平台，依赖安全过滤器和审核管道来阻止有害内容。然而，这些系统仍然存在安全漏洞，需要研究其脆弱性以改进防护机制。

Method: 提出系统性的提示词越狱攻击研究，无需模型访问、优化或对抗训练。引入视觉越狱技术分类法，包括艺术重构、材质替换、伪教育框架、生活美学伪装和模糊动作替换等策略，通过将不安全意图隐藏在良性语义上下文中来利用提示词审核和视觉安全过滤的弱点。

Result: 在多个最先进的文本到图像系统中评估这些攻击，证明简单的语言修改可以可靠地规避现有安全防护并生成受限图像。在所有测试模型和攻击类别中观察到攻击成功率高达74.47%。

Conclusion: 研究结果凸显了表面层提示词过滤与检测生成媒体系统中对抗性意图所需语义理解之间的关键差距，表明当前安全措施存在严重不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法通常依赖句子级的视觉-语言对齐，难以利用细粒度语言线索（如空间关系和对象属性），而这些线索对于区分相似特征的物体至关重要。这些线索在不同定位阶段扮演不同角色，需要相应利用以提供更明确的指导。

Method: 1. 将语言表达解耦为全局上下文、空间关系和对象属性三个部分；2. 采用渐进式跨模态调制器，通过"调查-定位-验证"方案动态调制视觉注意力，实现从粗到细的视觉-语言对齐；3. 引入跨尺度融合模块处理遥感图像的大尺度变化；4. 使用语言引导校准解码器在预测时细化跨模态对齐；5. 统一的多任务头支持指称表达理解和分割任务。

Result: 在两个基准数据集（RRSIS-D和RISBench）上的广泛实验表明，ProVG始终优于现有方法，实现了新的最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有形状合成方法在捕捉复杂表面细节、处理任意拓扑结构（包括任意亏格）以及平衡重建精度与计算效率方面存在不足。需要一种既能精确表示复杂几何细节，又能高效计算且保持模型简洁性的方法。

Method: 1. 在表面内部体积中优化放置参考点，通过联合成本函数最大化稀疏性、中心性和表面可见性；2. 对每个参考点通过光线投射采样可见距离场；3. 使用快速球谐变换计算SH系数；4. 应用可配置低通滤波器增强几何保真度；5. 基于邻近性使用局部一致性约束细化输出。

Result: SHARC在重建精度和时间效率方面均优于现有最先进方法，同时不牺牲模型简洁性。该方法能够有效合成任意拓扑结构（包括任意亏格）的形状。

Conclusion: SHARC通过球谐函数表示距离场和优化参考点放置，提供了一种高效、精确且简洁的形状合成方法，在几何细节捕捉和计算效率方面表现出色，为复杂形状表示提供了新思路。
...
TL;DR: FTPFusion：基于时序扰动和稀疏跨模态交互的频率感知红外与可见光视频融合方法，通过高低频分量协同建模，在保持时空稳定性的同时保留高频细节。


<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法在保持时间稳定性和保留空间细节之间存在矛盾：要么局限于帧级增强而缺乏时序建模，要么依赖重型时空聚合而牺牲高频细节。
...
TL;DR: 提出一种基于几何的角膜反射光斑检测与匹配方法，将多个光斑视为结构化星座而非独立斑点，提高多LED眼动追踪的鲁棒性和可复现性。


<details>
  <summary>Details</summary>
Motivation: 角膜反射光斑检测在瞳孔-角膜反射眼动追踪中至关重要，但现有方法通常作为启发式算法嵌入大型系统，难以在不同硬件设置间复现。需要一种可复现、可评估的标准化方法。

Method: 提出2D几何驱动的星座式多光斑检测与匹配流程，受空间星体识别启发，将光斑视为结构化星座。引入相似性-布局对齐（SLA）程序，将星座匹配适配到多LED眼动追踪的特定约束。框架包含受控过检测、自适应候选回退、外观感知评分和可选语义布局先验，同时保持检测与对应关系明确分离。

Result: 在公开多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系。代码、预设和评估脚本已开源，支持透明复现、比较和数据集标注。

Conclusion: 该方法为角膜反射光斑检测提供了可复现的标准化解决方案，通过几何驱动的星座匹配方法提高了多LED眼动追踪系统的鲁棒性和跨硬件兼容性。
...
TL;DR: KnowMVG提出了一种知识先验和全局-局部注意力增强框架，用于提升医学视觉定位任务中视觉语言模型的空间定位精度，通过在解码过程中显式增强空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学视觉定位任务中虽然展现出多模态推理能力，但由于仅依赖潜在嵌入而缺乏显式定位先验，导致空间定位精度不足。需要解决这一限制以提供更精确的临床决策支持。
...
Conclusion: KnowMVG通过知识先验和全局-局部注意力增强，有效解决了视觉语言模型在医学视觉定位中的空间定位精度不足问题，为临床决策提供了更精确的可解释视觉证据。
...
TL;DR: 该研究探索了直接从波束成形前的天线级距离-多普勒张量学习空间结构的可行性，替代了传统先波束成形再处理的流程，通过双啁啾共享权重编码器和跨模态监督验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统汽车雷达感知流程通常先通过波束成形构建角度域表示，再应用基于学习的模型。本研究旨在探索是否可以直接从波束成形前的天线级距离-多普勒测量中学习有意义的空间结构，避免显式的角度域构建和手工信号处理阶段。

Method: 使用6发8收（48个虚拟天线）商用汽车雷达，采用A/B啁啾序列FMCW发射方案，其中有效发射孔径随啁啾变化。采用双啁啾共享权重编码器对波束成形前的天线级RD张量进行端到端数据驱动训练。监督信号基于LiDAR，采用可见性感知的跨模态监督，通过射线可见性建模雷达视场和遮挡感知的LiDAR可观测性。通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线评估发射配置对几何可恢复性的影响。

Result: 结果表明，无需显式角度域构建或手工信号处理阶段，可以直接从波束成形前的天线级RD张量学习空间结构。通过鸟瞰图占用作为几何探针进行评估，验证了方法的几何可恢复性。不同发射配置（啁啾类型）对几何恢复能力有显著影响。

Conclusion: 空间结构可以直接从波束成形前的天线级距离-多普勒张量中学习，无需传统的角度域构建流程。该方法为雷达感知提供了新的表示学习范式，具有简化处理流程和端到端优化的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 早教图像描述对自动化活动理解和教育评估至关重要，但现有方法面临两大挑战：缺乏大规模领域特定数据集，以及传统训练范式在提升专业物体描述能力方面存在局限

Method: 1) 构建ECAC数据集：包含256,121张真实世界图像，配备专家级标注和细粒度标签；2) 提出TTS评估指标：专门衡量专业玩具识别准确性；3) 设计RSRS混合训练框架：动态切换强化学习和监督微调，将零奖励困难样本重新路由到监督学习；4) 开发KinderMM-Cap-3B：领域适应的多模态大语言模型

Result: KinderMM-Cap-3B模型在TTS指标上达到51.06，显著超越现有最佳基线，同时保持卓越的描述质量，展示了在专业教育应用中的潜力

Conclusion: ECAC数据集和RSRS框架有效解决了早教图像描述中的领域适应和训练稳定性问题，为专业教育应用提供了高质量的自动化描述解决方案
...
TL;DR: 该论文提出AMIMV方法，通过不对称多图像多视图增强策略解决医学影像分类中的数据稀缺和类别不平衡问题，并在11个MedMNIST数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临两大挑战：1) 缺乏大量标注训练数据；2) 数据不平衡问题（常见类别数据丰富，罕见类别数据稀缺）。虽然自监督学习(SSL)方法在一定程度上解决了第一个问题，但SSL方法在医学影像分类中对不平衡数据的鲁棒性研究很少。
...
Result: 在MedMNIST数据集上的实验结果显示：retinaMNIST提升4.25%，tissueMNIST提升1.88%，DermaMNIST提升3.1%。AMIMV方法有效改善了医学影像分类在数据不平衡情况下的性能。

Conclusion: AMIMV方法通过不对称多图像多视图增强策略，有效解决了医学影像分类中的数据稀缺和类别不平衡问题，为医学影像分析中的自监督学习提供了新的解决方案，特别是在长尾分布和有限监督场景下表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外与可见光视频融合方法多为静态图像融合设计，无法有效处理视频帧间运动；现有视频融合方法虽通过跨帧交互提升时间一致性，但计算成本高昂。

Method: 提出端到端视频融合框架MAVFusion，采用运动感知稀疏交互机制：利用光学流识别多模态序列中的动态区域，对稀疏动态区域分配计算密集的跨模态注意力以捕捉显著过渡和促进模态间信息交换；对静态背景区域采用轻量级弱交互模块保持结构和外观完整性。
...
<details>
  <summary>Details</summary>
Motivation: 前列腺在mpMRI中的精确分割对临床和研究应用至关重要，但手动分割耗时且存在观察者间差异，通用分割工具在前列腺特定任务中精度不足，需要专门的多模态分割方法。

Method: 使用nnU-Net v2深度学习框架，整合多模态mpMRI数据（T2加权成像、扩散加权成像DWI、表观扩散系数ADC图），在PI-CAI数据集的981个病例上进行训练，采用5折交叉验证和外部验证。

Result: 交叉验证平均Dice得分为0.96±0.00，在Hospital La Fe的54例独立外部测试集上Dice得分为0.82，而通用分割工具TotalSegmentator仅得0.15分，主要由于腺体欠分割。

Conclusion: 任务特定的多模态分割策略对前列腺分割至关重要，提出的方法展现了良好的泛化能力，已容器化并可作为即用推理工具，有望可靠集成到临床研究流程中。
...
TL;DR: MyEgo是首个评估多模态大语言模型在自我中心视频中理解、记忆和推理相机佩戴者能力的自我中心视频问答数据集，揭示了当前模型在自我定位和长期记忆方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在需要自我定位的个性化问答任务中表现不佳，缺乏针对自我中心视频中理解相机佩戴者的系统评估。研究者旨在填补这一空白，探索模型在理解"我的物品"、"我的活动"和"我的过去"方面的能力。

Method: 研究者创建了MyEgo数据集，包含541个长视频和5K个个性化问题。对多种MLLM变体进行基准测试，包括开源与专有模型、思考与非思考模型、不同规模模型，并分析显式推理、模型缩放和显式证据提供对性能的影响。

Result: 顶级闭源和开源模型准确率分别仅为~46%和36%，远低于人类表现（差距约40%和50%）。显式推理和模型缩放未能带来一致改进。提供相关证据时模型有所改善，但随时间推移增益下降，表明模型在跟踪和记忆"我"和"我的过去"方面存在局限性。

Conclusion: 研究强调了自我定位和长期记忆在实现自我中心视频个性化问答中的关键作用。MyEgo数据集和分析将推动自我中心个性化辅助领域的进一步发展。
...
TL;DR: SDesc3D：基于短文本描述的3D室内场景生成框架，通过多视角结构先验和区域功能推理解决现有方法在物理合理性和细节丰富度上的不足


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的3D场景生成方法在短文本条件下存在物理合理性差和细节不足的问题，主要因为它们过度依赖明确的组合对象语义关系和空间关系线索，需要增强3D推理能力，特别是先验整合和空间锚定能力

Method: 提出SDesc3D框架：1）多视角场景先验增强，通过聚合多视角结构知识丰富欠明确的文本输入；2）功能感知布局锚定，利用区域功能进行隐式空间锚定，并进行分层布局推理；3）迭代反思-修正方案，通过自修正进行渐进式结构合理性优化

Result: 在短文本条件下的3D室内场景生成任务上，该方法优于现有方法，表现出更好的物理合理性和细节丰富度

Conclusion: SDesc3D通过整合多视角结构先验和区域功能推理，有效提升了短文本条件下3D场景生成的物理合理性和语义一致性，为交互式3D环境构建提供了更实用的解决方案
...
TL;DR: 论文提出交互式跟踪新范式，构建首个大规模基准InteractTrack，评估发现现有跟踪器在交互场景中表现不佳，并提出基于动态记忆机制的新基线方法IMAT。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要在非交互、发射后不管的模式下运行，无法适应需要人机协同的真实世界场景。为解决这一局限性，需要引入允许用户通过自然语言指令随时引导跟踪器的新范式。

Method: 提出交互式跟踪新范式，构建包含150个视频、密集边界框标注和时间戳语言指令的InteractTrack基准，设计全面评估协议评估25个代表性跟踪器，并提出基于动态记忆机制的IMAT基线方法。

Result: 评估显示最先进的跟踪方法在交互场景中失败，传统基准上的强性能无法迁移到交互场景。IMAT基线方法通过动态记忆机制学习用户反馈并相应更新跟踪行为。

Conclusion: 该基准、协议和基线为开发更智能、自适应和协作的跟踪系统奠定了基础，弥合了自动感知与人工引导之间的差距。
...
TL;DR: Curia-2是一个用于CT和MRI分析的十亿参数视觉Transformer基础模型，通过改进预训练策略和表示质量，在放射学数据分析上超越现有基础模型
...
Method: 基于Curia框架，改进预训练策略和表示质量，扩展到十亿参数视觉Transformer，首次实现多模态CT和MRI基础模型。将CuriaBench扩展为2D切片模型和3D体积模型两个评估轨道

Result: Curia-2在视觉任务上超越所有基础模型，在临床复杂任务（如病灶检测）上与视觉语言模型竞争相当。模型权重将公开以促进研究
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型存在视觉注意力惯性问题，注意力在早期解码步骤后基本保持静态，无法支持认知推理所需的组合理解。现有幻觉缓解方法主要针对感知幻觉，对需要对象间关系推理的认知幻觉效果有限。

Method: 提出无需训练的惯性感知视觉激发方法，将认知推理建模为视觉注意力的动态响应。该方法选择相对于历史注意力趋势动态出现的视觉标记，区分表现出惯性行为的标记，并引入惯性感知惩罚机制来防止注意力过度集中和局部区域的持久性。

Result: 实验表明该方法在各种基础多模态大语言模型和多个幻觉基准测试中均有效，特别是在认知幻觉方面表现突出。

Conclusion: 视觉注意力惯性是多模态大语言模型认知幻觉的关键因素，通过打破这种惯性模式可以有效提升模型的组合推理能力，无需训练的惯性感知视觉激发方法为解决认知幻觉问题提供了有效途径。
...
Method: 提出Resonance4D框架，将3D高斯泼溅与材料点法耦合，通过轻量级但物理表达性强的监督实现。核心创新包括：1）双域运动监督（DMS），结合空间结构一致性（局部变形）和频域谱一致性（振荡和全局动态模式）；2）零样本文本提示分割与模拟引导初始化，自动将高斯分解为对象部件级区域，支持全材料参数的联合优化
...
TL;DR: MTLSI-Net：通过线性注意力实现多任务密集预测，解决标准自注意力二次复杂度问题，在保持高性能的同时降低计算成本


<details>
  <summary>Details</summary>
Motivation: 多任务密集预测中，由于高分辨率特征上标准自注意力的二次复杂度，捕获全局跨任务交互仍然具有挑战性，需要更高效的交互机制

Method: 提出MTLSI-Net，包含三个关键组件：1)多任务多尺度查询线性融合块，使用共享全局上下文矩阵以线性复杂度捕获跨尺度跨任务依赖；2)语义令牌蒸馏器，将冗余特征压缩为紧凑语义令牌，蒸馏关键跨任务知识；3)跨窗口集成注意力块，通过双分支架构将全局语义注入局部特征，保持全局一致性和空间精度

Result: 在NYUDv2和PASCAL-Context数据集上的大量实验表明，MTLSI-Net实现了最先进的性能，验证了其在多任务学习中的有效性和效率

Conclusion: MTLSI-Net通过线性注意力机制成功解决了多任务密集预测中的全局跨任务交互问题，在保持高性能的同时显著降低了计算复杂度，为多任务学习提供了高效解决方案
...
TL;DR: ProDiG：一种渐进式扩散引导框架，通过几何感知因果注意力模块和距离自适应高斯模块，从航拍图像生成地面级视图和一致的3D场景模型


<details>
  <summary>Details</summary>
Motivation: 现有方法在从航拍图像生成地面级视图时面临极端视角变化、中间观测缺失和大尺度变化等挑战。现有方法要么后处理渲染结果导致几何不一致，要么依赖多高度地面真值数据（通常难以获取）。高斯泼溅和基于扩散的细化方法在小变化下有效，但在大尺度航拍到地面视角差距下失效。

Method: 提出ProDiG（渐进式高度高斯泼溅）框架：1）渐进式合成中间高度视图；2）几何感知因果注意力模块将极线结构注入参考视图扩散；3）距离自适应高斯模块根据相机距离动态调整高斯尺度和不透明度。这些组件共同实现渐进式、几何基础的细化，无需额外地面真值视点。

Result: 在合成和真实世界数据集上的广泛实验表明，ProDiG能够生成视觉逼真的地面级渲染和一致的3D几何，在视觉质量、几何一致性和对极端视角变化的鲁棒性方面显著优于现有方法。

Conclusion: ProDiG通过渐进式扩散引导框架成功解决了从航拍图像生成地面级视图的挑战，实现了无需多高度地面真值数据的几何一致细化，为极端视角变化下的3D场景重建提供了有效解决方案。
...
TL;DR: DR-Seg提出解耦-校正框架，将CLIP特征分解为语义主导和结构主导子空间，针对性增强遥感图像开放词汇分割的结构细节，同时保持语言对齐语义完整性。


<details>
  <summary>Details</summary>
Motivation: 遥感开放词汇分割需要语言对齐识别和细粒度空间划分。CLIP具有语义泛化能力但缺乏结构细节，现有方法引入DINO特征但将CLIP视为单一语义空间，无法定位结构增强需求，可能破坏CLIP语义完整性。

Method: DR-Seg框架：1) 基于CLIP通道功能异质性，解耦为语义主导和结构主导子空间；2) 针对性使用DINO增强结构子空间而不扭曲语言对齐语义；3) 先验驱动图校正模块在DINO指导下注入高保真结构先验；4) 不确定性引导自适应融合模块动态集成精炼分支和原始CLIP分支。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进性能。

Conclusion: DR-Seg通过解耦CLIP特征的功能异质性，实现了针对性的结构增强，在保持语言对齐语义完整性的同时显著提升了遥感开放词汇分割的边界划分能力。
...
TL;DR: LinkS²Bench是首个评估视觉语言模型在无人机-卫星协同空间智能方面的基准，包含17.9k高质量问答对，覆盖12个细粒度任务，揭示了现有模型在跨视图动态对齐方面的显著差距。


<details>
  <summary>Details</summary>
Motivation: 无人机与卫星的协同空间智能对应急响应和安全操作至关重要，但现有基准局限于孤立的无人机视频或静态卫星图像，无法评估动态局部到全局的空间映射能力，这一研究空白亟待填补。

Method: 构建LinkS²Bench基准，将1022分钟动态无人机视频与覆盖200+ km²的高分辨率卫星图像关联；采用LMM辅助流水线和严格人工标注，创建17.9k高质量问答对，涵盖感知、定位、关系、推理四个维度的12个细粒度任务；设计跨视图对齐适配器来改善模型性能。

Result: 评估18个代表性VLM显示，与人类基线相比存在显著差距，准确跨视图动态对齐是关键瓶颈；跨视图对齐适配器显著提升模型性能；微调实验证实LinkS²Bench在推进VLM适应复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估VLM在无人机-卫星协同空间智能方面的空白，揭示了跨视图动态对齐的关键挑战，并通过对齐适配器展示了改进路径，为推进复杂空间推理的VLM研究提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 在医学成像、生物学和物理学等领域，图像内容的空间采样通常不均匀，信息丰富的模式在特定坐标上很少出现，而背景在这些位置占主导地位。这导致自编码器偏向于重建主要模式，丢失细粒度细节，在罕见空间输入下产生模糊重建，特别是在空间数据不平衡的情况下。

Method: 提出两个互补组件：1）基于自熵的损失函数，对统计上不常见的空间位置进行加权；2）样本传播机制，在训练过程中选择性地重新暴露模型难以重建的样本。该方法专门针对空间不平衡问题，鼓励模型关注统计上罕见的位置。

Result: 在具有受控空间不平衡条件的模拟数据集以及三个不受控制的真实世界数据集（涵盖物理、生物和天文领域）上进行验证。该方法在各种重建指标上优于基线方法，特别是在空间不平衡分布下表现更佳。

Conclusion: 研究结果强调了批次中数据表示的重要性，并在无监督图像重建中突出了罕见样本的价值。该方法通过针对空间不平衡的专门设计，提高了重建一致性，为处理空间非均匀采样问题提供了有效解决方案。
...
TL;DR: IndoorCrowd是一个多场景室内人群数据集，包含检测、实例分割和多目标跟踪任务，在四个校园地点采集，包含31个视频和9,913帧，提供人类验证的分割掩码，并评估了三个基础模型自动标注器的性能。
...
Method: 在四个校园地点（ACS-EC、ACS-EG、IE-Central、R-Central）采集31个视频（9,913帧，5fps），提供人类验证的实例分割掩码。使用620帧控制子集评估三个基础模型自动标注器（SAM3、GroundingSAM、EfficientGroundingSAM），使用Cohen's κ、AP、精确率、召回率和掩码IoU指标。另外2,552帧子集支持MOTChallenge格式的多目标跟踪。使用YOLOv8n、YOLOv26n、RT-DETR-L与ByteTrack、BoT-SORT、OC-SORT建立检测、分割和跟踪基线。

Result: 场景分析显示难度差异显著：ACS-EC场景具有79.3%的密集帧和平均实例尺度60.8像素，是最具挑战性的场景。自动标注器评估提供了基准性能数据，多目标跟踪子集支持连续身份跟踪。

Conclusion: IndoorCrowd数据集填补了大规模真实室内人群数据集的空白，为检测、分割和跟踪任务提供了基准，并揭示了场景特性（如人群密度、尺度和遮挡）对算法性能的显著影响。
...
<details>
  <summary>Details</summary>
Motivation: 链式思维推理显著提升了多模态模型在语言引导分割任务中的性能，但其生成冗长推理过程带来的高昂计算成本限制了实际应用。需要一种既能保持推理能力又能大幅降低计算开销的高效方法。

Method: WISE采用"思考两次"范式：1)训练阶段：模型生成结构化序列（简洁推理→最终答案→详细解释），通过自回归条件约束确保简洁推理足以生成详细解释；2)使用自蒸馏目标联合奖励语义保真度和简洁性；3)推理阶段：采用WISE-S策略，通过简洁性指令提示激活学习的简洁策略，仅输出简洁推理。

Result: 在ReasonSeg基准测试中，WISE-S实现了58.3 cIoU的零样本SOTA性能，同时将平均推理长度从112个token减少到23个token，压缩近5倍，显著降低了计算成本。

Conclusion: WISE通过结构化训练和简洁推理策略，成功解决了链式思维推理的计算效率问题，在保持高性能的同时大幅降低了推理开销，为多模态模型的实时应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 构建高质量的多语言和非英语视觉语言模型面临的主要障碍是：其他语言的视觉问答数据集在规模和领域覆盖上都有限制。英语可以通过聚合现有VQA资源构建大规模训练数据集，但这一策略不适用于日语等语言。

Method: 收集异构源数据（图像、图像-文本对、PDF文档），通过多种策略生成VQA对：1）基于VLM的问答生成，2）翻译，3）文本渲染。构建了包含约920万实例的日本多模态后训练数据集Jagle。

Result: 使用Jagle训练的22亿参数模型在日语任务上表现优异：在十个日语评估任务中平均得分超过InternVL3.5-2B，接近Qwen3-VL-2B-Instruct（差距在5分以内）。结合Jagle和FineVision训练不仅不损害英语性能，反而比单独使用FineVision表现更好。

Conclusion: Jagle作为最大的日本多模态后训练数据集，通过创新的数据生成策略解决了日语VLM训练数据稀缺问题。实验证明该方法有效，能训练出高性能的日语VLM且保持英语能力。作者开源了数据集、模型和代码以促进可复现性和未来研究。
...
TL;DR: 提出了一种自动评估虚拟人皮肤色调保真度的系统方法，通过整合肤色提取、纹理重着色、实时渲染和定量分析，发现较深肤色的色度误差更高。
...
Method: 提出全自动工作流程：整合肤色和光照提取、纹理重着色、实时渲染和定量色彩分析。使用芝加哥人脸数据库，比较基于脸颊区域采样和全脸分析的多维掩码两种肤色提取策略，结合TRUST框架进行光照隔离。将提取的肤色应用于MetaHuman纹理并在多种光照配置下渲染，使用CIELAB色彩空间的ΔE度量和个体类型学角度(ITA)客观评估肤色一致性。

Result: 分析了约19,848个渲染实例，结果显示提取策略表现出表型依赖性行为，较深肤色的色度误差始终更高。

Conclusion: 该方法无需人工干预，计算成本低，支持大规模评估，揭示了虚拟人生成流程中肤色保真度的系统性偏差，特别是对较深肤色的不一致性。
...
TL;DR: 提出多物体可供性定位新任务，解决在混乱多物体场景中基于隐含语言意图选择正确物体的问题，构建首个基准数据集CompassAD并开发CompassNet框架


<details>
  <summary>Details</summary>
Motivation: 现有3D可供性方法主要评估孤立单物体，通常需要查询中提供明确类别名称，无法处理现实场景中多个物体共享相同可供性但只有一个是任务上下文下合适选择的情况（混淆对问题）

Method: 提出CompassNet框架，包含两个专门模块：实例边界交叉注入（ICI）将语言-几何对齐约束在物体边界内防止跨物体语义泄漏；双层对比细化（BCR）在几何组和点级别强制执行区分，增强目标与混淆表面之间的差异

Result: 构建首个基准数据集CompassAD，包含30个混淆物体对、16种可供性类型、6,422个场景和88K+查询-答案对；实验显示在可见和未见查询上均达到最先进结果；在机器人机械臂上部署证实能有效转移到真实世界混淆多物体场景的抓取任务

Conclusion: 形式化了意图驱动指令下的多物体可供性定位新任务，解决了混淆对挑战；提出的CompassNet框架通过ICI和BCR模块有效处理语言-几何对齐和区分问题；在合成和真实世界场景中均验证了方法的有效性
...
TL;DR: PLUME提出了一种潜在推理框架，用连续潜在状态的自回归展开替代显式思维链，在保持推理优势的同时大幅降低计算开销，实现30倍推理加速


<details>
  <summary>Details</summary>
Motivation: 现有通用多模态嵌入方法通过生成显式思维链来提升复杂查询意图理解，但存在显著推理开销，且将丰富的多模态证据压缩到狭窄的文本瓶颈中

Method: PLUME采用潜在推理框架，用短自回归连续潜在状态展开替代显式思维链；引入语义锚点引导的转换适配器，在固定计算预算下引导不同推理轨迹；采用渐进式显式到潜在课程学习，将显式推理行为逐步转移到隐藏状态计算

Result: 在78任务的MMEB-v2基准测试中，PLUME优于强显式思维链基线，同时将推理从数百个生成token减少到少于10个潜在步骤，实现超过30倍的推理加速

Conclusion: 结构化潜在计算可以在不产生显式理由生成开销的情况下保留中间推理的优势，为实际检索系统提供了更强更高效的范式，特别适用于证据密集、结构复杂且难以通过显式中间理由组织的检索场景
...
TL;DR: GroundVTS：一种用于视频时间定位的Vid-LLM架构，通过查询引导的细粒度视觉令牌采样机制，聚焦信息最丰富的视频片段，解决现有方法因均匀帧采样导致的关键帧稀疏和时序信息丢失问题。
...
Method: 提出Grounded Visual Token Sampling (GroundVTS)架构：1）采用细粒度的查询引导机制过滤视觉令牌，保留关键时空信息；2）引入渐进优化策略，使LLM适应视觉特征的非均匀分布，增强时序依赖建模能力。

Result: 在三个标准VTG基准测试中，GroundVTS显著优于现有方法：在时刻检索任务上mIoU提升7.7点，在高光检测任务上mAP提升12.0点。

Conclusion: GroundVTS通过聚焦信息丰富的时序片段和渐进优化策略，有效解决了Vid-LLMs在视频时间定位中的关键帧稀疏问题，显著提升了时序建模精度和定位性能。
...
TL;DR: LatentUM提出了一种在共享语义潜在空间中表示所有模态的统一模型，消除了视觉理解与生成之间像素空间中介的需求，实现了高效的交错跨模态推理和生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型需要像素解码作为视觉理解与生成之间的桥梁，由于两者使用分离的视觉表示，导致效率低下且效果不佳。需要一种更有效的跨模态推理和生成方法。

Method: LatentUM将所有模态表示在共享的语义潜在空间中，避免了像素空间的中介。这种设计自然支持灵活的交错跨模态推理和生成，减轻编解码器偏差并增强跨模态对齐。

Result: LatentUM在视觉空间规划基准上达到最先进性能，通过自反思推动视觉生成极限，并支持在共享语义潜在空间中预测未来视觉状态的世界建模。

Conclusion: 共享语义潜在空间表示消除了像素解码需求，显著提高了计算效率，增强了跨模态对齐，为交错跨模态推理和生成提供了更有效的统一模型框架。
...
TL;DR: 提出CoRegOVCD框架，通过一致性正则化解决开放词汇变化检测中的噪声和碎片化问题，实现无需训练的概念查询变化检测


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法通常假设固定的标签空间，无法回答用户自定义的查询。开放词汇变化检测允许查询任意概念，但在无需训练的设置下，由于外观变化、弱跨概念竞争和空间连续性等因素，直接比较不同时间点的密集概念响应会产生噪声、碎片化和语义不可靠的变化证据。
...
Conclusion: CoRegOVCD通过一致性正则化框架有效解决了开放词汇变化检测中的噪声和碎片化问题，实现了更可靠、空间一致的变化检测，无需额外训练即可显著提升性能。
...
TL;DR: 论文分析了面部动作单元检测中标准评估协议（被试专属交叉验证）存在的问题，发现交叉验证本身引入了可测量的随机方差，而跨数据集验证（留一数据集出）能提供更稳定和可解释的结果。


<details>
  <summary>Details</summary>
Motivation: 当前面部动作单元检测研究中，被试专属交叉验证是标准评估协议，但报告的性能提升往往很小。研究者怀疑交叉验证本身可能引入了随机方差，导致结果不稳定，需要更可靠的评估方法来准确衡量模型性能。

Method: 采用两种评估方法：1）在BP4D+数据集上进行重复3折被试专属分割，测量交叉验证的随机方差；2）使用留一数据集出协议在五个动作单元数据集上进行跨数据集鲁棒性评估，分析领域级不稳定性。

Result: 研究发现：1）重复3折被试专属分割产生了±0.065的平均F1经验噪声底限，低流行度动作单元的方差更大；2）操作点指标（如F1）比阈值无关指标（如AUC）波动更大；3）模型排名在不同折分配下可能改变；4）留一数据集出协议消除了分区随机性，揭示了单数据集交叉验证中不可见的领域级不稳定性。

Conclusion: 交叉验证中经常报告的性能提升可能落在协议方差范围内，因此这些提升可能并不显著。留一数据集出交叉验证能提供更稳定和可解释的研究发现，建议在面部动作单元检测研究中采用这种更可靠的评估协议。
...
TL;DR: 该论文提出了一种反射生成方法，通过向基础扩散模型注入反射位置和外观的先验信息，并采用类型感知的模型设计，构建了首个大规模物体反射数据集DEROBA，生成了物理一致且视觉逼真的反射效果。
...
Conclusion: 本研究提出了一种有效的反射生成方法，通过注入先验信息和类型感知设计，结合大规模数据集DEROBA，成功实现了物理一致且视觉逼真的反射生成，填补了反射生成领域的研究空白。
...
<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但理解其工作原理仍具挑战性，尤其是在视觉领域。现有可解释性工具往往关注孤立组件或专家导向分析，缺乏对完整推理管道的引导式端到端理解。

Method: 开发ViT-Explainer系统，提供从patch tokenization到最终分类的集成可视化。系统结合动画演示、patch级注意力覆盖图和视觉适配的Logit Lens，支持引导和自由探索两种模式。

Result: 6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了现有可解释性工具的空白，为Vision Transformer提供了引导式的端到端可视化理解工具，有助于提升模型透明度和可解释性。
...
TL;DR: UniDriveVLA通过专家解耦和混合Transformer架构解决自动驾驶VLA模型中空间感知与语义推理的矛盾，在多个任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶VLA模型面临空间感知与语义推理的权衡困境：直接采用2D VLM空间感知有限，增强3D表示又会损害VLM的推理能力。这种矛盾源于空间感知和语义推理在共享参数中的耦合优化。
...
Result: 在nuScenes开环评估和Bench2Drive闭环评估中达到最先进性能。在3D检测、在线建图、运动预测、驾驶导向VQA等广泛任务上表现出色，展示了作为自动驾驶统一模型的广泛适用性。

Conclusion: UniDriveVLA通过专家解耦架构有效解决了自动驾驶VLA模型中空间感知与语义推理的冲突，实现了感知与推理能力的协同提升，为自动驾驶系统提供了统一且强大的基础模型。
...
TL;DR: SCALE是一个轻量级确定性框架，通过语义和置信度感知的列表能量排序实现零样本骨架动作识别，避免显式的骨架-文本对齐，在未见类别上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖显式的骨架-文本对齐，当动作名称无法充分描述细粒度动态或未见类别语义混淆时，这种方法变得脆弱。需要更鲁棒的框架来处理语义模糊和类别混淆问题。
...
Conclusion: SCALE通过能量排序框架有效解决了零样本骨架动作识别中的语义模糊问题，避免了脆弱的显式对齐，实现了更好的类别区分和语义组织。
...
<details>
  <summary>Details</summary>
Motivation: 基础视觉Transformer在需要细粒度空间理解的任务中存在局限性，主要原因是固定的预训练分辨率和固有的粗粒度patch级表示。在密集预测场景（如开放词汇分割）中，高分辨率输入对准确的像素级推理至关重要，但现有滑动窗口方法计算成本高昂。

Method: 提出SPAR（Single-Pass Any-Resolution ViT），一种分辨率无关的密集特征提取器。通过特征回归损失，将细粒度步长的滑动窗口教师模型的空间推理能力蒸馏到单次推理学生模型中，无需架构修改或像素级监督。

Result: 在开放词汇分割任务中，SPAR将单次推理基线提升了高达10.5 mIoU，甚至超过了教师模型的性能，证明了在高分辨率推理中的有效性。

Conclusion: SPAR通过知识蒸馏实现了高效的高分辨率推理，解决了ViT在密集预测任务中的分辨率限制问题，为开放词汇分割等任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全方法通常依赖模型微调或精选数据集，这会降低生成质量或限制可扩展性。需要一种无需训练、可扩展且保持生成质量的安全控制方法。

Method: 利用视觉-语言基础模型编码的丰富语义表示作为现成的监督信号，通过在每个采样步骤注入干净潜在估计的梯度反馈，将安全引导构建为基于能量的采样问题。

Result: 在NSFW红队测试基准上达到最先进的鲁棒性，实现有效的多目标引导，同时在良性非目标提示上保持高生成质量。框架兼容扩散模型和流匹配模型，并能泛化到多样视觉概念。
...
TL;DR: Omni123是一个3D原生基础模型，在单一自回归框架内统一文本到2D和文本到3D生成，利用丰富的2D数据作为几何先验来改进3D表示。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在统一文本和图像理解与生成方面表现优异，但扩展到3D领域面临挑战，主要原因是高质量3D数据稀缺导致3D合成约束不足。现有方法通常依赖间接流程，在2D中编辑然后通过优化提升到3D，牺牲了几何一致性。

Method: 将文本、图像和3D表示为共享序列空间中的离散标记，利用丰富的2D数据作为几何先验。引入交错X-to-X训练范式，协调异构配对数据集上的多样化跨模态任务，无需完全对齐的文本-图像-3D三元组。通过在自回归序列中遍历语义-视觉-几何循环（如文本到图像到3D到图像），联合强制执行语义对齐、外观保真度和多视图几何一致性。

Result: 实验表明，Omni123显著改进了文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径。

Conclusion: 该研究提出了一个统一的3D原生基础模型，通过利用2D数据作为几何先验，在单一自回归框架内实现了文本到2D和文本到3D生成的统一，为解决3D数据稀缺问题提供了新思路。
...
TL;DR: VOID是一个视频物体移除框架，专门处理物体移除后需要改变下游物理交互的复杂场景，通过视觉语言模型识别受影响区域并引导视频扩散模型生成物理一致的替代结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频物体移除方法擅长处理物体"背后"的内容修复和外观层面的伪影（如阴影和反射），但当移除的物体与其他物体有更显著的交互（如碰撞）时，当前模型无法修正这些交互，产生不合理的物理结果。

Method: 1) 使用Kubric和HUMOTO生成新的配对数据集，包含需要改变下游物理交互的反事实物体移除场景；2) 推理时，使用视觉语言模型识别场景中受移除物体影响的区域；3) 用这些区域引导视频扩散模型生成物理一致的反事实结果。

Result: 在合成数据和真实数据上的实验表明，与先前的视频物体移除方法相比，VOID方法在物体移除后能更好地保持一致的场景动态，产生更物理合理的结果。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D化身建模面临保真度与泛化性的关键权衡：基于多视角工作室数据的方法保真度高但泛化性差；基于大规模野外数据的方法泛化性好但质量低。需要一种既能保持高质量又能泛化到大规模人群的解决方案。

Method: 提出预训练/后训练范式：首先在100万个野外视频上进行预训练，学习外观和几何的广泛先验；然后在高质量策划数据上进行后训练，增强表现力和保真度。模型采用编解码器架构，实现前馈推理。

Result: LCA能够泛化到不同发型、服装和人口统计特征，同时提供精细的面部表情和手指级关节控制，保持强身份保留。模型展现出对重新光照和宽松服装支持的涌现泛化能力，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA首次展示了大规模3D化身建模的预训练/后训练范式，成功解决了保真度与泛化性的权衡问题，为高质量、可泛化到世界规模人群的3D化身建模提供了有效解决方案。
...
TL;DR: 提出Steerable Visual Representations，一种可通过自然语言引导的视觉表示方法，在保持基础表示质量的同时，能够聚焦于图像中任意指定对象


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉Transformer（如DINOv2、MAE）虽然提供通用图像特征，但倾向于关注最显著视觉线索，无法引导至次要概念；而多模态LLM虽可通过文本提示引导，但其表示偏向语言中心化，在通用视觉任务中效果下降

Method: 通过轻量级交叉注意力将文本直接注入视觉编码器的各层（早期融合），而非像CLIP等模型在编码后进行融合（晚期融合），从而创建可引导的视觉表示

Result: 方法在表示可引导性基准测试中表现优异，能够聚焦于图像中任意所需对象，同时保持底层表示质量；在异常检测和个性化对象区分任务上匹配或超越专用方法，并展现对分布外任务的零样本泛化能力

Conclusion: 提出的Steerable Visual Representations成功解决了现有视觉表示方法的局限性，实现了通过自然语言精确引导视觉特征的能力，为通用视觉任务提供了更灵活有效的表示框架
...
TL;DR: 提出场景化视觉定位新任务，建立RSC基准测试，包含段落级场景描述而非字面指称，开发ScenGround课程学习方法，揭示现有模型系统性缺陷


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型可通过匹配显著命名类别成功。需要探索更具挑战性的场景化视觉定位，要求从角色、意图和关系上下文中推理目标而非显式命名。

Method: 引入Referring Scenario Comprehension (RSC)基准，包含段落长度文本描述对象角色、用户目标和上下文线索，标注可解释难度标签。提出ScenGround课程推理方法，结合监督预热和难度感知强化学习。

Result: RSC包含约31k训练样本、4k域内测试样本和3k分布外样本。实验表明场景化查询暴露了现有模型在标准基准中未揭示的系统性失败，课程训练提高了在挑战性切片上的性能并迁移到标准基准。

Conclusion: 场景化视觉定位是视觉定位中一个互补且更具挑战性的设置，RSC基准和ScenGround方法为这一方向提供了评估框架和参考点，揭示了现有模型的局限性并展示了课程学习的有效性。
...
TL;DR: ActionParty：首个可同时控制多个主体的视频世界模型，通过引入主体状态令牌解决现有视频扩散模型中动作绑定的基本问题
...
Method: 提出ActionParty模型，引入主体状态令牌（潜在变量）持续捕捉每个主体的状态；通过空间偏置机制联合建模状态令牌和视频潜在表示，实现全局视频帧渲染与个体动作控制主体更新的解耦

Result: 在Melting Pot基准测试中评估，展示了首个能够同时在46个多样化环境中控制多达7个玩家的视频世界模型；结果显示在动作跟随准确性和身份一致性方面有显著改进，同时能够通过复杂交互实现鲁棒的自回归主体跟踪

Conclusion: ActionParty成功解决了多主体视频世界模型中的动作绑定问题，通过主体状态令牌和空间偏置机制实现了对多个主体的同时精确控制，为生成式视频游戏提供了有效的解决方案
...
TL;DR: ModMap是一个原生多视角多模态的3D异常检测与分割框架，通过跨模态特征映射和视角相关建模，在SiM3D基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常独立处理不同视角，缺乏对多视角和多模态数据的统一建模。工业3D异常检测需要同时利用深度和RGB信息，并有效整合多个视角的特征。

Method: 1. 采用跨模态特征映射范式学习跨模态和跨视角的特征映射；2. 通过特征调制显式建模视角依赖关系；3. 引入跨视角训练策略利用所有可能的视角组合；4. 训练并公开了针对工业数据集的基础深度编码器；5. 通过多视角集成和聚合进行异常评分。

Result: 在SiM3D基准测试中，ModMap取得了最先进的性能，大幅超越了先前的方法。SiM3D是首个为3D异常检测和分割设计的多视角多模态基准。

Conclusion: ModMap通过原生多视角多模态框架、跨模态特征映射和视角相关建模，为3D异常检测和分割提供了有效的解决方案，显著提升了性能。
...
TL;DR: 提出一个从AAA游戏中提取的大规模动态数据集，包含400万连续帧的RGB和G-buffer数据，用于提升生成式逆渲染和正向渲染的真实感和时间一致性。
...
Method: 采用新颖的双屏拼接捕获方法，从视觉复杂的AAA游戏中提取了400万连续帧（720p/30 FPS）的同步RGB和五个G-buffer通道数据，涵盖多样化场景、视觉效果和环境条件。同时提出基于视觉语言模型（VLM）的评估协议来测量语义、空间和时间一致性。

Result: 使用该数据集微调的逆渲染器实现了优异的跨数据集泛化能力和可控生成。VLM评估与人类判断高度相关。结合工具包，正向渲染器允许用户通过文本提示从G-buffer编辑AAA游戏的风格。

Conclusion: 该大规模动态数据集显著提升了双向渲染的性能，通过高质量数据和创新评估方法推动了生成式逆渲染和正向渲染在真实世界场景中的应用。
...
TL;DR: 该论文研究了在线交流中常见的重复延长形式（RLF）对情感分析的重要性，创建了首个多领域RLF数据集Lengthening，并提出了可解释指令调优框架ExpInstruct来提升LLMs对RLF的理解和解释能力。


<details>
  <summary>Details</summary>
Motivation: 在线交流中人们经常使用非正式表达方式（如表情包、表情符号），但一种独特的强调风格——重复延长形式（RLF）多年来一直被忽视。本研究旨在探索RLF对情感分析的重要性，以及语言模型是否能理解这种表达形式。

Method: 1) 基于先前语言学研究成果，创建了首个多领域RLF数据集Lengthening（85万个样本）；2) 提出了可解释指令调优框架ExpInstruct，这是一个两阶段的指令调优框架；3) 提出了一种统一的方法来量化语言模型对非正式表达的理解能力。

Result: 1) RLF句子是富有表现力的表达，可作为文档级情感的特征标志；2) RLF对在线内容分析具有潜在价值；3) 微调的预训练语言模型在性能上可以超越零样本GPT-4，但在解释能力上不如GPT-4；4) ExpInstruct能够用有限样本提升开源LLMs，使其在RLF任务上达到与零样本GPT-4相当的性能和解释能力。

Conclusion: RLF对情感分析具有重要意义，语言模型需要专门优化才能更好地理解和解释这种非正式表达形式。ExpInstruct框架为提升LLMs对RLF的理解提供了有效途径，有助于改进在线内容分析任务。
...
TL;DR: 该研究通过训练时强化学习和测试时并行思考两种互补方法，扩展竞争性编程的推理令牌预算，使用16线程16轮并行思考系统在AetherCode的456个难题上超越GPT-5-high


<details>
  <summary>Details</summary>
Motivation: 解决竞争性编程中单次生成推理在强化学习训练下因完全注意力机制而成本高昂的问题，同时探索如何有效扩展推理令牌预算以提高模型性能

Method: 采用两种互补方法：1) 训练时强化学习，包括验证RL预热和随机裁剪技术；2) 测试时多轮并行思考管道，将令牌预算分配到多个线程和轮次中，进行生成、验证和精炼

Result: 从Seed-OSS-36B开始，完整系统使用16线程和每线程16轮，平均每个问题使用760万令牌，在AetherCode的456个难题上超越了GPT-5-high，并在pass@1上匹配了基础RL模型的oracle pass@16

Conclusion: 通过训练时强化学习和测试时并行思考的协同作用，可以有效扩展竞争性编程的推理令牌预算，显著提升模型在复杂编程问题上的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏评估科学主张与多模态证据一致性的规模、领域多样性和视觉复杂性，需要更真实的数据集来评估这种对齐
...
Conclusion: M2-Verify填补了评估科学主张与多模态证据一致性的空白，展示了现有模型的局限性，提供了数据集的使用指南和实用性证明
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型学习人类偏好的方法面临根本性挑战，因为奖励建模依赖于微妙、主观的比较而非明确的标签。现有方法在标准成对偏好设置下性能有限（ROC AUC低于0.74），需要更好地捕捉人类判断的多维特性。

Method: 提出混合特征增强框架，在文本表示中融入可解释信号：响应长度、拒绝指标、毒性分数和提示-响应语义相似度。使用Anthropic HH-RLHF数据集评估10个不同的大语言模型，并集成SHAP和LIME进行细粒度可解释性分析。

Result: 特征增强方法在所有模型上都带来了一致的改进，最高达到0.84 ROC AUC和显著更高的成对准确率，其中DeBERTa-v3-Large表现最佳。可解释性分析显示模型决策依赖于情境化的安全性和支持性框架而非孤立关键词。

Conclusion: 通过增强文本表示与可解释特征，可以显著改善语言模型对人类偏好的学习。该方法不仅提升了预测性能，还通过可解释性分析揭示了偏好学习的机制，为理解模型决策和减少偏见放大提供了洞见。
...
Method: 1. 从现有的逐步推理轨迹语料库开始，将每个轨迹分解为自包含的子问题-子程序对，构建包含3200万个紧凑程序性知识条目的数据存储。2. 在推理时，通过轻量级的思维内提示让模型表达核心子问题，在其推理轨迹内检索相关子程序，并在多样检索到的子程序作为隐式程序先验下进行推理。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory始终优于使用文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时缩放基线。在更高推理预算下，相比无检索提升达19.2%，相比最强计算匹配基线提升7.9%。消融研究表明，这些收益来自两个关键因素：源轨迹的广泛程序覆盖范围，以及分解和检索设计，共同实现了程序性知识的有效提取和重用。

Conclusion: Reasoning Memory通过系统性地检索和重用程序性知识，显著提升了语言模型在复杂推理任务上的表现。该方法证明了程序性知识重用对测试时缩放的重要性，为改进推理模型提供了新方向。
...
TL;DR: 该研究通过定位实体选择性MLP神经元，揭示了语言模型回答实体中心事实问题的内部机制，发现早期层存在稀疏的、因果可操作的实体检索神经元


<details>
  <summary>Details</summary>
Motivation: 语言模型能够回答许多实体中心的事实问题，但其内部机制尚不清楚。本研究旨在探索语言模型在处理实体相关事实查询时的内部工作机制

Method: 使用模板化提示定位实体选择性MLP神经元，通过因果干预验证这些神经元在PopQA问答示例中的作用。在从PopQA中选取的200个实体上进行实验，分析神经元的分布和功能

Result: 定位的神经元主要集中在早期层；负向消融导致实体特异性遗忘，而在占位符处注入神经元激活能改善答案检索；许多实体仅需激活单个神经元即可恢复实体一致性预测；神经元对别名、缩写、拼写错误和多语言形式具有鲁棒性；效果显著但不普遍，流行实体覆盖率更高

Conclusion: 研究发现了稀疏的、因果可操作的访问点，可用于分析和调节实体条件化的事实行为，支持实体检索的规范化解释而非纯粹的深度渐进丰富
...
<details>
  <summary>Details</summary>
Motivation: 翻译过程中的打字停顿通常被认为反映了翻译问题的存在，但如何确定区分自动化与反思性翻译过程的停顿阈值一直存在争议。本文旨在比较现有方法并提出更优的解决方案。

Method: 比较了三种计算停顿阈值的现有方法，并提出了一种新的计算生产单元中断的方法进行评估

Result: 通过对不同方法的比较分析，提出了改进的生产单元中断计算方法，并对其进行了评估

Conclusion: 新的生产单元中断计算方法为更准确地识别翻译过程中的认知负荷和问题提供了更好的工具，有助于深化对翻译过程的理解
...
TL;DR: 本文提出基于预测效度的基准测试质量评估框架，通过收集大规模项目级数据集WILD，结合改进的多维项目反应理论和自适应项目选择，实现用少量样本高效预测模型在未见任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模语言模型基准测试数量庞大但效率低下，研究表明模型性能可由少量潜在能力因子解释。需要建立更高效、有原则的基准测试方法，但缺乏比较不同方法质量的客观标准。本文提出基于预测效度的评估框架，即基准测试质量应体现在其预测模型在未见任务上性能的效率。

Method: 1) 收集WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务（来自27个数据集）上的109,564个独特项目-模型响应对；2) 结合改进的多维项目反应理论模型和基于最优实验设计的自适应项目选择方法；3) 引入成本感知折扣因子优化选择标准；4) 在不同预算约束下分析预测模型在未见任务上性能的能力。

Result: 1) 仅观察16个项目后，就能以小于7%的平均绝对误差预测112个保留基准任务的性能；2) 引入成本感知折扣因子后，达到7%MAE所需的总token数从141,000减少到22,000，评估成本降低85%；3) WILD数据集首次支持在大规模多样化未见任务集合上分析不同预测技术的效果。

Conclusion: 基于预测效度的基准测试框架结合改进的多维项目反应理论和自适应项目选择，能够显著提高大规模语言模型评估的效率。该方法在极少量样本下就能准确预测模型在未见任务上的性能，并通过成本优化大幅降低评估开销，为高效、有原则的基准测试提供了可行方案。
...
TL;DR: 使用词嵌入和随机森林模型，研究发现印地语中梵语和波斯-阿拉伯语源的同义词在分布模式上存在可区分的差异，表明上下文编码了词源信号，同义词可能反映与起源相关的细微但系统性的区别。


<details>
  <summary>Details</summary>
Motivation: 研究同义词现象，特别是绝对同义词理论上不应存在，因为它们不会扩展语言的表达能力。然而，即使同义词表示相同的概念，它们可能反映不同的视角或承载不同的文化关联，这些主张很少被定量测试。印地语中梵语和波斯-阿拉伯语借词与其梵语对应词共存，形成了许多同义词对，为研究提供了理想材料。

Method: 使用印地语同义词的词嵌入数据训练随机森林模型，仅基于分布数据（不考虑语义内容）来分类词语的梵语或波斯-阿拉伯语起源。该方法旨在测试是否能够仅从使用模式中区分词源。

Result: 随机森林模型成功地将印地语同义词按其梵语或波斯-阿拉伯语起源进行分类，即使这些词在语义上无关。这表明使用模式保留了词源的痕迹，上下文编码了词源信号。

Conclusion: 研究提供了定量证据，表明上下文编码词源信号，同义词可能反映与起源相关的细微但系统性的区别。结果支持同义词可以提供不同视角的观点，词源相关的词语可能形成不同的概念子空间，创造一种由历史起源塑造的新型语义框架。总体而言，结果突显了上下文在捕捉超越传统语义相似性的细微区别方面的能力。
...
TL;DR: 研究发现引用粒度（句子级、段落级、文档级）对模型归因质量有显著影响，中等粒度（段落级）表现最佳，过细或过粗的引用都会损害归因效果，且模型规模越大受细粒度约束的负面影响越明显。


<details>
  <summary>Details</summary>
Motivation: 引用粒度（句子级、段落级或文档级）是归因生成的关键设计选择。虽然细粒度引用通常更便于人工验证，但其对模型性能的影响尚未充分研究。本文旨在探索不同引用粒度如何影响模型的归因质量和生成可靠性。

Method: 分析了四种不同规模的模型（8B-120B），比较了不同引用粒度（句子级、段落级、多段落级）对归因质量的影响。研究了细粒度引用约束如何破坏语义依赖关系，以及过粗引用如何引入噪声干扰。

Result: 1. 强制细粒度引用会使归因质量下降16-276%（相比最佳粒度）；2. 归因质量在中间粒度（段落级）达到峰值；3. 细粒度引用破坏了证据归因所需的语义依赖，而过粗引用引入了干扰噪声；4. 性能差距随模型规模非单调变化，细粒度约束对更大模型的负面影响更显著；5. 最优引用粒度在提升归因质量的同时保持甚至提高了答案正确性。

Conclusion: 仅为了便于人工验证而优化细粒度引用忽视了模型约束，会损害归因忠实性和生成可靠性。有效的归因需要将引用粒度与模型的自然语义范围对齐，而非一味追求最细粒度。
...
TL;DR: 该论文对LLM中过度自信表达进行电路级机制分析，发现特定MLP块和注意力头导致置信度膨胀，并提出针对性推理时干预改善校准


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生事实错误的答案，并且会过度自信地表达这种错误，这种过度自信的表达会误导用户并削弱置信分数作为可靠不确定性信号的可靠性，但其内部机制尚不清楚

Method: 围绕三个轴进行电路级机制分析：将语言化置信度捕获为可微分的内部信号，识别导致其膨胀的电路，并利用这些见解进行针对性推理时重新校准；在两个指令调优的LLM和三个数据集上进行实验

Result: 发现在中晚期层集中的一组紧凑的MLP块和注意力头，在最终标记位置一致地写入置信度膨胀信号；针对性推理时干预显著改善了校准

Conclusion: LLM中的语言化过度自信由可识别的内部电路驱动，可以通过针对性干预来缓解
...
TL;DR: 该研究通过对129,451首波斯诗歌的分析，发现波斯诗歌中的象征系统不是固定不变的，而是一个长期演变的系统，其内部权重和连接随时间变化。


<details>
  <summary>Details</summary>
Motivation: 传统计算方法将波斯诗歌材料扁平化为孤立的词语或宽泛的文档语义，忽略了波斯诗学中一个实用的组织单位：相关形式以家族形式传播，并通过重复关系获得力量。研究旨在捕捉这种象征系统的动态演变。

Method: 使用包含129,451首诗歌的语料库，将重复形式整合为可追溯的家族，将意象材料与神圣和宫廷参照分离，并在多层图中映射它们的关系。按11个伊斯兰世纪进行分箱分析。

Result: 象征核心相对稀疏，参照成分更密集，两者之间的连接区域是选择性的而非扩散的。某些家族（如Shab、Ruz、Khaak）广泛分布，酒器、花园空间、火焰和抒情声音后期增强，而声望编码和英雄宫廷词汇早期权重更大。模块性增加，跨范围连接减少，宫廷桥梁减弱，神圣桥梁增强。枢纽位置也发生变化。

Conclusion: 波斯象征主义不是一个固定的剧目，而是一个长期存在的系统，其内部权重和连接随时间变化，展现了动态演变而非静态传承。
...
TL;DR: 论文提出了Magic, Madness, Heaven, Sin框架，将大语言模型的输出变化建模为同质-异质轴，根据任务规范目标（认知、交互、社会、安全）进行价值评估，强调输出变化是任务目标塑造的属性而非模型固有特征。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中关于输出变化的术语零散，主要因为任务的规范目标很少明确说明。研究者需要统一的框架来理解不同任务背景下输出变化的价值评估，避免优化一个目标时无意中损害其他目标。

Method: 提出Magic, Madness, Heaven, Sin框架，将输出变化建模为同质-异质轴。将任务组织到四个规范语境中：认知（事实性）、交互（用户效用）、社会（代表性）、安全（鲁棒性）。分析每个语境中的失败模式和词汇，并应用框架分析所有成对的跨语境交互。

Result: 框架揭示了优化一个目标（如提高安全性）可能无意中损害人口统计代表性或创造性多样性。通过分析跨语境交互，展示了不同规范目标之间的权衡和潜在冲突。

Conclusion: 需要基于语境评估输出变化，将其重新定义为由任务目标塑造的属性，而非模型的固有特征。这有助于更细致地理解大语言模型输出变化的价值，并避免跨语境优化时的意外后果。
...
<details>
  <summary>Details</summary>
Motivation: 研究指令遗忘范式是否适用于扩散模型等生成模型，探索自然语言指令在图像生成模型中进行概念抑制的有效性

Method: 通过跨多个概念和提示变体的受控实验，分析CLIP文本编码器和去噪过程中的交叉注意力动态，评估指令遗忘在扩散模型中的效果

Result: 扩散模型在仅依赖自然语言遗忘指令时，系统性地无法抑制目标概念；遗忘指令未能持续减少对目标概念标记的关注，导致目标概念表征在整个生成过程中持续存在
...
TL;DR: 提出模型合并框架，通过合并临床基础模型和通用指令模型来解决医学领域微调中的灾难性遗忘问题，实现高效医学领域适应


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学领域用于临床文档以减少临床医生负担，但研究发现当使用特定医学数据集微调时，LLMs经常"遗忘"大量指令遵循能力，这是将通用LLMs应用于临床的关键挑战

Method: 提出模型合并框架，通过基于插值的合并方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，以获取既具有临床任务强性能又保留指令遵循能力的领域适应模型

Result: 综合评估显示合并模型能有效缓解灾难性遗忘，保留临床领域专业知识，保持指令遵循能力。模型合并策略在严重受限监督下(如64-shot vs. 256-shot)达到与完全微调基线相当的性能，展示训练效率

Conclusion: 权重空间合并为开源LLMs适应临床应用提供了高度可扩展的解决方案，促进在资源受限的医疗环境中更广泛部署
...
TL;DR: Swift-SVD：一种激活感知的闭式压缩框架，通过增量协方差聚合和单次特征值分解，实现无训练、快速、最优的层间低秩近似，在保证理论最优的同时显著提升压缩效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的部署受限于静态权重和动态键值缓存的存储与带宽需求。现有SVD压缩方法存在两个关键局限：一些方法在重建误差上非最优，另一些虽理论最优但实际效率低下。

Method: 提出Swift-SVD框架：1）增量聚合给定输入批次下输出激活的协方差；2）聚合后执行单次特征值分解；3）使用有效秩分析局部层间可压缩性；4）设计动态秩分配策略，联合考虑局部重建损失和端到端层重要性。

Result: 在6个LLM和8个数据集上的实验表明，Swift-SVD优于现有最优基线，在实现最优压缩精度的同时，端到端压缩时间提升3-70倍。

Conclusion: Swift-SVD提供了一个同时保证理论最优、实际高效和数值稳定的激活感知闭式压缩框架，为LLM部署中的内存和带宽约束提供了硬件友好的解决方案。
...
TL;DR: 针对印尼K-12教师AI使用情况的全国性调查显示，AI在教学中应用增加但分布不均，主要用于减轻教学准备工作量，但仍面临通用输出、基础设施和情境适配等挑战。
...
Result: 1. AI在教学方法、内容开发和教学媒体方面的使用增加，但采用不均衡；2. 小学教师使用更一致，高中教师参与较少；3. 职业生涯中期教师更重视AI，印尼东部地区教师认为AI价值更高；4. 教师主要用AI减轻教学准备工作量（评估、课程规划、材料开发）；5. 通用输出、基础设施限制和有限的情境适配阻碍了有效的课堂整合。

Conclusion: 研究揭示了印尼教师AI使用的现状和挑战，强调了开发更情境化、支持性的AI系统的重要性，为制定适合当地的教育AI政策提供了实证基础。
...
TL;DR: 该论文通过分析9个数据集的24K声明验证样本，发现现有基准主要测试证据提取能力，而多句合成和数值推理严重不足，高基准分数主要反映检索+蕴含能力而非复杂推理。


<details>
  <summary>Details</summary>
Motivation: 尽管声明验证领域进展迅速，但缺乏对现有基准实际测试推理能力的系统性理解。作者旨在揭示这些基准数据集实际评估的推理类型及其局限性。

Method: 使用GPT-4o-mini为9个数据集的24K声明验证样本生成结构化推理轨迹，分析推理类型分布。使用紧凑的1B参数推理验证器分析五种错误类型，并按领域（通用、科学、数学）进行错误特征分析。

Result: 发现直接证据提取占主导地位（约80%），而多句合成和数值推理严重不足（各约10%）。数据集层面存在显著偏差：某些数据集几乎完全测试词汇匹配，而其他数据集约一半情况需要信息合成。错误特征因领域而异：通用领域主要受词汇重叠偏差影响，科学领域因过度谨慎，数学领域因算术推理失败。

Conclusion: 高基准分数主要反映检索+蕴含能力而非复杂推理能力。需要构建更具挑战性的评估套件，更好地测试验证系统所需的推理能力。提出了改进评估套件的具体建议。
...
<details>
  <summary>Details</summary>
Motivation: 现有情感支持对话方法在有效支持深层上下文理解方面面临挑战，需要更好的机制来增强对上下文的情感推理能力
...
TL;DR: 研究发现：在相同问题集上，使用DeepSeek-R1-0528和gpt-oss-120b生成的CoT轨迹进行SFT训练时，虽然DeepSeek数据训练损失更低，但泛化性能更差，揭示了训练损失与泛化性能之间的悖论。


<details>
  <summary>Details</summary>
Motivation: 探究不同来源的CoT轨迹如何影响大语言模型的泛化性能，理解为什么训练损失与泛化性能之间存在不一致性，为解决这一悖论提供理论依据。

Method: 使用DeepSeek-R1-0528和gpt-oss-120b在相同问题集上生成已验证的CoT轨迹进行对比研究，分析token级SFT损失和步骤级推理行为，提出通过过滤频繁分支轨迹的方法改进SFT泛化。

Result: DeepSeek-R1数据训练损失更低但泛化性能更差，原因是其发散式分支探索模式；过滤频繁分支轨迹后，DeepSeek-R1数据训练的模型在AIME25上提升5.1%，BeyondAIME上提升5.5%，五个基准平均提升3.6%。

Conclusion: CoT轨迹的推理模式（收敛式演绎vs发散式探索）对SFT泛化性能有重要影响，训练损失不能准确反映泛化能力，通过轨迹过滤可以显著改善模型推理性能。
...
TL;DR: EndoASR：针对胃肠内窥镜场景的领域自适应语音识别系统，通过两阶段适应策略显著提升转录准确率和临床可用性，在多中心真实临床环境中验证了其鲁棒性和实时性能。


<details>
  <summary>Details</summary>
Motivation: 传统ASR系统在胃肠内窥镜临床环境中可靠性有限，主要受限于领域特定术语和复杂声学条件，需要专门针对该医疗场景优化的语音识别接口。
...
Conclusion: 领域自适应的ASR系统可作为胃肠内窥镜中人机协作的可靠接口，在多中心真实临床环境中表现出一致的性能，且改进的ASR质量直接增强了下游结构化信息提取和临床医生-AI交互。
...
TL;DR: 该论文系统综述和比较了LLM智能体中的记忆方法，提出了统一框架，在基准测试上全面评估现有方法，并设计出超越SOTA的新记忆方法。


<details>
  <summary>Details</summary>
Motivation: 记忆是LLM智能体处理长期复杂任务的核心模块，现有记忆方法缺乏在相同实验设置下的系统性比较，需要统一框架和全面评估。

Method: 1) 提出统一框架整合现有智能体记忆方法；2) 在两个知名基准上广泛比较代表性记忆方法；3) 利用现有方法模块设计新的记忆方法。

Result: 1) 提供了现有方法的全面分析；2) 设计的新记忆方法超越了现有最先进方法；3) 识别了有前景的未来研究方向。

Conclusion: 对现有记忆方法的深入理解能为未来研究提供宝贵新见解，统一框架和系统性比较有助于推进LLM智能体记忆模块的发展。
...
<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临声学模式模糊和标注数据不足的挑战，特别是在真实场景中情感边界不清晰的情况下，纯数据驱动模型效果有限

Method: 基于LLM推理的人机协作框架：1) 使用声学特征模型提供置信度和特征级证据；2) 置信度路由机制区分简单和模糊样本；3) 模糊样本交由LLM进行基于结构化规则的深度推理；4) 迭代优化策略通过错误分析和规则更新持续改进系统

Result: 在包含2,764个样本的越南语语音数据集（三个情感类别：平静、愤怒、恐慌）上达到86.59%准确率和0.85-0.86的Macro F1分数，数据集具有高标注一致性（Fleiss Kappa = 0.8574）

Conclusion: 该工作证明了数据驱动模型与人类推理结合的重要性，为低资源环境下的语音情感识别提供了鲁棒且模型无关的方法，能有效处理模糊和难以分类的样本
...
TL;DR: 本文提出了一种针对保加利亚语文本的毒性内容检测方法，通过构建本体论和标注数据集，训练BERT模型实现毒性分类，同时避免误判医疗术语和少数群体相关内容。


<details>
  <summary>Details</summary>
Motivation: 当前在线通信中的毒性内容检测方案经常无意中屏蔽有价值的信息，包括医学术语和与少数群体相关的文本。针对保加利亚语，需要一种更细致的方法来识别毒性内容，同时保留对重要信息的访问。

Method: 1. 提出一个建模保加利亚语中潜在毒性词汇的本体论；2. 构建包含4,384个手动标注句子的数据集，涵盖四个类别：毒性语言、医学术语、非毒性语言、少数群体相关术语；3. 训练基于BERT的毒性语言分类模型。

Result: 训练的BERT模型在毒性语言分类中达到了0.89的宏观F1分数。该模型可直接应用于实际环境，并可作为毒性内容检测系统的组件进行集成。

Conclusion: 本研究开发的方法能够更细致地检测保加利亚语中的毒性内容，同时避免误判医疗术语和少数群体相关内容。该方法具有跨多种在线平台和内容审核系统的应用潜力。
...
TL;DR: LiveMathematicianBench是一个基于arXiv最新论文的动态数学推理基准测试，通过逻辑分类和证明草图引导的干扰项设计，评估LLMs在科研级数学问题上的真实理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准测试存在合成设置和数据污染问题，无法真实评估大语言模型在科研级数学问题上的推理能力。随着LLMs越来越多地集成到科学工作流程中，需要建立严谨、防污染的评估体系。

Method: 1) 使用模型训练截止后发布的arXiv论文构建动态多选题基准；2) 建立13类逻辑分类法（蕴含、等价、存在性、唯一性等）；3) 采用证明草图引导的干扰项生成管道，基于高级证明策略构建似是而非的错误选项；4) 引入抗替换机制区分答案识别与实质推理。

Result: 基准测试远未饱和：最佳模型Gemini-3.1-pro-preview仅达到43.5%。在抗替换评估下，准确率大幅下降：GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问能带来一致的准确率提升。

Conclusion: LiveMathematicianBench提供了一个可扩展、防污染的测试平台，用于研究LLMs在科研级数学推理方面的能力，揭示了当前模型在实质数学理解上的局限性，并证明高级证明策略对模型推理有帮助。
...
TL;DR: 本文提出基于指令微调和离散控制令牌的领域无关可控自动文本简化框架，发现小模型可实现竞争力但可控性依赖训练数据中目标属性的充分变化，现有评估指标不足，需基于误差的度量方法。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化(CATS)通常将可控性视为解码问题，并使用不能准确反映控制程度的指标进行评估。研究发现可控性在ATS中受到数据和评估方法的显著限制，需要更有效的框架和评估体系。

Method: 提出基于指令微调和离散控制令牌的领域无关CATS框架，使用三种模型家族(Llama、Mistral、Qwen；1-14B)和四个领域(医学、公共管理、新闻、百科全书文本)进行实验，分析训练数据变化对可控性的影响。

Result: 较小模型(1-3B)具有竞争力，但可靠的可控性强烈依赖于训练数据是否编码足够的目标属性变化。可读性控制(FKGL、ARI、Dale-Chall)学习一致，而压缩控制表现不佳因现有语料库中信号变异性有限。标准简化和相似性指标不足以衡量控制效果。

Conclusion: 需要基于误差的度量方法来评估目标-输出对齐，朴素的数据分割可能引入分布不匹配从而破坏训练和评估。训练数据的属性变化是CATS系统可靠可控性的关键因素。
...
TL;DR: DEFT框架通过数据筛选和分布引导提升RLHF效率，减少训练时间同时增强对齐能力和泛化能力
...
Method: 提出DEFT框架：1）基于语言模型输出分布和偏好数据差异分布计算差异分布奖励；2）使用该奖励从原始数据中筛选高质量子集；3）将筛选数据整合到现有对齐方法中引导模型输出分布

Result: DEFT增强的方法在不对齐能力和泛化能力上均优于原始方法，且训练时间显著减少

Conclusion: DEFT通过数据筛选和分布引导有效提升了RLHF的效率，在减少训练成本的同时保持了模型的泛化能力
...
TL;DR: PLOT提出了一种基于最优传输的token级损失函数，用于增强LLMs的对齐微调，通过将偏好学习建模为最优传输问题来提升性能、降低计算成本并捕获全局token关系。


<details>
  <summary>Details</summary>
Motivation: 现有LLM偏好学习方法存在性能提升有限、计算成本高、超参数敏感以及未能充分建模全局token级关系等局限性，需要更有效且理论扎实的解决方案。

Method: PLOT通过最优传输问题形式化偏好学习，引入token级损失函数，利用token嵌入捕获语义关系，实现全局感知的优化，同时保持LLM原始分布的稳定性。

Result: 在人类价值观和逻辑与问题解决两大类偏好、涵盖七个子偏好的实验中，PLOT一致提升了对齐性能，同时保持了流畅性和连贯性。

Conclusion: 最优传输为偏好学习提供了理论扎实的方法论框架，PLOT证明了其作为LLM偏好学习原则性方法的有效性，为相关研究提供了新见解。
...
TL;DR: 提出自适应占位符补全(APC)框架，通过在不确定性高的位置输出显式占位符，让用户直接填充，相比传统硬补全(Hard Completion)可降低19%-50%的编辑成本。


<details>
  <summary>Details</summary>
Motivation: 传统LLM代码补全采用硬补全范式，即使在上下文不足时也强制生成完整代码。分析300万真实交互发现，61%的生成建议在被接受后仍需编辑或被拒绝（尽管与用户后续代码相似度超过80%），表明模型常在特定token位置做出错误预测。

Method: 提出自适应占位符补全(APC)协作框架，在不确定性高的位置输出显式占位符，用户可通过IDE导航直接填充。将代码补全形式化为不确定性下的成本最小化问题，基于填充占位符比纠正错误成本更低的观察，证明存在临界熵阈值，超过该阈值时APC的期望成本严格低于硬补全。通过过滤真实编辑日志构建训练数据，设计基于成本的奖励函数进行强化学习。

Result: 在1.5B-14B参数模型上的广泛评估表明，APC将期望编辑成本降低了19%到50%，同时保持了标准硬补全的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态对话情感识别方法过于强调融合而忽视噪声特征的不确定性和细粒度推理，需要提高对噪声的鲁棒性并增强上下文建模能力
...
TL;DR: 比较不同方法在心力衰竭短期死亡率预测中的表现，发现基于实体的多模态transformer模型效果最佳，而大语言模型表现不稳定


<details>
  <summary>Details</summary>
Motivation: 心力衰竭的短期死亡率预测具有挑战性，特别是仅依赖结构化电子健康记录数据时。需要评估不同方法在预测准确性方面的表现，以找到最可靠的临床决策支持方案。

Method: 在法国心力衰竭队列中评估多种方法：1）仅文本模型；2）仅结构化数据模型；3）多模态融合模型；4）大语言模型方法。比较了基于CLS嵌入与实体级表示的文本处理方法，以及不同模态融合策略。

Result: 1）在文本处理中，使用实体级表示比仅用CLS嵌入能获得更好的预测效果；2）监督式多模态融合（结合文本和结构化变量）达到最佳整体性能；3）大语言模型在不同模态和解码策略中表现不一致，仅文本提示优于结构化或多模态输入。
...
Method: 提出神经符号方法，结合非上下文文本嵌入（fastText）与符号概念特征，包括体裁、主题和说服技巧，以增强分类鲁棒性。
...
TL;DR: 提出了一种基于置换多面体交换距离最小化的数学框架，用于衡量词序/手势顺序的最优性，发现跨语言手势至少77%最优，并引入二次分配问题作为统一语言优化问题的理论框架。


<details>
  <summary>Details</summary>
Motivation: 研究语言中词序/手势顺序是否遵循交换距离最小化原则，即源顺序与目标顺序之间的交换操作距离越小，认知成本越低，越可能被采用。

Method: 使用置换多面体（permutohedron）表示所有排列的结构，其中顶点是排列，相邻顶点通过交换相邻元素相连。开发数学框架测量词序变化相对于交换距离最小化的最优程度，并引入二次分配问题（QAP）作为统一优化问题的理论框架。

Result: 跨语言手势顺序至少达到77%的最优性，多次达到最优的情况不太可能是偶然的，表明交换距离最小化在沟通系统中确实起作用。

Conclusion: 建立了研究词序/手势顺序相对于交换距离最小化最优性的理论基础，提出二次分配问题作为语言研究中多种优化问题的统一框架，并假设了统一各种语言原则（包括交换距离最小化）的最优分配一般原则。
...
TL;DR: 论文提出了一种三步骤的自动质量保证方法，用于评估机器翻译基准数据集的质量，并发布了清理后的EU20数据集版本


<details>
  <summary>Details</summary>
Motivation: 机器翻译的基准数据集虽然降低了成本并提供了规模，但噪声、结构丢失和质量不均等问题削弱了对其的信任。关键不仅在于能否翻译，更在于能否大规模测量和验证翻译的可靠性

Method: 采用三步自动质量保证方法：(1) 结构语料库审计与针对性修复；(2) 使用神经度量COMET（无参考和基于参考）进行质量分析，并与翻译服务（DeepL/ChatGPT/Google）比较；(3) 基于LLM的跨度级翻译错误分析

Result: 趋势一致：COMET分数较低的数据集在跨度级别表现出更高的准确性/误译错误比例（特别是HellaSwag；ARC相对干净）。基于参考的COMET在MMLU上对人工编辑样本的评估也指向相同方向。发布了清理/修正后的EU20数据集和可复现代码

Conclusion: 自动质量保证提供了实用、可扩展的指标，有助于优先审查工作——补充而非替代人工黄金标准
...
TL;DR: SAFE是一个动态基准测试框架，通过可验证的实体序列替代未接地的思维链，解决多跳QA基准测试中LLMs虚假正确性的问题。


<details>
  <summary>Details</summary>
Motivation: 多跳QA基准测试经常奖励大型语言模型的虚假正确性，掩盖了未接地或有缺陷的推理步骤。需要转向更严格的推理评估。

Method: SAFE框架包含两个阶段：1) 训练时验证：建立原子错误分类和基于知识图谱的验证管道，消除标准基准中的噪声监督；2) 推理时验证：在验证数据集上训练的反馈模型实时检测未接地的推理步骤。

Result: 实验结果表明：1) SAFE在训练时暴露了现有基准的关键缺陷，识别出高达14%的实例为不可回答；2) 在推理时显著优于标准基线，平均准确率提升8.4个百分点，同时保证可验证的推理轨迹。

Conclusion: SAFE框架通过可验证的实体序列和动态验证机制，为多跳QA提供了更严格的推理评估方法，有效解决了虚假正确性问题。
...
TL;DR: 该论文研究了高斯扩散模型在处理离散数据时遇到的采样困难，发现DDPM求解器在临界采样区间内会进入低密度区域导致样本质量下降，提出了结合自条件和q采样的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在连续域生成建模中已成为标准方法，但在处理离散数据时仍面临挑战。作者旨在探究高斯扩散模型为何难以从连续空间中表示为δ分布混合的离散分布中采样，特别是DDPM求解器在采样过程中的问题。

Method: 使用玩具随机层次模型分析离散分布采样问题，识别出临界采样区间。提出结合自条件和q采样的改进方法：在临界区间内从DDPM切换到q采样，并结合自条件技术。在文本、编程代码和蛋白质等多个领域的条件和非条件任务上进行验证。

Result: 研究发现DDPM在临界采样区间内会进入低密度区域，产生超出分布的输入，从而降低样本质量。现有启发式方法（自条件和q采样）能缓解此问题。结合自条件并在临界区间内从DDPM切换到q采样的方法，在真实数据上显著提高了生成质量。

Conclusion: 高斯扩散模型处理离散数据时，DDPM求解器在临界采样区间内存在进入低密度区域的问题。通过结合自条件技术和在关键区间切换到q采样，可以有效改善离散数据生成质量，该方法在多个领域验证有效。
...
TL;DR: 研究分析了自监督语音模型训练过程中语言结构表征的涌现规律，发现不同层次的语言结构在层间分布和学习轨迹上存在显著差异，受预训练目标定义层次的影响


<details>
  <summary>Details</summary>
Motivation: 探究自监督语音模型在训练过程中何时以及如何学习到语言结构表征，理解不同语言结构在模型中的编码规律

Method: 分析六个Wav2Vec2和HuBERT模型在荷兰语语音数据上的中间检查点，研究多种语言结构在不同层的编码模式和学习轨迹

Result: 发现不同层次的语言结构在层间分布和学习轨迹上存在显著差异，这些差异部分可由其与声学信号的抽象程度以及输入信息整合的时间尺度解释；预训练目标的定义层次强烈影响语言结构的层间组织模式和学习轨迹

Conclusion: 自监督语音模型中语言结构的编码模式和学习轨迹具有结构特异性，预训练目标的定义层次是影响这些模式的关键因素，高阶预测任务能诱导更大的并行性
...
TL;DR: 提出BidirLM方法，将因果生成语言模型转化为双向编码器，通过系统消融实验确定关键因素，引入权重合并和多领域数据混合策略，并融合专业模型能力，在文本、视觉和音频表示基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转化为双向编码器的方法存在三个主要问题：缺乏训练目标共识、大规模时出现灾难性遗忘、无法灵活整合专业生成模型生态系统。需要开发一种通用、可扩展且能融合专业能力的方法。

Method: 1. 在Gemma3和Qwen3模型上进行系统消融实验，识别成功适应的关键因素，特别是常被忽略的先验掩码阶段的重要性。2. 引入双重策略：线性权重合并和轻量级多领域数据混合，以缓解灾难性遗忘。3. 通过合并专业因果模型来增强编码器，实现模态和领域特定能力的无缝转移。

Result: 开发出BidirLM系列编码器（共5个），在文本、视觉和音频表示基准测试中优于现有替代方法。该方法开源且适用于任何因果解码器LLM。

Conclusion: 提出了一种通用、可扩展的将因果生成语言模型转化为双向编码器的配方，解决了现有方法的局限性，实现了与专业模型的灵活集成，并在多模态表示任务上取得了优越性能。
...
TL;DR: GOOSE是一种无需训练的推测解码框架，通过构建自适应脊柱树结构，利用高质量上下文匹配令牌形成深度链，低质量预测令牌形成宽分支，显著提升LLM推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在构建候选令牌树时未区分不同来源令牌的质量差异。研究发现上下文匹配令牌和统计预测令牌的接受率存在显著差异（中位数差距约6倍），这为优化树结构提供了机会。

Method: GOOSE构建自适应脊柱树：将高接受率的上下文匹配令牌组织成深度链（脊柱），在每个节点上为低接受率的预测令牌创建宽分支。这种各向异性结构突破了平衡树的深度限制，同时保持验证预算不变。

Result: 在5个LLM（7B-33B）和5个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下比平衡树基线性能提升12-33%。

Conclusion: 通过区分不同来源令牌的质量差异并构建各向异性树结构，GOOSE显著提升了推测解码的效率，证明了自适应树结构在优化LLM推理中的重要性。
...
TL;DR: RRPO是一种通过强化学习直接对齐重排序与LLM生成质量的框架，使用LLM反馈优化上下文效用，无需人工标注，显著优于现有重排序方法


<details>
  <summary>Details</summary>
Motivation: 当前重排序模型通常基于静态人工标注的相关性标签进行优化，与下游生成过程解耦，导致信息检索指标认为相关的文档往往无法为LLM提供生成精确答案所需的实际效用
...
Conclusion: RRPO通过直接对齐重排序与LLM生成质量，解决了传统重排序模型与下游生成过程的错配问题，为检索增强生成提供了更有效的重排序解决方案
...
TL;DR: 提出prosodic ABX框架，扩展ABX判别任务以评估自监督语音模型对韵律对比的敏感性，无需显式标签


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型对音位对比敏感，但其对韵律对比的敏感性尚未直接测量，需要开发无需大量数据和标签的评估方法

Method: 扩展ABX判别任务框架为prosodic ABX，构建并发布英语和日语最小对数据集，结合现有普通话数据集，评估英语重音、日语声调重音和普通话声调的对比敏感性
...
TL;DR: 提出稳定性过滤方法，通过筛选行为稳定的边界来构建更有效的引导向量，显著提升数学推理性能


<details>
  <summary>Details</summary>
Motivation: 当前基于关键词匹配检测推理行为边界的方法存在严重缺陷，大多数检测到的边界行为不稳定，无法在相同前缀下重现，导致引导信号被稀释

Method: 1) 建立概率模型将内在推理行为形式化为具有上下文依赖触发概率的随机事件；2) 提出稳定性过滤，仅保留目标行为能一致重现的边界；3) 结合内容子空间投影去除残差问题特定噪声

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0%），构建的引导向量可在同架构模型家族中迁移，无需重新提取，提升Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）性能

Conclusion: 稳定性过滤能有效识别真正的行为信号，构建的引导向量质量更高，可跨模型迁移，为训练自由控制推理行为提供了更可靠的方法
...
TL;DR: GaelEval是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化知识问答三个任务。评估19个LLM发现，Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超过人类基线(78.1%)，专有模型优于开源模型，盖尔语提示带来小幅稳定优势。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在未官方支持的语言中表现出"影子"能力，但其性能参差不齐且缺乏有效衡量。对于苏格兰盖尔语这类形态句法丰富的少数民族语言，现有翻译基准无法捕捉结构能力，需要更全面的评估框架。

Method: 提出GaelEval基准，包含三个部分：1)专家编写的形态句法多项选择题任务；2)文化背景翻译基准；3)大规模文化知识问答任务。评估19个LLM，并与30名流利说话者的人类基线对比，分析专有vs开源模型、盖尔语提示vs英语提示的效果。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超过人类基线(78.1%)。专有模型一致优于开源模型。盖尔语提示带来+2.4%的小幅稳定优势。在文化任务上，领先模型超过90%准确率，但大多数系统在盖尔语提示下表现更差，且绝对分数相对于手动基准有所膨胀。

Conclusion: 前沿模型在盖尔语语法的多个维度上达到超人类表现，盖尔语提示有正面效果，专有模型与开源模型存在性能差距。GaelEval为少数民族语言评估提供了首个全面基准，揭示了LLM在未支持语言中的能力分布。
...
TL;DR: 该论文介绍了在SOMD 2026跨文档软件提及共指消解共享任务中的参与情况，比较了两种无需微调的方法：模糊匹配和上下文感知表示，分析了它们在噪声条件下的表现差异和计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 软件提及共指消解是一个研究不足的任务，需要探索高效且鲁棒的解决方案。论文旨在比较两种无需微调的方法在跨文档软件提及共指消解任务中的表现，特别关注它们对噪声的鲁棒性和计算效率。

Method: 提出了两种无需微调的方法：1) 模糊匹配：基于词汇字符串相似性的方法；2) 上下文感知表示：结合提及级别和文档级别嵌入的方法。通过噪声注入实验（边界噪声和提及替换）和推理时间分析来评估两种方法的性能差异。

Result: 在SOMD 2026共享任务中排名第二，两种方法在所有三个子任务中都取得了竞争性表现（CoNLL F1为0.94-0.96）。上下文感知表示在官方测试集上始终比模糊匹配高1分。噪声研究表明：边界噪声增加时，上下文感知表示仅损失0.07 F1，而模糊匹配损失0.20；提及替换时，模糊匹配下降更平缓（0.52 vs 0.63）。推理时间分析显示模糊匹配随语料规模呈超线性扩展，而上下文感知表示近似线性扩展。

Conclusion: 系统选择应同时考虑上游提及检测器的噪声特征和目标语料的规模。对于大规模语料，上下文感知表示是更高效的选择。软件名称的高表面规律性降低了对复杂语义推理的需求。作者发布了代码以支持这一研究不足任务的未来工作。
...
Method: 提出Neuro-RIT框架，从密集适应转向精确驱动的神经元对齐。首先使用基于归因的神经元挖掘显式分离处理相关与不相关上下文的神经元，然后引入两阶段指令调优策略：通过功能失活处理不相关上下文的专属神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏

Result: 在多样化QA基准测试上的广泛实验表明，Neuro-RIT始终优于强基线方法和鲁棒性增强方法

Conclusion: 通过神经元级精确调整而非粗粒度参数更新，Neuro-RIT显著提升了检索增强语言模型对噪声上下文的鲁棒性，为鲁棒性增强提供了新的范式
...
TL;DR: L3TR是一个基于大语言模型的列表式人才推荐框架，通过块注意力机制和局部位置编码解决位置偏见和并发令牌偏见问题，相比点式方法显著提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的人才推荐系统多采用点式范式，需要重复处理文本且无法捕捉候选人之间的关联关系，导致高令牌消耗和次优推荐。同时LLM存在位置偏见和"迷失在中间"问题。

Method: 提出L3TR框架：1)采用隐式策略利用LLM的潜在输出；2)引入块注意力机制和局部位置编码增强文档间处理并缓解位置偏见；3)提出ID采样方法解决训练和推理阶段候选集大小不一致问题；4)设计位置偏见和令牌偏见的检测方法及无训练去偏方法。

Result: 在两个真实世界数据集上的广泛实验验证了L3TR的有效性，相比现有基线方法取得了持续改进。

Conclusion: L3TR通过列表式处理范式解决了传统点式方法的局限性，有效缓解了LLM在人才推荐中的位置偏见和并发令牌偏见问题，为基于LLM的推荐系统提供了新的解决方案。
...
Method: 1) 创建CV-18 NER数据集：基于Arabic Common Voice 18语料库，按照Wojood模式（21种实体类型）进行人工NER标注；2) 对比级联系统（ASR+文本NER）和端到端模型（基于Whisper和AraBEST-RQ）；3) 分析阿拉伯语特定自监督预训练和多语言弱监督对任务的影响。

Result: 端到端系统显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。阿拉伯语特定自监督预训练在ASR上表现良好，而多语言弱监督在联合语音到实体学习中更有效，大模型在低资源环境下更难适应。

Conclusion: 端到端方法在阿拉伯语语音NER任务上具有优势，CV-18 NER数据集为阿拉伯语语音NER研究提供了首个公开基准，相关数据和模型已开源发布。
...
TL;DR: 本文研究如何从大语言模型中生成全面的有效回答集，提出多样性覆盖率指标评估模型表现，发现不同模型在不同提示上表现各异，进而开发了基于查询的路由器来选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 当面对允许多个有效答案的提示时，全面生成这些答案是满足多样化用户需求的第一步。现有研究缺乏评估模型生成全面有效回答集能力的方法，且不同模型在不同类型提示上的表现存在差异。

Method: 1. 提出多样性覆盖率指标，衡量预测答案集中每个独特答案的质量得分相对于相同数量最佳可能答案集的比例；2. 评估18个大语言模型在生成多样化回答方面的表现；3. 开发基于查询的路由器，预测每个查询的最佳模型；4. 在NB-Wildchat数据集上训练和评估路由器，并测试其在NB-Curated数据集和不同答案生成提示策略上的泛化能力。

Result: 1. 评估发现没有单一模型在所有开放提示上都能主导生成多样化回答；2. 每个提示都存在显著优于其他模型的特定模型；3. 训练的路由器在NB-Wildchat上表现优于单一最佳模型基线（26.3% vs 23.8%）；4. 路由器在跨域数据集NB-Curated和不同答案生成提示策略上展现出良好的泛化能力。

Conclusion: 本文为研究在拥有多个模型时生成全面答案奠定了基础，提出的多样性覆盖率指标和基于查询的路由器方法能够有效提升生成全面回答集的能力，为构建更全面的问答系统提供了新思路。
...
TL;DR: 论文分析了语言模型扩展新词汇时的均值初始化问题，提出基于语言基础的初始化方法GTI，在多个生成推荐基准测试中表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型扩展新词汇时普遍采用均值初始化策略，但作者通过谱分析和几何诊断发现这种方法会导致新词嵌入坍缩到退化子空间，消除词间差异，使得后续微调难以完全恢复，表明词初始化是词汇扩展的关键瓶颈。

Method: 提出"基于语言基础的词初始化假设"，并实现为GTI方法：在微调前，仅使用配对的语言监督将新词映射到预训练嵌入空间中具有语义意义的独特位置。该方法包含轻量级的基础化阶段，确保新词嵌入具有丰富的词间结构。

Result: 在多个生成推荐基准测试（包括工业规模和公共数据集）中，GTI在大多数评估设置中优于均值初始化和现有的辅助任务适应方法。分析表明，基于基础的嵌入在微调过程中保持了更丰富的词间结构。

Conclusion: 词初始化质量是词汇扩展的关键瓶颈，基于语言基础的初始化方法能够更好地利用模型的通用知识处理新词领域，为语言模型扩展提供了更有效的策略。
...
Method: 数字孪生反事实框架：使用数字孪生模拟器在潜在结果框架内生成反事实结果，建立孪生保真度假设层次结构（边际保真度→联合保真度→结构保真度），提出五层验证架构将不可证伪的模拟器转化为可检验的假设

Result: 1）将因果量分解为可边际验证的（ATE、CATE、QTE）和依赖copula的（ITE分布、获益/损害概率、处理效应方差）；2）提供边界、敏感性和不确定性量化工具使copula依赖显式化；3）建立了因果主张的可检验性层次
...
TL;DR: IDEA2是一个结合大型语言模型和专家在环的半自动化工作流，用于从需求文档中提取能力问题，通过迭代反馈循环提高能力问题的接受度和相关性。


<details>
  <summary>Details</summary>
Motivation: 能力问题提取是本体工程中的关键但资源密集型瓶颈，存在领域专家和本体工程师之间的沟通鸿沟，需要更高效、协作的方法来改进这一过程。

Method: IDEA2采用半自动化工作流，核心是迭代循环：1) LLM从需求文档中初步提取能力问题；2) 领域专家在协作平台上进行共同创建式评审和反馈；3) LLM根据反馈迭代重新表述被拒绝的能力问题直至达成共识。整个过程使用溯源模型跟踪能力问题的完整生命周期。

Result: 在科学数据和文化遗产两个真实场景中验证，IDEA2能够加速需求工程过程，提高能力问题的接受度和相关性，在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2通过整合LLM和专家协作，有效解决了能力问题提取的瓶颈问题，提供了透明、可复现的工作流，显著提高了本体工程需求阶段的效率和质量。
...
Method: 1）引入世界中心多智能体系统（WMAS）概念；2）从本体论显式性、规范性等维度对世界进行分类；3）提出语义模型作为表示此类世界的数学形式化方法；4）开发Ontobox平台作为WMAS的实现。

Result: 提出了WMAS理论框架，建立了世界分类维度，开发了语义模型形式化方法，并实现了Ontobox平台作为WMAS的具体实现，为结构化领域多智能体系统提供了新的架构范式。
...
TL;DR: 提出基于角色的LLM框架，通过分配专业角色（政策分析师、法律策略专家、食品系统专家）来自动化健康食品政策的信息抽取，解决现有方法中的幻觉、误分类和遗漏问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在健康食品政策领域的信息抽取面临多种挑战：政策文档结构多样且不一致导致幻觉、误分类和遗漏。需要更可靠、透明的方法来自动化政策信息抽取。

Method: 提出基于角色的LLM框架，分配三个专业角色：LLM政策分析师（元数据和机制分类）、LLM法律策略专家（识别复杂法律方法）、LLM食品系统专家（分类食品系统阶段）。框架通过将结构化领域知识（法律机制定义、分类标准）融入角色特定提示来模拟专家分析工作流。

Result: 使用Healthy Food Policy Project数据库中的608个健康食品政策进行评估，与零样本、少样本和思维链基线（基于Llama-3.3-70B）比较。所提框架在复杂推理任务中表现优异，为健康政策信息抽取提供了可靠透明的自动化方法。

Conclusion: 基于角色的LLM框架通过模拟专家工作流和融入结构化领域知识，有效解决了健康食品政策信息抽取中的幻觉和误分类问题，为政策分析提供了可靠透明的自动化方法。
...
TL;DR: 该研究提出AI自动化是一个连续体，包括"崩溃浪潮"（AI能力在少数任务上突然激增）和"上升潮汐"（AI能力更连续、广泛地提升）。基于对3000多个文本任务的评估，研究发现上升潮汐是AI自动化的主要形式，AI在广泛任务上表现良好且快速改进。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AI自动化的发展模式，特别是检验两种假设：崩溃浪潮（AI能力在特定任务上突然突破）与上升潮汐（AI能力在广泛任务上持续渐进提升）。这有助于预测AI对经济和劳动力市场的影响。

Method: 研究基于美国劳工部O*NET分类，构建了3000多个文本型任务的数据集。通过超过17000次由相关职业工作者进行的评估，测试AI模型在这些任务上的表现。研究追踪了AI能力随时间的变化趋势。

Result: 研究发现几乎没有崩溃浪潮的证据，但存在显著的上升潮汐效应。AI在广泛任务上表现良好且快速改进：2024年第二季度，AI模型能以约50%成功率完成人类需要3-4小时的任务；到2025年第三季度，成功率预计提升至约65%。如果当前趋势持续，到2029年LLM在大多数文本任务上的平均成功率将达到80%-95%。

Conclusion: AI自动化主要表现为上升潮汐模式，即能力在广泛任务上持续渐进提升，而非在少数任务上突然突破。这种能力提升将对经济和劳动力市场产生重要影响，但组织采用AI可能需要更长时间。研究结果有助于更准确地预测AI发展轨迹及其社会经济影响。
...
TL;DR: 该研究探讨了LLMs中的认知偏差，建立了LLM CogBias基准测试四种偏差类型，发现偏差在激活空间中呈线性可分，通过激活导向技术可显著减少偏差而不损害模型能力。
...
Method: 定义了LLM认知偏差为系统性、可复现地偏离可计算基准正确答案的偏差，建立了LLM CogBias基准测试，围绕四种认知偏差家族（判断、信息处理、社会和反应）进行评估。使用线性探针和对比设计分析偏差在激活空间中的表征，并应用激活导向技术来调节偏差行为。

Result: 研究发现认知偏差在所有四种家族中系统性出现，其程度和去偏差响应强烈依赖于偏差家族：提示级去偏差显著减少反应偏差，但对判断偏差产生反效果。偏差在模型激活空间中编码为线性可分的方向。激活导向技术实现了26-32%的偏差分数减少，同时在25个基准测试上保持下游能力（Llama：可忽略的退化；Qwen：判断偏差最多下降19.0个百分点）。尽管不同模型间的偏差表征接近正交，但导向技术在不同架构中减少偏差的效果相似。

Conclusion: LLMs中的认知偏差对应着可识别的内部表征，可以通过激活导向技术有效缓解。不同模型间偏差表征虽然接近正交，但减少偏差的功能组织可能是共享的，这为理解和管理LLMs中的认知偏差提供了理论基础和技术途径。
...
### [158] [The Self Driving Portfolio: Agentic Architecture for Institutional Asset Management](https://arxiv.org/abs/2604.02279)
*Andrew Ang,Nazym Azimbayev,Andrey Kim*

Main category: cs.AI

TL;DR: 提出了一个由约50个专业智能体组成的资产配置管道，将投资者角色从分析执行转变为监督，通过多智能体协作、批评投票、方法创新和元学习优化实现自主投资决策


<details>
  <summary>Details</summary>
Motivation: 将投资者角色从传统的分析执行转变为监督，实现AI驱动的自主战略资产配置，通过多智能体系统模拟和超越人类投资团队的专业协作

Method: 构建包含约50个专业智能体的管道：1）生成资本市场假设；2）使用20多种竞争方法构建投资组合；3）智能体相互批评和投票；4）研究智能体提出新方法；5）元智能体比较历史预测与实际回报，重写代码和提示以改进性能；6）整个管道由投资政策声明约束

Result: 建立了完整的自主战略资产配置系统，能够通过多智能体协作、竞争性方法评估、创新方法生成和持续性能优化，在投资政策声明的约束下实现专业级的投资决策

Conclusion: 智能体AI能够将投资者角色转变为监督者，通过多智能体系统实现自主战略资产配置，投资政策声明可作为约束和指导自主智能体的有效框架，系统具备持续学习和改进能力
...
### [159] [RIFT: A RubrIc Failure Mode Taxonomy and Automated Diagnostics](https://arxiv.org/abs/2604.01375)
*Zhengyang Qi,Charles Dickens,Derek Pham,Amanda Dsouza,Armin Parchami,Frederic Sala,Paroma Varma*

Main category: cs.AI

TL;DR: 提出RIFT分类法系统诊断评分标准质量，包含8种失效模式，分为3大类，通过人工标注验证一致性，并开发自动化质量指标


<details>
  <summary>Details</summary>
Motivation: 当前基于评分标准的评估在LLM基准测试和训练中被广泛使用，但缺乏从聚合或下游信号中诊断评分标准质量问题的系统方法，需要填补这一空白

Method: 使用扎根理论开发RIFT分类法，从五个不同基准中迭代标注评分标准直至饱和；评估分类法一致性通过人工标注者间一致性测量；提出自动化评分标准质量指标并与人工标注对齐

Result: 开发了包含8种失效模式的RIFT分类法，分为可靠性失效、内容有效性失效和后果有效性失效三类；人工标注者间一致性达87%配对一致性和0.64平均Cohen's kappa；自动化指标与人工失效模式标注对齐，F1分数最高达0.86
...
### [160] [Leveraging the Value of Information in POMDP Planning](https://arxiv.org/abs/2604.01434)
...
<details>
  <summary>Details</summary>
Motivation: POMDP在状态和转移不确定性下的规划具有理论优势，但维度灾难和历史诅咒使得在有限规划时间内获得高性能策略仍具挑战性。许多POMDP问题中，信息价值(VOI)在信念空间中变化，这为优化计算分配提供了机会。
...
Conclusion: 通过利用信息价值在信念空间中的变化特性，提出的VOIMCP框架能够更高效地分配计算资源，在有限规划时间内获得更好的策略性能，为大规模POMDP规划提供了有效解决方案。
...
### [161] [When AI Gets it Wong: Reliability and Risk in AI-Assisted Medication Decision Systems](https://arxiv.org/abs/2604.01449)
...
<details>
  <summary>Details</summary>
Motivation: AI系统在医疗和药学领域应用日益广泛，但在高风险用药管理场景中，即使单个错误推荐也可能导致严重患者伤害。传统评估指标无法充分反映AI系统在真实决策中的可靠性，需要关注错误发生机制及其临床影响。

Method: 通过一系列受控模拟场景（涉及药物相互作用和剂量决策），分析不同类型的系统失败，包括遗漏相互作用、错误风险标记和不适当剂量推荐。研究关注错误发生方式及其后果，而非仅依赖聚合性能指标。

Result: 研究发现AI在用药相关场景中的错误可能导致不良反应、无效治疗或延误护理，特别是在缺乏足够人工监督的情况下。系统失败包括遗漏关键相互作用、错误标记风险以及提供不适当剂量建议。

Conclusion: 该研究为医疗AI评估提供了可靠性导向的视角，强调理解失败行为和真实世界影响的重要性。在药学实践等安全关键领域，需要将传统性能指标与风险感知评估方法相结合，并关注AI决策透明度不足和过度依赖AI推荐的风险。
...
### [162] [A Multi-Agent Human-LLM Collaborative Framework for Closed-Loop Scientific Literature Summarization](https://arxiv.org/abs/2604.01452)
...
Method: 开发多智能体、人在回路系统，整合LLM、结构化AI和人类科学家，通过专门智能体进行论文筛选、数据提取、模型拟合和结果总结，人类监督确保可靠性，生成结构化报告并支持迭代精炼。

Result: 系统应用于材料科学，分析了钨在氦离子辐照下的文献，发现氦气泡生长与辐照剂量和温度呈指数相关，为聚变反应堆等离子体面向材料提供见解。

Conclusion: AI辅助的文献综述能够揭示科学模式并加速发现，Elhuyar系统展示了整合AI和人类专业知识进行深度文献分析的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法主要关注可行实例的解决方案生成，缺乏对不可行性检测的显式处理，需要一种能够同时处理可行性和不可行性检测的统一框架。

Method: 提出不可行性感知框架：1）引入新的数学规划公式和可证明的零阶段不可行性筛选，构建可验证的训练数据集；2）使用精确优化流程生成的数据对8B参数LLM进行监督微调；3）利用LLM输出作为下游局部搜索的初始解。

Result: 微调模型相比GPT-5.2整体准确率提升达30%；LLM引导的初始解在下游局部搜索中相比从零开始提供高达2倍的加速。

Conclusion: 该框架成功将LLM应用于组合优化问题的不可行性检测和解决方案生成，通过可验证数据构建和微调实现了性能提升，并为不完美的LLM输出提供了实用的下游优化加速方法。
...
TL;DR: P-COD是一种通过文档间关系检测异常值的新方法，利用语料库中已验证的同行信息来调整置信度分数，提高大型语言模型从科学文献中提取数据的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前方法（如提示工程和思维链提示）专注于单个文档，但未能考虑语料库中文档间的关系，导致大型语言模型在从大型文本语料库中提取数据时产生幻觉。需要一种能够利用文档间关系来提高提取准确性的方法。

Method: 提出Peer Context Outlier Detection (P-COD)方法，在科学文献摘要化应用领域中，通过比较提取的数据与语料库中已验证的同行信息来调整置信度分数。相似实验设置的论文应得出相似结论，利用这一关系来标记低置信度结果供专家审查，高置信度且得到同行验证的结果被视为可靠。

Result: 在6个科学领域的实验中，异常值检测精度高达98%。该方法减少了幻觉，增强了对自动化系统的信任，使研究人员能够专注于模糊案例，从而简化数据提取工作流程。

Conclusion: P-COD通过利用文档间关系显著提高了大型语言模型从科学文献中提取数据的准确性，减少了幻觉，为研究人员提供了更可靠的数据提取工具。
...
<details>
  <summary>Details</summary>
Motivation: 元表面逆向设计需要大量计算电磁学专业知识和求解器特定软件工程技能，现有基于大语言模型的系统缺乏跨任务可重用工作流知识的积累，限制了逆向设计工作流的自主性和可访问性。

Method: 开发了一个智能体框架，包含编码智能体、演化技能工件和基于物理仿真的确定性评估器，通过迭代精炼求解器特定策略而不修改模型权重或底层物理求解器，实现跨任务的技能演化。

Result: 在包含多个元表面逆向设计任务类型的基准测试中，演化技能将分布内任务成功率从38%提升至74%，标准通过率从0.510提升至0.870，平均尝试次数从4.10降至2.30；在分布外任务上，二进制成功率变化不大，但最佳边际改进、错误组成变化和智能体行为表明工作流知识的部分迁移。

Conclusion: 技能演化的主要价值在于围绕可靠计算引擎积累可重用的求解器特定专业知识，为实现更自主和可访问的元表面逆向设计工作流提供了一条实用路径。
...
TL;DR: 本文研究了将高维表示自编码器（RAE）整合到连续自回归（AR）模型中的挑战，提出了RAE-AR框架，通过分布归一化和高斯噪声注入解决建模复杂性和曝光偏差问题，使表示自编码器在AR模型中达到与VAE相当的性能。
...
Method: 提出RAE-AR框架，针对AR模型的两个主要挑战：1）复杂的分词级分布建模，通过分布归一化进行分词简化以降低建模难度；2）高维度放大的训练-推理差距（曝光偏差），通过在训练中加入高斯噪声注入来增强预测鲁棒性。
...
TL;DR: 研究发现LLM训练中归一化层与优化器存在交互效应：Dynamic Erf与Muon优化器组合时性能显著下降，而Dynamic Tanh无此问题，揭示了erf在Muon快速谱范数增长下的饱和与尺度盲区失效模式。


<details>
  <summary>Details</summary>
Motivation: 传统上LLM训练中将归一化层和优化器视为独立设计选择，但本研究通过实验发现这种假设可能不成立，特别是Dynamic Erf与Muon优化器组合时存在显著的负交互效应，这在实际训练中容易被忽视。

Method: 采用3x2因子设计实验，在10亿参数规模、1000训练步数下，对比Dynamic Erf、Dynamic Tanh和RMSNorm三种归一化层与AdamW、Muon两种优化器的组合性能。通过分析erf在Muon快速谱范数增长下的失效模式，提出EMA混合方法（重新引入运行尺度估计）和降低Derf的alpha参数两种解决方案。

Result: Dynamic Erf与Muon组合时性能显著下降：与RMSNorm的差距从AdamW下的+0.31 nats扩大到Muon下的+0.97 nats（约3倍）。Dynamic Tanh作为有界归一化对照组未显示类似惩罚。EMA混合方法恢复了约84%的性能差距，降低Derf的alpha从0.5到0.3恢复了约80%差距。使用Derf默认alpha与Muon组合会产生0.66-nat的交互惩罚，且不产生NaN或发散，在短期试验中容易被忽视。

Conclusion: 归一化层与优化器选择存在显著交互效应，不应视为独立设计。Dynamic Erf与Muon组合的失效源于erf在快速谱范数增长下的饱和和尺度盲区问题。通过调整参数或引入尺度估计可缓解此问题，但需要系统评估不同组合的兼容性，避免在短期试验中遗漏重要交互效应。
...
TL;DR: NED-Tree框架通过句子级提取和递归树结构解决LLM在非线性运筹问题建模中的语义对齐问题，在10个基准测试中达到72.51%平均准确率。
...
Method: NED-Tree采用(a)句子级提取策略确保参数映射的鲁棒性和可追溯性；(b)递归树结构自适应地将复杂非线性项分解为求解器兼容的子元素。

Result: 在10个基准测试中，NED-Tree实现了72.51%的平均准确率，建立了新的最先进水平，是首个通过元素分解解决非线性建模困难的框架。

Conclusion: NED-Tree成功弥合了运筹问题建模语义与代码语义之间的差距，并引入了专门针对复杂非线性、多约束运筹问题的NEXTOR基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习与验证推理（RLVR）方法通常需要正确性信号或批评注释来指导模型精炼答案，这限制了方法的可扩展性。ThinkTwice旨在开发一种无需额外监督信号的联合训练方法，让模型同时学习推理和自精炼能力。

Method: ThinkTwice采用两阶段训练框架：第一阶段优化模型解决推理问题，第二阶段优化模型精炼自己的解决方案。两个阶段都使用相同的二元正确性奖励，无需正确性信号或批评注释。基于Group Relative Policy Optimization（GRPO）实现，在每对训练步骤中交替进行推理和精炼优化。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性的在线策略优化基线。在Qwen3-4B上，ThinkTwice在AIME基准上比GRPO在精炼前提升5个百分点，在一次自精炼步骤后提升11.5个百分点（pass@4指标）。训练动态分析显示存在隐式的"纠正-强化"课程：早期精炼主要纠正错误，随着模型改进自然转向保持已正确的解决方案。

Conclusion: ThinkTwice证明了推理和自精炼的联合训练是RLVR的一种原则性且有效的方法论。该方法无需额外监督信号，通过两阶段优化实现了性能的显著提升，并揭示了训练过程中的隐式课程学习机制。
...
TL;DR: LLMs在教学中主要使用贝叶斯最优推理而非简单启发式，能像人类一样推断学习者知识状态，但提示干预不一定改善教学效果


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在教学决策时是使用复杂的心理推理（推断学习者知识状态）还是简单的启发式规则，通过受控任务比较LLMs与人类的教学策略

Method: 使用带奖励标注的有向图任务，让LLMs作为模拟教师观察假设学习者的轨迹，揭示单一边缘以改善学习者路径规划。将LLMs的选择与人类认知模型（贝叶斯最优教师、弱贝叶斯变体、启发式基线、非心理化效用模型）进行拟合比较，并进行模型对比（BIC）

Result: 大多数LLMs表现良好，策略在试验间变化小，图间表现与人类相似。贝叶斯最优教学最能解释大多数模型的选择。脚手架干预虽能引导模型遵循推理或奖励导向的提示，但未能可靠改善在启发式不一致测试图上的教学，有时反而降低性能

Conclusion: LLMs在教学决策中主要采用贝叶斯最优推理策略，认知模型拟合揭示了LLMs的辅导策略，提示合规性不能保证更好的教学决策
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖监督微调，缺乏代码执行环境暴露，且即使最先进的MLLMs也难以有效进行自校正。需要一种能够通过环境交互和迭代自校正来生成更准确可执行代码的方法。

Method: 提出MM-ReCoder模型，采用基于GRPO的两阶段多轮自校正强化学习策略：第一阶段通过共享首轮增强模型自校正能力，第二阶段通过全轨迹优化提升编码能力。模型通过与环境的交互和迭代自校正学习生成更准确的代码。
...
<details>
  <summary>Details</summary>
Motivation: 现有拒绝特征选择方法依赖特征在有害提示上的激活强度，这往往捕捉表面信号而非拒绝决策的因果因素。需要更可靠的方法来识别因果介导拒绝行为的特征。

Method: 提出CRaFT框架，通过评估特征在拒绝边界附近提示上对模型拒绝-合规决策的影响来排序特征，使用电路引导方法识别因果相关的拒绝特征。

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率从6.7%提升至48.2%，在多个越狱基准测试中优于基线方法。

Conclusion: 电路影响力比激活强度是更可靠的特征选择标准，能更好地识别因果介导拒绝行为的特征，为理解LLM拒绝机制提供了新视角。
...
TL;DR: 提出Metric Freedom (F)作为技能效用的先验预测器，通过两阶段自适应蒸馏框架，在保持性能的同时显著降低多智能体系统的协调开销和延迟。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)虽然能处理复杂任务，但存在协调开销大、上下文碎片化和脆弱的阶段排序等问题。将MAS蒸馏为单智能体技能可以绕过这些成本，但缺乏关于何时以及蒸馏什么的原则性指导，导致经验结果不一致（技能提升从28%改进到2%退化）。

Method: 引入Metric Freedom (F)作为技能效用的先验预测器，通过Mantel测试量化输出多样性与得分方差之间的耦合程度，测量度量评分景观的拓扑刚性。基于F提出两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"度量上提取工具和知识同时丢弃限制性结构以保持探索；阶段2专门针对"刚性"度量(F ≲ 0.6)进行计算密集的迭代优化，消除轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个度量上的评估显示，F能强烈预测技能效用(ρ = -0.62, p < 0.05)。相同的智能体轨迹在刚性度量和自由度量下产生截然相反的技能提升效果，表明技能效用本质上是度量层面的属性。自适应智能体匹配或超越原始MAS，同时将成本降低高达8倍，延迟降低高达15倍。

Conclusion: 技能效用不是由任务决定的，而是由评估度量决定的。Metric Freedom (F)作为第一个先验预测器，能有效指导自适应蒸馏过程，在保持多智能体系统性能的同时显著降低协调开销和延迟。
...
Method: 提出GraphWalk框架，为LLM配备最小正交图操作集（遍历、查询等），通过工具调用实现序列化图导航，每个工具调用代表可验证的推理步骤

Result: 在所有测试模型家族中，基于工具的遍历方法相比上下文基线均有显著提升，且随着规模增大优势更明显；在迷宫遍历任务中，非推理模型完全无法解决而GraphWalk能成功

Conclusion: GraphWalk作为问题无关、无需训练的工具化框架，能显著提升LLM在图结构上的多跳推理能力，特别是在大规模企业级知识图谱场景下，相比传统方法具有明显优势
...
Method: 在AWS Bedrock平台上比较四种指令调优模型：Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct和Mistral 7B Instruct，评估指标包括分类准确性、响应稳定性和令牌级成本，研究零样本和少样本提示方法的效果

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间达到了最有利的平衡

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中性能与成本效益最佳的选择，研究为生产环境中的LLM应用提供了成本感知的评估框架
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型暴露去噪轨迹，为推理时控制提供天然信号；理想幻觉缓解框架应利用这一模型原生信号而非依赖外部训练的幻觉分类器

Method: 提出承诺不确定性定位：识别交叉链熵超过无监督阈值的token位置；开发OSCAR框架：运行N条并行去噪链，计算交叉链香农熵检测高不确定性位置，基于检索证据进行针对性重掩码

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA上使用LLaDA-8B和Dream-7B，OSCAR显著减少幻觉内容并提高事实准确性；其基于熵的不确定性信号优于专门训练的检测器

Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，这是自回归模型不具备的；OSCAR展示了利用模型原生信号进行推理时幻觉缓解的有效性
...
TL;DR: EnviSmart是一个用于环境FAIR数据管理的生产系统，通过三轨知识架构和角色分离的多智能体设计来解决LLM驱动工作流中的可靠性问题，在校园存储基础设施上部署，成功检测并阻止了坐标转换错误。


<details>
  <summary>Details</summary>
Motivation: 将LLM驱动的智能体嵌入环境FAIR数据管理很有吸引力，因为它们可以外部化操作知识并在异构数据和不断发展的规范中进行规模化数据管理。然而，用概率性工作流替换确定性组件改变了故障模式：LLM管道可能生成看似合理但不正确的输出，这些输出通过表面检查并传播到不可逆的操作中，如DOI分配和公开发布。
...
Result: 比较了两个生产部署：大学GIS中心生态档案（849个数据集）作为单智能体基线；SF2Bench复合洪水基准（2,452个监测站和8,557个文件，跨越39年）验证了多智能体工作流。多智能体方法提高了效率和可靠性：由单个操作员在两天内完成，通过跨部署重复使用工件；审计交接检测并阻止了影响所有2,452个站的坐标转换错误。代表性事件（ISS-004）展示了基于边界的遏制，检测延迟10分钟，零用户暴露，80分钟解决。

Conclusion: EnviSmart通过架构方法解决了LLM驱动数据管理中的可靠性挑战，将三轨知识架构与角色分离的多智能体设计相结合，在不可逆操作前恢复故障停止语义。该系统已成功部署，并在PEARC 2026会议上被接受。
...
Method: CORAL框架采用长时运行的智能体，通过共享持久记忆、异步多智能体执行和心跳干预实现自主探索、反思和协作，并提供隔离工作空间、评估器分离、资源管理等安全保障机制

Result: 在10个多样化任务上取得新的SOTA结果，改进率比固定进化搜索基线高3-10倍且评估次数更少；在Anthropic内核工程任务中，四个协同进化智能体将最佳得分从1363周期提升至1103周期

Conclusion: 增强智能体自主性和多智能体进化能显著改善开放式发现效果，CORAL框架通过知识重用和多智能体探索通信机制实现了这一目标
...
Method: 采用Gang-of-Four模式语言提出七种本体感知设计模式：本体检查点、休眠感知管道、漂移哨兵、双本体层、具化断路器、术语版本门和监管合规适配器，并通过初级保健AI系统参考架构和糖尿病风险预测场景进行说明。

Result: 建立了包含七种设计模式的完整设计词汇，其中三种模式在现有系统中有部分先例，四种模式为首次正式描述，提供了可组合的参考架构但缺乏运行时基准测试。

Conclusion: 该研究为构建对本体扭曲具有弹性的临床AI系统提供了理论基础和设计模式词汇，但需要在实际生产系统中进行未来评估，且受限于德国和欧盟监管环境。
...
TL;DR: BACM-RL：基于强化学习的预算感知上下文管理框架，通过序列决策优化长时推理中的上下文压缩策略，在有限上下文预算下提升LLM智能体性能
...
Result: 在组合式多目标QA和长时网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置下获得超过1.6倍的性能提升，在预算缩减时仍保持优势

Conclusion: BACM-RL通过强化学习优化上下文压缩策略，有效解决了长时推理中的上下文预算约束问题，为LLM智能体在资源受限环境下的高效运行提供了解决方案
...
<details>
  <summary>Details</summary>
Motivation: 长期记忆对智能体保持历史一致性至关重要，但大量交互数据的积累会导致检索噪声增加、计算延迟上升，特别是在资源受限的个人设备上会超出模型的推理能力。

Method: 提出分层记忆编排（HMO）框架，构建三层目录结构：紧凑的主缓存（存储近期和关键记忆与用户画像）、高优先级次层、全局历史档案。用户画像驱动记忆在层次间的重新分配，将符合长期模式的记录提升到更活跃层级，降低不相关信息的重要性。

Result: 在多个基准测试中达到最先进性能。OpenClaw等生态系统的实际部署表明，HMO显著提升了智能体的流畅性和个性化程度。

Conclusion: HMO通过用户中心化的分层记忆管理，在需要时精确呈现历史知识，同时保持精简高效的活跃搜索空间，解决了长期记忆扩展带来的性能瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要针对同源模型（来自相同预训练主干），但在开放模型生态系统中，有用专家模型常基于不同架构（如Llama、Qwen、Mistral），直接权重空间融合因架构不匹配、潜在基不对齐和跨源冲突而变得不适定
...
Conclusion: HeteroFusion为解决异构语言模型融合问题提供了一种有效方法，通过拓扑对齐和冲突感知机制克服了不同架构模型间的融合障碍，在开放模型生态系统中具有重要应用价值
...
TL;DR: 研究基于中国视频平台数据，发现AIGC与HGC在创作消费行为上存在差异：AIGC创作者通过高产量获得与HGC相当的总体参与度，但消费者仍偏好HGC；算法分发机制能调节AIGC的竞争关系。
...
Method: 利用中国领先视频分享平台的纵向数据集，包含数千万用户数据，分析AIGC与HGC在创作和消费行为上的差异特征。
...
<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断对飞行安全至关重要，但在资源受限的边缘设备上部署深度学习模型面临计算能力和可解释性的双重挑战。现有方法难以同时满足轻量化、高精度和可解释性的需求。
...
Conclusion: LiteInception框架在效率、准确性和可解释性之间取得了有利平衡，通过轻量化设计、自适应调节和可追溯证据链，为边缘设备上的航空故障诊断提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AnIML作为分析化学和生物学领域的XML标准，其表达灵活性导致不同利益相关者产生分歧解释，破坏了该模式原本设计的互操作性，阻碍了数据驱动的科学发现。
...
Result: 成功开发了AnIML本体论，形式化了AnIML语义并与Allotrope数据格式对齐，验证了本体在支持跨系统和跨实验室互操作性方面的有效性。

Conclusion: AnIML本体论通过形式化语义和与Allotrope的对齐，为解决异构实验数据系统间的语义互操作性提供了有效解决方案，支持未来的跨系统和跨实验室数据交换。
...
TL;DR: 提出基于SAT的二维单规格板材切割问题求解框架，通过需求展开、板材分配变量和不可行方向消除规则，在Cui-Zhao基准测试中优于现有求解器


<details>
  <summary>Details</summary>
Motivation: 二维单规格板材切割问题(2D-CSSP)是制造业中的核心任务，需要从标准板材上切割矩形零件以满足需求并最小化浪费。该问题比装箱问题更复杂，因为每种零件类型需要多个副本，导致组合爆炸。现有求解器在处理该问题时存在效率限制，需要更有效的求解方法。

Method: 1. SAT框架：将每种零件类型按需求展开，每个副本分配板材变量，仅当副本分配到同一板材时才激活非重叠约束；2. 不可行方向消除规则：当零件只有一种方向能放入板材时，固定旋转变量；3. 三种最小化板材数方法：非增量SAT+二分搜索、增量SAT+跨迭代子句重用、加权部分MaxSAT。

Result: 在Cui-Zhao基准测试套件中，最佳SAT配置比OR-Tools、CPLEX和Gurobi多证明2-3倍实例为最优，获得更小的最优性间隙。不同SAT方法的相对性能取决于旋转约束：无旋转时增量SAT最强，旋转增加公式大小时非增量SAT更有效。

Conclusion: SAT方法在二维板材切割问题上显著优于传统求解器，通过需求展开和不可行方向消除规则有效处理组合复杂性。增量SAT在无旋转时表现最佳，而非增量SAT在旋转约束增加时更有效，为制造业切割优化提供了新的高效解决方案。
...
Method: 提出领域情境化概念图(DCG)框架，将领域信息写入关系并解释为模态世界约束。采用(C, R at D, C')形式，其中at D标记关系成立的世界。形式上通过领域索引的必要性算子解释关系，使真值、推理和冲突检查都限定在相关世界范围内。开发了Kripke风格语义、紧凑谓词系统、Prolog实现，以及到RDF、OWL和关系数据库的映射。

Result: DCG框架实现了三个结果：1) 模糊概念能在表示点被消歧；2) 无效断言能针对其领域被挑战；3) 跨领域关系能通过显式谓词连接。为领域本身提供了结构化和可计算的角色。

Conclusion: 领域应被视为知识表示的一部分而非补充标注。DCG通过将领域信息结构化地嵌入表示内部，解决了概念语义随领域变化的核心问题，为知识系统提供了更健壮和精确的表示框架。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过量化视觉依赖度并动态重塑token级优势，解决传统RLVR方法中学习信号稀释的问题，显著提升多模态推理性能。
...
Method: 提出Token Visual Dependency概念，通过KL散度量视觉输入的信息增益；基于此引入Perception-Grounded Policy Optimization框架，采用阈值门控、质量守恒机制动态重塑token级优势，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。

Result: 在Qwen2.5-VL系列模型上，基于七个挑战性多模态推理基准测试，PGPO平均提升模型性能18.7%；理论和实证分析证实PGPO能有效降低梯度方差、防止训练崩溃，并作为强大的正则化器。

Conclusion: PGPO通过细粒度信用分配解决了RLVR中的学习信号稀释问题，实现了稳健的感知基础多模态推理，为强化学习在多模态领域的应用提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 临床电子健康记录预测面临高维度、异质性、类别不平衡和分布偏移等挑战。虽然表格上下文学习和检索增强方法在通用基准上表现良好，但它们在临床环境中的行为尚不明确。需要评估这些方法在不同数据规模、特征维度、结果稀有性和跨队列泛化能力下的表现。

Method: 1. 建立多队列电子健康记录基准，比较经典方法、深度表格方法和表格上下文学习方法；2. 发现PFN基表格上下文学习方法在低数据量下样本效率高，但在异质性和不平衡增加时，基于朴素距离的检索会退化；3. 提出AWARE框架，使用监督嵌入学习和轻量级适配器实现任务对齐的检索。

Result: AWARE框架在极端不平衡情况下将AUPRC提升高达12.2%，且随着数据复杂性增加，性能提升更加明显。PFN基表格上下文学习方法在低数据量下表现良好，但随着异质性和不平衡增加，朴素距离检索方法性能下降。

Conclusion: 检索质量和检索-推理对齐是部署表格上下文学习于临床预测的关键瓶颈。AWARE框架通过任务对齐的检索策略有效解决了这些问题，为临床电子健康记录预测提供了更可靠的解决方案。
...
TL;DR: 提出CG-iLAO*算法，通过约束生成技术避免考虑昂贵的动作，显著减少Bellman备份中考虑的动作数量，提高随机最短路径问题求解效率


<details>
  <summary>Details</summary>
Motivation: 传统SSP算法使用启发式函数指导状态选择，但Bellman备份仍需考虑所有适用动作，即使启发式表明某些动作代价过高，导致计算资源浪费

Method: 将启发式搜索重新表述为线性规划问题，引入约束生成技术，开发CG-iLAO*算法，在iLAO*基础上通过约束生成避免考虑昂贵动作

Result: CG-iLAO*仅考虑iLAO* 40%的动作（某些问题低至1%），平均减少3.5倍动作代价计算，比iLAO*快2.8倍，比LRTDP快3.7倍

Conclusion: 约束生成技术能有效避免考虑昂贵动作，显著提升SSP问题求解效率，为启发式搜索算法提供了新的优化方向
...
<details>
  <summary>Details</summary>
Motivation: 在多类分类中，传统监督通常使用硬标签或概率分布，但现实场景中监督信息可能以可能性分布形式存在，表达对类别的分级可信度。需要一种能够处理这种模糊监督信息的学习方法。

Method: 1. 从可能性分布构造非空闭凸可容许概率分布集，结合概率相容性和线性形状约束；2. 对模型输出的正概率向量计算其在可容许集上的Kullback-Leibler投影；3. 使用Dykstra算法和负熵相关的Bregman投影计算投影，提供显式公式；4. 通过最小化预测与其投影之间的散度来训练模型。

Result: 在合成数据和基于ChaosNLI数据集的真实自然语言推理任务上的实验表明：提出的投影算法足够高效，可用于实际应用；基于投影的学习目标能够提升预测性能。

Conclusion: 该方法为处理可能性监督的多类分类问题提供了一种有效的学习框架，通过构造可容许概率集和KL投影，能够在保持可能性分布定性结构的同时进行模型训练，在实际应用中表现出良好的性能。
...
Method: 基于一阶逻辑构建Qiana框架，使其可有限公理化。该框架允许对公式和上下文进行量化，支持上下文内使用次协调逻辑（允许矛盾存在），并设计了不同的设计替代方案。

Result: 展示了Qiana能够表示时间性、事件演算和模态逻辑。框架与现有的一阶逻辑定理证明器兼容，因为它是基于一阶逻辑且可有限公理化的。

Conclusion: Qiana是一个灵活的逻辑框架，能够在特定上下文中进行推理，支持对公式和上下文的量化，允许上下文内矛盾，并且与现有的一阶逻辑工具兼容，适用于多种逻辑表示需求。
...
TL;DR: NeuroPose-AHM是一个基于知识的异常头部运动数据集，通过多LLM框架从1430篇文献中提取构建，包含57种神经疾病的2756条患者组记录，并展示了在颈部肌张力障碍中的四任务分析应用。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了AI驱动诊断工具的发展。本研究旨在填补这一空白。

Method: 采用多LLM提取框架从1430篇同行评审文献中构建NeuroPose-AHM数据集，包含57种神经疾病的2756条患者组记录。对颈部肌张力障碍应用四任务框架：1）多标签AHM类型分类；2）构建头颈严重程度指数（HNSI）；3）验证HNSI与真实患者数据的临床相关性；4）分析运动类型概率与HNSI分数的关联。

Result: LLM提取可靠性高（kappa = 0.822），AHM类型分类F1分数达0.856。HNSI成功整合了异质临床评分量表，与真实患者数据严重程度比例（6.7%）一致。运动类型概率与HNSI分数存在显著相关性（p < 0.001）。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，为神经性异常头部运动研究提供了分析工具，数据集已在Zenodo公开可用。
...
TL;DR: SenseMath基准测试评估LLMs的数字结构感知能力，发现模型能使用计算捷径但缺乏人类数字感的结构理解


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常默认逐步计算，即使存在高效数值捷径。这引发了一个基本问题：它们是否表现出类似人类的数字感，即识别数值结构、在适当情况下应用捷径、在不适用时避免使用的能力？

Method: 引入SenseMath基准测试，包含4,800个项目，涵盖八个捷径类别和四个数字规模，具有匹配的强捷径、弱捷径和控制变体。支持三个评估设置：捷径使用、适用性判断和问题生成。

Result: 评估五个LLMs（从GPT-4o-mini到Llama-3.1-8B）显示：当明确提示时，模型能采用捷径策略并在捷径适用项目上获得显著准确率提升（高达15%），但在标准思维链提示下，它们自发使用此类策略的情况少于40%。这种能力仅限于使用层面；模型系统性地将捷径过度泛化到不适用的问题，并且无法从头生成有效的捷径问题。
...
TL;DR: 提出基于Transformer掩码自编码器的无标签关节级步态异常检测与运动学校正框架，仅使用正常步态数据进行训练，通过两阶段推理实现异常关节定位与轨迹校正。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的步态分析方法大多依赖疾病标签的监督分类器，难以泛化到异质性病理表现。需要一种无需疾病标签、能够解释性定位步态异常并校正运动学轨迹的方法。

Method: 使用仅包含150名成年人正常步态序列的标记无标记多摄像头运动捕捉系统数据，训练Transformer掩码自编码器。推理时采用两阶段流程：1)通过遮挡单个关节并测量与学习到的正常先验的偏差来估计关节不一致性分数；2)从编码器输入中排除标记的异常关节，基于剩余时空上下文重建完整骨架，在标记位置生成校正后的运动学轨迹。

Result: 在10名保留的正常参与者模拟七种步态异常的验证中，该方法能准确定位生物力学不一致的关节，显著减少所有分析关节的角度偏差（效应量大），并保持正常运动学特征。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能增强大语言模型的推理能力，但在多轮智能体任务中存在挑战：次优动作会产生误导性观察，这些观察进一步削弱后续决策，形成错误累积的反馈循环，使得标准探索策略在长时程交互和环境随机性面前失效。

Method: 提出ProCeedRL（Process Critic with Explorative Demonstration RL），将探索从被动选择转变为主动干预。该方法使用过程级批评器实时监控交互，并结合基于反思的演示来指导智能体停止错误累积。通过从探索性演示和在线策略样本中学习，提升探索效率。

Result: ProCeedRL显著超越了模型饱和的探索性能，展示了实质性的探索优势。在复杂的深度搜索和具身任务上，该方法显著提高了探索效率并实现了优越的性能表现。

Conclusion: 通过主动干预错误累积的反馈循环，ProCeedRL为多轮智能体任务的强化学习提供了一种有效的解决方案，能够显著提升探索效率和任务性能，特别是在面对长时程交互和环境随机性的挑战时。
...
TL;DR: ATBench是一个用于评估LLM智能体安全性的轨迹级基准测试，通过三维风险分类、异构工具池和长上下文延迟触发协议，提供结构化、多样化和现实的评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在交互多样性不足、安全失败可观测性粗糙和长视野现实性弱的问题，需要更全面评估LLM智能体在现实部署中的多步交互风险。

Method: 基于三维风险分类（风险来源、失败模式、现实危害）构建轨迹，使用异构工具池和长上下文延迟触发协议，包含1000个轨迹（503安全/497不安全），平均9.01轮交互和3.95k tokens，通过规则和LLM过滤加人工审核确保数据质量。

Result: ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性，支持分类分层分析、跨基准比较和长视野失败模式诊断，包含1954个调用的工具，工具池涵盖2084个可用工具。

Conclusion: ATBench为LLM智能体安全性评估提供了结构化、多样化和现实的轨迹级基准，能够捕捉多阶段风险涌现，支持更全面的安全分析和诊断。
...
### [198] [The Latent Space: Foundation, Evolution, Mechanism, Ability, and Outlook](https://arxiv.org/abs/2604.02029)
...
TL;DR: 这篇综述论文系统性地梳理了语言模型中潜在空间的研究现状，将其组织为五个视角：基础、演进、机制、能力和展望，旨在为下一代智能系统提供统一的计算范式。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，潜在空间逐渐成为语言模型的核心计算基质。然而，当前研究缺乏对潜在空间的系统性梳理和统一框架。本文旨在填补这一空白，为理解潜在空间作为通用计算范式提供基础。

Method: 采用综述研究方法，将现有工作组织为五个顺序视角：基础（定义和范围界定）、演进（发展历程）、机制（从架构、表示、计算和优化四个维度分析）、能力（展示潜在空间支持的七大能力谱系）和展望（未来研究方向）。

Result: 构建了一个统一且最新的潜在空间研究图景，系统性地分析了潜在空间在语言模型中的作用机制和支撑能力，识别了四个主要技术发展方向和七大能力领域。

Conclusion: 潜在空间正在成为语言模型的核心计算范式，能够克服显式空间的结构性限制。本文的综述框架不仅为现有研究提供参考，更为理解潜在空间作为下一代智能系统的通用计算范式奠定基础。
...
### [199] [AI in Insurance: Adaptive Questionnaires for Improved Risk Profiling](https://arxiv.org/abs/2604.02034)
...
Method: 结合大型语言模型和替代数据源（社交媒体图像分析、地理数据分类、检索增强生成）创建个性化自适应问卷，通过行业合作伙伴移动应用进行实验验证

Result: 传统问卷风险评估准确率略高，但GPT驱动的自适应版本所需问题更少，用户体验更流畅且更受欢迎
...
### [200] [Diff-KD: Diffusion-based Knowledge Distillation for Collaborative Perception under Corruptions](https://arxiv.org/abs/2604.02061)
...
Conclusion: Diff-KD通过整合扩散生成式精炼与知识蒸馏，能够主动恢复损坏观测中的清洁语义，显著提升了多智能体协同感知在现实损坏条件下的鲁棒性
...
### [201] [LLM-as-a-Judge for Time Series Explanations](https://arxiv.org/abs/2604.02118)
*Preetham Sivalingam,Murari Mandal,Saurabh Deshpande,Dhruv Kumar*

Main category: cs.AI

TL;DR: 该论文研究了在无参考设置下使用大语言模型作为时间序列解释的生成器和评估器，构建了包含350个案例的合成基准，发现生成任务存在模式依赖性且在某些查询类型上失败严重，而评估任务则更加稳定可靠。


<details>
  <summary>Details</summary>
Motivation: 评估基于时间序列数据的大语言模型生成的自然语言解释的事实正确性是一个开放挑战。现有评估方法有限：基于参考的相似性指标和一致性检查模型需要真实解释，而传统时间序列方法仅处理数值且无法评估自由形式的文本推理。因此，缺乏无需预定义参考或任务特定规则就能直接验证解释是否忠实于底层时间序列数据的通用方法。

Method: 在无参考设置下研究大语言模型作为时间序列解释的生成器和评估器。构建包含350个时间序列案例的合成基准，涵盖七种查询类型，每个案例配有正确、部分正确和错误的解释。评估模型在四个任务上的表现：解释生成、相对排名、独立评分和多异常检测。评估器基于模式识别、数值准确性和答案忠实度分配三元正确性标签。

Result: 结果显示出明显的不对称性：生成任务高度依赖模式，在某些查询类型上表现出系统性失败，准确率从季节性下降和波动率变化的0.00-0.12到结构突变的0.94-0.96不等；而评估任务更加稳定，即使模型自身输出不正确，也能正确排名和评分解释。这表明基于数据的大语言模型评估在时间序列解释领域是可行的。

Conclusion: 研究证明了基于数据的大语言模型评估在时间序列解释领域的可行性，并突显了它们作为时间序列领域中数据驱动推理的可靠评估器的潜力。评估任务比生成任务更加稳定，为无参考设置下的时间序列解释评估提供了新方法。
...
TL;DR: TRACE-Bot：一种用于检测LLM驱动社交机器人的统一双通道框架，通过联合建模隐式语义表示和AIGC增强的行为模式，在公开数据集上达到98.46%和97.50%的准确率。
...
Method: 提出TRACE-Bot统一双通道框架：1）从个人信息数据、交互行为数据和推文数据构建细粒度表示；2）双通道架构通过预训练语言模型捕获语言表示，通过多维活动特征（增强了SOTA AIGC检测器信号）捕获行为异常；3）融合表示通过轻量级预测头进行分类。
...
<details>
  <summary>Details</summary>
Motivation: 当前在航空等安全关键领域部署AI系统时，EASA指南要求证明AI/ML组件的操作设计域（ODD）完全覆盖，但现有方法在高维参数空间中缺乏可扩展性和形式化基础，无法满足完整性要求，且缺乏标准化的工程方法来连接抽象ODD定义与可验证证据

Method: 提出了一种结构化多步骤ODD覆盖验证方法，整合了参数离散化、基于约束的过滤和基于关键性的维度约简，基于先前AI空中防撞研究的仿真数据进行验证

Result: 该方法能够验证高维空间中的ODD覆盖，展示了定义和实现满足EASA完整性要求的覆盖度量的系统化工程方法

Conclusion: 该方法填补了当前标准化工程方法的空白，支持在更高维度验证ODD覆盖，推进了"安全设计"方法，同时符合EASA标准
...
TL;DR: 该研究评估了LLM生成的胸部CT报告日语翻译的教育适用性，发现放射科医师与LLM评估者之间存在显著差异，LLM评估者强烈偏好LLM输出，而放射科医师意见也不一致，表明自动化LLM评估不足以替代专家评审。


<details>
  <summary>Details</summary>
Motivation: 放射学报告的准确翻译对于多语言研究、临床沟通和放射学教育至关重要，但基于LLM的翻译评估有效性尚不明确。研究旨在评估LLM生成的胸部CT报告日语翻译的教育适用性，并比较放射科医师评估与LLM作为评估者的结果。

Method: 研究分析了CT-RATE-JPN验证集中的150份胸部CT报告。将每份英文报告的人工编辑日语翻译与DeepSeek-V3.2生成的LLM翻译进行比较。一名认证放射科医师和一名放射科住院医师独立进行盲法配对评估，评估4个标准：术语准确性、可读性、整体质量和放射科医师风格真实性。同时，3个LLM评估者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同的配对。使用QWK和百分比一致性评估一致性。

Result: 放射科医师与LLM评估者之间的一致性接近零（QWK=-0.04至0.15）。两名放射科医师之间的一致性也很差（QWK=0.01至0.06）。放射科医师1认为59%的病例术语等效，并在可读性（51%）和整体质量（51%）方面偏好LLM翻译。放射科医师2认为75%的病例可读性等效，并在整体质量方面偏好人工编辑翻译（40% vs 21%）。所有3个LLM评估者在所有标准上都强烈偏好LLM翻译（70%-99%），并在>93%的病例中认为其更具放射科医师风格。

Conclusion: LLM生成的翻译通常被认为自然流畅，但两名放射科医师评估存在显著差异。LLM作为评估者表现出对LLM输出的强烈偏好，与放射科医师的一致性可忽略不计。对于翻译放射学报告的教育用途，仅依赖自动化LLM评估是不够的；专家放射科医师评审仍然至关重要。
...
Conclusion: 该方法为理解生成式AI系统如何感知和优先处理输入信息提供了一种轻量级、模型无关且计算高效的可视化解决方案。
...
### [206] [When to ASK: Uncertainty-Gated Language Assistance for Reinforcement Learning](https://arxiv.org/abs/2604.02226)
...
### [207] [Answering the Wrong Question: Reasoning Trace Inversion for Abstention in LLMs](https://arxiv.org/abs/2604.02230)
*Abinitha Gourabathina,Inkit Padhi,Manish Nagireddy,Subhajit Chaudhury,Prasanna Sattigeri*

Main category: cs.AI

TL;DR: 提出Trace Inversion方法，通过比较原始查询与从模型推理轨迹重构的查询之间的相似度，来提升推理模型在问答任务中的弃权能力


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在部署时需要有效的弃权能力，但推理模型在这方面表现较差。模型幻觉导致的弃权失败可以被重新解释为模型回答了错误的问题，而非错误地回答了问题

Method: 提出查询错位框架和Trace Inversion方法：首先生成模型的推理轨迹，仅基于轨迹重构模型最可能回答的查询，然后比较原始查询与重构查询的相似度，低相似度表明模型可能回答错误并标记为弃权

Result: 在四个前沿LLM和九个弃权QA数据集上的广泛实验表明，Trace Inversion在36个设置中的33个中击败了竞争基线，有效提升了弃权性能

Conclusion: Trace Inversion通过查询错位框架提供了一种新的、最先进的弃权方法，显著提高了推理模型在不确定情况下的弃权能力
...
Result: 在金融、医疗和AI治理三个监管语料库上评估，De Jure在金融领域实现一致且单调的质量提升，3次迭代内达到峰值性能；在医疗和AI治理领域保持高性能；下游合规问答评估中，基于De Jure提取规则的响应在73.8%（单规则检索）到84.0%（广泛检索）情况下优于先前工作。
...
<details>
  <summary>Details</summary>
Motivation: 研究情感语气在用户查询中对大语言模型行为的影响，探索情感框架是否可以作为有效的干预手段来提升LLM性能

Method: 在六个基准领域（数学推理、医学问答、阅读理解、常识推理和社交推理）测试静态情感前缀的影响；引入EmotionRL自适应情感提示框架，根据每个查询自适应选择情感框架

Result: 静态情感前缀通常只产生小的准确性变化；情感影响在社交基础任务中更可变；更强的情感用词仅引起适度额外变化；自适应情感选择比固定情感提示产生更可靠的性能提升
...
### [210] [Novel Memory Forgetting Techniques for Autonomous AI Agents: Balancing Relevance and Efficiency](https://arxiv.org/abs/2604.02280)
...
Result: 比较分析显示，该方法将长对话F1分数提升至超过0.583的基线水平，提高了记忆保留一致性，减少了错误记忆行为，且未增加上下文使用量。
