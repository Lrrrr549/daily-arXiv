...
TL;DR: 该论文提出了一种新的步态识别视觉模态SKETCH，通过边缘检测器从RGB图像中提取高频结构线索，并设计了SKETCHGAIT多模态框架，在多个数据集上取得了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要基于轮廓和解析表示。轮廓表示稀疏且缺乏内部结构细节，解析表示依赖上游人体解析器导致性能不稳定。需要探索新的步态表示范式。

Method: 从结构视角重新审视步态表示，提出SKETCH作为新视觉模态，通过边缘检测器从RGB图像中无标签地提取高频结构线索。进一步提出SKETCHGAIT框架，包含两个独立流进行模态特定学习，以及轻量级早期融合分支捕捉结构互补性。

Result: 在SUSTech1K数据集上达到92.9% Rank-1准确率，在CCPG数据集上达到93.1%平均Rank-1准确率，验证了所提模态和框架的有效性。

Conclusion: SKETCH作为一种新的步态识别视觉模态，能够有效捕捉高频结构线索，与标签引导的解析表示在语义上解耦且结构上互补，SKETCHGAIT框架实现了优越的识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法受限于小规模标注数据集（包含显式设计历史或BRep标签），而数百万未标注3D网格未被利用，限制了可扩展CAD生成的进展

Method: 1. 将BRep表示为参数化曲面片集合（如Bézier曲面）；2. 使用可微分细分方法生成网格；3. 通过点级监督在3D数据集上进行大规模训练；4. 引入GPT-5生成的CADCap-1M数据集（100万+描述）

Result: 在ABC和Objaverse基准测试中，DreamCAD在文本、图像和点云模态上均达到最先进性能，提升了几何保真度，获得超过75%的用户偏好

Conclusion: DreamCAD通过点级监督实现可编辑BRep的直接生成，无需CAD特定标注，解决了数据稀缺问题，为可扩展CAD生成提供了新途径
...
TL;DR: ABRA通过对抗性表示增强解决细胞成像中的生物批次效应，将批次效应缓解建模为领域泛化问题，在RxRx1基准上实现了最先进的siRNA扰动分类性能。
...
Method: 提出对抗性批次表示增强(ABRA)，将批次效应缓解建模为领域泛化问题。通过参数化特征统计量为结构化不确定性来显式建模批次间统计波动，采用min-max优化框架在表示空间中主动合成最坏情况的生物批次扰动，并利用严格的角几何边界保持细粒度类别可区分性。为防止对抗探索中的表示崩溃，引入了协同分布对齐目标。

Result: 在RxRx1和RxRx1-WILDS大规模基准上的广泛评估表明，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性表示增强有效缓解了细胞成像中的生物批次效应，将批次效应缓解重新定义为领域泛化问题，在保持类别可区分性的同时提高了模型对未见生物批次的泛化能力。
...
TL;DR: 提出PFS（后融合稳定器），一种轻量级模块，通过稳定BEV特征统计、抑制传感器退化区域和自适应残差校正，提升现有BEV融合检测器在域偏移和传感器故障下的鲁棒性，无需修改原始架构或重新训练。
...
Method: PFS作为轻量级模块，在现有检测器的中间BEV表示上操作，生成精炼的特征图供原始检测头使用。设计包括：稳定域偏移下的特征统计、抑制传感器退化影响的空间区域、通过残差校正自适应恢复弱化线索。作为近似恒等变换设计，保持性能的同时提升鲁棒性。

Result: 在nuScenes基准测试中，PFS在多种故障模式下取得SOTA结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，仅需3.3M参数，保持轻量级。

Conclusion: PFS提供了一种实用的后融合稳定方法，可无缝集成到现有BEV融合检测器中，显著提升在域偏移和传感器故障下的鲁棒性，同时保持轻量级和性能。
...
Method: 提出CBM-Suite框架：1) 基于熵的指标量化概念集的内在适用性；2) 在概念激活和分类器之间插入非线性层解决线性化问题；3) 使用线性教师探针引导的蒸馏损失缩小准确率差距；4) 系统性分析不同视觉编码器、视觉语言模型和概念集对准确率和可解释性的影响

Result: 广泛评估表明CBM-Suite能产生更准确的模型，并为改进基于概念的可解释性提供见解

Conclusion: CBM-Suite系统性地解决了概念瓶颈模型的关键挑战，通过量化概念适用性、确保概念相关性、缩小准确率差距和提供系统性分析，推动了概念可解释性模型的发展
...
Conclusion: iFID是一种简单有效的指标，能准确预测扩散模型的生成质量，为评估生成模型提供了新的视角，并解释了重建指标与生成质量负相关的原因。
...
TL;DR: 提出Implicit Error Counting (IEC)方法，通过隐式枚举错误而非依赖理想参考答案来构建奖励信号，解决了多有效输出任务中参考答案缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR和RaR方法依赖理想参考答案来生成评估标准，但许多现实任务存在多个有效输出且缺乏单一理想答案。本文识别了这一"无参考"设置的空白，并提出IEC方法来填补这一空白。

Method: 提出Implicit Error Counting (IEC)方法：1) 枚举响应中的错误而非检查正确部分；2) 在任务相关轴上应用严重性加权评分；3) 将错误转换为校准的每方面奖励。为避免显式枚举的噪声问题，采用隐式分数发射和组校准两个关键设计。在虚拟试衣(VTO)案例中，进一步提出Cascaded Error Counting (CEC)作为评估指标。

Result: 1) CEC指标能很好跟踪人类偏好(60% top-1 vs. 30%其他方法)；2) 在MDressBench基准上，IEC在所有指标上优于RaR(CEC: 5.31 vs. 5.60平坦参考；5.20 vs. 5.53非平坦)；3) 在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越6个基线方法。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号。IEC方法为缺乏单一理想答案的多有效输出任务提供了一种有效的后训练强化学习奖励设计方法。
...
TL;DR: SemVID：一种无需训练的视频时序定位剪枝框架，通过保留关键证据和跨帧连接性，仅用12.5%视觉token实现95.4% mIoU，速度提升5.8倍


<details>
  <summary>Details</summary>
Motivation: 视频时序定位（VTG）需要处理长未修剪视频，使用视频语言模型（VLM）成本过高。现有视觉token剪枝方法直接应用于VTG会导致性能严重下降，因为VTG依赖边界敏感证据和跨帧推理链。

Method: 提出SemVID框架，基于两个VTG特定剪枝原则：证据保留（ER）和连接强度（CS）。首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三类token：对象token（多样化关键证据）、运动token（捕捉有意义转换并作为跨帧中继）和少量上下文token（保持场景连续性）。

Result: 在VTG基准测试中，SemVID仅用12.5%视觉token就能保持95.4% mIoU，预填充速度提升5.8倍，在相同预算下始终优于先前方法。

Conclusion: SemVID通过构建紧凑而连贯的token子集，实现了VTG任务中准确性和效率的良好平衡，验证了证据保留和连接强度原则在视频时序定位剪枝中的重要性。
...
TL;DR: OWL是一种基于视觉运动线索的感知函数，通过两种基本视觉运动线索直接计算值，无需先验环境知识，实现3D场景重建和相机航向估计。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，特别是如何在缺乏先验环境知识、相机运动信息或静止环境假设的情况下，仅从原始视觉运动图像序列中实现3D感知。

Method: 基于两种基本视觉运动线索：1) 注视点附近点的感知局部视觉逼近，2) 刚性物体相对于注视点的感知旋转。通过像素级并行计算，直接从原始视觉运动图像序列中提取OWL值，无需测量相对瞬时方向范围和方向平移等物理量。

Result: 仿真实验表明OWL能够实现3D物体的几何恒常性，仅凭视觉运动线索就能实现缩放3D场景重建。该方法提供了一种替代的实时3D点相对运动表示。

Conclusion: OWL填补了理论概念与机器人、自主导航实际应用之间的空白，可能成为下一代自主系统的构建模块，为机器感知提供新视角，对自然感知、行为心理学和神经功能理解也有启示。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准，包含46,000+多模态检索候选项和747个开放可验证问题，揭示当前MLLMs在异构多模态检索中的瓶颈。
...
Method: 构建MultiHaystack基准，包含超过46,000个跨文档、图像和视频的多模态检索候选项，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的已验证证据项，要求跨模态证据定位和细粒度推理。

Result: 模型在提供对应证据时表现良好，但从完整语料库中检索证据时性能显著下降。最强检索器E5-V仅达到40.8%的Recall@1，GPT-5等SOTA MLLMs在提供对应证据时推理准确率为80.86%，但在top-5检索下降至51.4%。

Conclusion: 异构多模态检索是MLLMs的主要瓶颈，MultiHaystack作为有价值的测试平台，揭示了小规模评估掩盖的底层限制，促进了多模态系统中检索中心化的发展。
...
TL;DR: 本文提出Audiovisual Geolocation框架，通过可解释的感知和推理解决地理定位中的视觉模糊问题，结合音频和视觉线索实现高精度全球定位。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基于图像的定位方面有所改进，但精确的全球地理定位仍面临挑战，主要因为视觉景观固有的模糊性以及音频线索的潜力尚未被充分挖掘。

Method: 提出三阶段框架：1) 感知阶段使用混合自回归稀疏自编码器将噪声音频分解为语义基础的"声学原子"；2) 多模态推理阶段使用通过组相对策略优化微调的MLLM将声学原子与视觉特征融合；3) 精确预测阶段在S^2流形上使用黎曼流匹配进行定位。

Result: 实验表明该框架显著优于单模态基线，证明声景的可解释感知提供了关键的正交信号，结合多模态推理可实现高精度全球定位。

Conclusion: 音频线索为地理定位提供了与视觉互补的重要信息，通过可解释的声学感知和多模态推理，能够有效解决视觉模糊问题，实现精确的全球地理定位。
...
TL;DR: Any2Full是一个单阶段、领域通用、模式无关的深度补全框架，通过尺度提示适配预训练的单目深度估计模型，实现稀疏深度到稠密深度的鲁棒转换。


<details>
  <summary>Details</summary>
Motivation: 现有RGBD融合深度补全方法受限于特定训练分布和深度模式，泛化能力差；而基于单目深度估计的两阶段方法需要显式相对-度量对齐，计算开销大且引入结构失真。

Method: 将深度补全重新定义为预训练单目深度估计模型的尺度提示适配。设计尺度感知提示编码器，从稀疏输入中提取尺度线索并编码为统一尺度提示，引导模型生成全局尺度一致的预测，同时保持其几何先验。

Result: Any2Full在鲁棒性和效率方面表现优异：相比OMNI-DC在平均AbsREL指标上提升32.2%，相比PriorDA在相同MDE骨干下实现1.4倍加速，建立了通用深度补全的新范式。

Conclusion: Any2Full通过尺度提示适配的单阶段框架，实现了领域通用、模式无关的深度补全，在保持几何先验的同时显著提升了鲁棒性和效率，为通用深度补全提供了新范式。
...
TL;DR: 该研究提出了一个自动化流程，将ImageNet训练集转换为多标签数据集，无需人工标注，通过自监督视觉Transformer进行无监督物体发现，生成高质量多标签注释，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准采用单标签假设，但许多图像包含多个物体，导致标签噪声并限制了学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，其中多个物体共存并共同贡献于语义理解，使模型能够学习更丰富、更鲁棒的表征。

Method: 开发了一个自动化流程：使用自监督视觉Transformer进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后将该分类器应用于所有区域，在整个数据集中生成一致的多标签注释。

Result: 生成的多标签注释在定性评估中与人类判断高度一致。与传统单标签方案相比，使用多标签监督训练的模型在不同架构上均获得更好的域内准确率（在ReaL上最高提升2.0 top-1准确率，在ImageNet-V2上提升1.5），并在下游任务中展现出更强的迁移能力（在COCO上提升4.2 mAP，在VOC上提升2.3 mAP）。

Conclusion: 准确的多标签注释对于提升分类性能和表征学习至关重要。该研究提供的自动化流程和生成的多标签注释为改进ImageNet训练集提供了有效解决方案，证明了多标签监督在视觉识别任务中的价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术时间线创建方法存在缺陷：术后报告通常模糊不清，而手动标注手术视频又极其耗时。需要一种折中方案来自动化生成结构化手术时间线

Method: 采用基于CLIP的多模态框架，将手术视频帧与文本手势描述对齐。使用CLIP视觉编码器提取视频帧表示，文本编码器嵌入手势句子到共享嵌入空间，通过微调模型提升视频手势与文本标记的对齐度

Result: 训练后的模型能够预测视频帧的手势和阶段，从而构建结构化手术时间线。该方法利用预训练多模态表示桥接视觉手势和文本叙述

Conclusion: 提出的方法能够自动从手术视频创建手术时间线和叙述，减少外科医生手动视频审查和标注的需求，为工具辅助手术提供高效的时间线生成方案
...
TL;DR: Icarus：一种能够学习全动态范围物理捕捉户外图像曝光范围的全天候天空模型，通过条件生成环境贴图，实现用户可控的太阳和云层定位以及大气纹理控制。
...
Method: 提出Icarus模型，能够学习全动态范围物理捕捉户外图像的曝光范围，支持条件生成环境贴图，允许用户直观定位太阳和云层，并扩展当前最先进技术以实现用户控制的大气纹理。

Result: Icarus可与全动态范围物理捕捉户外图像或参数化天空模型互换，在基于图像的照明中实现了前所未有的准确性、真实感、光照方向性（阴影）和色调。

Conclusion: Icarus作为全天候天空模型，解决了现有深度学习方法在高动态范围太阳区域建模的局限性，为视觉艺术、虚拟现实和科学应用提供了更准确、真实的环境贴图生成方案。
...
TL;DR: LayerBind：无需训练的扩散模型区域布局控制方法，通过分层建模和绑定实现精确的区域生成与遮挡顺序控制


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成中的区域指令布局控制方法存在两个主要问题：基于训练的方法会继承数据偏见并降低图像质量；现有技术难以处理遮挡顺序，限制了实际应用。作者观察到空间布局和遮挡在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出。

Method: LayerBind采用两阶段方案：1) 实例初始化阶段：利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在指定早期步骤根据层顺序融合形成具有预设布局的统一潜在表示；2) 语义增强阶段：通过分层注意力增强强化区域细节并保持遮挡顺序，采用顺序分层注意力路径与标准全局路径并行，更新在层透明度调度器下组合。

Result: LayerBind无需训练且即插即用，可作为区域和遮挡控制器应用于各种扩散变换器。除了生成功能外，还原生支持可编辑工作流，允许灵活修改实例或重新排列可见顺序。定性和定量结果都证明了LayerBind的有效性。

Conclusion: LayerBind解决了现有区域布局控制方法的局限性，通过分层建模和绑定实现了精确的区域生成和遮挡顺序控制，具有强大的创意应用潜力，为文本到图像生成提供了更实用的区域控制解决方案。
...
TL;DR: BM25-V将文本检索中的BM25算法应用于视觉Transformer的稀疏自动编码器激活，实现高效、可解释的图像检索，作为密集重排的高召回第一阶段检索器。


<details>
  <summary>Details</summary>
Motivation: 密集图像检索虽然准确，但存在可解释性差、归因困难、大规模计算成本高等问题。需要一种高效且可解释的检索方法。

Method: 使用稀疏自动编码器(SAE)从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分算法进行检索。BM25的逆文档频率(IDF)权重能有效抑制常见低信息词，突出稀有判别性词。

Result: 在七个基准测试中，BM25-V的Recall@200≥0.993，仅需重排K=200个候选就能恢复接近密集检索的准确率（平均差距0.2%）。SAE在ImageNet-1K上训练一次即可零样本迁移到七个细粒度基准，检索决策可归因于特定视觉词及其IDF贡献。

Conclusion: BM25-V提供了一种高效、可解释的图像检索方法，作为密集重排的第一阶段检索器，能在大规模检索中实现高召回率，同时保持可归因性和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型计算延迟高，实时应用成本过高。视频潜在补丁存在时间冗余性，可以通过利用这种冗余性来减少计算量。

Method: 提出Latent Inter-frame Pruning with Attention Recovery (LIPAR)框架：1) 检测并跳过重复的潜在补丁计算；2) 引入注意力恢复机制，近似被剪枝token的注意力值，避免视觉伪影。

Result: 视频编辑吞吐量平均提升1.45倍，在NVIDIA A6000上达到12.2 FPS（基线为8.4 FPS）。生成质量不受影响，无需额外训练即可与模型集成。

Conclusion: LIPAR有效连接了传统压缩算法与现代生成流水线，通过利用时间冗余性显著提升视频生成效率，为实时应用提供可行方案。
...
TL;DR: Cog2Gen3D：一个3D认知引导的扩散框架，通过语义-几何双流图融合实现物理合理且结构合理的3D生成
...
Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：将多模态编码为语义和几何表示，并进一步提取逻辑表示；2）3D潜在认知图：将不同表示结构化为双流语义-几何图，通过基于共通的交叉注意力融合获得3D认知图；3）认知引导的潜在扩散：利用融合的3D认知图作为条件，指导3D高斯生成的潜在扩散过程。

Result: 在基于Marble World Labs构建的验证子集上进行广泛实验，证明Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的3D认知图框架确保了3D生成的物理合理性和结构合理性，为解决3D生成中的空间几何约束问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性之间的根本权衡：2D方法因过度裁剪而受限，3D技术则因脆弱的优化流程在极端运动下失效。需要一种能兼顾两者优势的解决方案。

Method: VS3R框架协同整合前馈3D重建与生成式视频扩散：1）联合估计相机参数、深度和掩码以确保全场景可靠性；2）混合稳定渲染模块融合语义和几何线索实现动态一致性；3）双流视频扩散模型通过结合结构引导与语义锚点来修复遮挡区域和矫正伪影。

Result: VS3R在多样化的相机模型上实现了高保真、全帧稳定的视频处理，在鲁棒性和视觉质量方面显著优于现有最先进方法。

Conclusion: VS3R通过协同整合3D重建与生成式视频扩散，成功解决了视频稳定中的几何鲁棒性与全帧一致性之间的权衡问题，为各种场景提供了可靠的高质量稳定解决方案。
...
TL;DR: 该研究提出了TumorChain框架，通过多模态链式推理进行肿瘤分析，结合3D CT扫描和临床文本，提升诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 临床放射学和精准肿瘤学需要准确的肿瘤分析，包括早期检测、病变特征描述和病理级风险评估。链式思维推理在此场景中尤为重要，因为它能实现从影像发现到临床印象再到病理结论的逐步解释，提高可追溯性并减少诊断错误。

Method: 研究构建了TumorCoT数据集（150万条带CoT标注的VQA指令与3D CT扫描配对），并提出TumorChain多模态交错推理框架。该框架紧密耦合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理，实现视觉证据的定位、结论的聚合以及经过多轮自我精炼后的病理预测。

Result: 实验显示，在病变检测、印象生成和病理分类方面相比强基线模型有持续改进，并在DeepTumorVQA基准测试中表现出强大的泛化能力。这些结果凸显了多模态推理在临床实践中实现可靠且可解释的肿瘤分析的潜力。

Conclusion: 该研究通过构建大规模基准数据集和创新的多模态推理框架，为临床肿瘤分析提供了可靠且可解释的解决方案，显著提升了诊断准确性和推理一致性，具有重要的临床应用价值。
...
TL;DR: PatchCue：一种基于图像块的视觉提示范式，通过将图像分割为块并在块级别表示视觉提示，增强视觉语言模型的视觉推理能力，优于像素级边界框和点级提示。
...
Method: 提出PatchCue：基于块的视觉提示范式。将图像分割为块，在块级别表示视觉提示。采用两阶段训练方法：1）冷启动监督微调，使模型输出块级提示；2）强化学习，使用过程监督的提示奖励来指导中间视觉推理步骤。

Result: 在多个VLM和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue能持续提升整体模型性能。块级提示在性能上优于像素级边界框和点级提示。

Conclusion: PatchCue提供了一种更有效且认知对齐的视觉推理范式，通过块级视觉提示显著增强了视觉语言模型的视觉推理能力，为多模态理解任务提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 合成新视角的生成对机器人导航有积极影响：地面机器人拍摄的场景可生成鸟瞰图引导空中机器人；空中视角可生成地面视角帮助无人机识别地面机器人观察过的地点。需要系统评估合成新视角在视频地点识别中的效果。

Method: 使用5个公开VPR图像数据库和7种典型图像相似性方法，对合成新视角在VPR中进行系统评估。分析不同规模合成视角添加对识别效果的影响。

Result: 对于小规模合成视角添加，新视角能改善VPR识别统计效果。对于大规模添加，视角变化幅度的重要性不如添加视角数量和数据集图像类型重要。

Conclusion: 合成新视角在VPR中具有实用价值，特别是在小规模添加时能提升识别性能。实际应用中应更关注视角添加数量和数据集特性，而非单纯追求视角变化幅度。
...
TL;DR: 提出CORE-Seg框架，首个用于推理驱动复杂病灶分割的多样化思维链基准ComLesion-14K，通过语义引导提示适配器集成推理与分割，采用渐进训练策略和自适应双粒度奖励机制，在病灶分割任务上取得SOTA结果。
...
Method: 提出CORE-Seg端到端框架：1) 构建ComLesion-14K基准，首个多样化思维链基准用于推理驱动复杂病灶分割；2) 通过语义引导提示适配器集成推理与分割；3) 设计从SFT到GRPO的渐进训练策略；4) 采用自适应双粒度奖励机制缓解奖励稀疏问题。

Result: 方法取得SOTA结果：平均Dice系数37.06%（比第二名基线高14.89%），失败率降至18.42%。

Conclusion: CORE-Seg框架成功将推理与分割集成，通过语义引导提示适配器和渐进训练策略，在复杂病灶分割任务上显著优于现有方法，为医学图像分割的认知推理分析提供了有效解决方案。
...
TL;DR: CylinderSplat：一种用于全景3D高斯溅射的前馈框架，通过圆柱形Triplane表示和双分支架构解决稀疏视图下的遮挡问题，在单视图和多视图全景新视角合成中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯溅射方法在全景图像处理中存在挑战：多视图成本体积在稀疏视图场景下难以解决遮挡问题；标准笛卡尔Triplane表示无法有效捕捉360°场景的固有几何结构，导致失真和混叠。
...
TL;DR: BlackMirror：一种用于黑盒场景下检测文本到图像模型中后门攻击的新框架，通过分析生成图像中部分语义模式的稳定操纵来检测后门，而非依赖图像级相似性。


<details>
  <summary>Details</summary>
Motivation: 现有后门检测方法通常基于图像级相似性分析，假设后门触发的生成图像具有强一致性。然而，这些方法难以应对新兴的后门攻击，其中后门生成的图像在视觉上可能呈现多样性。本文观察到：在各种后门攻击中，只有生成图像中的部分语义模式被稳定操纵，而其余内容保持多样或良性。

Method: BlackMirror包含两个组件：1) MirrorMatch：将视觉模式与相应指令对齐以检测语义偏差；2) MirrorVerify：评估这些偏差在不同提示下的稳定性，以区分真正的后门行为与良性响应。这是一个无需训练、即插即用的通用框架，可在模型即服务（MaaS）应用中部署。

Result: 综合实验表明，BlackMirror在广泛的攻击范围内实现了准确的检测。该框架能够有效应对视觉多样化的后门攻击，超越了传统基于图像相似性的方法。

Conclusion: BlackMirror通过分析部分语义模式的稳定操纵而非整体图像相似性，为黑盒文本到图像模型的后门检测提供了有效解决方案。该训练免费框架具有通用性，可轻松集成到MaaS应用中，为实际部署提供了实用工具。
...
TL;DR: PixARMesh是一种从单张RGB图像自回归重建完整3D室内场景网格的方法，通过统一模型联合预测物体布局和几何结构，生成可直接用于下游应用的高质量网格。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖隐式有向距离场和后续布局优化，缺乏统一的场景重建框架。PixARMesh旨在直接从单张图像生成完整、连贯且可直接使用的3D场景网格，避免复杂的后处理流程。

Method: 基于点云编码器，通过像素对齐的图像特征和跨注意力机制融入全局场景上下文，实现从单张图像的空间推理。采用自回归方式从统一的令牌流（包含上下文、姿态和网格）生成场景，产生紧凑的高保真几何网格。
...
TL;DR: RAC（Rectified Flow Auto Coder）是一种基于Rectified Flow的新型自编码器，替代传统VAE，通过多步解码、双向推理和可校正的直线路径，在降低41%参数和70%计算成本的同时，提升了重建和生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径通常为曲线且不可校正。RAC旨在通过Rectified Flow的直线路径特性，实现可校正的多步解码，同时通过双向推理减少参数，提升生成质量并降低计算成本。

Method: 1. 采用Rectified Flow的多步解码机制，在流时间步上应用解码器，实现直线且可校正的解码路径；2. 通过时间反转实现双向推理，解码器同时作为编码器使用，减少参数；3. 生成式解码方法允许在路径上校正潜在变量，缩小重建-生成差距。

Result: 实验表明，RAC在重建和生成质量上均超越当前最优的VAE模型，同时参数减少约41%，计算成本降低约70%。

Conclusion: RAC通过Rectified Flow的直线解码路径和双向推理机制，有效解决了传统VAE的重建-生成差距问题，在显著降低参数和计算成本的同时，提升了模型性能，为生成模型提供了新的架构思路。
...
TL;DR: 提出InnoAds-Composer单阶段框架，通过重要性分析和条件路由实现产品海报的三条件（主体、文字、风格）高效控制，解决多阶段方法的主体保真度低、文字不准确、风格不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段流程，难以同时控制主体、文字和风格，且存在主体保真度差、文字不准确、风格不一致的问题。需要开发能够高效实现三条件控制的单阶段框架。

Method: 提出InnoAds-Composer单阶段框架：1）通过重要性分析确定各层和时间步对条件的响应性，将每个条件仅路由到最敏感位置，缩短活动token序列；2）设计文本特征增强模块（TFEM），结合字形图像和字形裁剪特征提升中文文字渲染准确性；3）构建首个同时包含主体、文字和风格条件的高质量电商产品海报数据集。

Result: 实验表明InnoAds-Composer在电商产品海报生成任务上显著优于现有方法，且未明显增加推理延迟。构建的数据集为训练和评估提供了重要基准。

Conclusion: InnoAds-Composer通过重要性分析和条件路由实现了高效的三条件控制，解决了多阶段方法的问题，为产品海报生成提供了有效的单阶段解决方案。
...
TL;DR: 提出三种偏差缓解技术改进概念瓶颈模型的公平性：top-k概念过滤减少信息泄露、移除偏见概念、对抗性去偏，在公平性与性能权衡方面优于先前工作


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型理论上通过将图像映射到人类可解释概念来增强可解释性和公平性，但实际应用中存在概念泄露无关语义信息的问题，且在性别偏见减少方面效果有限

Method: 提出三种偏差缓解技术：1) 使用top-k概念过滤器减少信息泄露；2) 移除偏见概念；3) 对抗性去偏。这些方法旨在改进概念瓶颈模型的公平性

Result: 提出的去偏概念瓶颈模型在公平性与性能权衡方面优于先前工作，为公平且可解释的图像分类提供了重要进展

Conclusion: 通过三种偏差缓解技术改进的概念瓶颈模型在保持可解释性的同时显著提升了公平性，为实现公平且可解释的图像分类提供了有效途径
...
Method: 提出CollabOD轻量级协作检测框架，包含三个核心策略：1) 结构细节保留；2) 跨路径特征对齐；3) 定位感知的轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构，并采用统一的细节感知检测头

Result: 该框架在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头在不引入额外部署开销的情况下提高了回归鲁棒性，代码已开源

Conclusion: CollabOD通过协同优化结构细节保留、特征对齐和轻量化设计，有效解决了无人机图像中小目标检测的关键挑战，在保持计算效率的同时提升了检测性能和鲁棒性
...
TL;DR: 提出Art3D框架，从几何精确的视差估计转向艺术连贯的视差合成，通过双路径架构分离全局深度参数与局部艺术效果，学习专业3D电影数据，初步实现影视级3D转换


<details>
  <summary>Details</summary>
Motivation: 当前2D转3D方法虽几何准确但艺术性不足，无法复现专业3D电影的沉浸感和情感共鸣体验。传统几何重建范式将艺术意图（如零平面偏移、局部深度塑造）误认为数据噪声或模糊性，需要新的艺术驱动范式

Method: 提出Art3D框架，采用艺术视差合成新范式。使用双路径架构解耦全局深度参数（宏观意图）与局部艺术效果（视觉笔触），通过间接监督从专业3D电影数据中学习。引入初步的影视对齐量化评估方法

Result: 实验表明Art3D在复现关键局部出屏效果和与影视3D内容的全局深度风格对齐方面展现出潜力，为艺术驱动转换工具的新类别奠定基础

Conclusion: 从物理精确的视差估计转向艺术连贯的视差合成是必要的，Art3D作为初步框架展示了这一新范式的可行性，为开发艺术驱动的3D转换工具提供了基础
...
TL;DR: Pano3DComposer：一种高效的端到端全景图像到3D场景生成框架，通过解耦对象生成与布局估计，使用Object-World Transformation Predictor将局部坐标对象转换到世界坐标，并引入Coarse-to-Fine对齐机制提升几何一致性，可在约20秒内生成高保真3D场景。


<details>
  <summary>Details</summary>
Motivation: 当前组合式图像到3D场景生成方法存在两个主要问题：1）依赖耗时迭代布局优化或不灵活的联合对象-布局生成；2）大多基于有限视场角透视图像，难以创建完整的360度环境。需要一种高效且能处理全景图像的解决方案。

Method: 提出Pano3DComposer框架，核心包括：1）Object-World Transformation Predictor：将现成图像到3D模型生成的3D对象从局部坐标转换到世界坐标，基于Alignment-VGGT架构，使用目标对象裁剪、多视角对象渲染和相机参数预测变换；2）伪几何监督训练解决生成对象与真实对象间的形状差异；3）Coarse-to-Fine对齐机制：针对未见域输入图像，通过场景渲染反馈迭代优化几何一致性。

Result: 在合成和真实世界数据集上，该方法在图像/文本到3D任务中实现了优越的几何精度，能够在RTX 4090 GPU上约20秒内生成高保真3D场景，显著提升了生成效率和质量。

Conclusion: Pano3DComposer通过解耦对象生成与布局估计，结合Object-World Transformation Predictor和Coarse-to-Fine对齐机制，提供了一种高效、准确的端到端全景图像到3D场景生成解决方案，克服了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大规模预训练视觉模型难以直接应用于3D骨架数据，主要因为数据格式差异、大规模骨架数据集稀缺，以及需要在不增加额外模型分支的情况下将骨架数据融入多模态动作识别。

Method: 提出Skeleton-to-Image Encoding (S2I)，通过基于身体部位语义划分和排列关节，将骨架序列转换为图像格式，并调整到标准图像尺寸，从而能够利用预训练视觉模型进行自监督骨架表示学习。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上的实验表明，该方法在自监督骨架表示学习方面具有有效性和泛化能力，特别是在跨格式评估的挑战性场景下表现优异。

Conclusion: S2I为骨架数据提供了统一的图像格式表示，首次实现了预训练视觉模型在骨架分析中的应用，有效解决了骨架数据异质性问题，为多模态动作识别提供了新思路。
...
TL;DR: ProCap将变化描述从静态图像对比重构为动态过程建模，通过两阶段框架学习变化过程的潜在动态表示，实现更准确的变化描述生成。
...
Conclusion: ProCap通过将变化建模从静态图像比较重构为动态过程建模，显著提升了变化描述的质量，能够更好地捕捉和理解变化的时间动态。
...
TL;DR: 该报告探讨了巴基斯坦制造的手术器械及其制造缺陷，通过深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）开发自动缺陷检测方法，使用4,414张高分辨率图像数据集进行分析，旨在提升制造质量标准和患者安全。


<details>
  <summary>Details</summary>
Motivation: 现代医疗环境中，手术器械的质量标准对临床成功至关重要。即使是微小缺陷也可能导致致命后果，因此制造精度极为重要。该报告旨在通过自动缺陷检测技术识别和纠正裂纹、锈蚀、结构不规则等关键缺陷，防止制造商经济损失并保障患者生命安全。

Method: 1. 与巴基斯坦Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作，获取行业洞察；2. 开发包含4,414张高分辨率图像的新数据集；3. 集成深度学习架构包括YOLOv8、ResNet-152和EfficientNet-b4进行自动缺陷检测；4. 采用自动光学检测（AOI）工具进行质量保证。

Result: 报告提供了对巴基斯坦制造手术器械及其相关缺陷的全面分析，开发了基于深度学习的新型自动缺陷检测方法。通过与行业领先企业的合作，获得了对缺陷分析的实际见解，为制造商、医疗专业人员和监管机构提供了有价值的资源。

Conclusion: 通过整合行业专业知识和尖端技术，特别是深度学习架构的自动缺陷检测方法，能够显著提升手术器械制造的质量标准。这种质量保证方法有助于创造更可靠的医疗环境，保障患者安全并减少制造商的经济损失。
...
Method: 提出两阶段编码机制：1)跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据，结合MLLMs捕获复杂时间模式和上下文理解；2)ISTS编码从历史观测中提取互补的时序特征，包括多视图嵌入融合和时间-变量编码器；3)自适应查询特征提取器压缩MLLMs学习到的token；4)带模态感知门控的多模态对齐模块缓解模态差异

Result: 在真实数据上的广泛实验验证了所提解决方案的有效性

Conclusion: MM-ISTS通过融合视觉、文本和时间序列模态，有效解决了不规则采样时间序列预测中上下文语义学习和细粒度时间模式捕获的问题
...
TL;DR: RAID是一个用于驾驶员风险感知研究的大规模数据集，包含4,691个标注视频片段，涵盖多样交通场景。作者提出了一个弱监督风险物体识别框架，通过建模驾驶员意图与响应关系来识别潜在风险源，并在RAID和HDDS数据集上分别取得了20.6%和23.1%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞移动性是智能车辆系统的关键目标，这需要理解驾驶员的风险感知——这是一个复杂的认知过程，受驾驶员对外部刺激的自愿响应和周围道路使用者对自车的关注度共同影响。当前缺乏专门用于驾驶员风险感知和上下文风险评估的大规模数据集。

Method: 1. 引入RAID数据集：包含4,691个标注视频片段，涵盖多样交通场景，标注内容包括驾驶员意图操作、道路拓扑、风险情况（如横穿行人）、驾驶员响应和行人关注度。2. 提出弱监督风险物体识别框架：通过建模驾驶员意图操作与响应之间的关系来识别潜在风险源。3. 分析行人关注度在风险评估中的作用。

Result: 提出的方法在RAID数据集上比现有最先进方法性能提升20.6%，在HDDS数据集上提升23.1%。实验评估证明了该方法的有效性，并展示了所提数据集的价值。

Conclusion: RAID数据集为驾驶员风险感知研究提供了重要资源，提出的弱监督风险物体识别框架通过建模驾驶员意图与响应关系有效识别风险源，行人关注度分析进一步丰富了风险评估维度。该方法在两个数据集上的显著性能提升验证了其有效性。
...
TL;DR: RepKAN是一种新颖的遥感图像分类架构，结合CNN的结构效率和KAN的非线性表示能力，通过双路径设计实现物理可解释性，在EuroSAT和NWPU-RESISC45数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 标准CNN和Transformer在遥感图像分类中通常作为不可解释的黑盒模型，缺乏物理可解释性。需要一种既能保持高性能又能提供明确物理解释的架构。
...
Conclusion: RepKAN具有作为未来可解释视觉基础模型骨干架构的显著潜力，为遥感图像分类提供了高性能且物理可解释的解决方案。
...
### [40] [Beyond Static Frames: Temporal Aggregate-and-Restore Vision Transformer for Human Pose Estimation](https://arxiv.org/abs/2603.05929)
*Hongwei Fang,Jiahang Cai,Xun Wang,Wenwu Yang*

Main category: cs.CV

TL;DR: TAR-ViTPose：一种用于视频2D人体姿态估计的时序聚合与恢复视觉Transformer，通过联合中心时序聚合和全局恢复注意力机制提升姿态估计的稳定性和准确性。
...
Result: 在PoseTrack2017基准测试上相比单帧基线ViTPose提升+2.3 mAP，超越现有最先进的视频姿态估计方法，同时在真实应用中实现更高的运行时帧率。

Conclusion: TAR-ViTPose通过有效的时序信息聚合和恢复机制，显著提升了视频姿态估计的鲁棒性和准确性，为视频理解任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经辐射场和3D高斯泼溅方法依赖耗时的逐场景优化，限制了实时部署；而新兴的前馈高斯泼溅方法缺乏显式流形几何结构，无法直接用于仿真。需要一种既能高效重建又具备显式几何的方法。

Method: 提出前馈三角形基元生成框架，包含像素对齐的三角形生成模块，并引入相对3D点云监督以增强几何学习的稳定性和一致性。直接从标定多视角图像预测连续三角形表面。

Result: 方法实现了高效重建，同时保持与标准图形和机器人仿真器的无缝兼容性。实验验证了其有效性。

Conclusion: 该方法通过单次前向传播生成仿真就绪模型，解决了现有方法依赖逐场景优化和缺乏显式几何的问题，为机器人和仿真应用提供了高效的3D重建解决方案。
...
TL;DR: 该研究通过分析视觉语言模型（VLMs）的中间激活，探究其在自动驾驶相关简单视觉任务中的失败原因，发现视觉概念在线性编码上的差异，并识别出感知失败和认知失败两种失败模式。


<details>
  <summary>Details</summary>
Motivation: VLMs在自动驾驶应用中日益普及，旨在利用其推理和泛化能力处理长尾场景。然而，这些模型经常在自动驾驶高度相关的简单视觉问题上失败，且失败原因尚不清楚。研究旨在通过分析模型中间激活来理解这些失败的根本原因。

Method: 创建仅针对特定视觉概念不同的反事实图像集，然后使用四种最先进VLM的激活训练线性探针来区分这些图像。通过分析中间激活，评估特定视觉概念在线性编码的程度，识别视觉信息流动的瓶颈。

Result: 结果显示：1）物体或智能体存在等概念被显式线性编码；2）物体或智能体方向等空间视觉概念仅通过视觉编码器保留的空间结构隐式编码；3）即使概念在线性编码，模型仍可能回答错误；4）识别出两种失败模式：感知失败（所需视觉信息未线性编码）和认知失败（视觉信息存在但未与语言语义正确对齐）；5）物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 该研究提高了对VLMs在自动驾驶相关简单视觉任务中失败案例的理解，揭示了视觉概念在线性编码上的差异，并识别了感知和认知两种失败模式。这些发现有助于改进VLMs在自动驾驶应用中的可靠性。
...
TL;DR: OD-RASE是一个通过检测导致交通事故的道路结构并将其与基础设施改进相连接来增强自动驾驶系统安全性的框架


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时仍有局限，当前道路基础设施是为人类驾驶员设计的，安全改进通常只在事故发生后进行，这种被动方法对需要主动风险缓解的自动驾驶系统构成重大挑战

Method: 1. 基于道路交通系统专业知识形式化本体论；2. 使用大规模视觉语言模型生成基础设施改进建议；3. 通过本体驱动数据过滤增强可靠性；4. 在事故前道路图像上自动标注改进建议构建新数据集；5. 提出OD-RASE模型，结合LVLM和扩散模型生成改进建议和改进后道路环境的生成图像

Result: 实验表明，本体驱动数据过滤能够高精度预测导致事故的道路结构和相应的改进计划
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在说话头生成中存在高推理延迟、时间不稳定（闪烁和身份漂移）以及困难语音条件下视听对齐不完美的问题，需要在实际计算受限环境中实现实用的扩散基说话头生成

Method: 采用参考条件的潜在扩散框架，通过教师-学生蒸馏：使用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器，显著减少推理步骤；结合身份锚定和时间正则化减少身份漂移和帧间闪烁；基于音素的音频条件提供粗粒度唇部运动控制

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强大教师的大部分重建行为，同时实现显著降低的延迟推理；包括CPU和边缘计算的延迟特性测量表明边缘部署可行性

Conclusion: 蒸馏扩散模型可以在保持教师模型重建能力的同时实现低延迟推理，这是迈向计算受限环境下实用扩散基说话头生成的初步步骤
...
<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法虽然能生成自然面部动作，但通常只能表达固定目标情感，缺乏人类在传达信息时表现出的连续变化自然表情能力。

Method: 提出TIE-TFG模型，采用时序密集情感波动建模技术，能够根据输入文本生成相应的情感变化序列，驱动合成视频中连续的面部表情变化。

Result: 广泛评估表明该方法在产生平滑情感过渡方面表现卓越，在不同情感状态下保持高质量视觉效果和运动真实性。

Conclusion: 成功实现了情感连续说话人脸生成新任务，提出的TIE-TFG模型能够有效管理动态情感变化，生成具有自然情感过渡的逼真说话视频。
...
<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统中的关键任务，直接影响后续处理步骤。随着数字化时代的发展，这一问题受到广泛关注，需要更鲁棒可靠的解决方案。

Method: 提出新颖的倾斜估计方法：在2D离散傅里叶幅度谱上应用自适应径向投影，提取文档图像的主导倾斜角度。同时创建高质量评估数据集DISE-2021。

Result: 所提方法表现出鲁棒性和可靠性，在所有对比方法中性能最优。提供了傅里叶基方法的全面改进分析。

Conclusion: 基于自适应径向投影的傅里叶方法在文档倾斜估计任务中具有优越性能，为实际应用提供了有效的解决方案。开源代码可供使用。
...
TL;DR: LucidNFT：基于多奖励强化学习的流匹配真实图像超分辨率框架，通过Lucidity一致性评估器、解耦优势归一化和大规模真实退化数据集解决语义幻觉和优化瓶颈问题


<details>
  <summary>Details</summary>
Motivation: 生成式真实图像超分辨率存在语义和结构幻觉问题，输出可能看起来清晰但与低分辨率证据不忠实。基于偏好的强化学习虽然适合，但面临三个主要挑战：缺乏退化鲁棒的低分辨率参考忠实度信号、rollout组优化瓶颈导致优势崩溃、以及真实退化覆盖有限限制rollout多样性

Method: 提出LucidNFT框架：1) LucidConsistency - 退化鲁棒的语义评估器，使低分辨率锚定的忠实度可测量和可优化；2) 解耦优势归一化策略 - 在融合前保留每个低分辨率条件rollout组内的目标间对比，防止优势崩溃；3) LucidLR - 大规模真实世界退化图像集合，支持鲁棒的强化学习微调

Result: 实验表明LucidNFT持续改进基于流的真实图像超分辨率基线，在多样真实世界场景中实现更好的感知-忠实度权衡，并保持稳定的优化动态

Conclusion: LucidNFT通过引入可测量的忠实度评估、防止优势崩溃的优化策略和高质量训练数据，有效解决了生成式真实图像超分辨率中的语义幻觉问题，实现了更好的感知质量与忠实度平衡
...
<details>
  <summary>Details</summary>
Motivation: 少样本分割面临结构错位和跨视角不一致的挑战，特别是在外观或视角变化较大的情况下。现有方法在处理视角变化和复杂结构时表现不佳，需要一种能够同时建模结构一致性和前景判别性的统一框架。

Method: VINE引入空间-视角图在骨干特征上，空间图捕捉局部几何拓扑，视角图连接不同视角的特征以传播视角不变的结构语义。从支持-查询特征差异中推导判别性先验来捕捉类别特定对比，重新加权SAM特征以强调显著区域并重新校准骨干激活以改善结构聚焦。前景增强的SAM特征和结构丰富的ResNet特征通过掩码交叉注意力逐步集成，生成类别一致的原型作为SAM解码器的自适应提示。
...
TL;DR: Place-it-R1是一个端到端视频对象插入框架，利用多模态大语言模型(MLLM)进行环境感知推理，通过Think-then-Place范式实现物理一致的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术主要关注视觉保真度而非物理因果关系，导致插入的对象与环境物理不一致。需要一种能够理解物理场景并进行环境感知推理的方法。

Method: 采用Think-then-Place范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化(DPO)，通过MLLM评分优化视觉自然度；3) 推理时MLLM迭代触发细化循环，形成闭环系统；4) 提供两种用户可选模式：增强物理合理性的灵活模式和保持场景完整性的标准模式。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，能够实现物理一致性的视频对象插入。

Conclusion: 该框架通过利用MLLM的环境感知推理能力，在物理合理性和视觉保真度之间提供了可控的权衡，实现了物理一致的视频对象插入。
...
<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制导致二次计算成本，只能处理短序列；因果注意力变体如StreamVGGT虽然支持流式处理，但KV缓存会持续增长，在数百帧内耗尽GPU内存，无法实现真正长序列的流式推理

Method: 提出OVGGT框架：1）自选择性缓存：利用FFN残差幅度压缩KV缓存，与FlashAttention完全兼容；2）动态锚点保护：保护坐标关键令牌不被驱逐，抑制长轨迹中的几何漂移

Result: 在室内、室外和超长序列基准测试中，OVGGT能够在恒定VRAM限制下处理任意长视频，同时达到最先进的3D几何精度

Conclusion: OVGGT通过训练无关的方法解决了流式视频3D重建中的内存和计算瓶颈，实现了真正长序列的实时处理能力
...
TL;DR: VLM-RobustBench是一个评估视觉语言模型在真实世界图像失真下鲁棒性的基准测试，涵盖49种增强类型，133种损坏设置。研究发现当前VLMs在语义上强大但在空间变换上脆弱，几何失真对性能影响最大。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在标准高质量数据集上表现良好，但它们在真实世界图像失真下的性能尚未被充分理解。需要系统评估VLMs对各种图像扰动的鲁棒性，以了解其实际应用中的局限性。

Method: 构建VLM-RobustBench基准测试，涵盖49种增强类型（噪声、模糊、天气、数字和几何扰动），在分级严重程度（低/中/高）和二元变换下评估，产生133种损坏设置。评估来自四个家族的VLMs（Qwen、InternVL、Molmo、Gemma），使用两个互补基准：MMBench（视觉基础）和MMMU-Pro（推理导向）。

Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动通常比视觉上严重的光度损坏更能降低性能。特别是低严重度的glass_blur使MMBench准确率平均下降约8个百分点，而最大的性能下降来自重采样和几何失真（如上采样、弹性变换），最高可达34个百分点。

Conclusion: 当前视觉语言模型在语义理解上强大但在空间变换上脆弱，需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。
...
TL;DR: 提出基于两阶段策略（目标分割+识别）的开放词汇目标识别框架，无需复杂重训练和人工标注，通过CLIP生成图像和文本嵌入，引入CNN/MLP方法增强编码灵活性，使用SVD构建共享表示空间，通过嵌入相似度匹配实现识别。
...
Method: 采用两阶段策略：1）目标分割；2）识别。裁剪目标区域后，使用CLIP生成目标级图像嵌入和类别级文本嵌入。为减少对CLIP的依赖，引入CNN/MLP方法提取CNN特征图，用MLP对齐视觉特征与文本嵌入。通过SVD构建共享表示空间，最后通过嵌入相似度匹配实现识别。

Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无训练、基于CLIP的编码（无SVD）实现了最高的平均AP，优于当前最先进方法。同时验证了CNN/MLP图像编码在OVOR中的潜力。

Conclusion: 提出的OVOR框架通过简化两阶段策略有效解决了现有方法的局限性，无需复杂训练和标注，在多个基准数据集上表现优异，为开放词汇目标识别提供了高效实用的解决方案。
...
### [53] [Reflective Flow Sampling Enhancement](https://arxiv.org/abs/2603.06165)
*Zikai Zhou,Muyao Wang,Shitong Shao,Lichen Bai,Haoyi Xiong,Bo Han,Zeke Xie*

Main category: cs.CV

TL;DR: 提出RF-Sampling，一种专为流匹配模型设计的无训练推理增强框架，通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，提升生成质量和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 现有推理时增强策略主要适用于传统扩散模型，在流模型（如FLUX）上表现不佳，需要专门为流模型设计有效的推理增强方法。

Method: 提出反射流采样（RF-Sampling），基于理论推导的框架，利用文本表示的线性组合并结合流反转，使模型能够探索与输入提示更一致的噪声空间。

Result: 在多个基准测试中，RF-Sampling一致提升了生成质量和提示对齐，是首个在FLUX上展示一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流匹配模型提供了有效的推理增强解决方案，通过理论推导和实验验证了其优越性，填补了流模型推理增强的技术空白。
...
TL;DR: 提出两阶段框架解决字形相似度学习难题：先在标注人造字母表上训练对比学习编码器作为教师模型，再通过师生蒸馏扩展到历史文字系统，实现监督对比学习与无监督发现的结合。


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统相似度度量学习面临根本挑战：人造字母表中的单个字素可以可靠标注，但不同文字系统之间的历史关系不确定且有争议。需要解决这一认识论约束。

Method: 两阶段框架：1) 在有标注的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；2) 通过师生蒸馏扩展到历史文字系统，学生模型在教师知识指导下学习无监督表示，同时自由发现潜在的跨文字相似性。

Result: 在多样化的文字系统上实验表明，该方法实现了有效的少样本字形识别和有意义的文字聚类，无需真实进化关系标注。非对称设置使学生能够学习变形不变嵌入，同时继承干净示例的判别结构。

Conclusion: 该框架桥接了监督对比学习和无监督发现，既能在不同系统间建立硬边界，又能反映潜在历史影响的软相似性，为解决文字系统相似度学习中的认识论约束提供了有效方案。
...
TL;DR: 提出CR-QAT框架，通过课程量化感知训练和文本中心关系知识蒸馏，解决开放词汇目标检测模型在极端低比特量化时视觉-语言对齐和区域关系结构退化的问题。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测（OVOD）通过视觉-语言对齐实现新类别检测，但大模型尺寸限制了在资源受限设备上的部署。量化提供实用压缩方案，但研究发现朴素极端低比特（如4位）量化会严重破坏细粒度视觉-语言对齐并扭曲区域间关系结构。

Method: 提出课程关系量化感知训练（CR-QAT）框架，包含两个核心组件：1）课程QAT（CQAT）：通过模型分区进行渐进量化，实现误差隔离和稳定优化；2）文本中心关系知识蒸馏（TRKD）：针对任务相关模块，构建文本锚定的成对相似性矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下持续优于现有QAT基线，分别实现了高达38.9%和40.9%的相对AP提升。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测模型在极端量化时的性能退化问题，通过课程式渐进量化和文本中心关系知识蒸馏的结合，在保持模型压缩效率的同时显著提升了量化模型的视觉-语言对齐能力和关系结构保持能力。
...
TL;DR: DeepSight是首个专门用于深度感知的多模态大语言模型，通过构建深度图像-文本对数据集和深度指令数据集，改进CLIP的ViT编码器以更好地捕捉深度连续变化，显著提升三维场景理解和空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在图像描述和视觉问答等任务上表现优异，但难以准确解释视觉数据中的深度信息。深度图像（单通道灰度图）的像素值直接反映深度线索，具有独特特性，可用于改进空间推理。然而，现有方法面临深度数据有限、简单通道复制不足等挑战。

Method: 1. 构建新颖的深度图像-文本对数据集和深度指令数据集：使用GLPN模型从视觉图像生成深度图，利用GPT-4策划相应的深度指令，并通过LLaVA验证。2. 改进CLIP的ViT编码器：融入局部物体信息，更有效地捕捉深度细微连续变化。3. 建立全面的深度问答基准：基于现有深度图像数据集开发，严格评估典型深度图场景的理解能力。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专用深度MLLM，通过创新的数据集构建和模型架构改进，有效解决了现有MLLM在深度信息解释方面的不足，为三维场景理解提供了重要技术突破。
...
TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准数据集，专注于动态场景下的挑战，包含剧烈自运动和复杂表观轨迹，包含42个视频序列和170万+边界框标注。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角MOT基准缺乏真实世界中的动态复杂性，通常只包含可预测的相机运动和线性运动模式，无法反映无人机快速移动和敏捷机动带来的观测挑战。

Method: 构建了DynUAV基准数据集，包含42个视频序列，超过170万个边界框标注，覆盖车辆、行人以及挖掘机、推土机、起重机等工业类别。数据集特别强调剧烈自运动导致的复杂表观轨迹。

Result: 与现有基准相比，DynUAV引入了由自运动带来的显著挑战，包括剧烈的尺度变化、视角变化和运动模糊。对最先进跟踪器的综合评估显示它们在处理这种动态条件下的检测和关联交织挑战时存在局限性。

Conclusion: DynUAV建立了一个严格的基准，可作为推动真实世界无人机视角MOT进展的苛刻测试平台。所有资源将公开提供，以促进该领域的研究发展。
...
TL;DR: CoE是一个无需训练的多模态摘要框架，通过事件链和分层事件图进行结构化推理，解决了现有方法在领域特定监督、跨模态对齐和时序建模方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：1) 依赖领域特定监督；2) 隐式融合导致跨模态对齐弱；3) 平面时序建模缺乏事件转换。需要一种更鲁棒、可解释且能跨领域泛化的方法。

Method: 提出CoE框架，基于分层事件图(HEG)进行结构化推理。HEG将文本语义编码为显式的事件层次结构，支撑跨模态对齐和时序推理。CoE通过事件链引导关键视觉线索定位、建模事件演化和因果转换，并通过轻量级风格适配进行领域对齐。

Result: 在八个多样化数据集上的实验表明，CoE持续优于最先进的视频CoT基线，平均提升：ROUGE +3.04、CIDEr +9.51、BERTScore +1.88，展现了其鲁棒性、可解释性和跨领域泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法通常假设固定长度的历史观测，但在真实场景中（如遮挡或感知范围限制）观测长度可变且可能极短，导致性能大幅下降。需要一种能够自适应不同观测长度的统一预测框架。

Method: 提出TaPD框架，包含两个协同模块：观测自适应预测器（OAF）和时序回填模块（TBM）。OAF基于渐进式知识蒸馏（PKD），通过层次特征回归将长时观测的"教师"知识迁移到短时观测的"学生"模型中。引入余弦退火蒸馏权重方案平衡预测监督和特征对齐。TBM针对极短历史，基于场景演化回填缺失的历史片段。采用解耦的预训练-重构-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的大量实验表明，TaPD在所有观测长度下均优于强基线方法，在极短输入条件下提升尤其显著，并能以即插即用方式提升其他预测器（如HiVT）的性能。

Conclusion: TaPD通过渐进式知识蒸馏和时序回填的统一框架，有效解决了可变历史长度下的轨迹预测问题，显著提升了短观测条件下的预测性能，具有良好的通用性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 从可见图像估计人类注视目标对机器人理解人类注意力至关重要，但现有方法在泛化性神经架构和训练范式方面仍面临挑战。预训练视觉基础模型为定位注视目标提供了有前景的途径，但多模态线索（眼睛、头部姿态、手势、上下文特征）的整合需要自适应且高效的解码机制。

Method: 提出GazeMoE端到端框架，受大型视觉语言模型中混合专家（MoE）自适应领域专业知识的启发，通过MoE模块从冻结的基础模型中选择性利用注视目标相关线索。为处理注视目标分类（框内vs框外）的类别不平衡并增强鲁棒性，结合了类别平衡辅助损失和策略性数据增强（包括区域特定裁剪和光度变换）。

Result: 在基准数据集上的广泛实验表明，GazeMoE实现了最先进的性能，在具有挑战性的注视估计任务上优于现有方法。

Conclusion: GazeMoE通过自适应选择多模态线索和有效处理类别不平衡，为注视目标估计提供了有效的解决方案，代码和预训练模型已开源。
...
TL;DR: RePer-360：一种面向单目全景深度估计的失真感知自调制框架，通过几何对齐引导模块和自条件AdaLN-Zero机制，在保留预训练透视先验的同时适应全景域，仅需1%训练数据即可超越标准微调方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度基础模型在透视图像上训练表现良好，但在360°全景图像上泛化能力差，主要原因是透视域与全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，成本高昂。

Method: 提出RePer-360框架：1）设计轻量级几何对齐引导模块，从ERP和CP两种互补投影中提取调制信号，引导模型适应全景域而不覆盖预训练透视知识；2）引入自条件AdaLN-Zero机制，生成像素级缩放因子以减少透视与全景域间的特征分布差距；3）使用立方体贴图域一致性损失提升训练稳定性和跨投影对齐。

Result: RePer-360仅使用1%训练数据即可超越标准微调方法。在相同域内训练设置下，RMSE指标提升约20%。该方法在保留预训练透视先验的同时有效适应全景域。
...
TL;DR: HiPP-Prune：分层偏好条件结构化剪枝框架，用于视觉语言模型，通过策略生成全局剪枝蓝图，平衡任务效用、幻觉鲁棒性和压缩率


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型剪枝面临挑战：压缩不仅影响任务效用，还会加剧物体幻觉问题，即使在相同稀疏度下也会出现不同的视觉定位能力退化

Method: 分层偏好条件结构化剪枝框架，将剪枝视为多目标下的条件资源分配。通过单一策略调用生成全局剪枝蓝图，决策分解为总体稀疏度预算和层间分配。引入视觉敏感度信号（基于视觉token与语言隐藏状态间的注意力流），防止过度剪枝视觉关键层。使用计划级组相对策略优化（GRPO）优化剪枝计划

Result: 在LLaVA模型上的POPE和ScienceQA实验表明，HiPP-Prune能够发现多样化的非支配剪枝计划，在匹配稀疏度预算下提供可控的鲁棒性-效用权衡

Conclusion: HiPP-Prune框架成功解决了VLM剪枝中的多目标权衡问题，通过分层决策和视觉敏感度集成，实现了在保持任务效用的同时控制幻觉风险的可控剪枝
...
TL;DR: DEX-AR是一种针对自回归视觉语言模型的新型可解释性方法，通过动态头过滤和序列级过滤机制，生成逐令牌和序列级别的2D热图来解释模型决策过程。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法主要针对分类任务设计，难以处理现代自回归VLM的复杂逐令牌生成过程以及视觉和文本模态之间的复杂交互。

Method: DEX-AR通过计算逐令牌生成过程中注意力图的层梯度来解释自回归VLM。该方法包含两个关键创新：1) 动态头过滤机制，识别专注于视觉信息的注意力头；2) 序列级过滤方法，聚合逐令牌解释同时区分视觉基础和纯语言令牌。

Result: 在ImageNet、VQAv2和PascalVOC上的评估显示，DEX-AR在基于扰动和基于分割的指标上都表现出持续改进，特别是在使用新的归一化困惑度度量时。

Conclusion: DEX-AR为自回归视觉语言模型提供了一种有效的可解释性方法，能够处理复杂的多模态交互和逐令牌生成过程，为理解这些先进模型的决策机制提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉特效(VFX)制作需要专业知识和高成本生产流程，现有AIGC系统面临特效数据稀缺、超自然/风格化效果建模困难等挑战，且通常需要逐特效微调，限制了系统的可扩展性和泛化能力。

Method: EffectMaker采用语义-视觉双路径引导机制：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索。同时构建了包含130k视频、3k特效类别的EffectData数据集。

Result: 实验表明EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了可扩展且灵活的自定义VFX生成范式，无需逐特效微调即可实现准确、可控、特效一致的合成。

Conclusion: EffectMaker通过统一的推理-生成框架解决了VFX生成中的数据稀缺和泛化问题，其语义-视觉双路径引导机制和大型合成数据集为定制化视觉特效生成提供了可扩展的解决方案。
...
TL;DR: K-MaT是一种无需低端模态训练图像的提示学习框架，通过知识锚定流形传输将决策结构从高端医学影像模态迁移到低端模态，在跨模态基准测试中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有大规模生物医学视觉语言模型在高端影像模态（如CT）上适应良好，但迁移到前线低端模态（如X光）时容易陷入模态特定捷径，导致性能崩溃。需要一种无需低端训练图像的跨模态迁移方法。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间；3）实现零样本跨模态部署。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）中取得SOTA：平均准确率调和均值44.1%（BiomedCoOp为42.0%），宏F1分数36.2%。在乳腺成像任务中有效缓解灾难性遗忘（CoOp在低端模态准确率降至27.0%）。

Conclusion: 通过最优传输对齐提示流形为医学视觉语言模型的零样本跨模态部署提供了高效途径，能有效迁移决策结构并保持跨模态鲁棒性能。
...
TL;DR: 提出MOSIV框架，用于从视频中进行多物体系统识别，直接优化连续、每物体的材料参数，通过可微分模拟器和几何目标指导，在接触丰富的多物体交互场景中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单物体场景或使用固定材料原型集的离散材料分类，无法有效处理多物体系统识别问题，需要新的框架来处理连续、每物体的材料参数优化。

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续、每物体的材料参数，通过从视频中提取的几何目标进行指导，并创建了包含接触丰富的多物体交互的合成基准数据集。

Result: 在新建的基准测试中，MOSIV相比适应后的基线方法，在接地准确性和长时程模拟保真度方面都有显著提升，成为该新任务的强基线方法。

Conclusion: 物体级别的细粒度监督和几何对齐的目标对于复杂多物体设置中的稳定优化至关重要，MOSIV为多物体系统识别任务建立了有效的基准框架。
...
TL;DR: StruVis是一个通过结构化视觉表示增强文本到图像生成推理的新框架，避免中间图像生成的计算成本，同时提供视觉结构感知。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成的推理框架存在两个主要问题：纯文本推理缺乏视觉上下文，容易忽略空间和视觉元素；文本-图像交错推理虽然提供视觉参考但计算成本高，且受限于生成器的表示能力。需要一种既能感知视觉结构又计算高效的推理方法。

Method: 提出StruVis框架，使用基于文本的结构化视觉表示作为中间推理状态，让多模态大语言模型在纯文本推理过程中有效"感知"视觉结构。通过结构化视觉引导的推理解锁模型的推理潜力，且该框架与生成器无关，可无缝集成各种文本到图像生成器。

Result: 在推理型文本到图像生成基准测试中取得显著性能提升：在T2I-ReasonBench上获得4.61%增益，在WISE上获得4%增益。

Conclusion: StruVis通过结构化视觉表示有效解决了现有推理框架的局限性，既保持了计算效率又提供了视觉结构感知能力，可广泛应用于各种文本到图像生成器。
...
Method: 将提示敏感性重新表述为组间一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，提出包含质量引导的组正则化和带停止梯度策略的logit级一致性约束的训练框架
...
TL;DR: OA-SORT是一个即插即用、无需训练的遮挡感知多目标跟踪框架，通过OAM、OAO和BAM三个模块解决2D MOT中因部分遮挡导致的定位成本混淆问题，在多个数据集上显著提升跟踪性能。
...
Method: 提出OA-SORT框架，包含三个核心模块：1) OAM分析物体遮挡状态，引入高斯图减少背景影响；2) OAO利用遮挡状态缓解成本混淆；3) BAM利用遮挡状态抑制估计不稳定性。框架为即插即用且无需训练。

Result: 在DanceTrack测试集上达到63.1% HOTA和64.2% IDF1；将框架集成到其他四个跟踪器后，HOTA和IDF1平均提升2.08%和3.05%，验证了遮挡感知的可重用性。

Conclusion: 遮挡处理对MOT至关重要，OA-SORT通过创新的遮挡感知机制有效解决了定位成本混淆问题，框架具有良好的通用性和可重用性，能显著提升多种跟踪器的性能。
...
TL;DR: PSIVG框架通过将物理模拟器集成到视频扩散过程中，生成物理一致的视频，解决了现有扩散模型违反物理定律的问题
...
Conclusion: PSIVG通过物理模拟器与视频扩散模型的协同，显著提高了生成视频的物理一致性，为物理感知的视频生成提供了新思路
...
TL;DR: 该论文提出使用分层子采样处理VGG16超列，并研究集成学习在稀疏超列上的性能，在脑肿瘤数据集上显著提升了小样本情况下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 超列（多尺度特征向量）在图像分割中具有潜力，但实际应用较少，主要原因是处理密集超列的计算复杂度随训练集大小线性增长。需要解决这一计算瓶颈，特别是在小样本情况下。

Method: 采用分层子采样处理VGG16生成的超列，减少计算复杂度。研究集成学习方法（堆叠和投票集成）在稀疏超列上的性能，并与简单逻辑回归分类器进行比较。

Result: 在脑肿瘤数据集上，当N≤20的极端小样本情况下，简单逻辑回归分类器表现最佳。在10%分层子采样率下，N=20时最佳平均Dice得分为0.66，相比标准多尺度UNet基线有24.53%的显著提升（p=3.07e-11）。

Conclusion: 分层子采样能有效降低超列计算复杂度，在小样本情况下，简单分类器（如逻辑回归）比复杂集成方法更有效，能显著改善分割性能并缓解过拟合问题。
...
TL;DR: CLoPA是一种持续适应策略，通过微调SAM模型的小部分参数来提升医学图像分割性能，无需改变推理流程，在八个医学分割任务中快速达到专家水平。


<details>
  <summary>Details</summary>
Motivation: 现有零样本分割模型（如SAM）在多样化医学成像任务中无法稳定达到专家级性能，而标注活动产生的任务特定标注数据流为在线模型适应提供了自然补充机会。

Method: 提出CLoPA持续适应策略：1）仅微调SAM模型的小部分参数；2）基于标注缓存进行训练；3）通过轻量级片段调度触发；4）无需新参数或改变推理流程；5）完全在现有标注工作流内操作。

Result: 在八个Medical Segmentation Decathlon任务中，CLoPA快速将性能提升至专家水平，即使SAM先前失败的任务也能成功，多数增益在单个训练片段后实现。不同参数组调优效果取决于任务特性和数据规模，复杂几何目标需要更深层特征对齐。

Conclusion: CLoPA通过持续适应策略有效弥补了零样本分割模型在医学图像任务中的性能差距，提供了一种实用且高效的在线模型优化方法，特别适用于具有复杂几何特征的挑战性分割任务。
...
TL;DR: 提出FontUse数据集和标注流程，通过数据驱动方法改进文本到图像模型在排版控制方面的能力，使模型能更准确地根据提示生成符合要求的字体样式


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在排版控制方面存在局限性，用户请求的排版外观经常被忽略或仅被弱遵循，需要专门的数据驱动方法来改进字体样式生成能力

Method: 构建大规模排版数据集FontUse（约7万张图像），使用分割模型和多模态大语言模型自动标注用户友好提示、文本区域位置和OCR识别字符串；通过微调现有生成器，使其能够一致地解释样式和使用场景条件

Result: 实验表明，使用该流程训练的模型在多样化提示和布局下，生成的文本渲染比竞争基线更符合提示要求；提出了基于Long-CLIP的评估指标来衡量生成排版与请求属性的一致性

Conclusion: 通过数据中心的标注流程构建排版数据集，能够显著提升文本到图像模型在字体样式控制方面的能力，使模型能够更准确地根据用户提示生成符合要求的排版效果
...
TL;DR: 提出GvU方法，通过统一多模态模型内部的理解能力来增强生成质量，使用token级文本-图像对齐奖励机制，让模型同时作为教师和学生，通过自监督强化学习框架迭代提升生成能力。
...
Method: 提出token级内在文本-图像对齐奖励机制GvU，使统一多模态模型同时作为教师和学生：使用理解分支评估自身输出来指导生成。基于此设计自监督强化学习框架，让模型通过基于理解的内在奖励信号迭代提升生成质量，无需外部监督。
...
TL;DR: GenHOI：一种轻量级增强方法，通过时间平衡和空间选择性机制向预训练视频生成模型注入参考对象信息，显著提升手-物体交互视频合成的物理合理性和对象一致性。


<details>
  <summary>Details</summary>
Motivation: 手-物体交互（HOI）是数字人类视频合成的核心挑战，现有方法存在局限性：HOI重演方法在域内训练和评估，难以泛化到复杂野外场景；一体化视频编辑模型虽具更广泛鲁棒性，但仍存在对象外观不一致等HOI特定问题。

Method: 提出GenHOI框架，包含两个核心组件：1）时间平衡机制：提出Head-Sliding RoPE，为参考令牌分配头部特定的时间偏移，均匀分布其影响跨帧，减轻3D RoPE的时间衰减，改善长距离对象一致性；2）空间选择性机制：设计两级空间注意力门，将对象条件注意力集中在HOI区域并自适应缩放其强度，在保持背景真实性的同时增强交互保真度。

Result: 在未见过的野外场景上进行广泛定性和定量评估，GenHOI在物理合理性、对象一致性和交互保真度方面显著优于最先进的HOI重演方法和一体化视频编辑方法。

Conclusion: GenHOI通过轻量级增强预训练视频生成模型，有效解决了HOI视频合成中的关键挑战，实现了在复杂野外场景中生成物理合理且对象一致的手-物体交互视频。
...
TL;DR: Curious-VLA框架通过两阶段设计解决VLA模型中的探索-利用困境：IL阶段使用可行轨迹扩展和归一化轨迹表示，RL阶段采用自适应多样性感知采样和跨度驾驶奖励，在Navsim基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 识别出自主VLA模型存在窄策略限制问题：模仿学习倾向于限制探索，导致后续强化学习阶段因反馈多样性不足而过早饱和，限制了模型性能潜力。

Method: 提出Curious-VLA两阶段框架：IL阶段引入可行轨迹扩展策略生成多个物理有效轨迹，并使用步进归一化轨迹表示适应多样化数据；RL阶段采用自适应多样性感知采样优先选择高多样性样本，并引入跨度驾驶奖励配合焦点风格加权来扩大奖励值跨度，提高对驾驶质量的敏感性。

Result: 在Navsim基准测试中取得最先进结果：PDMS 90.3，EPDMS 85.4，Best-of-N PDMS达到94.8，证明了该框架在释放VLA模型探索潜力方面的有效性。

Conclusion: Curious-VLA通过解决探索-利用困境，成功提升了VLA模型的性能，代码已开源供进一步研究使用。
...
TL;DR: AI系统通过45,139张超声图像训练，诊断胎儿口面裂的敏感性和特异性超过93%和95%，匹配资深放射科医生水平，显著优于初级医生，并能提升初级医生6%的敏感性。
...
Method: 开发了一个人工智能系统，在来自22家医院的9,215名胎儿的45,139张超声图像上进行训练，用于诊断胎儿口面裂。系统作为医疗副驾驶使用，并通过涉及24名放射科医生和实习生的试点研究评估其对专业知识发展的影响。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生表现相当，显著优于初级放射科医生。作为医疗副驾驶使用时，能将初级放射科医生的敏感性提升6%以上。试点研究表明，该模型能够改善罕见病症的专业知识发展。
...
TL;DR: SUREON是一个从外科教学视频中提取专家推理注释的大规模视频问答数据集，用于训练具备外科推理能力的人工智能模型。


<details>
  <summary>Details</summary>
Motivation: 当前外科AI缺乏理解手术意图、决策理性和风险预测等高级推理能力，主要原因是难以大规模标注包含专家推理的训练数据。外科教学视频中的专家讲解恰好包含了这些推理信息。

Method: 通过多智能体管道从外科学术视频中系统性地提取和结构化监督信号，定义了12个问题类别（包括安全评估、决策理性和预测等），创建了包含20.68万问答对的数据集。提出了SureonVLM（监督微调）和SureonVLM-R1（使用Group Relative Policy Optimization训练）两种模型。

Result: SUREON数据集包含13.47万个视频片段、170种手术类型和20.68万问答对，并提供了354个专家验证的基准测试。两种模型在SUREON基准测试中准确率超过84%，在标准外科感知任务上优于通用领域模型，SureonVLM-R1展现出从视觉上下文推断手术意图等显式推理行为。

Conclusion: 从外科教学视频中提取的监督信号能够有效训练具备外科推理能力的AI模型，填补了当前外科AI在理解手术意图和决策理性方面的空白，为开发更智能的外科辅助系统提供了新途径。
...
Conclusion: BEVLM成功桥接了BEV的空间一致性与LLMs的语义理解能力，显著提升了自动驾驶系统的3D空间推理和决策性能
...
TL;DR: 提出FedARKS框架解决FedDG-ReID中全局特征局限和均匀聚合问题，通过RK机制提取鲁棒局部特征，KS机制选择高质量客户端知识


<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征和简单平均聚合，难以捕捉域不变局部细节，且忽略客户端在鲁棒特征提取能力上的差异，稀释高质量客户端贡献

Method: 提出FedARKS框架，包含RK（鲁棒知识）机制提取域不变局部特征，KS（知识选择）机制根据客户端特征提取能力差异进行选择性聚合

Result: 未在摘要中提供具体实验结果，但方法旨在提升模型在未见域的泛化能力，同时保护客户端数据隐私

Conclusion: FedARKS通过鲁棒局部特征提取和差异化知识选择机制，解决了现有联邦域泛化行人重识别方法的局限性，有望提升模型泛化性能
...
TL;DR: 论文研究了视觉语言模型在结构化色彩失真下的系统性感知弱点，发现即使场景对人类仍可识别，模型也会产生自信但荒谬的预测，而人类表现显著优于模型。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在基准测试中表现良好，但它们存在系统性感知弱点：像素值的结构化大幅变化可能导致模型产生自信但荒谬的预测，即使底层场景对人类来说仍然容易识别。这种人类与模型之间的感知差距需要深入研究。

Method: 使用空间色彩混合方法，这是一种程序化的色彩失真家族，在自然图像上叠加结构化模式（包括RGB和Ostwald色彩系统）。引入了八种空间色彩混合变体的框架，在四个数据集上评估了来自三个模型家族的九个视觉语言模型。还进行了包含61名参与者的人类研究，比较人类与模型在相同失真条件下的表现。

Result: 随着失真程度的增加，所有模型和数据集上的准确率急剧下降；扩大语言模型规模并不能可靠地缓解这种失败。在动物识别数据集上，人类在相同失真条件下显著优于视觉语言模型。简单的受人类启发的预处理步骤能够为几种失真类型恢复相当一部分性能。

Conclusion: 视觉语言模型在结构化色彩失真下存在系统性感知弱点，人类表现显著优于模型。受人类启发的预处理和工具使用可以作为提高视觉语言模型鲁棒性的实用策略。
...
TL;DR: 提出虚拟治疗框架，将非小细胞肺癌放疗期间的肿瘤演进建模为剂量感知的多模态条件图像转换问题，用于合成反映治疗诱导解剖变化的随访CT图像。


<details>
  <summary>Details</summary>
Motivation: 预测放疗期间的肿瘤演进是临床关键挑战，特别是当纵向变化同时受解剖结构和治疗影响时。现有方法难以准确模拟治疗诱导的解剖变化。

Method: 提出虚拟治疗框架，将NSCLC进展建模为剂量感知的多模态条件图像转换问题。基于CT扫描、基线临床变量和指定辐射剂量增量，合成随访CT图像。在222名III期NSCLC患者的895个CT扫描数据集上评估，生成过程以递送剂量增量、人口统计学和肿瘤相关临床变量为条件。基准测试了基于GAN和扩散的代表性模型，包括2D和2.5D配置。

Result: 定量和定性结果表明，基于扩散的模型能更一致地从多模态、剂量感知的条件中受益，产生比基于GAN的基线更稳定、解剖学上更合理的肿瘤演进轨迹。

Conclusion: 虚拟治疗框架作为NSCLC体内治疗监测和自适应放疗研究的工具有潜力，扩散模型在模拟治疗诱导的解剖变化方面表现优于GAN模型。
...
TL;DR: FreeOcc：一种无需训练、基于基础模型的3D语义和全景占据预测方法，利用预训练模型从多视角图像恢复语义和几何信息，在未见环境中实现3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的3D占据预测方法通常需要昂贵的密集3D监督或针对目标域数据进行训练，限制了在未见环境中的部署。需要一种无需训练、能泛化到新环境的解决方案。

Method: 1) 使用可提示的基础分割模型提取每视角全景先验，通过提示到分类规则获取语义；2) 使用重建基础模型恢复度量3D点云；3) 通过深度和置信度感知过滤将可靠标签提升到3D；4) 跨时间融合并使用确定性细化堆栈进行体素化；5) 通过拟合和合并当前视角3D边界框候选恢复实例，实现无需学习3D模型的全景占据预测。

Result: 在Occ3D-nuScenes数据集上：1) 无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当；2) 作为伪标签生成管道训练下游模型时达到21.1 RayIoU，超越先前弱监督基线；3) 在无需训练和弱监督全景占据预测上分别达到3.1 RayPQ和3.9 RayPQ，建立新基线。

Conclusion: FreeOcc证明了基于基础模型的感知是实现无需训练3D场景理解的实用途径，通过利用预训练模型的能力，避免了昂贵的3D监督和特定域训练，在未见环境中实现了有效的语义和全景占据预测。
...
<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在乳腺超声图像分割中，由于标注数据极度有限，伪标签不稳定导致监督不准确和性能下降。现有视觉语言模型在医学图像上效果有限，因为领域特定提示难以迁移
...
Result: 在四个乳腺超声数据集上的实验表明，该方法在仅使用2.5%标注数据时，性能与全监督模型相当，显著优于现有半监督方法

Conclusion: 该方法范式易于扩展：对于其他成像模态或疾病，仅需全局外观描述即可获得可靠的伪监督，能够在有限标注下实现可扩展的半监督医学图像分割
...
TL;DR: JOPP-3D是一个开放词汇的语义分割框架，通过联合利用全景图像和点云数据，实现语言驱动的场景理解，在开放和封闭词汇的2D和3D语义分割任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决跨视觉模态（3D点云和全景图像）语义分割的挑战，主要问题包括标注数据稀缺和固定标签模型的有限适应性。需要开发能够通过自然语言查询生成语义掩码的开放词汇框架。

Method: 将RGB-D全景图像转换为对应的切向透视图像和3D点云，利用这些模态提取和对齐基础视觉语言特征。通过自然语言查询在两种输入模态上生成语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验评估表明，JOPP-3D能够生成跨全景和3D域的一致且有语义意义的分割结果。在开放和封闭词汇的2D和3D语义分割任务上相比现有最优方法有显著提升。

Conclusion: JOPP-3D框架成功实现了跨全景图像和点云数据的语言驱动语义分割，解决了标注数据稀缺和模型适应性有限的问题，为开放词汇场景理解提供了有效解决方案。
...
Method: 首先在MRI体积数据上预训练3D扩散模型以建立生成先验，然后使用近端策略优化（PPO）进行微调。微调过程采用新颖的奖励系统，整合了2D切片评估和3D体积分析，形成多尺度反馈机制。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，RL反馈有效引导生成过程向更高质量分布。定量分析显示Fréchet Inception Distance（FID）显著改善，且合成数据在肿瘤和疾病分类下游任务中表现出比非优化基线更好的实用性。

Conclusion: 提出的强化学习多尺度反馈方法能够有效提升3D扩散模型的生成质量，弥合标准训练目标与临床需求之间的差距，为医学图像生成提供了更高质量的合成数据。
...
TL;DR: 该论文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器在利用生成能力方面的不足，提升语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费扩散分割方法未能有效利用更强大扩散模型的生成能力，主要存在两个问题：跨注意力图在多头多层之间存在差异，难以形成统一全局表示；即使有全局图，由于不同文本标记的分数不平衡，也无法直接转化为准确的语义相关性用于分割。

Method: 提出两种技术：1) 自动聚合：解决多头多层跨注意力图之间的差异，生成统一的全局表示；2) 逐像素重缩放：解决不同文本标记之间的分数不平衡问题，使全局图能更准确地转化为语义相关性用于分割。

Result: 在标准语义分割基准测试中评估了该方法，并进一步将其集成到生成技术中，展示了改进的性能和广泛的适用性。

Conclusion: 通过自动聚合和逐像素重缩放两种技术，训练免费扩散分割方法能够更好地利用扩散模型的生成能力，从而在语义分割任务上获得更好的性能，并具有广泛的适用性。
...
TL;DR: 提出基于骨架时序动作分割的点监督框架，仅需每个动作片段标注一帧，通过多模态骨架数据、原型相似度方法和伪标签集成实现高效分割


<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，点监督方法可显著减少标注成本同时保持性能

Method: 使用预训练统一模型编码多模态骨架数据（关节、骨骼、运动信息），提出原型相似度方法并与能量函数、约束K-Medoids聚类结合生成伪标签，采用多模态伪标签集成指导模型训练

Result: 在PKU-MMD（X-Sub和X-View）、MCFS-22和MCFS-130数据集上建立新基准，实验表明该方法达到竞争性性能，甚至超越部分全监督方法，同时显著减少标注工作量

Conclusion: 提出的点监督框架为骨架动作分割提供了高效解决方案，在减少标注成本的同时保持良好性能，为智能系统感知和响应人类活动提供了实用方法
...
TL;DR: VG3S是一个新颖的3D语义占用预测框架，通过将视觉基础模型的几何感知能力注入到基于高斯泼溅的占用建模中，显著提升了预测精度。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的占用预测方法虽然计算效率高，但严重依赖准确的几何线索，这在纯视觉范式中往往不足。需要将视觉基础模型强大的几何感知能力注入到占用预测中，以弥补这一缺陷。

Method: 提出VG3S框架，包含一个即插即用的分层几何特征适配器。该适配器通过特征聚合、任务特定对齐和多尺度重构，有效转换冻结视觉基础模型的通用token，充分利用其丰富的3D几何先验。

Result: 在nuScenes占用基准测试中，VG3S相比基线实现了12.6%的IoU和7.5%的mIoU显著提升。该框架能够无缝泛化到不同的视觉基础模型，一致地提升占用预测精度。

Conclusion: VG3S成功地将视觉基础模型的几何感知能力与高斯泼溅占用建模相结合，证明了整合预训练几何感知视觉基础模型先验的巨大价值，为自动驾驶场景理解提供了更强大的解决方案。
...
TL;DR: 提出EntON方法，在3D高斯泼溅中引入特征值熵优化的邻域致密化策略，通过交替优化框架平衡几何精度与渲染质量


<details>
  <summary>Details</summary>
Motivation: 标准3DGS生成的高斯中心与物体几何对齐不佳，而表面重建方法常牺牲光度精度。需要一种能同时保持几何准确性和渲染质量的方法

Method: 提出特征值熵优化的邻域致密化策略EntON，计算每个高斯中心k近邻协方差矩阵特征值的特征值熵，量化局部结构有序度。在交替优化框架中结合标准梯度致密化和特征值熵感知致密化，优先在低熵（有序、平坦）区域致密化，在高熵（无序、球形）区域修剪
...
Method: 1) 识别最小有意义的伪造单元——词标记，并按语音自然语言边界对齐数据预处理；2) 引入法证特征重对齐模块，将预训练语义空间表示映射到可区分的法证流形；3) 设计基于伪影的非对称损失函数，动态抑制真实梯度并优先处理细微法证伪影。

Result: WAFL在数据集内和跨数据集设置下均显著优于现有最先进方法，同时需要更少的可学习参数并保持高计算效率。

Conclusion: WAFL通过将时序伪造定位重新定义为离散词级二元分类任务，解决了特征粒度不对齐和计算成本高的问题，为时序伪造检测提供了高效且准确的解决方案。
...
TL;DR: HCF-RES是一个多模态3D指代表达分割框架，通过分层视觉语义分解和渐进式多级融合，在稀疏点云基础上引入2D视觉语义，显著提升对细粒度描述的3D物体定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理匹配多个或零目标的复杂自然语言描述，需要多模态融合来提升细粒度理解能力。

Method: 提出HCF-RES框架：1) 分层视觉语义分解：利用SAM实例掩码指导CLIP编码，获取像素级和实例级双粒度特征，在2D到3D投影中保持物体边界；2) 渐进式多级融合：通过模态内协作、2D语义与3D几何特征的跨模态自适应加权、以及语言引导的细化进行表征集成。

Result: 在ScanRefer和Multi3DRefer两个基准测试上均取得了最先进的结果。

Conclusion: HCF-RES通过有效融合2D视觉语义与3D几何信息，显著提升了3D指代表达分割的性能，特别是在处理复杂自然语言描述和多目标/零目标匹配场景方面表现出色。
...
TL;DR: NOVA提出了一种基于大语言模型的自回归范式，将3D多目标跟踪从传统的距离匹配转向生成式时空语义建模，实现了开放词汇的跨类别泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法受限于闭集假设和"语义盲"启发式方法，无法有效处理开放世界中的未知目标类别。需要一种能够泛化到未知目标、同时利用语义信息保持身份一致性的新范式。

Method: NOVA将3D轨迹重构为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为原则性的下一步序列补全过程。通过编码物理运动连续性和深度语言先验，利用语言空间的层次结构解决细粒度语义模糊性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上表现出优越性能。在nuScenes数据集上，NOVA在Novel类别上达到22.41%的AMOTA，相比基线获得20.21%的绝对提升，仅使用0.5B参数的自回归模型。

Conclusion: NOVA通过生成式时空语义建模范式，成功解决了3D多目标跟踪中的开放词汇泛化问题，将跟踪从传统的距离匹配转向基于语义的自回归序列补全，显著提升了跨类别跟踪性能。
...
TL;DR: 提出StrSR框架，通过谱和轨迹正则化解决扩散变换器在真实图像超分辨率中的一步蒸馏问题


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在真实图像超分辨率中表现出潜力，但其迭代采样计算成本高，需要一步蒸馏。现有的一步蒸馏方法在DiT上存在轨迹不匹配和严重的网格状周期性伪影问题

Method: 提出StrSR框架，采用非对称判别蒸馏架构来弥合轨迹差距，并设计频率分布匹配策略来抑制DiT特有的高频谱泄漏引起的周期性伪影

Result: StrSR在真实图像超分辨率任务中实现了最先进的性能，在定量指标和视觉感知方面都表现出色

Conclusion: StrSR通过谱和轨迹正则化有效解决了扩散变换器在真实图像超分辨率中的一步蒸馏挑战，为高效高质量的超分辨率提供了解决方案
...
<details>
  <summary>Details</summary>
Motivation: 真实世界的3D体素标注存在结构伪影和动态拖尾效应等固有噪声，但现有方法未系统研究这些不可靠监督对自动驾驶系统安全性的影响，需要建立专门基准和鲁棒框架。

Method: 提出DPR-Occ框架，通过双源部分标签推理构建可靠监督：结合时间模型记忆和表示级结构亲和力，动态扩展和修剪候选标签集，保留真实语义同时抑制噪声传播。

Result: 在SemanticKITTI上的实验表明，DPR-Occ在极端噪声下防止了几何和语义崩溃。即使在90%标签噪声下，相比现有标签噪声学习方法，仍能获得显著性能提升（mIoU提升2.57%，IoU提升13.91%）。

Conclusion: 通过连接标签噪声学习和3D感知，OccNL基准和DPR-Occ框架为动态环境中安全关键的机器人感知提供了可靠基础，填补了现有研究空白。
...
Result: 在三个广泛使用的基准数据集上，ADiVA显著优于最先进方法（如在AWA2和SUN上分别提升4.7%和6.1%），且可作为插件增强现有生成式ZSL方法。
...
Method: 提出FlowMotion框架：1）利用流基T2V模型的预测输出直接提取运动表示；2）基于早期潜在预测编码的丰富时序信息设计流指导；3）引入速度正则化策略稳定优化并确保运动平滑演化
...
<details>
  <summary>Details</summary>
Motivation: 锥束计算机断层扫描（CBCT）在牙科成像中广泛应用，但高密度物体（如牙科种植体）会产生严重伪影，影响图像质量和诊断准确性。现有基于扩散模型的种植体修复方法仅处理独立的2D投影，忽略了投影间的相关性，导致重建图像不一致。
...
Conclusion: 提出的基于垂直分数扩散模型的3D种植体修复方法通过建模投影序列的3D分布，解决了现有2D方法忽略投影相关性的问题，为改善CBCT图像质量和临床诊断准确性提供了有前景的解决方案。
...
TL;DR: 提出一种小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器建模全局语义、小波分解提取多频特征、专家混合模块聚合上下文信息，在14个工业和医学数据集上验证有效性。
...
Conclusion: 提出的方法通过结合全局语义建模、多频特征提取和上下文聚合，显著提升了零样本异常检测的性能。
...
Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估，相比无监督方法取得了最佳性能。具体而言，在S3DIS数据集Area-5上实现了47.1%的mIoU，超过了经典全监督方法PointNet的2.5%。

Conclusion: 提出的P-SLCR方法通过原型库驱动的结构学习和一致性推理，有效解决了无监督点云语义分割的挑战，在多个数据集上取得了优于现有无监督方法甚至某些全监督方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程计算成本过高，难以用于交互式使用和长时程推演。虽然特征缓存可以在不训练的情况下加速推理，但针对单模态扩散设计的策略在迁移到世界模型时效果不佳，主要面临两个特定障碍：多模态耦合和空间变化导致的令牌异质性，以及非均匀时间动态（少数困难令牌驱动误差增长）。

Method: 提出WorldCache缓存框架，包含两个核心技术：1）曲率引导的异构令牌预测：使用基于物理的曲率分数估计令牌可预测性，对具有方向突变的混沌令牌应用Hermite引导的阻尼预测器；2）混沌优先自适应跳过：累积曲率归一化的无量纲漂移信号，仅在瓶颈令牌开始漂移时重新计算。

Result: 在扩散世界模型上的实验表明，WorldCache实现了高达3.7倍的端到端加速，同时保持98%的推演质量，在资源受限场景下展现出显著优势和实用性。

Conclusion: WorldCache是针对扩散世界模型的高效缓存框架，通过解决令牌异质性和非均匀时间动态这两个关键挑战，显著加速推理过程而不损失推演质量，为资源受限环境下的世界模型应用提供了实用解决方案。
...
TL;DR: LATO是一种新颖的拓扑保持潜在表示，通过顶点位移场和稀疏体素VAE实现可扩展的流匹配式显式3D网格生成，无需等值面提取或启发式网格化。


<details>
  <summary>Details</summary>
Motivation: 现有3D网格生成方法通常依赖等值面提取或启发式网格化，难以同时保持复杂几何形状和良好拓扑结构，且推理效率较低。需要一种能够直接生成显式网格表示的方法。

Method: LATO将网格表示为基于表面的顶点位移场，使用稀疏体素VAE压缩为结构化体素潜在表示。解码器通过渐进细分和修剪体素来实例化顶点位置，专用连接头查询体素潜在特征直接预测顶点对之间的边连接性。采用两阶段流匹配过程：首先生成结构体素，然后细化体素级拓扑特征。

Result: 相比基于等值面/三角形的扩散模型和自回归生成方法，LATO能够生成具有复杂几何形状和良好拓扑结构的网格，同时推理效率显著提高。

Conclusion: LATO提供了一种新颖的拓扑感知潜在表示，实现了可扩展的流匹配式显式3D网格合成，在保持几何复杂性和拓扑质量的同时提高了生成效率。
...
Method: 1. 构建DentalProbe数据集（5000张图像，专家标注诊断轨迹）；2. 开发Reinspection驱动的强化学习框架（基于规则的奖励和条件诊断驱动奖励）；3. 提出OralGPT-Plus智能视觉语言模型，支持迭代对称感知诊断推理；4. 建立MMOral-X基准测试（300个开放式问题，多难度级别区域标注）
...
<details>
  <summary>Details</summary>
Motivation: 获取密集的像素级标注成本高昂，稀疏标注作为弱监督替代方案存在性能差距，需要利用3D场景重建作为辅助监督信号来提升性能

Method: 采用双师生架构，利用前馈式3D重建从2D视频恢复3D几何结构，在2D图像和重建的3D点云之间强制语义一致性，通过几何监督传播稀疏标注

Result: Rewis3d在稀疏监督下实现最先进性能，比现有方法提升2-7%，无需额外标注或推理开销

Conclusion: 3D几何结构为弱监督语义分割提供了强大的监督信号，前馈式3D重建技术能够有效传播稀疏标注，显著提升分割性能
...
TL;DR: CHMv2是一个基于高分辨率光学卫星影像和深度估计模型的全球米级分辨率冠层高度图，相比现有产品在精度、偏差和细节保持方面有显著提升。


<details>
  <summary>Details</summary>
Motivation: 机载激光扫描（ALS）提供的高精度冠层高度数据在全球范围内分布不均，限制了森林碳储量估算、恢复与退化监测以及栖息地结构评估等应用。

Method: 基于DINOv3构建深度估计模型，利用高分辨率光学卫星影像生成冠层高度图；通过大规模地理多样性训练数据扩展、自动化数据整理与配准、针对冠层高度分布定制的损失函数和数据采样策略实现性能提升。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，更好地保持了冠层边缘和林窗等精细结构；在独立ALS测试集以及数千万个GEDI和ICESat-2观测数据上验证了其在主要森林生物群落中的一致性能。

Conclusion: CHMv2提供了一个全球覆盖、米级分辨率的冠层高度图，解决了现有产品在精度和可用性方面的限制，为全球森林监测和生态研究提供了重要工具。
...
TL;DR: REACT++是基于REACT架构改进的实时场景图生成模型，通过高效特征提取和原型空间内的主体-客体交叉注意力机制，在保持物体检测精度的同时提升关系预测准确率并降低延迟，成为当前最快的SGG模型。


<details>
  <summary>Details</summary>
Motivation: 现有场景图生成方法往往只关注关系预测准确性、物体检测准确性或延迟降低中的单一目标，缺乏同时平衡这三个关键指标的解决方案。为了满足实时应用需求，需要开发能够在性能与推理速度之间取得更好权衡的模型。

Method: 基于REACT架构，引入高效特征提取和原型空间内的主体-客体交叉注意力机制。通过优化特征表示和注意力计算，在保持计算效率的同时增强关系建模能力。

Result: REACT++在现有SGG模型中达到最高推理速度，相比前代REACT版本速度提升20%，关系预测准确率平均提升10%，同时保持物体检测性能不受影响。

Conclusion: REACT++成功平衡了场景图生成中的三个关键目标：关系预测准确性、物体检测准确性和推理速度，为实时应用提供了有效的解决方案，代码已开源供社区使用。
...
TL;DR: 提出人机协同的拼图框架，通过交互式引导解决大规模考古碎片重建难题，在RePAIR基准测试中超越全自动和纯手动方法。


<details>
  <summary>Details</summary>
Motivation: 真实世界考古文物碎片重建面临侵蚀、缺失区域、不规则形状和大规模歧义等挑战，传统拼图求解器在碎片数量达数千时表现不佳，需要更有效的解决方案。

Method: 提出人机协同框架，将自动松弛标记求解器与交互式人工引导结合，引入迭代锚定和连续交互优化两种策略，允许用户锁定已验证位置、纠正错误并引导系统。

Result: 在多个RePAIR数据集上的实验表明，该混合方法在准确性和效率上显著优于全自动和手动基线，为大规模专家参与的文物重建提供了实用解决方案。

Conclusion: 人机协同框架通过结合自动求解和人工引导，有效解决了大规模考古碎片重建的复杂性和规模问题，为文化遗产保护提供了新的技术途径。
...
TL;DR: BEiT视觉Transformer在感知冲突中偏好凸性补全，研究发现这种偏好由特定注意力头（L0H9）控制，通过调节该头可改变补全决策边界


<details>
  <summary>Details</summary>
Motivation: 研究视觉Transformer在图形-背景组织中的感知歧义性，特别是探究BEiT模型如何实现经典的格式塔凸性先验，并定位这一先验在模型内部组件中的实现位置

Method: 使用基于飞镖形状合成图形的受控感知冲突实验，系统性地掩蔽既支持凹性补全也支持凸性补全的区域；通过logit归因将内部激活投影到离散视觉码本空间，分解注意力头的直接影响

Result: BEiT在感知冲突中可靠地偏好凸性补全；图形-背景组织在早期和中间层保持歧义，在后期层突然解决；识别出注意力头L0H9作为早期种子，引入对凸性的弱偏好；下调该单个注意力头可改变感知冲突的分布质量，使凹性证据能够引导补全

Conclusion: BEiT中的格式塔凸性先验由特定功能单元（如注意力头L0H9）实现，这些单元在Transformer子结构中可识别；通过调节单个注意力头可连续控制感知决策边界，揭示了Transformer内部感知偏好的可解释机制
...
<details>
  <summary>Details</summary>
Motivation: 在机器人物理限制或视障用户安全考虑等现实场景中，主动探索不可行。需要让智能体仅凭有限观察就能心理模拟未来轨迹并回答空间假设问题
...
Result: 实验表明：(1)心理探索对情境推理至关重要；(2)世界模型在WanderDream-Gen上表现优异；(3)想象力显著促进WanderDream-QA上的推理；(4)WanderDream数据在现实场景中具有显著可迁移性

Conclusion: WanderDream为无需主动探索的心理模拟推理提供了首个大规模基准，证明了心理探索在空间推理中的重要性，数据集在现实场景中具有良好迁移性
...
TL;DR: CaTok是一种1D因果图像分词器，通过MeanFlow解码器和REPA-A正则化，解决了视觉自回归模型中因果表示学习的问题，在图像重建和生成任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器存在两个主要问题：1）将2D图像块展平为非因果序列，或使用启发式排序，与"下一个token预测"模式不匹配；2）扩散自编码器要么缺乏因果性，要么引入不平衡性。需要一种能够支持因果自回归建模的视觉分词方法。

Method: CaTok包含三个核心组件：1）1D因果图像分词器，通过时间间隔选择token并与MeanFlow目标绑定；2）MeanFlow解码器，支持快速单步生成和高保真多步采样；3）REPA-A正则化，通过将编码器特征与视觉基础模型对齐来稳定和加速训练。

Result: 在ImageNet重建任务上达到SOTA：FID 0.75，PSNR 22.53，SSIM 0.674，且训练轮次更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功解决了视觉自回归建模中的因果表示学习问题，通过因果1D表示支持高效的图像生成，为视觉语言模型的统一建模提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活但难以通过提示或简单推理适应来控制，不适合有严格产品要求的用例。需要构建能够支持Pinterest图像编辑和增强用例的系统。

Method: 首先在多模态数据集上训练基础扩散模型，然后在任务特定数据集上快速微调变体，为单个用例生成专用模型。包括数据集策划、训练和推理的最佳实践。

Result: 在线A/B实验显示增强图像分别获得18.0%和12.5%的参与度提升。与人类评分者比较表明模型在这些任务上优于第三方模型。系统可推广到多图像场景合成和图像到视频生成等下游任务。

Conclusion: Pinterest Canvas通过基础模型加任务特定微调的方法，成功构建了能够满足严格产品要求的大规模图像生成系统，在多个用例中表现出色且具有良好泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究去噪生成模型中训练目标的设计选择，包括损失加权和输出参数化，旨在理清影响模型性能的各种因素，为实际训练提供指导

Method: 通过系统数值研究，分析训练选择与数据流形内在维度、模型架构和数据集大小的相互作用，实验涵盖具有可控几何形状的合成数据集和图像数据

Result: 使用定量指标（不同噪声水平的PSNR和生成质量的FID）比较训练目标，提供了关于设计选择的实用见解

Conclusion: 研究旨在理清训练流匹配模型时各种重要因素，而非提出新方法，为实际训练提供设计选择的实用指导
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型开发过度依赖暴力扩展，直接移植自然图像处理方法导致模型脆弱且计算昂贵，需要资源高效且泛化能力强的解决方案
...
Result: 在超过20万张来自4个机构、2种模态的图像上进行实验，在胸部和腹部CT数据集上均超越基线模型，在内部肌肉骨骼MRI图像上显示监督原则可跨模态迁移，计算需求降低数个数量级

Conclusion: 挑战了"规模即一切"的教条，证明了通过精心设计的监督而非数据堆叠可实现高效高性能RFM，使临床医生即使在笔记本电脑上也能开发SOTA模型
...
TL;DR: Match4Annotate：基于隐式神经表示的轻量级视频标注传播框架，支持点标注和掩码标注的帧内与跨视频传播，在临床超声数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 在医学影像等专业领域，逐帧视频标注成本高昂且耗时。现有标注传播方法存在局限性：视频跟踪器和分割模型需要逐视频初始化且无法跨视频泛化；传统对应性方法在低纹理场景表现不佳；密集特征匹配和一次性分割方法缺乏时空平滑性且不支持统一的点/掩码标注。

Method: 提出Match4Annotate框架：1）在测试时使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应性匹配；3）支持点标注和掩码标注的帧内与跨视频传播。

Result: 在三个具有挑战性的临床超声数据集上评估：1）跨视频传播性能达到SOTA，优于特征匹配和一次性分割基线；2）帧内传播与专用跟踪器性能相当；3）轻量级测试时优化特征匹配流程为可扩展标注工作流提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型存在三个问题：需要单独训练、目标不对齐、表现出意外的缩放行为。这种依赖性源于模型的训练目标（去噪任务）缺乏学习语义表征的激励

Method: 提出Self-Flow自监督流匹配范式，核心机制是双时间步调度：对token应用异质噪声水平，创建信息不对称，迫使模型从损坏输入中推断缺失信息，从而驱动同时学习强表征和生成能力

Result: 方法跨模态通用，支持多模态训练，遵循预期的缩放定律，在图像、视频和音频生成方面取得优越性能

Conclusion: Self-Flow通过将表征学习集成到生成框架中，消除了对外部模型的依赖，同时实现了强语义表征学习和高质量生成，为多模态生成提供了统一的解决方案
...
Conclusion: AV-Unified通过统一框架实现了跨多种音频-视觉场景理解任务的联合学习，标准化表示和多尺度感知设计使其能够全面理解复杂音频-视觉场景。
...
TL;DR: 提出一种针对漫反射直接飞行时间LiDAR的空间校准方法，通过扫描反光片恢复每个像素在RGB图像平面中的响应映射，解决标准LiDAR-RGB校准中单射线假设不适用的问题。


<details>
  <summary>Details</summary>
Motivation: 漫反射直接飞行时间LiDAR通过聚合大瞬时视场内的光子返回形成深度直方图，这违反了标准LiDAR-RGB校准所基于的单射线假设，需要新的空间校准方法来建立LiDAR像素与RGB图像之间的对应关系。

Method: 使用扫描的反光片配合背景减法，恢复每个LiDAR像素在共位RGB图像平面中的响应映射（足迹和相对空间灵敏度），从而建立明确的LiDAR到RGB对应关系。

Result: 在ams OSRAM TMF8828设备上验证了该方法，能够为跨模态对齐和融合提供明确的LiDAR-RGB对应关系。

Conclusion: 该方法解决了漫反射LiDAR与RGB相机校准的特殊挑战，通过简单的空间校准程序实现了有效的跨模态对齐，为LiDAR-RGB融合应用提供了实用解决方案。
...
TL;DR: 该论文提出了一种在扩散模型中处理语言否定的统一框架，将否定建模为扩散动态中语义引导的结构化可行性约束，无需重新训练模型参数。


<details>
  <summary>Details</summary>
Motivation: 否定是基本的语言运算符，但在基于扩散的生成系统中建模不足。现有方法缺乏对语言否定的形式化处理，无法统一处理多样化的否定现象。

Method: 将否定重新解释为语义更新方向上的凸约束集，将无分类器引导定义为语义更新方向，并通过投影到从语言结构导出的凸约束集来强制执行否定。该方法无需训练，与预训练扩散主干兼容。

Result: 方法实现了稳健的否定合规性，同时保持了视觉保真度和结构连贯性。建立了扩散生成模型中语言否定的首个统一表述，超越了表示级评估。

Conclusion: 提出了扩散模型中语言否定的形式化处理方法，创建了统一的否定处理框架，并引入了结构化的否定基准套件，为进一步研究奠定了基础。
...
TL;DR: 该研究分析了自动驾驶数据集中多源多模态数据的冗余问题，通过实验证明选择性去除冗余标签可以提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究过于关注算法设计而忽视数据质量分析，特别是冗余这一基本但未充分探索的数据质量问题。多源多模态数据中普遍存在冗余，但缺乏系统性建模和测量。

Method: 使用nuScenes和Argoverse 2数据集，建模和测量多源摄像头数据以及多模态图像-LiDAR数据的冗余。通过选择性去除冗余标签，评估对YOLOv8目标检测任务的影响。

Result: 实验表明：1）选择性去除共享视野摄像头中的冗余多源图像对象标签可提升检测性能（nuScenes中mAP50从0.66提升至0.70等）；2）AV2中去除4.1-8.6%标签后mAP50保持在0.64基线附近；3）多模态分析显示图像和LiDAR数据间存在显著冗余。
...
TL;DR: EgoReasoner：针对自我中心视频4D推理任务的两阶段框架，通过任务自适应思维模板和任务感知奖励函数，在少量样本上实现显著性能提升


<details>
  <summary>Details</summary>
Motivation: 自我中心视频理解因环境的动态4D特性而复杂，现有任务无关方法（通用思维链和统一强化学习）无法适应不同认知操作（空间锚定、时间跟踪、持续时间推理）的结构差异，导致性能不足

Method: 提出EgoReasoner两阶段框架：第一阶段使用任务自适应思维模板指导结构化思维链轨迹合成，通过监督微调使模型适应不同任务类型；第二阶段设计任务感知奖励函数验证实体基础、时间对齐和任务自适应逻辑一致性，通过GRPO进行强化微调选择性强化各推理路径

Result: 仅用16K样本训练的3B参数模型在HD-EPIC基准测试中达到37.5%平均准确率，超越Qwen2.5-VL-7B（25.7%）超过10个百分点

Conclusion: 通过将推理框架和奖励信号与任务认知结构对齐，EgoReasoner能够有效解决自我中心4D推理任务的结构差异问题，在少量数据上实现显著性能提升
...
TL;DR: Penguin-VL提出了一种紧凑型视觉语言模型，通过从纯文本LLM初始化视觉编码器来替代传统的对比预训练视觉编码器，在保持轻量级架构的同时实现了与大型VLMs相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前VLM发展主要依赖模型规模扩展，这阻碍了在计算受限的移动和边缘设备上的部署。同时，主流VLMs依赖通过大规模对比预训练（如CLIP/SigLIP）初始化的视觉编码器，但对比学习的目标与密集感知任务存在不匹配：对比学习优化的是判别性，强制了类别级不变性，抑制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而不是传统的对比预训练视觉编码器。这种方法解决了目标不匹配问题，解锁了更高的视觉保真度和数据效率。通过轻量级架构（如2B和8B参数规模），在保持计算效率的同时实现高性能。

Result: 在各种图像和视频基准测试中，Penguin-VL在数学推理方面与领先的VLMs（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务中超越它们。消融研究表明，Penguin-Encoder始终优于对比预训练的编码器，保留了密集感知和复杂推理所需的关键细粒度空间和时间线索。

Conclusion: 研究表明，改进视觉表示而非模型缩放是性能提升的主要驱动力。Penguin-Encoder作为传统对比预训练编码器的强大替代方案，适用于计算高效的VLMs，能够在资源受限的环境中实现高性能。这为在移动和边缘设备上部署高效VLMs提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用自回归架构，在架构设计上存在探索有效替代方案的空间。离散扩散模型在视觉理解和图像生成等领域显示出作为多模态系统骨干的潜力，但尚未被用于构建统一的多模态模型。

Method: 采用统一的掩码离散扩散模型直接捕获离散多模态标记的联合分布，支持双模态任务以及涉及多个模态的复杂场景，实现了文本、语音和图像的任意到任意转换。

Result: 在多样化基准测试中，该方法优于或与处理两个及以上模态的现有多模态系统表现相当，突显了扩散模型作为下一代多模态基础模型骨干的显著潜力。

Conclusion: Omni-Diffusion展示了离散扩散模型作为多模态系统统一骨干的有效性，为下一代多模态基础模型的发展提供了有前景的替代架构。
...
TL;DR: 该论文揭示了多模态大语言模型（MLLM）分类性能评估中的关键问题：评估协议和标注质量严重影响结果，导致与监督模型的比较结论相互矛盾。通过修正标签和评估协议，MLLM性能显著提升，缩小了与监督模型的差距。


<details>
  <summary>Details</summary>
Motivation: 当前关于MLLM与监督模型、视觉语言模型在分类任务上性能比较的研究存在矛盾结论。作者发现这些矛盾源于评估协议和标注质量问题，这些问题要么夸大要么低估MLLM性能。需要系统分析评估协议中的缺陷，并量化设计选择的影响。

Method: 1. 识别并修正常见评估协议中的关键问题：模型输出超出类别列表被丢弃、弱多选干扰项导致结果膨胀、开放世界设置因输出映射不佳而表现不佳。2. 量化常被忽视的设计选择影响：批次大小、图像顺序和文本编码器选择。3. 使用ReGT（ImageNet-1k的625个类别的多标签重新标注）进行评估。4. 进行受控案例研究，评估MLLM辅助人工标注的效果。

Result: 1. MLLM从修正标签中获益最大（提升达+10.8%），显著缩小了与监督模型的感知差距。2. 评估协议问题（输出丢弃、弱干扰项、映射不佳）导致MLLM性能被误判。3. 设计选择（批次大小、图像顺序、文本编码器）对准确率有显著影响。4. 较少依赖监督训练信号的模型对标注质量最敏感。5. MLLM可辅助人工标注：在困难案例中，标注者确认或整合了约50%的MLLM预测。

Conclusion: MLLM在分类任务上表现不佳主要是嘈杂标注和有缺陷评估协议的产物，而非真正的模型缺陷。修正评估协议和标注质量后，MLLM性能显著提升。MLLM对标注质量敏感，且能有效辅助大规模数据集标注工作，展示了其在数据整理中的潜力。
...
### [124] [MASFactory: A Graph-centric Framework for Orchestrating LLM-Based Multi-Agent Systems with Vibe Graphing](https://arxiv.org/abs/2603.06007)
*Yang Liu,Jinxuan Cai,Yishen Li,Qi Meng,Zedi Liu,Xin Li,Chen Qian,Chuan Shi,Cheng Yang*

Main category: cs.CL

TL;DR: MASFactory是一个基于图的框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可编辑的工作流规范，提供可复用组件和可视化工具。
...
Method: 提出MASFactory框架，采用Vibe Graphing方法将自然语言意图编译为可编辑的工作流规范并转换为可执行图，提供可复用组件、可插拔上下文集成和可视化工具。

Result: 在七个公共基准测试上评估MASFactory，验证了代表性MAS方法的再现一致性以及Vibe Graphing的有效性。

Conclusion: MASFactory通过图中心框架解决了当前LLM-based MAS工作流编排的局限性，提供了更高效、可复用和可扩展的解决方案。
...
TL;DR: Aletheia：基于RAG和LLM的浏览器扩展，通过证据解释、讨论中心和最新事实核查功能，提升假新闻检测的透明度和用户参与度
...
Method: 提出Aletheia浏览器扩展，结合检索增强生成（RAG）和大语言模型（LLM）检测假新闻并提供基于证据的解释；包含两个交互组件：讨论中心（用户对话平台）和"保持知情"功能（展示最新事实核查）

Result: 实验表明Aletheia在检测性能上优于最先进的基线方法；250名参与者的用户研究证实了系统的可用性和感知有效性

Conclusion: Aletheia展示了作为透明工具对抗在线假新闻的潜力，通过结合先进AI技术与用户参与机制，为解决假新闻检测的透明度和用户参与问题提供了有效方案
...
TL;DR: 该论文研究语法约束解码(GCD)，证明语法等价性不保证解码效率等价，引入结构歧义成本(SAC)度量，建立解码成本下界，并推导硬掩码解码的失真界限。


<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码中语法等价性与解码效率之间的脱节问题：即使语法在语言层面等价，其编译后的状态空间和在线解码成本可能显著不同，需要理论框架来分析和优化解码效率。

Method: 将GCD建模为自回归下一个token分布与基于上下文无关文法(CFG)编译的下推系统可达性预言机的耦合；引入结构歧义成本(SAC)度量增量打包解析森林增长；建立预言机不变性定理和引擎无关下界；推导硬掩码解码的失真界限。

Result: 证明语言等价文法产生相同的下一个token集合但状态空间和在线成本不同；给出规范语言a^n b^n的状态爆炸精确计数；证明特定CFG族上任何在线掩码引擎必须承担Ω(t²)每token工作；定义解码成本等价类并找到最小SAC代表；推导硬掩码解码的KL和总变差失真界限。

Conclusion: 语法等价性不保证解码效率等价，结构歧义成本是解码效率的关键度量；存在解码成本最小化的语法优化空间；理论结果为Transformer和MoE架构的语法约束解码提供了性能分析和优化基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法缺乏可解释性，用户难以理解检测决策的依据。需要开发一个既能准确检测又能提供透明解释的框架。
...
Conclusion: NOTAI.AI提供了一个可解释的机器生成文本检测框架，通过多特征融合和透明解释机制，增强了检测系统的可信度和用户理解。
...
TL;DR: CoT提示增强LLM推理但会增加PII泄露风险，论文提出模型无关框架量化11类PII的令牌级泄露风险，发现CoT持续增加高风险类别泄露，泄露程度与模型家族和推理预算相关，并评估多种轻量级门控方法。


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought（CoT）提示虽然能提升大型语言模型的推理能力，但会增加隐私风险，可能导致个人身份信息（PII）从提示中泄露到推理轨迹和输出中，即使模型被指示不得重复PII。需要系统研究这种推理时的直接PII泄露问题。

Method: 提出模型无关框架：1）将泄露定义为11种PII类型的风险加权令牌级事件；2）追踪泄露曲线作为允许CoT预算的函数；3）在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。同时评估四种轻量级推理时门控方法：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: CoT持续增加泄露，特别是高风险类别；泄露程度强烈依赖于模型家族和推理预算；增加推理预算可能放大或减弱泄露，取决于基础模型。在门控方法评估中，没有单一方法在所有模型或预算上占优，风险加权F1、Macro-F1和召回率表现各异。

Conclusion: 需要混合、风格自适应的门控策略来平衡效用和风险，在通用、可复现的协议下管理CoT推理中的PII泄露问题。研究强调了在利用CoT增强推理能力时，必须考虑隐私保护机制。
...
TL;DR: 本文通过扰动框架测试LLM道德判断的稳定性，发现视角转换比表面编辑更易改变判断，且评估协议选择是影响结果的最主要因素，表明LLM道德判断受叙事形式和任务框架共同影响。


<details>
  <summary>Details</summary>
Motivation: 随着人们越来越多地使用大型语言模型（LLM）获取日常道德和人际关系指导，但这些系统无法审视缺失的上下文，也无法像人类一样判断呈现的道德困境。因此需要测试LLM道德判断的稳定性和可操纵性。

Method: 引入道德判断扰动框架，使用来自r/AmItheAsshole的2,939个道德困境，生成三类内容扰动：表面编辑（词汇/结构噪声）、视角转换（声音和立场中立化）和说服线索（自我定位、社会证明、模式承认、受害者框架）。同时变化评估协议（输出顺序、指令放置和非结构化提示）。使用四种模型（GPT-4.1、Claude 3.7 Sonnet、DeepSeek V3、Qwen2.5-72B）进行评估，共129,156个判断。

Result: 表面扰动产生的翻转率较低（7.5%），基本在自一致性噪声范围内（4-13%），而视角转换导致显著更高的不稳定性（24.3%）。37.9%的困境对表面噪声稳健但在视角变化下翻转，表明模型将叙事声音作为实用线索。不稳定性集中在道德模糊案例中，特别是无人被指责的情景。说服扰动产生系统性方向性偏移。协议选择是所有因素中影响最大的：结构化协议间的一致性仅为67.6%（kappa=0.55），只有35.7%的模型-情景单元在所有三种协议中匹配。

Conclusion: LLM道德判断是由叙事形式和任务框架共同产生的，当结果依赖于呈现技巧而非道德实质时，会引发可重复性和公平性问题。这表明当前LLM的道德判断系统存在重大缺陷，需要更稳健的评估方法。
...
TL;DR: FreeTxt-Vi是一个免费开源的基于Web的双语越南语-英语文本分析工具包，结合语料库语言学和NLP技术，为非专业用户提供文本分析功能，并通过评估验证了其性能优势。


<details>
  <summary>Details</summary>
Motivation: 越南语作为广泛使用但在NLP中代表性不足的语言，缺乏易用的文本分析工具。现有平台技术门槛高，限制了非专业用户进行多语言文本分析。需要开发一个降低技术障碍、支持可重复研究并促进越南语语言资源发展的工具。

Method: 设计统一的双语NLP流水线，集成混合的VnCoreNLP和BPE分词策略、微调的TabularisAI情感分类器以及微调的Qwen2.5模型进行抽象摘要。系统结合语料库分析功能（如索引、关键词分析、词关系探索、交互可视化）和基于Transformer的NLP组件。

Result: 通过三部分评估（分词、情感分析、摘要）显示，该方法在越南语和英语上都取得了与广泛使用的基线模型相当或更优的性能。工具包支持可重复研究，促进了越南语语言资源的发展。

Conclusion: FreeTxt-Vi通过降低多语言文本分析的技术障碍，为非专业用户提供了强大的文本分析能力，特别适用于教育、数字人文、文化遗产和社会科学等领域，填补了越南语NLP工具的空缺。
...
TL;DR: 该研究使用RGB基准评估RAG系统在噪声、信息整合、负拒绝和反事实鲁棒性四种场景下的表现，比较了RGB基线RAG与基于知识图谱的GraphRAG系统，测试了三种GraphRAG定制化方案以提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: RAG系统旨在增强LLM的能力，使其能够访问预训练之外的知识，减少事实性幻觉。然而，检索到的不一致信息可能对LLM响应产生负面影响。RGB基准被引入用于评估RAG系统在此类条件下的鲁棒性，但需要进一步研究如何提升系统在现实场景中的可靠性。

Method: 使用RGB语料库评估LLM在四种场景下的表现：噪声鲁棒性、信息整合、负拒绝和反事实鲁棒性。对RGB RAG基线与基于知识图谱的GraphRAG系统进行对比分析。测试了三种GraphRAG定制化方案以改进鲁棒性。

Result: 结果显示GraphRAG定制化方案相比RGB基线有所改进，为设计更可靠的现实世界RAG系统提供了见解。

Conclusion: 基于知识图谱的RAG系统定制化能够提升在噪声、不一致信息等挑战性条件下的鲁棒性，为构建更可靠的现实世界RAG系统提供了有价值的指导。
...
TL;DR: 通过全球调查评估不同群体对文化在生成式AI中应如何呈现的理解与信念，提出文化敏感的开发建议


<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于全球对生成式AI应如何呈现文化的态度的实证证据，需要了解不同群体对文化的定义及其在AI中的表现期望

Method: 进行大规模全球调查，覆盖欧洲、北美、南美、亚洲和非洲地区，收集关于文化定义以及生成式AI应如何呈现文化制品、概念或价值观的数据，从社区直接提炼文化的工作定义

Result: 建立了对文化概念复杂性及其与生成式AI呈现关系的理解，形成了关于文化与生成式AI开发的一系列建议

Conclusion: 提出包括参与式方法、超越地理维度的特定文化维度（如宗教和传统）优先考虑，以及处理文化"红线"的敏感性框架等建议
...
TL;DR: 提出基于L-积的谱张量分解Transformer架构，通过将嵌入空间分解为p个独立谱子Transformer，在保持语义的同时实现约1/p的编码器参数压缩


<details>
  <summary>Details</summary>
Motivation: Transformer架构在扩展时面临参数增长和嵌入维度冗余问题，需要一种既能压缩参数又能保持性能的结构化方法

Method: 基于三阶张量L-积的结构化谱分解，将令牌表示重塑为谱张量切片，在变换域执行注意力与前馈操作，构建p个独立谱子Transformer的Tensor Transformer架构

Result: 在IMDB和AG News数据集上，编码器参数最多减少75%(p=4)仍保持竞争力；IMDB上匹配或优于标准基线，AG News上中等宽度有小幅精度下降但编码器减少4倍，BERT-base宽度下性能恢复持平

Conclusion: L-Transformer通过谱张量分解实现参数高效压缩，同时引入嵌入频率的归纳偏置，支持切片依赖的频率缩放以提升泛化能力，与现有训练流程兼容
...
TL;DR: 该立场论文以巴拉圭官方语言瓜拉尼语为案例，指出当前AI系统以文本为中心的设计忽视了口语优先的语言和原住民社区，提出了一种口语优先的多智能体架构作为替代方案。


<details>
  <summary>Details</summary>
Motivation: 当前人工智能和人机交互系统通常作为通用解决方案呈现，但其设计仍以文本为中心，未能充分服务以口语为主的语言和原住民社区。作者认为，除非AI的语言支持与真实的口语实践相一致，否则这种支持仍然不足。

Method: 提出一种口语优先的多智能体架构，替代标准的"文本到语音"流程。该架构将瓜拉尼语的自然语言理解与专门的对话状态智能体和社区主导的治理智能体解耦，创建了一个尊重原住民数据主权和双语现象的技术框架。

Result: 论文展示了一个技术框架，该框架超越了单纯的语言识别，将对话轮转、修复和共享上下文作为交互的主要焦点。通过将口语对话视为一等设计需求，确保数字生态系统能够赋能而非忽视多样化的语言实践。

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适应文本中心系统转变为将口语对话视为一等设计需求。只有这样，数字生态系统才能赋能而非忽视多样化的语言实践，尊重原住民数据主权和双语现象。
...
TL;DR: NERdME是一个包含200个手动标注README文件的数据集，包含超过10,000个标注跨度和10种实体类型，用于解决学术信息提取在代码仓库实现层面细节的不足。
...
Method: 创建了NERdME数据集：包含200个手动标注的README文件，超过10,000个标注跨度和10种实体类型。使用大型语言模型和微调的transformer模型进行基线实验，并进行下游实体链接实验。

Result: 基线结果显示论文层面和实现层面实体之间存在明显差异，表明扩展SIE基准测试以包含README文件中可用的实体类型具有价值。实体链接实验证明从README中提取的实体可以支持工件发现和元数据集成。

Conclusion: NERdME数据集填补了学术信息提取在实现层面细节的空白，README文件中的实体信息对于工件发现和元数据集成具有重要价值，扩展SIE基准测试以包含实现层面实体是必要的。
...
<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含关于患者生活经历、社会环境和护理参与的关键信息，这些因素强烈影响依从性、护理协调和健康公平。然而这些"患者声音"信号很少以结构化形式可用，限制了其在患者为中心的结果研究和临床质量改进中的应用。可靠提取此类信息对于大规模理解和解决健康结果的非临床驱动因素至关重要。
...
Conclusion: PVminerLLM能够实现患者生成文本中嵌入的社会和经验信号的大规模分析，为患者为中心的结果研究和临床质量改进提供了有效工具。代码和模型将公开发布，标注数据集可供研究使用。
...
TL;DR: 开发了一个用于大规模分析辅导对话的辅导行为分类法，通过混合演绎-归纳过程构建，包含四个主要类别，特别关注学习支持中的学生参与度区分。


<details>
  <summary>Details</summary>
Motivation: 需要系统分析辅导过程中教师的教学行为，以理解辅导的有效性，并为国家辅导观测站提供大规模辅导对话分析工具。

Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统的研究构建初步框架，然后通过专家标注者对真实辅导转录文本的迭代编码来精炼分类法。

Result: 开发了一个包含四个类别的辅导行为分类法：辅导支持、学习支持、社会情感与动机支持、后勤支持。学习支持进一步按学生参与度区分，包括引发学生推理和提供直接解释/答案的行为。
...
TL;DR: RouteGoT：一种预算可控、节点自适应的图结构推理路由框架，通过动态分配不同模型到不同推理节点，在保持或提升准确率的同时显著降低token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确率，但通常引入大量token消耗和延迟，且收益不稳定。作者发现图推理管道中存在阶段和节点异质性：高质量规划和最终综合需要强模型，而许多中间子任务可由轻量模型准确解决。

Method: 提出RouteGoT框架，包含：1）图内路由机制，优先为规划和综合分配强模型，同时基于预测难度为叶子子任务动态分配轻量模型和成本效益策略；2）集成显式预算约束的全局推理调度器，在用户指定token预算下控制图扩展，实现可预测的性能-成本权衡。

Result: 在推理、检索和多跳QA基准测试中，RouteGoT匹配或提高了准确率，同时显著减少token使用：相比AGoT平均提升8.1个百分点的准确率，减少79.1%的输出token。在变化预算目标和任务下，RouteGoT保持更优的成本-准确率权衡，展现出更好的鲁棒性。
...
TL;DR: HART是一个细粒度幻觉归因与证据检索框架，通过结构化建模解决LLM幻觉问题，包含跨度定位、机制归因、证据检索和因果追踪四个阶段，并构建首个结构化幻觉追踪数据集。
...
Method: 提出HART框架，将幻觉追踪形式化为包含四个阶段的结构化建模任务：1)跨度定位，2)机制归因，3)证据检索，4)因果追踪。基于此构建首个结构化幻觉追踪数据集，联合标注幻觉类型、错误机制和反事实证据集。
...
TL;DR: ReflexiCoder是一个基于强化学习的代码生成框架，通过内部化结构化推理轨迹（包括初始生成、错误与优化感知的反思、自我修正），使模型在推理时具备完全自主的自我反思和修正能力，无需外部反馈或执行引擎。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代码生成方法（"System 1"单次前向生成）在复杂算法任务上遇到性能瓶颈。现有的迭代优化策略主要依赖外部反馈、执行结果或计算昂贵的提示-响应循环，缺乏完全自主的内在反思能力。

Method: 提出ReflexiCoder强化学习框架，使用RL-zero训练范式配合细粒度奖励函数，优化整个反思-修正轨迹。模型学习如何调试，无需在推理时依赖真实反馈或执行引擎，将结构化推理轨迹内部化到模型权重中。

Result: ReflexiCoder-8B在7个基准测试中创下新SOTA：HumanEval 94.51%（Plus 87.20%）、MBPP 81.80%（Plus 78.57%）、BigCodeBench 35.00%、LiveCodeBench 52.21%、CodeForces 37.34%。推理计算开销减少约40%，token效率显著提升。

Conclusion: ReflexiCoder实现了从外部依赖优化到内在自主反思-修正的范式转变，在代码生成任务上达到或超越GPT-5.1等专有模型性能，同时显著提高计算效率，为LLM代码生成提供了新的研究方向。
...
TL;DR: ROSE提出了一种基于重排序的SparseGPT剪枝方法，通过优先剪枝潜在误差较大的权重，解决了原始SparseGPT左到右剪枝顺序在权重呈现列模式时的次优问题。


<details>
  <summary>Details</summary>
Motivation: SparseGPT等基于Hessian的二阶梯度剪枝方法虽然有效，但其预定义的左到右剪枝顺序在权重呈现列模式时会导致性能次优。本文研究剪枝顺序对SparseGPT框架的影响，旨在通过优化剪枝顺序提升剪枝效果。

Method: ROSE方法首先进行预剪枝识别候选剪枝权重，估计列损失和块损失。然后执行两级重排序：1）每个块内按列损失降序重排列；2）基于块损失重排块。引入块损失的相对范围作为指标识别列状层，实现整个模型的自适应重排序。

Result: 在主流LLM（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的大量实验结果表明，ROSE超越了原始SparseGPT和其他对比剪枝方法，证明了重排序策略的有效性。

Conclusion: 剪枝顺序对基于Hessian的LLM剪枝方法有重要影响。ROSE通过自适应重排序机制，优先剪枝潜在误差较大的权重，显著提升了剪枝性能，为高效LLM部署提供了更优的剪枝方案。
...
TL;DR: CoCA提出了一种置信度优先的LLM不确定性估计框架，通过GRPO强化学习联合优化置信度校准和答案准确性，使用分段信用分配避免奖励攻击。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法主要是答案优先的，即先生成答案再给出置信度，这种方法只能衡量特定回答的正确性，限制了实际可用性。需要一种置信度优先的范式，让模型在回答前输出置信度，作为模型在当前策略下正确回答问题的概率。

Method: 提出CoCA（协同优化的置信度和答案）框架，基于GRPO强化学习，通过分段信用分配联合优化置信度校准和答案准确性。该方法分别为置信度段和答案段分配独立的奖励和组相对优势，实现稳定的联合优化并避免奖励攻击。

Result: 在数学、代码和事实问答基准测试中，CoCA显示出改进的校准性和不确定性区分能力，同时保持了答案质量，从而支持更广泛的下游应用。

Conclusion: CoCA框架通过置信度优先的范式，结合GRPO强化学习和分段信用分配，实现了LLM置信度校准和答案准确性的联合优化，为可靠部署提供了更好的不确定性估计方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的分词方法BPE及其变体本质上是脚本无关的，主要针对英语等屈折语优化，无法有效处理黏着语的特点

Method: 论文未在摘要中明确说明具体方法，但指出了BPE分词在黏着语处理中的问题

Result: 统计分词器会将黏着语的单词碎片化为字节对块，破坏语素边界并导致token数量膨胀

Conclusion: 需要针对黏着语特点设计专门的分词方法，以更好地处理这类语言的形态学特性
...
TL;DR: 提出了ConStory-Bench基准测试，用于评估长文本故事生成中的叙事一致性，包含2000个提示和5类19种子类型的错误分类，并开发了自动检测管道ConStory-Checker。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇叙事时经常出现前后不一致的问题，如自相矛盾的事实、人物特征和世界规则。现有故事生成基准主要关注情节质量和流畅性，对一致性错误的研究不足。

Method: 创建了ConStory-Bench基准测试，包含2000个提示，涵盖四种任务场景，定义了5个错误类别和19个细分子类型。开发了ConStory-Checker自动管道，用于检测矛盾并为每个判断提供明确的文本证据。

Result: 评估多个LLM后发现：一致性错误在事实和时间维度最常见；倾向于出现在叙事中间部分；发生在token级熵较高的文本段中；某些错误类型倾向于共现。

Conclusion: 这些发现可以为改进长文本叙事生成的一致性提供指导。研究揭示了LLM在长文本生成中的一致性模式，为未来改进提供了实证基础。
...
TL;DR: 本文提出了一种基于LLM的集成系统方法，用于联合国安理会决议的语义标注，通过内容保留率和标签格式良好性两个评估指标来选择最佳输出，GPT-4.1在两项任务中表现最佳，而GPT-4.1-mini以20%的成本实现了可比性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能变异性构建集成系统，用于数据清洗和语义标注任务，解决LLM在处理挑战性文本时可能出现的幻觉、不必要添加或遗漏问题，确保语义标注的准确性和可靠性。

Method: 提出基于LLM的集成系统方法，引入内容保留率(CPR)和标签格式良好性(TWF)两个评估指标，通过多个GPT模型的多轮运行选择最佳输出，使用集成设计来优化清洗和标注内容。

Result: GPT-4.1在两项任务中表现最佳：数据清洗任务CPR为84.9%，语义标注任务CPR为99.99%、TWF为99.92%。较小的GPT-4.1-mini模型以20%的成本实现了与最佳模型相当的性能。集成系统能够为所有LLM模型和多次运行选择最优输出。

Conclusion: 通过集成设计和评估指标的应用，创建了一个可靠的LLM系统，能够在挑战性文本上执行语义标注任务，同时平衡了性能与成本效益。
...
TL;DR: 论文提出下一代动作预测（NAP）任务，通过标注360K个手机使用动作，开发LongNAP模型结合参数化和上下文学习来预测用户下一步行为，在LLM评估中显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 真正主动的AI系统需要预测用户的下一步行为，这需要超越稀疏提示信号的丰富信息，需要基于用户与计算机的多模态交互（截图、点击、传感器数据）的完整上下文进行推理。

Method: 1）使用视觉语言模型标注纵向自然计算机使用数据，发布开源标注流程；2）提出LongNAP用户模型，结合参数化和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹，检索相关历史轨迹，并在上下文中应用这些轨迹来预测未来动作。

Result: 标注了20名用户一个月连续手机使用的360K个动作（1800小时屏幕时间）。LongNAP在LLM评估指标（0-1相似度）上显著优于监督微调（提升79%）和提示基线（提升39%）。17.1%的预测轨迹与用户实际行为良好对齐（LLM-judge评分≥0.5），在高度自信预测中达到26%。模型能泛化到未见过的用户。

Conclusion: 通过学习用户行为的完整上下文来预测用户需求已成为可行任务，具有重大机会。LongNAP展示了结合参数化和上下文学习在长交互历史推理中的有效性，为真正主动的AI系统奠定了基础。
...
TL;DR: 该论文探讨了在语言模型训练和推理中考虑作者语言上下文的重要性，通过HuLM（人类感知语言建模）和HuFT（人类感知微调）方法，在8B Llama模型上验证了解决生态谬误能提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型训练和推理忽略了作者语言上下文之间的依赖关系（生态谬误）。先前研究表明，解决这种生态谬误能显著提升较小GPT模型（约124M参数）的性能。本研究旨在探究在更大规模的8B Llama模型中，通过HuLM任务建模作者语言上下文是否也能带来类似性能提升。

Method: 提出两种方法：1）HuLM（人类感知语言建模）：在预训练阶段考虑作者语言上下文，处理作者按时间顺序排列的其他文本；2）HuFT（人类感知微调）：在微调阶段使用作者上下文。实验采用QLoRA技术进行高效微调，比较了标准微调、HuFT微调以及基于QLoRA的HuLM持续预训练。

Result: 实验结果表明：1）在微调阶段解决生态谬误（使用HuFT）能提升8B模型性能；2）基于QLoRA的HuLM持续预训练产生的人类感知模型，在八个下游任务上仅通过线性任务分类器训练就能获得改进性能，显示出良好的泛化能力。

Conclusion: 在语言模型训练中考虑作者语言上下文（解决生态谬误）对于提升大规模模型性能具有重要价值。建模语言在其原始生成者（作者）上下文中的方法具有实用性和重要性，为语言模型发展提供了新方向。
...
TL;DR: 提出结构化风格重写框架，通过显式解耦风格为三个可解释维度（词汇特征、句法模式、语用风格），结合隐式风格条件策略，使小语言模型能够生成高保真度的风格化内容，在动漫角色领域超越更大基线的表现。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在角色扮演中面临数据稀缺和风格解耦复杂性的挑战，标准监督微调只能捕捉表层语义而无法重现角色的句法和语用细微差别，导致"角色出戏"的生成问题。

Method: 提出结构化风格重写框架：1) 将风格显式解耦为三个维度：词汇特征（通过PMI）、句法模式（基于PCFG规则）和语用风格；2) 引入隐式风格条件策略，通过思维链蒸馏在训练中利用显式推理轨迹作为强归纳偏置，使模型潜在表示与结构化风格特征对齐，推理时无需显式推理标记。

Result: 在动漫角色这一高风格化领域的广泛实验中，该方法使Qwen-1.7B模型在风格一致性和语义保真度上显著超越更大的基线模型（如4B标准监督微调），实现了数据高效范式。

Conclusion: 该方法为在消费级硬件上实现推理和部署提供了数据高效的范式，通过结构化风格解耦和隐式条件策略，解决了小语言模型在高度风格化角色扮演中的挑战。
...
TL;DR: 开发可解释模型预测幸福感并识别自适应/非自适应自我状态，整合人格特质与情境特征，理论驱动特征在保持可解释性同时具有竞争力


<details>
  <summary>Details</summary>
Motivation: 心理健康是动态过程而非固定特质，受个体倾向与情境交互影响。现有研究缺乏将心理学理论与计算建模结合来理解动态心理状态的方法

Method: 整合人格层面心理特质（韧性、认知扭曲、内隐动机）与基于Situational 8 DIAMONDS框架的语言推断情境特征，比较理论驱动特征与心理测量学语言模型嵌入

Result: 理论驱动特征在保持可解释性同时提供竞争力性能，定性分析显示预测幸福感的特征具有心理一致性
...
Conclusion: Track-SQL通过双提取模块有效解决了生成式语言模型在多轮Text-to-SQL中的局限性，显著提升了上下文理解和模式链接能力
...
TL;DR: ViewFusion：两阶段多视图空间推理框架，通过显式分离跨视图空间预对齐与问答，解决现有模型依赖单视图捷径的问题
...
Method: 提出ViewFusion两阶段框架：第一阶段进行空间预思考，推断跨视图的视角关系和空间变换，形成超越简单重描述的中间工作空间；第二阶段基于该工作空间进行问题驱动的推理生成最终预测。使用合成推理监督训练，随后通过GRPO进行强化学习

Result: 在MMSI-Bench上，ViewFusion相比Qwen3-VL-4B-Instruct准确率提升5.3%，在需要真正跨视图对齐的案例上提升最大

Conclusion: 显式分离跨视图空间预对齐与问答的框架能有效提升多视图空间推理性能，特别是在需要跨视图对齐的复杂案例中
...
TL;DR: 该研究评估了四种开源大语言模型（DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b、LLama3.3 70b）在奥地利德语A级考试作文自动评分中的应用效果，发现模型与人类评分者在细则维度上的最大一致性仅为40.6%，最终成绩匹配率仅32.8%，表明当前模型在实际评分环境中准确性不足。


<details>
  <summary>Details</summary>
Motivation: 自动化作文评分（AES）旨在通过减少教师评分工作量并减轻主观偏见来支持教学。虽然早期系统依赖手工特征和统计模型，但大语言模型（LLMs）的最新进展为评估学生写作提供了前所未有的灵活性。本研究特别关注将最先进的开源权重LLMs应用于奥地利德语A级考试文本的评分，重点考察基于评分细则的评估方法。

Method: 研究处理了包含101份匿名学生考试文本的数据集，涵盖三种文本类型。评估了四种LLMs模型（DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b、LLama3.3 70b），采用不同的上下文和提示策略。研究重点考察模型使用标准化评分细则进行德语作文评分的能力。

Result: LLMs在评分细则提供的子维度上与人类评分者的最大一致性为40.6%，最终成绩与人类专家给出的成绩匹配率仅为32.8%。结果表明，尽管较小模型能够使用标准化评分细则进行德语作文评分，但其准确性不足以在实际评分环境中使用。

Conclusion: 即使较小的模型能够使用标准化评分细则进行德语作文评分，其准确性仍不足以在实际评分环境中应用。当前的开源大语言模型在自动化作文评分任务中仍存在显著局限性，需要进一步改进才能达到实际应用标准。
...
TL;DR: 该研究通过持续预训练让大语言模型接触领域特定文本，模拟经验积累过程，发现模型能力呈双峰分布，存在"表达型通才"和"抑制型专才"两种最优类型，并揭示了"抑制优势"现象。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型开发过度依赖统一性能基准，偏向特定行为倾向（如自信），忽略了人类问题解决中多样化的风格和个性特质。研究旨在探索不同经验如何塑造机器个性并影响问题解决能力。

Method: 采用持续预训练方法，让模型以无监督方式接触领域特定文本，模拟经验积累过程。使用基于大五人格框架的机器人格量表量化模型变体的人格特质，分析其与语言风格和推理行为的关系。

Result: 发现模型能力呈双峰分布：在"表达型通才"和"抑制型专才"处达到峰值。识别出"抑制优势"现象——降低社交特质能提升复杂推理性能。建立了训练数据语言学特征（如祈使句频率、词汇多样性）与模型个性之间的因果关系。

Conclusion: 研究为"个性工程"提供了路线图，表明通过精心设计训练数据可以系统性地塑造模型个性，从而优化特定任务性能，实现更符合人类多样化问题解决风格的AI系统。
...
TL;DR: 该论文提出了一种处理省略三段论（enthymemes）的集成方法，结合LLM生成隐含前提、翻译逻辑公式和神经符号推理器验证逻辑蕴含。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提或主张是隐含的）。现有NLP方法能识别省略三段论但无法解码其底层逻辑，而基于逻辑的方法需要充足的知识库公式通过溯因推理解码。缺乏系统方法将省略三段论的文本组件转化为逻辑论证并生成解码所需的逻辑公式。

Method: 提出一个集成管道：1）使用大语言模型基于显式前提和主张生成中间隐含前提；2）使用另一个LLM将自然语言翻译为逻辑公式；3）基于SAT求解器的神经符号推理器确定逻辑蕴含。

Result: 在两个省略三段论数据集上评估管道，在精确率、召回率、F1分数和准确率方面显示出选择正确隐含前提的优异性能。

Conclusion: 该集成方法有效解决了省略三段论处理中文本到逻辑转换和逻辑蕴含验证的系统性问题，为现实世界论证的逻辑分析提供了实用解决方案。
...
Method: 提出基于图的框架：1) 将叙事表示为实体交互图；2) 在节点级引入因果估计，将构建的句子图提炼为最小因果子图，获得每个贡献的因果表示；3) 基于此表示构建分类流程

Result: 提出的分类流程在对立思维分类任务上超越了现有方法

Conclusion: 通过实体交互图和因果估计的图框架能更好地建模对立叙事中的结构化交互，提供更透明和因果可解释的分类方法
...
TL;DR: CRIMSON是一个基于临床的胸部X光报告生成评估框架，通过诊断正确性、上下文相关性和患者安全性来评估报告质量，并引入临床显著性分级和错误分类法。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标未能充分考虑完整的临床上下文，且常让正常或临床不显著的发现对总分产生不成比例的影响，缺乏对临床重要错误的优先级区分。

Method: CRIMSON整合患者年龄、指征和基于指南的决策规则等完整临床上下文，建立包含假阳性发现、遗漏发现和八种属性级错误的综合分类法，并为每个发现分配临床显著性等级（紧急、可行动非紧急、不可行动、预期/良性），实现基于严重程度的加权评估。

Result: CRIMSON在ReXVal上与六名认证放射科医生标注的临床显著错误数量高度一致（Kendall's tau=0.61-0.71；Pearson's r=0.71-0.84）；在RadJudge临床挑战场景中与专家判断一致；在RadPref偏好基准中与放射科医生偏好最匹配。

Conclusion: CRIMSON提供了一个临床基础的评估框架，能更准确地评估胸部X光报告生成质量，优先考虑临床重要错误，并发布了相关基准和模型以支持可重复评估。
...
TL;DR: MAPO：一种无评论家、高效的强化学习算法，通过法官模型提供密集过程反馈和蒙特卡洛回报传播长时程效应，结合混合优势估计器实现细粒度信用分配，在主观多轮对话任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 主观多轮对话任务（如情感支持）需要适应不断变化的用户状态并优化长时程交互质量的对话策略。然而，由于缺乏可靠的过程监督，强化学习在此类场景中面临挑战：仅基于结果的训练将多轮信用分配压缩为单一轨迹级奖励，而简单的轮级分组采样在交互环境中产生过高的计算成本。

Method: 提出MAPO算法：1）利用法官模型提供密集过程反馈；2）通过蒙特卡洛回报传播长时程效应；3）引入混合优势估计器，结合轮级归一化和批次级归一化，实现细粒度且可扩展的信用分配；4）无需评论家模型，提高训练效率。

Result: 在多个主观对话基准测试（EMPA、EmoBench、EQ-Bench）和不同模型规模（7B到32B）上，MAPO在训练稳定性和最终性能上均优于仅基于结果的GRPO和单级归一化基线。在EMPA上，成功率提升高达9个百分点，对话分数比7B基础模型提高+43.2。尽管仅在EMPA风格环境中训练，该方法在未见的情感智能基准测试上表现出良好泛化能力，在EmoBench上提升高达+4分，在EQ-Bench上提升+3.5分。

Conclusion: 密集过程监督与混合级归一化相结合，能够实现主观、开放式多轮对话的有效且可扩展的强化学习。该方法解决了长时程信用分配问题，提高了训练稳定性，并在多个基准测试和模型规模上表现出一致的性能改进和良好的泛化能力。
...
TL;DR: AI-CROWD协议通过集成多个大语言模型的输出生成共识标签，近似真实标注，解决大规模内容分析中缺乏人工标注基准的问题。
...
Method: AI-CROWD协议利用大语言模型集合的集体输出近似真实标注，通过多数投票聚合输出，并通过诊断指标分析一致/分歧模式，识别高置信度分类并标记潜在模糊性或模型特定偏差。

Result: 该协议生成基于共识的近似标注，而非声称是真实标注，通过收敛和发散推理获得结果，能够识别高置信度分类并标记潜在问题。

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过大语言模型集合的共识输出近似真实标注，克服了人工标注的局限性。
...
TL;DR: LIT-RAGBench是一个新的RAG生成器基准测试，涵盖集成、推理、逻辑、表格和弃权五大能力类别，使用虚构实体和场景评估模型在实际RAG部署中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖能力有限，无法在统一条件下同时评估多种实际应用所需的关键能力（如长上下文整合、多步推理、表格解释、证据缺失时弃权等），需要更全面的评估框架。

Method: 构建包含114个人工构建的日语问题（含机器翻译英文版）的数据集，定义五大评估类别（集成、推理、逻辑、表格、弃权），每个类别细分为实际评估方面，使用LLM-as-a-Judge进行评分，报告类别和整体准确率。

Result: 在API和开源模型测试中，没有模型整体准确率超过90%。基准测试能够清晰测量各模型在不同类别中的优势和弱点，为实际RAG部署中的模型选择和专用模型构建提供有价值指标。

Conclusion: LIT-RAGBench填补了现有评估与实际应用之间的差距，通过系统覆盖多类别组合模式，成为RAG生成器评估的全面基准，有助于模型选择和RAG专用模型的开发。
...
TL;DR: FlashPrefill是一个通过即时模式发现和阈值化实现超快速预填充的框架，在256K序列上达到27.78倍加速，在4K序列上仍保持1.71倍加速。


<details>
  <summary>Details</summary>
Motivation: 长上下文建模是大型语言模型的关键能力，但注意力机制的二次复杂度在计算密集的预填充阶段成为关键瓶颈。现有稀疏注意力机制通常存在搜索延迟过高或稀疏性不足的问题。
...
Conclusion: FlashPrefill通过创新的模式发现和阈值化技术，显著提升了长上下文建模的预填充效率，解决了现有稀疏注意力方法的局限性，为大规模序列处理提供了高效解决方案。
...
TL;DR: SPOT框架通过将显式思维链压缩为紧凑的潜在暂停标记，使用跨段语义对齐和冻结头部解码约束，在减少推理成本的同时提升推理性能并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 显式思维链推理虽然能提升大语言模型的推理性能，但会产生高推理成本（冗长的token级追踪）。现有方法主要通过简洁提示或步骤剪枝来减少开销，但这些方法主要截断模型输出而非内化模型思考。潜在推理提供了有前景的替代方案，但现有方法面临两个关键挑战：1) 依赖刚性的点对点对齐，难以捕捉整个推理段的密集、变长语义；2) 缺乏可解释性，潜在状态难以解码或审计。

Method: 提出SPOT框架，将显式思维链压缩为紧凑的潜在暂停标记，不强制固定响应模板。核心包括：1) 跨段语义对齐：使用Sinkhorn最优传输目标，将每个暂停标记软匹配到整个推理段的语义，克服步骤末端对齐的刚性；2) 冻结头部解码约束：保持潜在状态在冻结的预训练LM头部下可直接解码为token分布，实现对潜在思考的可读关键词解释。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成token数量37.5%，并为潜在推理过程提供忠实的语义解释。

Conclusion: SPOT框架通过跨段语义对齐和冻结头部解码约束，有效解决了潜在推理中的语义对齐刚性和可解释性不足问题，在减少推理成本的同时提升推理性能，并保持潜在思考过程的可解释性。
...
TL;DR: 该论文对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代大语言模型在印度、东亚和东南亚地区的文化对齐性进行了多语言审计，发现这些模型在宗教等敏感领域存在显著的文化错位，特别是对少数群体的宗教观点存在系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地部署在多语言、多文化环境中，但其主要依赖英语中心训练数据，可能导致与不同社会的多元文化价值观错位。研究者希望通过系统审计来评估LLMs在敏感文化领域（特别是宗教）的对齐程度。

Method: 采用多语言审计方法，通过log-probs/logits分析每个LLM的内部表征，将模型的观点分布与真实公众态度进行对比。研究聚焦印度、东亚和东南亚地区，以宗教作为文化对齐的棱镜。同时测试了人口统计提示和母语提示等轻量级干预措施的效果。

Result: 研究发现：1）流行模型在广泛社会问题上与公众意见基本一致，但在宗教观点上持续失败；2）模型无法准确代表少数群体的宗教观点，经常放大负面刻板印象；3）轻量级干预措施（人口统计提示和母语提示）只能部分缓解但无法消除文化差距；4）在偏见基准测试（CrowS-Pairs、IndiBias、ThaiCLI、KoBBQ）中显示出持续的危害和敏感情境下的代表性不足。

Conclusion: 研究结果强调了进行系统性、基于区域的审计以确保LLMs在全球部署中公平性的紧迫需求。当前LLMs在文化对齐方面存在显著缺陷，特别是在宗教等敏感领域，需要更深入的文化敏感性和代表性改进措施。
...
TL;DR: 研究探索ChatGPT模仿艺术家原创作品生成图像的能力，发现AI生成的模仿作品在色彩纹理相似度与构图概念感知相似度之间存在显著差距，建议使用多指标"风格转换仪表板"而非单一指标评估相似性。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉创意领域的潜力，特别是ChatGPT能否有意模仿原创艺术作品（绘画、素描、雕塑、装置等）生成新的图像，并评估AI生成作品与原始作品之间的相似性和艺术价值。

Method: 邀请来自罗马尼亚、保加利亚、法国、奥地利和英国的12位艺术家，每人提供3件原创作品；使用ChatGPT生成这些作品的模仿版本；结合人类评估（艺术家评分和评论）与计算方法（检测视觉和风格相似性或差异）进行综合分析。

Result: 研究发现色彩和纹理相似度与构图、概念和感知相似度之间存在显著差距；艺术家评论指出ChatGPT生成的当代艺术作品模仿品缺乏维度感、背景和意图感，更像是释义或近似引用而非能引发情感的有价值艺术品。

Conclusion: 建议使用包含互补指标的"风格转换仪表板"来评估模仿作品与原始作品的相似性，而非依赖单一风格指标；AI生成的模仿作品在艺术深度和情感表达方面仍有明显局限性。
...
TL;DR: 该研究将数学问题理解建模为数学实体关系抽取任务，使用Transformer模型实现99.39%的准确率，并引入可解释AI增强模型透明度。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为存在专业实体和复杂关系。需要开发能够自动提取数学实体关系的系统，以支持自动化问题解决、知识图谱构建和教育系统。

Method: 将数学问题解释建模为数学实体关系抽取任务，操作数作为实体，运算符作为关系。应用基于Transformer的模型自动提取关系，BERT表现最佳。引入SHAP进行可解释性分析，揭示特征重要性。

Result: BERT模型达到99.39%的最高准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的深入理解。
...
TL;DR: 大语言模型在条件推理任务中表现出与人类相似的领域特异性：在道义规则上推理更好，并显示出匹配偏误的错误模式


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型语言能力提升，其推理能力受到关注。人类推理在领域特定情境中表现更好，特别是在规范性而非纯形式语境中。先前研究比较了LLM与人类推理，但LLM推理的领域特异性仍未充分探索。本研究旨在系统考察LLM在道义规则下的条件推理能力。

Method: 引入新的Wason选择任务数据集，明确编码道义模态以系统区分道义条件句与描述性条件句。使用该数据集检验LLM在道义规则下的条件推理。进一步分析观察到的错误模式是否更好地由确认偏误（寻求支持规则的证据倾向）或匹配偏误（忽略否定并选择与规则词汇匹配项的倾向）解释。

Result: 结果显示，与人类相似，LLM在道义规则上推理更好，并显示出类似匹配偏误的错误。这表明LLM的性能在不同规则类型间存在系统性差异，其错误模式可以与该范式中已知的人类偏误相平行。

Conclusion: LLM的条件推理表现出与人类相似的领域特异性：在道义规则上表现更好，并显示出匹配偏误的错误模式。这表明LLM的推理能力在不同规则类型间存在系统性差异，其错误模式可以反映人类认知偏误。
...
TL;DR: 该研究对大型语言模型在英语作为第二语言写作自动评分中的四种主要范式进行了首次统一比较，发现集成k-SFT和RAG的方法在雅思写作任务2上达到93%的F1分数，揭示了不同方法在准确性、成本和鲁棒性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 先前研究通常孤立地考察各种LLM技术，缺乏对它们在英语作为第二语言写作自动评分中相对优势的系统比较。为了填补这一空白，研究者旨在建立一个统一基准，全面评估主要LLM-based AES范式在IELTS写作任务2上的表现。

Method: 研究在统一基准上评估了四种LLM-based AES方法：(1)基于编码器的分类微调；(2)零样本和少样本提示；(3)指令调优和检索增强生成；(4)监督微调结合直接偏好优化和RAG。研究使用IELTS写作任务2作为评估数据集。

Result: 结果显示不同方法在准确性、成本和鲁棒性之间存在明显权衡。最佳配置（集成k-SFT和RAG）取得了最强的整体结果，F1分数达到93%。这为英语L2写作自动评分提供了首个现代LLM策略的统一实证比较。

Conclusion: 该研究首次对现代LLM-based AES策略在英语作为第二语言写作评分中进行了统一实证比较，揭示了不同方法的性能权衡。集成k-SFT和RAG的方法表现出最佳整体性能，为自动评分写作任务提供了有前景的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究显示LLMs与人类共享相似的推理偏见，但批评LLMs可能不公平，因为人类推理不仅涉及形式演绎，还包括基于有限信息的溯因推理。需要探究LLMs在溯因推理中的表现，理解其与人类认知的差异

Method: 将三段论数据集转换为适合溯因推理的形式，评估最先进的大语言模型在转换后的溯因任务上的表现，分析其是否存在偏见

Result: 研究发现LLMs在溯因推理中表现出特定的偏见模式，揭示了模型在超越形式演绎的语境化推理方面的局限性

Conclusion: LLMs在溯因推理中存在系统性的偏见，需要改进其语境化推理能力以更好地模拟人类认知，这对理解机器与人类认知差距至关重要
...
TL;DR: PONTE是一个个性化可解释AI框架，通过闭环验证和适应过程生成可信的自然语言解释，解决了传统XAI方法忽视用户差异以及大语言模型存在幻觉的问题。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI方法大多采用"一刀切"范式，忽视了用户在专业知识、目标和认知需求方面的差异。虽然大语言模型可以将技术解释转化为自然语言，但存在忠实性和幻觉问题。需要一种能够适应不同用户需求并确保解释可靠性的个性化方法。

Method: PONTE是一个人在环框架，将个性化建模为闭环验证和适应过程而非提示工程。包含三个核心组件：(1)低维偏好模型捕捉风格要求；(2)基于结构化XAI工件的偏好条件生成器；(3)验证模块确保数值忠实性、信息完整性和风格对齐，可选支持检索增强论证。用户反馈迭代更新偏好状态，实现快速个性化。

Result: 在医疗和金融领域的自动和人工评估显示，验证-精炼循环显著提高了完整性和风格对齐性。人工研究进一步证实：预期偏好向量与感知风格之间高度一致；对生成随机性具有鲁棒性；质量评估持续积极。

Conclusion: PONTE通过将个性化建模为闭环验证过程，有效解决了可解释AI中的个性化需求和可靠性问题，为生成可信赖的自然语言解释提供了系统化框架。
...
TL;DR: 提出了一种支持多语言和口音、结合上下文感知的自动语音识别框架，通过轻量级投影模块连接冻结的语音编码器和仅解码器语言模型，并采用对比学习对齐语音与上下文表示。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语言设置和短孤立话语，而现有的上下文感知ASR存在两个关键挑战：有限的多语言支持，以及缺乏语音与上下文表示之间的原则性对齐。

Method: 通过轻量级投影模块将冻结的语音编码器和仅解码器语言模型结合，支持结构化上下文提示（包括对话历史和偏置词）引导转录。采用对比学习目标在共享嵌入空间中对齐语音和上下文表示。

Result: 在11种语言和5种英语方言的超过1,500小时真实世界对话语音评估中，上下文输入持续改善识别质量。对比对齐在不同上下文类型上提供额外增益，总体性能提升超过5%。

Conclusion: 上下文建模和跨模态对齐在多语言ASR中都具有重要性，所提框架在保持预训练模型模块性的同时，有效支持多语言和口音识别。
...
<details>
  <summary>Details</summary>
Motivation: 参与SemEval 2026的CLARITY共享任务，该任务旨在对政治话语中的歧义和回避技术进行分类，研究不同建模方法在理解政治话语策略方面的有效性。

Method: 研究了两种建模方法：(1) 直接预测清晰度标签；(2) 先预测回避标签，然后通过任务分类层次结构推导清晰度。还探索了多种辅助训练变体，并在回避优先的框架下评估了仅解码器模型的零样本性能。

Result: 两种建模方法表现相当。在基于编码器的模型中，RoBERTa-large在公共测试集上取得了最佳结果；而在隐藏评估集上，零样本GPT-5.2表现出更好的泛化能力。

Conclusion: 两种建模方法在政治话语歧义和回避技术分类任务中表现相似，不同模型在不同数据集上各有优势，为理解政治话语策略提供了有效的技术框架。
...
### [171] [Information-Theoretic Privacy Control for Sequential Multi-Agent LLM Systems](https://arxiv.org/abs/2603.05520)
*Sadia Asif,Mohammad Mohammadi Amiri*

Main category: cs.MA

TL;DR: 该论文研究了顺序多智能体LLM系统中的组合隐私泄露问题，提出了基于互信息的形式化分析框架和隐私正则化训练方法，证明隐私需要作为系统级属性处理。


<details>
  <summary>Details</summary>
Motivation: 顺序多智能体LLM系统在医疗、金融等敏感领域部署时，尽管单个智能体可能满足本地隐私约束，但敏感信息仍可能通过顺序组合和中间表示被推断出来。现有方法未能充分解决组合隐私泄露问题。

Method: 使用互信息形式化组合隐私泄露，推导了本地泄露在顺序执行下如何放大的理论边界。提出了隐私正则化训练框架，直接约束智能体输出与本地敏感变量之间的信息流。

Result: 在三个基准数据集上评估了不同深度的顺序智能体管道，展示了稳定的优化动态和一致、可解释的隐私-效用权衡。结果表明本地约束无法保证系统级隐私。

Conclusion: 智能体LLM系统中的隐私不能仅通过本地约束保证，必须在训练和部署期间作为系统级属性处理。提出的框架为构建隐私保护的顺序智能体系统提供了理论基础和实践方法。
...
### [172] [The Coordination Gap: Alternation Metrics for Temporal Dynamics in Multi-Agent Battle of the Exes](https://arxiv.org/abs/2603.05789)
*Nikolaos Al. Papadopoulos,Konstantinos Psannis*

Main category: cs.MA

TL;DR: 论文提出交替(ALT)指标来评估多智能体协调质量，发现传统指标会掩盖时间结构问题，Q学习策略在交替协调上表现比随机策略差81%


<details>
  <summary>Details</summary>
Motivation: 多智能体协调中存在个体优化与集体福利之间的根本矛盾，传统基于结果的指标（如效率和公平性）对时间结构不敏感，无法区分结构化交替与垄断或随机访问模式，公平性比率随智能体数量增加失去区分能力

Method: 引入完美交替(PA)作为参考协调机制，提出六种新颖的交替(ALT)指标作为时间敏感的协调质量观测指标；使用Q学习智能体作为最小自适应诊断基线，并与随机策略零过程进行比较

Result: 发现明显的测量失败：尽管传统指标显示高公平性（常超过0.9），但学习策略在ALT变体评估下表现比随机基线差81%；这种缺陷在两智能体情况下已存在，并随智能体数量增加而加剧

Conclusion: 高总回报可能与差的时间协调共存，传统指标可能严重误判涌现动态；强调时间感知观测指标对分析多智能体游戏协调的必要性，随机策略基线作为解释协调结果相对于机会水平行为的重要零过程
...
### [173] [Evaluating LLM Alignment With Human Trust Models](https://arxiv.org/abs/2603.05839)
*Anushka Debnath,Stephen Cranefield,Bastin Tony Roy Savarimuthu,Emiliano Lorini*

Main category: cs.MA

TL;DR: 对EleutherAI/gpt-j-6B模型进行白盒分析，研究其内部如何表征和推理信任概念，发现其信任表征与Castelfranchi社会认知模型最为一致


<details>
  <summary>Details</summary>
Motivation: 信任在人类交互和多智能体系统中至关重要，但目前对大型语言模型内部如何概念化和推理信任的理解有限。本研究旨在通过白盒分析方法探索LLM内部对信任的表征方式。

Method: 使用对比提示在LLM激活空间中生成信任及相关人际关系属性的嵌入向量。首先从五个成熟的人类信任模型中识别信任相关概念，通过计算60个一般情感概念的成对余弦相似度确定显著概念对齐阈值，然后测量LLM内部信任表征与推导出的信任相关概念之间的余弦相似度。

Result: EleutherAI/gpt-j-6B的内部信任表征与Castelfranchi社会认知模型最为一致，其次是Marsh模型。这表明LLM在其激活空间中编码了社会认知结构。

Conclusion: LLM以支持有意义的比较分析的方式编码社会认知结构，这有助于社会认知理论的发展，并为人类-AI协作系统的设计提供支持。
...
### [174] [RoboLayout: Differentiable 3D Scene Generation for Embodied Agents](https://arxiv.org/abs/2603.05522)
*Ali Shamsaddinlou*

Main category: cs.AI

TL;DR: RoboLayout扩展LayoutVLM，通过集成可达性约束和局部优化改进，生成适用于具身智能体交互的室内场景布局


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理和3D场景布局生成方面虽有进展，但生成的布局往往缺乏对具身智能体交互可行性的考虑，特别是在物理受限的室内环境中，需要生成既语义连贯又可供智能体实际交互的布局

Method: 1. 在LayoutVLM基础上扩展，集成显式的可达性约束到可微分布局优化过程中；2. 引入智能体感知推理，支持多种具身智能体（服务机器人、仓储机器人、不同年龄段人类、动物等）；3. 提出局部细化阶段，选择性重新优化有问题的物体放置，保持场景其余部分固定，提高收敛效率

Result: RoboLayout保持了LayoutVLM的强语义对齐和物理合理性，同时增强了面向智能体中心的室内场景生成的适用性，实验结果表明在不同场景配置下均表现良好

Conclusion: RoboLayout通过集成智能体感知推理和可达性约束，成功扩展了LayoutVLM框架，能够生成既语义连贯又可供多样具身智能体实际交互的室内场景布局，提高了布局生成在具身人工智能应用中的实用性
...
### [175] [Conversational Demand Response: Bidirectional Aggregator-Prosumer Coordination through Agentic AI](https://arxiv.org/abs/2603.06217)
*Reda El Makroum,Sebastian Zwickl-Bernhard,Lukas Kranzl,Hans Auer*

Main category: cs.AI

TL;DR: 本文提出对话式需求响应（CDR），一种基于智能体AI的双向自然语言协调机制，连接聚合商和产消者，在保持自动化可扩展性的同时增强透明度、可解释性和用户自主权。
...
Method: 开发双层多智能体架构：聚合商智能体调度灵活性请求，产消者家庭能源管理系统通过调用基于优化的工具评估可交付性和成本效益。支持双向自然语言交互，包括产消者发起的偏好变更上游通信。

Result: 概念验证评估显示交互在12秒内完成。架构展示了智能体AI如何弥合聚合商-产消者协调差距，在保持自动化需求响应可扩展性的同时，提供透明度、可解释性和用户自主权。

Conclusion: 对话式需求响应通过智能体AI实现双向自然语言协调，解决了现有机制的局限性。所有系统组件（包括智能体提示、编排逻辑和仿真接口）已开源发布，支持可重复性和进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: LLM能将自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决策难以保证这些要求。研究发现工业R&D中存在确定性约束执行与对话灵活性之间的竞争需求

Method: 提出schema-gated orchestration原则：模式作为组合工作流级别的强制执行边界；将需求操作化为执行确定性(ED)和对话灵活性(CF)；使用多模型协议评估20个系统，通过3个LLM家族的15个独立会话进行评分

Result: 多模型LLM评分显示高度一致性（ED的α=0.80，CF的α=0.98），验证了多模型评分可作为专家评估的替代方案；系统分析揭示了经验帕累托前沿，无系统同时实现高灵活性和高确定性，但在生成式和流程中心化极端之间存在收敛区域

Conclusion: schema-gated架构通过分离对话与执行权限，有望解耦灵活性与确定性的权衡，提出三个操作原则：执行前澄清、约束的计划-执行编排、工具到工作流级别的门控
...
### [177] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 论文评估了大型语言模型对思维链（CoT）的控制能力，发现模型对CoT的控制能力显著低于对最终输出的控制能力，且CoT控制能力受模型规模、RL训练、计算资源和问题难度等因素影响。


<details>
  <summary>Details</summary>
Motivation: 思维链监控是检测现代推理模型不当行为和理解其动机的有前景工具，但如果模型能够控制其在思维链中的表达内容，可能会破坏CoT的可监控性。因此需要评估这种不良能力——CoT控制能力。

Method: 引入CoT-Control评估套件，包含要求模型在遵循CoT指令的同时解决问题的任务（如在回答遗传学问题时不使用"染色体"一词）。评估不同模型在激励、监控告知、对抗性提示优化等条件下的CoT控制能力。

Result: 推理模型的CoT控制能力显著低于输出控制能力（如Claude Sonnet 4.5的CoT控制率仅2.7%，而输出控制率达61.9%）。CoT控制能力随模型规模增大而提高，但随RL训练、测试时计算量增加和问题难度提高而降低。对抗性提示优化未能显著提升控制能力。

Conclusion: 目前CoT控制能力不太可能成为CoT可监控性的失效模式，但低控制能力的机制尚不明确。鉴于其对维护CoT可监控性的重要性，建议前沿实验室在未来模型中跟踪CoT控制能力。
...
### [178] [Evolving Medical Imaging Agents via Experience-driven Self-skill Discovery](https://arxiv.org/abs/2603.05860)
*Lin Fan,Pengyu Dai,Zhipeng Deng,Haolin Wang,Xun Gong,Yefeng Zheng,Yafei Ou*

Main category: cs.AI

TL;DR: MACRO：自演进、经验增强的医学智能体，通过从已验证的执行轨迹中自主发现有效的多步骤工具序列，将其合成为可复用的复合工具，实现从静态工具组合到经验驱动工具发现的转变，提升多步骤编排精度和跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 临床图像解读本质上是多步骤且工具为中心的，现有LLM智能体将工具集和调用策略视为静态部署后不变，这种设计在真实世界领域漂移、跨任务和不断演进的诊断需求下变得脆弱，预定义工具链经常退化并需要昂贵的手动重新设计。

Method: MACRO从已验证的执行轨迹中自主识别反复出现的有效多步骤工具序列，将其合成为可复用的复合工具，并注册为新的高级原语。采用轻量级图像特征记忆将工具选择基于视觉-临床上下文，通过GRPO-like训练循环强化可靠调用发现的复合工具，实现闭环自改进。

Result: 在多种医学影像数据集和任务上的广泛实验表明，自主复合工具发现持续改进多步骤编排精度和跨领域泛化能力，优于强基线和近期最先进的智能体方法，弥合了脆弱静态工具使用与自适应、上下文感知临床AI辅助之间的差距。

Conclusion: MACRO通过经验驱动的工具发现和合成，实现了从静态工具组合到自演进、经验增强医学智能体的转变，为临床AI辅助提供了更适应、上下文感知的解决方案，代码将在接受后公开。
...
### [179] [The World Won't Stay Still: Programmable Evolution for Agent Benchmarks](https://arxiv.org/abs/2603.05910)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设静态环境，具有固定的模式（schema）和工具集，忽略了现实世界的演化性质以及智能体对环境变化的鲁棒性。需要一种可扩展且可控的方式来演化智能体环境，以更好地评估智能体对现实世界动态的适应性。

Method: 提出ProEvolve框架，其核心是一个类型化关系图，统一显式地表示环境的数据、工具和模式。在此形式化基础上，添加、删除或修改能力被表达为图变换，这些变换在工具、模式和数据访问之间一致地传播更新。ProEvolve能够：(1) 将演化动态编程为图变换以自动生成环境；(2) 通过子图采样和编程实例化任务沙箱。

Result: 通过将单个环境演化为200个环境和3,000个任务沙箱来验证ProEvolve，并据此对代表性智能体进行基准测试。

Conclusion: ProEvolve提供了一个可编程的框架，用于可控且可扩展地演化智能体环境，解决了现有基准测试中环境静态性的局限性，能够更好地评估智能体对现实世界动态变化的适应性。
...
### [180] [Aggregative Semantics for Quantitative Bipolar Argumentation Frameworks](https://arxiv.org/abs/2603.06067)
...
<details>
  <summary>Details</summary>
Motivation: 现有定量双极论证框架中的渐进语义在处理攻击者和支持者时通常采用对称方式，但在实际应用中攻击者和支持者往往扮演非对称角色。需要一种更灵活、可解释性更强的方法来处理这种非对称性。

Method: 提出聚合语义家族，采用三阶段计算：1) 计算攻击者的全局权重；2) 计算支持者的全局权重；3) 将这两个值与论证的固有权重进行聚合。这种方法将攻击者和支持者分开处理，保持双极性特征。

Result: 通过讨论三个聚合函数应满足的性质及其与经典渐进语义原则的关系，展示了聚合语义的灵活性。通过简单示例和500种聚合语义的测试比较，说明了该方法能够产生广泛的行为模式，提高了可解释性和参数化能力。

Conclusion: 聚合语义通过将计算分解为三个独立且可解释的步骤，保持了比文献中更进一步的双极性，产生了更易理解的渐进语义，为定量双极论证框架提供了更灵活、更可解释的语义计算方法。
...
### [181] [Offline Materials Optimization with CliqueFlowmer](https://arxiv.org/abs/2603.06082)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习方法在计算材料发现中日益流行，但基于最大似然训练的生成模型在探索材料空间有吸引力的区域方面效果有限，需要更有效的优化方法

Method: 提出CliqueFlowmer模型，将基于团的离线模型优化（MBO）与Transformer和流生成技术融合，直接在生成过程中优化目标材料属性

Result: CliqueFlowmer在优化能力验证中表现出色，其生成的材料性能显著优于传统生成基线方法

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的替代方法，通过将直接优化融入生成过程，能够更好地探索材料空间，已开源代码支持跨学科研究
...
### [182] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
TL;DR: 提出基于强化学习的决策支持框架，用于城市洪水适应规划，通过整合降雨预测、洪水建模、交通模拟和影响量化，在气候不确定性下学习平衡投资与影响的适应性策略。
...
Method: 提出基于强化学习(RL)的决策支持框架，构建为综合评估模型(IAM)。整合降雨预测和洪水建模、交通模拟、基础设施和交通直接间接影响量化。RL方法学习平衡投资维护成本与避免影响的适应性策略。

Result: 在哥本哈根内城2024-2100年案例研究中，测试多种适应方案及不同气候情景。结果显示该框架优于传统优化方法：发现协调的空间和时间适应路径；学习影响减少与适应投资之间的权衡；产生更具韧性的策略。

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下进行适应性基础设施规划具有潜力。框架能够发现传统方法难以识别的协调适应路径和权衡关系。
...
### [183] [SAHOO: Safeguarded Alignment for High-Order Optimization Objectives in Recursive Self-Improvement](https://arxiv.org/abs/2603.06333)
...
<details>
  <summary>Details</summary>
Motivation: 递归自我改进正从理论走向实践，但迭代自我修改可能导致微妙的对齐漂移，需要实用的框架来监控和控制这种漂移风险。
