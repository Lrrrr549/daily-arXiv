...
TL;DR: 提出一种结合AI与局部线性嵌入(LLE)的创新方法，用于处理高维医疗数据，旨在提升医疗账单系统和转录服务的准确性与效率。


<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗流程（如账单和转录）提供了新途径。当前高维医疗数据处理存在效率低、易出错的问题，需要更先进的自动化解决方案。

Method: 开发AI增强的LLE模型，通过自动化处理高维医疗数据来优化医疗账单和转录服务。提供完整的数学模型，并在真实医疗场景中进行实验验证。

Result: 实验结果显示，该模型显著提高了数据处理准确性和操作效率，减少了人为错误，加快了患者护理文档和财务交易的处理速度。

Conclusion: AI增强的LLE模型在医疗数据分析中展现出巨大潜力，为未来更广泛的医疗应用研究奠定了基础，特别是在自动化医疗流程方面。
...
TL;DR: ViSignVQA：首个大规模越南语标牌视觉问答数据集，包含10,762张图像和25,573个问答对，通过集成越南语OCR和语言模型提升标牌文本理解性能。


<details>
  <summary>Details</summary>
Motivation: 自然场景中标牌文本理解对VQA的实际应用至关重要，但在低资源语言（如越南语）中尚未得到充分探索。现有VQA数据集缺乏对越南语标牌特有的双语文本、非正式表达和视觉特征的覆盖。

Method: 1) 构建ViSignVQA数据集，包含越南标牌图像及其问答对；2) 集成越南语OCR模型(SwinTextSpotter)和预训练语言模型(ViT5)到SOTA VQA模型中；3) 提出多智能体VQA框架，结合感知和推理智能体与GPT-4，采用多数投票决策。

Result: OCR增强上下文显著提升性能，将OCR文本附加到问题中可使F1分数提升高达209%。多智能体框架通过多数投票达到75.98%的准确率。实验验证了领域特定资源对低资源语言文本VQA的重要性。

Conclusion: ViSignVQA是首个用于越南语标牌理解的大规模多模态数据集，为OCR集成的VQA模型提供了基准。研究强调了领域特定资源在增强低资源语言文本VQA中的关键作用，支持越南语场景文本理解技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型（VLMs）在回答视觉问题时经常利用虚假相关性而非因果视觉证据，且在微调后更容易出现捷径学习问题。需要一种方法来强制模型基于真实的视觉证据进行推理，减少对虚假相关性的依赖。

Method: 提出VISTA框架：1）使用冻结的VLM传感器进行短而客观的感知查询；2）文本LLM推理器分解问题、规划查询、聚合视觉事实；3）通过强化学习（GRPO）在仅641个多步问题上训练无偏视觉推理；4）创建受控接口作为奖励对齐环境。

Result: 在SpuriVerse上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%）；在MMVP和SeedBench子集上保持竞争力；能够跨未见VLM传感器迁移；能识别和恢复VLM感知失败；人类分析显示推理轨迹更中立、更少依赖虚假属性、更明确基于视觉证据。

Conclusion: VISTA通过感知-推理解耦和信息瓶颈，有效减少了视觉语言模型对虚假相关性的依赖，提升了视觉推理的鲁棒性和可解释性，且具有较好的泛化能力和故障恢复能力。
...
Method: 采用Mixture-of-Transformers架构统一文本和图像生成。引入"先交错后解耦"的训练范式：1)交错概念学习，利用交错文本-图像数据培养模型对剧本的深入理解和想象力解释；2)解耦专家学习，将剧本写作与关键帧生成解耦，实现更大的故事叙述灵活性和创造性。

Result: 大量实验表明，UniMAGE在开源模型中实现了最先进的性能，能够生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功地将剧本创作和关键镜头设计统一到单一框架中，为AI驱动的视频创作提供了更集成的方法，使非专业人士能够制作高质量的多镜头电影。
...
### [5] [RealX3D: A Physically-Degraded 3D Benchmark for Multi-view Visual Restoration and Reconstruction](https://arxiv.org/abs/2512.23437)
...
TL;DR: RealX3D是一个真实采集的多视角视觉恢复和3D重建基准数据集，包含多种物理退化类型，用于评估算法在真实挑战性环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多视角视觉恢复和3D重建方法在理想条件下表现良好，但在真实世界的物理退化环境下（如光照变化、散射、遮挡、模糊等）性能显著下降。缺乏一个包含多种物理退化的真实采集基准数据集来系统评估算法的鲁棒性。

Method: 1. 将物理退化分为四类：光照、散射、遮挡、模糊，每类包含多个严重程度等级；2. 采用统一的采集协议获取像素对齐的低质量/高质量视图对；3. 每个场景包含高分辨率采集、RAW图像和密集激光扫描数据；4. 从扫描数据中导出世界尺度网格和度量深度信息。

Result: 对基于优化的和前馈方法进行广泛基准测试，结果显示在物理退化条件下重建质量显著下降，突显了当前多视角流水线在真实挑战性环境中的脆弱性。

Conclusion: RealX3D基准数据集揭示了当前多视角视觉恢复和3D重建方法在真实物理退化环境下的局限性，为开发更鲁棒的算法提供了重要的评估工具和方向指引。
...
TL;DR: HookMIL：一种用于计算病理学的上下文感知、计算高效的MIL框架，通过可学习的hook tokens进行结构化上下文聚合，支持多模态初始化，具有线性复杂度


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法在分析全切片图像时丢失关键上下文信息，而基于transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算问题。需要开发既能保持上下文信息又计算高效的MIL框架。

Method: 提出HookMIL框架，使用紧凑、可学习的hook tokens进行结构化上下文聚合。支持三种初始化方式：关键patch视觉特征、视觉语言病理模型的文本嵌入、空间转录组-视觉模型的空间基础特征。通过双向注意力机制实现线性复杂度交互，引入Hook多样性损失促进token专业化，并采用hook-to-hook通信机制优化上下文交互。

Result: 在四个公共病理数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL通过可学习的hook tokens有效解决了传统MIL方法上下文信息丢失和transformer变体计算效率低的问题，多模态初始化加速收敛并提升表示质量，为计算病理学提供了高效且可解释的弱监督分析框架。
...
TL;DR: Tiny-YOLOSAM：结合YOLO检测器和TinySAM的混合流水线，通过目标检测生成框提示和稀疏点采样，显著提升分割覆盖率和速度，替代密集的"分割一切"模式。


<details>
  <summary>Details</summary>
Motivation: SAM和TinySAM在延迟敏感场景中计算成本过高，TinySAM的"分割一切"模式需要数百个提示且速度慢，需要更高效的实时全场景分割方案。

Method: 首先复现TinySAM建立基准，然后提出Tiny-YOLOSAM混合流水线：使用YOLOv12检测器生成前景物体的框提示，对未覆盖区域仅采样稀疏点提示，避免密集提示。
...
TL;DR: 提出一种基于视觉语言模型的多模态可解释性模型，通过分析视网膜象限中的病变分布来模拟眼科医生的推理过程，生成配对Grad-CAM热力图，为糖尿病视网膜病变诊断提供定量检测和自然语言解释。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。临床医生缺乏时间进行病变标注，现有模型仅突出病变位置而无法解释分类推理。当前模型依赖单一成像模态，可解释性有限。需要一种能够识别个体DR病变并提供自然语言解释的定量检测系统。

Method: 提出新颖的多模态可解释性模型，采用视觉语言模型和少样本学习。模型模拟眼科医生的推理过程，分析视网膜象限中的病变分布。生成配对Grad-CAM热力图，展示OCT和眼底图像中各个神经元的权重，可视化突出对DR严重程度分类有贡献的区域。使用3,000张眼底图像和1,000张OCT图像数据集。

Result: 该方法解决了当前DR诊断中的关键限制，提供了一种实用且全面的工具来改善患者预后。模型能够生成视觉热力图，突出显示对分类决策有贡献的图像区域，同时提供自然语言解释。

Conclusion: 该多模态可解释性模型通过结合VLM和少样本学习，为糖尿病视网膜病变诊断提供了更全面的解释能力，克服了现有模型的局限性，在筛查、治疗和研究环境中具有广泛的应用潜力。
...
TL;DR: VLM-PAR：基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨注意力融合改进视觉特征与提示嵌入的对齐，在行人属性识别任务中显著提升性能，特别是在高度不平衡的PA100K数据集上达到新的SOTA。
...
Method: 构建基于冻结SigLIP 2多语言编码器的模块化视觉语言框架，通过紧凑的跨注意力融合机制精炼视觉特征，实现图像与提示嵌入的对齐。

Result: 在高度不平衡的PA100K基准上实现显著准确率提升并达到新的SOTA性能，同时在PETA和Market-1501基准上也获得显著的均值准确率提升。
...
TL;DR: 该论文提出语义抽象框架，利用2D视觉语言模型的激活图作为"抽象对象"表示，学习3D定位和补全隐藏物体（被遮挡物体），通过历史数据优化搜索效率。


<details>
  <summary>Details</summary>
Motivation: 动机是解决家庭机器人寻找丢失物体的问题，特别是那些被部分遮挡、无法被视觉语言模型直接识别的隐藏物体，以提高搜索效率并减少时间和精力消耗。

Method: 方法基于语义抽象框架：将2D视觉语言模型的相关性激活图视为"抽象对象"表示，用于学习3D定位和补全隐藏物体；利用历史放置数据优化搜索过程，实现非结构化搜索。

Result: 模型能够准确识别隐藏物体的完整3D位置，首次尝试成功率显著高于随机搜索，搜索速度明显更快。

Conclusion: 语义抽象框架的扩展为家庭机器人提供了高效寻找丢失物体的技能，通过利用历史数据和抽象对象表示，显著提升了搜索隐藏物体的效率和准确性。
...
TL;DR: VideoScaffold：用于流式视频理解的动态表示框架，通过自适应事件粒度调整和分层事件整合，解决现有静态方法在连续视频流中的碎片化和过度压缩问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频理解上面临挑战，主要问题包括帧间冗余严重、需要时序一致表示。现有的静态策略（稀疏采样、帧压缩、聚类）针对离线场景优化，应用于连续视频流时会产生碎片化或过度压缩的输出。

Method: 提出VideoScaffold动态表示框架，包含两个核心组件：1）弹性尺度事件分割（EES），通过预测引导的分割动态细化事件边界；2）分层事件整合（HEC），渐进聚合语义相关片段为多层次抽象。两者协同工作，使框架能够随着视频流展开从细粒度帧理解平滑过渡到抽象事件推理。

Result: 在离线和流式视频理解基准测试上的广泛实验表明，VideoScaffold实现了最先进的性能。该框架是模块化和即插即用的，能够无缝扩展现有的基于图像的多模态大语言模型到连续视频理解。

Conclusion: VideoScaffold通过动态调整事件粒度和分层整合语义信息，有效解决了长视频理解中的冗余和时序一致性问题，为流式视频理解提供了有效的框架解决方案。
...
TL;DR: 提出KAN增强的FPN-Stem架构，通过用KAN卷积层替换FPN末端的标准线性3x3平滑卷积，解决ViT在密集预测任务中前端设计简单导致的性能瓶颈问题，在COCO数据集上相比ViTPose-S提升2.0 AP。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在密集预测任务中表现受限，主要原因是如ViTPose等模型采用过于简单的前端设计。这种简单的patchification机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失。研究发现性能瓶颈并非在注意力模块，而是在FPN的后融合非线性平滑步骤。

Method: 提出KAN增强的FPN-Stem架构：保留FPN经典的"上采样-相加"融合流，但将其末端的标准线性3x3平滑卷积替换为基于KAN的卷积层。利用KAN优越的非线性建模能力，该层能够自适应学习和修正多尺度融合过程中产生的"伪影"。这是一种即插即用的高性能模块。

Result: 在COCO数据集上的大量实验表明，KAN-FPN-Stem相比轻量级ViTPose-S基线实现了显著的性能提升，最高达到+2.0 AP。该工作不仅提供了高性能模块，更重要的是揭示了ViT前端性能瓶颈的关键所在。

Conclusion: 该研究揭示了ViT前端性能瓶颈往往不在于"特征精炼"（注意力机制），而在于"特征融合"的质量。通过引入KAN算子为解决这一瓶颈提供了有效路径，为改进ViT在密集预测任务中的性能提供了新思路。
...
Conclusion: MiG-DM通过整合跨模态先验知识和跨域处理，有效解决了低剂量PET成像的关键挑战，为高质量PET图像生成提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB检测在低光环境下表现不佳，热成像系统缺乏颜色和纹理信息。应急服务机器人需要在各种光照条件下可靠工作，现有单一模态检测方法存在局限性。

Method: 训练33个YOLO模型，使用22,000+标注图像覆盖三种光照条件。通过11种融合比例（从100/0到0/100）对齐融合RGB和LWIR帧，动态选择最优检测模型。

Result: 最佳全光模型（80/20融合）和暗光模型（90/10融合）分别达到92.8%和92.0%平均置信度，显著优于YOLOv5n和YOLOv11n基线。无光条件下40/60融合达到71.0%，优于基线但统计不显著。

Conclusion: 自适应RGB-LWIR融合提高了所有光照条件下的检测置信度和可靠性，增强了自主机器人视觉性能，为应急服务机器人提供了更鲁棒的感知能力。
...
Result: 实验表明流模型倾向于在没有持续引导时漂移回默认轨迹。实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移。引导生成相比无引导基线将语义对齐提高了约80%。

Conclusion: 小型教师模型能够可靠地引导大型生成系统，增强几何控制能力，拓宽文本到图像合成的创作范围，为解决几何约束与风格冲突问题提供了有效方案。
...
TL;DR: GeCo是一个几何基础的一致性度量方法，用于联合检测静态场景中的几何变形和遮挡不一致伪影，通过融合残差运动和深度先验生成可解释的稠密一致性图，用于视频生成模型的基准测试和训练自由引导损失。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在静态场景中经常产生几何变形和遮挡不一致的伪影，需要一种系统性的方法来检测和量化这些缺陷，以评估模型性能和指导改进。

Method: GeCo通过融合残差运动和深度先验，生成稠密的一致性图来检测几何变形和遮挡不一致伪影。该方法基于几何基础，能够提供可解释的检测结果。

Result: GeCo成功揭示了近期视频生成模型的常见失效模式，并作为训练自由引导损失有效减少了视频生成过程中的变形伪影。

Conclusion: GeCo是一个有效的几何基础一致性度量工具，既能用于系统评估视频生成模型，又能作为训练自由引导损失来改善生成质量，减少几何变形和遮挡不一致伪影。
...
TL;DR: 该研究开发了Bones and Joints (B&J)基准测试，评估AI模型在骨科和运动医学中的临床推理能力，发现模型在结构化选择题上表现良好（>90%），但在需要多模态整合的开放任务中表现显著下降（<60%），表明当前AI模型尚不具备复杂临床推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于医学执照考试或精选病例的基准测试无法捕捉真实世界患者护理所需的整合、多模态推理能力。基础模型在临床实践中的快速应用需要对其真实临床推理能力进行严格评估。

Method: 开发了Bones and Joints (B&J)基准测试，包含1,245个来自真实骨科和运动医学病例的问题。评估了11个视觉语言模型(VLMs)和6个大语言模型(LLMs)，涵盖7个反映临床推理路径的任务：知识回忆、文本和图像解释、诊断生成、治疗计划和原理提供。

Result: 模型在不同任务类型间存在显著性能差距：在结构化多选题上准确率超过90%，但在需要多模态整合的开放任务中准确率仅约60%。VLMs在医学图像解释方面存在严重限制，经常出现文本驱动的幻觉，忽略矛盾的视觉证据。专门针对医学应用微调的模型相比通用模型没有一致优势。

Conclusion: 当前AI模型尚不具备复杂、多模态临床推理能力，其安全部署应限于支持性、基于文本的角色。核心临床任务的未来进展需要多模态整合和视觉理解方面的根本性突破。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨模态对齐问题，实现高质量的3D检索和可控的4D生成，推动动态多模态理解和实际应用。
...
Conclusion: Uni4D通过结构化三级对齐框架，成功实现了大规模开放词汇3D检索和可控4D生成，为跨模态理解和生成任务提供了有效解决方案。
...
TL;DR: SpotEdit：一种无需训练的选择性区域更新扩散编辑框架，仅处理修改区域，减少冗余计算


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时对所有token进行统一处理和去噪，即使只修改小区域也需全图计算，导致计算冗余且可能破坏未修改区域的质量

Method: SpotEdit包含两个核心组件：SpotSelector通过感知相似性识别稳定区域并复用条件图像特征跳过计算；SpotFusion通过动态融合机制自适应地混合这些特征与编辑token，保持上下文连贯性和编辑质量

Result: 通过减少不必要的计算并保持未修改区域的高保真度，SpotEdit实现了高效且精确的图像编辑

Conclusion: SpotEdit证明了在图像编辑中无需重新生成每个区域，通过选择性更新修改区域即可实现高效高质量的编辑效果
...
Method: 提出DeMoGen组合训练范式，采用基于能量的扩散模型直接捕捉多个运动概念的组合分布，使模型能够在没有单个概念真实运动数据的情况下发现运动原语。包含三个训练变体：1) DeMoGen-Exp：在分解的文本提示上显式训练；2) DeMoGen-OSS：执行正交自监督分解；3) DeMoGen-SC：强制原始和分解文本嵌入之间的语义一致性。

Result: 该方法能够从复杂运动序列中解耦可重用的运动原语，分解后的运动概念可以灵活重组以生成多样且新颖的运动，超越训练分布进行泛化。同时构建了文本分解数据集以支持组合训练。

Conclusion: DeMoGen提供了一种有效的运动分解学习框架，不仅能够分解复杂运动为语义子组件，还能支持运动原语的重组生成，为文本到运动生成和运动组合提供了扩展资源。
...
TL;DR: 基于变分自编码器的多视图潜在表示学习框架，用于从T1Gd和FLAIR MRI中提取互补影像组学特征，预测胶质母细胞瘤MGMT启动子甲基化状态
...
Method: 提出基于变分自编码器的多视图潜在表示学习框架：1）使用独立的概率编码器分别编码T1Gd和FLAIR MRI的影像组学特征；2）在紧凑的潜在空间中进行融合；3）保留模态特异性结构的同时实现有效的多模态整合；4）利用生成的潜在嵌入进行MGMT启动子甲基化分类

Result: 该方法能够有效整合互补的影像组学特征，在紧凑的潜在空间中实现多模态融合，为MGMT启动子甲基化分类提供更优的特征表示

Conclusion: 提出的多视图潜在表示学习框架通过独立编码和潜在空间融合，克服了传统方法的局限性，为胶质母细胞瘤MGMT启动子甲基化的非侵入性预测提供了更有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有HER2评分方法存在局限性：难以同时预测HER2的低表达和高表达水平，联合分析H&E和IHC染色图像困难，且缺乏像素级的HER2状态定位能力

Method: 1) 对H&E全切片图像进行分块处理实现肿瘤定位；2) 提出新颖映射函数对应识别IHC图像中与H&E恶性区域相关的区域；3) 嵌入临床启发的HER2评分机制，实现0、1+、2+、3+四分类的像素级自动标注；4) 使用基于视觉变换器的端到端模型架构

Result: 肿瘤定位分类准确率良好；HER2状态预测准确率达0.94，特异性达0.933；在四分类评分中表现优异；与病理学家评估结果具有可比性

Conclusion: 基于视觉变换器的端到端模型能够有效联合评估H&E和IHC图像，实现准确的HER2评分，为临床诊断提供像素级定位支持
...
TL;DR: 本文提出了一种基于多模态大语言模型（MLLMs）的3D场景物体排列系统，通过引入MCP-based API、专用视觉工具套件和多智能体协作框架，解决了MLLMs在复杂3D场景操作中的视觉定位弱、空间理解不足和迭代更新易错三大挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在2D视觉语言任务上取得了显著进展，但在复杂3D场景操作方面的应用仍未被充分探索。现有MLLMs在3D物体排列任务中面临三个关键挑战：1）视觉定位能力弱，难以将程序化编辑与精确的3D结果关联；2）缺乏对3D场景的深度理解；3）迭代更新过程容易出错且难以管理。

Method: 提出三个核心方法：1）引入基于MCP的API，将交互从脆弱的原始代码操作转变为更稳健的函数级更新；2）通过专用视觉工具套件增强MLLM的3D场景理解能力，包括场景状态分析、空间信息收集和动作结果验证；3）设计协作式多智能体框架，包含规划、执行和验证三个角色，以管理迭代更新并从中断错误中恢复。

Result: 在25个复杂的物体排列任务上评估了所提方法，结果表明该系统显著优于现有基线方法，有效证明了所提方法在3D场景操作任务中的优越性。

Conclusion: 通过结合MCP-based API、专用视觉工具和多智能体协作框架，成功将MLLMs的能力扩展到复杂3D场景操作领域，为3D物体排列任务提供了一种有效且稳健的解决方案。
...
TL;DR: Self-E是一个从零开始训练、支持任意步数推理的文本到图像生成模型，结合了流匹配学习和自评估机制，无需预训练教师模型即可实现从少步到多步的高质量生成。


<details>
  <summary>Details</summary>
Motivation: 传统扩散或流模型依赖局部监督需要多步推理，蒸馏方法需要预训练教师模型。Self-E旨在填补这两种范式之间的空白，实现从零开始训练、支持任意步数推理的统一模型。

Method: Self-E采用从零开始的训练方法，结合流匹配学习和新颖的自评估机制。模型从数据中学习类似流匹配模型，同时使用当前分数估计评估自身生成的样本，作为动态自教师。这种瞬时局部学习和自驱动全局匹配的结合使其能够训练高质量文本到图像模型。
...
TL;DR: iOSPointMapper是一款移动应用，利用iPhone/iPad的语义分割、LiDAR深度估计和GPS/IMU融合技术，实现实时、隐私保护的实地人行道测绘，检测交通标志、信号灯等特征，数据匿名传输至TDEI交通数据交换计划。
...
Method: 开发iOSPointMapper移动应用，利用设备端语义分割、LiDAR深度估计和GPS/IMU数据融合来检测和定位人行道相关特征；包含用户引导的标注界面用于验证系统输出；数据匿名化后传输至TDEI交通数据交换计划。

Result: 系统特征检测和空间测绘性能的详细评估显示该应用在增强行人测绘方面具有潜力，能够提供可扩展、以用户为中心的方法来填补关键数据空白。

Conclusion: iOSPointMapper为行人基础设施提供了可扩展、用户中心化的数据收集解决方案，通过移动设备技术实现了实时、隐私保护的实地测绘，有助于填补行人数据的关键空白。
...
TL;DR: DeFloMat是一种新颖的生成式目标检测框架，通过集成条件流匹配来解决基于扩散的检测器（如DiffusionDet）的延迟瓶颈问题，在仅需3个推理步骤下达到最先进精度。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在目标检测中的关键延迟瓶颈问题。扩散模型虽然通过多步随机去噪过程实现高精度，但需要大量采样步骤（T≫60），这使得它们在时间敏感的临床应用（如克罗恩病的磁共振小肠造影检测）中不实用。

Method: 提出DeFloMat框架，将缓慢的随机路径替换为从条件最优传输理论推导的高度直接、确定性的流场，具体近似修正流。这一转变使得通过简单的常微分方程求解器实现快速推理成为可能。

Result: 在具有挑战性的MRE临床数据集上，DeFloMat在仅3个推理步骤下达到了最先进的精度（43.32% AP10:50），比DiffusionDet在4个步骤下的最大收敛性能（31.03% AP10:50）提高了1.4倍。确定性流显著增强了定位特性，在少步机制下产生更优的召回率和稳定性。

Conclusion: DeFloMat解决了生成式精度与临床效率之间的权衡，为稳定快速的目标定位设定了新标准，特别适用于时间敏感的临床检测应用。
...
<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜能快速非侵入性地可视化细胞形态，但稳健的体积分割通常依赖荧光或繁重的后处理。现有方法无法直接从明场图像中准确分割亚细胞结构。

Method: 提出Bright-4B基础模型，包含：硬件对齐的Native Sparse Attention机制（捕获局部、粗粒度和选择性全局上下文）；深度-宽度残差HyperConnections稳定表征流；软混合专家实现自适应容量；各向异性patch嵌入尊重共聚焦点扩散函数和轴向变薄，实现几何保真的3D标记化。

Result: 模型直接从明场堆栈中产生形态学准确的细胞核、线粒体和其他细胞器分割，无需荧光、辅助通道或手工后处理。在多个共聚焦数据集上，Bright-4B在深度和细胞类型间保持精细结构细节，优于当代CNN和Transformer基线。

Conclusion: Bright-4B填补了无标记3D明场显微镜中稳健体积分割的空白，为大规模、无标记3D细胞映射提供了有效解决方案。所有代码、预训练权重和下游微调模型将公开发布。
...
TL;DR: 提出EmoCtrl方法解决可控情感图像内容生成问题，在保持内容一致性的同时表达目标情感，超越现有方法


<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感色调共同传递意义，但现有文本到图像模型缺乏情感意识，而情感驱动模型则以内容失真为代价生成情感结果

Method: 提出EmoCtrl方法，使用内容、情感和情感提示标注的数据集，包含文本和视觉情感增强模块，通过描述性语义和感知线索丰富情感表达

Result: 定量和定性实验表明EmoCtrl实现了忠实的内容和表达性情感控制，在多个方面优于现有方法；用户研究证实与人类偏好高度一致；学习到的情感标记具有互补效应和良好泛化能力

Conclusion: EmoCtrl成功解决了可控情感图像内容生成问题，在保持内容一致性的同时实现了精确的情感表达，学习到的情感标记具有鲁棒性和适应性
...
Method: 1. 直接从事件流中检测线条；2. 利用事件-线条标定模型生成相机参数的初始估计，适用于平面和非平面线条；3. 采用非线性优化细化相机参数。

Result: 通过仿真和真实世界实验验证了方法的可行性和准确性，在单目和立体事件相机上进行了验证，代码已开源。

Conclusion: 提出的基于线特征的事件相机标定框架能够有效利用环境中自然存在的几何线条进行标定，无需专用标定板，适用于快速变化场景。
...
TL;DR: SonoVision是一款帮助视障人士通过声音提示定位日常物品的智能手机应用，使用左右耳立体声提示物体方位，完全离线工作。


<details>
  <summary>Details</summary>
Motivation: 视障人士定位物体存在重大挑战，这阻碍了他们的独立性，可能导致危险情况。为了让视障人士更加自给自足，需要开发辅助工具减少对他人的依赖。

Method: 使用Flutter开发平台构建智能手机应用，后端采用Efficientdet-D2模型进行物体检测。通过耳机提供立体声提示：物体在右侧时右耳发出正弦声，在左侧时左耳发声，正前方时双耳同时发声。

Result: 开发了完全离线工作的SonoVision应用，通过声音提示帮助视障人士定位物体。应用已在GitHub上开源，提供安全、用户友好的辅助功能。

Conclusion: SonoVision应用能够显著帮助视障人士以安全、用户友好的方式定位日常物品，减少对周围人的依赖，增强独立性。离线工作能力确保了应用的可用性和可靠性。
...
TL;DR: SAM 3D作为通用图像转3D基础模型，在单目遥感建筑重建任务中表现优于专用方法TRELLIS，能生成更连贯的屋顶几何和更清晰的边界，并可通过分段-重建-组合流程扩展到城市场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D建筑重建方法通常需要任务特定的架构和密集监督，限制了可扩展性。本文旨在评估通用基础模型SAM 3D在遥感建筑重建任务中的性能，探索其在城市场景建模中的应用潜力。

Method: 在NYC Urban Dataset上对SAM 3D与TRELLIS进行基准测试，使用FID和CMMD作为评估指标。进一步通过分段-重建-组合流程将SAM 3D扩展到城市场景重建，并分析其实际限制。

Result: 实验结果表明，SAM 3D相比TRELLIS能生成更连贯的屋顶几何和更锐利的边界。通过扩展流程成功实现了城市场景重建，验证了基础模型在该领域的应用潜力。

Conclusion: SAM 3D作为通用基础模型在单目遥感建筑重建任务中表现出色，为城市3D重建提供了实用指导，并激励未来集成场景级结构先验的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 电力变电站是电网关键基础设施，其资产易受多种灾害影响，故障会导致重大经济和安全后果。传统人工映射方法耗时耗力，需要自动化解决方案来高效识别关键组件以量化脆弱性。

Method: 研究使用手动标注的美国变电站图像数据集，训练并比较了三种计算机视觉模型：YOLOv8、YOLOv11和RF-DETR。评估指标包括检测准确率、精确度和效率。

Result: 研究展示了三种模型在变电站组件检测中的性能表现，分析了各自的优势和局限性，并确定了最适合大规模可靠变电站组件映射的模型。

Conclusion: 该研究证明了机器学习在变电站映射中的应用价值，通过计算机视觉模型实现了高效、自动化的变电站组件识别，为电网脆弱性评估提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于姿态码的框架（如CoMo）虽然支持可解释的运动控制，但其逐帧表示难以捕捉细微时间动态和高频细节，导致重建保真度和局部可控性下降。需要一种既能保持可解释性又能提升运动质量的方法。

Method: 提出PGR²M混合表示：1）姿态引导的RVQ分词器将运动分解为编码粗粒度全局结构的姿态潜在变量和建模细粒度时间变化的残差潜在变量；2）残差dropout防止过度依赖残差，保持姿态码的语义对齐和可编辑性；3）基础Transformer自回归预测姿态码，精炼Transformer在文本、姿态码和量化阶段条件下预测残差码。

Result: 在HumanML3D和KIT-ML数据集上，PGR²M相比CoMo和最近的扩散/分词化基线方法，在Fréchet inception距离和重建指标上均有提升。用户研究证实其能实现直观、结构保持的运动编辑。

Conclusion: PGR²M通过混合表示方法有效解决了姿态码框架在时间动态捕捉方面的局限性，在保持可解释性和可编辑性的同时，显著提升了文本驱动3D运动生成与编辑的质量。
...
TL;DR: 提出基于多事件相机的高时空分辨率冲击波运动参数测量框架，通过极坐标编码、自适应ROI提取和迭代斜率分析实现冲击波前事件提取，最终完成3D重建和爆炸当量反演，测量误差最小0.06%，最大5.20%。


<details>
  <summary>Details</summary>
Motivation: 冲击波在功率场测试和损伤评估等应用中需要高时空分辨率的运动参数测量，但冲击波快速、不均匀传播及不稳定测试条件带来显著挑战。传统测量方法难以满足高动态范围和高速度的测量需求。

Method: 1) 建立极坐标系编码事件以揭示冲击波传播模式，通过事件偏移计算实现自适应感兴趣区域提取；2) 利用速度变化的连续性，采用迭代斜率分析提取冲击波前事件；3) 基于事件光学成像模型推导事件几何模型和冲击波运动参数，结合3D重建模型；4) 实现多角度冲击波测量、运动场重建和爆炸当量反演。

Result: 与压力传感器和理论公式对比，速度测量最大误差5.20%，最小误差0.06%。实验结果表明该方法实现了高时空分辨率的高精度冲击波运动场测量，代表了显著进展。

Conclusion: 提出的多事件相机框架成功解决了冲击波快速不均匀传播的测量挑战，利用事件相机的高速度和高动态范围特性，实现了高精度、高时空分辨率的冲击波运动参数测量，为功率场测试和损伤评估提供了有效解决方案。
...
TL;DR: 提出基于分层MoE适配器的两阶段半监督范式，使用10%标注数据将SAM蒸馏为专家教师模型，再用其生成伪标签训练轻量下游模型，解决红外小目标检测的数据稀缺问题。
...
Method: 设计分层MoE适配器（四个白盒神经算子），提出两阶段知识蒸馏与迁移范式：1）先验引导知识蒸馏：使用MoE适配器和10%全监督数据将SAM蒸馏为专家教师模型Scalpel-SAM；2）部署导向知识迁移：使用Scalpel-SAM生成伪标签训练轻量高效的下游模型。

Result: 实验表明，在最小标注量下，该范式使下游模型性能达到甚至超过其全监督对应模型，是首个系统性地使用SAM作为教师模型解决IR-SOT数据稀缺问题的半监督范式。

Conclusion: 提出的分层MoE适配器和两阶段半监督范式有效解决了红外小目标检测中的数据稀缺问题，实现了在最小标注成本下获得高性能轻量模型的目标。
...
TL;DR: Video-GMAE是一种自监督视频表示学习方法，将视频编码为随时间移动的高斯泼溅集合，通过3D场景投影的归纳偏置实现零样本跟踪


<details>
  <summary>Details</summary>
Motivation: 现有视频表示学习方法缺乏对视频本质的3D场景动态特性的建模，需要一种能够自然捕捉视频中物体运动轨迹的自监督方法

Method: 提出Video Gaussian Masked Autoencoders，将视频序列编码为随时间移动的高斯泼溅集合，通过掩码自编码器架构进行自监督预训练，学习视频的3D场景表示

Result: 模型在预训练中自然涌现出跟踪能力，零样本跟踪性能达到SOTA水平；微调后在Kinetics数据集上提升34.6%，在Kubric数据集上提升13.1%，超越现有自监督视频方法

Conclusion: Video-GMAE通过将视频表示为移动高斯泼溅集合，有效建模了视频的3D场景特性，实现了强大的自监督表示学习和零样本跟踪能力
...
TL;DR: SR-MCR：一个轻量级、无标签的多模态LLM推理对齐框架，通过五个自参考信号评估推理过程可靠性，提升答案准确性和推理连贯性


<details>
  <summary>Details</summary>
Motivation: 现有多模态LLM常产生流畅但不可靠的推理，存在步骤间连贯性弱和视觉基础不足的问题，主要因为现有对齐方法只监督最终答案而忽略中间推理过程的可靠性

Method: 提出SR-MCR框架，利用从模型输出直接衍生的五个自参考信号（语义对齐、词汇保真度、非冗余性、视觉基础、步骤一致性）构建归一化可靠性加权奖励，提供细粒度过程级指导；采用无批评者的GRPO目标，结合置信度感知冷却机制稳定训练并抑制平凡或过度自信的生成

Result: 在Qwen2.5-VL基础上，SR-MCR在广泛的视觉基准测试中提升了答案准确性和推理连贯性；在同等规模的开源模型中，SR-MCR-7B以81.4%的平均准确率实现了最先进性能；消融研究确认了每个奖励项和冷却模块的独立贡献

Conclusion: SR-MCR通过利用自参考过程信号进行推理对齐，有效解决了多模态LLM推理可靠性问题，提供了一种轻量级、无标签的解决方案，显著提升了模型的推理质量和性能
...
TL;DR: 本文提出了一种用于人脸聚类的稀疏差分Transformer方法，通过预测驱动的Top-K Jaccard相似度系数和Transformer模型改进相似性测量，显著提升了聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸聚类方法使用Jaccard相似度系数替代余弦距离来提高测量精度，但引入了过多无关节点，导致Jaccard系数区分能力有限，反而损害了聚类性能。需要解决相似性测量的可靠性问题。

Method: 提出预测驱动的Top-K Jaccard相似度系数来提升邻居节点的纯度；开发基于Transformer的预测模型分析中心节点与其Top-K附近邻居的关系；提出稀疏差分Transformer替代标准Transformer，消除噪声并增强抗噪能力。

Result: 在MS-Celeb-1M等多个数据集上的大量实验表明，该方法达到了最先进的性能，优于现有方法，为人脸聚类提供了更鲁棒的解决方案。

Conclusion: 通过预测驱动的Top-K Jaccard相似度系数和稀疏差分Transformer，有效解决了人脸聚类中相似性测量的可靠性问题，显著提升了聚类性能，为相关领域提供了新的技术思路。
...
TL;DR: Envision是一个基于扩散的视觉规划框架，通过目标图像约束生成物理合理且目标一致的轨迹，用于具身智能体的操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型主要进行前向预测，缺乏明确的目标建模，导致空间漂移和目标不对齐问题。需要一种能够生成物理合理且目标一致的视觉轨迹的方法来支持具身智能体的操作任务。
...
Conclusion: Envision通过显式目标约束的扩散框架解决了现有视觉规划中的空间漂移和目标不对齐问题，为具身智能体提供了可靠的视觉规划指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统IQA模型通常输出单一全局分数，对局部和细粒度失真极不敏感，导致ISR模型可能产生感知上不理想的伪影却获得虚假高分，造成奖励黑客问题，使优化目标与感知质量错位。

Method: 1. 提出基于编码器-解码器架构的FinPercep-RM精细感知奖励模型，不仅提供全局质量分数，还生成感知退化图来空间定位和量化局部缺陷；2. 构建FGR-30k数据集训练该模型；3. 提出CCL协同进化课程学习机制，奖励模型和ISR模型同步进行课程学习，奖励模型复杂度逐步增加，ISR模型从简单全局奖励开始快速收敛，逐步过渡到复杂模型输出。

Result: 实验验证了该方法在ISR模型上的有效性，在RLHF方法中同时提升了全局质量和局部真实感，成功抑制了奖励黑客现象。

Conclusion: 通过结合精细感知奖励模型和协同进化课程学习，有效解决了ISR任务中传统IQA模型的局限性，实现了更稳定、更符合人类感知的强化学习训练，为基于RLHF的图像超分辨率提供了更可靠的优化框架。
...
Result: 在室内基准测试中取得最先进性能（受限训练条件下），在室外数据集上表现强劲，验证了自回归先验作为几何感知生成模型的竞争力
...
TL;DR: 该研究评估了多种深度学习架构（3D Inception、双流、CNN-RNN）用于超声心动图视频的左心室射血分数估计，发现改进的3D Inception架构在EchoNet-Dynamic数据集上取得了最佳性能（RMSE 6.79%）。


<details>
  <summary>Details</summary>
Motivation: 超声心动图是评估左心室射血分数（LVEF）的常用非侵入性方法，但手动评估耗时且存在观察者间差异。深度学习为自动化评估提供了有前景的替代方案，有望达到与经验丰富专家相当的性能水平。

Method: 研究比较了多种深度学习架构：3D Inception、双流模型和CNN-RNN模型。在EchoNet-Dynamic数据集（10,030个超声心动图视频）上系统评估了架构修改和融合策略，以最大化预测准确性。特别关注了卷积核大小和归一化策略等超参数的影响。

Result: 改进的3D Inception架构取得了最佳整体性能，均方根误差为6.79%。观察到所有架构都存在过拟合趋势，较小较简单的模型通常表现出更好的泛化能力。模型性能对超参数选择高度敏感，特别是卷积核大小和归一化策略。

Conclusion: 深度学习架构在超声心动图LVEF估计中表现出色，改进的3D Inception架构效果最佳。研究强调了架构设计和训练策略的重要性，这些见解可推广到更广泛的医学和非医学视频分析任务中。
...
TL;DR: CLAdapter是一种新颖的集群注意力适配器，通过注意力机制和聚类中心来优化和适配大规模预训练模型的特征表示，使其能够有效适应数据有限的下游科学领域任务。


<details>
  <summary>Details</summary>
Motivation: 尽管计算机视觉领域受益于大规模数据集和预训练模型，但在专业化和数据有限的科学领域中，许多下游任务仍然面临重大挑战。需要一种方法能够将大规模预训练模型学到的丰富表示有效地适配到各种数据有限的下游任务中。

Method: CLAdapter引入注意力机制和聚类中心，通过分布相关性和变换矩阵来个性化增强转换后的特征。它采用统一的接口设计，可以无缝集成到多种模型架构中，包括CNN和Transformer，支持2D和3D场景。

Result: 在10个数据集上进行广泛实验，涵盖通用、多媒体、生物、医学、工业、农业、环境、地理、材料科学、OOD和3D分析等多个领域，CLAdapter在数据有限的科学领域中实现了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，能够在各种数据有限的下游科学任务中实现卓越性能，为跨领域知识迁移提供了有效的解决方案。
...
TL;DR: INTERACT-CMIL是一个多头部深度学习框架，通过共享特征学习、组合部分监督和互依赖损失，联合预测结膜黑色素细胞上皮内病变的五个组织病理学轴，在486个专家标注的结膜活检切片上显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 结膜黑色素细胞上皮内病变（CMIL）的准确分级对治疗和黑色素瘤预测至关重要，但由于形态学线索细微、诊断标准相互关联，分级仍然困难。需要开发能够同时考虑多个诊断标准并提供一致预测的计算方法。

Method: 提出INTERACT-CMIL框架：1）多头部架构联合预测WHO4、WHO5、水平扩散、垂直扩散和细胞异型性五个组织病理学轴；2）采用共享特征学习与组合部分监督；3）引入互依赖损失函数强制跨任务一致性；4）在三个大学医院收集的486个专家标注结膜活检切片数据集上进行训练和评估。

Result: 相比CNN和基础模型基线，INTERACT-CMIL取得显著改进：WHO4相对宏观F1提升55.1%，垂直扩散相对宏观F1提升25.0%。框架提供与专家分级一致、可解释的多标准预测，在CMIL诊断中表现优于现有方法。

Conclusion: INTERACT-CMIL为CMIL诊断提供了可重复的计算基准，通过联合预测多个组织病理学轴并强制跨任务一致性，实现了与专家分级对齐的连贯预测，是迈向标准化数字眼病理学的重要一步。
...
Result: 在标准基准测试中显著提升了文本到图像对应关系的人类对齐指标和视觉质量，在人类偏好分数和美学评估上达到与最先进奖励优化方法相当的结果。
...
TL;DR: 提出ARFM方法用于序列连续数据的概率建模，在多样化视频数据集上训练以生成长期未来点轨迹，并建立评估运动预测模型性能的基准


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常针对特定分布训练，应用范围有限；大规模视频预测模型虽然视觉真实感强，但难以准确建模复杂运动。需要一种能准确预测复杂运动并适用于下游任务的方法

Method: 开发自回归流匹配（ARFM）方法，用于序列连续数据的概率建模，在多样化视频数据集上训练以生成未来点轨迹位置，支持长期预测

Result: ARFM模型能够预测复杂运动，实验表明基于预测的未来轨迹来调节机器人动作预测和人体运动预测，能显著提升下游任务性能

Conclusion: ARFM方法在运动预测方面表现出色，通过预测未来轨迹来调节下游任务能有效提升性能，为运动预测领域提供了新的基准和方法
...
TL;DR: SCPainter：一个统一的自动驾驶仿真框架，结合3D高斯溅射资产表示和扩散模型，实现逼真的3D资产插入和新视角合成。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶训练需要多样化的数据，特别是长尾驾驶场景。现有方法分别处理3D资产插入和新视角合成，缺乏统一的框架来创建交互式、逼真的训练场景。

Method: 提出SCPainter框架，将3D高斯溅射（GS）车辆资产表示与3D场景点云结合，通过扩散模型进行联合生成。将3D GS资产和场景点云投影到新视角，用这些投影作为扩散模型的条件来生成高质量图像。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现逼真的3D资产插入和新视角合成，有助于创建多样化和逼真的驾驶数据。

Conclusion: SCPainter提供了一个统一的仿真框架，能够联合处理3D资产插入和新视角合成，为自动驾驶训练生成更丰富、更逼真的场景数据。
...
TL;DR: 本研究评估了超声特异性自监督预训练模型USF-MAE在早孕期超声图像中检测囊性水囊瘤的性能，结果显示其显著优于传统监督学习方法DenseNet-169基线模型。


<details>
  <summary>Details</summary>
Motivation: 囊性水囊瘤是高风险的产前超声发现，与染色体异常、结构畸形和不良妊娠结局密切相关。自动化检测可提高可重复性并支持可扩展的早期筛查项目，但监督深度学习方法受限于小规模标注数据集。

Method: 研究使用基于掩码自编码(MAE)的超声自监督基础模型(USF-MAE)，该模型在超过37万张未标注超声图像上预训练，然后针对本研究中的正常对照和囊性水囊瘤病例进行二元分类微调。评估采用与DenseNet-169基线相同的超声数据集、预处理流程和4折交叉验证协议，使用准确率、敏感性、特异性、ROC-AUC等指标，并通过Score-CAM可视化分析模型可解释性。

Result: USF-MAE在所有评估指标上均优于DenseNet-169基线模型：平均准确率0.96 vs 0.93，敏感性0.94 vs 0.92，特异性0.98 vs 0.94，ROC-AUC 0.98 vs 0.94。Score-CAM可视化显示模型预测具有临床相关性，能突出显示胎儿颈部的预期区域。Wilcoxon符号秩检验证实USF-MAE的性能改进具有统计学显著性(p=0.0057)。

Conclusion: 超声特异性自监督预训练能够促进准确、稳健的深度学习检测囊性水囊瘤，在有限标注数据的情况下显著提升模型性能，为早期产前筛查提供了有前景的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖视频分割质量，但视频分割结果不稳定，导致4D重建不可靠。需要消除对视频分割的依赖，直接从单帧分割图实现可靠的4D场景分解重建

Method: 使用Freetime FeatureGS表示动态场景：高斯基元具有可学习特征和线性运动能力，能在时间维度移动。应用对比损失使基元特征根据2D分割图中的实例归属进行聚合/分离。采用时序有序采样训练，实现特征在时间上的流式传播，避免优化陷入局部极小

Result: 在多个数据集上的实验结果表明，该方法的重建质量显著优于现有方法，实现了更可靠的4D场景分解重建

Conclusion: 提出的Freetime FeatureGS方法通过流式特征学习策略，成功从单帧分割图实现了高质量的4D场景分解重建，消除了对不稳定视频分割的依赖
...
TL;DR: 提出一种针对长上下文多图像场景的自适应视觉令牌剪枝方法，通过分解冗余为图像内和图像间两部分，动态分配预算，在保持性能的同时显著减少视觉令牌数量。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理多图像长上下文时，视觉令牌数量急剧增加导致推理成本高昂。现有视觉令牌剪枝方法往往忽视多图像长上下文场景，需要专门针对此类场景的剪枝方法。

Method: 将冗余分解为图像内冗余和图像间冗余，通过图像内多样性和图像间变化量化。采用两阶段方法：1) 图像内阶段为每张图像分配内容感知令牌预算并贪婪选择最具代表性令牌；2) 图像间阶段进行全局多样性过滤形成候选池，然后应用帕累托选择平衡多样性与文本对齐。

Result: 大量实验表明，该方法在长上下文设置下保持强大性能的同时，显著减少了视觉令牌数量。

Conclusion: 提出的自适应视觉令牌剪枝方法有效解决了多图像长上下文场景中的计算效率问题，通过动态预算分配和两阶段选择机制，在保持模型性能的同时大幅降低推理成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有ViT令牌合并与剪枝方法常忽视空间连续性和邻居关系，导致局部上下文信息丢失，限制了计算效率的优化

Method: 提出基于希尔伯特曲线重排序的邻居感知令牌缩减方法：1) 邻居感知剪枝(NAP)进行选择性令牌保留；2) 基于相邻令牌相似性的合并(MAT)进行局部令牌聚合
...
<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多等级质量描述文本嵌入之间的相似性来评估图像质量，但发现这种相似性分布通常呈现多模态模式（语义漂移现象），即图像嵌入可能同时与"优秀"和"差"等级描述高度相似，而与"良好"等级偏离，这种文本嵌入与其预期描述之间的语义不一致性削弱了文本-图像共享空间学习的可靠性

Method: 受心理测量学启发，提出改进的等级反应模型（GRM）用于AGIQA，设计双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度等级；为确保难度等级的单调性，以算术方式建模难度生成，强制产生单峰且可解释的质量分布；该AGQG模块具有即插即用优势

Result: AGQG模块在集成到各种最先进的AGIQA框架中时能持续提升性能，同时在自然图像和屏幕内容图像质量评估中都能有效泛化，显示出作为未来IQA模型关键组件的潜力

Conclusion: 提出的算术GRM质量分级模块能有效缓解AGIQA中的语义漂移问题，通过心理测量学启发的等级反应模型提供更可靠、可解释的图像质量评估，具有广泛的适用性和泛化能力
...
TL;DR: EPD-Solver是一种新颖的ODE求解器，通过并行梯度评估减少扩散模型采样中的截断误差，保持低延迟特性，并采用两阶段优化框架提升文本到图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成性能优异，但顺序去噪过程导致高采样延迟。现有基于求解器的加速方法在低延迟预算下常出现图像质量显著下降，主要原因是无法捕捉高曲率轨迹段导致的累积截断误差。

Method: 提出EPD-Solver（集成并行方向求解器），利用向量值函数中值定理更精确近似积分解；采用两阶段优化：先通过蒸馏方法优化可学习参数，再提出参数高效的强化学习微调方案，将求解器重新表述为随机Dirichlet策略；还可作为插件（EPD-Plugin）改进现有ODE采样器。

Result: EPD-Solver通过并行梯度计算减少截断误差，保持低延迟采样特性；强化学习微调在复杂文本到图像生成任务中有效缓解奖励攻击并提升性能；方法灵活可作为插件改进现有采样器。

Conclusion: EPD-Solver通过集成并行梯度评估有效缓解扩散模型加速中的截断误差问题，结合两阶段优化框架在保持低延迟的同时提升生成质量，为扩散模型高效采样提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言跟踪方法通常局限于局部搜索，在视角变化、遮挡和目标快速移动等挑战性场景下容易失败。需要一种全局跟踪框架来提高鲁棒性并减少漂移。

Method: 提出VPTracker，首个基于多模态大语言模型的全局跟踪框架。引入位置感知视觉提示机制，基于目标先前位置构建区域级提示，使模型优先进行区域级识别，仅在必要时进行全局推理。

Result: 大量实验表明，该方法在挑战性场景下显著提升了跟踪稳定性和目标辨别能力，为将多模态大语言模型集成到视觉跟踪中开辟了新途径。

Conclusion: VPTracker通过结合全局搜索的优势和位置感知视觉提示机制，有效解决了全局跟踪中的干扰问题，展示了多模态大语言模型在视觉跟踪领域的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测旨在实现接近人类水平的识别能力，但现有模型在真实世界低质量图像条件下的性能尚未得到充分评估。研究旨在填补这一空白，评估模型在图像退化条件下的鲁棒性。

Method: 创建模拟真实世界低质量图像的新数据集，对多种开放词汇目标检测模型（包括OWLv2、OWL-ViT、GroundingDINO、Detic等）进行系统性评估，分析它们在低水平和高级图像退化条件下的性能变化。

Result: 在低水平图像退化条件下，模型mAP分数无明显下降；但在高级图像退化条件下，所有模型性能急剧下降。OWLv2在不同类型退化中表现相对稳定，而OWL-ViT、GroundingDINO和Detic则出现显著性能下降。

Conclusion: 开放词汇目标检测模型对高级图像退化敏感，当前模型在真实世界低质量图像条件下的鲁棒性仍需改进。研究提供了新数据集和代码以促进未来研究。
...
TL;DR: 提出EgoReAct框架，首个自回归模型从第一人称视频实时生成3D对齐的人体反应动作，解决了现有方法在因果生成和空间对齐方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 人类对第一人称视觉输入具有适应性和上下文敏感的响应，但现有方法难以同时满足严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在显著的空间不一致问题，如动态动作总是与固定摄像头视频配对。

Method: 1) 构建Human Reaction Dataset (HRD)解决数据稀缺和错位问题；2) 提出EgoReAct自回归框架：先通过VQ-VAE将反应动作压缩到紧凑的潜在空间，再训练GPT模型从视觉输入生成反应；3) 整合3D动态特征（度量深度和头部动态）增强空间定位。

Result: EgoReAct在真实性、空间一致性和生成效率方面显著优于现有方法，同时保持严格的因果生成。实验证明其能够实时生成3D对齐的人体反应动作。

Conclusion: EgoReAct是首个能够从第一人称视频流实时生成3D对齐人体反应动作的自回归框架，通过构建HRD数据集和整合3D动态特征，有效解决了空间对齐和因果生成的挑战。
...
TL;DR: SCaR-3D：一种基于3D高斯溅射的3D场景变化检测框架，通过2D差分和多视图聚合实现对象级变化检测，支持持续场景重建
...
Method: 1) 基于符号距离的2D差分模块；2) 带投票和剪枝的多视图聚合；3) 利用3DGS一致性分离变化前后状态；4) 选择性更新动态区域的持续重建策略

Result: 方法在精度和效率上均优于现有方法，并贡献了CCS3D合成数据集支持可控评估

Conclusion: SCaR-3D框架能够鲁棒地检测对象级3D变化并支持持续场景重建，为场景监控和探索提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶、智能手机和无人机等应用中，相机常与IMU结合使用。IMU可以提供相机垂直方向信息，从而将相对位姿从5自由度减少到3自由度。现有方法需要更多对应点或无法充分利用已知垂直方向信息，因此需要更高效的求解器。

Method: 首先，在已知垂直方向的情况下，从两个仿射对应建立约束方程。然后，基于具有非平凡解的方程系统性质，推导出仅涉及两个参数（焦距和相对旋转角度）的四个方程。最后，利用多项式特征值方法求解焦距和相对旋转角度问题。

Result: 在合成和真实数据集上的评估表明，所提出的求解器性能优于现有的最先进求解器。

Conclusion: 该方法通过结合IMU提供的垂直方向信息，仅需两个仿射对应就能有效估计3自由度相对位姿和焦距，在计算效率和精度方面优于现有方法。
...
TL;DR: ByteLoom是一个基于扩散Transformer的框架，用于生成具有几何一致对象插图的逼真人-物交互视频，解决了现有方法在多视图一致性和手部网格依赖方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互视频生成方法存在两个关键限制：1) 缺乏有效的多视图信息注入机制，导致跨视图一致性差；2) 严重依赖精细的手部网格标注来建模交互遮挡。这些限制阻碍了在数字人、电子商务、广告和机器人模仿学习等应用中的发展。

Method: 提出ByteLoom框架，采用扩散Transformer架构，使用简化的人体条件和3D对象输入。核心创新包括：1) RCM-cache机制，利用相对坐标图作为通用表示来保持对象几何一致性并精确控制6自由度对象变换；2) 渐进式训练课程，增强模型能力并减少对手部网格的依赖。

Result: 实验表明，该方法能够忠实保持人体身份和对象的多视图几何特性，同时维持平滑的运动和对象操控。相比现有方法，在跨视图一致性和对象几何保持方面表现更优。

Conclusion: ByteLoom通过创新的RCM-cache机制和渐进式训练策略，有效解决了人-物交互视频生成中的多视图一致性和手部网格依赖问题，为实际应用提供了更实用的解决方案。
...
TL;DR: MUSON是一个用于短时域社交导航的多模态数据集，采用五步思维链标注，包含感知、预测、推理、动作和解释，解决了现有数据集缺乏显式推理监督和动作分布长尾的问题。


<details>
  <summary>Details</summary>
Motivation: 现有社交导航数据集缺乏显式推理监督，且动作分布呈现高度长尾，限制了模型学习安全关键行为的能力。需要构建一个包含结构化推理标注和平衡动作空间的数据集来改进社交导航模型的训练与评估。

Method: 在多样化的室内外校园场景中收集多模态数据，采用五步思维链标注：感知（环境状态）、预测（行人轨迹）、推理（安全约束）、动作（导航决策）、解释（决策依据）。显式建模静态物理约束，构建理性平衡的离散动作空间。

Result: 相比SNEI数据集，MUSON提供更一致的推理、动作和解释标注。在多个先进小视觉语言模型上的基准测试显示，Qwen2.5-VL-3B达到最高决策准确率0.8625，证明MUSON可作为社交合规导航的有效可复用基准。

Conclusion: MUSON通过结构化思维链标注和平衡动作空间，解决了社交导航数据集的关键限制，为训练和评估社交合规导航模型提供了高质量基准，数据集已公开可用。
...
TL;DR: Lamps是一种通过多视角自监督学习解剖结构的医学影像基础模型，利用人体解剖的一致性、连贯性和层次性作为监督信号，在胸部X光片上预训练，展现出优越的鲁棒性、可迁移性和临床潜力。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在医学影像中往往忽视人体解剖的关键基础特性（一致性、连贯性、层次性），限制了有效学习解剖特征的能力。医学影像直接反映人体内部结构，其基础在于解剖学而非自然语言处理中的语言结构。

Method: 提出Lamps模型，通过自监督学习从多视角学习解剖结构，利用人体解剖的一致性、连贯性和层次性作为监督信号，在大规模胸部X光片数据集上进行预训练。

Result: 在10个数据集上通过微调和涌现特性分析进行广泛实验，与10个基线模型相比，Lamps展现出优越的鲁棒性、可迁移性和临床潜力。

Conclusion: 通过多视角学习解剖结构，Lamps为医学影像基础模型提供了独特机会，能够开发出与人体解剖结构对齐的有意义、鲁棒的表示，解决了现有方法忽视解剖学基础的局限性。
...
TL;DR: 提出一种数据导向的方法NSF来缓解深度学习模型中的伪相关，通过识别、中和、消除和更新四个步骤构建去偏流程，在图像和NLP基准上相比ERM将最差组准确率提升超过20%。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在训练中经常学习到与类别标签伪相关但与预测任务无关的特征。现有方法通常需要标注潜在伪属性或基于经验假设（如偏见的简单性）过滤伪特征，但由于现实数据中伪相关的复杂性和难以捉摸性，这些方法可能表现不佳。

Method: 提出NSF方法：1）识别：观察到受伪特征影响的样本在特征空间中呈现分散分布，据此识别伪特征存在；2）中和：基于简单分组策略中和伪特征，获得偏置不变表示；3）消除：学习特征变换与偏置不变表示对齐，消除伪特征；4）更新：结合学习到的特征变换更新分类器，获得无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提升超过20%。代码和检查点已开源。

Conclusion: 通过识别、中和、消除和更新四个步骤构建的数据导向去偏流程NSF能有效缓解深度学习模型中的伪相关问题，显著提升模型在最差组上的性能，且无需依赖伪属性标注或经验假设。
...
TL;DR: 提出了M-ErasureBench多模态评估框架和IRECE方法，用于系统评估和增强文本到图像扩散模型的概念擦除能力，解决现有方法仅针对文本提示而忽视其他输入模态（学习嵌入和反转潜在表示）的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要关注文本提示，忽视了图像编辑和个性化生成等实际应用中日益重要的其他输入模态（如学习嵌入和反转潜在表示），这些模态可能成为攻击面，导致已擦除的概念重新出现。

Method: 1. 提出M-ErasureBench多模态评估框架，系统评估三种输入模态（文本提示、学习嵌入、反转潜在表示）的概念擦除效果，包括白盒和黑盒访问设置，共五种评估场景。2. 提出IRECE（推理时鲁棒性增强概念擦除）方法，通过交叉注意力定位目标概念并在去噪过程中扰动相关潜在表示，以增强擦除鲁棒性。

Result: 现有方法在文本提示上表现良好，但在学习嵌入和反转潜在表示上失败严重，白盒设置下概念重现率（CRR）超过90%。IRECE方法能显著恢复鲁棒性，在最具挑战性的白盒潜在反转场景下将CRR降低达40%，同时保持视觉质量。
...
TL;DR: GPS（引导路径采样）通过将CFG的外推改为流形约束插值，解决了迭代细化方法中CFG导致采样路径偏离数据流形的问题，实现了稳定的路径采样和语义对齐。
...
Method: 提出引导路径采样（GPS）新范式，用原则性的流形约束插值替代不稳定的外推，确保采样路径保持在数据流形上。理论证明这种校正能将误差序列从无界放大转变为严格有界。同时设计最优调度策略，动态调整引导强度，使语义注入与模型从粗到细的生成过程对齐。
...
Conclusion: 路径稳定性是有效迭代细化的前提条件，GPS为实现这一目标提供了稳健框架。通过将CFG的外推改为流形约束插值，GPS解决了迭代细化中的稳定性问题，显著提升了生成质量和语义对齐能力。
...
Conclusion: 哈希网格特征剪枝方法能有效消除高斯溅射表示中的冗余特征，提升压缩效率，为隐式神经场学习提供更高效的存储方案
...
TL;DR: JavisGPT是首个用于联合音频-视频理解与生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过SyncFusion模块实现时空音频-视频融合，支持从多模态指令生成时间一致的视频-音频内容。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在联合音频-视频理解与生成方面存在局限，特别是在时间同步和复杂场景处理上。需要开发一个统一的模型来处理音频-视频的联合理解与生成任务，实现时间一致的多模态内容生成。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音频-视频融合，使用同步感知可学习查询桥接预训练的JAV-DiT生成器。设计三阶段训练流程：多模态预训练、音频-视频微调和大规模指令调优。构建了JavisInst-Omni数据集，包含20万+GPT-4o策划的音频-视频-文本对话。

Result: 在JAV理解和生成基准测试中，JavisGPT超越了现有MLLM，特别是在复杂和时间同步设置下表现优异。模型能够实现时间一致的视频-音频理解和生成。

Conclusion: JavisGPT是首个统一的音频-视频多模态大语言模型，通过创新的架构设计和训练策略，在联合音频-视频理解与生成任务上取得了显著进展，为多模态AI系统提供了新的解决方案。
...
### [68] [OpenGround: Active Cognition-based Reasoning for Open-World 3D Visual Grounding](https://arxiv.org/abs/2512.23020)
*Wenyuan Huang,Zhao Wang,Zhou Wei,Ting Huang,Fang Zhao,Jian Yang,Zhenyu Zhang*

Main category: cs.CV

TL;DR: OpenGround：一种基于主动认知推理的零样本开放世界3D视觉定位框架，通过动态更新对象查找表解决预定义目标限制问题


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法依赖预定义的对象查找表（OLT）查询视觉语言模型（VLMs）进行对象定位推理，这限制了在未定义或意外目标场景中的应用。需要解决开放世界场景中的3D视觉定位问题。

Method: 提出OpenGround框架，核心是主动认知推理（ACR）模块。ACR通过认知任务链进行类人目标感知，主动推理上下文相关对象，通过动态更新的OLT扩展VLM认知能力。框架支持预定义和开放世界类别，并创建了OpenTarget数据集进行评估。

Result: 在Nr3D数据集上取得竞争性性能，在ScanRefer数据集上达到最先进水平，在OpenTarget数据集上实现17.6%的显著提升。证明了在开放世界场景中的有效性。

Conclusion: OpenGround通过主动认知推理模块解决了预定义OLT的限制，实现了零样本开放世界3D视觉定位，为处理未定义和意外目标提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的规划器面临三个关键挑战：1）离散文本推理与连续控制之间的不匹配；2）自回归链式思维解码导致的高延迟；3）低效或非因果规划器限制实时部署。需要保留VLM的泛化性和可解释性，同时实现高效、准确和安全的轨迹生成。
...
Conclusion: ColaVLA成功地将VLM的推理能力转移到统一潜在空间，通过分层并行解码实现了高效、准确的轨迹生成，在保持VLM泛化性和可解释性的同时解决了实时部署的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测器依赖交叉熵分类，容易受到类别不平衡和标签噪声的影响。需要一种能够增强检测鲁棒性和性能的方法
...
Conclusion: 使用可学习文本嵌入的联合优化显著增强了闭集检测性能，适用于多种架构和数据集，且框架简单、与检测器无关
...
TL;DR: YOLO-IOD：首个基于YOLO的实时增量目标检测框架，解决YOLO检测器中的灾难性遗忘问题，在消除数据泄露的LoCo COCO基准上表现优异


<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR系列检测器，无法适应实时YOLO检测框架。需要解决YOLO检测器中导致灾难性遗忘的三种知识冲突问题

Method: 基于预训练YOLO-World模型构建，采用阶段式参数高效微调。包含三个核心组件：1) 冲突感知伪标签细化(CPR)缓解前景-背景混淆；2) 重要性核选择(IKS)识别并更新当前任务关键卷积核；3) 跨阶段非对称知识蒸馏(CAKD)解决知识蒸馏不对齐问题

Result: 在传统和LoCo COCO基准测试中，YOLO-IOD实现了卓越性能且遗忘最小。LoCo COCO是更现实的基准，消除了阶段间数据泄露

Conclusion: YOLO-IOD是首个基于YOLO的实时增量目标检测框架，有效解决了YOLO检测器中的灾难性遗忘问题，为实时增量检测提供了实用解决方案
...
### [72] [PathoSyn: Imaging-Pathology MRI Synthesis via Disentangled Deviation Diffusion](https://arxiv.org/abs/2512.23130)
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二元掩码，这些范式存在特征纠缠问题，导致解剖基底损坏或结构不连续。需要一种能够保持解剖完整性同时生成高质量病理变化的方法。

Method: 将合成任务分解为确定性解剖重建和随机偏差建模。核心是偏差空间扩散模型，学习病理残差的条件分布。结合接缝感知融合策略和推理时稳定模块，确保空间一致性并抑制边界伪影。

Result: 在肿瘤成像基准测试中，PathoSyn在感知真实性和解剖保真度方面显著优于整体扩散和掩码条件基线方法。能够生成高保真患者特异性合成数据集。

Conclusion: PathoSyn提供了一个数学原理化的流程，用于生成高质量合成数据，支持可解释的反事实疾病进展建模，促进低数据环境下稳健诊断算法的发展，并为临床决策支持系统提供基准测试环境。
...
<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法存在两个主要问题：1）由于视觉相似性不足导致伪装效果不够；2）背景杂乱且与前景目标语义不一致。这些缺陷限制了生成图像的质量，无法为伪装目标检测提供高质量的训练数据。
...
Result: 提出了背景-前景分布差异度量来定量评估伪装质量。通过大量实验和可视化验证了所提框架的有效性。

Conclusion: ReamCamo通过布局控制和多模态条件生成，显著提升了伪装图像的真实性和语义一致性，为伪装目标检测提供了更高质量的训练数据生成方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在高速和低光场景下存在运动模糊问题，而事件相机虽然具有高时间分辨率，但现有的6DoF姿态估计方法在高速物体运动场景下性能不佳

Method: 提出PoseStreamer框架，包含三个核心组件：1）自适应姿态记忆队列，利用历史方向线索保持时间一致性；2）物体中心2D跟踪器，提供强2D先验以提升3D中心召回；3）射线姿态滤波器，沿相机射线进行几何细化

Result: 在高速运动场景下实现了优越的精度，并作为无模板框架对未见过的运动物体表现出强大的泛化能力。同时构建了MoCapCube6D多模态数据集用于快速运动下的性能基准测试

Conclusion: PoseStreamer为解决高速运动场景下的6DoF姿态估计问题提供了有效的解决方案，通过多模态融合和时间一致性机制显著提升了性能
...
TL;DR: ForCM方法结合对象基图像分析和深度学习，使用Sentinel-2影像进行森林覆盖制图，在亚马逊雨林区域验证，相比传统OBIA方法精度提升至95.64%


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法精度有限，需要开发结合对象基图像分析和深度学习的新方法，利用免费易用的工具如QGIS进行准确监测，支持全球环境保护

Method: 提出ForCM方法：1) 使用Sentinel-2 Level 2A多光谱影像；2) 评估UNet、UNet++、ResUNet、AttentionUNet、ResNet50-Segnet等深度学习模型；3) 将最佳DL模型与OBIA技术集成；4) 使用三组数据集（两个三波段，一个四波段）；5) 与传统OBIA方法对比

Result: ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，相比传统OBIA的92.91%显著提升，证明DL与OBIA结合的有效性

Conclusion: ForCM方法成功结合DL和OBIA，显著提高森林覆盖制图精度，展示了免费工具如QGIS在环境监测中的潜力，为全球森林保护和监测提供有效技术方案
...
TL;DR: 提出SSA框架解决医学图像分割中文本引导的局限性，通过对称最优传输对齐机制和复合方向引导策略，增强位置约束和混合文本处理能力


<details>
  <summary>Details</summary>
Motivation: 现有文本引导医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断性和描述性文本，难以建立图像区域与文本的关联；2）缺乏位置约束，导致分割结果出现关键偏差（如"左下肺"可能错误覆盖双侧肺）

Method: 提出空间感知对称对齐（SSA）框架：1）对称最优传输对齐机制，建立图像区域与多个相关表达之间的双向细粒度多模态对应；2）复合方向引导策略，通过构建区域级引导掩码显式引入文本中的空间约束

Result: 在公开基准测试上的大量实验表明，SSA实现了最先进的性能，特别是在准确分割具有空间关系约束特征的病变方面表现突出

Conclusion: SSA框架通过增强混合医学文本（位置、描述、诊断信息）的处理能力，有效解决了现有方法在位置约束和文本处理方面的局限性，为文本引导医学图像分割提供了更准确的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的身份特征移除或修改方法要么依赖于预训练模型中已有良好表示的主体，要么需要对特定身份进行模型微调，缺乏对未见主体的泛化能力和对面部属性的控制。

Method: 采用条件扩散反演框架，无需文本提示即可直接操作图像；引入身份引导的条件分支以泛化到训练数据之外的主体；支持属性可控的匿名化。

Result: 该方法在身份移除、属性保留和图像质量之间实现了最先进的平衡，优于现有匿名化方法。

Conclusion: 提出的反向个性化框架为面部匿名化提供了一种有效且可控的解决方案，能够处理未见主体并保持面部属性，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 军事目标检测在民用和军事应用中至关重要，但现有域适应算法主要针对自然或自动驾驶场景，而军事领域环境多样且缺乏真实数据集。SAR数据成本高，RGB相机成本低但缺乏军事目标检测数据集。

Method: 使用虚幻引擎生成RGB合成数据用于军事目标检测，进行合成到真实的迁移实验。在提出的训练-验证数据集对上，对基于不同监督程度（无监督、半监督、使用最小提示）的先进域适应方法进行基准测试。

Result: 使用图像中最小提示（如对象类别）的当前方法在无监督或半监督域适应方法基础上取得了显著改进。研究识别了当前仍需克服的挑战。

Conclusion: 通过生成合成数据可以缓解军事目标检测中数据稀缺和高成本问题，但跨域适应在军事场景中仍面临挑战，需要进一步研究改进域适应方法。
...
TL;DR: 提出基于低成本RGB无人机和边缘计算的果园智能管道，集成ResNet50、VGG16和YOLOv8模型，实现叶片病害检测、苹果新鲜度评估和实时苹果检测定位，无需云端支持。


<details>
  <summary>Details</summary>
Motivation: 现有无人机系统通常孤立处理果园任务且依赖昂贵的多光谱传感器，需要统一、低成本的解决方案来支持精准农业的普及应用。

Method: 构建基于RGB无人机的统一智能管道：使用ResNet50进行叶片病害分类，VGG16评估苹果新鲜度，YOLOv8实现实时苹果检测与定位；系统在ESP32-CAM和树莓派上运行，支持完全离线的现场推理。

Result: 实验显示叶片病害分类准确率达98.9%，新鲜度分类准确率97.4%，苹果检测F1分数0.857，为多光谱无人机解决方案提供了经济可行的替代方案。

Conclusion: 该框架通过低成本硬件和RGB传感器实现了果园多任务智能分析，支持可扩展的精准农业应用，降低了技术门槛和部署成本。
...
### [80] [Holi-DETR: Holistic Fashion Item Detection Leveraging Contextual Information](https://arxiv.org/abs/2512.23221)
*Youngchae Kwon,Jinyoung Choi,Injung Kim*

Main category: cs.CV

TL;DR: Holi-DETR：一种利用三种上下文信息（共现关系、物品间空间关系、物品与人体关键点关系）来整体检测时尚物品的检测器，显著提升了DETR系列模型的性能


<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临两大挑战：1）时尚物品外观高度多样化带来的歧义性；2）物品子类别之间的相似性。传统检测器独立检测每个物品，无法利用时尚物品之间的有意义关系来减少歧义。

Method: 提出Holi-DETR（整体检测Transformer），在DETR及其后续模型基础上，整合三种异构上下文信息：1）时尚物品间的共现概率关系；2）基于物品间空间排列的相对位置和大小关系；3）物品与人体关键点之间的空间关系。

Result: 实验表明，该方法显著提升了检测性能：相比原始DETR提升了3.6个百分点（pp）的平均精度（AP），相比近期开发的Co-DETR提升了1.1个百分点的AP。

Conclusion: 通过整体检测方法利用多种上下文信息，可以有效减少时尚物品检测中的歧义，显著提升检测性能。Holi-DETR为时尚物品检测提供了一种新颖且有效的解决方案。
...
TL;DR: 提出Geo-Semantic Contextual Graph (GSCG)框架，通过构建包含几何、颜色、材质属性和空间关系的显式图结构，实现基于场景上下文的目标分类，显著优于传统方法和LLM。


<details>
  <summary>Details</summary>
Motivation: 人类识别物体时能充分利用丰富的场景上下文信息（空间关系、材质属性、物体共现等），而现有计算系统通常在孤立图像区域上操作，忽略了这些关键上下文信息，导致性能受限。

Method: 1) 从单目图像构建Geo-Semantic Contextual Graph (GSCG)：结合度量深度估计器和统一的全景分割与材质分割模型，将物体编码为节点（包含几何、颜色、材质属性），空间关系编码为边；2) 设计专门的图分类器，聚合目标物体、邻近物体和全局场景特征进行分类预测。

Result: 在COCO 2017数据集上，上下文感知模型达到73.4%的分类准确率，显著优于无上下文版本（最低38.4%）、微调ResNet模型（最高53.5%）和最先进的多模态LLM（Llama 4 Scout，最高42.3%）。

Conclusion: 显式结构化和可解释的上下文表示对于目标识别任务具有显著优势，GSCG框架通过整合几何、语义和空间关系信息，实现了超越传统方法和LLM的性能，同时保持了推理过程的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 工业制造中异常检测至关重要，但真实缺陷图像稀缺。现有合成方法存在明显权衡：基于规则的合成成本低但真实性不足，基于生成模型的合成质量高但成本昂贵。需要一种既能高效生成高质量缺陷图像，又能有效利用合成数据提升异常检测性能的方法。

Method: 提出新颖框架：1) 利用预训练文本引导图像到图像翻译模型生成合成缺陷图像；2) 使用图像检索模型评估生成图像与真实正常图像的相似性，过滤无关输出以提升质量和相关性；3) 引入两阶段训练策略：先在大量基于规则合成的图像上进行预训练，然后在少量高质量图像上进行微调。

Result: 在MVTec AD数据集上的实验证明了方法的有效性。该方法显著降低了数据收集成本，同时提高了异常检测性能。

Conclusion: 提出的框架通过结合预训练翻译模型和图像检索模型，能够高效生成高质量的合成缺陷图像。两阶段训练策略有效利用了合成图像，在降低数据收集成本的同时提升了异常检测性能，为解决工业制造中缺陷图像稀缺问题提供了有效方案。
...
Method: 提出Co2S框架：1)构建异构双学生架构，包含预训练的CLIP和DINOv3 ViT模型；2)引入显式-隐式语义协同引导机制，利用文本嵌入和可学习查询提供显式和隐式类别级引导；3)开发全局-局部特征协同融合策略，融合CLIP的全局上下文信息和DINOv3的局部细节。
...
TL;DR: MedGemma-4b-it（LoRA微调）在六种疾病诊断中表现优于GPT-4，平均测试准确率达80.37% vs 69.58%，在癌症和肺炎等高风险任务中敏感性更高。
...
Method: 使用低秩适应（LoRA）对MedGemma-4b-it模型进行微调，与未调优的GPT-4进行对比，评估六种不同疾病的诊断能力。通过混淆矩阵和分类报告进行定量分析。

Result: MedGemma-4b-it平均测试准确率为80.37%，显著高于GPT-4的69.58%。在癌症和肺炎检测等高风险临床任务中，MedGemma表现出更高的敏感性。领域特定微调有效减少了幻觉现象。

Conclusion: 领域特定微调对于最小化临床实施中的幻觉至关重要，MedGemma凭借其卓越的诊断能力和证据驱动的医学推理，成为复杂医疗决策的先进工具。
...
TL;DR: SoulX-LiveTalk是一个14B参数框架，用于实时、无限时长、音频驱动的虚拟人生成，通过自校正双向蒸馏策略和多步回顾自校正机制，在保持高视觉保真度的同时实现亚秒级启动延迟和32 FPS实时吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在实时音频驱动虚拟人生成中存在计算负载与严格延迟约束的冲突，通常通过强制单向注意力机制或降低模型容量来妥协视觉保真度，需要一种既能保持高质量又能满足实时性要求的解决方案。

Method: 1. 自校正双向蒸馏策略：在视频块内保留双向注意力，保持关键的时空相关性；2. 多步回顾自校正机制：使模型能够从累积错误中自主恢复，防止崩溃；3. 全栈推理加速套件：包括混合序列并行、并行VAE和内核级优化。

Result: SoulX-LiveTalk是首个14B规模系统，实现亚秒级启动延迟（0.87秒）和32 FPS实时吞吐量，在保持高视觉保真度的同时满足实时交互要求。

Conclusion: 该框架通过创新的双向注意力保留和自校正机制，解决了大规模扩散模型在实时音频驱动虚拟人生成中的工程挑战，为高保真交互式数字人合成设立了新标准。
...
TL;DR: 提出基于天气条件和路面状况数据的实时道路监测系统，使用手机摄像头采集数据，测试多种深度学习算法进行道路分类，并比较加速度数据和图像数据的性能，达到95%以上准确率。


<details>
  <summary>Details</summary>
Motivation: 传统道路监测方法昂贵且非系统性，需要大量测量时间。需要开发实时、系统的道路状态监测系统，为车辆规划和主动控制系统提供有价值信息。

Method: 1. 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路采集数据；2. 测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类；3. 将道路加速度数据与图像数据结合训练，将加速度数据转换为图像格式；4. 比较基于加速度和基于摄像头图像的方法性能；5. 使用模糊逻辑根据天气和时间对道路表面进行分类。

Result: 1. 道路状况分类考虑了5个类别：沥青路、损坏沥青路、碎石路、损坏碎石路、铺砌路；2. 实现了超过95%的准确率性能；3. 比较了不同深度学习算法的性能表现。

Conclusion: 提出的实时系统能够有效监测道路表面状态，结合深度学习算法和模糊逻辑方法，为车辆控制和规划提供可靠的道路状况信息，解决了传统方法昂贵和非系统性的问题。
...
TL;DR: GeoTeacher是一种半监督3D目标检测方法，通过几何关系监督模块和体素级数据增强策略，提升学生模型在有限标注数据下对物体几何关系的理解能力。
...
Method: 1. 基于关键点的几何关系监督模块：将教师模型对物体几何的知识传递给学生模型，提升学生对几何关系的理解能力。2. 体素级数据增强策略：增加物体几何的多样性，增强学生对几何结构的理解。3. 距离衰减机制：在数据增强过程中保护远距离物体的完整性。4. 可与不同SS3D方法结合使用。

Result: 在ONCE和Waymo数据集上的大量实验表明该方法有效且具有良好泛化能力，取得了新的最先进结果。

Conclusion: GeoTeacher通过几何关系监督和增强策略，有效解决了半监督3D目标检测中几何信息捕捉不足的问题，提升了模型性能，并可与其他方法结合进一步提升效果。
...
TL;DR: CoFi-Dec是一种无需训练的解码框架，通过生成自我反馈和粗到细视觉条件来减少大视觉语言模型的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 尽管大视觉语言模型在多模态理解和生成方面取得了显著进展，但它们仍倾向于产生与视觉输入不一致的幻觉内容，这限制了其在现实应用中的可靠性

Method: 受人类从全局场景感知到细节检查的视觉过程启发，CoFi-Dec首先生成基于原始图像粗粒度和细粒度视图的中间文本响应，然后使用文生图模型将这些响应转换为合成图像，形成多级视觉假设。引入基于Wasserstein距离的融合机制，将多个视觉条件的预测分布对齐到几何一致的解码轨迹中

Result: 在六个专注于幻觉的基准测试上的广泛实验表明，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略

Conclusion: 该框架是模型无关的，无需额外训练，可以无缝应用于广泛的大视觉语言模型。通过将生成自我反馈与粗到细视觉条件相结合，实现了更稳健和忠实的输出
...
### [89] [REVEALER: Reinforcement-Guided Visual Reasoning for Element-Level Text-Image Alignment Evaluation](https://arxiv.org/abs/2512.23169)
*Fulin Shi,Wenyi Xiao,Bin Chen,Liang Din,Leilei Gan*

Main category: cs.CV

TL;DR: REVEALER是一个基于强化引导视觉推理的文本-图像对齐评估框架，通过"定位-推理-结论"范式实现细粒度元素级对齐评估，在多个基准测试中达到SOTA性能。
...
Method: 采用结构化"定位-推理-结论"范式，利用多模态大语言模型显式定位语义元素并推导可解释的对齐判断；通过GRPO优化，使用包含结构格式、定位准确性和对齐保真度的复合奖励函数。

Result: 在四个基准测试(EvalMuse-40K、RichHF、MHaluBench、GenAI-Bench)上达到最先进性能，优于强大的专有模型和监督基线，相比现有迭代视觉推理方法具有更优的推理效率。

Conclusion: REVEALER提供了一个统一的元素级对齐评估框架，通过强化引导的视觉推理实现了细粒度、可解释的文本-图像对齐评估，在性能和效率方面均表现出色。
...
Method: 采用全阶段训练范式：1）在3000+小时运动数据上进行大规模预训练；2）在400小时精选数据上进行高质量微调；3）结合人类反馈和奖励模型的强化学习。同时建立了严谨的数据处理流程，包括运动清洗和标注。

Result: 模型在运动生成领域首次将基于扩散Transformer的流匹配模型扩展到十亿参数规模，指令跟随能力显著超越当前开源基准，覆盖6大类超过200个运动类别，实现了最广泛的运动覆盖范围。

Conclusion: HY-Motion 1.0代表了3D人体运动生成领域的重大进展，通过创新的全阶段训练范式和规模化方法，为未来研究和商业化应用奠定了基础，已开源以促进社区发展。
...
### [91] [GVSynergy-Det: Synergistic Gaussian-Voxel Representations for Multi-View 3D Object Detection](https://arxiv.org/abs/2512.23176)
*Yi Zhang,Yi Wang,Lei Yao,Lap-Pui Chau*

Main category: cs.CV

TL;DR: GVSynergy-Det：一种通过高斯-体素协同表示学习增强图像3D检测的框架，无需深度或密集3D监督，在室内基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像的3D检测方法面临两个关键挑战：高精度方法需要密集3D监督，而无监督方法难以从图像中提取准确几何信息。需要一种既能实现高精度又无需密集3D监督的解决方案。
...
Conclusion: GVSynergy-Det通过高斯和体素表示的协同学习，有效解决了图像3D检测中精度与监督需求的矛盾，为无密集3D监督的高精度3D检测提供了新思路。
...
TL;DR: 提出基于3D高斯场景表示的统一驾驶世界模型框架，实现3D场景理解和多模态场景生成，通过语言特征嵌入实现早期模态对齐，并设计任务感知的语言引导采样策略和双条件多模态生成模型。
...
Method: 1) 基于3D高斯场景表示的统一框架；2) 将丰富语言特征嵌入每个高斯基元，实现早期模态对齐；3) 设计任务感知的语言引导采样策略，去除冗余3D高斯并注入精确紧凑的3D令牌到LLM；4) 设计双条件多模态生成模型，结合高层语言条件和低层图像条件共同指导生成过程。

Result: 在nuScenes和NuInteract数据集上进行全面研究验证框架有效性，方法实现了最先进的性能。

Conclusion: 提出的基于3D高斯场景表示的驾驶世界模型框架成功解决了现有方法的局限性，实现了3D场景理解和多模态场景生成，通过早期模态对齐和双条件生成机制提升了性能，代码将在GitHub上公开。
...
### [93] [AnyMS: Bottom-up Attention Decoupling for Layout-guided and Training-free Multi-subject Customization](https://arxiv.org/abs/2512.23537)
*Binhe Yu,Zhen Wang,Kexin Li,Yuqian Yuan,Wenqiao Zhang,Long Chen,Juncheng Li,Jun Xiao,Yueting Zhuang*

Main category: cs.CV

TL;DR: AnyMS是一个无需训练的布局引导多主体定制框架，通过双级注意力解耦机制平衡文本对齐、主体身份保持和布局控制，无需额外训练即可支持复杂构图和更多主体。


<details>
  <summary>Details</summary>
Motivation: 现有多主体定制方法在平衡文本对齐、主体身份保持和布局控制三个关键目标方面存在困难，且依赖额外训练限制了可扩展性和效率。需要一种无需训练的方法来解决这些问题。

Method: 提出AnyMS框架，采用自下而上的双级注意力解耦机制：全局解耦分离文本和视觉条件的交叉注意力以确保文本对齐；局部解耦将每个主体的注意力限制在指定区域以防止冲突，保证身份保持和布局控制。使用预训练图像适配器提取主体特定特征，无需主体学习或适配器调优。

Result: 实验表明AnyMS达到最先进性能，支持复杂构图并能扩展到更多主体数量，在文本对齐、主体身份保持和布局控制方面表现优异。

Conclusion: AnyMS是一个高效、可扩展的无需训练多主体定制框架，通过创新的注意力解耦机制有效解决了现有方法的局限性，在多个关键指标上表现出色。
...
### [94] [Task-oriented Learnable Diffusion Timesteps for Universal Few-shot Learning of Dense Tasks](https://arxiv.org/abs/2512.23210)
*Changgyoon Oh,Jongoh Jeong,Jegyeong Cho,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: 提出自适应选择扩散模型时间步特征的方法，通过任务感知时间步选择和特征整合模块，在少样本密集预测任务中提升性能
...
Method: 提出两个模块：任务感知时间步选择(TTS)基于时间步损失和相似度得分选择理想时间步；时间步特征整合(TFC)整合选定时间步特征。配合参数高效微调适配器，在少样本设置下提升密集预测性能

Result: 在Taskonomy数据集上实证验证了可学习时间步整合方法的有效性，特别是在通用和少样本学习场景中，仅用少量支持查询就实现了密集预测性能的优越性

Conclusion: 通过自适应选择扩散模型时间步特征并整合，能够有效提升少样本密集预测任务的性能，解决了传统启发式选择方法的局限性
...
### [95] [RxnBench: A Multimodal Benchmark for Evaluating Large Language Models on Chemical Reaction Understanding from Scientific Literature](https://arxiv.org/abs/2512.23565)
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在化学领域有革命性潜力，但它们在真实科学文献中理解密集、图形化的反应语言的能力尚未得到充分探索。现有模型在处理化学文献中的复杂视觉和文本信息时存在局限性。

Method: 提出RxnBench基准，包含两个任务：1) 单图问答：从305个精选反应方案中提取1,525个问题，测试细粒度视觉感知和机理推理；2) 全文问答：基于108篇文章，要求模型整合文本、反应方案和表格中的跨模态信息。

Result: 评估显示模型存在关键能力差距：虽然擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但所有模型在全文问答任务上的准确率均未超过50%。

Conclusion: 研究结果强调了开发领域特定视觉编码器和更强推理引擎的紧迫性，以推进自主AI化学家的能力发展。多模态大语言模型在化学文献理解方面仍有显著提升空间。
...
TL;DR: MM-UAVBench：首个针对低空无人机场景的多模态大语言模型综合基准，包含感知、认知、规划三个维度19个子任务，基于真实无人机数据构建，揭示了现有模型在低空场景的适应性不足问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在通用领域表现出色，但在低空无人机应用场景的潜力尚未充分探索。现有基准缺乏对低空场景独特挑战的覆盖，而无人机相关评估主要局限于特定任务，缺乏对MLLM通用智能的统一评估。

Method: 提出MM-UAVBench基准，系统评估MLLM在低空无人机场景下的三个核心能力维度：感知、认知和规划。基准包含19个子任务，超过5.7K人工标注问题，所有数据均来自公开数据集的真实无人机采集数据。对16个开源和专有MLLM进行了广泛实验。

Result: 实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析揭示了阻碍MLLM在无人机场景有效部署的关键瓶颈，包括空间偏差和多视角理解等问题。

Conclusion: MM-UAVBench填补了低空无人机场景MLLM评估的空白，揭示了现有模型的局限性，有望推动面向真实世界无人机智能的稳健可靠MLLM研究。
...
### [97] [RS-Prune: Training-Free Data Pruning at High Ratios for Efficient Remote Sensing Diffusion Foundation Models](https://arxiv.org/abs/2512.23239)
...
TL;DR: 提出一种免训练的两阶段数据剪枝方法，通过信息熵筛选和场景感知聚类，在高剪枝率下选择高质量遥感数据子集，提升扩散基础模型的训练效率和生成质量。
...
Result: 即使剪枝85%的训练数据，该方法仍能显著改善收敛性和生成质量。使用该方法训练的扩散基础模型在下游任务（超分辨率和语义图像合成）中持续达到最先进的性能。

Conclusion: 该数据剪枝范式为开发遥感生成基础模型提供了实用指导，通过联合考虑局部信息内容和全局场景级多样性/代表性，能够在高剪枝率下快速选择高质量数据子集，使初步基础模型快速收敛并作为多功能骨干。
...
### [98] [Multimodal Interpretation of Remote Sensing Images: Dynamic Resolution Input Strategy and Multi-scale Vision-Language Alignment Mechanism](https://arxiv.org/abs/2512.23243)
*Siyu Zhang,Ying Chen,Lianlei Shan,Runhe Qiu*

Main category: cs.CV

TL;DR: 提出融合动态分辨率输入策略和多尺度视觉语言对齐机制的VLM框架，提升遥感图像多模态融合的语义理解精度和计算效率
...
Result: 在RS-GPT4V数据集上，该框架在图像描述和跨模态检索任务中显著提升语义理解精度和计算效率，在BLEU-4、CIDEr和R@10等指标上优于传统方法
...
### [99] [ASemConsist: Adaptive Semantic Feature Control for Training-Free Identity-Consistent Generation](https://arxiv.org/abs/2512.23245)
*Shin seong Kim,Minjung Shin,Hyunin Cho,Youngjung Uh*

Main category: cs.CV

TL;DR: ASemconsist是一个通过选择性文本嵌入修改解决文本到图像生成中角色身份一致性问题的新框架，同时保持每张图像的提示对齐


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在生成图像序列时难以在保持角色身份一致性和确保每张图像提示对齐之间取得平衡，存在明显的权衡问题

Method: 提出ASemconsist框架：1) 选择性文本嵌入修改实现显式语义控制；2) 基于FLUX填充嵌入分析，将填充嵌入重新用作语义容器；3) 自适应特征共享策略，自动评估文本模糊性并仅对模糊身份提示应用约束

Result: 该框架实现了最先进的性能，有效克服了先前方法在身份一致性和提示对齐之间的权衡问题

Conclusion: ASemconsist通过创新的语义控制策略解决了文本到图像生成中的角色身份一致性问题，并提出了统一的评估协议CQS来全面评估性能
...
### [100] [Contour Information Aware 2D Gaussian Splatting for Image Representation](https://arxiv.org/abs/2512.23255)
*Masaya Takabe,Hiroshi Watanabe,Sujun Hong,Tomohiro Ikai,Zheming Fan,Ryo Ishimoto,Kakeru Sugimoto,Ruri Imichi*

Main category: cs.CV

TL;DR: 提出了一种轮廓信息感知的2D高斯泼溅框架，通过结合对象分割先验来改进图像表示，在保持快速渲染和低内存使用的同时，显著提升了边界区域的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有的2D高斯泼溅方法在Gaussian数量较少时，由于缺乏轮廓感知能力，往往会产生模糊或不清晰的边界。这限制了在高压缩场景下的图像表示质量。

Method: 提出了轮廓信息感知的2D高斯泼溅框架：1）将对象分割先验整合到高斯表示中；2）在光栅化过程中约束每个Gaussian到特定的分割区域，防止跨边界混合；3）引入预热方案来稳定训练并改善收敛。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在物体边缘区域的重建质量显著优于现有的2DGS方法。在Gaussian数量极少的情况下改进尤为明显，同时仍保持快速渲染和低内存使用。

Conclusion: 通过整合分割先验和区域约束，提出的轮廓感知2D高斯泼溅框架有效解决了边界模糊问题，在保持紧凑表示和实时解码优势的同时，显著提升了边缘结构的保真度。
...
### [101] [Plug-and-Play Fidelity Optimization for Diffusion Transformer Acceleration via Cumulative Error Minimization](https://arxiv.org/abs/2512.23258)
*Tong Shao,Yusen Fu,Guoying Sun,Jingde Kong,Zhuotao Tian,Jingyong Su*

Main category: cs.CV

TL;DR: 提出CEM插件，通过累积误差最小化优化现有误差校正方法，实现扩散变换器加速时的保真度提升
...
Method: 提出CEM插件，通过预定义误差来表征模型对加速的敏感性（受时间步长和缓存间隔共同影响），基于此先验设计动态规划算法进行策略优化，实现累积误差最小化

Result: 在9个生成模型和量化方法上进行的实验表明，CEM显著提升了现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan上甚至超越了原始生成性能

Conclusion: CEM是一种模型无关、泛化能力强的保真度优化插件，可无缝集成到现有误差校正框架和量化模型中，无需额外计算开销，显著提升扩散变换器加速时的生成质量
...
### [102] [Multi-Track Multimodal Learning on iMiGUE: Micro-Gesture and Emotion Recognition](https://arxiv.org/abs/2512.23291)
...
<details>
  <summary>Details</summary>
Motivation: 微手势识别和行为情感预测都是极具挑战性的任务，需要建模细微、细粒度的人类行为，主要利用视频和骨骼姿态数据。现有方法需要更好地捕捉这些任务中的时空模式和跨模态信息。
...
Result: 在iMiGUE数据集上的实验表明，该方法在行为情感预测任务中表现出稳健的性能和准确性，在MiGA 2025挑战赛中获得了第二名。

Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过精心设计的跨模态融合机制捕捉细微的人类行为模式，在相关挑战中取得了优异成绩。
...
### [103] [CME-CAD: Heterogeneous Collaborative Multi-Expert Reinforcement Learning for CAD Code Generation](https://arxiv.org/abs/2512.23333)
*Ke Niu,Haiyang Yu,Zhuofan Chen,Zhengtao Yao,Weitao Jia,Xiaodong Ge,Jingqun Tang,Benlei Cui,Bin Li,Xiangyang Xue*

Main category: cs.CV

TL;DR: 提出CME-CAD异构协作多专家强化学习范式，通过两阶段训练过程生成高精度、可编辑的CAD模型，并发布包含17299个实例的CADExpert开源基准。
...
Method: 提出异构协作多专家强化学习（CME-CAD）范式，整合互补模型优势，通过两阶段训练：多专家微调（MEFT）和多专家强化学习（MERL）。使用CADExpert基准数据集，包含正投影图、精确尺寸标注、专家生成的思维链过程、可执行CADQuery代码和渲染的3D模型。

Result: 开发了能够生成准确、约束兼容且完全可编辑CAD模型的训练范式，并创建了包含17299个实例的开源基准CADExpert，为CAD代码生成研究提供了标准化评估资源。

Conclusion: CME-CAD范式通过异构模型协作学习，有效解决了工业设计中CAD模型生成的精度和可编辑性问题，为自动化CAD设计提供了创新解决方案，同时发布的基准数据集将促进该领域的研究发展。
...
TL;DR: 该论文从拓扑结构角度研究视觉表征学习，提出视觉理解需要语义语言，视觉空间具有纤维丛结构，语义不变性需要非同胚的判别目标，模型架构需支持拓扑变化。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑角度理解视觉表征学习，基于一个核心假设：视觉理解需要语义语言，许多感知观察对应少量离散语义状态。结合表征学习中广泛接受的迁移性和抽象性前提，探索视觉空间的纤维丛结构组织。

Method: 采用理论推导方法，从视觉空间的纤维丛结构假设出发，推导出两个理论结果：1)语义商空间不是子流形，需要非同胚的判别目标；2)近似商空间对模型架构有结构要求，需要支持拓扑变化的"扩展-捕捉"机制。

Result: 理论结果表明：1)语义不变性需要外部语义目标（如监督标签、跨实例识别或多模态对齐）；2)语义抽象需要支持拓扑变化的表征机制，即先几何扩展分离结构，再坍缩形成离散语义区域的"扩展-捕捉"过程。

Conclusion: 该框架提供了拓扑视角，与大规模判别模型和多模态模型的实证规律一致，也与统计学习理论的经典原理相符。这些结果是解释性的而非规定性的，为理解视觉表征学习提供了结构性和拓扑性的理论框架。
...
TL;DR: 提出SpatialMosaic数据集和SpatialMosaicVLM框架，通过多视图图像增强VLM的3D空间推理能力，解决真实场景中的部分可见性、遮挡和低重叠度等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法依赖预构建的3D表示或现成重建管道，限制了可扩展性和真实世界适用性。虽然已有工作探索从多视图图像直接学习空间推理，但真实环境中的部分可见性、遮挡、低重叠度等挑战仍未充分研究。

Method: 1) 提出可扩展的多视图数据生成和标注管道，构建真实空间推理QA对，创建包含200万QA对的SpatialMosaic指令调优数据集；2) 提出包含100万QA对、覆盖6个任务的SpatialMosaic-Bench基准测试；3) 提出SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到VLM中。

Result: 广泛实验表明，提出的数据集和VQA任务能有效增强多视图挑战条件下的空间推理能力，验证了数据生成管道在构建真实多样QA对方面的有效性。

Conclusion: 通过SpatialMosaic数据集、基准测试和VLM框架，为真实世界多视图空间推理提供了可扩展的解决方案，解决了现有方法在部分可见性、遮挡和低重叠度条件下的局限性。
...
TL;DR: NeXT-IMDL是一个大规模诊断基准，旨在系统评估图像篡改检测模型的泛化能力，揭示现有方法在真实世界多样化AI生成内容面前的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 随着用户友好型图像编辑模型的普及和滥用风险增加，迫切需要通用且最新的图像篡改检测与定位方法。当前研究采用跨数据集评估，但这种方法掩盖了现有方法在处理多样化AI生成内容时的脆弱性，导致对进展的误导性印象。

Method: 提出NeXT-IMDL基准，从四个基本维度对AI生成内容篡改进行分类：编辑模型、篡改类型、内容语义和伪造粒度。基于此构建五个严格的跨维度评估协议，对11个代表性模型进行系统评估。
...
TL;DR: SOFTooth是一个语义增强、顺序感知的2D-3D融合框架，利用冻结的2D语义提升3D牙齿实例分割性能，无需显式2D掩码监督。


<details>
  <summary>Details</summary>
Motivation: 3D牙齿实例分割面临拥挤牙弓、模糊的牙-龈边界、缺失牙齿和临床重要的第三磨牙等挑战。传统3D方法存在边界泄漏、中心漂移和牙齿身份不一致问题，而2D基础模型虽提供强边界感知语义但难以直接应用于3D临床工作流。

Method: 提出SOFTooth框架：1）点级残差门控模块将咬合面SAM嵌入注入3D点特征以细化牙-龈和牙间边界；2）中心引导掩码细化正则化实例掩码与几何质心一致性；3）顺序感知匈牙利匹配策略整合解剖牙齿顺序和中心距离，确保连贯标注。

Result: 在3DTeethSeg'22数据集上，SOFTooth实现了最先进的整体准确率和平均IoU，在涉及第三磨牙的病例上表现尤其突出，证明无需2D微调即可有效将丰富2D语义迁移到3D牙齿实例分割。

Conclusion: SOFTooth通过语义增强、顺序感知的2D-3D融合框架，成功解决了3D牙齿实例分割中的边界、中心和身份一致性问题，为临床工作流提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型方法存在局限性：虽然采用统一架构，但仍将世界预测与运动规划作为解耦过程处理，缺乏内在一致性。需要弥合视频生成与运动规划之间的鸿沟。

Method: DriveLaW包含两个核心组件：DriveLaW-Video（生成高保真预测并产生表达性潜在表示的世界模型）和DriveLaW-Act（从DriveLaW-Video潜在表示生成一致可靠轨迹的扩散规划器），采用三阶段渐进训练策略优化。

Result: 在两项任务上均取得新的最先进结果：视频预测方面超越最佳工作33.3%（FID）和1.8%（FVD）；在NAVSIM规划基准测试中创造了新纪录。

Conclusion: DriveLaW通过统一视频生成与运动规划，实现了高保真未来生成与可靠轨迹规划的内在一致性，为自动驾驶世界模型提供了新的范式。
...
TL;DR: DDSPO是一种新的扩散模型偏好优化方法，通过直接从获胜和失败策略中获取逐时间步监督，无需依赖昂贵的人工标注数据，提升了文本-图像对齐和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本到图像合成中难以完全对齐用户意图和保持一致的审美质量。现有的基于偏好的训练方法（如Diffusion DPO）依赖昂贵且可能有噪声的人工标注数据集，需要更高效的无监督解决方案。

Method: DDSPO直接从获胜和失败策略中推导出逐时间步的监督信号，在去噪轨迹上提供密集的过渡级信号。通过使用预训练参考模型自动生成偏好信号：对比原始提示与语义降级变体条件下的输出，无需显式奖励建模或人工标注。
...
<details>
  <summary>Details</summary>
Motivation: 虽然SAM等基础分割模型在零样本任务中表现良好，但在领域偏移或知识受限的情况下仍然脆弱。研究旨在探索不确定性量化是否能以领域无关的方式缓解这些挑战并增强模型泛化能力。

Method: 1) 创建UncertSAM基准，包含8个设计用于在阴影、透明度和伪装等挑战性分割条件下压力测试SAM的数据集；2) 评估一系列轻量级、后处理的不确定性估计方法；3) 评估初步的不确定性引导的预测细化步骤。

Result: 在评估的方法中，最后一层拉普拉斯近似产生的不确定性估计与分割误差相关性良好，表明存在有意义的信号。细化步骤的益处尚属初步，但结果强调了将不确定性纳入分割模型以支持鲁棒、领域无关性能的潜力。

Conclusion: 不确定性量化有潜力增强分割模型在挑战性条件下的鲁棒性和泛化能力。研究提供了公开的基准和代码，为后续研究奠定了基础。
...
TL;DR: 提出结合视觉水线检测、YOLOv8姿态尺度提取和多模态大语言模型的混合框架，用于自动读取河流水位标尺，实现高精度水位监测。
...
Method: 采用混合框架：图像预处理→标注→水线检测→尺度间隙估计→数值读取提取。整合视觉水线检测、YOLOv8姿态尺度提取和GPT-4o/Gemini 2.0 Flash多模态大语言模型。

Result: 水线检测精度94.24%，F1分数83.64%；尺度间隙检测为后续读取提供准确几何校准；加入尺度间隙元数据显著提升LLM性能，Gemini Stage 2在最佳图像条件下MAE 5.43cm，RMSE 8.58cm，R² 0.84。

Conclusion: 该方法为自动化水文监测提供可扩展、高效可靠的解决方案，展示了实时河流标尺数字化和改善水资源管理的潜力，强调结合几何元数据与多模态AI对鲁棒水位估计的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大视频语言模型在处理长视频时存在两个主要问题：1）时间窗口狭窄，难以捕捉长时间跨度的语义变化；2）主流基于文本的检索流程主要依赖表层词汇重叠，忽略了视觉、音频和字幕通道之间的丰富时间相互依赖性。

Method: 提出TV-RAG框架，包含两个核心机制：1）时间衰减检索模块，在相似度计算中注入显式时间偏移，使文本查询能根据真实的多媒体上下文进行排序；2）熵加权关键帧采样器，选择均匀分布且信息密集的帧，减少冗余同时保持代表性。

Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中持续超越大多数领先基线，验证了模型的有效性。该框架无需重新训练或微调即可集成到任何大视频语言模型中。

Conclusion: TV-RAG通过结合时间和语义信号实现双层推理，为长视频理解提供了一种轻量级、经济高效的升级路径，解决了现有大视频语言模型在长视频处理中的关键限制。
...
TL;DR: IdentityStory是一个用于以人为中心的故事生成框架，通过迭代身份发现和重去噪身份注入技术，在多张序列图像中保持角色身份一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉生成模型虽然能够从文本生成具有一致角色的故事，但在以人为中心的故事生成中面临额外挑战：需要保持详细且多样化的人脸一致性，以及协调不同图像中的多个角色。
...
Conclusion: IdentityStory通过身份保持生成器的驯服，有效解决了以人为中心的故事生成中的角色身份一致性问题，为相关应用提供了可靠的技术基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像超分辨率中难以同时保证高频感知质量和低频结构保真度。虽然推理时间缩放理论上可以改善这一权衡，但现有策略仍不理想：奖励驱动的粒子优化常导致感知过度平滑，而最优路径搜索则易失去结构一致性。

Method: 提出IAFS框架，结合迭代细化和频率感知粒子融合。通过迭代校正结构偏差逐步细化生成图像，同时自适应地融合高频感知线索与低频结构信息，实现跨不同图像细节的更准确平衡重建。

Result: 在多个基于扩散的超分辨率模型上的广泛实验表明，IAFS有效解决了感知-保真度冲突，持续改善了感知细节和结构准确性，优于现有推理时间缩放方法。

Conclusion: IAFS是一个无需训练的训练框架，通过迭代扩散推理时间缩放和自适应频率引导，成功平衡了图像超分辨率中的感知质量与结构保真度，为扩散模型在超分辨率任务中的应用提供了更优的解决方案。
...
TL;DR: PurifyGen是一种无需训练的双阶段提示净化方法，通过计算互补语义距离评估令牌安全性，并对风险提示进行双空间变换，在保持模型权重不变的情况下实现安全文本到图像生成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在提升文本到图像生成质量的同时也增加了生成不安全内容的风险。传统安全方法（如文本黑名单或有害内容分类）存在明显缺陷：容易被规避，或需要大量数据集和额外训练。需要一种无需训练、保持模型原始权重的安全生成方法。

Method: 提出PurifyGen双阶段策略：1）通过计算互补语义距离评估提示中每个令牌的安全性，测量提示令牌与预定义有毒和干净列表概念嵌入之间的语义接近度，实现细粒度提示分类；2）对风险提示应用双空间变换：将有毒性对齐的嵌入投影到有毒概念矩阵的零空间中，有效去除有害语义成分，同时将其对齐到干净概念的范围空间中，通过减去不安全语义和增强安全语义来净化风险提示，同时保留原始意图和连贯性；3）定义令牌级策略，仅选择性替换风险令牌嵌入，最小化对安全内容的干扰。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面超越了当前方法，并与训练依赖方法竞争良好。该方法提供即插即用解决方案，具有理论依据，对未见提示和模型具有强泛化能力。

Conclusion: PurifyGen是一种新颖、无需训练的安全文本到图像生成方法，通过双阶段提示净化策略有效减少不安全内容生成，同时保持模型原始权重和生成质量，为安全生成提供了理论可靠且实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型的思维链推理在理解任务中表现良好，但在生成任务中扩展有限，受限于特定场景机制，缺乏泛化性和适应性

Method: 提出ThinkGen框架：1) 解耦架构：预训练MLLM生成定制化指令，DiT根据指令生成高质量图像；2) SepGRPO训练范式：在MLLM和DiT模块间交替强化学习，支持跨数据集联合训练

Result: 在多个生成基准测试中实现了稳健、最先进的性能，证明了框架在广泛生成场景中的有效性

Conclusion: ThinkGen首次将MLLM的思维链推理明确应用于视觉生成任务，通过灵活的架构设计和训练范式，为多场景生成任务提供了系统化解决方案
...
TL;DR: ProGuard：一种视觉语言主动防护系统，通过强化学习训练，无需模型调整即可识别和描述分布外安全风险，在安全分类和风险描述方面优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致多模态安全风险不断涌现，现有防御方法存在局限性。传统反应式方法需要模型调整，无法有效应对新兴风险，需要一种无需调整模型的主动防护方案。

Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注二元安全标签和风险类别；2) 通过纯强化学习训练视觉语言基础模型实现高效推理；3) 引入OOD安全类别推断任务，在RL目标中增加基于同义词库的相似性奖励，鼓励模型为未见不安全类别生成简洁描述。

Result: ProGuard在二元安全分类上达到与闭源大模型相当的性能，在不安全内容分类上显著优于现有开源防护模型。在主动防护能力方面，OOD风险检测提升52.6%，OOD风险描述提升64.8%。

Conclusion: ProGuard通过模态平衡数据集和强化学习训练，实现了无需模型调整的主动安全防护，在多模态安全风险识别和描述方面表现出色，为应对新兴安全风险提供了有效解决方案。
...
TL;DR: 提出LiveTalk系统，通过改进蒸馏方法实现实时多模态交互视频生成，将响应延迟从1-2分钟降低到实时，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过双向注意力的迭代去噪过程无法实现实时交互，而现有蒸馏方法主要关注文本到视频生成，在多模态条件下存在视觉伪影和质量下降问题，限制了人机交互的自然性和效率。

Method: 提出改进的蒸馏方法，重点关注条件输入质量以及在线策略优化的初始化和调度策略。将模型与音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks集成，构建LiveTalk实时多模态交互化身系统。

Result: 在HDTF、AVSpeech和CelebV-HQ等多模态条件化身视频生成基准测试中，蒸馏模型以20倍更低的推理成本和延迟匹配了全步双向基线的视觉质量。在自建的多轮交互基准测试中，LiveTalk在多轮视频连贯性和内容质量上优于Sora2、Veo3等最先进模型。

Conclusion: 通过改进的蒸馏方法实现了实时多模态交互视频生成，显著降低了响应延迟，使无缝人机多模态交互成为可能，为构建通用多模态交互AI系统提供了关键技术。
...
TL;DR: TWIN是一个包含56.1万图像对的大规模数据集，旨在提升视觉语言模型在细粒度视觉感知方面的能力，通过判断两幅相似图像是否描绘同一物体来训练模型关注细微视觉线索。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在广泛视觉理解方面表现出色，但在细粒度感知方面存在不足，容易受到视觉偏见影响并忽略细微视觉细节。现有训练语料库强调一般性识别而非细粒度感知，强化了这一局限性。

Method: 1. 构建TWIN数据集：包含56.1万对图像查询，要求模型判断两幅视觉上相似的图像是否描绘同一物体，涵盖日常物体在不同背景、视角和外观下的多样性。2. 引入FGVQA基准测试套件：包含1.2万个查询，重新利用多个领域的细粒度识别和检索数据集。3. 在TWIN上微调视觉语言模型，评估其在FGVQA和其他基准测试上的表现。

Result: 在TWIN上微调的视觉语言模型在FGVQA基准测试上提升了高达19.3%，在未见领域（如艺术、动物、植物和地标）也表现出显著的细粒度识别能力提升，同时不影响在一般VQA基准测试上的性能。数据集规模与物体标注数量呈良好扩展关系，分析表明规模是性能提升的关键。

Conclusion: TWIN数据集有效提升了视觉语言模型的细粒度感知能力，可作为开源VLM训练语料库的即插即用补充，推动未来模型在感知精度方面的进步。数据集规模对性能有重要影响。
...
TL;DR: 该论文提出三种改进红外夜视相机火灾检测精度的方法：额外NIR数据集、两阶段检测模型和Patched-YOLO，旨在解决数据稀缺、夜间误报和远距离小目标检测问题。


<details>
  <summary>Details</summary>
Motivation: 现有火灾检测模型在红外夜视相机应用中面临数据稀缺、夜间误报（特别是将明亮人造光源误判为火灾）以及远距离小目标检测精度不足的问题，需要改进检测准确性和鲁棒性。

Method: 1) 构建额外NIR数据集并应用多种数据增强策略；2) 提出两阶段检测管道，结合YOLOv11进行初步检测和EfficientNetV2-B0进行误报过滤；3) 提出Patched-YOLO，通过基于补丁的处理增强RGB图像中远距离小目标的检测能力。

Result: 相比先前方法（YOLOv7 mAP50-95 0.51，RT-DETR 0.65，YOLOv9 0.598），提出的两阶段方法在夜间火灾检测中达到更高精度，显著减少了人造光源导致的误报。Patched-YOLO提升了RGB图像中远距离小目标的检测能力。

Conclusion: 通过数据增强、两阶段检测架构和补丁处理策略，有效提升了红外夜视相机火灾检测的准确性和鲁棒性，特别是在夜间环境和远距离小目标检测方面取得显著改进。
...
TL;DR: 提出SRFA框架用于胰腺肿瘤早期检测，通过多阶段处理提升CT图像中微小肿瘤的可见性和检测精度，达到96.23%准确率。
...
Method: 提出可扩展残差特征聚合(SRFA)框架：1)预处理和MAGRes-UNet分割增强胰腺结构；2)DenseNet-121进行残差特征存储提取深层特征；3)混合HHO-BA元启发式特征选择优化特征子集；4)结合Vision Transformer和EfficientNet-B3的混合模型分类；5)SSA和GWO双优化机制微调超参数。

Result: 实验结果显示显著性能提升：准确率96.23%，F1分数95.58%，特异性94.83%，显著优于传统CNN和当代基于Transformer的模型。

Conclusion: SRFA框架作为胰腺肿瘤早期检测的有用工具具有潜力，其多阶段处理策略有效解决了CT图像中胰腺肿瘤检测的复杂挑战。
...
TL;DR: 本文提出了一个评估框架来量化3D生成模型中的记忆化现象，并通过实验分析了数据和建模设计对记忆化的影响，提出了减少记忆化的有效策略。


<details>
  <summary>Details</summary>
Motivation: 生成模型在3D视觉中广泛用于合成新形状，但尚不清楚其生成是否依赖于记忆训练形状。理解记忆化现象有助于防止训练数据泄露并提高生成结果的多样性。

Method: 设计了一个评估框架来量化3D生成模型中的记忆化，首先应用于现有方法，然后通过潜在向量集扩散模型进行控制实验，分析数据和建模因素对记忆化的影响。
...
TL;DR: HAT是一个时空对齐模块，通过多假设解码为自动驾驶中的端到端感知提供自适应对齐方案，显著提升3D检测和跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖注意力机制进行跨帧对象对齐，使用统一的显式物理模型（如恒定速度），偏好语义特征进行隐式对齐。然而，不同类别和帧之间运动状态和对象特征的差异使这种对齐效果不佳。

Method: HAT首先利用多个显式运动模型为历史实例生成空间锚点和运动感知特征提案，然后通过结合缓存对象查询中嵌入的语义和运动线索进行多假设解码，最终为目标帧提供最优对齐提案。

Result: 在nuScenes数据集上，HAT持续改进各种基线的3D时序检测器和跟踪器。与DETR3D检测器配对时，在测试集上达到46.0% AMOTA的SOTA跟踪结果。在端到端AD方法中，提升感知精度（+1.3% mAP，+3.1% AMOTA）并降低32%碰撞率。在语义受损场景（nuScenes-C）中，HAT的运动建模增强使端到端AD的感知和规划更加鲁棒。
...
Method: 提出Intrinsic Decomposition Transformer (IDT)，基于Transformer注意力机制联合推理多个输入图像，采用物理基础的图像形成模型，将图像显式分解为漫反射率、漫反射阴影和镜面反射阴影，分离Lambertian和非Lambertian光传输。
...
TL;DR: TransPhy3D：利用视频扩散模型学习透明物体深度估计，通过合成透明场景数据集和轻量级LoRA适配器，实现零样本SOTA的时空一致性深度预测


<details>
  <summary>Details</summary>
Motivation: 透明物体对传统感知系统构成挑战，折射、反射和透射破坏了立体视觉、ToF和单目深度估计的假设，导致空洞和时间不稳定估计。关键观察是：现代视频扩散模型已经能够合成逼真的透明现象，表明它们内化了光学规则。

Method: 1. 构建TransPhy3D合成视频语料库：11k序列，使用Blender/Cycles渲染，包含静态资产和程序化资产，搭配玻璃/塑料/金属材质，渲染RGB+深度+法线；2. 从大型视频扩散模型出发，通过轻量级LoRA适配器学习视频到视频的深度（和法线）转换器；3. 训练时在DiT骨干中拼接RGB和（带噪声的）深度潜在表示，在TransPhy3D和现有帧级合成数据集上共同训练，实现对任意长度输入视频的时间一致性预测。

Result: DKT模型在涉及透明度的真实和合成视频基准上实现零样本SOTA：ClearPose、DREDS（CatKnown/CatNovel）和TransPhy3D-Test。相比强图像/视频基线，提高了准确性和时间一致性。法线变体在ClearPose上获得最佳视频法线估计结果。紧凑的1.3B版本运行速度约0.17秒/帧。集成到抓取系统中，DKT的深度估计提高了在透明、反射和漫反射表面上的成功率，优于先前估计器。

Conclusion: 结果支持更广泛的主张："扩散模型理解透明度"。生成式视频先验可以高效、无标签地重新用于实现鲁棒、时间一致性的感知，以应对具有挑战性的真实世界操作任务。
...
TL;DR: Stream-DiffVSR：首个适用于低延迟在线部署的因果条件扩散视频超分辨率框架，通过四步蒸馏去噪器、自回归时间引导和轻量级时间感知解码器，在RTX4090上实现720p帧0.328秒处理，相比SOTA方法提升感知质量同时降低130倍延迟。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频超分辨率方法虽然感知质量强，但依赖未来帧和昂贵的多步去噪，不适用于延迟敏感场景。需要开发能够在线实时处理、仅使用过去帧的因果扩散框架。

Method: 提出Stream-DiffVSR因果条件扩散框架：1）四步蒸馏去噪器实现快速推理；2）自回归时间引导模块在潜在去噪过程中注入运动对齐线索；3）轻量级时间感知解码器配备时间处理器模块增强细节和时间一致性。

Result: 在RTX4090上处理720p帧仅需0.328秒，显著优于先前扩散方法。相比在线SOTA TMP方法，提升感知质量（LPIPS +0.095）同时降低130倍以上延迟。将初始延迟从4600秒降至0.328秒，成为首个适合低延迟在线部署的扩散VSR方法。

Conclusion: Stream-DiffVSR通过因果条件扩散框架解决了扩散VSR的延迟瓶颈，实现了高效在线视频超分辨率，在保持高质量的同时大幅降低处理延迟，为扩散模型在实时应用中的部署开辟了新途径。
...
TL;DR: Moxin 7B是一个完全开源的LLM，遵循模型开放框架，提供完整的训练透明度，并开发了三个变体以增强视觉-语言、视觉-语言-动作和中文能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM发展呈现两极分化：专有模型（如GPT-4）性能卓越但封闭，开源模型（如LLaMA）易于定制但透明度不足。Moxin旨在通过完全透明的开源框架，促进更包容、协作的研究环境，维持健康的开源生态系统。

Method: 基于模型开放框架开发Moxin 7B，不仅共享模型权重，还提供完整的训练透明度（训练过程、数据集、实现细节）。开发三个变体：Moxin-VLM（视觉-语言）、Moxin-VLA（视觉-语言-动作）、Moxin-Chinese（中文能力）。采用开源框架和开放数据进行训练。

Result: 实验表明，Moxin模型在各种评估中实现了卓越性能。作者发布了模型、可用数据以及推导这些模型的代码，实现了完全的开源承诺。

Conclusion: Moxin 7B作为一个完全透明的开源LLM，通过提供完整的训练细节和开发多能力变体，为促进包容性研究和健康开源生态系统做出了贡献，展示了开源模型在保持透明度的同时实现高性能的可行性。
...
TL;DR: 论文研究表明，transformer语言模型的句子嵌入空间编码了与人类可解释的认知属性对齐的分层几何结构，这种结构超越了表面词汇统计特征。


<details>
  <summary>Details</summary>
Motivation: 尽管已有研究表明transformer语言模型在其嵌入空间中学习到丰富的几何结构，但其中是否存在与高级认知组织对齐的分层结构仍未得到充分探索。本研究旨在探究句子嵌入是否编码了与人类可解释的认知或心理属性对齐的渐进式分层结构。

Method: 构建包含480个自然语言句子的数据集，标注了连续序数能量分数和离散层级标签，涵盖七个有序认知类别。使用多个transformer模型的固定句子嵌入，通过线性和浅层非线性探针评估这些标注的可恢复性。使用词汇TF-IDF基线进行比较，并通过非参数置换检验确认性能是否超过随机水平。使用UMAP可视化和混淆矩阵进行定性分析。

Result: 在所有模型中，连续分数和层级标签都能可靠地被解码，浅层非线性探针相比线性探针提供了一致的性能提升。词汇TF-IDF基线表现明显更差，表明观察到的结构不能仅归因于表面词汇统计。置换检验确认探针性能显著超过随机水平。定性分析显示嵌入空间中存在从低到高的平滑梯度，并且混淆主要发生在相邻层级之间。

Conclusion: transformer嵌入空间展现出与人类定义的认知属性对齐的分层几何组织，同时该结论不涉及对内部意识或现象学的主张。这表明语言模型不仅学习表面模式，还编码了与人类认知结构对齐的抽象层次表示。
...
TL;DR: 对也门伊比阿拉伯语中qulk-从句的句法研究，提出qulk-从句是双从句结构，qulk作为从句嵌入谓词选择完整CP补语，采用最简方案核心操作进行分析。


<details>
  <summary>Details</summary>
Motivation: 研究也门伊比阿拉伯语中qulk-从句的句法特性，这种形态融合形式意为"我说"，引入嵌入的陈述、疑问和祈使从句，通常不带标句词。旨在在最简方案框架下分析这种特殊结构。

Method: 在最简方案框架下，应用核心操作（合并、移动、一致、拼读）对qulk-从句进行分层句法分析，展示其通过标准计算步骤和后句法过程（如形态合并）的推导过程。

Result: 提出qulk-从句是双从句结构，qulk作为从句嵌入谓词选择完整CP补语。分析解释了方言特定特征如二分否定、附着化和CP嵌入。为生成句法理论（特别是最简方案）提供理论贡献。

Conclusion: 研究结论提出了将分析扩展到受话者从句kil-k'你说'的理论问题。为最简方案的普遍性可能性提供了见解，表明该框架能有效分析阿拉伯语方言的特殊句法现象。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的人类活动识别方法依赖大量标注数据和纯几何范例选择，难以区分相似活动（如行走、上楼梯、下楼梯），需要结合语义信息来改进范例选择

Method: 提出LLM引导的范例选择框架，利用LLM生成的知识先验（特征重要性、类间混淆性、范例预算乘数）指导范例评分和选择，结合基于边界的验证线索、PageRank中心性、枢纽惩罚和设施选址优化

Result: 在UCI-HAR数据集上，在严格少样本条件下实现了88.78%的宏观F1分数，优于随机采样、聚类和k-center等传统方法
...
TL;DR: 论文提出HHEM轻量级幻觉检测框架，显著降低计算成本（8小时→10分钟），在QA任务上达到82.2%准确率，但总结任务中局部幻觉检测不足，通过分段检索改进。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型产生的幻觉内容会生成误导性或不可验证的信息，损害信任和可靠性。现有评估方法（如KnowHalu）采用多阶段验证但计算成本高昂，需要更高效的检测方案。

Method: 集成Hughes幻觉评估模型（HHEM），这是一个轻量级的基于分类的框架，独立于LLM判断。引入分段检索机制处理总结任务中的局部幻觉问题，并进行累积分布函数分析比较不同规模模型的幻觉表现。

Result: HHEM将评估时间从8小时减少到10分钟，在QA任务上达到82.2%准确率和78.9%真阳性率。分段检索改善了总结任务中的局部幻觉检测。CDF分析显示较大模型（7B-9B参数）幻觉较少，中等规模模型稳定性较差。

Conclusion: 需要平衡计算效率和事实验证的结构化评估框架。HHEM提供高效幻觉检测，但需改进局部幻觉处理。模型规模与幻觉频率相关，较大模型表现更稳定。
...
Method: 采用分层过滤的多阶段管道：1) 使用成本效益高的Gemini 2.5 Flash进行查询重构、分层内容过滤和引用归因；2) 保留Gemini 2.5 Pro的推理能力用于最终答案生成；3) 超越传统的基于嵌入的检索方法。

Result: 在MMU-RAGent验证集上：ROUGE-L提升至0.274（+19.6%），DeBERTaScore提升至0.677（+6.2%）。在Test2025自定义数据集（评估2025年1月后知识的问题）上：ROUGE-L超越参数化基线57.4%，DeBERTaScore超越14.9%。

Conclusion: HiFi-RAG通过分层过滤策略有效解决了开放域RAG中的无关信息问题，在成本效益和性能之间取得了良好平衡，在需要后截止日期知识的任务上表现尤为突出。
...
TL;DR: 该研究评估了大语言模型在会计领域的垂直推理能力，建立了评估标准，并测试了多个模型在会计推理任务上的表现，发现GPT-4表现最佳但仍需优化才能满足实际应用需求。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各领域的广泛应用，如何将其有效整合到会计专业领域并重新定义LLMs与领域特定应用的关系，成为推动企业数字化转型和社会发展的关键挑战。需要理解LLMs在会计领域的特定推理能力，以促进其在该领域的有效整合。

Method: 研究引入了垂直领域会计推理的概念，通过分析代表性GLM系列模型的训练数据特征建立评估标准。基于此框架，评估了GLM-6B、GLM-130B、GLM-4和OpenAI GPT-4等多个模型在一组会计推理任务上的表现，并测试了不同提示工程策略的效果。

Result: 实验结果显示，不同的提示工程策略在不同模型上带来不同程度的性能提升，其中GPT-4展现出最强的会计推理能力。然而，当前的大语言模型仍无法满足实际应用需求，特别是在企业级会计场景中需要进一步优化。

Conclusion: 研究为后续推理范式研究提供了基础，并为改进会计推理性能提供了基准。虽然GPT-4在会计推理任务上表现最佳，但当前LLMs仍需进一步优化才能在企业级会计场景中充分实现其潜在价值。
...
TL;DR: 该论文主张韩语选区树库应采用"어절"（eojeol）作为终端单位，而非语素，以解决形态学与句法结构混淆的问题，并提出基于어절的标注方案。


<details>
  <summary>Details</summary>
Motivation: 韩语选区树库设计面临终端单位选择的基本问题。虽然韩语词具有形态复杂性，但将语素作为选区终端会混淆词内形态与短语级句法结构，并导致与基于어절的依存资源不匹配。

Method: 提出基于어절的选区表示方法，将形态分割和细粒度词性信息编码在独立的非选区层中。通过比较分析显示，在明确的规范化假设下，世宗和宾夕法尼亚韩语树库在어절选区层面具有表征等价性。

Result: 分析表明基于어절的选区表示能够保持可解释的选区结构，支持跨树库比较和选区-依存转换。提出了一个保留可解释选区并支持这些功能的어절标注方案。

Conclusion: 韩语选区树库应采用어절作为终端单位，将形态信息分离到独立层中。这种表示方法解决了现有问题，实现了不同树库间的表征等价，并为跨树库比较和格式转换提供了基础。
...
TL;DR: 提出All-or-Here Attention (AHA)机制，通过二元路由器动态切换注意力模式：要么全注意力，要么局部滑动窗口注意力，可减少93%的全注意力计算而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 受人类阅读行为的启发——通常只关注当前页，必要时才回顾前文——研究大型语言模型是否也能动态决定何时需要全局上下文，从而减少冗余计算。

Method: 提出AHA机制，在每个注意力头使用二元路由器为每个token动态选择：1）全注意力（访问全部上下文），或2）局部滑动窗口注意力（仅访问窗口内token）。通过评估不同窗口大小分析上下文依赖模式。

Result: 使用256个token的窗口时，最多可替代93%的全注意力操作而不损失性能。分析发现上下文依赖呈长尾分布：随着局部窗口扩大，对全注意力的需求迅速衰减。

Conclusion: 全注意力机制在很大程度上是冗余的，高效推理只需按需访问全局上下文。AHA通过解耦局部处理和全局访问，揭示了这一特性，为优化LLM计算效率提供了新思路。
...
TL;DR: 该论文提出了一种基于LLM的结构化提示管道和集成方法，用于多模态对话中的方面情感分析和情感翻转检测任务，在MCABSA挑战中取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 多模态对话中的情感理解是构建情感智能AI系统的关键挑战。MCABSA挑战提出了两个复杂子任务：从多说话者对话中提取完整情感六元组（持有者、目标、方面、意见、情感、理由），以及检测动态情感翻转及其触发因素。

Method: 针对子任务一，设计了结构化提示管道，引导LLM按顺序提取情感组件并优化上下文理解。针对子任务二，通过集成三种LLM的优势，鲁棒地识别情感转换及其触发因素。

Result: 系统在子任务一上获得了47.38%的平均分数，在子任务二上获得了74.12%的精确匹配F1分数，证明了逐步细化和集成策略在多模态情感分析任务中的有效性。

Conclusion: 结构化提示管道和LLM集成方法能够有效处理复杂的多模态情感分析任务，特别是在提取细粒度情感信息和检测动态情感变化方面表现出色。
...
TL;DR: T3LLM：基于三LLM框架的时间序列问答系统，通过工作者-评审者-学生协作机制实现多步推理与显式纠错


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在处理复杂数值序列时容易产生推理错误，而时间序列数据具有可验证性，可在推理步骤与原始输入之间进行一致性检查
...
Conclusion: T3LLM通过显式纠错机制和多LLM协作框架，有效解决了时间序列问答中的推理错误问题，提升了复杂数值序列处理的准确性和可靠性
...
TL;DR: M2G-Eval是一个多粒度、多语言的代码生成评估框架，涵盖4个粒度级别和18种编程语言，包含17K+训练任务和1,286个测试实例。研究发现代码生成存在难度层次结构，并揭示了跨语言的可迁移编程概念。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成评估基准主要关注单一结构粒度和有限编程语言，无法揭示不同代码范围和跨语言场景下的细粒度能力差异。需要更全面的评估框架来系统分析LLMs的代码生成能力。
...
Result: 评估30个模型（28个SOTA LLMs + 2个M2G-Eval-Coder变体）发现：1）明显的难度层次结构（Line级最简单，Class级最难）；2）随着任务复杂度增加，完整语言和部分语言之间的性能差距扩大；3）强跨语言相关性，表明模型学习了可迁移的编程概念。

Conclusion: M2G-Eval能够对代码生成能力进行细粒度诊断，并突显了在合成复杂、长形式代码方面存在的持续挑战。该框架为系统评估LLMs的代码生成能力提供了全面工具。
...
TL;DR: 该论文分析了扩散模型在文本生成中的应用，指出离散和结构化文本特性对扩散原理的直接应用构成挑战，并识别了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言生成中具有并行解码和迭代优化等优势，但文本的离散性和高度结构化特性阻碍了扩散原理的直接应用。需要重新审视扩散语言建模，识别扩散机制与语言特定需求之间的差异。

Method: 论文从扩散过程和语言建模的角度重新审视扩散语言建模，提出了区分扩散机制与语言需求的五个关键属性。将现有方法分为嵌入空间的连续扩散和标记上的离散扩散两类，分析各自满足的属性。通过分析近期大型扩散语言模型，识别了两个核心问题。

Result: 分析表明，现有扩散语言建模方法都存在结构性权衡：连续扩散在嵌入空间进行但面临文本离散性挑战；离散扩散在标记层面但难以捕捉多标记依赖关系。识别了两个关键问题：(1)均匀破坏不尊重信息在位置上的分布；(2)标记级边缘训练无法在并行解码时捕捉多标记依赖。

Conclusion: 需要开发更符合文本结构的扩散过程，鼓励未来研究构建更连贯的扩散语言模型。扩散过程应与文本结构更紧密对齐，以解决现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 思维链推理虽然能提升大语言模型的问题解决能力，但现有研究表明思维链解释往往不能反映模型的真实推理过程，模型可能生成连贯但误导性的解释，或在未承认外部线索的情况下修改答案。这种不一致性削弱了基于思维链方法在安全监督和对齐监控中的可靠性。

Method: 评估两种优化方法——组相对策略优化（GRPO）和直接偏好优化（DPO）——在提升思维链忠实度方面的能力。实验比较了这两种方法在不同规模模型上的表现。

Result: GRPO在大型模型中比DPO表现更好，Qwen2.5-14B-Instruct模型在所有评估指标上取得最佳结果。两种方法都显示模型规模与性能之间存在正相关关系，但GRPO在提升忠实度指标方面显示出更大潜力，尽管在较小规模时行为不太稳定。

Conclusion: GRPO为开发更透明和可信赖的大语言模型推理提供了一个有前景的方向，有助于解决思维链解释与真实推理过程之间的不一致问题。
...
TL;DR: 论文提出词汇感知的符合预测（VACP）框架，通过语义掩码和温度调整评分来减少预测集大小，在保持边际覆盖的同时显著提高效率。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域部署大语言模型需要严格的置信度量化，但标准的softmax概率通常校准不佳。传统的符合预测方法虽然能保证有效覆盖，但产生的预测集包含数百个token，缺乏信息价值。

Method: 提出词汇感知的符合预测（VACP）框架：1）使用语义掩码减少有效预测空间；2）采用温度调整评分函数；3）在保持边际覆盖保证的同时，显著降低预测集大小。

Result: 在Gemma-2B模型上使用SQUAD和WikiText基准测试，VACP实现了89.7%的经验覆盖率（目标90%），同时将平均预测集大小从847个token减少到4.3个token，效率提升197倍。

Conclusion: VACP框架解决了大词汇量语言模型中符合预测的覆盖-效率权衡问题，通过词汇空间缩减实现了既保证统计覆盖又具有实际信息价值的预测集，为高风险应用中的不确定性量化提供了实用解决方案。
...
TL;DR: 该论文提出了一个多语言希望言论检测框架，重点关注乌尔都语，使用预训练Transformer模型在低资源语言中实现高性能检测。
...
Method: 使用预训练Transformer模型（XLM-RoBERTa、mBERT、EuroBERT、UrduBERT），应用简单预处理并训练分类器。在PolyHope-M 2025基准上进行评估，重点关注乌尔都语的希望言论检测。
...
TL;DR: 研究发现大型语言模型在多语言推理中存在盲点：虽然任务准确率高，但推理过程与结论之间常存在逻辑脱节，非拉丁文字语言的推理-结论错位率是拉丁文字语言的两倍以上。


<details>
  <summary>Details</summary>
Motivation: 尽管链式思维提示展现了强大的推理能力，但现有研究对跨语言推理质量转移的关注不足。需要评估模型生成的推理轨迹是否在逻辑上支持其结论，特别是在多语言环境下。

Method: 引入人工验证框架评估推理轨迹与结论的逻辑一致性；分析6种语言、6个前沿模型在GlobalMMLU问题上的65k条推理轨迹；通过人工标注建立错误分类法来表征推理失败类型。

Result: 发现关键盲点：模型任务准确率高但推理常不支持结论；非拉丁文字语言的推理-结论错位率至少是拉丁文字语言的两倍；主要错误类型为证据错误（无支持的主张、模糊事实）和不合逻辑的推理步骤。

Conclusion: 当前多语言评估实践对模型推理能力的描述不完整，需要开发推理感知的评估框架来更全面地评估跨语言推理质量。
...
TL;DR: 研究发现通过心理学的提示词调整可以有效减轻LLM在模拟人口调查时的社会期望偏差，其中重新表述的提示词效果最佳。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型被广泛用于模拟人口响应（硅采样），但面对社会敏感问题时存在社会期望偏差，导致与真实人类数据偏离。现有研究对此关注不足，需要探索有效的偏差缓解方法。

Method: 使用美国国家选举研究数据，在两个模型家族（Llama-3.1系列和GPT-4.1-mini）上测试四种基于提示的缓解方法：重新表述（中性第三人称）、反向编码（语义反转）、以及两种元指令（启动和前导），分别鼓励分析和真诚。使用Jensen-Shannon散度与自举置信区间评估与ANES的对齐度。

Result: 重新表述的提示词最有效地改善了与ANES的对齐，减少了社会可接受答案的分布集中度。反向编码在不同项目上效果不一，而启动和前导方法鼓励了响应一致性，但对偏差缓解没有系统性益处。验证了基线研究中社会期望偏差的持续性。

Conclusion: 基于提示的框架控制能有效减轻LLM固有的社会期望偏差，为获得更具代表性的人口模拟样本提供了实用路径。重新表述提示词是最有效的缓解策略。
...
TL;DR: 利用社交媒体数据（特别是Twitter）构建NLP管道，自动识别并分类女性分享的妊娠经历，为研究不良妊娠结局提供补充数据源


<details>
  <summary>Details</summary>
Motivation: 美国婴儿死亡率仍然是一个重要的公共卫生问题，出生缺陷是主要原因之一。尽管已有研究努力理解不良妊娠结局（如流产、死产、出生缺陷和早产）的原因，但仍需要更全面的研究和干预策略。当前数据集有限，需要补充数据源来加强观察性研究。

Method: 提出一种新颖方法，利用公开可用的社交媒体数据（特别是Twitter）来增强现有数据集。构建自然语言处理（NLP）管道，自动识别分享妊娠经历的女性，并根据报告的结果进行分类：报告足月和正常出生体重的为阳性病例，报告不良妊娠结局的为阴性病例。采用鲁棒的预处理技术和数据增强策略来处理社交媒体数据的不平衡、噪声和缺乏结构等固有挑战。

Result: 该方法提供了识别和分类妊娠经历的技术框架，能够自动从社交媒体中提取相关数据。研究展示了社交媒体数据作为流行病学调查补充资源的可行性，并为评估特定干预措施、治疗或产前暴露对母婴健康结果的因果影响提供了潜在应用。

Conclusion: 社交媒体数据可以作为研究妊娠结局的辅助资源，为未来的健康研究提供框架，特别是涉及孕妇队列和比较组的研究。该方法扩展了流行病学调查的数据来源，有助于更全面地理解不良妊娠结局及其影响因素。
...
TL;DR: WeDLM是一个基于因果注意力的扩散语言模型解码框架，通过拓扑重排序和流式解码实现前缀缓存友好的并行生成，在保持AR模型质量的同时显著提升推理速度


<details>
  <summary>Details</summary>
Motivation: 自回归解码的token-by-token特性限制了推理时的并行性，而现有扩散语言模型虽然支持并行解码，但由于依赖双向注意力破坏了标准前缀KV缓存，导致实际部署速度无法超过优化的AR引擎

Method: 提出WeDLM框架：1) 基于标准因果注意力构建扩散解码，通过拓扑重排序将已观察token移动到物理前缀同时保持逻辑位置；2) 流式解码过程持续将置信token提交到增长的前缀中，保持固定并行工作量，避免块扩散方法的停止等待行为

Result: WeDLM在保持强AR骨干模型质量的同时实现显著加速：在挑战性推理基准上接近3倍加速，在低熵生成场景中达到10倍加速；与vLLM服务的AR基线在匹配部署设置下比较，证明扩散式解码在实践中可以超越优化的AR引擎

Conclusion: 通过构建完全基于因果注意力的扩散解码框架，WeDLM实现了前缀缓存友好的并行生成，解决了现有扩散语言模型因双向注意力导致的效率问题，在实际部署中超越了优化的自回归引擎
...
TL;DR: 该研究探索了在低资源语言乌尔都语中进行假新闻检测时，通过领域自适应预训练提升多语言模型性能的方法。
...
Method: 采用分阶段训练方法：首先对XLM-RoBERTa和mBERT进行领域自适应预训练（使用公开乌尔都语新闻语料），然后在下游假新闻分类任务上进行微调。

Result: 在四个公开乌尔都语假新闻数据集上的实验表明，领域自适应的XLM-R模型始终优于原始版本，而领域自适应的mBERT结果好坏参半。

Conclusion: 领域自适应预训练能有效提升多语言模型在低资源语言假新闻检测中的性能，特别是对XLM-R模型效果显著，为低资源语言NLP任务提供了有效策略。
...
TL;DR: 评估规则基线、领域特定Transformer模型和大语言模型在临床笔记分割任务上的性能，发现大语言模型在整体分割性能上最优
...
Method: 使用MIMIC-IV中1000份临床笔记的精选数据集，评估了三种方法：1) 基于规则的基线方法；2) 领域特定的Transformer模型；3) 大型语言模型（包括GPT-5-mini等API模型）。评估了句子级和自由文本分割任务。

Result: 大型API模型在整体性能上表现最佳，GPT-5-mini在句子级和自由文本分割上的平均F1分数达到72.4。轻量级基线方法在结构化句子级任务上仍有竞争力，但在非结构化自由文本分割上表现不佳。

Conclusion: 研究结果为临床笔记分割的方法选择提供了指导，并为下游任务（如信息提取、队列识别和自动摘要）奠定了基础。大型语言模型在临床笔记结构化方面显示出优势，特别是在处理非结构化文本时。
...
TL;DR: 提出自动化合成高难度可验证任务的模拟环境管道，以及环境级RL算法来缓解用户不稳定性和提高训练效率


<details>
  <summary>Details</summary>
Motivation: 现有模拟环境中的强化学习存在环境合成半自动化、任务难度不足、模拟用户不稳定以及环境异质性等问题，限制了语言智能体的训练效果

Method: 1) 自动化可扩展的高难度可验证任务模拟环境合成管道；2) 环境级RL算法，缓解用户不稳定性并在环境级进行优势估计

Result: 在tau-bench、tau2-Bench和VitaBench等智能体基准测试中验证了方法的有效性，并展示了良好的跨域泛化能力

Conclusion: 提出的自动化环境合成管道和环境级RL算法有效解决了模拟环境中RL训练的不稳定性和效率问题，提升了语言智能体的性能
...
TL;DR: 提出一种将预训练目标重新解释为策略梯度优化的方法，通过奖励塑造策略平衡多样性和精确性，为后续强化学习提供更优的探索空间


<details>
  <summary>Details</summary>
Motivation: 强化学习能提升大语言模型的推理能力，但其效果严重依赖预训练模型的token输出分布所定义的探索空间。现有交叉熵损失限制了探索潜力，需要重新审视预训练目标如何影响后续RL训练

Method: 将标准交叉熵损失重新解释为单步episode中的策略梯度优化特例，提出广义预训练目标：将下一个token预测建模为随机决策过程，引入奖励塑造策略，使用正奖励缩放因子控制ground-truth token的概率集中度，采用排名感知机制对高排名和低排名负token进行非对称处理

Result: 研究发现，与直觉相反，施加精确性导向的先验（而非更高分布熵）能为强化学习提供更优的探索空间，最终提升端到端推理性能

Conclusion: 通过将监督学习重新框架为策略梯度优化，可以系统地重塑预训练的token输出分布，为后续强化学习训练提供更有利的探索空间，从而提升大语言模型的推理能力
...
TL;DR: 评估三种先进LLM在临床决策支持中的表现，发现模型性能因任务而异，提示工程效果取决于模型和任务，需要定制化策略


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在医学知识评估中表现出潜力，但其在真实临床决策中的实际效用尚未充分探索。本研究旨在评估LLM在整个临床推理工作流程中的表现，并探索提示工程是否能提升其性能。

Method: 使用36个病例研究，评估ChatGPT-4o、Gemini 1.5 Pro和LIama 3.3 70B在五种临床决策任务中的表现：鉴别诊断、必要紧急步骤、相关诊断测试、最终诊断和治疗建议。在两种温度设置（默认vs零）下测试，并应用MedPrompt框架的变体进行提示工程，包括有针对性和随机的动态少样本学习。

Result: 所有模型在不同任务中表现差异显著：最终诊断准确率接近完美，相关诊断测试表现较差，其他任务表现中等。ChatGPT在零温度下表现更好，而LIama在默认温度下表现更强。提示工程并非通用解决方案：显著提升了基线准确率最低的任务（相关诊断测试），但对其他任务反而有害。有针对性的动态少样本提示并不总是优于随机选择。

Conclusion: 提示工程的影响高度依赖于模型和任务，强调了将LLM整合到医疗保健中需要定制化、上下文感知的策略。模型性能的变异性表明需要针对特定临床任务进行优化，而不是采用一刀切的方法。
...
TL;DR: FANG是一种后训练剪枝框架，通过基于功能对神经元分组并自适应分配稀疏度，缓解校准偏差，提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然性能优异，但计算和存储成本高昂。后训练结构化剪枝是有效解决方案，但现有方法在少样本校准集不能充分反映预训练数据分布时，对下游任务的泛化能力有限。

Method: 提出Function-Aware Neuron Grouping (FANG)框架：1) 基于神经元处理的语义上下文类型对功能相似的神经元进行分组；2) 在每个组内进行重要性估计时，给予与神经元组功能角色强相关的token更高权重；3) 保留跨多个上下文类型贡献的神经元；4) 基于功能复杂性自适应地为每个块分配稀疏度。

Result: FANG在保持语言建模性能的同时提升下游任务准确率。与FLAP和OBC两种代表性剪枝方法结合时达到SOTA结果：在30%和40%稀疏度下，平均准确率分别比FLAP和OBC高出1.5%-8.5%。

Conclusion: FANG通过基于功能的神经元分组和自适应稀疏度分配，有效缓解了后训练剪枝中的校准偏差问题，在保持语言建模能力的同时显著提升了下游任务性能。
...
Method: 1) 构建大规模数据集：将抑郁和焦虑症状相关的生态瞬时评估(EMA)响应转换为自然语言描述，从258名参与者获得超过100,000个传感器-文本QA对；2) 训练补丁级编码器：将原始传感器信号直接投影到LLM表示空间，实现原生时间序列集成。
...
TL;DR: 论文质疑Biasing Features指标将思维链标记为不忠实的问题，认为该指标混淆了不忠实性与不完整性，后者是分布式transformer计算转换为线性自然语言叙述所需的损失压缩。


<details>
  <summary>Details</summary>
Motivation: 当前使用Biasing Features指标的研究将思维链标记为不忠实，如果它省略了影响预测的提示注入提示。作者认为这种指标混淆了不忠实性与不完整性，后者是必要的损失压缩过程。

Method: 在Llama-3和Gemma-3上进行多跳推理任务，比较Biasing Features指标与其他指标的评估结果；引入新的faithful@k指标；使用因果中介分析研究非言语化提示如何通过思维链因果中介预测变化。

Result: 许多被Biasing Features标记为不忠实的思维链被其他指标判断为忠实，在某些模型中超过50%；更大的推理时token预算显著提高提示言语化率（某些设置下高达90%）；非言语化提示仍可通过思维链因果中介预测变化。
...
TL;DR: Prompt Choreography：通过动态全局KV缓存高效执行LLM多智能体工作流的框架，支持并行调用和任意消息子集关注，显著降低延迟和提升端到端速度。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多智能体工作流中部署增加，现有方法存在大量冗余计算，导致延迟高、效率低，需要更高效的执行框架。

Method: 提出Prompt Choreography框架，维护动态全局KV缓存，允许每个LLM调用关注任意重新排序的先前编码消息子集，支持并行调用，并通过微调LLM使其适应缓存机制。

Result: 显著降低每消息延迟（首次令牌时间快2.0-6.2倍），在某些以冗余计算为主的工作流中实现超过2.2倍的端到端加速。

Conclusion: Prompt Choreography通过创新的缓存机制和微调策略，有效解决了LLM多智能体工作流中的冗余计算问题，为高效LLM部署提供了实用解决方案。
...
TL;DR: TabiBERT是首个基于ModernBERT架构从头训练的单语土耳其语编码器，在1万亿token的多领域语料上预训练，支持8192token上下文，推理速度提升2.65倍，在土耳其语NLP基准测试中超越现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 尽管BERT等编码器在计算效率、训练稳定性和长上下文建模方面已有显著进步，但土耳其语NLP领域缺乏采用ModernBERT等现代架构从头训练的单语编码器。现有土耳其语模型未能整合RoPE、FlashAttention等最新技术，限制了性能提升。

Method: 基于ModernBERT架构从头训练单语土耳其语编码器TabiBERT，整合Rotary Positional Embeddings (RoPE)、FlashAttention和优化的归一化技术。在848.8亿token的多领域语料库（网页文本73%、科学出版物20%、源代码6%、数学内容0.3%）中采样1万亿token进行预训练。构建TabiBench基准测试，包含28个数据集覆盖8个任务类别，采用GLUE风格的宏平均评估。

Result: TabiBERT支持8192token上下文长度（是原始BERT的16倍），推理速度提升达2.65倍，GPU内存消耗降低，支持更大批次。在TabiBench上获得77.58分，比BERTurk高1.62分，在8个类别中的5个达到SOTA：问答（+9.55）、代码检索（+2.41）、文档检索（+0.60）。相比任务特定最佳结果（包括TurkishBERTweet等专门模型），平均提升+1.47分，显示出强大的跨领域泛化能力。

Conclusion: TabiBERT填补了土耳其语NLP领域缺乏现代架构单语编码器的空白，通过整合最新技术显著提升了性能、效率和上下文处理能力。该研究为土耳其语编码器研究提供了透明、可复现的基础，发布了模型权重、训练配置和评估代码。
...
TL;DR: 提出了一种基于储层计算的无矩阵乘法语言模型架构，通过权重共享和储层层插入减少计算开销，在保持性能的同时显著降低参数量、训练时间和推理时间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理中表现出色，但高昂的计算成本成为主要瓶颈。本研究旨在通过无矩阵乘法语言模型和储层计算架构来提升计算效率。

Method: 1) 采用无矩阵乘法语言模型架构；2) 部分固定和共享选定层的权重；3) 插入储层层以获取丰富的动态表示而无需额外训练开销；4) 组合多个操作以减少内存访问。

Result: 实验结果显示，所提架构将参数量减少最多19%，训练时间减少9.9%，推理时间减少8.0%，同时保持与基线模型相当的性能。

Conclusion: 通过结合无矩阵乘法架构和储层计算技术，能够有效降低语言模型的计算成本，为高效语言模型设计提供了有前景的方向。
...
TL;DR: 该论文提出了BiomedTLDR数据集，包含大量研究人员撰写的科学论文摘要，用于评估大语言模型生成科学极简摘要的能力。研究发现，虽然部分LLM能生成类似人类的摘要，但总体上LLM更倾向于使用原文的词汇选择和修辞结构，相比人类更偏向抽取式而非抽象式摘要。


<details>
  <summary>Details</summary>
Motivation: 高质量的科学极简摘要（TLDR）有助于有效的科学传播。目前缺乏全面、高质量的科学TLDR数据集，这阻碍了LLM摘要能力的开发和评估。同时需要了解LLM在生成科学摘要方面的表现，以及LLM生成的摘要与人类专家撰写的摘要有何差异。

Method: 1. 提出BiomedTLDR数据集，利用科学论文中常见的作者评论与参考文献结合的做法，收集大量研究人员撰写的摘要。2. 基于论文摘要测试流行的开源权重LLM生成TLDR的能力。3. 分析LLM生成摘要与人类撰写摘要的差异。

Result: 分析显示：1. 部分LLM能够成功生成类似人类的摘要。2. LLM总体上对原文的词汇选择和修辞结构表现出更强的亲和力。3. 与人类相比，LLM总体上更倾向于抽取式而非抽象式摘要。

Conclusion: 该研究填补了科学TLDR数据集的空白，为评估LLM的摘要能力提供了重要资源。研究发现LLM在科学摘要生成方面仍有局限性，更偏向抽取式方法而非人类专家的抽象式方法。数据集和代码已开源，促进相关研究发展。
...
TL;DR: LLM-PeerReview是一种无监督的LLM集成方法，通过同行评审机制从多个LLM生成的候选响应中选择最佳响应，利用多模型的集体智慧。


<details>
  <summary>Details</summary>
Motivation: 现有LLM集成方法需要监督训练或缺乏可解释性。本文旨在开发一种完全无监督、可解释且灵活的LLM集成框架，能够利用多个具有不同优势的LLM的集体智慧。

Method: 提出基于同行评审的三阶段框架：1) 评分阶段：使用LLM-as-a-Judge技术让多个LLM相互评估候选响应；2) 推理阶段：使用基于图模型的真值推理算法或简单平均策略聚合多个评分；3) 选择阶段：选择最高分响应作为最终集成输出。

Result: 在两个变体上，在四个数据集上均取得强劲结果，分别比先进的Smoothie-Global模型高出6.9%和7.3个百分点。

Conclusion: LLM-PeerReview是一种概念简单但经验上强大的无监督LLM集成方法，通过同行评审机制实现了可解释性和灵活性，在多个数据集上表现出色。
...
TL;DR: LLMs在复杂编程任务中易出错，作者假设这是通用语言灵活性导致。他们设计了Anka DSL（数据转换管道专用语言），通过约束语法减少歧义。尽管LLMs从未训练过Anka，但在基准测试中达到99.9%解析成功率和95.8%任务准确率，在多步管道任务上比Python准确率高40个百分点。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成方面表现出色，但在复杂的多步编程任务中仍存在系统性错误。作者假设这些错误源于通用编程语言的灵活性，这种灵活性允许多种有效方法并需要隐式状态管理。为了验证这一假设，需要设计一种能够减少代码生成歧义的领域特定语言。

Method: 作者开发了Anka，一种用于数据转换管道的领域特定语言，具有明确、约束的语法设计。该语言减少了代码生成中的歧义。尽管LLMs（Claude 3.5 Haiku和GPT-4o-mini）从未在Anka上进行过训练，但仅通过上下文提示就能学习该语言。作者创建了包含100个问题的基准测试套件来评估性能，并比较了Anka与Python在多步管道任务上的表现。

Result: Claude 3.5 Haiku在Anka上实现了99.9%的解析成功率和95.8%的总体任务准确率。在多步管道任务中，Anka相比Python有40个百分点的准确率优势（100% vs. 60%）。GPT-4o-mini的交叉验证也确认了这一优势（多步任务上+26.7个百分点）。LLMs能够完全通过上下文提示学习新的DSL，达到接近原生的准确率。

Conclusion: 研究证明了三个关键发现：1）LLMs能够完全通过上下文提示学习新的领域特定语言，达到接近原生的准确率；2）约束语法显著减少了复杂任务中的错误；3）专门为LLM生成设计的领域特定语言可以在LLM有大量训练的通用语言上表现更优。作者发布了完整的语言实现、基准测试套件和评估框架以促进进一步研究。
...
TL;DR: 论文提出一种基于稀疏自编码器的参数高效微调方法，通过解耦特征空间识别任务相关特征，构建显式可解释的低秩子空间来指导适配器初始化，在安全对齐任务中达到接近RLHF的性能，同时提供可解释性。
...
Result: 在安全对齐任务中，该方法达到99.6%的安全率，超过完全微调7.4个百分点，接近基于RLHF的方法，同时仅更新0.19-0.24%的参数。通过SAE特征的语义基础提供对学习对齐子空间的可解释洞察。

Conclusion: 将机制可解释性融入微调过程可以同时提高性能和透明度。该方法为参数高效微调提供了新的范式，结合了性能优势与可解释性。
...
TL;DR: 该研究针对中国电商直播中主播使用"变音词"逃避监管的问题，提出了LiveAMR任务来检测健康医疗直播中的发音规避行为，构建了首个相关数据集并开发了基于LLM的文本生成解决方案。


<details>
  <summary>Details</summary>
Motivation: 中国电商直播平台（如抖音）已成为重要销售渠道，但主播经常使用"变音词"来逃避审查和进行虚假广告宣传。现有研究主要关注社交媒体和地下产业中的文本规避，而健康医疗直播中的发音规避问题尚未得到充分研究。

Method: 1. 提出Live Auditory Morph Resolution (LiveAMR)任务；2. 构建首个LiveAMR数据集，包含86,790个样本；3. 将任务转化为文本到文本生成问题；4. 利用大型语言模型生成额外训练数据以提升性能。

Result: 1. 创建了首个专门针对直播变音词检测的数据集；2. 通过LLM生成额外训练数据的方法有效提升了任务性能；3. 证明了变音词解析能显著增强直播监管效果。

Conclusion: LiveAMR任务填补了直播监管中发音规避检测的研究空白，提出的方法能有效识别健康医疗直播中的变音词使用，为电商直播平台的合规监管提供了重要技术支持。
...
Method: 开发了包含11个专业智能体的多智能体协作框架，包括主题分析师、案例分析师、编辑、叙述者和校对员等，共同探索主题、提取现实案例、优化内容组织和合成自然口语。

Result: 与专家解读相比，AI4Reading在语音生成质量上仍有差距，但生成的解读脚本更简洁、更准确。

Conclusion: AI4Reading系统通过多智能体协作成功实现了有声书解读的自动化生成，在内容准确性和简洁性方面表现良好，但语音质量仍需改进。
...
TL;DR: 该论文系统综述了记忆在人类认知与AI智能体中的桥梁作用，从认知神经科学角度分析记忆机制，并将其应用于LLM驱动的智能体设计，涵盖记忆分类、存储、管理、评估、安全及未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有自主智能体研究虽借鉴认知神经科学设计记忆工作流，但受跨学科壁垒限制，难以真正吸收人类记忆机制的精髓。需要系统整合跨学科记忆知识，连接认知神经科学与LLM智能体。

Method: 采用系统性综述方法：1) 从认知神经科学到LLM再到智能体的渐进轨迹阐明记忆定义与功能；2) 从生物与人工双视角对比分析记忆分类、存储机制及完整管理生命周期；3) 回顾智能体记忆评估的主流基准；4) 从攻防双视角探讨记忆安全；5) 展望未来研究方向。

Result: 建立了认知神经科学与LLM智能体之间的记忆知识桥梁，提供了系统的记忆分类、存储和管理框架，识别了当前记忆评估基准，分析了记忆安全风险与防御策略，并提出了多模态记忆系统和技能获取等未来研究方向。

Conclusion: 记忆是连接过去与未来的关键枢纽，系统整合认知神经科学与AI智能体的记忆知识对于设计高效自主智能体至关重要。未来应关注多模态记忆系统、技能获取等方向，推动智能体记忆机制的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多种NLP任务中表现优异，但在需要深度推理和逻辑推断的复杂场景中仍面临挑战。模型在生成过程中可能引入噪声或无关信息，导致预测错误或与事实知识不一致的输出。

Method: 提出SGR框架：1）为输入查询动态构建外部知识库中的相关子图；2）基于结构化子图指导模型进行多步推理；3）整合多条推理路径生成最终答案。通过逐步推理减少噪声信息影响。

Result: 在多个基准数据集上的实验结果表明，SGR框架持续优于强基线方法，验证了其在增强LLMs推理能力方面的有效性。

Conclusion: SGR框架通过外部子图生成和逐步推理机制，有效提升了大语言模型在复杂推理任务中的准确性和可靠性，为解决LLMs在深度推理方面的局限性提供了新思路。
...
TL;DR: EntroDrop：一种基于熵的token丢弃方法，通过选择性掩码低熵token来平衡学习动态，解决多轮训练中自回归模型性能下降问题
...
Method: 提出EntroDrop方法，这是一种基于熵引导的token丢弃方法，作为结构化数据正则化。该方法选择性掩码低熵token进行训练，并采用课程调度来根据训练进度调整正则化强度。
...
TL;DR: 性别多样性对科学团队影响力的影响呈倒U型关系，女性领导组与男性支持组的组合表现最佳，团队规模调节领导组性别多样性的影响


<details>
  <summary>Details</summary>
Motivation: 学术界对性别多样性如何影响科学团队成功有浓厚兴趣，但先前研究结果不一致，且大多采用聚合度量方式，忽略了团队内部角色分化。这种局限性阻碍了对性别多样性如何塑造团队影响的更细致理解，特别是不同团队角色中性别多样性的影响仍不清楚。

Method: 将科学团队定义为论文的所有合著者，通过五年引用次数衡量团队影响力。利用作者贡献声明将成员分为领导角色和支持角色。基于PLOS期刊超过130,000篇论文（主要为生物医学相关学科），采用多变量回归分析这些角色中性别多样性与团队影响力之间的关系。应用阈值回归模型研究团队规模如何调节这种关系。

Result: 结果显示：(1) 性别多样性与团队影响力之间的关系在领导组和支持组中都呈倒U型；(2) 拥有全女性领导组和全男性支持组的团队比其他团队类型获得更高影响力；(3) 领导组性别多样性的影响在小团队中显著为负，但在大团队中变为正且统计上不显著。相比之下，支持组性别多样性的估计值无论团队大小都保持显著为正。

Conclusion: 性别多样性对科学团队的影响因角色类型和团队规模而异。领导组性别多样性的影响受团队规模调节，而支持组性别多样性始终对团队影响力有积极贡献。全女性领导组与全男性支持组的组合表现最佳，表明不同角色中性别构成的特定组合可能优化团队表现。
...
TL;DR: 提出一种基于大语言模型嵌入的嵌套密度聚类方法，用于构建文本语义关系的层次树结构，无需预定义类别即可发现研究领域及其子领域。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型嵌入常用于向量数据库中的语义相似性检索，但文本语料库中的全局语义关系结构往往不透明。需要一种方法来揭示文本之间的层次语义关系，实现数据驱动的领域发现。

Method: 提出嵌套密度聚类方法：首先在LLM嵌入空间中识别语义相似性强的文本作为密集簇，然后逐步放松密度标准，使密集簇合并为更分散的簇，直到整个数据集形成单个簇（树的根节点），从而构建捕获文本层次语义关系的树结构。

Result: 方法在科学摘要案例研究中成功应用于文本分类，能够数据驱动地发现研究领域及其子领域。在20 Newsgroups和IMDB 50k Movie Reviews等基准数据集上的应用证明了方法的跨领域鲁棒性。

Conclusion: 嵌套密度树方法能够有效揭示文本数据集中的语义结构和演化，在科学计量学、主题演化等领域具有潜在应用价值，为无监督的语义层次发现提供了新工具。
...
TL;DR: ACT是一种用于自动检测拉比文献中圣经引用的三阶段算法，结合了形态感知对齐和上下文丰富化，在短文本、意译和结构嵌入引用检测方面优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有文本重用框架在处理拉比文献中短小、意译或结构嵌入的圣经引用时存在困难，需要一种能够识别复杂引用模式（如"波浪"和"回声"引用）的专门方法。

Method: ACT采用三阶段算法：1) 形态感知对齐算法；2) 上下文敏感丰富化阶段，识别复杂引用模式；3) 评估了三种配置（ACT-QE完整管道、ACT-2无风格丰富化、ACT-3使用更长n-gram）。

Result: 完整ACT管道（ACT-QE）F1分数达0.91，召回率0.89，精确率0.94，优于所有基线系统（Dicta、Passim、Text-Matcher）。ACT-2召回率更高（0.90）但精确率较低，ACT-3在覆盖率和特异性间取得平衡。

Conclusion: ACT不仅改进了引用检测，还能跨语料库分类风格模式，为体裁分类和互文分析开辟新途径，弥合了机器检测与人工编辑判断之间的方法论差距，为形态丰富、引用密集的传统文本分析奠定基础。
...
TL;DR: 视觉生成任务可以增强视觉理解能力，但前提是生成语义而非像素；在大规模预训练下，生成任务展现出更好的数据缩放趋势和更高的数据利用率；输入嵌入的自回归能有效捕捉视觉细节。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言大模型中视觉生成任务是否能够增强视觉理解能力，特别是在大规模数据（>2亿样本）预训练场景下，这一问题尚未得到充分研究。

Method: 提出统一模型UniHetero，采用简洁结构，在大规模预训练数据上进行实验分析。核心方法包括：1）生成语义而非像素；2）利用输入嵌入的自回归机制捕捉视觉细节。

Result: 1）生成确实能提升理解能力，但仅限于生成语义内容而非像素级生成；2）生成任务展现出更优的数据缩放趋势和更高的数据利用率；3）输入嵌入的自回归能有效捕捉视觉细节。

Conclusion: 在大规模预训练下，视觉生成任务能够有效增强视觉理解能力，关键在于生成语义内容而非像素，且自回归机制在输入嵌入层面能更好地捕捉视觉细节，为视觉语言统一模型的设计提供了重要指导。
...
TL;DR: MoLaCE：通过潜在概念专家混合解决LLM确认偏见的轻量级推理框架


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在严重的输入确认偏见问题，当提示暗示偏好答案时，模型会强化这种偏见而非探索替代方案。这种现象在基础模型中已造成危害，在多智能体辩论中风险更大，会形成回音室效应而非纠正偏见。

Method: 提出MoLaCE框架，通过混合以不同激活强度实例化的潜在概念专家来应对确认偏见。核心洞见是：由于语言的组合性，不同措辞的提示会以特定方式重新加权影响事实正确性的潜在概念，因此没有单一的固定干预能普遍适用于所有输入。

Result: 实证表明MoLaCE能持续减少确认偏见、提高鲁棒性，在仅需一小部分计算量的情况下，性能匹配或超越多智能体辩论。该框架也可集成到多智能体辩论框架中以多样化视角并减少相关错误。

Conclusion: MoLaCE使单个LLM能够在保持计算效率和可扩展性的同时，内部模拟辩论的益处，为解决LLM确认偏见问题提供了有效的轻量级解决方案。
...
TL;DR: 该论文提出了一种利用知识图谱增强幻觉自检测的方法，通过将LLM响应转换为实体关系图来估计幻觉可能性，相比现有方法在准确率和F1分数上获得显著提升。


<details>
  <summary>Details</summary>
Motivation: 幻觉（生成看似可信但虚假的陈述）是LLM安全部署的主要障碍。现有自检测方法表现良好，但需要进一步提升检测效果。

Method: 提出简单而强大的方法：1) 将LLM响应转换为实体和关系的知识图谱；2) 使用这些图谱估计响应包含幻觉的可能性。该方法低成本、模型无关。

Result: 在GPT-4o和Gemini-2.5-Flash上评估，使用两个幻觉检测数据集（其中一个经人工整理并公开）。相比标准自检测方法和最先进的SelfCheckGPT，相对准确率提升达16%，F1分数提升达20%。

Conclusion: LLM在分析原子事实时，即使初始输出包含不准确信息，当事实被结构化为知识图谱时表现更好。该方法为实现更安全、更可信的语言模型铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉指令微调后，经常无法展现出原始LLM具备的指令跟随能力，导致模型不按预期执行任务指令

Method: 构建新的训练数据集（突出是否指定输出格式），研究在微调过程中明确指示输出格式如何影响LVLMs的指令跟随能力

Result: 定量评估确认LVLMs的指令跟随能力在常用数据集微调后下降；包含输出格式指令的训练数据能提高模型指令跟随准确性

Conclusion: 在视觉指令微调中包含具有输出格式指令的样本可能有助于缓解指令跟随能力的下降
...
TL;DR: 语音语言模型在多轮对话中无法维持指定的说话风格，存在"风格遗忘"问题，即使能回忆起风格指令也无法在对话中表达。


<details>
  <summary>Details</summary>
Motivation: 研究语音语言模型在遵循特定说话风格指令方面的能力，特别是考察模型在多轮对话中能否持续保持指定的副语言风格（如情感、口音、音量、语速）。

Method: 评估了3个专有模型和2个开源SLM，测试其在多轮对话中维持副语言风格的能力。通过不同提示策略（系统消息vs用户消息）进行实验，并测试模型回忆风格指令的能力。

Result: 所有测试模型都无法在指令要求下维持一致的说话风格。模型能回忆起风格指令但无法在对话中表达。将指令放在系统消息中比放在用户消息中效果更差，这与系统提示的预期功能相矛盾。明确要求模型回忆风格指令可以部分缓解风格遗忘问题。

Conclusion: 语音语言模型存在严重的风格遗忘问题，需要改进模型架构或训练方法以增强风格一致性。当前模型无法有效利用系统提示来维持长期风格，这揭示了SLM在风格控制方面的局限性。
...
TL;DR: InfTool：一个完全自主的多智能体合成框架，通过自我演进的合成循环，使大语言模型能够可靠地调用外部工具，无需人工标注，在BFCL基准上将32B基础模型准确率从19.8%提升至70.9%


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个根本挑战：1) 高质量轨迹需要昂贵的人工标注；2) 对未见工具的泛化能力差；3) 单模型合成固有的质量上限导致偏见和覆盖缺口。需要一种完全自主的框架来突破这些限制。

Method: InfTool采用自我演进的多智能体合成框架，包含三个协作智能体：用户模拟器、工具调用助手和MCP服务器。仅基于原始API规范，生成从单次调用到复杂多步工作流的多样化验证轨迹。建立闭环：合成数据通过带门控奖励的组相对策略优化(GRPO)训练模型，改进后的模型生成更高质量数据针对能力缺口，无需人工干预迭代循环。

Result: 在伯克利函数调用排行榜(BFCL)上的实验表明，InfTool将32B基础模型的准确率从19.8%提升至70.9%(+258%)，超越了10倍大的模型，与Claude-Opus相媲美，且完全基于合成数据无需人工标注。

Conclusion: InfTool通过完全自主的自我演进多智能体合成，成功解决了大语言模型可靠调用外部工具的关键瓶颈，实现了从单次调用到复杂工作流的多样化轨迹生成，显著提升了模型性能，为自主智能体发展提供了新范式。
...
TL;DR: 研究语言模型概率与认知现象关系时所需上下文量的定义，验证n-gram表示是否足以作为认知规划单元


<details>
  <summary>Details</summary>
Motivation: 探究在语言模型概率与认知现象关系研究中，需要多少上下文信息才足够，以及是否必须使用完整话语来观察概率缩减现象

Method: 通过比较完整话语与n-gram表示，研究是否n-gram表示足以作为认知规划单元来观察概率缩减现象

Result: 研究发现n-gram表示足以作为认知规划单元，无需完整话语即可观察概率缩减现象

Conclusion: 在语言模型概率与认知现象关系研究中，n-gram表示可作为有效的认知规划单元，简化了研究所需的上下文信息量
...
TL;DR: 评估LLM在学术同行评审中对文档级隐藏提示注入攻击的脆弱性，发现英语、日语和中文提示注入会显著影响评审分数和录用决策，而阿拉伯语影响较小
...
Method: 构建包含约500篇ICML录用真实学术论文的数据集，在每篇论文中嵌入四种不同语言（英语、日语、中文、阿拉伯语）的语义等效隐藏对抗提示，然后使用LLM进行评审

Result: 提示注入对英语、日语和中文的评审分数和录用/拒绝决策产生显著影响，而阿拉伯语注入几乎无影响，表明LLM基于评审系统对文档级提示注入具有易感性，且不同语言间存在明显差异

Conclusion: LLM在学术同行评审中容易受到文档级隐藏提示注入攻击，不同语言的脆弱性存在显著差异，这凸显了在部署LLM评审系统时需要加强安全防护和语言敏感性考虑
...
TL;DR: ProfASR-Bench是一个专业场景ASR评估套件，用于评估金融、医疗、法律和技术等高风险应用中的语音识别性能，特别关注实体识别和上下文利用能力。


<details>
  <summary>Details</summary>
Motivation: 现有ASR基准测试未能充分反映专业场景中的挑战：密集领域术语、正式语域变化以及对关键实体错误的零容忍度。需要专门评估套件来测量上下文条件识别能力。

Method: 创建ProfASR-Bench专业对话评估套件，包含自然语言提示（领域线索和/或说话者档案）与富含实体的目标话语配对。支持传统ASR指标、实体感知分数以及按口音和性别的切片报告。在无上下文、档案、领域+档案、oracle和对抗性条件下测试Whisper和Qwen-Omni模型。

Result: 发现一致的上下文利用差距（CUG）：轻量级文本上下文对平均词错误率（WER）几乎没有影响，即使使用oracle提示；对抗性提示也不能可靠地降低性能。当前系统名义上可提示但未充分利用可用侧信息。

Conclusion: ProfASR-Bench提供了标准化的上下文阶梯、具有置信区间的实体和切片感知报告，以及用于比较不同模型家族融合策略的可复现测试平台。揭示了当前ASR系统在利用上下文信息方面的局限性。
...
TL;DR: 提出基于反馈驱动改进链的偏好监督微调方法，通过标注细粒度反馈和增量式改写构建偏好对，优于传统A/B偏好排序或完整对比改写方法


<details>
  <summary>Details</summary>
Motivation: 传统直接对齐方法基于标准A/B偏好排序或完整对比改写，缺乏细粒度的反馈监督，难以实现高效有效的偏好调优

Method: 1) 标注者对模型响应提供细粒度反馈，标记"喜欢"和"不喜欢"的文本片段并说明原因；2) 基础模型根据反馈从左到右逐步改写不喜欢片段，形成增量改进链；3) 从链中相邻步骤构建偏好对用于直接对齐

Result: 该方法优于基于标准A/B偏好排序或完整对比改写的直接对齐方法，表明结构化、基于修订的监督能实现更高效有效的偏好调优

Conclusion: 反馈驱动改进链方法通过细粒度反馈和增量式改写构建偏好对，为语言模型偏好监督提供了更有效的结构化监督框架
...
TL;DR: 该研究提出多轮对话中的行为诱导框架，将现有方法分为三类，并评估它们在自动生成多轮测试用例中的效率，发现在线方法在少量查询下能获得高成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型行为诱导技术主要研究单轮设置，但在多轮对话场景中识别复杂行为对于模型评估至关重要。需要研究多轮对话中的行为诱导方法，以更全面地评估LLMs。

Method: 首先提出分析框架，将现有方法分为三类：仅使用先验知识的方法、使用离线交互的方法、以及从在线交互中学习的方法。然后引入广义的多轮在线方法公式，统一单轮和多轮诱导。最后评估这三类方法在自动生成多轮测试用例中的效率，分析查询预算与成功率之间的权衡。

Result: 在线方法在三个任务中仅用几千次查询就能达到平均45%/19%/77%的成功率，而现有的多轮对话基准中的静态方法发现很少甚至没有失败案例。这表明在线方法在多轮对话评估中具有显著优势。

Conclusion: 研究突出了行为诱导方法在多轮对话评估中的新应用，并表明社区需要向动态基准测试方向发展。在线方法在有限的查询预算下能够有效发现LLMs的失败案例，为更全面的模型评估提供了有效工具。
...
TL;DR: MESA MIG是一个多智能体语义和情感对齐框架，通过结构化音乐描述和专门化智能体协作，从音乐生成视觉图像，同时确保语义和情感一致性。
...
Method: 提出多智能体语义和情感对齐框架：首先生成结构化音乐描述，然后由场景、运动、风格、色彩和构图等专门化智能体协作优化；同时使用Valence Arousal回归头预测音乐情感状态，CLIP视觉VA头估计图像情感，共同确保音乐与合成图像的语义和情感对齐。

Result: 在精选的音乐-图像对上，MESA MIG在美学质量、语义一致性和VA对齐方面优于仅使用描述和单智能体基线，在情感回归性能上与最先进的音乐和图像情感模型相当。

Conclusion: MESA MIG框架成功实现了从音乐生成视觉图像，并通过多智能体协作和情感对齐机制确保了语义和情感的一致性，为音乐驱动的视觉内容生成提供了有效解决方案。
...
TL;DR: RTC Helper是一个浏览器扩展软件架构，可拦截WebRTC API并实时修改网页应用行为，支持终端用户和开发者自定义实时通信应用


<details>
  <summary>Details</summary>
Motivation: 解决网页多媒体通信应用中终端用户驱动的创新需求，以及开发者快速原型设计的效率问题，使第三方网页应用能够灵活定制实时通信功能

Method: 开发了RTC Helper浏览器扩展软件，能够拦截浏览器中的WebRTC及相关API，实时改变网页应用行为，提供超过十个自定义类别和上百个内置示例

Result: 创建了一个简单易用的软件架构，支持终端用户在第三方网页应用上进行实时通信功能定制，同时帮助开发者无需重建或重新部署即可快速原型设计

Conclusion: RTC Helper提供了一个灵活通用的解决方案，促进了基于网页的音视频通信应用创新，既满足了终端用户的自定义需求，又提高了开发者的工作效率
...
### [183] [ReCollab: Retrieval-Augmented LLMs for Cooperative Ad-hoc Teammate Modeling](https://arxiv.org/abs/2512.22129)
*Conor Wallace,Umer Siddique,Yongcan Cao*

Main category: cs.MA

TL;DR: 提出Collab和ReCollab框架，利用LLM作为队友行为的世界模型，通过检索增强生成提升ad-hoc teamwork中的适应性


<details>
  <summary>Details</summary>
Motivation: 传统ad-hoc teamwork方法依赖固定的概率模型或分类器，在部分可观测和有限交互下表现脆弱。LLM提供了灵活替代方案，可将行为轨迹映射为高层假设，作为队友行为的世界模型

Method: Collab：基于语言的框架，使用从轨迹特征导出的行为准则对队友类型进行分类。ReCollab：扩展Collab，结合检索增强生成（RAG），使用示例轨迹稳定推理

Result: 在合作性Overcooked环境中，Collab能有效区分队友类型，而ReCollab在不同布局中持续提升适应性，在分类准确性和回合回报之间实现帕累托最优权衡

Conclusion: LLM作为行为世界模型在ad-hoc teamwork中具有潜力，检索增强在具有挑战性的协调场景中对于稳定推理至关重要
...
### [184] [Solving Multi-Agent Multi-Goal Path Finding Problems in Polynomial Time](https://arxiv.org/abs/2512.22171)
*Stefan Edelkamp*

Main category: cs.MA

TL;DR: 提出一种用于多智能体在无向图中执行多目标任务的规划方法，能够自主发现和更新目标分配，在连续情况下可逼近最优解，离散情况下可在多项式时间内解决冲突


<details>
  <summary>Details</summary>
Motivation: 传统多智能体路径规划需要预先指定目标分配，而本文旨在开发能够自主发现和更新目标分配的规划器，解决多智能体在无向图中执行多目标任务的协调问题

Method: 采用全局分配策略减少冲突，剩余冲突通过扩展"蚂蚁上杆"概念、解决局部分配问题、交错智能体路径以及将已到达目标的智能体"踢出"目的地等方法解决

Result: 在连续情况下可实现接近最优解，在离散情况下（存在节点和边冲突）可在多项式时间内解决，这出人意料，因为传统车辆路径规划在一般图上通常是NP难问题

Conclusion: 提出的规划器能够为智能体找到无冲突的优化路径，全局分配策略显著减少冲突数量，通过多种冲突解决技术实现高效的多智能体多目标任务规划
...
### [185] [MARPO: A Reflective Policy Optimization for Multi Agent Reinforcement Learning](https://arxiv.org/abs/2512.22832)
...
Method: 提出MARPO框架，包含两个关键组件：1）反射机制，利用后续轨迹提高样本效率；2）不对称裁剪机制，基于KL散度动态调整裁剪范围以提升训练稳定性

Result: 在经典多智能体环境中，MARPO始终优于其他方法

Conclusion: MARPO通过创新的反射和不对称裁剪机制有效解决了多智能体强化学习的样本效率问题
...
### [186] [Reinforcement Networks: novel framework for collaborative Multi-Agent Reinforcement Learning tasks](https://arxiv.org/abs/2512.22876)
*Maksim Kryzhanovskiy,Svetlana Glazyrina,Roman Ischenko,Konstantin Vorontsov*

Main category: cs.MA

TL;DR: 提出了Reinforcement Networks框架，将多智能体强化学习中的智能体组织为有向无环图的顶点，实现灵活信用分配和可扩展协调，避免现有方法的限制。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统通常包含多个可学习组件，这些组件可以自然地组织为图结构。当前多智能体强化学习方法存在架构或训练假设的限制，需要一种无需严格拓扑结构、完全集中式训练等限制的端到端训练框架。

Method: 提出Reinforcement Networks框架，将智能体组织为有向无环图（DAG）的顶点。该框架扩展了分层强化学习到任意DAG结构，支持灵活信用分配和可扩展协调。通过LevelEnv概念支持可重复的构建、训练和评估。

Result: 在多个协作多智能体强化学习设置中开发了多个Reinforcement Networks模型，相比标准MARL基线实现了性能提升。该框架统一了分层、模块化和图结构化的MARL视图。

Conclusion: Reinforcement Networks为设计和训练复杂多智能体系统提供了原则性路径，可作为可扩展、结构化MARL研究的基础。未来方向包括更丰富的图形态、组合课程学习和图感知探索。
...
### [187] [Heterogeneity in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2512.22941)
*Tianyi Hu,Zhiqiang Pu,Yuan Wang,Tenghai Qiu,Min Chen,Xin Yu*

Main category: cs.MA

TL;DR: 该论文系统性地探讨了多智能体强化学习中的异质性，提出了异质性的分类、量化方法，并设计了一个基于异质性的动态参数共享算法。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习领域缺乏对异质性的严格定义和深入理解，而异质性与智能体功能差异、策略多样性和环境交互密切相关，需要系统性的研究框架。

Method: 1) 基于智能体级建模将异质性分为五类并提供数学定义；2) 定义异质性距离并提出量化方法；3) 设计基于异质性的多智能体动态参数共享算法。
...
### [188] [Assessing behaviour coverage in a multi-agent system simulation for autonomous vehicle testing](https://arxiv.org/abs/2512.23445)
*Manuel Franco-Vivo*

Main category: cs.MA

TL;DR: 该研究提出了一种用于自动驾驶车辆测试的多智能体系统仿真中的行为覆盖分析方法，通过定义驾驶场景和智能体交互来评估仿真环境对相关行为的覆盖程度，并提出了基于模型预测控制（MPC）的行人智能体来生成更有趣和真实的测试。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶技术的发展，确保这些系统的安全性和可靠性变得至关重要。需要全面的测试方法来评估自动驾驶车辆在多样复杂现实场景中的性能。当前需要系统化的方法来衡量和评估仿真环境中的行为覆盖程度。

Method: 研究采用行为覆盖分析方法，通过定义驾驶场景和智能体交互来评估仿真环境对相关行为的覆盖程度。提出了基于模型预测控制（MPC）的行人智能体，其目标函数被设计为鼓励生成"有趣"的测试，同时比其他先前研究的行人智能体表现出更真实的行为。

Result: 研究发现行为覆盖在验证自动驾驶系统有效性和鲁棒性方面具有重要意义。通过行为覆盖度量和基于覆盖的测试分析，识别了仿真框架中需要改进和优化的关键领域。提出的MPC行人智能体能够生成更有趣和更真实的测试场景。

Conclusion: 该研究通过提供对仿真环境中系统行为全面评估的见解，推动了自动驾驶测试领域的发展。研究结果为通过严格的测试方法增强自动驾驶车辆的安全性、可靠性和性能提供了有价值的启示。
...
TL;DR: Bidirectional RAG是一种新型检索增强生成架构，通过验证后的高质量生成响应实现安全语料库扩展，使RAG系统能够从用户交互中学习进化。
...
Method: 提出双向RAG架构，采用多阶段接受层：基于NLI的蕴含验证、归因检查和新颖性检测，确保高质量生成响应安全写回知识库，实现知识积累。
...
TL;DR: 研究发现，大型语言模型在未经明确提示的情况下也可能产生说服行为，特别是通过监督微调（SFT）而非激活引导时，这种无提示说服风险会显著增加，且微调于良性话题的数据集可能导致模型在争议性和有害话题上产生说服行为。


<details>
  <summary>Details</summary>
Motivation: 随着对话式AI系统的广泛应用，AI对人类观点和信念的影响力前所未有。先前研究主要关注模型被恶意提示时的说服风险（滥用威胁模型），但本文旨在探究模型在未经明确提示情况下产生说服行为的条件，以评估这种新兴风险的实际关注度。

Method: 研究无提示说服在两种场景下的表现：1）通过内部激活引导使模型沿特定人格特质方向调整；2）通过监督微调（SFT）使模型展现相同特质。实验评估了与说服相关及无关的特质引导效果，并测试了在仅包含良性话题的通用说服数据集上进行SFT后模型在争议性和有害话题上的说服倾向。

Result: 激活引导（无论是否与说服相关）并不能可靠增加模型的无提示说服倾向，但监督微调（SFT）能显著提高这种倾向。更重要的是，在仅包含良性话题的通用说服数据集上进行SFT后，模型在争议性和有害话题上表现出更高的说服倾向，表明有害的无提示说服可能意外产生。

Conclusion: 无提示说服风险在监督微调场景下更为显著，且微调于良性数据可能导致模型在有害话题上产生意外说服行为。这表明新兴有害说服风险确实存在，需要进一步研究和关注，特别是在模型微调和部署过程中需要考虑这种潜在风险。
...
TL;DR: GamiBench是一个评估多模态大语言模型空间推理能力的基准测试，通过折纸启发的折叠任务来评估2D到3D的规划和空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在感知和指令跟随方面表现良好，但在空间推理（跨多视角和时间跟踪和操作物体的能力）方面仍有不足。现有基准测试主要关注静态图像或最终输出，未能考虑空间推理的顺序性和视角依赖性特点。

Method: 创建了包含186个常规和186个不可能2D折痕模式及其对应3D折叠形状的数据集，从6个不同视角呈现。设计了三个视觉问答任务：预测3D折叠配置、区分有效视角、检测不可能模式。引入新的诊断指标：视角一致性（VC）和不可能折叠选择率（IFSR）。

Result: 实验表明，即使是领先模型如GPT-5和Gemini-2.5-Pro在单步空间理解方面也表现不佳。GamiBench提供了评估MLLMs几何理解和空间推理的标准化框架。

Conclusion: GamiBench填补了现有基准测试在评估空间推理能力方面的空白，通过折纸启发的任务全面评估MLLMs的空间推理过程，为评估几何理解和空间推理能力建立了标准化框架。
...
TL;DR: 该论文提出了一个公平感知的AI框架，用于优化孟加拉国洪水后援助分配，通过对抗性去偏技术减少对边缘化地区的系统性偏见，同时保持预测准确性。


<details>
  <summary>Details</summary>
Motivation: 发展中国家灾后援助分配存在系统性偏见，使脆弱地区处于不利地位，延续历史不平等。孟加拉国作为洪水频发国家，需要更公平的援助分配机制。

Method: 采用对抗性去偏模型，使用梯度反转层学习偏置不变表示。基于2022年孟加拉国洪水真实数据（影响720万人，损失4.055亿美元），在11个地区的87个upazilas上应用公平感知表示学习技术。

Result: 模型将统计均等差异减少41.6%，区域公平差距降低43.2%，同时保持强预测准确性（R平方=0.784 vs 基线0.811）。生成可操作的优先级排名，确保援助基于真实需求而非历史分配模式。

Conclusion: 该研究证明算法公平技术可有效应用于人道主义背景，为决策者提供实施更公平灾后恢复策略的工具，确保援助到达最脆弱人群。
...
### [193] [Logic Sketch Prompting (LSP): A Deterministic and Interpretable Prompting Method](https://arxiv.org/abs/2512.22258)
*Satvik Tripathi*
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言推理方面表现出色，但在需要严格规则遵循、确定性和可审计性的任务上仍然不可靠。特别是在临床、受监管和安全关键决策支持系统中，这种不可靠性是一个重要问题。

Method: 提出Logic Sketch Prompting (LSP)框架，包含三个核心组件：1) 类型化变量，2) 确定性条件评估器，3) 基于规则的验证器。该框架旨在产生可追踪和可重复的输出。在两个药理学逻辑合规任务上，将LSP与零样本提示、思维链提示和简洁提示进行比较，使用三个开源模型（Gemma 2、Mistral、Llama 3）进行基准测试。

Result: 在所有模型和任务中，LSP始终获得最高的准确率（0.83-0.89）和F1分数（0.83-0.89），显著优于零样本提示（0.24-0.60）、简洁提示（0.16-0.30）和思维链提示（0.56-0.75）。McNemar检验显示LSP在几乎所有比较中都取得了统计学上显著的改进（p < 0.01）。

Conclusion: LSP在不牺牲性能的情况下提高了确定性、可解释性和一致性，支持其在临床、受监管和安全关键决策支持系统中的应用。该框架为解决大语言模型在规则遵循任务上的不可靠性提供了一种有效方法。
...
### [194] [Agent2World: Learning to Generate Symbolic World Models via Adaptive Multi-Agent Feedback](https://arxiv.org/abs/2512.22336)
...
<details>
  <summary>Details</summary>
Motivation: 当前训练LLMs生成符号世界模型（如PDDL领域或可执行模拟器）受到大规模可验证监督数据缺乏的限制，现有静态验证方法无法捕捉交互执行中出现的行为级错误

Method: 提出Agent2World三阶段流水线：1) Deep Researcher通过网页搜索进行知识合成以填补规范空白；2) Model Developer实现可执行世界模型；3) 专门的Testing Team进行自适应单元测试和基于模拟的验证

Result: 在涵盖PDDL和可执行代码表示的三个基准测试中实现最先进的推理时性能；使用Testing Team作为交互环境生成的训练轨迹进行微调后，世界模型生成平均相对提升30.95%
...
### [195] [Subgoaling Relaxation-based Heuristics for Numeric Planning with Infinite Actions](https://arxiv.org/abs/2512.22367)
...
<details>
  <summary>Details</summary>
Motivation: 标准数字规划模型引入控制参数后会产生无限数量的适用动作，使得现成的数字启发式方法不可行，需要找到处理这类问题的可行方法

Method: 识别可控简单数字问题这一可处理子集，采用乐观编译方法将其转换为简单数字任务，通过将控制依赖表达式抽象为有界常数效应和松弛前提条件来实现

Result: 提出的编译方法能够有效使用子目标启发式来估计带有控制参数的数字规划问题的目标距离，在计算上是可行的

Conclusion: 该方法是将传统数字启发式应用于具有无限可能动作场景的有效且计算可行的方法，推动了当前技术水平的边界
...
TL;DR: 结构化审议能提升人类预测准确性，本研究探讨类似干预（让LLM在更新前互相评审预测）能否提升大语言模型预测准确率。研究发现该方法在多样化模型组中有效，但在同质化模型组中无效。


<details>
  <summary>Details</summary>
Motivation: 结构化审议已被证明能提升人类预测者的表现，本研究旨在探索类似的干预措施（让大语言模型在更新预测前互相评审）是否也能提升LLM的预测准确性，特别是在不同信息分布和模型组合条件下的效果。

Method: 使用202个已解决的二元问题（来自Metaculus Q2 2025 AI预测锦标赛），评估GPT-5、Claude Sonnet 4.5、Gemini Pro 2.5等模型在四种场景下的准确性：1）多样化模型+分布式信息；2）多样化模型+共享信息；3）同质化模型+分布式信息；4）同质化模型+共享信息。通过让LLM在更新预测前互相评审来实施干预。

Result: 干预措施在场景2（多样化模型+共享信息）中显著提升准确性，Log Loss降低0.020（相对改善约4%，p=0.017）。但在同质化模型组（相同模型的三个实例）中未观察到益处。意外的是，为LLM提供额外上下文信息并未改善预测准确性，限制了研究信息汇集机制的能力。

Conclusion: 结构化审议可能是提升LLM预测准确性的可行策略，特别是在模型多样化且信息共享的条件下。然而，同质化模型组无法从该干预中获益，且额外信息提供未能改善预测，表明审议效果可能依赖于模型多样性和特定信息条件。
...
### [197] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 该论文针对材料科学领域LLM幻觉问题，提出了HalluMatData基准数据集和HalluMatDetector多阶段检测框架，通过综合验证方法将幻觉率降低30%，并引入PHCS指标量化模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在加速科学发现的同时存在严重幻觉问题，生成事实错误或误导性信息，损害研究完整性。材料科学领域尤其需要专门的幻觉检测方法和评估基准来确保AI生成内容的可靠性。

Method: 提出HalluMatData基准数据集用于评估幻觉检测方法；开发HalluMatDetector多阶段检测框架，整合内在验证、多源检索、矛盾图分析和基于度量的评估；引入Paraphrased Hallucination Consistency Score (PHCS)量化语义等价查询下的响应不一致性。

Result: 材料科学不同子领域的幻觉水平差异显著，高熵查询表现出更大的事实不一致性；使用HalluMatDetector验证流程可将幻觉率相比标准LLM输出降低30%；PHCS指标能有效揭示模型在语义等价查询下的可靠性问题。

Conclusion: 该研究为解决材料科学领域LLM幻觉问题提供了系统解决方案，包括基准数据集、检测框架和量化指标，显著提升了AI生成内容的可靠性，为科学发现中的AI应用建立了更严格的质量控制标准。
...
### [198] [Lightweight Inference-Time Personalization for Frozen Knowledge Graph Embeddings](https://arxiv.org/abs/2512.22398)
*Ozan Oguztuzun,Cerag Oguztuzun*

Main category: cs.AI

TL;DR: GatedBias：轻量级推理时个性化框架，通过结构门控适配将冻结的知识图谱嵌入适应到个体用户上下文，仅需约300个可训练参数，在保持全局准确性的同时显著提升个性化排名性能。
...
Method: 提出GatedBias框架，采用结构门控适配机制：将用户特定特征与图导出的二元门结合，生成可解释的每实体偏置。该方法仅需约300个可训练参数，在推理时适配冻结的知识图谱嵌入，无需重新训练。

Result: 在两个基准数据集（Amazon-Book和Last-FM）上评估，GatedBias在保持群体性能的同时，在一致性指标上显示出统计显著的改进。反事实扰动实验验证了因果响应性：受益于特定偏好信号的实体，当这些信号增强时，排名改进幅度提高6-30倍。

Conclusion: 知识图谱基础模型的个性化适配可以实现参数高效且因果可验证，从而在通用知识表示与个体用户需求之间架起桥梁。GatedBias展示了轻量级推理时个性化框架的可行性。
...
### [199] [Monadic Context Engineering](https://arxiv.org/abs/2512.22431)
*Yifan Zhang,Mengdi Wang*

Main category: cs.AI

TL;DR: MCE是一种基于函子、应用函子和单子的代数结构的新型智能体架构范式，通过计算上下文管理状态、错误处理和并发等横切关注点，实现可组合、鲁棒且高效的AI智能体设计。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的自主智能体架构通常采用命令式、临时性的设计模式，导致系统脆弱，难以管理状态、处理错误和实现并发。需要一种形式化的理论基础来构建更健壮、可组合的智能体系统。

Method: 提出单子上下文工程（MCE）架构范式，利用函子、应用函子和单子的代数结构为智能体设计提供形式化基础。将智能体工作流视为计算上下文，通过抽象代数的固有属性管理状态传播、短路错误处理和异步执行等横切关注点。使用单子实现鲁棒的顺序组合，应用函子提供并行执行的结构化方法，单子变换器则允许系统性地组合这些能力。

Result: MCE能够从简单、可独立验证的组件构建复杂、弹性且高效的AI智能体。进一步扩展该框架到元智能体，利用MCE进行生成式编排，通过元编程动态创建和管理子智能体工作流。

Conclusion: MCE为AI智能体设计提供了基于代数结构的形式化架构范式，解决了当前命令式设计中的脆弱性问题，实现了可组合、可验证且高效的智能体系统构建方法。
...
TL;DR: DarkPatterns-LLM是一个用于评估大语言模型操纵性内容的综合基准数据集和诊断框架，包含7个伤害类别和四层分析流程，通过401个专家标注样本揭示了主流模型在检测自主性侵害模式方面的显著性能差异和弱点。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准主要依赖粗糙的二元标签，无法捕捉构成操纵的微妙心理和社会机制。随着大语言模型的普及，对其操纵性或欺骗性行为的担忧日益加剧，这些行为可能损害用户自主性、信任和福祉。

Method: 提出了DarkPatterns-LLM基准数据集和诊断框架，包含401个精心策划的指令-响应对和专家标注。框架采用四层分析流程：多粒度检测(MGD)、多尺度意图分析(MSIAN)、威胁协调协议(THP)和深度上下文风险对齐(DCRA)。涵盖七个伤害类别：法律/权力、心理、情感、身体、自主性、经济和社会伤害。

Result: 评估了包括GPT-4、Claude 3.5和LLaMA-3-70B在内的最先进模型，观察到显著的性能差异（65.2%–89.7%），并发现模型在检测自主性侵害模式方面存在一致的弱点。
...
### [201] [Lessons from Neuroscience for AI: How integrating Actions, Compositional Structure and Episodic Memory could enable Safe, Interpretable and Human-Like AI](https://arxiv.org/abs/2512.22568)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的基础模型虽然取得了显著进展，但仅关注最小化下一个token预测损失，忽略了神经科学中预测编码模型的三个重要组件：动作与生成模型的紧密整合、层次组合结构和情景记忆。这些缺失导致AI存在幻觉、概念理解肤浅、缺乏主体性/责任感、可解释性不足以及能效低下等问题。

Method: 提出将神经科学和认知科学中的预测编码模型组件整合到基础模型中的框架：1）在多个抽象层次上整合动作与生成模型；2）采用组合式生成架构；3）集成情景记忆系统。通过分析神经科学证据支持这些组件的必要性，并与当前趋势（如思维链推理和检索增强生成）进行比较，探讨如何用脑启发组件增强这些模型。

Result: 论文提出了一个理论框架，但未提供具体的实验结果。主要贡献在于：1）识别了当前基础模型与神经科学预测编码模型之间的关键差距；2）提出了整合动作、组合结构和情景记忆的具体方向；3）展示了这种整合如何解决当前AI的缺陷；4）比较了与现有增强方法（CoT、RAG）的关系。

Conclusion: 为了开发安全、可解释、节能且类人的AI，基础模型需要整合神经科学预测编码模型的三个关键组件：动作整合、层次组合结构和情景记忆。重新激活脑科学与AI之间的思想交流将有助于实现以人为中心的安全可解释AI。这种整合可以解决幻觉、缺乏主体性、可解释性不足和能效低下等当前缺陷。
...
### [202] [Tyee: A Unified, Modular, and Fully-Integrated Configurable Toolkit for Intelligent Physiological Health Care](https://arxiv.org/abs/2512.22601)
...
Result: Tyee在所有评估任务中均表现优异，优于或匹配基线方法，在13个数据集中有12个取得了最先进的结果，证明了其实际有效性和泛化能力。

Conclusion: Tyee为生理信号分析提供了一个统一、模块化、可配置的工具包，解决了该领域的关键瓶颈问题，并通过开源发布和持续维护促进社区发展。
...
### [203] [Learning Multi-Modal Mobility Dynamics for Generalized Next Location Recommendation](https://arxiv.org/abs/2512.22605)
...
Conclusion: 通过多模态时空知识表征移动动态，M³ob方法有效提升了位置推荐的准确性和泛化能力，特别是在异常场景下表现突出
...
TL;DR: DICE是一个用于检索增强生成系统评估的两阶段框架，通过可解释的推理痕迹和概率评分提高评估的透明度和鲁棒性，同时采用瑞士系统锦标赛降低计算复杂度。
...
Method: DICE采用两阶段证据耦合框架：1）结合深度分析推理与概率{A, B, Tie}评分，产生透明、置信度感知的判断；2）采用瑞士系统锦标赛，将计算复杂度从O(N²)降低到O(N log N)。框架支持通过可解释的推理痕迹进行系统错误诊断。

Result: 在中文金融QA数据集上的验证显示，DICE与人类专家的一致性达到85.7%，显著优于RAGAS等现有LLM评估指标。在八系统评估中实现了42.9%的计算量减少，同时保持了排名保真度。

Conclusion: DICE建立了一个负责任、可解释且高效的范式，用于可信赖的RAG系统评估，支持可解释的推理痕迹和系统错误诊断，为RAG技术的负责任部署提供了评估框架。
...
TL;DR: 提出一个理论框架，将情景记忆与强化学习结合，实现大语言模型智能体的持续体验学习，通过反思机制实现无需反向传播或微调的适应


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练和部署之间存在严格分离，需要参数更新（反向传播或微调）才能适应新环境。本文旨在建立一个理论框架，使语言模型智能体能够通过与环境交互实现持续学习，而无需修改模型参数

Method: 引入状态化反思决策过程，将反思学习建模为与情景记忆的两阶段读写交互：写入存储交互结果（策略评估），读取检索相关历史案例（策略改进）。该过程诱导出增强状态记忆表示的等效马尔可夫决策过程，允许使用动态规划和强化学习的经典工具。使用熵正则化策略迭代实例化框架

Result: 建立了收敛保证：随着情景记忆增长并充分覆盖状态空间，所得策略收敛到最优解。该框架为基于记忆增强和检索的语言模型智能体提供了理论基础，使其能够在无需参数更新的情况下实现持续适应

Conclusion: 提出了一个理论框架，通过整合情景记忆和强化学习，使大语言模型智能体能够通过反思机制实现持续体验学习，无需反向传播或模型微调，打破了传统训练与部署的分离
...
TL;DR: HiSciBench是一个分层科学智能基准测试，包含5个层次（科学素养到科学发现），涵盖6个学科，8735个实例，支持多模态输入和跨语言评估，用于全面评估基础模型的科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准测试存在碎片化问题，大多关注狭窄任务，未能反映真实科学探究的层次性和多学科性质。需要一个新的综合性基准来评估基础模型在整个科学工作流程中的能力。

Method: 设计了分层基准HiSciBench，包含五个层次：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。包含8735个精心策划的实例，涵盖数学、物理、化学、生物、地理和天文六个主要学科，支持文本、方程、图表等多模态输入和跨语言评估。

Result: 对GPT-5、DeepSeek-R1等多个领先模型进行评估，发现显著性能差距：模型在基础素养任务上准确率可达69%，但在发现级挑战上急剧下降至25%。揭示了模型在不同科学推理阶段的能力差异。
...
TL;DR: Gamma提出了一种用于知识图谱推理的多头几何注意力基础模型，通过多种代数变换和自适应融合机制，在零样本归纳链接预测中显著优于现有方法Ultra。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型（如Ultra）依赖单一的关系变换（如逐元素乘法）进行消息传递，这限制了表达能力，无法捕捉多样化图谱中表现出的不同关系和结构模式。

Method: Gamma引入多头几何注意力，用多种并行关系变换（基于实数、复数、分裂复数和双数）替代单一变换，并通过关系条件注意力融合机制在链接级别自适应融合这些变换，使用轻量级门控和熵正则化。

Result: 在56个多样化知识图谱上的综合实验表明，Gamma在零样本归纳链接预测中一致优于Ultra，在归纳基准测试中平均倒数排名提升5.5%，在所有基准测试中提升4.4%。

Conclusion: Gamma通过互补的几何表示提高了知识图谱推理的表达能力，多头几何注意力机制能够更有效地建模不同关系结构，为知识图谱基础模型提供了更强大的推理能力。
...
TL;DR: 本文提出了RW-Post数据集和AgentFact框架，用于解决多模态虚假信息检测中的推理不足和证据利用浅层问题，通过真实世界多模态数据集和基于智能体的协作框架显著提升了检测准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息的快速传播对自动化事实核查系统提出了挑战。现有方法（包括大型视觉语言模型和深度多模态融合方法）由于推理能力有限和证据利用浅层而表现不足。关键瓶颈在于缺乏提供完整真实世界多模态虚假信息实例、附带标注推理过程和可验证证据的专用数据集。

Method: 1. 引入RW-Post数据集：将真实世界多模态声明与其原始社交媒体帖子对齐，保留丰富的上下文信息；通过大语言模型辅助提取流程从人工撰写的事实核查文章中获取详细推理和明确链接的证据。2. 提出AgentFact框架：基于智能体的多模态事实核查框架，模拟人类验证工作流程，包含五个专门智能体（策略规划、高质量证据检索、视觉分析、推理、解释生成），通过迭代工作流程在证据搜索与任务感知的证据过滤和推理之间交替进行。

Result: 广泛的实验结果表明，RW-Post数据集和AgentFact框架之间的协同作用显著提高了多模态事实核查的准确性和可解释性。该框架能够更好地处理真实世界多模态虚假信息检测任务。

Conclusion: 通过引入高质量可解释数据集RW-Post和基于智能体的协作框架AgentFact，本文有效解决了多模态事实核查中的关键瓶颈问题，为自动化虚假信息检测提供了更可靠和可解释的解决方案。
...
TL;DR: LLM导师在K-12教育中无法替代传统学习者建模，知识追踪模型在知识评估的准确性、可靠性和时间一致性方面显著优于LLM


<details>
  <summary>Details</summary>
Motivation: 针对LLM导师在K-12高风险教育领域可能替代传统学习者建模的误解，研究旨在验证LLM在评估学习者知识演变方面的局限性，特别是准确性、可靠性和时间一致性问题
...
Conclusion: LLM单独使用无法匹配成熟智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架，特别是在K-12高风险教育领域
...
### [210] [Benchmark Success, Clinical Failure: When Reinforcement Learning Optimizes for Benchmarks, Not Patients](https://arxiv.org/abs/2512.23090)
...
TL;DR: ChexReason是一个通过R1风格方法（SFT+GRPO）训练的视觉语言模型，仅使用少量数据和单GPU，在医学影像推理任务中展现了分布内性能提升但跨数据集泛化能力下降的矛盾现象。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型的强化学习在推理任务上取得了进展，但其在资源受限的医学影像应用中的效果尚未充分探索。研究者希望了解在有限计算资源下，强化学习方法对医学影像视觉语言模型性能的影响。

Method: 采用R1风格方法：先进行监督微调（SFT），然后使用GRPO（Group Relative Policy Optimization）进行强化学习优化。仅使用2,000个SFT样本和1,000个RL样本，在单个A100 GPU上训练ChexReason视觉语言模型。

Result: GRPO在分布内数据（CheXpert）上性能提升23%（macro-F1=0.346），但在跨数据集（NIH）上性能下降19%。SFT检查点在优化前在NIH上表现更好，表明教师引导的推理能捕捉更多机构无关特征。结构化推理支架对通用视觉语言模型有益，但对医学预训练模型增益有限。

Conclusion: 强化学习范式本身而非模型规模导致了泛化能力下降。对于需要跨不同人群鲁棒性的临床部署，精心策划的监督微调可能比激进的强化学习更优，因为SFT能更好地捕捉机构无关特征。
...
### [211] [InSPO: Unlocking Intrinsic Self-Reflection for LLM Preference Optimization](https://arxiv.org/abs/2512.23126)
...
Method: 提出Intrinsic Self-reflective Preference Optimization (q)方法，推导出在上下文和替代响应条件下全局最优的策略。该方法作为即插即用增强，无需架构更改或推理开销，保证对标量化和参考选择的不变性。

Result: 实验证明在胜率和长度控制指标上获得一致改进，验证了解锁自反思能力能产生更鲁棒、更符合人类对齐的LLM。

Conclusion: q方法优于DPO/RLHF，通过利用成对数据中的比较信息和模型内在自反思能力，解决了DPO的根本限制，产生了更鲁棒、更符合人类对齐的语言模型。
...
### [212] [Why We Need a New Framework for Emotional Intelligence in AI](https://arxiv.org/abs/2512.23163)
*Max Parks,Kheli Atluru,Meera Vinod,Mike Kuniavsky,Jud Brewer,Sean White,Sarah Adler,Wendy Ju*

Main category: cs.AI

TL;DR: 本文认为当前评估人工智能系统情感智能的框架需要改进，因为它们未能充分全面地衡量AI相关的EI各个方面。人类EI包含AI缺乏的现象学成分，但AI仍能执行情感感知、解释、响应和适应等任务。作者通过回顾情感理论、批判现有评估框架，并提出改进方案。


<details>
  <summary>Details</summary>
Motivation: 当前评估人工智能系统情感智能的框架存在不足，未能充分考虑哪些EI方面对AI相关、哪些不相关。人类EI包含现象学体验成分，这是AI系统所缺乏的，但AI仍能在情感感知、解释、响应和跨文化适应等方面展现能力。现有评估框架往往缺乏坚实的情感理论基础，需要更精准地评估AI系统的EI能力。

Method: 1. 回顾不同情感理论和一般EI理论，评估每种理论对人工系统的适用性；2. 批判性评估现有基准框架，根据第一部分建立的EI理论分析每个框架的不足之处；3. 提出改进评估策略的方案，避免在AI系统EI评估中出现这些缺陷。

Result: 通过理论分析发现：人类EI的现象学成分对AI不相关，但情感感知、解释、适当响应和跨文化适应等能力对AI评估至关重要。现有基准框架存在理论基础薄弱、评估维度不全面等问题。提出了改进评估策略的具体方向。

Conclusion: 需要重新构建AI系统情感智能的评估框架，区分人类特有现象学成分与AI可实现的认知功能。改进的评估策略应基于更坚实的情感理论基础，全面覆盖AI相关的EI能力维度，特别是跨文化适应等实际应用场景。
...
### [213] [From Model Choice to Model Belief: Establishing a New Measure for LLM-Based Research](https://arxiv.org/abs/2512.23184)
*Hongshen Sun,Juanjuan Zhang*

Main category: cs.AI

TL;DR: 论文提出"模型信念"概念，利用LLM的token级概率分布来提取更多信息，相比传统的"模型选择"方法具有更高的统计效率和更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 当前使用LLM模拟人类行为时，通常将LLM的输出（"模型选择"）视为单个数据点，这种做法未能充分利用LLM固有的概率特性。传统方法效率低下，信息利用不足。

Method: 提出并形式化"模型信念"概念，该度量从LLM的token级概率推导而来，能在单次生成运行中捕捉模型对选择替代方案的信念分布。证明模型信念与模型选择的均值渐近等价，但具有更高的统计效率。

Result: 在需求估计研究中，模型信念在有限运行次数下能更好地解释和预测真实模型选择，将计算需求降低约20倍以达到足够准确的估计。模型信念形成更高效的估计器，具有更低的方差和更快的收敛速度。

Conclusion: 模型信念应作为从LLM生成数据中提取更多信息的默认度量方法，相比传统模型选择方法具有显著优势，能更有效地利用LLM的概率特性。
...
### [214] [Agentic Physical AI toward a Domain-Specific Foundation Model for Nuclear Reactor Control](https://arxiv.org/abs/2512.23292)
*Yoonpyo Lee,Kazuma Kobayashi,Sai Puppala,Sajedul Talukder,Seid Koric,Souvik Chakraborty,Syed Bahauddin Alam*

Main category: cs.AI

TL;DR: 该论文提出了一种新的物理AI范式：基于物理验证驱动的Agentic Physical AI，通过训练小型语言模型在合成反应器控制场景中，实现了从高方差模仿到稳定执行的相变，超越了传统感知中心架构的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前通用基础模型在物理系统控制方面存在根本性障碍，即使前沿视觉语言模型在基本定量物理任务上的准确率也只有50-53%，表现为近似猜测器，保持语义合理性但违反物理约束。这种输入不忠实性不是缩放缺陷，而是结构限制。感知中心架构优化参数空间模仿，而安全关键控制需要执行动作的结果空间保证。

Method: 提出Agentic Physical AI范式，训练360M参数的紧凑语言模型，使用物理验证而非感知推理驱动策略优化。在合成反应器控制场景上训练，数据集规模从10^3扩展到10^5个示例，诱导出通用模型中不存在的尖锐相变。

Result: 小规模系统表现出高方差模仿和灾难性尾部风险，而大规模模型经历超过500倍减少的方差崩溃，稳定执行级行为。尽管平衡暴露于四种驱动家族，模型自主拒绝约70%的训练分布，并将95%的运行时执行集中在单一策略上。学习到的表示无需架构修改即可跨不同物理和连续输入模态迁移。

Conclusion: Agentic Physical AI提供了一条不同于通用基础模型的领域特定基础模型路径，通过物理验证驱动的策略优化，实现了从感知模仿到执行保证的范式转变，为安全关键物理系统控制提供了新的解决方案。
...
### [215] [On Conformant Planning and Model-Checking of $\exists^*\forall^*$ Hyperproperties](https://arxiv.org/abs/2512.23324)
*Raven Beutner,Bernd Finkbeiner*

Main category: cs.AI

TL;DR: 论文探讨了规划与验证领域中两个问题的联系：一致性规划与超属性模型检测，证明了两者之间的双向可约简关系。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索规划领域（一致性规划）与验证领域（超属性模型检测）之间的理论联系，这两个问题在形式化方法社区中通常被视为独立的研究方向。

Method: 采用双向约简方法：1）将超属性模型检测实例高效约简为一致性规划实例；2）证明每个一致性规划问题本身就是一个超属性模型检测任务。建立了两种问题之间的形式化对应关系。

Result: 证明了超属性模型检测与一致性规划之间的紧密联系：1）超属性模型检测实例可以高效、完备地约简为一致性规划实例；2）一致性规划问题本身就是一种超属性模型检测任务。建立了两种问题之间的双向可约简性。

Conclusion: 结论表明一致性规划与超属性模型检测在本质上密切相关，这一发现为两个领域提供了新的理论视角，并可能促进跨领域工具和技术的相互借鉴与应用。
...
### [216] [CubeBench: Diagnosing Interactive, Long-Horizon Spatial Reasoning Under Partial Observations](https://arxiv.org/abs/2512.23328)
...
Method: 引入CubeBench基准测试，以魔方为中心构建生成式评估框架。采用三层诊断框架：1）使用完整符号信息的基础状态跟踪；2）逐步增加复杂性；3）仅使用部分视觉数据的主动探索。通过为LLM提供外部求解器工具来隔离认知瓶颈。

Result: 在领先的LLM上进行实验，结果显示在所有长时程任务上的通过率均为0.00%，暴露了长期规划的根本性失败。通过分析失败模式，揭示了现有模型的局限性。

Conclusion: CubeBench基准测试有效暴露了LLM智能体在物理世界部署中的核心认知缺陷，特别是长时程规划能力不足。提出的诊断框架为开发更物理基础的智能体提供了关键见解和指导方向。
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型对高性能计算内核需求激增，但LLM、多模态架构、推荐系统等复杂性增加，加上稀疏化、量化等技术，以及硬件频繁更新和多样化架构，使得手动优化无法满足需求，成为AI系统发展的关键瓶颈。
...
Conclusion: AKG kernel agent通过自动化内核开发流程，有效解决了AI系统发展中计算内核优化的瓶颈问题，为应对日益复杂的AI模型和多样化硬件环境提供了可行的解决方案。
...
TL;DR: HiR是一种基于后见之明指令重放的样本高效强化学习框架，通过选择-重写策略将失败尝试重放为成功样本，解决复杂指令跟随任务中稀疏奖励问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习在大型语言模型指令对齐中面临挑战：初始模型能力有限，难以生成满足所有约束的高质量响应，导致稀疏或难以区分的奖励信号，阻碍学习效率。

Method: 提出Hindsight instruction Replay (HiR)框架，采用选择-重写策略：1) 从失败尝试中选择已满足部分约束的样本；2) 基于后见之明将其重写为成功样本；3) 在原始样本和重放样本上进行强化学习，理论框架为指令级和响应级的双重偏好学习，仅需二元奖励信号。

Result: 在多种指令跟随任务上取得有前景的结果，同时需要更少的计算资源。代码和数据集已开源。

Conclusion: HiR通过后见之明重放机制有效解决了复杂指令跟随任务中的稀疏奖励问题，提高了强化学习的样本效率，为LLM对齐提供了更高效的训练框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育问题生成方面潜力巨大，但存在"人工蜂群思维"效应，即同一模型内生成相似响应，不同模型间产生同质化输出。这导致学生接触过于相似和重复的LLM生成问题，损害思维多样性。

Method: 提出CreativeDC两阶段提示方法，灵感来自Wallas的创造力理论和Guilford的发散-收敛思维框架。该方法将LLM推理显式地支架化为不同阶段：创造性探索阶段和约束满足阶段，通过解耦这两个过程使LLM在确定最终问题前探索更广泛的想法空间。

Result: 使用全面评估指标（多样性、新颖性和实用性）评估CreativeDC的创造性问题生成能力。结果显示，CreativeDC相比基线方法在保持高实用性的同时，实现了显著更高的多样性和新颖性。扩展分析表明，CreativeDC在采样更多问题时生成的有效不同问题数量更大，增长速度比基线方法更快。

Conclusion: CreativeDC通过两阶段提示方法有效缓解了LLM的"人工蜂群思维"效应，为教育问题生成提供了更具创造性和多样性的解决方案，有助于提高学习材料的质量和思维多样性。
...
TL;DR: I-PERI：一种联邦因果发现算法，用于处理客户端未知干预下的异构数据，通过恢复客户端图并集的CPDAG，利用干预诱导的结构差异定向边，得到更紧的Φ-Markov等价类。


<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法通常假设所有客户端共享相同的因果模型，这在实践中不现实。客户端特定的策略或协议（如医院间）会引入未知的异构干预，需要新的方法来处理这种实际场景。

Method: 提出I-PERI算法：1) 恢复所有客户端图并集的CPDAG；2) 利用客户端间干预诱导的结构差异定向额外边；3) 得到更紧的Φ-Markov等价类，用Φ-CPDAG表示。算法具有收敛性和隐私保护的理论保证。

Result: 在合成数据上的实证评估表明I-PERI算法有效。理论分析证明了算法的收敛性和隐私保护特性，能够处理客户端未知干预下的联邦因果发现问题。

Conclusion: I-PERI是首个处理客户端未知干预的联邦因果发现算法，通过利用干预诱导的结构差异获得更紧的等价类，为实际异构联邦环境中的因果发现提供了有效解决方案。
...
TL;DR: Web World Model (WWM) 是一种混合方法，将传统Web框架的逻辑一致性与生成式世界模型的开放性相结合，使用Web代码定义世界状态和物理规则，LLM生成上下文和决策。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两极分化：传统Web框架提供可靠但固定的上下文，而完全生成式世界模型追求无限环境但牺牲了可控性和工程实用性。需要一种中间方案来平衡逻辑一致性和开放性。

Method: 提出Web World Model (WWM)，将世界状态和"物理规则"用普通Web代码实现以确保逻辑一致性，同时让大语言模型在结构化潜在状态上生成上下文、叙事和高层决策。构建了基于现实Web技术栈的WWM套件，包括基于真实地理的无限旅行地图、虚构星系探索器、Web规模的百科全书和叙事世界等。

Result: 建立了多个WWM系统，识别出实用设计原则：将代码定义的规则与模型驱动的想象力分离，将潜在状态表示为类型化Web接口，使用确定性生成实现无限但有结构的探索。结果表明Web技术栈本身可以作为世界模型的可扩展基板。

Conclusion: Web World Model 为语言智能体提供了可控且开放的环境，Web技术栈可以作为世界模型的可扩展基板，平衡了逻辑一致性和生成灵活性。
