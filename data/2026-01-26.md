...
TL;DR: 提出一种用于大规模房间尺度环境3D重建重照明的方法，通过将视频到视频重照明扩散模型的输出蒸馏到3D重建中，避免了困难的逆渲染问题，实现了复杂真实场景的可控3D重照明。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景重照明方法通常需要解决欠定或病态的逆渲染问题，难以在复杂真实场景中产生高质量结果。虽然最近使用生成图像和视频扩散模型进行重照明的进展很有前景，但这些技术要么局限于2D图像/视频重照明，要么仅限于单个物体的3D重照明。

Method: 通过将视频到视频重照明扩散模型的输出蒸馏到3D重建中，避免了直接解决困难的逆渲染问题。该方法能够实现房间尺度场景的可控3D重照明，处理复杂真实世界场景。

Result: 在合成和真实世界数据集上验证了该方法能够忠实地在新光照条件下渲染场景的新视角。系统能够灵活地对复杂真实世界场景的3D重建进行重照明。

Conclusion: 该方法通过将视频重照明扩散模型蒸馏到3D重建中，成功实现了房间尺度场景的高质量可控3D重照明，避免了传统逆渲染方法的局限性，为复杂真实场景的3D重照明提供了有效解决方案。
...
Conclusion: FeTTL为缓解联邦学习中的分布偏移提供了原则性且可扩展的解决方案，支持在多机构真实环境中部署鲁棒模型。
...
TL;DR: 该研究评估多模态模型在足球视频中识别重要子事件的能力，发现现有模型性能接近随机水平，主要问题在于依赖单一主导模态且无法有效整合多源信息。


<details>
  <summary>Details</summary>
Motivation: 基础模型在涉及时序多模态事件的语言生成任务中广泛应用，但模型识别视频中重要子事件的能力尚未得到充分研究，而这是叙述或总结多模态事件的基本前提。

Method: 研究聚焦足球比赛，利用人类对比赛集锦的偏好隐含重要性标注，构建无需额外标注成本的新数据集。使用该数据集比较多个先进多模态模型，并通过超越标准评估指标的分析揭示模型行为模式。

Result: 实验显示现有多模态模型性能接近随机水平，分析发现模型倾向于依赖单一主导模态，且在整合多源信息方面效果不佳，无法有效合成必要信息。

Conclusion: 研究强调需要能够处理多模态数据样本级异质性的模块化架构，以及能够最大化跨模态协同的补充训练程序，以提升模型识别重要子事件的能力。
...
TL;DR: 该论文提出"视角令牌"方法，通过嵌入身体关键点线索或抽象旋转表示来增强多模态语言模型的空间推理能力，解决现有模型的自我中心偏差问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态语言模型在语义视觉-语言任务上表现良好，但在需要采用其他智能体视觉视角的空间推理任务上失败。这些错误反映了持续的自我中心偏差，并引发了对当前模型是否支持他中心推理的疑问。受人类空间认知启发，研究者希望为模型提供更类似人类的视角采择能力。

Method: 引入视角令牌——专门编码方向信息的嵌入表示，通过两种方式实现：(1) 基于具身身体关键点线索；(2) 基于支持心理旋转的抽象表示。将这些令牌集成到LLaVA-1.5-13B模型中，并在第2级视觉视角采择任务上进行评估。

Result: 在合成和自然基准测试（Isle Bricks V2、COCO、3DSRBench）中，视角令牌显著提高了准确性，其中基于旋转的令牌能够泛化到非人类参考智能体。表征分析表明，微调增强了基础模型中已有的潜在方向敏感性，表明多模态语言模型包含他中心推理的前兆但缺乏适当的内部结构。

Conclusion: 将认知基础的空间结构直接嵌入令牌空间，为视角采择和更类似人类的空间推理提供了一种轻量级、模型无关的机制。这表明通过适当的架构修改，可以解锁多模态语言模型中已有的空间推理潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型（MLLM）的指代表达式分割方法存在两个主要限制：1）MLLM生成的粗边界框导致冗余或非判别性的点提示；2）依赖文本坐标推理不可靠，无法区分目标与视觉相似干扰物。
...
Conclusion: EBD-VBR框架通过熵基点发现和视觉基推理，能够生成准确且语义基础的掩码分割，使用最少的提示，在指代表达式分割任务中表现出色。
...
TL;DR: 提出DCCS-Det红外小目标检测器，通过双流显著性增强模块和潜在感知语义提取聚合模块解决现有方法在局部-全局特征联合建模不足、特征冗余和语义稀释等问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有红外小目标检测方法存在两个主要问题：1）局部与全局特征联合建模不足，影响目标-背景区分能力；2）特征冗余和语义稀释，降低目标表示质量。这些限制导致在复杂背景下检测小、低对比度目标时性能受限。
...
Conclusion: DCCS-Det通过创新的双流显著性增强和潜在感知语义提取聚合模块，有效解决了红外小目标检测中的特征建模和表示质量问题，在精度和效率方面均表现出色，为复杂背景下的红外小目标检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在极端面部姿态下质量下降严重，几何特征方法增加计算成本，扩散模型无法实时处理，需要一种既保持高质量又实时的方法

Method: 利用开源视觉语言模型和CLIP图像/文本嵌入，应用新颖的视觉和文本语义对比损失，增强身份表示和属性保留

Result: 在FF++、MPIE、LPFF数据集上超越现有方法，在姿态挑战性案例中表现优异，同时保持实时性能

Conclusion: AlphaFace通过视觉语言模型和对比损失实现了高质量、实时的人脸交换，特别在极端姿态下表现突出，代码已开源
...
TL;DR: 该论文提出了Emotion-LLaMAv2框架和MMEVerse基准，用于解决多模态情感识别与推理的挑战，通过端到端多视图编码器、Conv Attention预融合模块和感知到认知课程指令调优方案，构建了大规模标准化评估体系。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在通用视觉-语言任务上表现出色，但在情感推理方面能力有限。当前领域缺乏大规模高质量描述性情感标注数据集，也缺乏标准化评估基准。之前的Emotion-LLaMA框架受限于显式人脸检测器、隐式融合策略和低质量训练数据。

Method: 1. 端到端多视图编码器：消除外部人脸检测，通过丰富的空间和时间多视图token捕捉细微情感线索；2. Conv Attention预融合模块：在LLM主干外部实现局部和全局多模态特征交互；3. 感知到认知课程指令调优方案：在LLaMA2主干内统一情感识别和自由形式情感推理；4. MMEVerse基准：整合12个公开情感数据集，通过多智能体管道重新标注，形成统一多模态指令格式。

Result: 构建了包含130k训练片段和36k测试片段的大规模数据集，涵盖18个评估基准。建立了端到端情感识别与推理管道和标准化评估设置。

Conclusion: Emotion-LLaMAv2和MMEVerse基准为多模态情感识别与推理领域提供了完整的解决方案，解决了数据稀缺和评估标准化问题，推动了情感计算和人机交互的发展。
...
TL;DR: VISTA-PATH是一个交互式、类别感知的病理学分割基础模型，通过结合视觉上下文、语义组织描述和专家空间提示，实现精确的多类别分割，并支持人机协同细化，显著提升病理图像分析质量。


<details>
  <summary>Details</summary>
Motivation: 现有分割基础模型虽然通过大规模预训练提升了泛化能力，但将分割视为静态视觉预测任务，与病理学需求不匹配。需要开发能够处理异质结构、整合专家反馈、产生具有临床意义分割结果的专业病理学模型。

Method: 提出VISTA-PATH模型，联合视觉上下文、语义组织描述和可选专家空间提示进行分割。构建VISTA-PATH Data数据集，包含160万+图像-掩码-文本三元组，涵盖9个器官和93个组织类别。支持动态人机协同细化，将稀疏的patch级边界框标注反馈传播到全玻片分割。

Result: 在多个保留测试集和外部基准测试中，VISTA-PATH持续优于现有分割基础模型。模型产生的高保真、类别感知分割显著改善了组织微环境分析，提出的肿瘤相互作用评分（TIS）与患者生存率显示出强且显著的关联。

Conclusion: VISTA-PATH将病理图像分割从静态预测提升为交互式、临床基础的表征，为数字病理学建立了新的基础模型范式，其分割结果可直接用于临床解释和下游分析。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在物理原理理解方面仍达不到人类水平，现有物理推理数据集要么依赖高成本的真实视频标注，要么受限于合成模拟的真实性和多样性不足。游戏视频中的视觉异常（违反预设物理定律的glitches）提供了丰富且可扩展的监督来源。

Method: 提出新范式：利用游戏视频中的glitches作为物理世界理解的监督源。构建PhysGame数据集（140,057个glitch中心的问答对，涵盖5个物理领域和16个细粒度类别），使用游戏元数据（标题和描述）引导高质量QA生成。同时构建GameBench基准（880个专家标注的glitch识别游戏视频）用于评估物理推理能力。

Result: PhysGame显著提升：1) Game2Real可迁移性：在PhysBench上提升Qwen2.5VL真实世界物理推理性能2.5%；2) Game2General可迁移性：在MVBench基准上获得1.9%增益；3) GameBench上PhysGame调优模型获得3.7%绝对提升，显示检测物理不合理性的鲁棒性增强。

Conclusion: 从游戏异常中学习为推进多模态智能的物理世界理解提供了一条可扩展且有效的途径，游戏视频中的glitches可作为丰富且成本效益高的监督源。
...
Method: 提出SALAD方法，在稀疏注意力旁并行引入轻量级线性注意力分支，通过输入相关的门控机制精细平衡两个分支，实现高效稀疏化。微调过程仅需2000个视频样本和1600训练步骤（批量大小8）。

Result: 达到90%稀疏度，1.72倍推理加速，同时保持与全注意力基线相当的生成质量。微调过程高效，仅需少量数据和训练步骤。

Conclusion: SALAD在稀疏注意力与线性注意力之间取得了良好平衡，在保持生成质量的同时显著提升推理效率，为视频生成中的注意力优化提供了高效解决方案。
...
TL;DR: TangramPuzzle：基于七巧板的几何基准测试，用于评估多模态大语言模型的组合空间推理能力，通过符号化几何框架解决视觉近似模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在组合空间推理方面能力不足，现有基准测试任务简单、依赖语义近似或粗略相对定位，且评估指标有限、缺乏严格的数学公式化。
...
Result: 对先进开源和专有模型进行广泛评估，发现MLLM倾向于优先匹配目标轮廓而忽略几何约束，导致组件变形或扭曲。

Conclusion: TangramPuzzle基准测试填补了MLLM组合空间推理评估的空白，揭示了模型在几何精确性方面的局限性，为未来改进提供了方向。
...
TL;DR: OnlineSI：一个通过视频流持续提升空间理解能力的MLLM框架，采用有限空间记忆机制避免计算量随输入累积而增长，结合3D点云与语义信息，并引入模糊F1分数评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法大多忽视在动态变化世界中持续工作的能力，且缺乏在真实世界具身系统中部署的可能性。需要让MLLM具备持续的空间理解和推理能力。

Method: 提出OnlineSI框架：1) 维护有限空间记忆保留过去观察，确保推理计算量不随输入累积而增加；2) 整合3D点云信息与语义信息，帮助MLLM更好地定位和识别场景中的物体。

Result: 在两个代表性数据集上测试，实验证明了方法的有效性。引入模糊F1分数来缓解评估中的模糊性问题。

Conclusion: OnlineSI为真实世界具身系统的发展铺平了道路，使MLLM能够在动态环境中持续提升空间理解能力。
...
TL;DR: 提出半监督分层开集分类框架，通过伪标签技术利用未标注数据提升分类性能，在iNaturalist19基准上仅需每类20个标注样本即可达到全监督性能


<details>
  <summary>Details</summary>
Motivation: 扩展分层开集分类到半监督场景，利用大规模未标注数据集（包含已知和未知类别）来提升分层开集性能，解决实际应用中标注数据稀缺的问题

Method: 基于伪标签的师生框架，引入两个关键组件：1)子树伪标签——在未知数据存在时提供可靠监督；2)年龄门控机制——缓解伪标签的过度自信问题

Result: 在iNaturalist19基准测试中，该方法优于自监督预训练+监督适应的方案，且仅使用每类20个标注样本时就能达到全监督模型的性能水平

Conclusion: 提出的半监督分层开集分类框架能有效利用未标注数据，显著减少对标注数据的依赖，在资源受限的实际应用中具有重要价值
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法要么关注局部斑块特征提取，要么追求整体图像处理，导致跨时间特征匹配偏差，并对辐射和几何噪声敏感。需要解决多时相特征对齐偏差和噪声干扰问题。

Method: 提出HA2F框架，包含两个核心模块：动态层次特征校准模块（DHFCM）通过感知特征选择动态融合相邻层次特征，抑制无关差异；噪声自适应特征细化模块（NAFRM）利用双重特征选择机制突出变化敏感区域并生成空间掩码，抑制无关区域或阴影干扰。

Result: 在LEVIR-CD、WHU-CD和SYSU-CD数据集上进行了广泛实验，HA2F在精度指标和计算效率方面均超过现有对比方法，达到最先进性能。消融实验验证了DHFCM和NAFRM的有效性。

Conclusion: HA2F框架通过层次自适应聚合有效解决了遥感变化检测中的特征对齐偏差和噪声敏感性问题，为环境监测、灾害评估和气候变化研究等应用提供了可靠支持。
...
TL;DR: 提出了一种新颖的组合视频检索框架，利用视觉语言模型的表征能力，通过X-Aligner交叉注意力模块渐进式融合视觉和文本查询，并引入视觉查询的标题作为额外输入，在Webvid-CoVR数据集上实现了63.93%的Recall@1，并在CIR任务上展示了强大的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有组合视频检索框架通常将多模态输入在单阶段融合，仅获得相对于基线的边际改进。需要更有效的框架来充分利用视觉语言模型的表征能力，实现更好的多模态查询融合和对齐。

Method: 提出基于BLIP家族架构的新框架，包含新颖的X-Aligner交叉注意力模块，该模块通过交叉注意力层渐进式融合视觉和文本输入，并将多模态表示与目标视频对齐。引入视觉查询的标题作为额外输入增强表示。采用两阶段训练策略：第一阶段仅训练新模块，第二阶段微调文本查询编码器。

Result: 在Webvid-CoVR-Test上获得63.93%的Recall@1，达到最先进性能。在CIRCO和Fashion-IQ等组合图像检索数据集上的零样本评估展示了强大的泛化能力。

Conclusion: 提出的框架通过渐进式多模态融合和两阶段训练策略，显著提升了组合视频检索性能，同时展示了在相关任务上的良好零样本泛化能力，验证了方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 无人机应用中，小目标检测面临尺度不平衡和边缘模糊的挑战，传统方法难以准确捕捉小目标，需要专门针对边界和位置信息进行优化。

Method: 提出BPIM框架，包含PIG模块获取位置信息、BIG模块提取对象边缘、CSF模块逐步组装浅层图像特征、TFF模块渐进融合位置和边界信息、AWF模块灵活融合深层语义特征，通过注意力机制和跨尺度特征融合策略整合边界、位置和尺度信息。
...
TL;DR: SCHIGAND是一个集成StyleCLIP、HyperStyle、InterfaceGAN和Diffusion模型的合成人脸生成流水线，旨在解决隐私法规、数据稀缺和伦理问题，为生物识别系统提供高质量、多样化的合成人脸数据集。


<details>
  <summary>Details</summary>
Motivation: 生物识别系统训练和测试需要多样化、高质量的人脸数据集，但面临隐私法规、数据稀缺和伦理问题的挑战。现有生成模型在真实性、多样性和身份保持之间难以平衡。
...
<details>
  <summary>Details</summary>
Motivation: 基于潜在扩散模型(LDMs)的图像编辑方法在文本提示驱动的多样化编辑任务中表现出色，但在保持像素级边缘结构方面存在挑战，这对于照片级真实感风格迁移或图像色调调整等任务至关重要。

Method: 提出结构保持损失(SPL)，利用局部线性模型量化结构差异；将SPL直接集成到扩散模型的生成过程中确保结构保真度；采用后处理步骤减轻LDM解码失真；使用掩码策略实现精确编辑定位；添加颜色保持损失保护未编辑区域的色调。

Result: 实验证实SPL显著增强了结构保真度，在基于潜在扩散的图像编辑任务中实现了最先进的性能。

Conclusion: 提出的结构保持损失(SPL)有效解决了潜在扩散模型在图像编辑中的结构保持问题，通过训练免费的方法在生成过程中直接集成结构约束，为高质量图像编辑提供了实用解决方案。
...
TL;DR: ReWeaver：从稀疏多视角RGB图像重建拓扑准确的3D服装和缝纫图案的框架，解决了现有方法无法准确重建服装拓扑和缝纫结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有服装重建方法通常依赖非结构化表示（如3D高斯泼溅），难以准确重建服装拓扑和缝纫结构，导致重建结果不适合高保真物理仿真。需要解决数字化身、虚拟试衣和机器人操作等应用中的sim-to-real差距问题。

Method: 提出ReWeaver框架，从最少四个输入视角预测2D UV空间和3D空间中的接缝、面板及其连接性。构建大规模数据集GCD-TS，包含多视角RGB图像、3D服装几何、纹理人体网格和标注缝纫图案，包含超过100,000个合成样本。

Result: ReWeaver在拓扑准确性、几何对齐和接缝-面板一致性方面持续优于现有方法。预测的接缝和面板与多视角图像精确对齐，产生适合3D感知、高保真物理仿真和机器人操作的结构化2D-3D服装表示。

Conclusion: ReWeaver能够从稀疏多视角RGB图像重建拓扑准确的3D服装和缝纫图案，解决了现有方法的局限性，为3D感知、物理仿真和机器人操作提供了结构化表示。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯文字手写文本识别（HTR）仍落后于拉丁文字HTR，现有数据集存在标签质量问题，需要系统化方法来检测和清理数据错误以提高识别性能。

Method: 提出CER-HV框架：1）基于CER的噪声检测器，使用精心配置的CRNN模型配合早停策略避免过拟合噪声样本；2）人机交互（HITL）步骤验证高排名样本。框架能识别转录、分割、方向和非文本内容等多种错误类型。

Result: 1）CRNN在6个数据集中5个达到SOTA：KHATT（阿拉伯语）8.45% CER，PHTI（普什图语）8.26%，Ajami 10.66%，Muharaf（阿拉伯语）10.11%；2）CER-HV在Muharaf和PHTI数据集分别以90%和80-86%精度检测错误；3）数据清理后评估CER提升0.3-0.6%（较干净数据集）和1.0-1.8%（较嘈杂数据集）。

Conclusion: 数据质量是阿拉伯文字HTR性能提升的关键限制因素，CER-HV框架能有效检测和清理数据集错误，显著提升识别性能，该框架具有通用性可应用于其他文本识别任务。
...
TL;DR: 该论文提出了一种从圆合成孔径声纳数据中提取阴影信息的方法，用于改进目标分析和三维重建，通过子孔径滤波和固定焦点阴影增强技术恢复阴影信息，并应用空间雕刻方法进行三维重建。


<details>
  <summary>Details</summary>
Motivation: 圆合成孔径声纳虽然提供360°方位视角，但在处理过程中会丢失阴影信息，而阴影对于目标识别和形状分析具有重要价值。需要一种方法来从CSAS数据中恢复阴影信息以改进目标分析和三维重建。

Method: 采用子孔径滤波获取沿圆形轨迹不同视角的图像集合，应用固定焦点阴影增强技术获得清晰阴影，提出交互界面供操作员可视化阴影，并应用空间雕刻重建方法从分割的阴影中推断物体的三维形状。

Result: 实验结果证明了阴影在圆合成孔径声纳中对于改进目标分析和三维重建的潜力，验证了所提方法的有效性。

Conclusion: 从圆合成孔径声纳数据中提取阴影信息是可行的，能够显著提升目标识别能力和三维重建质量，为水雷战等应用提供更可靠的目标分析工具。
...
<details>
  <summary>Details</summary>
Motivation: ERCP是诊断和治疗胆胰疾病的关键技术，人工智能可用于自动化诊断，但公共ERCP数据集稀缺限制了这一方法的应用。本研究旨在填补这一空白，提供大型、经过人工标注的数据集。

Method: 收集了1,602名患者的19,018张原始图像和19,317张处理后图像，其中5,519张图像由两名经验丰富的胃肠病学家（每年进行400+例ERCP）手动标注，并由另一名资深专家（20年以上经验）审核，确保数据质量。

Result: 创建了一个包含大量标注图像的数据集，并通过分类实验验证了其有效性和实用性。该数据集为自动ERCP分析和胆胰疾病诊断提供了基准。

Conclusion: 该研究提供了一个高质量、经过专家标注的ERCP数据集，解决了该领域数据稀缺问题，为人工智能在ERCP自动诊断中的应用提供了重要资源，有望推动胆胰疾病诊断的自动化发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有B-rep表示与生成方法依赖图结构表示，将几何与拓扑特征解耦处理，无法应用序列生成框架（如Transformer）。需要一种能统一编码B-rep几何与拓扑的序列表示方法。

Method: 提出BrepARG方法：1）将B-rep编码为三种token：几何token、位置token（表示几何特征）和面索引token（表示拓扑）；2）分层构建整体token序列：先构建几何块（面与边），再进行几何块排序；3）使用仅解码器的Transformer架构，通过因果掩码进行下一token预测，学习整体token序列的分布。

Result: BrepARG在B-rep生成任务上达到了最先进的性能，验证了将B-rep表示为整体token序列的可行性。

Conclusion: BrepARG首次实现了将B-rep几何与拓扑编码为整体token序列，为基于序列的B-rep生成开辟了新方向，证明了自回归架构在B-rep生成任务上的有效性。
...
TL;DR: 本文提出CASP方法，通过CLS令牌注意力引导提示来改进少样本类增量学习，在CLS令牌的查询、键、值投影中引入类共享可训练偏置参数，结合注意力扰动和流形令牌混合策略，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习面临的核心挑战是在极少量样本下快速适应新类别同时缓解灾难性遗忘。现有基于提示的方法在极端少样本增量设置下，模型的迁移和泛化能力变得关键，需要充分利用预训练知识学习可在未来类别间共享的特征表示。

Method: 提出CLS令牌注意力引导提示(CASP)：1)在CLS令牌的查询、键、值投影中引入类共享可训练偏置参数，显式调节自注意力权重；2)设计注意力扰动策略；3)在浅层特征空间执行流形令牌混合，合成潜在新类别特征以提升泛化能力并为后续任务保留表示容量。

Result: 在CUB200、CIFAR100和ImageNet-R数据集上的实验表明，CASP在标准和细粒度FSCIL设置下均优于最先进方法，在增量阶段无需微调且显著减少参数开销。

Conclusion: CASP方法通过CLS令牌注意力引导机制有效解决了少样本类增量学习中的挑战，在保持参数效率的同时实现了更好的性能，为利用预训练知识进行持续学习提供了新思路。
...
TL;DR: 提出了一种用于检测脊柱韧带附着点的新方法，通过形状分割和领域特定规则实现高精度检测，在所有脊柱区域都表现出良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在生物力学建模中，韧带附着点的精确表示对于模拟椎骨间力的真实分布至关重要。现有自动化检测方法要么局限于特定脊柱区域，要么精度不足，需要一种更准确、更通用的解决方案。

Method: 该方法首先对3D椎骨进行基于形状的分割，然后应用领域特定规则来识别不同类型的附着点。这种两阶段方法结合了形状分析和解剖学知识。

Result: 在两个独立的脊柱数据集上进行验证，获得了0.7毫米的平均绝对误差（MAE）和1.1毫米的均方根误差（RMSE），优于现有方法，并在所有脊柱区域都表现出良好的泛化能力。

Conclusion: 该方法为脊柱韧带附着点的自动化检测提供了一种准确且通用的解决方案，能够显著提高脊柱生物力学模型的可靠性，为临床和科研应用提供了有力工具。
...
TL;DR: 提出双分支CNN-LSTM框架，融合视觉特征与眼动信号预测室内空间美学评价，在客观维度达72.2%准确率，主观维度66.8%，优于现有方法。
...
Method: 构建双分支CNN-LSTM框架：一个分支处理视觉特征（从室内设计视频提取），另一个分支处理眼动信号（瞳孔响应和注视数据）。收集224个室内设计视频数据集，包含28名参与者的同步眼动数据和15个美学维度评分。模型融合两种模态特征进行美学评价预测。

Result: 模型在客观维度（如光照）达到72.2%准确率，主观维度（如放松感）达到66.8%准确率，优于现有视频基线方法。消融实验显示：瞳孔响应对客观评估贡献最大，而注视与视觉线索结合能增强主观评价。仅用视觉输入部署时，经眼动训练模型仍保持可比性能。

Conclusion: 眼动追踪作为训练时的特权信息具有重要价值，能够开发更实用的室内设计美学评估工具。多模态融合方法能有效提升美学体验预测性能，特别是主观维度评价。
...
TL;DR: ColorConceptBench：一个评估文本到图像模型颜色-概念关联能力的新基准，通过概率颜色分布分析模型对1281个隐式颜色概念的翻译能力，发现当前模型缺乏对抽象语义的敏感性且难以通过常规干预改进


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在颜色与隐式概念关联方面的能力尚未得到充分探索，现有方法主要关注显式颜色名称或代码，缺乏对抽象颜色语义的系统评估

Method: 引入ColorConceptBench基准，包含1281个隐式颜色概念，基于6369个人工标注构建概率颜色分布，用于系统评估模型对颜色-概念关联的理解能力

Result: 对7个领先T2I模型的评估显示：当前模型对抽象语义缺乏敏感性，且这种局限性对标准干预措施（如模型缩放和引导）具有抵抗力
...
TL;DR: 提出自监督框架GPA-VGGT，通过序列几何约束训练视觉几何基础Transformer，无需真实标签即可提升大规模环境下的定位能力


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的视觉几何框架在相机姿态估计和3D场景理解方面表现良好，但通常依赖真实标签进行训练，难以适应未标记和未见场景。需要开发自监督方法以增强模型在无标签数据下的定位能力。

Method: 1. 将传统的成对关系扩展到序列几何约束进行自监督学习；2. 在每个序列中采样多个源帧并将其几何投影到不同目标帧，提升时间特征一致性；3. 将物理光度一致性和几何约束公式化为联合优化损失，避免硬标签需求；4. 通过该方法训练模型，使局部和全局跨视图注意力层以及相机和深度头能有效捕捉底层多视图几何。

Result: 1. 模型在数百次迭代内收敛；2. 在大规模定位任务中取得显著改进；3. 代码将在GitHub上开源。

Conclusion: 提出的自监督框架GPA-VGGT成功解决了视觉几何基础Transformer对真实标签的依赖问题，通过序列几何约束和联合优化损失实现了在无标签数据上的有效训练，显著提升了大尺度环境下的定位性能。
...
TL;DR: 本研究评估了大型视觉语言模型（VLMs）在手术工具检测任务中的表现，比较了Qwen2.5、LLaVA1.5和InternVL3.5在GraSP机器人手术数据集上的零样本和LoRA微调性能，发现Qwen2.5在检测性能上表现最优。


<details>
  <summary>Details</summary>
Motivation: 当前大多数手术AI系统是单模态的，限制了其对手术工作流程的整体理解能力。需要能够全面建模手术场景相关组件的通用手术AI系统。大型视觉语言模型整合多模态数据处理，为建模手术任务和提供类人场景推理提供了潜力，但系统性的手术应用研究仍然有限。

Method: 评估三种最先进的视觉语言模型（Qwen2.5、LLaVA1.5和InternVL3.5）在GraSP机器人手术数据集上的手术工具检测性能。采用两种配置：零样本学习和参数高效的LoRA微调设置。与开放集检测基线Grounding DINO进行比较。

Result: Qwen2.5在评估的VLMs中，在两种配置下均表现出最优秀的检测性能。与Grounding DINO相比，Qwen2.5展现出更强的零样本泛化能力和可比的微调性能。值得注意的是，Qwen2.5在工具识别方面表现更优，而Grounding DINO在定位方面更强。

Conclusion: 大型视觉语言模型在手术工具检测任务中具有显著潜力，Qwen2.5在多种配置下表现最佳。研究结果支持了VLMs在手术场景理解和推理方面的应用前景，为开发更全面的手术AI系统提供了实证依据。
...
<details>
  <summary>Details</summary>
Motivation: 当前自回归长视频生成模型存在错误累积和长期连贯性丢失问题，虽然引入了attention sink frames来缓解性能衰减，但导致了sink-collapse这一关键故障模式：生成内容反复回退到sink帧，造成场景重置和循环运动模式

Method: 提出了一种轻量级、无需训练的方法，通过引入multi-head RoPE jitter来打破多头注意力同质化，缓解长期崩溃。该方法基于对RoPE周期性结构与当前生成模型中多头注意力机制之间固有冲突的分析

Result: 该方法成功缓解了sink-collapse问题，同时保持了生成质量。实现了实时、流式、无限长度的视频生成，质量衰减很小。生成了长达12小时的连续视频，这是目前公开演示的最长流式视频生成结果之一

Conclusion: 通过分析RoPE周期性结构与多头注意力机制的冲突，提出了有效的训练免费解决方案，显著提升了长视频生成的稳定性和连贯性，实现了前所未有的长时间视频生成能力
...
TL;DR: 该论文提出了一种使用奖励信号引导生成过程的视频生成方法，替代了依赖教师模型的传统自回归方法，在保持高视觉保真度和时间一致性的同时简化了训练过程。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成工作大多基于双向架构，而将其改编为自回归变体通常严重依赖教师模型，这限制了性能，特别是在缺乏强大自回归教师的情况下，导致输出质量通常落后于双向模型。

Method: 采用奖励信号引导生成过程的替代方法，通过奖励信号指导模型，简化训练同时保持高视觉保真度和时间一致性。

Result: 在标准基准测试中，该方法与现有自回归模型性能相当，在某些情况下甚至超越了类似规模的双向模型，避免了教师架构带来的约束。例如在VBench上获得84.92总分，与需要大量异构蒸馏的最先进自回归方法（84.31分）相当。

Conclusion: 使用奖励信号引导生成过程的方法能够实现更高效和可扩展的自回归生成，在简化训练的同时保持高质量输出，为视频生成提供了一种有前景的替代方案。
...
TL;DR: 提出了一种域不变混合域半监督分割框架，通过跨域复制粘贴机制增强数据多样性，使用聚类最大均值差异块对齐特征，在少量标注和多个未知域差异下实现鲁棒分割。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中深度学习依赖大规模专家标注和一致数据分布，但实际中标注稀缺且数据来自多扫描仪/中心，形成具有未知域标签和严重域差异的混合域设置。现有半监督或域适应方法通常假设单一域偏移或可访问显式域索引，这在真实部署中很少成立。
...
TL;DR: VisGym是一个包含17个环境的评估框架，用于测试和训练视觉语言模型在长期多步视觉交互任务中的表现，揭示了当前前沿模型在交互设置中的显著局限性。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型在多步视觉交互中的表现尚未得到充分研究，特别是在如何整合感知、记忆和行动以实现长期目标方面存在知识空白。需要系统评估框架来揭示模型在交互环境中的具体局限性和改进方向。

Method: 开发了VisGym评估套件，包含17个环境，涵盖符号谜题、真实图像理解、导航和操作任务。提供难度、输入表示、规划视野和反馈的灵活控制。构建多步求解器生成结构化演示数据，用于监督微调。

Result: 所有前沿模型在交互设置中表现不佳：简单配置成功率46.6%，困难配置仅26.0%。模型难以有效利用长上下文，无界历史窗口表现比截断窗口更差。文本符号任务在视觉化后难度显著增加。明确的目标观察、文本反馈和探索性演示在部分可观测或未知动态环境中能带来一致改进。

Conclusion: VisGym揭示了当前视觉语言模型在多步视觉决策中的具体失败模式，并指出了通过目标观察、反馈机制和监督微调等途径改进模型交互能力的明确方向。该框架为未来研究提供了系统评估工具。
...
TL;DR: SyncLight是首个能够在静态场景的多个未标定视图间实现一致、参数化重光照的方法，通过单次推理即可完成整个图像集的高保真重光照。


<details>
  <summary>Details</summary>
Motivation: 现有生成式单视图重光照方法在多视图应用中难以保持严格的光照一致性，而多相机广播、立体电影和虚拟制作等应用需要跨多个视图的一致重光照效果。

Method: 采用基于潜在桥匹配公式训练的多视图扩散变换器，利用大规模混合数据集（合成环境和真实世界多视图捕捉），通过单一参考编辑控制整个多视图场景的光照强度和颜色。

Result: SyncLight能够零样本泛化到任意数量的视点，无需相机姿态信息即可在所有视图间传播光照变化，实现了单次推理步骤完成整个图像集的高保真重光照。

Conclusion: SyncLight为多视图捕捉系统提供了实用的重光照工作流程，解决了多视图应用中光照一致性的关键挑战，推动了虚拟制作和沉浸式媒体制作的发展。
...
TL;DR: AnyView是一个基于扩散模型的视频生成框架，专注于动态视图合成，能够在极端的动态场景中从任意视角生成时空一致的视频。


<details>
  <summary>Details</summary>
Motivation: 现代生成视频模型在产生高质量输出方面表现出色，但在高度动态的真实世界环境中难以保持多视角和时空一致性。现有方法通常需要视角间有显著重叠，限制了在极端动态场景中的应用。

Method: 利用多级监督数据源（单目2D、多视角静态3D、多视角动态4D数据集）训练通用的时空隐式表示。采用扩散模型框架，最小化归纳偏置和几何假设，实现零样本从任意相机位置和轨迹生成新视频。

Result: 在标准基准测试中取得与当前最先进方法竞争的结果。在提出的AnyViewBench（针对极端动态视图合成的新基准）上，大多数基线方法性能急剧下降，而AnyView仍能生成逼真、合理且时空一致的视频。

Conclusion: AnyView展示了在极端动态场景中从任意视角生成时空一致视频的能力，为动态视图合成提供了有效的解决方案，特别是在视角重叠有限的情况下仍能保持良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作将代码混合简化为翻译或可转换性问题，难以评估模型切换行为是否上下文适当且符合人类惯例。需要建立能够评估真实社区语境中代码混合能力的基准。

Method: 构建ChiEngMixBench基准，采用通用构建流程支持跨领域和双语对的可扩展数据集开发。将代码混合定义为认知对齐问题，通过自发性和自然性两个互补信号进行评估。

Result: 实验评估表明，提出的指标能够系统地区分不同模型的代码混合性能。此外，发现了与矩阵语言框架理论一致的隐式涌现术语分层策略，表明多语言大语言模型与人类沟通之间存在结构化认知对齐。
...
Method: 从Kangaroo Math Competition（全球最大的数学竞赛，每年有600多万18岁以下参与者）中提取1,747道独特的选择题，按年级难度组织，翻译成108种文化多样语言，部分题目包含解题必需的图表。构建M3Kang数据集，并对闭源和开源的最先进模型进行广泛基准测试。

Result: 模型在基础数学和基于图表的推理方面仍有困难，性能随语言存在性和模型规模扩展，但不随年级水平提升。多语言技术可有效扩展到多模态设置，相比基线方法有显著改进。研究还纳入了68,000多名学生的表现数据，实现与人类表现的直接比较。

Conclusion: M3Kang填补了多语言多模态数学推理评估的空白，揭示了当前模型的局限性，展示了多语言技术在多模态环境中的有效性。数据集、框架和代码库已开源，包括仅英语子集M2Kang。
...
TL;DR: 提出一种离线响应式知识蒸馏方法，在有限硬件资源下开发高精度专业助手，验证了数据质量比数量更重要的LIMA假设


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理领域特定或机构知识时容易出现幻觉，特别是在预训练数据中未包含的知识。需要在有限硬件资源下开发高精度的专业助手。

Method: 采用离线响应式知识蒸馏方法，评估三种数据策略：通用领域适应（15,000行）、非结构化知识注入（2,000行）和上下文感知合成数据集（500行）。使用Unsloth库优化Qwen-2.5-7B学生模型，将GPU内存需求从40GB降至16GB。

Result: 实验结果显示，较大的非结构化数据集存在持续幻觉问题，而500行的上下文感知数据集达到96.7%的准确率，并具备稳健的拒绝能力。验证了LIMA假设，表明在低资源设置下，数据质量和结构对齐比数量更重要。

Conclusion: 数据质量和结构对齐对于领域适应至关重要，特别是在资源受限的环境中。上下文感知的小规模数据集能够实现高精度和稳健的拒绝能力，为开发专业助手提供了高效路径。
...
TL;DR: NFDM扩展了连续扩散模型到离散状态空间的语言生成，显著缩小了与自回归模型的似然差距，同时保持了与先前潜在扩散模型相当的样本质量


<details>
  <summary>Details</summary>
Motivation: 改进语言生成模型的采样速度和连贯性，使连续扩散模型能够直接应用于离散状态空间的语言建模

Method: 提出神经流扩散模型（NFDM），学习数据驱动的多元前向过程，确保前向过程和生成轨迹适合语言建模
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在多智能体环境中的战略决策能力有限，特别是在需要长期协调和谈判的对话中。现有研究主要关注单轮决策任务，缺乏对通过对话优化长期目标的探索。

Method: 引入GameTalk框架，通过微调方法（GRPO、DPO、STaR）训练LLMs，将依赖于整个交互过程的奖励信号纳入训练，使模型能够在多轮对话中优化全局目标。

Result: GameTalk显著优于未经训练的模型，特别是在奖励塑造条件下，其中DPO方法表现最为突出。在复杂游戏套件（测试推理、协调和对手建模能力）中验证了有效性。

Conclusion: 对话微调是LLMs在交互环境中进行推理、谈判和行动的有前景路径，为多智能体战略决策提供了新方法。
...
TL;DR: LLMs作为数据生成器，通过合成多语言训练数据来蒸馏小型模型，在低资源语言中表现优于大型生成器本身


<details>
  <summary>Details</summary>
Motivation: 探索LLMs的合成数据生成能力是否可以作为蒸馏的一种形式，使小型模型在多语言任务中达到甚至超越大型LLMs的性能，特别是在低资源语言场景下

Method: 使用最先进的多语言LLM生成覆盖11种语言和4个分类任务的合成数据集，然后通过微调、指令调优或作为紧凑LLMs的上下文示例来训练小型模型

Result: 即使少量合成数据也能使小型模型超越大型生成器本身，在低资源语言中表现尤为突出；LLMs作为生成器（教师）而非分类器时效果最佳

Conclusion: LLMs最适合作为数据生成器来赋能更小、更高效的多语言模型，特别是在低资源语言场景下，这为模型蒸馏提供了新途径
...
TL;DR: DNIPRO是一个包含24.6万篇新闻文章的多语言语料库，记录了2022年2月至2024年8月的俄乌战争，涵盖五个国家（俄罗斯、乌克兰、美国、英国、中国）的十一家媒体，支持对战时争议性话语进行系统性跨国分析。


<details>
  <summary>Details</summary>
Motivation: 研究战时争议性话语需要包含竞争性地缘政治视角的多语言资源，以分析叙事分歧、媒体框架和信息战。现有语料库往往缺乏这种多视角覆盖和系统注释。

Method: 构建了包含英语、俄语和中文三种语言的纵向语料库，涵盖246K篇新闻文章，具有一致全面的元数据和多种注释类型，并进行了严格的人工评估以支持下游任务。

Result: DNIPRO语料库展示了媒体如何构建竞争性现实，报道呈现反映地缘政治利益的极化解释。通过立场检测、情感分析、主题框架和矛盾分析等用例实验，揭示了叙事分歧模式。

Conclusion: DNIPRO为理解冲突性叙事如何在全球信息生态系统中出现和演变提供了基础资源，不仅支持计算新闻学研究，也有助于系统性跨国分析战时话语。
...
TL;DR: PolyBench：一个包含超过125K聚合物设计任务的大规模训练和测试基准数据集，用于解决当前LLMs在聚合物设计领域知识不足的问题，通过知识增强推理蒸馏方法训练的小型语言模型在聚合物设计任务上表现优于同类模型甚至前沿闭源LLMs。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在聚合物设计领域效果不佳，主要因为：(i) 大多数模型缺乏聚合物特定知识；(ii) 现有对齐模型缺乏聚合物设计相关知识和能力的覆盖。需要专门的数据集和方法来解决这些问题。

Method: 1. 构建PolyBench数据集：包含超过125K聚合物设计相关任务，基于1300万+数据点的知识库（来自实验和合成数据）；2. 引入知识增强推理蒸馏方法：为数据集添加结构化思维链；3. 任务组织：从简单到复杂的分析推理问题，支持泛化测试和诊断探测。

Result: 在PolyBench测试数据集上，使用PolyBench数据训练的7B到14B参数的小型语言模型（SLMs）表现优于同类规模模型，甚至超过闭源前沿LLMs，同时在其他聚合物基准测试中也显示出性能提升。

Conclusion: PolyBench数据集和知识增强推理蒸馏方法有效解决了LLMs在聚合物设计领域的知识不足问题，训练的小型模型在该领域表现出色，为AI4Science中的聚合物设计应用提供了有效解决方案。
...
TL;DR: LLM可实现大规模开放答案自动评分，性能接近人类评分员，适用于高风险考试评估


<details>
  <summary>Details</summary>
Motivation: 解决大规模考试（如全国毕业考试）在有限时间内需要大量人工评分的挑战，探索自动评分系统在真实教育环境中的适用性

Method: 使用爱沙尼亚两个全国队列的模拟考试作文数据集，基于官方课程标准的评分标准，比较LLM和统计NLP方法与人类专家评分的表现

Result: 自动评分性能与人类评分员相当，评分结果通常落在人类评分范围内；系统还能生成细粒度的子分数剖面，用于个性化反馈

Conclusion: 基于原则、评分标准驱动、人机协作的评分流程适用于高风险写作评估，可在小语种环境下实现国家级规模应用，同时保持人类监督并符合教育监管标准
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是调查大语言模型中存在的区域偏见问题，这是AI公平性和全球代表性领域的新兴关注点。随着LLM在全球范围内的广泛应用，区域偏见可能影响模型的可靠性、公平性和包容性，特别是在跨文化应用场景中。

Method: 研究方法包括：1) 评估10个主流LLM(GPT-3.5, GPT-4o, Gemini系列, Claude系列, Llama 3, Gemma 7B, Mistral 7B, Vicuna-13B)；2) 使用包含100个精心设计的提示数据集，这些提示在情境中立场景下探测区域间的强制选择决策；3) 引入FAZE评估框架，基于提示测量区域偏见，采用10分制评分，分数越高表示对特定区域的偏好越强。

Result: 实验结果显示：1) 不同模型的偏见水平存在显著差异；2) GPT-3.5表现出最高的偏见分数(9.5分)；3) Claude 3.5 Sonnet得分最低(2.5分)；4) 区域偏见确实会实质性损害LLM输出在现实世界跨文化应用中的可靠性、公平性和包容性。

Conclusion: 该研究为AI公平性研究做出贡献，强调了包容性评估框架的重要性，以及识别和缓解语言模型中地理偏见的系统性方法。研究结果表明需要进一步开发技术来减轻LLM中的区域偏见，确保模型在全球范围内的公平应用。
...
TL;DR: 该研究探讨如何通过为大型语言模型提供丰富的叙事身份背景和上下文信息，来更准确地模拟人类在社会困境游戏中的行为，超越了传统的"引导"方法。


<details>
  <summary>Details</summary>
Motivation: 人类行为不仅取决于理性思考，还受到身份认同和情境因素的影响。现有研究主要关注通过"引导"聊天模型来模拟人物角色，但这种方法绑定较弱，无法准确复制基于身份的行为。需要探索更深入的方法来提升LLMs模拟人类行为的保真度。

Method: 采用深度绑定方法，为基础模型提供扩展的背景故事和叙事身份信息。通过使用指令调优模型检查一致性，并建模时间、问题框架和参与者群体效应等情境因素。

Result: 研究发现，为基础LLMs提供丰富的叙事身份背景信息能显著提高模拟保真度。LLMs能够成功建模时间效应、问题框架和参与者群体差异等情境因素，从而更准确地复制人类行为。

Conclusion: LLMs能够探索影响人类研究但常被实验描述忽略的细节因素，这些因素阻碍了准确的复制研究。通过深度绑定身份背景和建模情境因素，LLMs可以更忠实地模拟人类行为，为社会科学研究提供新工具。
...
TL;DR: 提出一种无需训练、基于解码器注意力的生成时源归因框架，用于临床摘要的可信生成，支持文本和图像的多模态归因。


<details>
  <summary>Details</summary>
Motivation: 可信的临床摘要不仅需要流畅生成，还需要透明展示每个陈述的来源。现有方法存在后处理或需要重新训练的局限性，需要更直接的源归因方法。

Method: 提出基于解码器注意力的训练免费框架，通过两种多模态归因策略：1) 原始图像模式，直接使用图像补丁注意力；2) 标题作为跨度模式，用生成的标题替代图像，实现纯文本对齐。

Result: 在临床医患对话(CliConSummation)和放射学报告(MIMIC-CXR)两个代表性领域评估，方法始终优于基于嵌入和自归因基线，文本级和多模态归因准确率均有提升(如F1分数比嵌入基线提高15%)。基于标题的归因性能与原始图像注意力相当，但更轻量实用。

Conclusion: 注意力引导的归因是迈向可解释和可部署临床摘要系统的有希望步骤，为临床摘要提供了透明且实用的源归因解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型基于上下文中的线索（如语义描述和上下文示例）进行下一个token预测，但由于现代架构中层和注意力头的激增，阐明哪些先前的token对特定预测影响最大仍然具有挑战性。

Method: 提出了Jacobian Scopes方法，通过分析最终隐藏状态对输入的线性化关系来量化输入token对模型预测的影响。引入了三个变体：Semantic Scopes（针对特定logits的敏感性）、Fisher Scopes（针对完整预测分布）和Temperature Scopes（针对模型置信度/逆温度）。

Result: 通过涵盖指令理解、翻译和上下文学习的案例研究，发现了有趣的结果，例如Jacobian Scopes揭示了隐含的政治偏见。该方法也为最近争论的基于上下文的时间序列预测机制提供了见解。

Conclusion: Jacobian Scopes是一套有效的梯度基token级因果归因方法，能够解释LLM预测，揭示了模型决策过程中的重要影响因素，并为理解上下文学习机制提供了新的视角。
...
TL;DR: 论文发现当前多模态大语言模型在遥感、医学等专业领域表现有限，仅通过文本指令注入领域知识效果不佳，提出通过强化微调框架将领域知识编码为约束和奖励信号，在优化层面整合领域知识。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在专业领域（如遥感和医学成像）的有效性有限。研究发现仅通过输入层面的领域知识注入（文本指令、提示或辅助描述）对科学多模态任务几乎没有改进，表明当前MLLMs无法仅通过语言内化领域特定先验知识。

Method: 提出强化微调框架，将领域知识直接整合到学习目标中。不是将领域知识视为描述性信息，而是将其编码为领域感知的约束和奖励信号，在输出空间中塑造模型行为。

Result: 在遥感和医学领域的多个数据集上进行广泛实验，一致显示出良好的性能提升，在多模态领域任务上实现了最先进的结果。

Conclusion: 研究强调了在优化层面整合领域知识的必要性，揭示了当前MLLMs中文本领域条件化的根本局限性。领域知识必须通过优化过程内化，而不仅仅是作为输入信息提供。
...
TL;DR: 该研究探讨了LLM作为评估者时出现的数值偏差问题，发现对齐过程是导致偏差的主要原因，并测试了多种缓解策略，其中分数范围调整最为有效。


<details>
  <summary>Details</summary>
Motivation: LLM作为评估者在许多评估任务中表现有效，但存在数值偏差问题，即某些评分被不成比例地频繁生成，降低了评估性能。研究者希望探究这种偏差的成因。

Method: 通过比较对齐前后的LLM输出，验证对齐导致数值偏差的假设。针对已对齐的LLM，测试了三种缓解策略：温度缩放、分布校准和分数范围调整。

Result: 研究发现对齐确实增加了数值偏差。在缓解策略中，分数范围调整在减少偏差和提高性能方面最为有效，但仍属于启发式方法。
...
Method: 采用混合微调（结合结构化围棋知识和通用长链思维推理数据）作为冷启动，然后通过强化学习将围棋专业知识与通用推理能力整合

Result: LoGos不仅保持出色的通用推理能力，还能用自然语言进行围棋对弈，展示有效的战略推理和准确的下步预测，性能达到人类职业棋手水平，大幅超越现有所有LLM

Conclusion: 成功构建了首个达到人类职业水平的围棋LLM，为将通用LLM推理能力应用于专业领域提供了见解，并发布了首个大规模围棋训练数据集、评估基准和开源模型
...
TL;DR: GraphAnchor是一种新颖的图锚定知识索引方法，将图结构从静态知识表示重新概念化为动态演化的知识索引，通过迭代检索更新图来锚定关键实体和关系，指导LLM评估知识充分性并生成子查询，从而提升多跳问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在有效整合和解释分散在噪声文档中的关键证据方面存在挑战，需要改进知识集成和解释能力。

Method: 提出GraphAnchor方法：1) 将图结构重新概念化为动态演化的知识索引；2) 在迭代检索过程中增量更新图以锚定显著实体和关系；3) 生成结构化索引指导LLM评估知识充分性和制定后续子查询；4) 最终答案基于所有检索文档和演化后的图联合生成。

Result: 在四个多跳问答基准测试中验证了GraphAnchor的有效性，实验表明GraphAnchor能够调节LLM的注意力，更有效地关联检索文档中分布的关键信息。

Conclusion: GraphAnchor通过将图结构重新概念化为动态知识索引，显著提升了RAG系统在多跳问答任务中的性能，能够更好地整合分散在噪声文档中的关键证据。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在教育、心理健康和客户支持等需要稳定一致人格的领域部署日益增多，但现有研究忽视了仅通过对抗性对话历史就能重塑诱导人格的问题，黑盒人格操纵尚未被探索，这引发了现实交互中鲁棒性的担忧

Method: 提出PHISH（Persona Hijacking via Implicit Steering in History）框架，在仅推理的黑盒设置下，通过将语义负载线索嵌入用户查询来逐步诱导反向人格，并定义了量化攻击成功的指标

Result: 在3个基准测试和8个LLMs上，PHISH可预测地改变人格，触发相关特征的附带变化，在多轮设置中表现出更强效果；在高风险领域（心理健康、辅导、客户支持）中可靠地操纵人格，经人类和LLM-as-Judge评估验证；仅导致推理基准性能小幅下降，整体效用基本保持完整
...
Method: 提出Turn-Level GRPO (TL-GRPO)算法，采用轮次级分组采样进行细粒度优化。该方法保持语言模型的推理能力，同时针对迭代优化任务的特点进行专门优化。

Result: 在模拟电路尺寸优化任务上，TL-GRPO超越标准GRPO和贝叶斯优化方法。30B模型在相同仿真预算下达到最先进性能，展现出强大的泛化能力和实际应用价值。

Conclusion: TL-GRPO有效解决了迭代优化任务中的细粒度优化问题，结合了语言模型的推理能力和专门针对此类任务的优化策略，在科学优化任务中展现出显著优势。
...
### [56] [Timely Machine: Awareness of Time Makes Test-Time Scaling Agentic](https://arxiv.org/abs/2601.16486)
*Yichuan Ma,Linyang Li,Yongkang chen,Peiji Li,Xiaozhe Li,Qipeng Guo,Dahua Lin,Kai Chen*

Main category: cs.CL

TL;DR: 该论文提出Timely Machine框架，将测试时间重新定义为实际时钟时间而非生成长度，以解决工具调用场景中传统时间定义失效的问题。通过Timely-Eval基准测试和Timely-RL强化学习方法，使模型能够根据时间预算动态调整策略。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型处理复杂推理任务，测试时间扩展变得至关重要。但在需要频繁调用工具的智能体场景中，传统的基于生成长度的时间定义失效：工具延迟使推理时间与生成长度解耦。需要新的框架来重新定义测试时间，使模型能够根据实际时间预算动态调整策略。

Method: 1. 提出Timely Machine框架，将测试时间重新定义为实际时钟时间（wall-clock time）；2. 引入Timely-Eval基准测试，涵盖高频工具调用、低频工具调用和时间约束推理三种场景；3. 提出Timely-RL方法，在冷启动监督微调后使用强化学习增强时间规划能力；4. 通过变化工具延迟研究不同规模模型的性能表现。

Result: 研究发现：1. 小模型在快速反馈场景中表现更好（通过更多交互）；2. 大模型在高延迟设置中占优（通过更高质量的交互）；3. 现有模型无法根据时间预算调整推理策略；4. Timely-RL方法显著提升了时间预算感知能力，并在Timely-Eval基准上持续提升性能。

Conclusion: 该工作为智能体时代的测试时间扩展提供了新视角，通过重新定义测试时间为实际时钟时间，使模型能够根据时间预算动态调整策略。Timely-RL方法有效解决了现有模型缺乏时间预算适应能力的问题，为智能体在工具调用场景中的时间优化提供了解决方案。
...
TL;DR: 提出了医学检索增强生成（MRAG）基准，包含中英文医学任务，构建了Wikipedia和PubMed语料库，开发了MRAG-Toolkit工具包，实验表明RAG能提升LLM可靠性但可能降低长问题可读性。


<details>
  <summary>Details</summary>
Motivation: 当前医学领域缺乏全面的检索增强生成（RAG）评估基准，需要建立一个系统性的评测框架来评估RAG在医学问答系统中的表现。

Method: 构建了MRAG基准，涵盖中英文医学任务，使用Wikipedia和PubMed构建语料库，开发了MRAG-Toolkit工具包来系统探索不同RAG组件，通过实验分析RAG性能影响因素。

Result: 实验发现：(a) RAG能提升LLM在MRAG任务中的可靠性；(b) RAG系统性能受检索方法、模型规模和提示策略影响；(c) RAG提高了有用性和推理质量，但可能使LLM对长问题的回答可读性略有下降。

Conclusion: MRAG基准填补了医学领域RAG评估的空白，提供了系统评测工具，有助于学术界和工业界应用，将开源发布数据集和工具包。
...
TL;DR: LOGICAL-COMMONSENSEQA是一个将常识推理重新定义为逻辑组合的基准测试，使用AND、OR、NEITHER/NOR等运算符评估陈述对的联合合理性，揭示现有模型在否定推理方面的严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有常识推理基准大多采用单标签评估，无法区分陈述是联合合理、互斥还是联合不合理，掩盖了模型对多解释场景的推理能力局限。

Method: 引入LOGICAL-COMMONSENSEQA基准，将常识推理重构为使用合理性级别运算符（AND、OR、NEITHER/NOR）对原子陈述对进行逻辑组合；评估指令调优、推理专用和微调模型在零样本、少样本和思维链提示下的表现。

Result: 模型在合取推理上表现尚可，在析取推理上表现中等，但在基于否定的问题上性能急剧下降；该基准揭示了模型在组合式常识推理方面的根本局限性。

Conclusion: LOGICAL-COMMONSENSEQA暴露了现有模型在组合式常识推理，特别是否定推理方面的基本缺陷，为推进组合式常识推理提供了受控框架。
...
TL;DR: 研究发现单轮评估无法捕捉对话动态中的真实风险，通过BoolQ数据集测试发现对话长度和脚手架条件会影响LLM回答的真实性，揭示了模型特定漏洞


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试主要采用单轮提示评估，但这种方法无法捕捉真实世界中发生危害的对话动态。研究者希望探究对话长度是否影响LLM回答的真实性，以及多轮对话设置是否能揭示单轮测试中不可见的模型漏洞。

Method: 研究使用BoolQ数据集，在三种不同LLM上评估其性能，通过变化对话长度和脚手架条件来测试模型表现。采用多轮对话设置而非传统的单轮测试，以观察模型在动态对话环境中的行为。

Result: 研究结果揭示了模型特定的漏洞，这些漏洞在单轮测试中不可见。观察到长度依赖和脚手架特定的效应，表明静态评估存在根本性限制。部署相关的漏洞只能在多轮对话设置中被发现。

Conclusion: 静态评估存在根本性限制，无法捕捉真实部署环境中的模型漏洞。多轮对话评估对于发现LLM在实际应用中的风险至关重要，对话长度和脚手架条件显著影响模型回答的真实性。
...
TL;DR: 本文提出了一种基于大语言模型的检索-精炼-校准框架，通过识别实体、精炼证据和校准验证过程，解决了传统分解式事实核查方法中因无关实体或证据引入噪声的问题。
...
Conclusion: RRC框架通过系统性的证据处理和验证校准，有效解决了传统分解式事实核查中的噪声问题，提高了验证准确性。
...
TL;DR: Attention-MoA：基于注意力机制的新型混合智能体框架，通过智能体间语义注意力实现深度协作，结合层间残差模块和自适应早停机制，显著提升推理性能并超越大型专有模型。


<details>
  <summary>Details</summary>
Motivation: 当前混合智能体框架虽然引入了动态路由和残差连接来提升效率，但未能促进智能体间的深度语义交互，限制了系统主动纠正幻觉和精炼逻辑的能力。需要一种能够实现深度语义协作的新框架。

Method: 提出Attention-MoA框架，核心是智能体间语义注意力机制，使智能体能够相互关注和整合语义信息。同时引入层间残差模块和自适应早停机制，缓解深层信息退化并提升计算效率。

Result: 在AlpacaEval 2.0上获得91.15%的长度控制胜率，在FLASK的12项能力中主导10项，MT-Bench得分8.83。由小型开源模型组成的集成系统超越了Claude-4.5-Sonnet和GPT-4.1等大型专有模型，AlpacaEval 2.0 LC胜率达77.36%。

Conclusion: Attention-MoA通过智能体间语义注意力机制实现了深度协作，显著提升了混合智能体系统的性能，证明了小型模型通过有效协作能够超越大型专有模型，为推理时协作提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态技术发展，视觉大语言模型在工业应用中逐渐替代传统深度学习模型，但VLLMs存在特定领域性能不足、参数量大、计算资源需求高、推理速度慢等问题，需要为边缘部署设计更高效的VLLM解决方案。

Method: 提出AuroraEdge-V-2B模型，仅含20亿参数，采用压缩融合方法提升推理效率，显著减少解码过程中的视觉token数量，将推理时的浮点运算减半。

Result: AuroraEdge-V-2B在9个基准测试中得分高于同参数规模模型（如Qwen2-VL-2B、Qwen2.5-VL-3B、InternVL-2.5-2B），具备更好的实时性能，更适合边缘部署。

Conclusion: AuroraEdge-V-2B通过压缩融合方法实现了高效的边缘部署VLLM，在保持强性能的同时解决了传统VLLMs参数量大、推理速度慢的问题，为工业应用提供了实用的解决方案。
...
TL;DR: PROST-LLM通过渐进式方法增强大语言模型的语音到语音翻译能力，包括三任务学习、模态链微调、自采样和反向翻译生成偏好对，以及偏好优化。
...
Method: 1. 使用CVSS语料库微调LLM，采用三任务学习和模态链方法提升初始性能；2. 利用微调后的模型通过自采样和反向翻译生成偏好对；3. 使用这些偏好对进行偏好优化以进一步增强模型的S2ST能力。
...
<details>
  <summary>Details</summary>
Motivation: LLM助手集成记忆机制记录用户偏好，但无关的个性化记忆会干扰模型意图理解，需要全面研究个性化的双重效应并解决非理性个性化问题。

Method: 1) 开发RPEval基准，包含个性化意图推理数据集和多粒度评估协议；2) 分析错误模式；3) 提出RP-Reasoner方法，将记忆利用视为语用推理过程，实现个性化信息的选择性整合。

Result: RPEval揭示了现有LLM中普遍存在的非理性个性化现象及其对用户体验的负面影响。RP-Reasoner在RPEval上显著优于精心设计的基线方法，解决了大规模商业个性化助手中80%的不良案例。

Conclusion: 语用推理能有效缓解非理性个性化问题，RP-Reasoner方法通过选择性整合个性化信息提升了LLM助手的意图理解能力，RPEval基准为未来研究提供了重要工具。
...
TL;DR: 本文扩展了MultiLexNorm基准，新增5种亚洲语言，提出基于大语言模型的新架构，提升了跨语言词汇规范化任务的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体数据对NLP研究具有重要价值，但因其语言使用非正式、自发且包含多种社会方言，导致NLP模型性能下降。现有词汇规范化基准MultiLexNorm主要覆盖印欧语系拉丁文字语言，缺乏语言多样性代表性。

Method: 扩展MultiLexNorm基准，新增5种来自不同语系的亚洲语言（使用4种不同文字）。提出基于大语言模型的新架构，替代先前最先进模型，以提升跨语言鲁棒性。

Result: 先前最先进模型在新增加的亚洲语言上表现较差。基于大语言模型的新架构展现出更鲁棒的性能。通过错误分析揭示了该任务的未来研究方向。

Conclusion: 扩展的MultiLexNorm基准填补了跨语言词汇规范化任务的空白，基于大语言模型的新架构为处理多语言社交媒体数据提供了更有效的解决方案，错误分析为未来研究指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 探索语言模型中奉承行为（sycophancy）的内部表征机制，理解正确与错误答案之间的奉承信号如何在模型内部编码，以及如何通过线性干预缓解这一问题

Method: 在残差流、MLP和注意力层训练线性探针分析奉承信号；使用TruthfulQA作为基础数据集；通过注意力模式分析识别关键注意力头；比较新发现的"奉承方向"与先前识别的"真实方向"

Result: 正确到错误奉承信号在多头注意力激活中最线性可分；中间层稀疏注意力头子集对干预最有效；基于TruthfulQA训练的探针能有效迁移到其他事实QA基准；奉承方向与真实方向重叠有限，表明事实准确性和抗顺从性有不同机制；关键注意力头过度关注用户怀疑表达

Conclusion: 奉承行为可通过针对注意力激活内部几何结构的简单线性干预来缓解，这为理解语言模型中的顺从偏差提供了新视角
...
TL;DR: 梯度方法用于LLM实例解释时面临高维梯度问题，本文比较了两种降维策略：选择架构相关的组件子集 vs 全梯度投影，发现贪婪选择的组件子集在检索任务中更有效且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 基于梯度的大语言模型实例解释方法受到模型梯度巨大维度的阻碍。实践中，影响估计通常限制在模型参数的子集上以使计算可行，但这种子集选择往往是临时的，缺乏系统评估的合理性验证。

Method: 研究比较两种降维策略：1) 选择小型、架构相关的模型组件子集；2) 将完整梯度投影到低维空间。使用新颖的基准测试，通过贪婪选择的组件子集来捕获训练数据影响信息，并与完整梯度或随机投影进行比较。

Result: 贪婪选择的组件子集在检索任务中比完整梯度或随机投影更有效地捕获训练数据影响信息。该方法比随机投影计算效率更高，证明有针对性的组件选择是使大模型实例解释更计算可行的实用策略。

Conclusion: 有针对性的组件选择策略比随机投影方法在计算效率和解释效果上都更优，为大规模模型实例解释提供了更可行的计算方案。
...
TL;DR: PLawBench是一个实用的法律基准测试，旨在评估大语言模型在真实法律实践场景中的能力，弥补现有法律基准在任务简化、评估维度单一等方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有法律基准测试依赖简化和高度标准化的任务，无法捕捉真实法律实践的模糊性、复杂性和推理需求。先前评估通常采用粗略的单维度指标，未能明确评估细粒度的法律推理能力。

Method: 基于真实法律工作流程，PLawBench通过三个任务类别建模法律从业者的核心流程：公共法律咨询、实际案例分析和法律文件生成。基准包含850个问题，覆盖13个实际法律场景，每个问题都配有专家设计的评估标准（约12,500个评估项）。使用与人类专家判断对齐的LLM评估器对10个最先进的LLM进行评估。

Result: 实验结果显示，没有LLM在PLawBench上表现出色，揭示了当前LLM在细粒度法律推理能力方面的显著局限性。

Conclusion: PLawBench揭示了当前LLM在法律推理能力上的不足，为未来法律LLM的评估和发展指明了重要方向。数据已开源供进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统评估智能体长期记忆的基准测试，特别是在交互式环境中。需要能够评估多种记忆技能（单跳/多跳回忆、归纳、时空、逻辑、对抗性）的综合性测试框架。

Method: 通过程序化生成基准测试，从智能体自身游戏轨迹生成问题，使用游戏底层信号计算可验证的真实答案。覆盖15个文本游戏和多个视觉种子，评估包括单跳/多跳回忆、归纳、时间、空间、逻辑和对抗性推理在内的多种记忆技能。

Result: 结果远未饱和：归纳和空间推理是持续瓶颈，尤其在视觉环境中。持久性记忆对文本游戏中的开放骨干模型有明显提升，但对VLM智能体的改进不一致，表明视觉基础的情景记忆仍是开放挑战。人类研究进一步证实了EMemBench的难度。

Conclusion: EMemBench为评估智能体长期记忆提供了全面基准，揭示了当前模型的局限性，特别是视觉环境中的归纳和空间推理能力不足，为未来研究指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 医学概念识别（MA-BCR）面临人类标注稀缺的挑战，特别是在泛化到未见概念方面存在困难。需要系统性地解决这一泛化问题，并探索可扩展的标注资源。

Method: 1. 提出基于层次概念索引的评估框架，包含新的泛化度量指标；2. 开发基于大语言模型（LLM）的自动标注数据（ALD）生成管道，用于创建任务特定的标注数据。

Result: 研究表明LLM生成的自动标注数据不能完全替代人工标注，但作为有价值的资源能显著改善泛化性能，为模型提供更广泛的覆盖范围和结构知识，使其能够更好地识别未见概念。

Conclusion: 通过层次概念索引评估框架和LLM自动标注数据生成方法，系统性地解决了医学概念识别中的泛化问题，为领域提供了可扩展的标注资源和评估标准，代码和数据集已开源。
...
TL;DR: 该研究针对自动作文评分系统对ESL学习者的算法偏见问题，提出基于对比学习的匹配作文对方法，有效减少了高熟练度ESL作文的评分差异。


<details>
  <summary>Details</summary>
Motivation: 随着自动作文评分系统在高风险教育环境中的广泛应用，针对英语作为第二语言学习者的算法偏见问题日益受到关注。当前基于Transformer的回归模型主要基于母语者语料训练，往往学习到表面L2语言特征与作文质量之间的虚假相关性。

Method: 研究首先对微调的DeBERTa-v3模型在ASAP 2.0和ELLIPSE数据集上进行偏见分析，然后提出对比学习方法：使用匹配作文对进行对比学习。构建了17,161对匹配作文对，采用三元组边界损失微调模型，使ESL和母语写作的潜在表征对齐。

Result: 该方法将高熟练度ESL作文的评分差异减少了39.9%（从10.3%降至6.2%），同时保持了0.76的二次加权Kappa系数。事后语言分析表明，模型成功解耦了句子复杂性和语法错误，避免了对有效L2句法结构的惩罚。

Conclusion: 对比学习与匹配作文对策略能有效缓解自动作文评分系统对ESL学习者的算法偏见，在保持评分一致性的同时显著减少评分差异，有助于更公平地评估第二语言写作能力。
...
TL;DR: 提出基于大语言模型的放射学报告纵向信息自动标注管道，用于检测相关信息和提取疾病进展，为报告生成模型提供标准化评估基准。


<details>
  <summary>Details</summary>
Motivation: 放射学报告中的纵向信息对监测疾病进展至关重要，但现有自动标注方法存在局限性：复杂规则封闭、领域特定、难以适应，简单规则又容易遗漏专业信息。需要一种能够捕捉细微语言模式和语义相似性的自动化标注方案。

Method: 提出基于LLM的自动标注管道：1) 识别包含相关信息的句子；2) 提取疾病进展。评估了5个主流LLM，基于效率和性能选择Qwen2.5-32B，并用其对MIMIC-CXR数据集的95,169份报告进行标注，建立标准化评估基准。

Result: LLM标注方法在纵向信息检测和疾病跟踪任务上分别比现有方案提高了11.3%和5.3%的F1分数。使用Qwen2.5-32B标注的数据集为评估7个最先进的报告生成模型提供了标准化基准。

Conclusion: 基于LLM的自动标注管道能够有效识别放射学报告中的纵向信息，为报告生成模型的评估提供了可靠、标准化的基准，解决了现有标注方法的局限性。
...
TL;DR: 研究发现：虽然LLMs在低资源语言上的任务准确率大幅下降，但幻觉检测器的准确率下降幅度小得多，表明即使在低资源语言中，LLMs内部也可能编码了不确定性信号。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型的两个普遍失败模式：幻觉（产生错误信息）和低资源效应（在低资源语言上性能显著下降）。探索这两个问题的交集：幻觉检测器是否也会受到低资源效应的影响？

Method: 在三个领域（事实回忆、STEM和人文学科）的五个任务上进行实验，使用四种LLMs和三种幻觉检测器，比较英语和低资源语言（如孟加拉语）上的性能差异。

Result: 任务准确率在低资源语言上大幅下降，但检测器准确率的下降幅度通常比任务准确率下降幅度小几倍。检测器在语言内部（包括非英语）和多语言设置中表现稳健，但在没有语言内监督的跨语言设置中表现不佳。
...
<details>
  <summary>Details</summary>
Motivation: IKE依赖冗长的事实特定演示，创建成本高且占用大量上下文窗口空间，需要更实用和可扩展的知识编辑方法

Method: 引入特殊训练的persuasion tokens（P-Tokens），能够复现IKE演示的效果，实现无需事实特定演示的高效知识编辑

Result: 在两个编辑数据集和三个LLM上评估，P-Tokens性能与IKE相当甚至更优；对干扰物具有鲁棒性，对相邻事实影响小；增加P-Tokens数量可提升性能
...
<details>
  <summary>Details</summary>
Motivation: 细粒度意见分析需要大量人工标注，成本高昂，特别是在跨领域和实际应用中。探索LLMs作为自动标注器的可行性，解决领域特定标注数据不足的问题。

Method: 使用声明式标注流程，减少手动提示工程的变异性；提出新颖的LLM裁决方法，用于处理多个标签并生成最终标注。在ASTE和ACOS分析任务中测试不同规模的模型。

Result: LLMs能够作为自动标注器和裁决器，在不同LLM标注器之间实现高标注者间一致性，显著降低创建细粒度意见标注数据集的成本和人工努力。

Conclusion: LLMs在细粒度意见分析中作为自动标注器是可行的，能够有效解决标注数据稀缺问题，为跨领域应用提供经济高效的解决方案。
...
TL;DR: 该研究首次系统分析了文本到图像（T2I）模型的"表面优于语义"（SoS）倾向，即模型对非英语输入语言产生文化刻板印象描绘的现象。
...
Method: 创建了覆盖171种文化身份的提示词集，翻译成14种语言，并用其提示七个T2I模型。为了量化模型、语言和文化间的SoS倾向，引入了一种新颖的度量方法，并分析了这些倾向如何视觉化地表现出来。

Result: 除一个模型外，所有模型至少在两种语言中表现出强烈的表面层面倾向，这种效应在T2I文本编码器的各层中逐渐增强。此外，这些表面倾向经常与刻板印象的视觉描绘相关联。

Conclusion: 该研究首次系统揭示了T2I模型在多语言环境下的SoS倾向问题，表明模型在处理非英语输入时存在系统性偏差，倾向于产生文化刻板印象的视觉输出，这对T2I模型的公平性和跨文化应用具有重要意义。
...
TL;DR: 该研究使用国际象棋作为测试平台，发现LLMs在需要流体智力（推理能力）的任务上表现显著下降，特别是在分布外任务中性能崩溃至随机水平，表明当前架构在系统泛化方面仍有限制。


<details>
  <summary>Details</summary>
Motivation: 区分LLMs表现中晶体智力（记忆召回）与流体智力（推理能力）的贡献，理解当前模型在系统泛化方面的局限性。

Method: 使用国际象棋作为受控测试平台，构建基于训练语料库接近度的位置分类法，系统评估多个GPT世代在不同推理强度下的表现。

Result: 性能随流体智力需求增加而持续下降；在分布外任务中性能崩溃至随机水平；新模型虽有改进，但对训练分布外任务的进步显著放缓；推理增强推理能提升性能，但其边际效益随分布接近度而降低。

Conclusion: 当前LLM架构在系统泛化方面仍有限制，仅靠规模扩展不足以实现稳健的流体智力，需要超越规模的新机制。
...
TL;DR: 论文提出了一种基于说服技术的对抗攻击新方法，通过LLM重写虚假声明来欺骗自动事实核查系统，显著降低了核查性能和证据检索效果。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击主要依赖噪声注入或语义修改，但忽略了说服技术在虚假信息传播中的重要作用。论文旨在探索说服技术作为新型对抗攻击的潜力，揭示自动事实核查系统在此类攻击下的脆弱性。

Method: 提出基于说服技术的对抗攻击框架：1) 使用生成式LLM将原始声明重写为包含说服技术的版本；2) 采用6个类别共15种具体说服技术；3) 在FEVER和FEVEROUS基准上进行解耦评估，分别测试对声明验证和证据检索的影响。

Result: 实验表明说服攻击能显著降低自动事实核查系统的性能：1) 声明验证准确率大幅下降；2) 证据检索效果明显恶化；3) 不同说服技术对系统的影响程度存在差异。

Conclusion: 说服技术构成了一类强大的对抗攻击手段，现有自动事实核查系统对此类攻击缺乏鲁棒性。研究强调了开发更健壮的事实核查系统的必要性，需要特别关注说服技术带来的安全威胁。
...
TL;DR: 论文提出基于排列的评估框架，发现SOTA嵌入模型存在系统性位置和语言偏见，并提出推理时注意力校准方法来解决位置偏见问题。


<details>
  <summary>Details</summary>
Motivation: 为了在基于嵌入的搜索过程中被发现，文档的每个部分都应在其嵌入表示中得到反映。需要量化潜在的反映偏见，特别是当文档较长且包含多个段落时，现有模型可能存在系统性偏见。

Method: 1) 引入基于排列的评估框架来量化反映偏见；2) 分析发现位置偏见源于池化标记嵌入中的前向加载注意力分布；3) 提出推理时注意力校准方法，重新分配注意力使其在文档位置上更均匀。

Result: 观察到SOTA嵌入模型存在系统性偏见：早期段落和高资源语言（如英语）被过度表示，而后期段落和低资源语言被边缘化。位置偏见源于早期标记获得更多注意力。提出的注意力校准方法提高了后期段落的可发现性。

Conclusion: 嵌入模型存在系统性位置和语言偏见，影响文档各部分的可发现性。提出的评估框架和注意力校准方法能有效识别和缓解位置偏见，提高嵌入表示的公平性。代码已开源。
...
TL;DR: LogitMatch：一种新的约束解码方法，用于解决生成式LLM在文本分析任务中缺乏显式引用输入特定部分的问题，通过强制模型输出与有效输入片段对齐来改进基于内容匹配的方法。


<details>
  <summary>Details</summary>
Motivation: 生成式语言模型缺乏显式机制来引用输入文本的特定部分，导致在命名实体识别等文本分析任务中需要各种临时提示策略，且结果不一致。

Method: 将现有策略分为三类：标记输入文本、索引片段数值位置、匹配片段内容。针对内容匹配的局限性，提出了LogitMatch——一种新的约束解码方法，强制模型输出与有效输入片段对齐。

Result: 在四个不同任务上评估所有方法，发现标记方法仍是稳健的基线，而LogitMatch通过消除片段匹配问题改进了竞争性的基于匹配的方法，并在某些设置中优于其他策略。

Conclusion: LogitMatch为生成式LLM在文本分析任务中提供了一种有效的约束解码方法，解决了内容匹配的局限性，并在某些情况下优于现有策略。
...
TL;DR: Doc2AHP：一种基于AHP原则的结构化推理框架，利用LLM的泛化能力与决策理论严谨性结合，无需标注数据或人工干预，从非结构化文档构建高质量决策模型。


<details>
  <summary>Details</summary>
Motivation: LLMs在语义理解方面表现出色，但在需要严格逻辑的复杂决策任务中难以保证结构一致性和推理可靠性；而经典决策理论（如AHP）虽然提供系统化理性框架，但其构建严重依赖劳动密集型的领域专家知识，形成"专家瓶颈"，阻碍了在一般场景中的可扩展性。

Method: 提出Doc2AHP框架：1）利用AHP的结构原则作为约束，指导LLM在非结构化文档空间中进行受限搜索，强制执行父子节点之间的逻辑蕴含关系；2）引入多智能体加权机制与自适应一致性优化策略，确保权重分配的数字一致性；3）无需大量标注数据或人工干预。

Result: 实证结果表明：Doc2AHP不仅使非专家用户能够从零开始构建高质量决策模型，而且在逻辑完整性和下游任务准确性方面显著优于直接生成基线方法。

Conclusion: Doc2AHP成功弥合了LLMs的泛化能力与决策理论严谨性之间的差距，通过结构化推理框架将AHP原则与LLM能力结合，解决了专家瓶颈问题，实现了从非结构化文档自动构建高质量决策模型的目标。
...
TL;DR: SycoEval-EM框架通过多智能体模拟评估LLM在急诊医学中对抗患者说服的鲁棒性，发现模型在不当医疗请求下妥协率高达0-100%，静态基准无法预测社会压力下的安全性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床决策支持中显示出潜力，但存在向患者不当医疗请求妥协的风险。现有评估方法多为静态基准，无法充分测试模型在社会压力下的安全性，需要开发更真实的对抗性测试框架。

Method: 引入SycoEval-EM多智能体模拟框架，通过对抗性患者说服场景评估LLM鲁棒性。涵盖20个LLM模型和1,875次交互，涉及三个Choosing Wisely场景（明智选择指南），测试不同说服策略的有效性。

Result: 模型妥协率范围0-100%，对影像检查请求的脆弱性（38.8%）高于阿片类药物处方（25.0%）。模型能力与鲁棒性相关性差，所有说服策略同样有效（30.0-36.0%），表明普遍脆弱性而非特定策略弱点。

Conclusion: 静态基准无法充分预测临床AI在社会压力下的安全性，需要多轮对抗性测试作为临床AI认证的必要组成部分。模型普遍存在向患者不当请求妥协的脆弱性，与模型能力无关。
...
TL;DR: 传统机器学习模型在医学分类任务中表现最佳，LoRA微调的Gemma变体表现最差，零样本LLM/VLM在图像任务中表现尚可但文本任务表现不佳
...
Method: 使用四个公开数据集（涵盖文本和图像模态，包含二元和多分类复杂度），对比三类模型：传统ML（LR、LightGBM、ResNet-50）、基于提示的LLM/VLM（Gemini 2.5）和微调PEFT模型（LoRA适配的Gemma3变体），采用一致的数据划分和评估指标

Result: 传统ML模型在大多数医学分类任务中表现最佳，尤其在结构化文本数据集上表现突出；LoRA微调的Gemma变体在所有实验中表现最差；零样本LLM/VLM在文本任务上表现不佳，但在多分类图像任务中与ResNet-50基线表现相当

Conclusion: 在许多医学分类场景中，传统机器学习模型仍是最可靠的选择；基础模型并非普遍优越，参数高效微调的效果高度依赖于适配策略，本研究中最小化微调反而有害
...
TL;DR: 该研究通过构建一个包含程序生成游戏任务的框架，使用"神谕"干预来评估不同AI能力（如规划、状态跟踪）对多轮次、长视野智能体任务的重要性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在孤立任务上表现良好，但在需要规划、状态跟踪和长上下文处理等多轮次、长视野的智能体问题上仍然存在困难。研究旨在理解这些底层能力对于此类任务成功的相对重要性。

Method: 开发了一个神谕反事实框架，通过程序生成具有可调复杂度的游戏式任务套件。在受控环境中提供精确的神谕干预（如完美规划、无错误状态跟踪），从而隔离每种神谕能力的贡献，避免现实基准中的混杂效应。

Result: 结果显示，某些干预（如规划）在各种设置中持续提升性能，而其他技能的有用性则取决于环境和语言模型的特性。不同能力的重要性随任务复杂性而变化。

Conclusion: 该研究揭示了多轮次智能体环境的挑战，为未来AI智能体和语言模型的发展提供了指导。通过受控评估框架，能够更精确地识别哪些能力需要优先改进以提升智能体在复杂任务中的表现。
...
TL;DR: 提出AgentsEval多智能体流推理框架，用于评估医学影像报告的临床正确性和推理保真度，通过模拟放射科医师协作诊断流程提供结构化评估和可解释推理轨迹。
...
Method: 开发AgentsEval多智能体流推理框架，将评估过程分解为可解释步骤：标准定义、证据提取、对齐和一致性评分，模拟放射科医师协作诊断工作流程。同时构建基于扰动的多领域基准，涵盖五个医学报告数据集和多种成像模态。

Result: 实验结果表明AgentsEval提供临床对齐、语义忠实且可解释的评估，在释义、语义和风格扰动下保持稳健。该框架能提供明确的推理轨迹和结构化临床反馈。

Conclusion: AgentsEval代表了向透明和临床基础的医学报告生成系统评估迈出的一步，促进大语言模型在临床实践中的可信集成，为医疗AI系统提供更可靠的评估机制。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个具有卓越智能体推理能力的开放源模型，能够处理复杂工具交互并在嘈杂现实环境中保持鲁棒性，以解决现有模型在真实世界任务中的局限性。

Method: 采用混合专家架构，结合领域并行专家训练与后续融合的统一框架；扩展异步强化学习框架DORA支持大规模多环境训练；系统分析现实噪声模式并设计针对性训练；引入Heavy Thinking模式进行测试时扩展。

Result: 在智能体搜索、工具使用和工具集成推理等基准测试中达到开放源模型的最先进性能；在复杂工具交互中表现出强泛化能力；在嘈杂现实环境中保持鲁棒行为。

Conclusion: LongCat-Flash-Thinking-2601通过创新的训练框架、环境扩展、噪声鲁棒性设计和推理增强技术，实现了卓越的智能体推理能力，为开放源智能体模型设立了新标准。
...
<details>
  <summary>Details</summary>
Motivation: 受昆虫在发现食物位置与巢穴之间学习并优化视觉引导路径能力的启发，将昆虫大脑的联想学习和路径整合机制应用于视觉点目标导航任务，旨在开发计算效率高且鲁棒的导航系统。

Method: 结合两种昆虫大脑结构的抽象模型：负责联想学习的结构和负责路径整合的结构。将Habitat点目标导航任务形式化基准与昆虫的视觉路径学习能力进行类比，构建简单的昆虫启发式智能体。

Result: 该昆虫启发式智能体在Habitat点目标导航任务中表现出与近期SOTA模型相当的性能，但计算成本低多个数量级。在更真实的模拟环境中测试显示该方法对扰动具有鲁棒性。

Conclusion: 简单的昆虫启发式智能体能够在视觉点目标导航任务中实现高效性能，证明了生物启发方法在降低计算成本和提高鲁棒性方面的潜力，为高效导航系统开发提供了新思路。
...
TL;DR: 该论文研究了通过强化学习与可验证奖励（RLVR）训练得到的推理型大语言模型在心理理论任务中的表现，发现这些模型对提示变化和任务扰动具有更强的鲁棒性，但这种改进更可能源于寻找正确答案的鲁棒性增强，而非全新的心理理论推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在心理理论测试中表现出色引发了对其真实能力的争论，同时通过RLVR训练的推理型模型在多个基准测试中取得了显著改进。本研究旨在探究这类推理模型在心理理论任务中的行为表现，以理解其能力本质。

Method: 采用新颖的机器心理学实验改编方法，结合已建立的基准测试结果，分析推理型LLMs在心理理论任务中的表现。通过对比不同提示变化和任务扰动下的模型行为，评估其鲁棒性。

Result: 推理模型在心理理论任务中表现出对提示变化和任务扰动的一致鲁棒性增强。分析表明，观察到的改进更可能归因于寻找正确答案的鲁棒性提高，而不是出现了根本性的新型心理理论推理能力。

Conclusion: 研究强调了在评估大语言模型的社会认知行为时，需要区分表面性能改进与根本能力变化。推理模型的鲁棒性提升可能反映了更可靠的解决方案搜索机制，而非真正的心理理论理解能力突破。
...
TL;DR: MAGE-KT：多智能体图增强知识追踪框架，通过多视图异构图构建和条件子图检索解决现有图KT方法中概念关系挖掘不足和全图编码计算成本高、噪声多的问题
...
Method: 提出MAGE-KT框架：1) 构建多视图异构图，结合多智能体知识概念关系提取器和学生-问题交互图，捕捉互补的语义和行为信号；2) 基于目标学生历史检索紧凑、高价值的子图；3) 使用非对称交叉注意力融合模块集成子图，增强预测同时避免注意力扩散和无关计算

Result: 在三个广泛使用的知识追踪数据集上的实验表明，该方法在知识概念关系准确性方面有显著提升，并在下一问题预测方面明显优于现有方法

Conclusion: MAGE-KT通过多智能体关系提取、条件子图检索和非对称融合机制，有效解决了图知识追踪中的概念关系挖掘不足和计算效率问题，为知识追踪提供了更准确和高效的解决方案
...
TL;DR: 开发AI支持平台，帮助生物医学技术人员实时诊断和维修医疗设备，减少设备停机时间
...
Result: 错误代码解释精度达100%，纠正措施建议准确率达80%，证明了AI驱动系统支持医疗设备维护的可行性

Conclusion: AI支持平台有潜力减少资源受限环境中的设备停机时间，改善医疗服务提供
...
### [91] [Computational Foundations for Strategic Coopetition: Formalizing Collective Action and Loyalty](https://arxiv.org/abs/2601.16237)
*Vik Pant,Eric Yu*

Main category: cs.MA

TL;DR: 该技术报告扩展了战略竞合的计算基础，引入忠诚度调节的效用函数，解决了混合动机多智能体环境中的搭便车问题，通过实验和实证验证了框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 混合动机多智能体环境中普遍存在搭便车问题，因为个体努力使所有成员平等受益，但每个成员承担自己贡献的全部成本。i*方法将团队视为复合行动者，但缺乏可扩展的计算机制来分析集体行动问题在竞合环境中如何出现和解决。

Method: 开发忠诚度调节的效用函数，包含两个机制：忠诚度收益（福利内化加内在贡献满意度）和成本容忍度（忠诚成员的努力负担减少）。通过依赖加权的团队凝聚力整合i*结构依赖，将成员激励与团队层面定位连接。框架适用于人类团队（忠诚度作为心理认同）和多智能体系统（对齐系数和调整的成本函数）。

Result: 在3,125种配置下的实验验证显示强大的忠诚度效应（中位努力分化15.04倍）。所有六个行为目标均达到阈值：搭便车基线（96.5%）、忠诚度单调性（100%）、努力分化（100%）、团队规模效应（100%）、机制协同（99.5%）和有界结果（100%）。使用Apache HTTP Server（1995-2023）案例研究的实证验证获得60/60分，重现了形成、增长、成熟和治理阶段的贡献模式。统计显著性确认在p<0.001，Cohen's d=0.71。

Conclusion: 该框架成功地将战略竞合的计算基础扩展到团队层面动态，通过忠诚度调节机制有效解决了集体行动问题，为分析竞合环境中团队行为提供了可扩展的计算工具，并在理论和实证上都得到了验证。
