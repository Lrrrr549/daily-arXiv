...
TL;DR: TP-Blend是一个无需训练的轻量级框架，通过双提示注意力融合实现对象和风格的同时编辑，解决了现有方法难以同时引入新对象和新风格的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本条件的扩散编辑器能够较好地处理单个对象替换，但在需要同时引入新对象和新风格时表现不佳。现有方法难以同时控制内容和外观的精确编辑。

Method: TP-Blend采用双提示注意力融合框架：1) 交叉注意力对象融合(CAOF)通过头级注意力平均定位空间标记，解决熵正则化最优传输问题，在完整多头维度上更新特征向量；2) 自注意力风格融合(SASF)通过细节敏感实例归一化注入风格，使用一维高斯滤波器分离高低频分量，并交换键值矩阵实现上下文感知纹理调制。

Result: 实验表明TP-Blend能够生成高分辨率、照片级真实的编辑结果，在内容和外观控制方面具有精确性，在定量保真度、感知质量和推理速度方面均超越现有基线方法。

Conclusion: TP-Blend通过创新的双提示注意力融合机制，成功解决了同时编辑对象和风格的挑战，为扩散模型的精确内容编辑提供了有效的训练免费解决方案。
...
### [2] [Where Does Vision Meet Language? Understanding and Refining Visual Fusion in MLLMs via Contrastive Attention](https://arxiv.org/abs/2601.08151)
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉-语言理解方面取得了显著进展，但其内部如何整合视觉和文本信息仍然缺乏深入理解。研究者希望揭示MLLMs中视觉-文本融合的机制和演化规律。

Method: 采用系统性的层级掩码分析，研究多种架构中视觉-文本融合的演化过程。进一步分析层级关注演化，并提出一种无需训练的对比关注框架，该框架建模早期融合层与最终层之间的转换，以突出有意义的关注转移。

Result: 研究发现融合在特定层而非均匀分布，某些模型在输出生成前出现视觉信号重新激活的"回顾"现象。观察到对无关区域的持续高关注噪声，以及对文本对齐区域逐渐增强的关注。提出的对比关注框架在多种MLLMs和基准测试中验证了分析结果，并提升了多模态推理性能。

Conclusion: 该研究首次系统揭示了MLLMs中视觉-文本融合的层级演化机制，发现了融合的特定层分布和"回顾"现象，提出的训练免费对比关注框架为理解和改进多模态推理提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D虚拟形象通常只有六种基本表情，缺乏真实的情感变化模拟能力。需要能够根据文本指令生成面部表情序列，并在任意两个表情之间创建平滑过渡，以扩展面部表情库并实现更自然的情感表达

Method: 1. 提出指令驱动面部表情分解器（IFED）模块，用于多模态数据学习并捕获文本描述与面部表情特征之间的相关性；2. 提出指令到面部表情转换（I2FET）方法，利用IFED和顶点重建损失函数来细化潜在向量的语义理解，根据给定指令生成面部表情序列；3. 提出面部表情过渡模型，用于生成面部表情之间的平滑过渡

Result: 在CK+和CelebV-HQ数据集上的广泛评估表明，所提模型优于现有最先进方法。结果显示该框架能够根据文本指令生成面部表情轨迹。文本提示允许对人类情感状态进行多样化描述，从而大大扩展了面部表情库及其之间的过渡

Conclusion: 提出的框架能够根据文本指令生成3D面部表情序列和表情间平滑过渡，扩展了面部表情的表达能力，有望在各种实际应用中找到用途
...
TL;DR: 本文提出了一种用于实时路边交通违规分析的边缘AI感知节点，集成YOLOv8 Nano、DeepSORT和OCR引擎，在NVIDIA Jetson Nano上实现高效能违规检测与安全事件发布。


<details>
  <summary>Details</summary>
Motivation: 印度等新兴经济体快速机动化导致执法不对称，2023年记录违规超1100万起，而警力密度仅为每4000辆车一名警官。传统监控和人工罚单无法应对此规模，需要自主、协同且节能的边缘AI感知基础设施。

Method: 系统集成YOLOv8 Nano进行高精度多目标检测，DeepSORT实现时序一致车辆跟踪，规则引导的OCR后处理引擎识别退化或多语言车牌。部署于NVIDIA Jetson Nano，通过TensorRT FP16量化优化，无需手动ROI校准。

Result: 系统在9.6W功耗下维持28-30FPS推理，违规检测准确率97.7%，OCR精度84.9%（覆盖5类违规：闯信号、斑马线违规、逆行、非法掉头、超速）。相比YOLOv4 Tiny等模型，mAP提升10.7%，精度/瓦特比提高1.4倍。

Conclusion: 路边边缘AI分析节点可通过V2X协议向联网车辆和ITS后端发布标准化安全事件（CAM/DENM类型），增强协同感知和主动道路安全管理，适用于IEEE智能车辆生态系统。
...
TL;DR: 论文提出了ROAD基准测试，用于评估LiDAR点云分类在同时面临域偏移和标签演化（类分裂、类扩展、类插入）时的持续学习能力，填补了3D感知领域的研究空白。


<details>
  <summary>Details</summary>
Motivation: 3D感知系统在现实应用中需要适应不断演化的物体定义和传感器域，但当前3D点云感知的持续学习和迁移学习研究相对不足，特别是在同时面临域偏移和标签偏移的情况下。

Method: 提出了ROAD基准测试，使用大规模数据集（Waymo、NuScenes、Argoverse2）评估零样本迁移、线性探测和持续学习方法，分析骨干架构、训练目标和持续学习方法的影响。

Result: 研究发现现有方法在现实偏移下存在局限性，为未来鲁棒3D感知研究建立了强基线。

Conclusion: ROAD基准测试填补了3D点云感知在同时面临域偏移和标签演化场景下的评估空白，揭示了现有方法的不足，为未来研究提供了重要基准。
...
TL;DR: Motive是一个基于梯度的运动归因框架，用于识别视频生成模型中影响运动动态的数据，并通过数据筛选提升视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型发展迅速，但数据如何影响运动动态的理解仍然不足。现有方法主要关注视觉外观而非运动动态，需要专门的运动归因框架来分析数据对时间动态的影响。

Method: 提出Motive框架：1）使用基于梯度的数据归因方法；2）通过运动加权损失掩码将时间动态与静态外观分离；3）实现高效可扩展的运动特定影响计算；4）应用于文本到视频模型，识别对运动有强影响的训练片段。

Result: 1）成功识别出显著影响运动动态的训练片段；2）基于Motive筛选的高影响力数据，在VBench上同时提升了运动平滑度和动态程度；3）与预训练基础模型相比，获得了74.1%的人类偏好胜率。

Conclusion: Motive是首个专门针对视频生成模型中运动动态而非视觉外观的归因框架，能够有效指导数据筛选，显著提升生成视频的时间一致性和物理合理性。
...
TL;DR: 3DGS-Drag：基于3D高斯泼溅的点式3D编辑框架，支持通过直观拖拽操作高效编辑真实3D场景的几何内容


<details>
  <summary>Details</summary>
Motivation: 2D图像已实现直观的拖拽编辑，但3D场景的几何编辑仍具挑战性。现有3D编辑方法存在局限：变形方法难以处理复杂几何修改，2D编辑方法缺乏3D一致性。需要一种能结合两者优势的解决方案。

Method: 采用双引导机制：1) 基于3D高斯泼溅的变形引导实现一致的几何修改；2) 扩散引导用于内容校正和视觉质量提升。结合渐进式编辑策略支持激进拖拽操作。支持运动变化、形状调整、修复和内容扩展等多种编辑类型。

Result: 在多种场景中验证有效性，在几何相关的3D内容编辑任务上达到SOTA性能。编辑效率高，单张RTX 4090 GPU仅需10-20分钟。

Conclusion: 3DGS-Drag成功填补了变形基和2D编辑基3D编辑方法之间的空白，实现了高效、直观的3D场景拖拽编辑，为3D内容创作提供了新范式。
...
TL;DR: AKT模型通过PKAN模块和PAA注意力机制，在无人机高分辨率图像中实现玉米点级定位，性能优于现有方法且计算效率更高


<details>
  <summary>Details</summary>
Motivation: 解决无人机高分辨率图像中玉米点级定位的三大挑战：1）极小的目标像素比（<0.1%）；2）超高分辨率图像（>3000×4000）二次注意力的计算成本过高；3）农业场景特有的稀疏目标分布和环境变化问题

Method: 提出加性Kolmogorov-Arnold Transformer（AKT）：1）用Pade Kolmogorov-Arnold Network（PKAN）模块替代传统多层感知机，增强小目标特征提取的功能表达能力；2）引入PKAN加性注意力（PAA）建模多尺度空间依赖关系并降低计算复杂度；3）构建包含1,928张高分辨率无人机图像和约501,000个点标注的PML数据集

Result: AKT平均F1分数达62.8%，优于最先进方法4.2%，同时减少12.6%的FLOPs并提高20.7%的推理吞吐量；在下游任务中，植株计数平均绝对误差为7.1，株距估计均方根误差为1.95-1.97厘米

Conclusion: 将Kolmogorov-Arnold表示理论与高效注意力机制相结合，为高分辨率农业遥感提供了一个有效的框架，显著提升了无人机图像中玉米点级定位的性能和效率
...
<details>
  <summary>Details</summary>
Motivation: 开发一个更高效的视觉搜索处理模型，通过减少自由参数来简化系统，为医学图像感知、计算机视觉、目标检测等应用提供优化框架

Method: 基于统计理想观察者模型，通过设置最小可提取图像特征阈值来实现整体视觉搜索处理，从而减少模型中的自由参数

Result: 创建了一个参数更少、系统更紧凑的新框架，适用于医学图像感知、计算机视觉、性能基准测试、特征选择评估等多个领域

Conclusion: 该统计理想观察者模型为视觉搜索处理提供了更高效的框架，在医学成像、计算机视觉、国防安全等领域具有广泛的应用前景
...
TL;DR: 该研究探讨了Gabor特征和GLCM纹理特征在建模早期视觉搜索行为中的关系，提出了两种特征融合管道来预测人类注视区域，并在模拟乳腺断层合成图像上验证了其有效性。
...
Method: 提出了两种特征融合管道来整合Gabor和GLCM特征，以缩小可能的人类注视区域范围。使用模拟数字乳腺断层合成图像进行评估，并与基于阈值的模型观察者进行比较。同时收集人类观察者的眼动追踪数据进行分析。

Result: 结果显示，提出的特征融合管道预测的注视候选区域与基于阈值的模型观察者具有定性一致性。观察到GLCM均值与Gabor特征响应之间存在强相关性，表明这些特征尽管形式不同，但编码了相关的图像信息。人类观察者的眼动数据进一步表明预测的注视区域与早期注视行为具有一致性。

Conclusion: 这些发现强调了结合结构特征和纹理特征对于建模视觉搜索的价值，并支持开发感知信息化的观察者模型。特征融合方法能够有效预测人类在复杂图像搜索任务中的早期注视行为。
...
TL;DR: CASHEW和CASHEW-RL：两种提升视觉语言模型多步推理稳定性的方法，通过聚合多个推理轨迹并进行视觉验证来提高预测一致性


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解和推理任务中表现优异，但其多步推理过程不稳定，相同输入下多次采样会产生不同的推理轨迹和不一致的最终预测

Method: 提出两种互补方法：1) CASHEW推理时框架，通过迭代聚合多个候选轨迹形成高质量推理路径，利用显式视觉验证过滤幻觉步骤；2) CASHEW-RL学习变体，通过Group Sequence Policy Optimization训练，使用复合奖励函数鼓励基于最小充分视觉证据的正确答案

Result: 在13个图像理解、视频理解和视频推理基准测试中取得显著性能提升，包括ScienceQA上+23.6个百分点和EgoSchema上+8.1个百分点的增益

Conclusion: CASHEW和CASHEW-RL通过稳定多步推理过程，显著提升了视觉语言模型的推理一致性和性能，特别是在需要复杂推理的任务中
...
TL;DR: 提出基于解码器的深度学习框架，用于生成符合增材制造约束的可制造3D结构


<details>
  <summary>Details</summary>
Motivation: 传统3D生成方法往往忽略制造约束，导致生成的结构无法实际打印。需要开发能同时考虑几何有效性和制造可行性的智能生成方法。

Method: 采用基于解码器的深度学习框架，将潜在表示解码为几何有效且可打印的3D对象。框架学习从抽象表示到有效3D几何的复杂映射函数，同时尊重悬垂角度、壁厚和结构完整性等制造约束。

Result: 方法在多种对象类别上验证有效，相比朴素生成方法显著提高了可制造性。成功实现了解码器生成结构的实际3D打印。

Conclusion: 神经解码器能够学习复杂映射，生成符合制造约束的3D几何结构，为增材制造提供了一种智能化的设计生成方法。
...
TL;DR: 提出一种无需训练、仅基于视觉的零样本异常检测框架DIVAD，通过预训练DDIM模型的反向过程重构图像，利用输入图像与重构图像之间的差异来检测和定位异常，无需细粒度提示或辅助模态。


<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法要么依赖语言模态生成细粒度提示进行定位，要么仅支持图像级分类缺乏空间精度。需要开发无需辅助模态、不依赖细粒度提示的纯视觉零样本异常检测方法。

Method: 基于预训练去噪扩散隐式模型（DDIM），给定输入图像和通用文本描述，通过反向过程获取潜在表示，从固定中间时间步开始去噪重构图像。由于扩散模型仅在正常数据上训练，重构结果呈现正常外观，输入与重构之间的差异揭示异常。

Result: 在VISA数据集上达到最先进性能，展示强大的异常定位能力，无需辅助模态，推动零样本异常检测研究摆脱对提示的依赖。

Conclusion: 提出的DIVAD框架为纯视觉零样本异常检测提供了一种简单有效的解决方案，通过扩散模型反向重构实现精确异常定位，减少对语言提示的依赖，为相关研究开辟新方向。
...
TL;DR: FigEx2是一个视觉条件化框架，用于从科学复合图中定位面板并生成面板级描述，通过噪声感知门控融合模块和强化学习优化实现多模态一致性，在跨学科领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 科学复合图将多个带标签的面板组合成单个图像，但实际流程中的标题常常缺失或仅提供图级摘要，这使得面板级理解变得困难。需要直接从复合图中定位面板并生成面板级描述。

Method: 提出FigEx2视觉条件化框架，包含噪声感知门控融合模块来过滤标记级特征以稳定检测查询空间。采用分阶段优化策略，结合监督学习和强化学习，利用CLIP对齐和BERTScore语义奖励来强制严格的多模态一致性。

Result: FigEx2在检测方面达到0.726 mAP@0.5:0.95的优越性能，在METEOR和BERTScore上显著优于Qwen3-VL-8B（分别高出0.51和0.24）。在物理和化学跨学科测试套件上表现出色，且无需微调即可实现出色的零样本迁移到分布外科学领域。

Conclusion: FigEx2通过创新的噪声感知融合模块和强化学习优化策略，有效解决了科学复合图的面板定位和描述生成问题，在跨学科领域展现出强大的零样本迁移能力，为科学图像理解提供了高质量解决方案。
...
TL;DR: 首个结合视觉语言引导的生物医学图像伪造生成与检测框架，通过扩散模型合成和视觉语言提示实现语义可控的伪造操作，并建立了大规模基准数据集和结构化状态空间建模检测方法。
...
Method: 提出首个视觉语言引导框架，结合扩散基合成与视觉语言提示，实现包括复制、拼接和区域移除在内的语义可控生物医学图像伪造。引入Rescind大规模基准数据集，并提出Integscan结构化状态空间建模框架，集成注意力增强的视觉编码与提示条件语义对齐，实现精确伪造定位。通过基于视觉语言模型的验证循环确保语义保真度。

Result: 在Rescind和现有基准上的大量实验表明，Integscan在检测和定位方面均达到最先进性能，为自动化科学完整性分析奠定了坚实基础。

Conclusion: 该研究建立了首个结合生成与检测的生物医学图像伪造分析框架，通过视觉语言引导实现语义可控的伪造操作和精确检测，为解决生物医学出版物中的图像篡改问题提供了有效工具。
...
TL;DR: 在CIFAR-10数据集上，通过在训练数据中策略性地引入10%的噪声数据（高斯噪声、椒盐噪声、高斯模糊），可以显著提升Resnet-18模型在完全噪声测试条件下的性能，同时对干净数据性能影响最小。


<details>
  <summary>Details</summary>
Motivation: 虽然高质量数据通常用于CNN训练，但现实世界输入常受噪声和失真影响。研究通过故意在训练数据中引入受控噪声来提升模型鲁棒性，探索数据质量与模型性能之间的关系。

Method: 使用CIFAR-10数据集，评估三种常见噪声（高斯噪声、椒盐噪声、高斯模糊）在不同强度和训练集污染水平下的影响。采用Resnet-18模型进行实验，分析不同噪声比例对模型性能的影响。

Result: 实验表明，在训练中仅加入10%的噪声数据就足以显著降低测试损失，并在完全噪声测试条件下提高准确率，同时对干净数据性能影响最小。噪声暴露可作为有效的正则化方法。

Conclusion: 策略性地暴露于噪声可以作为简单而有效的正则化器，在传统数据清洁度和现实世界鲁棒性之间提供实用的权衡，为提升模型在噪声环境下的性能提供了可行方案。
...
TL;DR: 提出DINO-AugSeg框架，利用DINOv3特征解决少样本医学图像分割问题，通过小波特征增强和上下文引导融合提升性能
...
Method: 提出DINO-AugSeg框架：1) WT-Aug小波特征增强模块，通过扰动频率分量丰富DINOv3特征多样性；2) CG-Fuse上下文引导融合模块，利用交叉注意力整合语义丰富的低分辨率特征和空间细节丰富的高分辨率特征
...
Method: 三阶段框架：1) 通过控制修复在领域特定背景中合成目标对象；2) 多模态评估验证生成输出，包括目标检测、美学评分和视觉语言对齐；3) 使用用户偏好分类器捕获主观选择标准

Result: 能够高效构建高质量、可部署的数据集，同时减少对广泛真实世界数据收集的依赖

Conclusion: 提出的自动化管道为生成领域特定合成数据集提供了有效解决方案，解决了分布偏移问题，提高了数据集构建的效率和实用性
...
TL;DR: PathoGen：基于扩散模型的病理图像生成框架，通过可控高保真病灶修复增强数据稀缺场景下的AI模型训练


<details>
  <summary>Details</summary>
Motivation: 组织病理学诊断AI模型开发面临专家标注病灶数据稀缺的瓶颈，特别是罕见病理和代表性不足的疾病亚型。现有数据增强方法无法生成足够真实的病灶形态，难以保持组织病理学特有的复杂空间关系和细胞结构特征。

Method: 提出PathoGen扩散生成模型，利用扩散模型的迭代细化过程，在良性组织病理图像中实现可控、高保真的病灶修复。该方法能够合成具有自然组织边界、保留细胞结构和真实染色特征的病灶。

Result: 在肾脏、皮肤、乳腺和前列腺四个不同诊断挑战的数据集上验证，定量评估显示PathoGen在图像保真度和分布相似性上优于条件GAN和Stable Diffusion等先进生成基线。使用PathoGen合成病灶增强训练集能提升下游分割性能，特别是在数据稀缺情况下优于传统几何增强方法。

Conclusion: PathoGen通过同时生成真实形态和像素级标注，有效克服了人工标注瓶颈，为在有限专家标注数据下开发可泛化的医疗AI系统提供了可扩展的途径。
...
<details>
  <summary>Details</summary>
Motivation: AVSS任务需要超越简单的发声物体识别，实现音频-视觉场景的语义理解。现有方法将任务分解为两个子任务，但需要更有效的多模态融合策略来处理运动物体和静止发声物体的挑战。

Method: 提出SSP协作框架：1) 使用预掩码技术结合光流捕捉运动动态；2) 引入两种文本提示（物体类别和场景描述）处理静止发声物体；3) 设计视觉-文本对齐模块实现跨模态融合；4) 采用后掩码技术训练模型学习光流模式。

Result: 实验结果表明SSP超越了现有的AVS方法，提供了高效且精确的分割结果。

Conclusion: SSP框架通过整合光流和文本提示，有效解决了AVSS任务中的运动物体和静止发声物体分割挑战，实现了更好的语义理解和分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法依赖零样本预测作为伪标签进行自训练，但在分布偏移下不可靠，主要受两个限制：模态间隙（视觉和文本模态间存在差距）和视觉干扰（视觉嵌入包含任务无关噪声）

Method: SubTTA通过语义子空间对齐增强零样本预测：1）提取两个模态的主子空间，通过最小化弦距离将视觉流形对齐到文本语义锚点；2）将对齐的视觉特征投影到任务特定的文本子空间，过滤任务无关噪声；3）在净化空间上执行标准TTA以细化决策边界

Result: 在多个基准测试和VLM架构上的广泛实验表明，SubTTA平均比最先进的TTA方法提升2.24%

Conclusion: 通过语义子空间对齐解决模态间隙和视觉干扰问题，能够显著提升视觉语言模型在分布偏移下的测试时适应性能
...
Method: 提出实例对齐字幕框架，将每个文本声明链接到特定对象实例，包含外观和运动属性。该方法能够识别谁导致了异常、每个实体在做什么、影响了谁以及解释的空间定位，实现可验证和可操作的推理。

Result: 对八个广泛使用的VAD基准进行了标注，并扩展了360度第一人称数据集VIEW360，增加了868个视频、8个位置和4种新异常类型，创建了VIEW360+综合测试平台。实验表明，实例级空间定位字幕揭示了当前基于LLM和VLM方法的显著局限性，为未来可信和可解释的异常检测研究提供了稳健基准。

Conclusion: 提出的实例对齐字幕方法为可解释视频异常检测提供了可验证的空间定位解释，解决了现有方法在多实体交互场景中的局限性，为构建更可信的异常检测系统奠定了基础。
...
TL;DR: 提出硬件-算法协同设计框架，结合定制空间可变曝光传感器与物理感知去雾算法，解决火箭发射极端成像条件下光学测量的挑战


<details>
  <summary>Details</summary>
Motivation: 火箭发射期间关键机械参数（如羽流场、冲击波结构、喷嘴振荡）的光学测量面临极端成像条件挑战：强烈燃烧产生密集颗粒雾霾和超过120dB的亮度变化，导致图像数据质量下降，影响后续摄影测量和速度测量分析

Method: 硬件-算法协同设计框架：1) 定制空间可变曝光传感器，单次拍摄获取多曝光数据，实现鲁棒雾霾评估；2) 物理感知去雾算法，动态估计雾霾密度，执行区域自适应照明优化，应用多尺度熵约束融合有效分离雾霾与场景辐射

Result: 在真实发射图像和受控实验中验证，该框架在恢复羽流和发动机区域的物理准确视觉信息方面表现出优越性能，为提取粒子速度、流动不稳定频率和结构振动等关键机械参数提供可靠图像基础

Conclusion: 该硬件-算法协同设计框架支持极端航空航天环境中的精确定量分析，通过有效解决极端成像条件下的雾霾和亮度变化问题，为关键机械参数的光学测量提供可靠解决方案
...
TL;DR: SISTA框架通过多级对齐改进医学视觉语言预训练，消除假阴性并实现图像-报告和补丁-单词对齐，提升下游任务性能
...
Method: 提出SISTA多级对齐框架：1) 在图像-报告层面，通过考虑报告间相似性消除假阴性；2) 在补丁-单词层面，有效对齐图像补丁与相关词标记

Result: 实验表明该框架在不同数据集的下游任务（图像分类、分割、目标检测）中提升迁移性能，特别是在有限标注数据的细粒度任务上取得显著改进

Conclusion: SISTA通过语义感知的实例和稀疏标记对齐，改进了医学视觉语言预训练，代码和预训练模型将公开
...
TL;DR: 基于PVRCNN++框架，通过数据增强和自训练策略改进跨平台3D目标检测，在RoboSense2025挑战赛中获得第三名
...
Method: 基于PVRCNN++框架，采用定制化数据增强缩小领域差距，结合伪标签自训练策略
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够模拟人类认知过程的动态3D场景理解系统，解决传统方法在连续场景理解和多次访问场景中的局限性，实现更智能的空间感知和记忆机制。

Method: 集成三个核心能力：1）多阶段运动线索框架识别动态对象；2）认知映射系统存储、回忆和更新多次访问的静态场景；3）因子图优化策略精化相机位姿。利用深度和相机位姿先验识别动态区域，匹配静态元素与记忆库。

Result: 在视频深度估计、相机位姿重建和3D映射任务中展示了最先进的性能，有效支持跨长时间序列和多次访问的连续场景理解。

Conclusion: CogniMap3D通过模仿人类认知过程，实现了高效的动态3D场景理解与重建，为智能空间感知系统提供了新的生物启发式解决方案。
...
TL;DR: 本文针对Zhang等人简单角点模型的理论缺陷，提出使用二阶高斯方向导数滤波器平滑高分辨率角点模型，推导出角点特征表示，并首次提出能准确检测相邻角点的高分辨率角点检测方法。


<details>
  <summary>Details</summary>
Motivation: 现有角点检测方法存在理论缺陷，特别是Zhang等人使用的简单角点模型忽略了相邻角点灰度信息的相互影响，导致在高分辨率图像中无法准确检测相邻角点。

Method: 使用二阶高斯方向导数(SOGDD)滤波器平滑两种典型高分辨率角点模型（END型和L型），推导出这些模型的SOGDD表示，发现高分辨率角点的多种特征，并提出新的高分辨率角点检测方法。

Result: 实验验证表明，所提方法在定位误差、对图像模糊变换的鲁棒性、图像匹配和3D重建方面均优于现有最先进方法。

Conclusion: 通过SOGDD滤波器处理高分辨率角点模型，能够准确获取相邻角点的强度变化信息，提出的新方法在多个性能指标上优于现有方法，解决了相邻角点检测的难题。
...
TL;DR: 多模态大语言模型在胃肠病学中展现出潜力，但在完整临床工作流程中的表现尚未得到验证。研究构建了GI-Bench基准测试，评估了12个MLLM在五阶段内镜工作流程中的表现，并与人类内镜医师进行对比。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在胃肠病学中显示出应用前景，但其在完整临床工作流程中的表现以及与人类专家的对比尚未得到系统验证。需要评估MLLM在胃肠内镜工作流程中的临床实用性。

Method: 构建了GI-Bench基准测试，涵盖20个细粒度病变类别。评估了12个MLLM在五阶段临床工作流程中的表现：解剖定位、病变识别、诊断、发现描述和管理。使用Macro-F1、平均交并比和多维度Likert量表将模型性能与3名初级内镜医师和3名住院医师进行对比。

Result: Gemini-3-Pro达到最先进性能。在诊断推理方面，顶级模型（Macro-F1 0.641）优于住院医师（0.492），与初级内镜医师相当（0.727；p>0.05）。但存在关键的"空间定位瓶颈"：人类病变定位（mIoU >0.506）显著优于最佳模型（0.345；p<0.05）。定性分析揭示了"流畅性-准确性悖论"：模型生成的报告在语言可读性上优于人类（p<0.05），但事实准确性显著较低（p<0.05），主要由于"过度解读"和视觉特征幻觉。

Conclusion: MLLM在胃肠内镜诊断推理方面已接近初级内镜医师水平，但在空间定位能力上仍存在显著瓶颈。模型存在流畅性-准确性悖论，即生成文本流畅但事实准确性不足。GI-Bench建立了动态排行榜，持续追踪MLLM在临床内镜中的性能演进。
...
<details>
  <summary>Details</summary>
Motivation: 多站点脑MRI数据聚合可增强深度学习模型训练，但会引入站点特异性变化（如扫描仪厂商、采集参数、成像协议差异）导致的非生物异质性，这会损害模型泛化能力。现有方法依赖有限的配对旅行者数据，难以有效分离风格与解剖结构，且大多仅处理单序列协调，限制了在实际多序列MRI场景中的应用。

Method: MMH采用两阶段框架：1）基于扩散的全局协调器，使用风格无关的梯度条件将MRI映射到序列特定的统一域；2）目标特定的微调器，将全局对齐图像适配到期望的目标域。使用三平面注意力BiomedCLIP编码器聚合多视图嵌入来表征体积风格信息，实现图像风格与解剖结构的显式分离。

Result: 在4,163个T1和T2加权MRI上的评估表明，MMH在图像特征聚类、体素级比较、组织分割以及下游年龄和站点分类任务中，均优于现有最先进方法。

Conclusion: MMH提供了一个有效的多站点多序列脑MRI协调框架，能够在不依赖配对数据的情况下实现风格与解剖结构的有效分离，显著提升了跨站点MRI数据的协调效果和下游任务性能。
...
TL;DR: MobiDiary是一个从IMU和Wi-Fi信号生成日常活动自然语言描述的多模态框架，通过统一传感器编码器和Transformer解码器实现跨模态泛化，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 智能家居中的人类活动识别对健康监测和辅助生活至关重要。基于视觉的系统存在隐私问题和环境限制（如遮挡），而传统方法局限于预定义标签，缺乏表达性。需要从物理信号直接生成自然语言描述来解决这些问题。

Method: 提出MobiDiary框架：1）使用统一传感器编码器处理IMU和Wi-Fi信号，利用运动诱导信号的共享归纳偏差；2）采用基于补丁的机制捕获局部时间相关性；3）集成异构放置嵌入统一不同传感器的空间上下文；4）将统一信号标记输入基于Transformer的自回归解码器，逐词生成连贯的活动描述。

Result: 在多个公共基准测试（XRF V2、UWash和WiFiTAD）上评估，MobiDiary在字幕生成指标（BLEU@4、CIDEr、RMC）上达到最先进性能，在连续动作理解方面优于专用基线，有效实现跨模态泛化。

Conclusion: MobiDiary成功地从异构物理信号生成自然语言活动描述，解决了视觉系统的隐私和环境限制问题，通过统一编码器有效桥接了物理信号与语言描述之间的语义鸿沟，为智能家居活动识别提供了新的解决方案。
...
TL;DR: 该研究分析了基于Vision Transformer(EVA-ViT)图像编码器的多模态方法，结合LlaMA或ChatGPT LLM，用于减少幻觉问题并检测胸部X光图像中的疾病。研究使用NIH胸部X光图像训练模型，并在图像检索增强生成(RAG)、文本检索增强生成和基线方法中进行比较。


<details>
  <summary>Details</summary>
Motivation: 研究的主要动机是解决多模态医学图像分析中的幻觉问题，特别是在结合视觉编码器和大型语言模型时。通过使用外部知识信息来减少模型生成错误或不可靠信息的问题，提高胸部X光疾病检测的准确性和可靠性。

Method: 研究方法包括：1) 使用EVA-ViT作为图像编码器；2) 结合LlaMA或ChatGPT LLM进行多模态分析；3) 采用三种不同方法：图像检索增强生成(RAG)、文本检索增强生成和基线方法；4) 使用NIH胸部X光图像数据集进行训练；5) 在图像RAG中使用KNN方法改进预测置信度和校准。

Result: 研究结果显示：1) 文本RAG通过使用外部知识信息有效减少了幻觉问题；2) 图像RAG通过KNN方法提高了预测置信度和校准；3) GPT LLM相比LlaMA LLM表现出更好的性能、更低的幻觉率和更好的预期校准误差(ECE)。

Conclusion: 该研究揭示了数据不平衡和复杂多阶段结构的挑战，但提出了大规模实验环境和平衡使用示例的建议。研究表明，结合外部知识的检索增强生成方法可以有效减少多模态医学图像分析中的幻觉问题，提高模型的可靠性和性能。
...
Result: 事件分割在复杂真实数据集上持续优于时间分割的LLM方法，甚至超越监督数据驱动方法；置信度度量能有效区分正确与错误预测

Conclusion: 事件分割与置信度估计相结合能显著提升零样本ADL识别性能，即使使用相对较小的LLM也能取得优异结果
...
TL;DR: HIPPO提出了一种面向视频-LLM的整体感知并行推测解码框架，通过语义感知的token保留和并行解码算法，在保持输出质量的同时实现高达3.51倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有面向视频-LLM的推测解码方法主要通过剪枝冗余视觉token来加速，但存在两个主要限制：1) 剪枝策略未能充分保留视觉语义token，降低了草稿质量和接受率；2) 即使进行激进剪枝，草稿模型的剩余推理成本仍限制了整体加速效果。

Method: HIPPO包含两个核心技术：1) 语义感知token保留方法，融合全局注意力分数与局部视觉语义，在高剪枝率下保留语义信息；2) 视频并行推测解码算法，解耦并重叠草稿生成和目标验证阶段。

Result: 在四个视频-LLM和六个基准测试上的实验表明，HIPPO相比原始自回归解码实现了高达3.51倍的加速，同时保持了输出质量。

Conclusion: HIPPO通过解决现有视频-LLM推测解码方法的局限性，实现了与纯文本LLM相当的推理加速效果，为视频-LLM的高效推理提供了有效的解决方案。
...
TL;DR: 论文引入KidVis基准，基于人类视觉发展理论评估MLLMs的基础视觉能力，发现即使最先进的GPT-5在6-7岁儿童掌握的六种原子视觉能力上表现显著落后，且存在"缩放定律悖论"。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在高级推理任务上表现出色，但尚不清楚它们是否具备与人类直觉相当的基础视觉原语能力。研究旨在通过人类视觉发展理论来评估MLLMs的基础视觉智能。
...
Conclusion: 尽管MLLMs在复杂推理任务上表现优异，但它们缺乏人类儿童已掌握的基础视觉原语能力，这限制了其实现真正的广义视觉智能。需要新的方法来解决这一基础能力差距。
...
Conclusion: M3SR通过多尺度、多感知的Mamba架构成功解决了光谱重建中的感知和尺度限制问题，在性能和计算效率方面均表现出色。
...
TL;DR: IQARAG是一个无需训练的新框架，利用检索增强生成(RAG)技术提升大型多模态模型在图像质量评估任务中的性能，通过检索语义相似但质量不同的参考图像为模型提供视觉感知锚点。


<details>
  <summary>Details</summary>
Motivation: 虽然大型多模态模型在图像质量评估任务中展现出强大的零样本能力，但要达到最先进性能通常需要计算成本高昂的微调方法。本文旨在开发一种无需训练的资源高效替代方案。

Method: 提出IQARAG框架，包含三个关键阶段：1)检索特征提取，2)图像检索，3)集成与质量分数生成。通过RAG检索语义相似但质量不同的参考图像及其平均意见分数，将这些信息整合到特定提示中，为LMM提供视觉感知锚点。

Result: 在KADID、KonIQ、LIVE Challenge和SPAQ等多个多样化IQA数据集上的广泛实验表明，IQARAG有效提升了LMM的IQA性能，提供了比微调更资源高效的替代方案。

Conclusion: IQARAG是一个无需训练的新框架，通过检索增强生成技术显著提升大型多模态模型在图像质量评估任务中的性能，为质量评估提供了资源高效的替代方案。
...
TL;DR: 提出YOLOBirDrone架构用于提高无人机和鸟类的检测分类精度，通过AELAN、MPDA和RMPDA模块增强特征提取，并引入BirDrone大规模数据集，检测准确率达到约85%


<details>
  <summary>Details</summary>
Motivation: 无人机在商业和国防应用中广泛使用，但也被用于针对性攻击，带来安全隐患。现有视觉无人机检测系统精度有限，难以区分无人机和小型鸟类，需要开发更准确的检测系统

Method: 提出YOLOBirDrone架构，包含自适应扩展层聚合(AELAN)、多尺度渐进双注意力模块(MPDA)和反向MPDA(RMPDA)，以保持形状信息并丰富局部和全局空间及通道特征。同时引入BirDrone大规模数据集，包含小型和挑战性目标

Result: 实验结果表明，YOLOBirDrone相比其他最先进算法在性能指标上有显著提升，在各种场景下的检测准确率达到约85%

Conclusion: YOLOBirDrone架构能有效提高无人机和鸟类的检测分类精度，解决了现有系统区分小型鸟类和无人机的难题，为无人机检测系统提供了更可靠的解决方案
...
Method: 提出了UM-Text统一多模态模型，包含：1）视觉语言模型（VLM）处理指令和参考图像，根据上下文信息精心设计文本内容和布局；2）UM-Encoder结合各种条件信息的嵌入，由VLM根据输入指令自动配置组合；3）区域一致性损失在潜在空间和RGB空间为字形生成提供更有效的监督；4）量身定制的三阶段训练策略；5）贡献了UM-DATA-200K大规模视觉文本图像数据集。

Result: 在多个公共基准测试上的广泛定性和定量结果表明，该方法实现了最先进的性能。

Conclusion: UM-Text通过统一的架构解决了视觉文本编辑中的风格一致性问题，结合VLM的上下文理解、UM-Encoder的条件信息融合、区域一致性损失和精心设计的训练策略，在视觉文本生成任务上取得了优异表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法在单图像超分辨率中虽然性能优异，但计算成本高。为提升效率，多数方法采用固定分组注意力，但这种组内注意力忽略了token相似性的固有不对称性，无法实现灵活的自适应注意力计算。

Method: 提出个体化探索Transformer(IET)，引入个体化探索注意力(IEA)机制，允许每个token自适应选择其自身的内容感知和独立的注意力候选，实现token自适应和非对称设计。

Result: 在标准超分辨率基准测试上的大量实验表明，IET在可比较的计算复杂度下实现了最先进的性能。

Conclusion: IET通过个体化探索注意力机制解决了现有固定分组注意力的局限性，实现了更精确的信息聚合，同时保持了计算效率，为单图像超分辨率任务提供了有效的解决方案。
...
TL;DR: 该论文系统研究了视觉语言模型在感知退化下的语义错位问题，发现即使分割指标下降不大，VLM也会出现严重安全风险，揭示了像素级鲁棒性与语义可靠性之间的脱节。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶和具身AI系统中，视觉语言模型被广泛部署，其感知可靠性对安全语义推理和决策至关重要。虽然现有VLM在多模态基准上表现良好，但它们在真实感知退化下的鲁棒性仍未被充分理解。

Method: 使用Cityscapes数据集上的语义分割作为代表性感知模块，引入感知现实的退化方法，这些退化仅导致传统分割指标适度下降。提出一套语言级错位指标来量化幻觉、关键遗漏和安全误判，并分析这些指标与分割质量在多个对比式和生成式VLM中的关系。

Result: 研究发现，即使上游视觉感知只有适度退化，下游VLM行为也会出现严重故障，包括幻觉对象提及、安全关键实体遗漏和不一致的安全判断。揭示了像素级鲁棒性与多模态语义可靠性之间的明显脱节。

Conclusion: 当前基于VLM的系统存在关键局限性，需要在安全关键应用中开发能明确考虑感知不确定性的评估框架。研究强调了感知退化对VLM语义可靠性的严重影响，为改进系统安全性提供了重要见解。
...
TL;DR: Geo-NVS-w是一个几何感知的新视角合成框架，专门处理非结构化野外图像集合，通过SDF几何表示和几何保持损失实现高质量渲染，同时显著降低能耗。
...
TL;DR: 提出LoGo框架用于地理空间点云的无源无监督域适应，通过局部类平衡原型估计和全局最优传输分布对齐解决长尾分布问题


<details>
  <summary>Details</summary>
Motivation: 地理空间点云语义分割面临跨区域域偏移问题，现有域适应方法需要源域数据，但由于隐私、法规和传输限制，源域数据通常不可用，因此需要研究无源无监督域适应（SFUDA）

Method: 提出LoGo框架：1）局部类平衡原型估计模块，采用类内独立锚点挖掘策略而非全局阈值过滤；2）全局最优传输分布对齐模块，将伪标签分配建模为全局优化问题；3）双重一致性伪标签过滤机制，保留局部多增强集成预测与全局最优传输分配一致的高置信度伪标签

Result: 未在摘要中明确说明具体实验结果，但方法设计旨在解决长尾分布导致的特征崩溃和头类过度主导问题，提高地理空间点云在域偏移下的分割性能

Conclusion: LoGo框架为地理空间点云的无源无监督域适应提供了一种有效解决方案，通过局部-全局双重共识机制缓解长尾分布带来的挑战
...
<details>
  <summary>Details</summary>
Motivation: 解决全景X光片上智齿冠周炎诊断的挑战，通过AI辅助系统整合解剖定位、病理分类和可解释性特征，提高诊断准确性和临床信任度

Method: 采用两阶段深度学习流程：第一阶段使用YOLOv8检测第三磨牙并基于Winter分类法分类其解剖位置和角度；第二阶段使用改进的ResNet-50架构检测提示冠周炎的影像特征；使用Grad-CAM增强可解释性

Result: YOLOv8组件达到92%精确度和92.5%平均精度；ResNet-50分类器对正常病例和冠周炎的F1分数分别为88%和86%；放射科医生报告Grad-CAM与诊断印象有84%一致性
...
Method: 1) 将BLIP-2与YOLO模型深度融合，利用YOLO的精确感知结果进行目标检测和实例分割；2) 设计基于K-Means聚类的内容感知关键帧采样机制，实现智能帧选择和时序特征拼接；3) 实施统一提示优化方案，将YOLO模型的结构化事件日志作为上下文信息注入BLIP-2输入。

Result: 该平台扩展了BLIP-2在无人机应用中的多任务能力，无需在无人机数据上进行任务特定微调，实现了轻量级架构处理视频级交互任务的能力。
...
TL;DR: SPARK是一个自标定的实时多相机点云重建框架，联合处理点云融合和相机外参不确定性，通过几何感知的在线外参估计和置信度驱动的点云融合策略，在动态场景中实现稳定重建。
...
Method: SPARK包含两个核心组件：(1) 几何感知的在线外参估计模块，利用多视图先验并强制跨视图和时间一致性以实现稳定自标定；(2) 置信度驱动的点云融合策略，在像素和点级别建模深度可靠性和可见性，以抑制噪声和视图依赖的不一致性。采用逐帧融合而非累积方式。

Result: 在真实世界多相机系统上的广泛实验表明，SPARK在外参精度、几何一致性、时间稳定性和实时性能方面优于现有方法，证明了其在大规模多相机3D重建中的有效性和可扩展性。

Conclusion: SPARK通过联合处理点云融合和外参不确定性，实现了稳定、实时的大规模多相机3D重建，解决了现有方法在多视图融合、外参不确定性和可扩展性方面的挑战。
...
TL;DR: 提出MMLGNet多模态框架，利用CLIP等视觉语言模型将HSI和LiDAR等异构遥感模态与自然语言语义对齐，通过双向对比学习在共享潜在空间中实现视觉特征与手工文本嵌入的对齐。
...
Method: 采用模态特定编码器，通过双向对比学习在共享潜在空间中对齐视觉特征与手工文本嵌入。受CLIP训练范式启发，使用简单的CNN编码器实现高维遥感数据与语言引导解释的桥接。

Result: 在两个基准数据集上，MMLGNet使用简单的CNN编码器超越了多个已建立的多模态视觉方法，展示了语言监督的显著优势。

Conclusion: 语言监督显著提升了多模态遥感数据的语义理解能力，MMLGNet为异构遥感模态与自然语言语义对齐提供了有效框架，代码已开源。
...
TL;DR: 提出一种基于局部块的面部动画重定向方法，可将源视频中的面部动画转移到风格化3D角色上，保持语义含义
...
Method: 提出三模块方法：自动块提取模块从源视频帧提取局部块；重演模块生成对应的目标局部块；权重估计模块计算每帧动画参数生成完整面部动画序列

Result: 实验表明该方法能成功将源面部表情的语义含义转移到面部特征比例差异较大的风格化角色上

Conclusion: 提出的局部块重定向方法能有效处理风格化3D角色的面部动画转移，保持语义含义，适用于面部结构差异显著的情况
...
TL;DR: 提出CardiacMind框架，通过心脏推理模板(CRT)和强化学习方案增强MLLM在超声心动图诊断中的推理能力，显著提升复杂心脏疾病的多视图诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有超声心动图基础模型未能有效捕捉定量测量与临床表现之间的关系，而医学推理多模态大语言模型(MLLM)需要昂贵的详细推理路径构建，且难以直接融入超声心动图先验知识。

Method: 提出心脏推理模板(CRT)提供复杂心脏疾病的逐步规范化诊断流程，以及CardiacMind强化学习方案，包含三个新颖奖励：程序数量奖励(PQtR)、程序质量奖励(PQlR)和超声心动图语义奖励(ESR)。

Result: 在15种复杂心脏疾病的多视图超声心动图诊断中提升48%，在CardiacNet-PAH上提升5%。用户研究显示93.33%的临床医生同意其推理逻辑与心脏病专家相似。

Conclusion: 通过引入心脏病专家思维模式，CardiacMind框架有效增强了MLLM在超声心动图诊断中的推理能力，为医学影像分析提供了新的强化学习范式。
...
TL;DR: 提出NAR方法，一种噪声自适应正则化方法，用于遥感多标签分类中的标签噪声问题，通过区分加性和减性噪声并采用置信度标签处理机制


<details>
  <summary>Details</summary>
Motivation: 遥感多标签分类中，由于使用专题产品或众包标注降低成本，常引入部分错误标注的标签噪声。现有方法通常将噪声标注视为监督信号，缺乏针对不同噪声类型（加性、减性、混合噪声）的自适应学习机制。

Method: 提出NAR方法：1）在半监督学习框架内明确区分加性和减性噪声；2）采用基于置信度的标签处理机制：高置信度标签保留，中等置信度暂时停用，低置信度通过翻转校正；3）结合早期学习正则化（ELR）稳定训练，减轻对噪声标签的过拟合。

Result: 在加性、减性和混合噪声场景下的实验表明，NAR相比现有方法持续提升鲁棒性。在减性和混合噪声条件下性能提升最为显著，表明自适应抑制和选择性校正噪声监督是有效的噪声鲁棒学习策略。

Conclusion: NAR通过明确区分噪声类型并采用自适应标签处理机制，为遥感多标签分类中的标签噪声问题提供了有效的解决方案，特别是在处理减性和混合噪声时表现出色。
...
TL;DR: 提出静态-动态协作(SDC)框架解决少样本类增量学习中的稳定性-可塑性困境，通过分离静态保留阶段和动态学习阶段实现新旧知识的平衡


<details>
  <summary>Details</summary>
Motivation: 解决少样本类增量学习(FSCIL)中的关键稳定性-可塑性困境：在有限数据下平衡旧知识的保留和新知识的获取

Method: 将FSCIL分为静态保留阶段(SRS)和动态学习阶段(DLS)：SRS阶段使用基础会话充分数据训练初始模型并保存静态记忆；DLS阶段引入动态投影器与静态记忆联合训练

Result: 在三个公共基准数据集和一个真实应用数据集上的实验表明，该方法相比其他竞争者达到了最先进的性能

Conclusion: 通过静态-动态协作框架实现了更好的稳定性-可塑性权衡，改进了旧知识的保留同时持续适应新类别
...
TL;DR: 本研究开发了一个结合稳健特征选择的放射组学框架，用于预测高级别浆液性卵巢癌患者对新辅助化疗的反应，通过整合不同特征选择方法提高了预测性能。


<details>
  <summary>Details</summary>
Motivation: 高级别浆液性卵巢癌通常在晚期诊断，伴有广泛的腹膜转移，治疗困难。约40%患者对新辅助化疗反应有限，需要非侵入性方法预测治疗反应以优化治疗策略。放射组学结合机器学习为预测化疗反应提供了有前景的途径。

Method: 引入了一个选择稳健放射组学特征的框架，采用自动化随机化算法模拟观察者间变异性，平衡特征稳健性和预测准确性。使用四种反应评估指标：化疗反应评分、RECIST标准、体积减少和直径减少。研究不同解剖部位的病灶，使用新辅助化疗前后的CT扫描进行特征提取和模型训练，并在独立队列中进行外部验证。

Result: 最佳预测性能出现在使用所有病灶组合预测体积减少时，AUC达到0.83。网膜病灶在化疗反应评分预测中表现最佳（AUC 0.77），盆腔病灶在直径减少预测中表现最佳（AUC 0.76）。
...
<details>
  <summary>Details</summary>
Motivation: RGB-T检测需要在正常条件下融合模态互补信息，但在极端条件下（某一模态质量差）需要分离模态以避免噪声干扰。现有方法难以平衡这两种需求。

Method: 提出模态解耦的MDQF框架：使用DETR-like检测器作为RGB和TIR的独立分支，在每级细化阶段通过查询选择与适配进行跨分支查询融合，排除退化模态并使用高质量查询修正预测。

Result: 实验表明MDQF优于现有RGB-T检测器，实现更好的模态独立性，且支持非配对RGB/TIR图像训练。

Conclusion: MDQF有效平衡了模态互补与分离，在极端条件下通过查询融合排除退化模态干扰，同时降低了对配对训练数据的需求。
...
<details>
  <summary>Details</summary>
Motivation: 建筑和城市规划中的概念设计阶段（特别是建筑体量设计）复杂且依赖设计师直觉和手动工作，需要数据驱动方法但缺乏合适数据集

Method: 1) 创建CoMa-20K数据集，包含详细体量几何、经济与功能数据、场地视觉表示；2) 将体量生成公式化为视觉语言模型的条件任务；3) 评估微调模型和大型零样本模型

Result: 实验揭示了任务的固有复杂性，同时展示了视觉语言模型生成上下文敏感体量方案的潜力，为数据驱动建筑设计建立了基础基准

Conclusion: CoMa-20K数据集和分析为数据驱动建筑设计提供了基础基准，并突出了未来研究的重要机会
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的分心驾驶检测器在真实场景中表现不佳，主要瓶颈在于模型将驾驶员特定外观特征（如服装、年龄、性别）与行为线索纠缠在一起，导致决策基于"驾驶员是谁"而非"驾驶员在做什么"。

Method: 提出主体解耦框架：1) 提取驾驶员外观嵌入并从图像嵌入中移除其影响，强调与分心相关的证据；2) 通过度量投影到Stiefel流形上正交化文本嵌入，在保持原始语义的同时提高可分离性。

Result: 实验表明该方法相比现有基线方法获得了一致的性能提升，证明了其在道路安全实际应用中的潜力。

Conclusion: 主体解耦框架能有效解决视觉语言模型中外观特征与行为线索纠缠的问题，提高分心驾驶检测的鲁棒性和可扩展性，为实际道路安全应用提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法充分覆盖多样化的危险情况，特别是具有时空动态的异常场景。虽然图像编辑模型有望合成此类危险，但生成包含移动、侵入和远距离物体的真实世界场景仍然具有挑战性。
...
Conclusion: HazardForge能够有效生成包含复杂危险场景的图像，MovSafeBench基准测试揭示了视觉语言模型在异常场景下的性能局限，特别是在运动理解方面存在明显不足。
...
TL;DR: 本文提出了一种基于物联网的智能鱼缸系统，通过集成ESP32微控制器、多种传感器（pH、TDS、温度、浊度）和执行器（伺服喂食器、水泵），实现实时水质监测和自动化控制，显著改善传统手动方法的不足。


<details>
  <summary>Details</summary>
Motivation: 维持鱼缸最佳水质对水生生物健康至关重要，但传统手动监测方法效率低下、劳动密集且容易出错，导致水质条件不理想。需要一种更高效、可靠的自动化解决方案。

Method: 系统采用ESP32微控制器集成pH、TDS、温度和浊度传感器，配合伺服喂食器和水泵执行器。架构包含边缘处理能力、通过Blynk物联网平台的云连接，以及具有可配置冷却期的智能警报机制，防止通知疲劳。

Result: 在10升鱼缸环境中实验评估显示，系统传感器平均准确率达96%，异常检测响应时间为1.2秒。自动喂食和水循环模块在长期测试中保持97%的运行可靠性，显著减少人工干预并确保稳定的水生环境。
...
TL;DR: EfficientFSL是一种专为ViT设计的查询式微调框架，通过引入可训练的前向块、组合块和支持-查询注意力块，在保持高性能的同时大幅减少计算开销和可训练参数。


<details>
  <summary>Details</summary>
Motivation: 尽管Vision Transformers等大型模型在少样本分类中表现出色，但微调这些模型需要大量GPU内存和训练时间，限制了它们在低资源场景中的应用。需要一种既能保持高性能又能显著降低计算开销的解决方案。

Method: 提出EfficientFSL框架，包含三个核心组件：1) 轻量级可训练前向块，用于合成任务特定的查询，以查询方式从预训练模型的中间表示中提取信息特征；2) 组合块，融合多层输出以增强特征表示的深度和鲁棒性；3) 支持-查询注意力块，通过调整原型以对齐查询集分布来缓解分布偏移。

Result: EfficientFSL在四个域内少样本数据集和六个跨域数据集上实现了最先进的性能，同时仅需极少的可训练参数，显著降低了计算开销。

Conclusion: EfficientFSL通过查询式微调框架有效解决了大型ViT模型在少样本分类中的计算效率问题，在保持高性能的同时大幅减少了资源需求，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 小样本类增量学习面临灾难性遗忘问题，现有方法使用外部存储器平等对待所有增量类，无法有效保存先前关键知识

Method: 提出CD²框架，包含数据集蒸馏模块（DDM）和蒸馏约束模块（DCM）。DDM在分类器指导下合成高度压缩的样本，让模型从少量增量样本中学习紧凑的类相关线索；DCM通过设计的损失函数约束先前学习的类分布，更充分地保存蒸馏知识

Result: 在三个公共数据集上的大量实验表明，该方法优于其他最先进的竞争对手

Conclusion: CD²框架通过数据集蒸馏和约束机制有效解决了小样本类增量学习中的灾难性遗忘问题，能够更充分地保存先前关键知识
...
<details>
  <summary>Details</summary>
Motivation: 视频视觉语言模型中的幻觉问题频繁且置信度高，现有不确定性度量方法往往无法与正确性对齐，需要专门针对视频时序结构的幻觉检测框架

Method: 提出VideoHEDGE框架：从干净视频片段及其光度和时空扰动变体中生成基准答案和多个高温生成结果，使用NLI或嵌入方法将文本输出聚类为语义假设，基于聚类概率质量计算三种可靠性分数（语义熵、RadFlag、视觉增强语义熵）

Result: 在SoccerChat基准测试中，使用LLM作为评判者获得二元幻觉标签。在三个7B视频VLM上，VASE始终获得最高的ROC-AUC，尤其在较大失真预算时表现优异；嵌入聚类与NLI聚类性能相当但计算成本更低；领域微调减少幻觉频率但校准改进有限

Conclusion: VideoHEDGE为视频问答中的幻觉检测提供了有效的模块化框架，VASE方法在检测性能上表现最佳，嵌入聚类提供了计算效率高的替代方案，领域微调对校准改进有限
...
TL;DR: MoCha：一种仅需单帧掩码的视频角色替换框架，通过条件感知RoPE和RL后训练增强身份一致性，使用UE5渲染、肖像动画合成和现有数据增强构建训练数据集


<details>
  <summary>Details</summary>
Motivation: 现有基于重建的方法需要逐帧分割掩码和显式结构指导（如骨骼、深度），在遮挡、角色-物体交互、异常姿态或复杂光照等场景中泛化能力有限，导致视觉伪影和时间不一致性

Method: 提出MoCha框架：1）仅需单帧任意掩码；2）引入条件感知RoPE适应多模态输入条件；3）采用RL后训练阶段增强面部身份一致性；4）构建三个专用数据集：UE5高保真渲染数据集、肖像动画技术合成的表情驱动数据集、现有视频-掩码对的增强数据集

Result: 大量实验表明，该方法显著优于现有最先进方法，在复杂场景中表现出更好的泛化能力和视觉质量

Conclusion: MoCha通过简化输入要求（仅需单帧掩码）和创新的训练策略，解决了视频角色替换中的泛化问题，为可控视频角色替换提供了更实用的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 传统放射组学特征在群体层面预定义，虽可解释但过于局限，无法捕捉患者特异性变异；端到端深度学习性能好但缺乏可解释性。需要兼具准确性和可解释性的方法用于临床采用。

Method: 1. 放射组学指纹：动态构建患者特异性特征集，通过图像条件预测器估计特征使用概率，结合全局系数的透明逻辑回归进行分类；2. 健康人格：使用扩散模型为每个患者合成无病理基线MRI，通过对比病理图像与健康人格的特征差异来定位疾病表现。

Result: 两种方法在三个临床任务上的表现与最先进深度学习模型相当或更优，同时支持多层次可解释性。案例研究展示了这些方法如何促进人类可解释的生物标志物发现和病理定位。

Conclusion: 放射组学指纹和健康人格是互补策略，能够为膝关节MRI自动评估提供准确且可解释的解决方案，支持临床采用和人类可解释的生物标志物发现。
...
TL;DR: SfMamba：一种基于视觉Mamba的源自由域自适应框架，通过通道序列扫描和语义一致打乱策略解决现有方法在感知场与计算效率之间的权衡问题


<details>
  <summary>Details</summary>
Motivation: 源自由域自适应（SFDA）在实际应用中面临数据隐私和存储限制的挑战，现有方法在领域不变特征学习中难以平衡感知场和计算效率。虽然Mamba的选择性扫描机制提供了长程依赖建模的线性复杂度解决方案，但视觉Mamba在捕获通道级频率特征和保持空间鲁棒性方面仍有局限

Method: 提出SfMamba框架，包含两个核心组件：1）通道级视觉状态空间块，通过通道序列扫描实现领域不变特征提取；2）语义一致打乱策略，在2D选择性扫描中打乱背景补丁序列同时保持预测一致性，以减少错误累积

Result: 在多个基准测试上的综合评估表明，SfMamba相比现有方法实现了持续更强的性能，同时保持了良好的参数效率

Conclusion: SfMamba为源自由域自适应提供了一个实用解决方案，通过充分利用视觉Mamba的选择性扫描机制，在领域对齐和空间鲁棒性方面取得了显著改进
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在医疗和自动驾驶等关键决策系统中的广泛应用，其不确定性估计的校准变得至关重要。然而，现有的VLM测试时提示调优文献主要关注提高判别性能，而忽略了校准这一维度。当前最先进的方法通过强制文本提示嵌入的完全正交性来增强可分离性，但这会导致语义相关类别被过度推开，使模型过于自信。

Method: 基于理论分析发现完全正交约束会使语义相关类别过度分离的问题，作者提出了语义正交校准（SoC）方法。该方法使用Huber-based正则化器，在强制原型分离的同时保持语义邻近性，实现平滑的校准效果。

Result: 通过全面的实证验证，研究表明SoC方法在多个数据集上一致地改善了校准性能，同时保持了有竞争力的判别能力。与先前基于正交性的方法相比，SoC在不确定性校准方面表现更优。

Conclusion: 该工作强调了在视觉语言模型测试时提示调优中考虑校准的重要性，提出的语义正交校准方法通过平衡原型分离和语义保持，有效解决了现有正交性方法导致的过度自信问题，为VLM的不确定性校准提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1）专注于像素级融合但忽视下游任务适应性；2）通过级联检测/分割模型隐式学习刚性语义，无法交互式满足多样化语义目标感知需求。需要一种能够交互式控制融合过程并同时提升任务性能的方法。

Method: 提出CtrlFuse框架，包含多模态特征提取器、参考提示编码器（RPE）和提示-语义融合模块（PSFM）。RPE通过输入掩码指导微调预训练分割模型，动态编码任务特定语义提示；PSFM将这些语义显式注入融合特征。通过并行分割和融合分支的协同优化，实现任务性能和融合质量的相互增强。

Result: 实验表明在融合可控性和分割准确性方面均达到最先进水平，适应后的任务分支甚至超越了原始分割模型的性能。

Conclusion: CtrlFuse通过交互式掩码提示实现了可控的图像融合，不仅提升了融合质量，还增强了下游语义分割任务的性能，为智能无人系统的环境感知提供了更灵活有效的解决方案。
...
TL;DR: 提出基于四大支柱（可迁移性、鲁棒性、可解释性、计算效率）的深度伪造检测可靠性评估框架，分析五种SOTA方法揭示进展与局限


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术具有正反两面应用，现有检测方法虽在分类性能上有所进展，但缺乏超越分类性能的全面评估框架，需要系统性可靠性评估

Method: 提出基于四大支柱的可靠性评估框架：1)可迁移性（跨数据集泛化能力）；2)鲁棒性（对抗攻击和真实世界变化下的稳定性）；3)可解释性（检测决策的可理解性）；4)计算效率（资源消耗与实时性）。应用该框架对五种最先进的深度伪造检测方法进行系统性分析

Result: 分析揭示了深度伪造检测领域的显著进展，但也暴露了关键局限性：方法在跨数据集泛化、对抗攻击鲁棒性、决策可解释性以及计算效率方面存在不足，需要进一步改进

Conclusion: 提出的四支柱评估框架为深度伪造检测提供了超越传统分类性能的全面评估标准，有助于推动更可靠、实用且可解释的检测方法发展，应对深度伪造带来的社会挑战
...
TL;DR: Salience-SGG提出了一种新的场景图生成框架，通过迭代显著性解码器强调具有显著空间结构的三元组，解决了传统无偏SGG方法因去偏策略而牺牲空间理解能力的问题。


<details>
  <summary>Details</summary>
Motivation: 场景图生成（SGG）面临长尾分布问题，少数谓词类别占主导而许多其他类别代表性不足，导致模型在罕见关系上表现不佳。现有的无偏SGG方法通过去偏策略解决此问题，但往往以牺牲空间理解为代价，导致过度依赖语义先验。

Method: 提出Salience-SGG框架，核心是迭代显著性解码器（ISD），该解码器强调具有显著空间结构的三元组。同时提出语义无关的显著性标签来指导ISD的训练。

Result: 在Visual Genome、Open Images V6和GQA-200数据集上的评估显示，Salience-SGG实现了最先进的性能，并通过成对定位平均精度指标证明，能够提升现有无偏SGG方法的空间理解能力。

Conclusion: Salience-SGG通过强调空间显著性，在保持无偏性能的同时改善了场景图生成的空间理解能力，为解决SGG中的长尾分布问题提供了新的有效途径。
...
TL;DR: ISLA（缺血性卒中病灶分析器）是一种用于急性缺血性卒中病灶分割的新型深度学习模型，通过系统优化损失函数、卷积架构、深度监督和注意力机制，在三个多中心数据库上训练，并在外部测试集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前急性缺血性卒中病灶分割的深度学习模型大多基于U-Net框架，但存在以下问题：1）不同模型主要在损失函数、深度监督、残差连接和注意力机制方面有差异；2）许多实现不公开可用；3）急性缺血性卒中病灶分割的最佳配置仍不明确。这阻碍了临床应用的标准化和可重复性。

Method: ISLA模型通过系统优化损失函数、卷积架构、深度监督和注意力机制来构建。模型在三个多中心数据库（总计超过1500名急性缺血性卒中参与者）的扩散MRI数据上进行训练。此外，研究还探索了无监督域适应方法以提高模型在外部临床数据集上的泛化能力。

Result: ISLA在外部测试集上优于两种最先进的急性缺血性卒中病灶分割方法。模型代码和训练好的模型将公开可用，以促进重用和可重复性。

Conclusion: ISLA通过系统优化深度学习架构组件，提供了一个鲁棒的急性缺血性卒中病灶分割框架。无监督域适应进一步提高了模型在外部数据集上的泛化能力，为临床诊断和管理提供了有效的自动化工具。
...
TL;DR: 提出一种基于CycleGAN的方法，将多通道荧光显微镜图像转换为伪H&E染色组织病理学图像，无需配对训练数据
...
Method: 使用Cycle-Consistent Adversarial Network (CycleGAN)进行无配对图像到图像转换，将C01和C02荧光通道合并为RGB，学习荧光和H&E域之间的双向映射。架构采用基于ResNet的生成器（残差块）和PatchGAN判别器，结合对抗损失、循环一致性损失和身份损失进行训练

Result: 在荧光显微镜数据集上的实验表明，该方法能生成逼真的伪H&E图像，在保持形态结构的同时采用H&E样颜色特征
...
<details>
  <summary>Details</summary>
Motivation: 牙科X光片诊断耗时且困难，因为诊断证据通常很细微。现有基于自然图像目标检测的方法难以检测视觉支持较少的牙科疾病，需要解决X光片中的视觉模糊性问题

Method: 提出DentalX方法，包含结构上下文提取模块，学习牙科解剖结构的语义分割作为辅助任务，提取有意义的结构上下文信息并集成到主要疾病检测任务中

Result: 在专用基准测试上的广泛实验表明，DentalX在两个任务上都显著优于先前方法，模型优化过程中自然实现了两个任务的相互促进

Conclusion: 通过利用口腔结构信息来缓解X光片中的视觉模糊性，DentalX能够有效检测细微牙科疾病，两个任务之间的相关性在模型优化过程中被有效捕捉
...
TL;DR: 提出自动合成3D视觉定位数据及推理过程的管道，并基于此微调LLM得到Reason3DVG-8B模型，仅用1.6%训练数据即超越先前方法


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉定位模型推理能力有限，现有方法需要大量人工标注数据，而合成数据方法性能提升有限且成本高昂，需要更高效的数据生成方案

Method: 开发自动合成3D视觉定位数据及对应推理过程的数据管道，利用生成数据对LLM进行微调，构建Reason3DVG-8B模型

Result: Reason3DVG-8B仅使用3D-GRAND方法1.6%的训练数据，性能即超越该先前LLM-based方法，证明了数据有效性和推理在3D视觉定位中的重要性

Conclusion: 提出的自动数据合成管道能高效生成高质量3D视觉定位数据，基于此微调的LLM模型在少量数据下即取得优异性能，验证了推理能力对3D视觉定位任务的关键作用
...
<details>
  <summary>Details</summary>
Motivation: SAM2等基于内存的视频对象分割方法在大视角变化下表现不佳，因为它们依赖外观特征；传统3D实例分割方法需要相机位姿、深度图和昂贵的预处理。需要一种既能处理大视角变化又无需额外输入的方法。

Method: 提出3AM训练时增强方法：1) 将MUSt3R的多层次3D感知特征通过轻量级Feature Merger融合到SAM2中；2) 提出视场感知采样策略，确保帧观察空间一致的对象区域；3) 结合SAM2的外观特征，实现基于空间位置和视觉相似性的几何一致识别。

Result: 在具有宽基线运动的挑战性数据集（ScanNet++、Replica）上，3AM显著优于SAM2及其扩展：在ScanNet++的Selected Subset上达到90.6% IoU和71.7% Positive IoU，比最先进的VOS方法分别提升15.9和30.4个百分点。

Conclusion: 3AM通过集成3D感知特征，成功解决了视频对象分割在大视角变化下的挑战，无需相机位姿或预处理，仅需RGB输入即可实现几何一致的分割，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像内容的普及，不可见水印已成为验证内容真实性的关键技术，各大平台已大规模部署水印方案。然而，评估这些方案对抗复杂去除攻击的脆弱性对于评估其可靠性和指导稳健设计至关重要。现有评估主要关注像素空间和频域攻击，但缺乏对语义保持变换下脆弱性的系统性研究。

Method: 提出零样本扩散框架，将水印去除重新定义为视角合成问题。核心思想是生成同一语义内容的感知一致替代视角（类似于从偏移视角重新观察场景），从而自然去除嵌入水印同时保持视觉保真度。方法采用冻结预训练模型，无需检测器访问或水印知识，在潜在空间中应用受控几何变换，并通过视角引导对应注意力机制在重建过程中保持结构一致性。

Result: 该方法在15种水印方法上实现了最先进的水印抑制效果，优于14种基线攻击方法，同时在多个数据集上保持了卓越的感知质量。实验表明，即使对像素空间和频域攻击具有鲁棒性的水印，在语义保持的视角变换下仍然脆弱。

Conclusion: 该研究揭示了不可见水印的一个根本性漏洞：水印对像素空间和频域攻击的鲁棒性并不能保证对语义保持的视角变换的抵抗力。这一发现为水印方案的稳健设计提供了重要指导，强调了需要考虑更广泛的攻击类型，特别是基于语义内容保持的变换。
...
### [73] [LLM Review: Enhancing Creative Writing via Blind Peer Review Feedback](https://arxiv.org/abs/2601.08003)
...
Method: 提出LLM Review框架，采用盲审机制：智能体交换针对性反馈但独立修订，保持创意轨迹的多样性；创建SciFi-100科幻写作数据集，结合LLM评分、人工标注和基于规则的新颖性指标进行综合评估

Result: LLM Review在多智能体基线方法中表现一致更优；使用该框架的小模型可以超越更大的单智能体模型，表明交互结构可以替代模型规模

Conclusion: 通过盲审机制的结构化交互能够有效解决多智能体协作中的创意同质化问题，为提升LLM的创意生成能力提供了新思路，交互结构设计可能比单纯增加模型规模更有效
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医疗任务中应用广泛，但在复杂现实场景下与医疗伦理的细致要求对齐仍未被充分探索，特别是在中文医疗伦理背景下缺乏专门的对齐框架。

Method: 1) 构建MedES动态场景中心基准，基于260个权威中文医疗、伦理和法律来源；2) 提出监护人循环框架，利用自动化评估器（专家标注数据训练，领域内准确率超97%）生成针对性提示和结构化伦理反馈；3) 通过监督微调和领域特定偏好优化对齐7B参数大语言模型。

Result: 在中文医疗伦理背景下，对齐后的模型在核心伦理任务上显著超越更大基线模型，在质量和综合评估指标上均有改进。

Conclusion: 该工作为中文医疗领域的大语言模型伦理对齐提供了实用且可适应的框架，表明通过模块化替换底层规范语料库，类似的对齐流程可在其他法律和文化环境中实例化。
...
TL;DR: 研究通过ValAct-15k数据集评估LLMs在现实决策场景中的价值对齐表现，发现模型间高度一致但与人差异大，且存在知识-行动差距和角色扮演厌恶现象。


<details>
  <summary>Details</summary>
Motivation: 价值对齐对AI安全和社会兼容性至关重要，但LLMs如何在现实决策场景中表征和执行人类价值观尚未充分探索。需要理解模型的价值表现与人类差异，以及是否存在知识-行动差距。

Method: 创建ValAct-15k数据集（包含3,000个来自Reddit的寻求建议场景），基于Schwartz基本人类价值观理论设计十个价值观维度。评估十个前沿LLMs（五个美国公司，五个中国公司）和55名人类参与者，使用场景问题和传统价值观问卷，分析决策一致性、价值表现对应性，并测试指令引导下的价值持有效果。

Result: 1. 模型间场景决策一致性接近完美（Pearson r≈1.0），而人类间差异巨大（r∈[-0.79, 0.98]）；2. 人类和LLMs都表现出自我报告价值观与实际行为间的弱对应性（r=0.4, 0.3），存在系统性知识-行动差距；3. 当被指令"持有"特定价值观时，LLMs性能下降高达6.6%，表现出角色扮演厌恶。

Conclusion: 对齐训练虽然能产生规范性的价值观趋同，但无法消除人类类似的知识与行动间的不一致性。LLMs在价值表现上显示出与人类相似的内在矛盾，这对AI安全和社会兼容性发展具有重要意义。
...
TL;DR: 该论文系统研究了AI文本检测器在不同生成条件下的泛化问题，通过语言特征分析解释了泛化性能差异的原因。


<details>
  <summary>Details</summary>
Motivation: AI文本检测器在域内基准测试中表现良好，但在面对不同生成条件（如未见过的提示、模型家族或领域）时泛化能力不足。先前研究虽然报告了这些泛化差距，但对根本原因缺乏深入理解。

Method: 构建了一个全面的基准测试，涵盖6种提示策略、7个大型语言模型和4个领域数据集，创建了多样的人类和AI生成文本集合。在不同生成设置上微调基于分类的检测器，评估其跨提示、跨模型和跨数据集的泛化能力。通过计算80个语言特征在训练和测试条件之间的特征偏移与泛化准确率的相关性来解释性能差异。

Result: 分析显示，特定检测器和评估条件的泛化性能与时态使用和代词频率等语言特征显著相关。研究发现泛化性能的差异可以通过训练和测试条件之间的语言特征偏移来解释。

Conclusion: 该研究通过系统的语言分析揭示了AI文本检测器泛化行为的根本原因，表明语言特征偏移是解释泛化性能差异的关键因素，为改进检测器的泛化能力提供了重要见解。
...
TL;DR: 提出三层次评估框架，用于评估视觉语言模型在跨文化艺术批评中的文化理解能力，包含自动化指标、基于量规的评分和校准评分三个层次。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉感知方面表现出色，但其解释艺术中文化意义的能力尚未得到充分验证，需要系统评估框架来衡量模型的文化理解深度。

Method: 三层次评估框架：第一层离线计算自动化覆盖率和风险指标；第二层使用单一主评审员在五个维度上应用量规评分；第三层通过等渗回归将第二层聚合分数校准到人类评分。

Result: 在152个样本的保留集上，校准后MAE减少5.2%；评估了15个VLMs在294个专家锚点（涵盖六种文化传统）上的表现；发现自动化指标不可靠、西方样本得分高于非西方样本、跨评审员尺度不匹配等问题。

Conclusion: 提出并验证了一个校准的文化理解评分框架，可用于模型选择和文化差距诊断，强调需要单一主评审员和明确校准来解决跨评审员尺度不匹配问题。
...
TL;DR: 该论文提出了一种构建多语言多模态事实核查数据集的新方法，通过聚合ClaimReview数据、抓取完整辟谣文章、规范化裁决结果，并利用LLM进行证据提取和理由生成，创建了法语和德语的事实核查数据集。


<details>
  <summary>Details</summary>
Motivation: 在线平台错误信息的快速传播迫切需要强大、最新、可解释且多语言的事实核查资源。现有数据集范围有限，通常缺乏多模态证据、结构化标注以及声明、证据和裁决之间的详细联系。

Method: 开发了一个全面的数据收集和处理流程：1) 聚合ClaimReview数据源；2) 抓取完整的辟谣文章；3) 规范化异构的声明裁决；4) 用结构化元数据和对齐的视觉内容进行丰富；5) 使用最先进的LLM和多模态LLM进行证据提取（按预定义类别）和理由生成（将证据与裁决联系起来）。

Result: 使用G-Eval和人工评估表明，该流程能够：1) 对不同组织或媒体市场的事实核查实践进行细粒度比较；2) 促进开发更可解释和基于证据的事实核查模型；3) 为未来多语言、多模态错误信息验证研究奠定基础。

Conclusion: 该研究提出的数据收集和处理流程成功构建了法语和德语的多模态事实核查数据集，解决了现有数据集的局限性，为开发更强大、可解释和多语言的事实核查系统提供了重要资源和方法论基础。
...
TL;DR: VULCA-Bench是一个多文化艺术评论基准，用于评估视觉语言模型在超越表面视觉感知的文化理解能力，包含7,410个图像-评论对，覆盖八种文化传统和中英双语。


<details>
  <summary>Details</summary>
Motivation: 现有VLM基准主要评估L1-L2能力（物体识别、场景描述和事实问答），而低估了高阶文化解释能力。需要专门基准来评估模型对文化内涵的深层理解。

Method: 采用五层框架（L1-L5，从视觉感知到哲学美学）操作化文化理解，实例化为225个文化特定维度，由专家撰写双语评论支持。数据集包含7,410个匹配的图像-评论对，覆盖八种文化传统。

Result: 初步结果表明，高层推理（L3-L5）始终比视觉和技术分析（L1-L2）更具挑战性。数据集、评估脚本和标注工具已在补充材料中以CC BY 4.0许可提供。

Conclusion: VULCA-Bench填补了现有VLM评估在文化理解方面的空白，为评估模型的多文化艺术解释能力提供了系统框架和基准资源。
...
<details>
  <summary>Details</summary>
Motivation: 传统NLP将文档视为独立无序样本，但在纵向研究中，文档嵌套于作者并按时间排序形成行为序列，需要新的建模和评估方法

Method: 提出纵向建模评估范式，更新四个NLP流程部分：1) 按人群和时间划分的评估分割；2) 分离个体间差异与个体内动态的准确度指标；3) 默认包含历史的序列输入；4) 支持不同粒度潜在状态的模型内部结构

Result: 在包含238名参与者的17k篇日记转录与PTSD症状严重程度数据上，传统文档级评估与生态有效建模评估得出显著不同甚至相反的结论

Conclusion: 需要从词序列评估范式转向行为序列范式，以更好地处理纵向研究中的嵌套时间序列数据
...
Method: DyCP（动态上下文管理）方法在查询时动态分割和检索相关记忆，保持对话的顺序结构而不需要预定义主题边界，支持高效、自适应的上下文检索

Result: 在三个长对话基准测试（LoCoMo、MT-Bench+、SCM4LLMs）和多个LLM上，DyCP一致提高了答案质量同时减少了响应延迟

Conclusion: 现代LLM虽然扩展了上下文窗口，但其实际的长上下文处理能力仍有差距，因此有效的上下文管理仍然至关重要，DyCP为此提供了一种有效的解决方案
...
TL;DR: 评估概念向量投影（CVP）在多领域、多语言和多情感维度中的可移植性，并检验其线性假设的有效性


<details>
  <summary>Details</summary>
Motivation: 人文学科中的情感分析常需要情境化的连续分数，CVP方法通过将情感建模为嵌入空间中的方向来提供此类分数，但其跨领域可移植性和基本假设尚未充分研究

Method: 评估CVP在不同体裁、历史时期、语言和情感维度上的表现，检验概念向量从一个语料库迁移到其他语料库的能力，并深入分析CVP的线性假设

Result: CVP表现出良好的可移植性，在一个语料库上训练的概念向量能够有效迁移到其他语料库且性能损失最小；但线性假设仅为近似成立，存在进一步改进空间
...
TL;DR: 研究发现大语言模型中的推理能力可以通过内部潜在特征的操控来激活，而不仅仅是依赖思维链提示，揭示了推理的内部机制


<details>
  <summary>Details</summary>
Motivation: 尽管思维链提示显著提升了大语言模型的推理性能，但其工作机制尚不明确，且不清楚它是否是触发模型推理的唯一机制。本研究旨在探究大语言模型推理的内部机制，验证思维链提示是否是其必要原因。

Method: 使用稀疏自编码器分析和干预大语言模型的内部表示，识别与推理行为因果相关的潜在特征。通过操控单个推理相关潜在特征来评估模型性能，比较潜在操控与标准思维链提示的效果。

Result: 研究发现：1）操控单个推理相关潜在特征可显著提升准确性，无需显式思维链提示；2）对于大型模型，潜在操控能达到与标准思维链提示相当的性能，同时输出更高效；3）推理导向的内部状态在生成早期即被触发，并能覆盖提示层级的反推理指令。

Conclusion: 大语言模型的多步推理由可被外部激活的潜在内部激活支持，思维链提示是激活该机制的有效但非唯一方式，而非其必要原因。这揭示了模型推理的内部工作机制。
...
TL;DR: 语言模型通过自回归输出链式连接即可实现通用计算，训练不增加计算表达能力而是提高可编程性


<details>
  <summary>Details</summary>
Motivation: 探讨语言模型的终极计算能力，澄清其与形式计算机的区别，研究自回归预测是否足以执行通用计算

Method: 证明语言模型的自回归输出链式连接足以执行通用计算，即能模拟任何算法在任何输入上的执行；研究随机初始化语言模型的计算能力

Result: 语言模型通过自回归输出链式连接可实现通用计算；随机初始化的语言模型在训练前就具备通用计算能力；训练主要改善可编程性而非计算表达能力

Conclusion: 语言模型的计算能力本质上是通用的，训练的作用是提高可编程性而非创造计算表达能力，这为理解语言模型的计算本质提供了新视角
...
TL;DR: 该论文提出了一个全面的置信度估计评估框架，重点关注LLM置信度在语义变化下的鲁棒性、稳定性和敏感性，揭示了现有方法在真实应用中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有置信度估计方法主要关注校准和区分度，忽略了LLM在语言变体环境下的关键问题：置信度应在语义等效的提示或答案变体中保持一致，而在答案含义不同时应发生变化。

Method: 提出了一个全面的置信度估计评估框架，包含三个新维度：1) 对提示扰动的鲁棒性；2) 跨语义等效答案的稳定性；3) 对语义不同答案的敏感性。通过该框架评估常见的LLM置信度估计方法。

Result: 研究发现，在传统校准或区分度指标上表现良好的置信度估计方法，往往在提示扰动下不够鲁棒，或对答案变化不够敏感。现有方法在这三个新维度上经常失败。

Conclusion: 该框架揭示了现有置信度评估在真实世界LLM应用中的局限性，为选择和设计更可靠的置信度估计方法提供了实用指导。需要开发在语义变化下表现更稳定的置信度估计方法。
...
Method: 提出自适应因果提示与思维草图框架（ACPS），利用结构因果模型推断查询对答案的因果效应，自适应选择适当干预（标准前门和条件前门调整），用简洁的Sketch-of-Thought替代冗长的CoT
...
TL;DR: ExoFormer通过引入外部锚定投影解决Transformer中早期层注意力投影复用问题，在性能、数据效率和注意力下沉方面均有提升，但存在表征坍缩现象。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer中早期层注意力投影作为残差复用时的基本矛盾：第一层既要作为深层稳定参考，又要作为有效计算模块。

Method: 提出ExoFormer，在顺序层栈外学习专用的外部锚定投影，将锚定角色与计算精化解耦；采用统一的归一化混合框架，研究不同系数粒度（元素级、头级、标量级）在所有注意力路径（查询、键、值、门控logits）上的应用。

Result: ExoFormer变体始终优于内部锚定对应模型；动态变体在下游任务准确率上比基线提高2.13个百分点，数据效率更高，用1.84倍更少的token达到基线验证损失；注意力下沉比标准门控注意力减少2倍；所有变体都表现出表征坍缩迹象。

Conclusion: 通过"卸载假说"解释表征坍缩：外部锚定保留基本token身份，使各层专门进行计算精化；释放代码和模型促进未来研究。
...
TL;DR: 该论文提出了推理模型置信度估计基准（RMCB），包含347,496个推理轨迹，用于评估大型推理模型在多步长输出中的置信度校准方法，发现文本编码器在区分性（AUROC 0.672）和结构化模型在校准性（ECE 0.148）之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）在多步长推理输出中的错误校准问题降低了其在高风险领域（如临床、金融、法律等）的可靠性，需要开发准确估计其置信度的方法，但目前缺乏全面的评估基准。

Method: 构建RMCB基准，包含来自6个不同架构LRMs的347,496个推理轨迹，覆盖高风险领域和复杂通用推理任务。使用该基准对10多种基于表示的方法进行大规模实证评估，包括序列模型、图模型和文本编码器。

Result: 发现区分性（AUROC）和校准性（ECE）之间存在持久权衡：文本编码器获得最佳AUROC（0.672），而结构化模型获得最佳ECE（0.148），没有单一方法在两方面都占优。增加架构复杂性并未显著优于简单序列基线。

Conclusion: RMCB是目前该任务最全面的基准，建立了严格的基线并揭示了当前基于表示的方法的局限性。需要在保持区分性的同时改进校准性，且仅依赖块级隐藏状态的方法存在性能上限。
...
### [89] [WISE-Flow: Workflow-Induced Structured Experience for Self-Evolving Conversational Service Agents](https://arxiv.org/abs/2601.08158)
*Yuqing Zhou,Zhuoer Wang,Jie Yuan,Hong Wang,Samson Koelle,Ziwei Zhu,Wei Niu*

Main category: cs.CL

TL;DR: WISE-Flow是一个工作流中心框架，通过从历史服务交互中提取可复用的程序性经验，使LLM智能体能够自我进化，减少错误并提高稳定性。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在用户服务中广泛部署，但在新任务中容易出错，倾向于重复相同的失败模式，且运行间变异性大。通过特定环境训练或手动修补来修复失败成本高且难以扩展。

Method: 提出WISE-Flow框架：1) 将历史服务交互转换为可复用的程序性经验，通过诱导具有前提条件增强的动作块的工作流；2) 部署时，将智能体的执行轨迹与检索到的工作流对齐；3) 执行基于前提条件的可行性推理，实现状态基础的下一个动作。

Result: 在ToolSandbox和τ²-bench上的实验显示，该方法在不同基础模型上均能带来一致的性能提升。

Conclusion: WISE-Flow通过工作流中心的方法使智能体能够自我进化，有效解决了LLM智能体在用户服务环境中的错误重复和变异性问题，为可扩展的智能体改进提供了新途径。
...
TL;DR: SwiftMem是一种查询感知的智能体记忆系统，通过专门的时空索引实现亚线性检索，解决了现有记忆系统全量检索的延迟瓶颈问题。
...
Method: 提出SwiftMem系统，采用查询感知的智能体记忆架构：1) 时间索引支持对数时间范围查询；2) 语义DAG-Tag索引通过分层标签结构将查询映射到相关主题；3) 嵌入-标签协同整合机制基于语义簇重组存储以改善缓存局部性。

Result: 在LoCoMo和LongMemEval基准测试中，SwiftMem相比最先进的基线方法实现了47倍的搜索加速，同时保持竞争性准确度，使记忆增强的LLM智能体能够实际部署。

Conclusion: SwiftMem通过专门的时空索引和存储重组机制，解决了智能体记忆系统的检索延迟瓶颈，为实时记忆增强LLM智能体的实际部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于因果中介分析得到的函数向量虽然能编码任务表示，但在关系知识表示和类比推理方面仍有提升空间。研究者希望探索通过微调函数向量来更好地编码关系知识，并支持复杂的类比推理任务。

Method: 1. 对通过因果中介分析得到的函数向量进行小样本微调（约20个词对）
2. 提出复合函数向量：将多个微调后的函数向量进行加权组合
3. 在推理时将复合向量插入LLM激活中，以增强类比推理能力

Result: 1. 微调后的函数向量在关系型词语补全任务上表现优于原始向量
2. 微调向量在关系词解码和语义关系的人类相似性判断对齐方面表现更好
3. 复合函数向量显著提升了在认知科学和SAT基准测试中的类比问题解决能力

Conclusion: 激活修补作为一种可控机制，在编码和操作关系知识方面具有巨大潜力，既能提升大语言模型的可解释性，又能增强其推理能力。该方法为关系知识表示和类比推理提供了有效的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM回答的事实正确性，但对清晰度自动评估的研究不足，特别是提示设计对政治问答清晰度评估的影响尚未充分探索

Method: 使用SemEval 2026共享任务的CLARITY数据集，比较GPT-3.5基线模型与GPT-5.2在三种提示策略下的表现：简单提示、思维链提示、带少样本示例的思维链提示

Result: GPT-5.2在清晰度预测上始终优于GPT-3.5基线，带少样本的思维链提示将准确率从56%提升到63%；思维链提示在回避检测上达到34%准确率最高；基于推理的提示将话题识别准确率从60%提升到74%

Conclusion: 提示设计能可靠提升高层次清晰度评估效果，但细粒度回避检测和话题识别仍然具有挑战性，即使使用结构化推理提示也难以完全解决
...
TL;DR: LogiSafetyGen框架将非结构化法规转换为时序逻辑预言机，通过逻辑引导模糊测试生成安全关键轨迹，并构建LogiSafetyBench基准测试评估LLM在功能正确性和合规性方面的表现。
...
Method: 提出LogiSafetyGen框架：1) 将非结构化法规转换为线性时序逻辑预言机；2) 采用逻辑引导模糊测试合成有效的安全关键轨迹；3) 构建LogiSafetyBench基准，包含240个人工验证的任务，要求LLM生成同时满足功能目标和潜在合规规则的Python程序。

Result: 评估13个SOTA LLM发现：1) 更大模型虽然功能正确性更好；2) 但经常优先考虑任务完成而非安全性；3) 导致不合规行为。

Conclusion: 需要开发能够平衡功能正确性和监管合规性的LLM系统，特别是在高风险领域，仅关注功能正确性不足以确保安全合规。
...
TL;DR: T-SPIN提出了一种基于三元组的自对弈微调方法，通过引入历史优势和熵约束，解决了SPIN方法中优化不稳定和训练-生成不一致的问题，在数据稀缺场景下显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: SPIN方法在利用自对弈微调适应数据稀缺的下游任务时存在两个主要问题：1）当前奖励优势在迭代过程中逐渐消失导致优化不稳定；2）使用参考策略导致训练时的奖励公式与生成时的评估指标不一致。需要一种更稳定且对齐的方法。

Method: T-SPIN包含两个关键设计：1）除了当前优势外，还引入历史优势，即迭代生成响应与初始策略生成的原型合成响应之间的比较，确保即使当前优势消失，历史优势仍能稳定优化；2）在自对弈框架中引入熵约束，理论证明支持无参考微调，消除了训练-生成不一致性。

Result: 在多个任务上的实验结果表明，T-SPIN不仅性能优于SPIN，而且在迭代过程中表现出稳定的演化。值得注意的是，与监督微调相比，T-SPIN仅使用25%的样本就能达到相当甚至更好的性能，突显了其在数据稀缺情况下的有效性。

Conclusion: T-SPIN通过整合历史优势和熵约束，有效解决了SPIN方法的局限性，提供了一种更稳定、更对齐的自对弈微调方法，特别适用于专家标注数据稀缺的下游应用场景。
...
TL;DR: GAG（生成增强生成）通过将私有专业知识作为额外专家模态，在表示层面对齐到冻结的基础模型，解决了私有知识注入中微调迭代成本高和RAG证据碎片化的问题。


<details>
  <summary>Details</summary>
Motivation: 在生物医学、材料和金融等高风险领域部署LLMs时，需要注入私有、领域特定知识，这些知识具有专有性、快速演化和公共预训练中代表性不足的特点。现有两种主要方法各有明显缺陷：微调迭代成本高且存在灾难性遗忘风险；RAG虽然保持基础模型完整，但在专业私有语料中因证据碎片化、检索漂移和长上下文压力而表现脆弱。

Method: 受多模态LLMs将异构模态对齐到共享语义空间的启发，提出Generation-Augmented Generation（GAG）。将私有专业知识视为额外的专家模态，通过紧凑的表示级接口对齐到冻结的基础模型，避免提示时的证据序列化，实现即插即用专业化和可扩展的多领域组合。

Result: 在两个私有科学QA基准测试（免疫学佐剂和催化材料）和混合领域评估中，GAG在两个基准上分别比强RAG基线提升15.34%和14.86%的专业性能，同时在六个开放通用基准上保持性能，并实现接近oracle选择激活的可扩展多领域部署。

Conclusion: GAG通过将私有专业知识作为专家模态在表示层面注入，有效解决了私有知识注入的挑战，在保持基础模型完整性的同时提升了专业性能，支持可扩展的多领域部署。
...
TL;DR: 研究发现MoE模型性能主要由总参数量和专家稀疏度决定，而非仅考虑总参数和激活参数，提出MoE设计应最大化总参数同时最小化专家稀疏度和专家数量


<details>
  <summary>Details</summary>
Motivation: 当前MoE语言模型设计主要基于总参数量（内存占用）和激活参数量（推理成本），但研究发现这两个因素不足以描述最优架构，需要更全面的分析框架

Method: 通过系统性研究分析MoE性能的决定因素，发现性能主要由总参数量（N_total）和专家稀疏度（s:=n_exp/n_topk）决定，并分析专家数量与top-k选择之间的相互作用

Result: 研究表明专家稀疏度中的n_exp和n_topk不会相互抵消，更大的专家数量会轻微损害性能（因需减少核心模型维度以满足内存约束），提出MoE设计应最大化总参数同时最小化专家稀疏度和专家数量

Conclusion: 为MoE架构设计提供了稳健框架，解决了架构模糊性问题，指导MoE设计应遵循最大化总参数、最小化专家稀疏度和专家数量的原则
...
TL;DR: Med-CoReasoner框架通过并行英语和本地语言推理、概念抽象与对齐，结合英语逻辑结构和本地临床知识，提升多语言医疗推理性能，在低资源语言上效果显著。


<details>
  <summary>Details</summary>
Motivation: 当前增强推理的大语言模型在英语医疗任务上表现良好，但在多语言场景中存在显著差距，本地语言推理能力较弱，限制了全球医疗公平部署。需要弥合这一多语言鸿沟。

Method: 提出Med-CoReasoner框架：1) 并行激发英语和本地语言推理；2) 将推理抽象为结构化概念；3) 通过概念级对齐和检索将本地临床知识整合到英语逻辑框架中。该方法结合了英语推理的结构鲁棒性和本地语言编码的实践专业知识。

Result: 在三个基准测试中，Med-CoReasoner将多语言推理性能平均提升5%，在低资源语言上提升尤为显著。模型蒸馏和专家评估进一步证实该方法能产生临床合理且文化基础扎实的推理轨迹。

Conclusion: Med-CoReasoner通过语言感知的协同推理框架有效弥合了多语言医疗推理差距，结合英语结构优势和本地临床知识，为全球医疗公平部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前工具集成推理范式虽然增强了LLMs的计算能力，但在长链思维（long CoT）中整合工具使用仍未被充分探索，主要由于训练数据稀缺以及在不损害模型内在长链推理能力的前提下整合工具使用的挑战。

Method: DART采用强化学习框架，在训练过程中构建动态展开树来发现有效的工具使用机会，在有望的位置分支以探索多样化的工具集成轨迹。随后，基于树的优势估计过程识别并奖励那些工具调用对解决方案有积极贡献的特定子轨迹，从而有效强化这些有益行为。

Result: 在AIME和GPQA-Diamond等具有挑战性的基准测试中，DART显著优于现有方法，成功地将工具执行与长链CoT推理相协调。

Conclusion: DART框架能够在无需人工标注的情况下，在长链推理过程中实现自发的工具使用，有效解决了工具集成与长链推理的协调问题。
...
TL;DR: D²Plan：一种双智能体动态全局规划范式，通过Reasoner和Purifier的协作解决检索增强LLM在多跳推理中的搜索链构建无效和推理劫持问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的检索增强LLM在多跳推理任务中面临两个关键失败模式：1）无效的搜索链构建，导致生成错误查询或遗漏关键信息检索；2）推理被外围证据劫持，使模型将干扰项误认为有效证据。这些问题源于累积上下文被关键证据和无关信息淹没。

Method: 提出D²Plan双智能体动态全局规划范式，包含Reasoner和Purifier两个组件：Reasoner在推理过程中构建显式全局规划并根据检索反馈动态调整；Purifier评估检索相关性并为Reasoner压缩关键信息。采用两阶段训练框架：在合成轨迹上进行监督微调冷启动，然后使用面向规划的奖励进行强化学习。

Result: 大量实验表明，D²Plan能够实现更连贯的多步推理，对无关信息具有更强的鲁棒性，在具有挑战性的QA基准测试中取得了优越性能。

Conclusion: D²Plan通过双智能体协作和动态全局规划有效解决了检索增强LLM在多跳推理中的关键失败模式，显著提升了复杂检索增强推理的能力和鲁棒性。
...
TL;DR: 研究通过提示工程提升GPT-4o-mini和gemini-1.5-flash在情感分析任务中的表现，发现不同提示策略需针对模型和任务特性进行定制。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过高级提示工程技术（如少样本学习、思维链提示、自一致性）来提升大型语言模型在情感分析任务中的性能，特别是针对不同模型架构和任务复杂度的适应性。

Method: 使用GPT-4o-mini和gemini-1.5-flash两个LLM，对比基线方法与高级提示技术（少样本学习、思维链提示、自一致性）。评估任务包括情感分类、基于方面的情感分析和反讽检测。使用准确率、召回率、精确率和F1分数作为评估指标。

Result: 高级提示技术显著提升情感分析性能：少样本方法在GPT-4o-mini中表现最佳，思维链提示使gemini-1.5-flash在反讽检测中的性能提升达46%。不同模型对提示策略的响应存在差异。

Conclusion: 提示策略需要根据特定LLM架构和任务语义复杂度进行定制。提示设计与模型特性和任务要求对齐是提升性能的关键，强调了模型-任务-提示三者协同优化的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 真实农业场景中的智能体系统需要处理从轻量级信息理解到复杂多步执行的各种任务，但现有方法采用统一执行范式，难以适应农业环境中任务复杂度差异大和工具可用性不完整的问题。

Method: 提出AgriAgent两级智能体框架，采用基于任务复杂度的分层执行策略：简单任务由模态特定智能体直接推理处理；复杂任务触发契约驱动规划机制，将任务形式化为能力需求，进行能力感知的工具编排和动态工具生成，支持多步可验证执行和故障恢复。

Result: 实验结果表明，与依赖统一执行范式的现有工具中心智能体基线相比，AgriAgent在复杂任务上实现了更高的执行成功率和鲁棒性。

Conclusion: AgriAgent通过分层执行策略有效解决了农业场景中任务复杂度差异和工具可用性不完整的问题，为真实农业环境中的智能体系统提供了更灵活和鲁棒的解决方案。
...
TL;DR: CLaS-Bench：首个多语言引导技术评估基准，用于系统评估LLM在32种语言上的语言强制行为，发现基于残差流的DiffMean方法表现最佳


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门评估LLM多语言引导技术效果的基准和评估协议，而引导技术作为比提示或微调更高效、可解释的模型适应方法，需要标准化评估工具

Method: 提出CLaS-Bench轻量级并行问题基准，包含32种语言；评估多种引导技术：残差流DiffMean干预、探针推导方向、语言特定神经元、PCA/LDA向量、稀疏自编码器和提示基线；通过语言控制和语义相关性两个维度测量性能，结合为调和平均引导分数

Result: 1. 简单的基于残差的DiffMean方法在所有语言中一致优于其他方法；2. 分层分析显示语言特定结构主要出现在后期层；3. 引导方向基于语言家族聚类；4. CLaS-Bench成为首个多语言引导标准化基准

Conclusion: CLaS-Bench为多语言引导提供了首个标准化评估框架，既支持语言表示的科学分析，又为低成本模型适应提供了实用评估工具，其中DiffMean方法在多语言引导中表现最优
...
<details>
  <summary>Details</summary>
Motivation: 心理操纵作为计算社会推理中的新兴任务，先前研究仅关注文本对话，忽视了操纵策略在语音中的表现形式。本研究旨在填补这一空白，探索语音模态如何影响心理操纵的检测和感知。

Method: 创建合成多说话者语音基准SPEECHMENTALMANIP，将基于文本的数据集通过高质量、声音一致的文本转语音技术转换为音频。使用少样本大型音频-语言模型和人工标注，评估模态对检测准确性和感知的影响。

Result: 模型在语音检测上表现出高特异性但召回率显著低于文本，表明训练中可能缺少声学或韵律线索。人类评估者在音频设置下也表现出类似的不确定性，凸显了操纵性语音固有的模糊性。

Conclusion: 研究强调了在多模态对话系统中进行模态感知评估和安全对齐的必要性，语音模态为心理操纵检测带来了独特的挑战，需要更全面的建模方法。
...
TL;DR: 该研究提出了一种基于学生个性特征的LLM个性化辅导框架，通过构建教学法与个性特征关联的分类体系，使LLM能够根据模拟学生个性调整辅导策略，提升教育效果。


<details>
  <summary>Details</summary>
Motivation: 当前LLM辅导系统未考虑学生个性特征，而不同辅导策略对不同个性的学生效果不同，策略不匹配可能对学生学习效果产生负面影响。需要开发能够适应学生个性的个性化LLM辅导系统。

Method: 1. 基于教育学文献构建教学法与个性特征关联的分类体系；2. 模拟师生对话，让LLM根据模拟学生个性调整辅导策略；3. 通过人类教师评估框架效果，并与两个基线方法比较。

Result: 人类教师一致偏好该方法而非基线方法；该方法增加了角色扮演等高影响力但不常用策略的使用频率；人类和LLM标注者都显著偏好这些策略。

Conclusion: 该研究为开发更个性化、更有效的教育应用中的LLM使用铺平了道路，证明了考虑学生个性特征对提升LLM辅导效果的重要性。
...
TL;DR: Latent-GRPO提出了一种基于潜在空间几何特性的内在奖励框架，通过迭代鲁棒质心估计算法生成密集连续奖励，在保持模型性能的同时实现2倍以上的训练加速。
...
Method: 提出Latent-GRPO框架，从潜在空间几何特性推导内在奖励。基于发现正确推理轨迹的终端token表示形成密集聚类而错误轨迹呈离群点的几何特性，引入迭代鲁棒质心估计算法，通过球面投影缓解幅度波动，并通过迭代聚合估计鲁棒的"真理质心"。

Result: 在多个数据集上的实验结果表明，该方法在保持模型性能的同时，相比基线实现了超过2倍的训练加速。广泛的实验结果展示了强大的泛化能力和鲁棒性。

Conclusion: Latent-GRPO通过利用潜在空间几何特性生成密集连续的内在奖励，有效解决了GRPO依赖外部验证器的问题，显著提升了训练效率和优化效果。
...
TL;DR: JudgeRLVR：两阶段判别-生成范式，通过先训练模型判别可验证答案，再微调生成，在保持准确率的同时显著减少生成长度，实现质量-效率的更好权衡。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR仅优化最终答案正确性，导致模型陷入冗长、无目标的探索，依赖试错而非结构化规划。启发式约束如长度惩罚会截断必要推理步骤，形成效率与验证的困难权衡。

Method: 提出JudgeRLVR两阶段范式：第一阶段训练模型判别具有可验证答案的解决方案响应；第二阶段用vanilla生成RLVR从判别器初始化微调同一模型。

Result: 相比使用相同数学领域训练数据的Vanilla RLVR，JudgeRLVR在Qwen3-30B-A3B上实现更好的质量-效率权衡：领域内数学任务平均准确率提升约3.7分，平均生成长度减少42%；领域外基准平均准确率提升约4.5分，展示增强的泛化能力。

Conclusion: 判别能力是高效生成的前提，通过学习区分有效解决方案，模型可以内化指导信号来剪枝搜索空间。JudgeRLVR的两阶段判别-生成范式在保持准确率的同时显著减少冗长，实现更好的效率-质量平衡。
...
TL;DR: 提出一个结合NLP和形式化验证的框架，用于开发具有同理心的治疗聊天机器人，通过Transformer提取对话特征，构建随机混合自动机模型，并使用统计模型检验验证同理心属性。


<details>
  <summary>Details</summary>
Motivation: 当前聊天机器人在心理健康治疗路径中作为支持工具的应用日益增多，但缺乏系统化的方法来规范和验证同理心这一关键非功能性需求。现有聊天机器人开发实践无法系统地指定或验证同理心。

Method: 提出一个集成自然语言处理和形式化验证的框架：1) 使用基于Transformer的模型提取对话特征；2) 将特征转换为二元治疗会话的随机混合自动机模型；3) 通过统计模型检验验证同理心相关属性；4) 通过策略合成指导智能体行为塑造。

Result: 初步结果显示：1) 形式化模型能够以良好保真度捕捉治疗动态；2) 专门设计的策略提高了满足同理心要求的概率。

Conclusion: 该框架为开发具有同理心的治疗聊天机器人提供了系统化方法，通过形式化验证确保同理心要求的满足，为心理健康支持工具的开发提供了新方向。
...
TL;DR: 论文研究大语言模型在普通文本提示下产生过度输出的故障模式，称为"溢出"，并提出BenchOverflow基准来评估和缓解这一问题


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在普通交互设置下产生过度输出的现象，这种现象会增加服务成本、延迟，并导致跨用户性能下降，同时带来经济和环境影响

Method: 提出BenchOverflow基准，包含9种无对抗后缀或策略规避的文本提示策略，使用标准化协议（5000个新token预算）评估9个开源和闭源模型，通过CSR指标和ECDF量化尾部风险

Result: 观察到长度分布的显著右移和重尾现象，溢出现象在不同模型家族和攻击向量间具有广泛可重复性但存在异质性，简单的简洁性提醒可以减轻右尾并降低CSR

Conclusion: 长度控制应被视为可测量的可靠性、成本和可持续性问题，而非风格怪癖；BenchOverflow为选择最小化资源浪费的部署和评估防御措施提供了实用基础
...
TL;DR: MHEL-LLaMo：一种结合小语言模型和大语言模型的无监督集成方法，用于多语言历史文本实体链接，无需微调即可超越现有方法。
...
Method: 提出MHEL-LLaMo无监督集成方法：1）使用多语言双编码器BELA进行候选实体检索；2）通过指令调优的LLM进行NIL预测和候选选择（提示链）；3）利用SLM置信度区分简单和困难样本，仅对困难样本应用LLM以降低计算成本并避免幻觉。

Result: 在6种欧洲语言（英语、芬兰语、法语、德语、意大利语、瑞典语）的4个19-20世纪历史文本基准测试中，MHEL-LLaMo无需微调即超越了最先进模型，为低资源历史实体链接提供了可扩展解决方案。

Conclusion: MHEL-LLaMo通过SLM和LLM的智能集成，有效解决了历史文本实体链接的挑战，在保持高性能的同时降低了计算成本，为低资源历史NLP任务提供了实用解决方案。
...
TL;DR: STAGE是一个统一的电影剧本叙事理解基准，包含知识图谱构建、场景级事件摘要、长上下文剧本问答和角色扮演四个任务，用于评估模型构建故事世界表示并进行一致性推理的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对问答或对话生成等独立子任务，缺乏评估模型构建连贯故事世界并在多种推理和生成任务中保持一致性的能力。电影剧本作为丰富的长篇叙事，包含复杂的人物关系、时序事件和对话驱动交互，需要更全面的评估框架。

Method: STAGE定义了四个基于共享叙事世界表示的任务：1)知识图谱构建，2)场景级事件摘要，3)长上下文剧本问答，4)剧本内角色扮演。基准提供150部英文和中文电影的清洗剧本、精心策划的知识图谱、事件和人物中心标注。

Result: STAGE基准包含150部电影的完整数据集，涵盖英文和中文，提供清洗后的剧本、知识图谱、事件和人物标注，支持对模型构建世界表示、抽象和验证叙事事件、长叙事推理以及生成人物一致性响应的全面评估。

Conclusion: STAGE为长篇电影剧本的叙事理解提供了一个统一的评估框架，通过四个相互关联的任务全面评估模型构建和使用故事世界表示的能力，填补了现有基准在评估叙事连贯性和一致性方面的空白。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地集成CoT等推理机制，显式推理暴露了推理时后门攻击的新攻击面。这些攻击注入恶意推理路径而不改变模型参数，且由于生成语言连贯的路径，能有效规避传统检测方法。

Method: 提出STAR（状态转移放大比）框架，通过分析输出概率偏移检测后门。该框架利用恶意输入诱导路径在模型通用知识中先验概率低但后验概率高的统计差异，量化状态转移放大，并采用CUSUM算法检测持续异常。

Result: 在多种模型（8B-70B）和五个基准数据集上的实验表明，STAR展现出强大的泛化能力，始终实现接近完美的性能（AUROC ≈ 1.0），效率比现有基线提高约42倍。该框架对试图绕过检测的自适应攻击也具有鲁棒性。

Conclusion: STAR框架为检测推理时后门攻击提供了一种高效、鲁棒的解决方案，通过分析输出概率的统计异常来识别恶意推理路径，在保持高检测性能的同时显著提升了效率。
...
TL;DR: 论文系统研究了无限维空间中无条件收敛的七种等价条件及其在计算算法中的应用，揭示了有限维与无限维收敛性质的差异。


<details>
  <summary>Details</summary>
Motivation: 无限维空间中条件收敛、无条件收敛和绝对收敛的区分对计算算法具有基础性影响。在有限维空间中这些概念一致，但在一般Banach空间中它们严格分离，这需要通过Dvoretzky-Rogers定理来理解。研究旨在为计算实践中的顺序无关和数值稳定的求和过程提供严格理论基础。

Method: 提出一个统一的特征定理，建立了无条件收敛的七种等价条件：排列不变性、网收敛、子级数检验、符号稳定性、有界乘子性质、弱一致收敛等。通过理论分析将这些经典泛函分析概念与计算实践联系起来。

Result: 系统表征了无条件收敛的多种等价条件，为算法稳定性分析提供了理论基础。这些结果直接指导了随机梯度下降中梯度累积的排列不变性，以及基于框架的信号处理中的系数阈值化等计算实践。

Conclusion: 该研究架起了经典泛函分析与当代计算实践之间的桥梁，为顺序无关和数值稳健的求和过程提供了严格的理论基础，揭示了无限维空间中收敛性质的微妙差异对计算算法的深刻影响。
...
TL;DR: Deep Research Bench II：一个包含132个研究任务、9430个细粒度二元评分标准的基准测试，用于评估深度研究系统生成报告的质量，涵盖信息回忆、分析和呈现三个维度。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究基准测试存在两个主要问题：1）未能充分测试系统分析证据和撰写连贯报告的能力；2）依赖过于粗糙或由LLM直接定义的评估标准，导致评分相对于人类专家存在偏差且难以验证或解释。

Method: 构建包含132个研究任务、覆盖22个领域的基准测试；为每个任务创建总计9430个细粒度二元评分标准，涵盖信息回忆、分析和呈现三个维度；通过四阶段LLM+人工流程构建评分标准，结合自动提取和超过400小时专家评审，确保标准具有原子性、可验证性且与人类专家判断一致。

Result: 评估多个最先进的深度研究系统发现，即使最强的模型也只能满足不到50%的评分标准，揭示了当前深度研究系统与人类专家之间存在显著差距。

Conclusion: Deep Research Bench II提供了一个严谨、可验证且与人类专家判断一致的评估框架，揭示了当前深度研究系统的局限性，并为未来系统改进提供了明确方向。
...
TL;DR: Ministral 3系列是参数高效的密集语言模型家族，包含3B、8B、14B三种规模，每个规模都有基础预训练、指令微调和推理三种变体，采用级联蒸馏方法训练，具备图像理解能力，开源许可为Apache 2.0。


<details>
  <summary>Details</summary>
Motivation: 针对计算和内存受限的应用场景，开发参数高效的密集语言模型，为不同需求提供多种规模的选择，同时确保模型具备通用性和复杂问题解决能力。

Method: 采用级联蒸馏方法，通过迭代剪枝和持续训练结合蒸馏技术来推导模型。每个模型都具备图像理解能力，并提供三种变体：基础预训练模型、指令微调模型和推理模型。

Result: 发布了Ministral 3系列模型，包含3B、8B、14B三种参数规模，每个规模都有三种变体，全部采用Apache 2.0开源许可，适用于计算和内存受限的应用场景。

Conclusion: Ministral 3系列为资源受限环境提供了高效的密集语言模型解决方案，通过级联蒸馏方法实现了参数效率，同时保持了多任务能力和图像理解功能，为广泛的应用场景提供了灵活选择。
...
TL;DR: ExpSeek：一种将经验干预从全局上下文转变为步骤级主动寻求的方法，通过熵阈值确定干预时机，显著提升Web智能体性能


<details>
  <summary>Details</summary>
Motivation: 现有Web智能体经验注入方法主要在任务执行前被动注入全局上下文，难以适应智能体与环境交互过程中动态变化的上下文观察，需要更细粒度的主动经验寻求机制

Method: 提出ExpSeek框架，实现步骤级主动经验寻求：1) 利用模型内在信号估计步骤级熵阈值来确定干预时机；2) 设计步骤级定制化经验内容

Result: 在Qwen3-8B和32B模型上的四个挑战性Web智能体基准测试中，ExpSeek分别实现9.3%和7.5%的绝对性能提升，验证了熵作为自触发信号的可行性，并发现4B小规模经验模型也能显著提升更大智能体模型的性能

Conclusion: 步骤级主动经验寻求相比全局被动经验注入更具优势，熵作为自触发信号具有可行性，小规模经验模型也能有效提升大型智能体性能，为Web智能体经验干预提供了新范式
...
TL;DR: OrderProbe是一个用于评估大语言模型结构重建能力的确定性基准，使用中文、日文和韩文的固定四字符表达式，支持精确匹配评分，并提出了超越恢复准确率的诊断框架。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在语义理解方面表现出色，但从乱序输入中重建内部结构的能力尚未得到充分探索。句子级恢复问题对于自动评估来说是不适定的，因为通常存在多种有效的词序。

Method: 引入OrderProbe基准，使用中文、日文和韩文的固定四字符表达式，这些语言具有唯一的规范顺序，因此支持精确匹配评分。进一步提出了一个诊断框架，评估模型在恢复准确率之外的多个维度：语义保真度、逻辑有效性、一致性、鲁棒性敏感度和信息密度。

Result: 在12个广泛使用的大语言模型上的实验表明，结构重建即使对于前沿系统仍然困难：零样本恢复准确率经常低于35%。观察到语义回忆和结构规划之间存在一致的分离，表明结构鲁棒性不是语义能力的自动副产品。

Conclusion: 结构重建是大语言模型面临的一个挑战性任务，需要专门的评估基准。语义能力和结构规划能力之间存在分离，表明需要专门的方法来提升模型的结构重建能力。
...
TL;DR: LALITA框架通过基于词汇和语言特征筛选源句子，在低资源环境下优化机器翻译训练数据选择，显著提升翻译质量并减少数据需求


<details>
  <summary>Details</summary>
Motivation: 低资源语言机器翻译面临数据获取成本高的问题，需要开发有效的数据筛选框架来优化训练数据选择，提高MT系统性能

Method: 提出LALITA框架，使用词汇和语言特征分析源句子，通过筛选复杂句子构建平行语料库，在50K到800K不同数据规模下测试

Result: LALITA显著提升翻译质量，在多种语言（印地语、奥里亚语、尼泊尔语、挪威尼诺斯克语、德语）上减少超过一半的数据需求

Conclusion: LALITA框架有效解决了低资源机器翻译的数据筛选问题，不仅降低训练成本，还展示了在数据增强方面的应用潜力
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要通过直接探测或人口统计角色工程来评估LLMs的政治倾向，但社会心理学认为政治意识形态是基本道德直觉的下游结果。研究者希望探究道德价值观与政治立场之间的因果关系，而非简单地分配人口统计角色。

Method: 将道德取向作为可控条件，通过让模型认可或拒绝特定道德价值观，然后使用政治罗盘测试评估由此产生的政治取向变化。将道德价值观作为"透镜"，观察道德条件化如何主动引导模型在经济和社会维度上的轨迹。

Result: 研究发现道德条件化能引起模型政治坐标的显著、特定于价值观的偏移。这些效应受角色框架和模型规模的系统性调节，并且在实例化相同道德价值观的替代评估工具中具有稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 潜在习语表达（PIEs）与特定语言社区的日常经验密切相关，是评估NLP系统语言和文化能力的理想测试对象。当前缺乏能够系统比较不同语言和文化中习语表达模式的多语言多模态数据集。

Method: 由语言专家创建，包含34种语言的平行数据集，每个PIE配有5张图像，涵盖从习语意义到字面意义的连续谱，包括语义相关和随机干扰项。文本和视觉组件均在多语言指南下制作。

Result: 创建了高质量的多语言多模态习语理解基准数据集，包含超过一万个项目，支持跨语言和跨模态的比较分析，能够评估模型在不同语言间的习语理解迁移能力。

Conclusion: XMPIE数据集为评估多语言和多模态习语理解提供了有价值的基准，有助于研究语言间习语理解的迁移性以及文本与视觉模态间的理解关联。
...
TL;DR: 该论文首次对安全语言生成进行理论分析，基于极限学习范式，证明安全语言识别不可行，安全语言生成至少与普通语言识别一样困难（同样不可行），并讨论了部分可解与不可解情况。


<details>
  <summary>Details</summary>
Motivation: 随着极限学习领域的发展，语言生成已被证明是可行的，但需要在实际应用中考虑安全语言生成的理论基础。现有研究缺乏对安全语言生成的理论分析，本文旨在填补这一空白。

Method: 基于极限学习的计算范式，形式化定义安全语言识别和安全语言生成任务。通过理论证明分析其计算复杂性，并探讨不同情况下的可解性。

Result: 1. 安全语言识别在极限学习模型下是不可能的；2. 安全语言生成至少与普通语言识别一样困难，因此也是不可能的；3. 识别了若干可解和不可解的具体情况。

Conclusion: 安全语言生成在理论上是极具挑战性的问题，其难度不低于传统语言识别。虽然存在某些特殊情况可能可解，但总体上安全语言生成在极限学习框架下是计算不可行的。
...
TL;DR: RULERS框架通过将自然语言评分标准编译为可执行规范，解决LLM作为评分者的对齐问题，显著提升与人类评分的一致性、稳定性和可验证性。


<details>
  <summary>Details</summary>
Motivation: LLM作为评分者的范式虽然具有可扩展性，但冻结的黑盒模型与人类评分标准对齐仍面临挑战，主要问题包括：提示敏感性导致的评分标准不稳定性、缺乏可审计证据的不可验证推理、以及与人类评分边界不匹配的尺度错位。

Method: 提出RULERS（Rubric Unification, Locking, and Evidence-anchored Robust Scoring）编译-执行框架：1）将评分标准编译为版本化不可变包；2）通过结构化解码强制执行确定性证据验证；3）应用轻量级Wasserstein后校准，所有操作均不更新模型参数。

Result: 在论文和摘要基准测试中，RULERS在人类一致性方面显著优于代表性基线方法，对对抗性评分标准扰动保持强稳定性，并使较小模型能够与较大的专有评分模型相媲美。

Conclusion: 可靠的LLM评分需要可执行的评分标准、可验证的证据和校准的尺度，而不仅仅是提示词设计。RULERS框架通过将自然语言评分标准转化为可执行规范，有效解决了LLM评分者的对齐问题。
...
TL;DR: LLMs在仇恨言论净化任务中存在虚假拒绝问题，本文系统研究了触发拒绝的上下文和语言偏见，并提出跨语言翻译策略来缓解此问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在仇恨言论净化任务中经常触发安全警报而拒绝执行，这种虚假拒绝行为影响了模型的实际应用效果，需要系统研究其触发机制和缓解方法

Method: 评估9个LLM在英语和多语言数据集上的表现，分析语义毒性和目标群体对拒绝率的影响，提出跨语言翻译策略（英译中净化后再译回）

Result: LLMs对语义毒性较高、针对特定群体（国籍、宗教、政治意识形态）的输入拒绝率更高；多语言数据集总体拒绝率低于英语，但仍存在系统性语言依赖偏见；跨语言翻译策略显著降低虚假拒绝率并保持内容完整性

Conclusion: LLMs在仇恨言论净化中存在系统性偏见导致的虚假拒绝问题，跨语言翻译提供了一种轻量有效的缓解方法，揭示了模型安全机制与任务执行之间的平衡挑战
...
TL;DR: 工业案例研究：开发智能体系统用于多方对话摘要，分享从评估方法到供应商锁定的全生命周期实践洞察
...
Method: 采用智能体架构进行工业案例研究，通过任务分解实现组件化优化，涵盖评估方法、架构设计、数据瓶颈分析和供应商锁定问题

Result: 提出了在需求演变和任务主观性下的鲁棒评估方法，展示了智能体架构如何支持组件级优化，识别了上游数据瓶颈，并揭示了LLM提示词可移植性差导致的供应商锁定现实

Conclusion: 智能体架构为构建可靠、适应性强的多方对话摘要系统提供了有效框架，但需要解决数据瓶颈和供应商锁定等实际问题，为从业者和未来研究提供了实用指导
...
TL;DR: QuantEval：一个评估大语言模型在量化金融领域能力的基准，涵盖知识问答、数学推理和策略编码三个维度，并包含CTA风格回测框架。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在金融量化任务中的评估零散且主要局限于知识问答，缺乏对数学推理和策略编码能力的系统性评估，需要更全面的基准来评估模型在真实交易工作流中的实用性。

Method: 构建QuantEval基准，包含三个评估维度：知识问答、量化数学推理、量化策略编码；引入CTA风格回测框架执行模型生成的策略并使用金融性能指标评估；对开源和专有LLMs进行评估；进行大规模监督微调和强化学习实验。

Result: 评估发现最先进的LLMs与人类专家存在显著差距，特别是在推理和策略编码方面；通过领域对齐数据的监督微调和强化学习实验显示出持续改进；发布了完整的确定性回测配置以确保严格可复现性。

Conclusion: QuantEval基准填补了LLMs在量化金融领域系统性评估的空白，通过多维度评估和真实回测框架提供了更现实的评估方法，有望促进LLMs在量化金融领域的研究和实际交易工作流中的应用。
...
TL;DR: RAGShaper：自动化合成RAG任务和鲁棒智能体轨迹的数据生成框架，通过信息树和对抗性干扰项提升智能体在噪声环境中的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 当前智能体RAG系统面临高质量训练数据稀缺的问题，传统人工标注无法规模化且难以捕捉处理检索失败所需的动态推理策略，阻碍了鲁棒智能体的开发

Method: 提出RAGShaper框架：1) InfoCurator构建包含感知和认知层面对抗性干扰项的密集信息树；2) 约束导航策略迫使教师智能体面对干扰项，生成显示错误纠正和噪声拒绝的轨迹

Result: 在合成语料上训练的模型显著优于现有基线，在噪声密集和复杂检索任务中表现出卓越的鲁棒性

Conclusion: RAGShaper通过自动化数据合成有效解决了RAG智能体训练数据稀缺问题，为开发鲁棒智能体提供了可扩展的解决方案
...
TL;DR: PrivGemo是一个保护隐私的知识图谱增强推理框架，通过双塔设计和内存引导的暴露控制，在保持原始KG知识本地的同时，支持在匿名视图上进行远程推理，解决了KG隐私保护中的结构泄露、不可控远程交互、脆弱的多跳多实体推理等问题。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱隐私处理方法主要关注实体名称掩码，但仍面临四个关键问题：语义掩码下的结构泄露、不可控的远程交互、脆弱的多跳多实体推理，以及稳定性和效率方面的有限经验复用。这些限制阻碍了私有KG在LLM增强推理中的安全应用。

Method: PrivGemo采用双塔设计：本地塔保留原始KG知识，远程塔在匿名视图上执行推理。通过超越名称掩码的匿名化处理限制语义和结构暴露，检索连接所有主题实体的匿名长跳路径支持多跳多实体推理，同时保持本地KG的验证。分层控制器和隐私感知经验内存减少不必要的探索和远程交互。

Result: 在六个基准测试上的综合实验表明，PrivGemo实现了整体最先进的结果，比最强基线提高了高达17.1%。此外，PrivGemo使较小模型（如Qwen3-4B）能够达到与GPT-4-Turbo相当的推理性能。

Conclusion: PrivGemo通过创新的隐私保护检索增强框架，有效解决了KG增强推理中的隐私泄露问题，在保持高性能的同时实现了隐私保护，为私有知识图谱的安全应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理大规模企业电子表格时存在困难，这些表格通常包含数千行数值数据、多个关联工作表以及嵌入的图表、收据等视觉内容。传统方法依赖单表压缩或全上下文编码，限制了可扩展性，且无法反映用户与复杂多模态工作簿的真实交互方式。

Method: 提出FRTR（From Rows to Reasoning）框架：1）将Excel工作簿分解为细粒度的行、列和块嵌入；2）采用基于互惠排名融合（RRF）的混合词汇-稠密检索；3）集成多模态嵌入以同时处理数值和视觉信息；4）构建FRTR-Bench基准，包含30个企业级工作簿、近400万个单元格和50多个嵌入图像。

Result: 在FRTR-Bench上，FRTR框架在Claude Sonnet 4.5上达到74%的答案准确率，相比之前最优方法的24%有显著提升。在SpreadsheetLLM基准上，FRTR在GPT-5上达到87%准确率，同时相比上下文压缩方法减少了约50%的token使用量。

Conclusion: FRTR框架通过细粒度分解、混合检索和多模态集成，有效解决了LLM在大规模企业电子表格推理中的挑战，显著提升了准确性和效率，为复杂多模态工作簿的智能分析提供了实用解决方案。
...
TL;DR: 该论文提出了Attributional NLI (Att-NLI)框架，将社会心理学原理融入自然语言推理，用于评估LLM在多智能体环境中进行归因推理（预测潜在意图）的能力，并通过Undercover-V游戏实验验证了神经符号方法在归因推理中的优越性。


<details>
  <summary>Details</summary>
Motivation: 传统自然语言推理(NLI)无法捕捉复杂交互系统中基于意图的细微推理，而预测观察行为背后潜在意图的归因推理能力对多智能体环境中的LLM至关重要，但目前尚未得到充分探索。

Method: 提出Attributional NLI框架，扩展NLI并融入社会心理学原理，评估智能体的溯因意图推理（生成潜在意图假设）和演绎验证（得出逻辑结论）能力。通过Undercover-V文本游戏实例化，比较三种LLM智能体：仅使用演绎推理的标准NLI智能体、采用溯因-演绎推理的Att-NLI智能体、以及使用外部定理证明器进行溯因-演绎推理的神经符号Att-NLI智能体。

Result: 实验显示归因推理能力存在明确层次：神经符号智能体始终优于其他类型，平均胜率达到17.08%。标准NLI智能体表现最差，Att-NLI智能体居中，表明神经符号方法在复杂归因推理任务中具有显著优势。

Conclusion: Att-NLI框架有助于开发具有复杂推理能力的智能体，同时突显了神经符号AI在构建多智能体环境中理性LLM智能体的潜在影响。归因推理是多智能体交互的关键能力，神经符号方法为此提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM-based方法在Text-to-SQL任务中通常包含完整数据库模式，导致上下文过长和预填充延迟增加。用户查询通常关注重复的表集合，这为跨查询共享KV缓存提供了机会，但当前推理引擎在处理表顺序不同的查询时会产生冗余的前缀缓存副本。

Method: 1. 离线预计算表表示作为KV缓存，在线查询所需缓存；2. 在计算表缓存时保持表间主外键关系；3. 构建Table Trie结构以支持推理期间高效KV缓存查找；4. 引入缓存管理系统，包括查询重排序策略提高缓存命中率和计算加载管道以并行化模型推理和缓存加载。

Result: TableCache在实验中实现了最高3.62倍的Time to First Token (TTFT)加速，性能下降可忽略不计。

Conclusion: 通过预计算表KV缓存、构建Table Trie结构和引入缓存管理系统，TableCache能有效减少Text-to-SQL任务中的推理延迟，显著提升TTFT性能，同时保持模型准确性。
...
TL;DR: 该论文提出了一种基于注意力的方法，将航空影像和地理定位文本在空间邻域内结合，用于预测环境变量，在瑞士的EcoWikiRS数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 文本作为生态学的新兴数据源，包含独特信息，可与地理空间数据互补，提供传统数据源无法捕捉的局部环境信息。然而，文本数据在生态学中的贡献定义不清，可用性稀疏且不规则，与地理空间数据的整合面临挑战。

Method: 提出基于注意力的方法，结合航空影像和地理定位文本在空间邻域内。方法整合视觉和文本表示与地理位置编码，通过注意力模块动态选择对预测任务有用的空间邻居。

Result: 在EcoWikiRS数据集上评估，预测SWECO25数据立方体的103个环境变量。该方法始终优于单位置或单模态（仅图像或仅文本）基线。按主题组分析变量，结果显示在气候、土壤、人口和土地利用/土地覆盖变量方面性能显著提升。

Conclusion: 结合文本和图像数据时包含空间上下文具有显著优势，注意力机制能有效整合多模态空间信息，提升环境变量预测性能。
...
TL;DR: 提出Multiplex Thinking方法，通过采样K个候选token并聚合其嵌入形成单一连续多路复用token，实现随机软推理机制，在保持词汇嵌入先验的同时压缩推理序列长度


<details>
  <summary>Details</summary>
Motivation: 大语言模型使用思维链（CoT）进行复杂推理时会产生冗长的token序列，而人类推理通常采用软推理方式，维护可能下一步的概率分布。现有离散CoT方法在序列长度和带宽效率方面存在不足

Method: 提出Multiplex Thinking随机软推理机制：在每个思考步骤中采样K个候选token，将其嵌入聚合为单一连续多路复用token。该方法保持词汇嵌入先验和标准离散生成的采样动态，同时诱导可处理的多路复用展开概率分布，支持基于策略的强化学习直接优化

Result: 在多个具有挑战性的数学推理基准测试中，Multiplex Thinking从Pass@1到Pass@1024均一致优于强离散CoT和RL基线方法，同时产生更短的序列长度

Conclusion: Multiplex Thinking是一种自适应的软推理机制，在模型置信度高时近似离散CoT，在不确定时紧凑表示多个可能下一步，在不增加序列长度的情况下提高推理性能，为语言模型推理提供了高效的新范式
...
### [132] [Emergent Coordination in Multi-Agent Systems via Pressure Fields and Temporal Decay](https://arxiv.org/abs/2601.08129)
*Roland Rodriguez*

Main category: cs.MA

TL;DR: 压力场协调范式通过共享压力梯度实现多智能体隐式协调，性能与显式层次控制相当，显著优于对话式协调


<details>
  <summary>Details</summary>
Motivation: 现有基于人类组织结构的显式协调模式（规划者-执行者、管理者-工作者等）存在协调开销随智能体数量和任务复杂度急剧增加的问题，需要更自然的协调机制

Method: 提出受自然协调机制启发的压力场协调范式：智能体在共享工件上局部操作，仅由可测量质量信号导出的压力梯度引导，通过时间衰减防止过早收敛；形式化为压力景观优化问题

Result: 在拉丁方约束满足任务的1078次试验中，压力场协调与层次控制性能相当（38.2% vs 38.8%解决率，p=0.94），均显著优于顺序（23.3%）、随机（11.7%）和对话式协调（8.6%）；时间衰减至关重要，禁用时最终压力增加49倍；简单问题解决率达87%；2-32个智能体间性能稳定

Conclusion: 通过共享压力梯度的隐式协调能够达到与显式层次控制相当的效果，同时显著优于显式对话式协调，表明约束驱动的涌现为多智能体AI提供了更简单且同样有效的基础
...
### [133] [MLLM-VADStory: Domain Knowledge-Driven Multimodal LLMs for Video Ad Storyline Insights](https://arxiv.org/abs/2601.07850)
*Jasmine Yang,Poppy Zhang,Shawndra Hill*

Main category: cs.MM

TL;DR: MLLM-VADStory是一个基于领域知识指导的多模态大语言模型框架，用于大规模量化分析视频广告故事情节，通过功能意图分析广告叙事结构，识别高效故事弧线以指导创意设计。


<details>
  <summary>Details</summary>
Motivation: 视频广告的叙事结构对广告效果有重要影响，但缺乏系统化、可扩展的分析方法。现有方法难以大规模理解广告故事情节的功能结构和效果影响，需要结合领域知识来指导多模态大语言模型进行量化分析。

Method: 提出MLLM-VADStory框架：1）将广告分割为功能单元；2）使用新颖的广告特定功能角色分类法对每个单元进行分类；3）跨广告聚合功能序列以恢复数据驱动的故事情节结构；4）应用于5万个社交媒体视频广告，分析故事型创意对视频留存率的影响。

Result: 在四个行业子垂直领域的5万个社交媒体视频广告分析中发现：故事型创意能提高视频留存率；识别出表现最佳的故事弧线，为广告主提供创意设计指导；框架证明了领域知识指导MLLMs在生成可扩展的视频广告故事情节洞察方面的价值。

Conclusion: MLLM-VADStory框架展示了使用领域知识指导多模态大语言模型生成可扩展视频广告故事情节洞察的价值，成为理解视频创意内容的通用工具，为广告创意设计提供数据驱动的决策支持。
...
### [134] [Bridging the Trust Gap: Clinician-Validated Hybrid Explainable AI for Maternal Health Risk Assessment in Bangladesh](https://arxiv.org/abs/2601.07866)
*Farjana Yesmin,Nusrat Shirmin,Suraiya Shabnam Bristy*

Main category: cs.AI

TL;DR: 提出混合可解释AI框架，结合模糊逻辑与SHAP解释，用于资源受限地区的孕产妇健康风险预测，通过临床医生验证获得信任


<details>
  <summary>Details</summary>
Motivation: 机器学习在孕产妇健康风险预测中虽有潜力，但在资源受限的临床环境中面临可解释性和信任度不足的关键障碍，需要开发既能提供准确预测又能获得临床医生信任的解释方法

Method: 开发混合可解释AI框架，结合前向模糊逻辑与后向SHAP解释；基于1,014份孕产妇健康记录构建模糊-XGBoost模型；通过14名孟加拉国医疗专业人员的系统性临床反馈进行验证

Result: 模型准确率达88.67%（ROC-AUC：0.9703）；71.4%的临床医生偏好混合解释方法，54.8%表示信任可用于临床；SHAP分析显示医疗可及性是主要预测因子，模糊风险评分排名第三验证了临床知识整合（r=0.298）

Conclusion: 结合可解释模糊规则与特征重要性解释能同时提升实用性和信任度，为孕产妇医疗保健中的可解释AI部署提供实用见解，但需整合产科史、孕周和连接障碍等关键缺失参数
...
### [135] [When Models Know When They Do Not Know: Calibration, Cascading, and Cleaning](https://arxiv.org/abs/2601.07965)
...
<details>
  <summary>Details</summary>
Motivation: 当模型能够识别自身未知情况时，会带来多种可能性。关键问题是如何让模型意识到自己不知道。现有研究表明校准可以提供可靠的置信度估计，但需要一种简单有效且通用的方法。

Method: 提出无需训练的通用校准方法，适用于视觉和语言模型。基于两个关键观察：1) 单个模型内置信度越高准确率越高；2) 验证集校准的模型在测试集上保持校准。应用包括：1) 基于校准优势路由的模型级联；2) 基于模型集成和校准置信度的数据清洗方法。

Result: 通过校准置信度的可比性，级联大小模型可在几乎不影响准确率的情况下提高效率；级联两个规模相当的模型可获得超越任一单独模型的性能。数据清洗方法在ImageNet和MMLU数据集上有效识别错误标注样本，平衡了精度和检测率。

Conclusion: 让模型识别自身未知情况是实现更高效、可靠、可信AI的实际步骤。提出的校准方法简单有效，为模型级联和数据清洗等应用提供了可靠基础。
...
### [136] [Reasoning over Precedents Alongside Statutes: Case-Augmented Deliberative Alignment for LLM Safety](https://arxiv.org/abs/2601.08000)
...
TL;DR: 论文提出CADA方法，通过案例增强的审慎对齐来提升LLM安全性，避免过度拒绝良性请求，相比仅使用详细安全规则的方法效果更好。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全对齐方法面临挑战：OpenAI的审慎对齐(DA)使用详细"代码式"安全规则，但在开源LLM中效果未充分研究。详细规则可能导致不一致的安全提升和系统性的帮助性下降。

Method: 提出CADA（案例增强审慎对齐）方法：使用强化学习训练LLM基于自生成的安全推理链进行决策，通过案例增强的简单代码而非详细规则来指导模型，避免对狭窄枚举规则的僵化遵守。

Result: CADA有效提升无害性，增强对抗攻击的鲁棒性，减少过度拒绝，同时在多样化基准测试中保持实用性。相比仅使用规则的DA方法，案例增强方法产生更稳健和泛化的安全行为。

Conclusion: 案例增强的审慎对齐为提升LLM安全性同时保持帮助性提供了实用替代方案，通过推理链而非详细规则实现更灵活的安全适应能力。
...
### [137] [Internal Deployment Gaps in AI Regulation](https://arxiv.org/abs/2601.08005)
*Joe Kwon,Stephen Casper*

Main category: cs.AI

TL;DR: 该论文分析了美欧前沿AI法规在内部部署系统监管方面的三大漏洞，探讨了这些漏洞的成因，并提出了可能的解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前前沿AI法规主要关注面向外部用户的系统部署，而忽视了企业内部部署的高风险应用场景。企业可能将高能力AI系统用于研发自动化、关键业务流程加速和处理敏感专有数据等内部用途，这些应用同样具有高风险性，但可能逃避现有法规的监管。

Method: 论文采用政策分析方法，系统研究了2025年美国与欧盟的前沿AI法规框架。通过识别监管漏洞、分析漏洞持续存在的原因（可测量性、激励措施和信息获取等方面的矛盾），并绘制了解决这些问题的潜在路径及其权衡。

Result: 识别出三个关键监管漏洞：1) 范围模糊性使内部系统逃避监管义务；2) 一次性合规评估无法捕捉内部系统的持续演进；3) 信息不对称削弱监管意识和监督能力。这些漏洞源于可测量性、激励机制和信息获取之间的固有矛盾。

Conclusion: 论文呼吁政策制定者应有意而非偶然地制定针对内部部署AI系统的监管政策。通过理解这些模式，可以更有效地设计监管框架，填补内部部署AI系统的监管空白，平衡创新与风险控制的需求。
...
### [138] [Integrating Attendance Tracking and Emotion Detection for Enhanced Student Engagement in Smart Classrooms](https://arxiv.org/abs/2601.08049)
*Keith Ainebyona,Ann Move Oguti,Joseph Walusimbi,Ritah Kobusingye*

Main category: cs.AI

TL;DR: SCASED系统结合物联网、人脸识别和情感识别技术，在智能教室中同时实现自动化考勤和学生学习情感状态监测，为教师提供课堂动态的实时洞察。


<details>
  <summary>Details</summary>
Motivation: 当前高等教育中的智能教室技术主要关注自动化考勤，而忽视了学生在课堂中的情感和认知参与度。这种局限性使得教师难以实时识别学生的参与度下降并调整教学策略。

Method: 提出SCASED系统，采用物联网架构，使用树莓派摄像头和OpenCV进行人脸检测，通过微调的MobileNetV2模型分类四种学习相关情感状态（投入、无聊、困惑、沮丧）。系统采用基于会话的机制管理考勤和情感监测，考勤记录一次后持续进行情感分析，数据通过云端仪表板可视化。

Result: 使用DAiSEE数据集进行实验评估，情感分类准确率达到89.5%。结果表明，将考勤数据与情感分析相结合，能为教师提供额外的课堂动态洞察，支持更及时响应的教学实践。

Conclusion: SCASED系统通过整合考勤跟踪和情感识别，为教师提供了更全面的课堂参与度监测工具，有助于实现更个性化的教学调整和响应式教学实践。
...
### [139] [Forecast Aware Deep Reinforcement Learning for Efficient Electricity Load Scheduling in Dairy Farms](https://arxiv.org/abs/2601.08052)
*Nawazish Alia,Rachael Shawb,Karl Mason*

Main category: cs.AI

TL;DR: 提出基于深度强化学习的乳牛场智能负载调度框架，通过改进PPO算法结合短期预测和自适应KL散度控制，在电池储能和热水系统调度中实现成本降低和电网依赖减少。


<details>
  <summary>Details</summary>
Motivation: 乳牛场是能源密集型行业，严重依赖电网电力。随着可再生能源集成增加，需要可持续能源管理来减少电网依赖并支持联合国可持续发展目标7（可负担清洁能源）。可再生能源间歇性给实时供需平衡带来挑战，现有RL调度方法通常假设完全了解未来价格或发电量，这在动态环境中不现实，且标准PPO变体依赖固定阈值导致训练不稳定。

Method: 提出深度强化学习框架用于乳牛场高效负载调度，重点关注电池储能和热水系统。提出两种改进PPO算法：1) Forecast Aware PPO：结合短期需求与可再生能源预测，使用基于小时和月份的残差校准；2) PID KL PPO：采用比例积分微分控制器自适应调节KL散度，实现稳定的策略更新。在真实乳牛场数据上训练。

Result: 方法相比PPO降低1%电力成本，相比DQN降低4.8%，相比SAC降低1.5%。在电池调度方面，PPO减少13.1%电网输入，展示了在现代乳牛场可持续能源管理中的可扩展性和有效性。

Conclusion: 提出的深度强化学习框架通过结合短期预测和自适应KL控制，有效解决了乳牛场能源调度中的实际问题，在降低成本和减少电网依赖方面表现出色，为现代乳牛场可持续能源管理提供了可行解决方案。
...
### [140] [A New Strategy for Verifying Reach-Avoid Specifications in Neural Feedback Systems](https://arxiv.org/abs/2601.08065)
...
<details>
  <summary>Details</summary>
Motivation: 现有后向可达性方法可扩展性有限，而前向可达性分析在验证神经反馈系统的可达-避免属性中占主导地位，需要更高效的后向分析方法
...
### [141] [Semantic Gravity Wells: Why Negative Constraints Backfire](https://arxiv.org/abs/2601.08070)
*Shailesh Rana*

Main category: cs.AI

TL;DR: 该论文首次对大型语言模型中负约束（"不要使用词X"指令）失败机制进行了全面研究，揭示了失败概率与语义压力之间的逻辑关系，并识别出两种不同的失败模式。


<details>
  <summary>Details</summary>
Motivation: 负约束是测试语言模型指令遵循能力的基本测试，但这类约束经常失败且失败机制不明确。论文旨在理解负约束失败的条件和机制。

Method: 引入语义压力作为量化指标，使用logit lens技术进行分层分析，识别失败模式，并通过激活修补验证因果机制。

Result: 发现违反概率与语义压力呈紧密逻辑关系(p=σ(-2.40+2.27·P₀))；失败时抑制信号显著减弱(5.2% vs 22.8%)；识别出两种失败模式：启动失败(87.5%)和覆盖失败(12.5%)；确认第23-27层是因果责任层。

Conclusion: 负约束设计存在根本性矛盾：明确提及禁止词汇反而会启动模型产生该词。这揭示了语言模型指令遵循机制的内在局限性。
...
### [142] [MemoBrain: Executive Memory as an Agentic Brain for Reasoning](https://arxiv.org/abs/2601.08079)
*Hongjin Qian,Zhao Cao,Zheng Liu*

Main category: cs.AI

TL;DR: MemoBrain是一个用于工具增强型智能体的执行记忆模型，通过构建依赖感知的记忆来管理长时推理过程中的中间状态和逻辑关系，解决上下文积累问题。
...
Method: 提出MemoBrain执行记忆模型，为工具增强型智能体构建依赖感知的记忆，捕获关键的中间状态及其逻辑关系。作为推理智能体的协同伙伴，MemoBrain在不阻塞执行的情况下组织推理进度，并主动管理工作上下文。具体机制包括：修剪无效步骤、折叠完成的子轨迹、在固定上下文预算下保持紧凑的高显著性推理主干。

Result: 在具有挑战性的长时程基准测试（包括GAIA、WebWalker和BrowseComp-Plus）上评估MemoBrain，相比强基线模型展示了一致的改进。

Conclusion: MemoBrain通过显式认知控制机制管理推理轨迹，而非被动积累上下文，为工具增强型智能体提供了维持长时程连贯推理的核心记忆能力。
...
TL;DR: 论文研究了交通事故后车辆绕行时的换道行为（post crash lane changes），发现其风险高于常规换道，开发了基于图注意力机制和交互感知的轨迹预测框架，显著提升了预测精度。


<details>
  <summary>Details</summary>
Motivation: 交通事故后车辆需要绕行障碍物进行换道，但目标车道车辆可能拒绝让行，增加了换道复杂性和碰撞风险。然而，事故后换道的行为特征和运动模式尚不明确，需要系统研究。

Method: 1. 从无人机视频中提取车辆轨迹构建事故后换道数据集；2. 通过实证分析比较事故后换道与强制换道、自由换道的差异；3. 开发基于图注意力的轨迹预测框架，将让行行为建模为辅助交互感知任务，指导条件变分自编码器和Transformer解码器进行轨迹预测。

Result: 1. 事故后换道持续时间更长、插入速度更低、碰撞风险更高；2. 79.4%的事故后换道涉及新跟随者至少一次不让行行为，远高于自由换道（21.7%）和强制换道（28.6%）；3. 提出的模型在不同预测时域上平均位移误差和最终位移误差均优于基线10%以上；4. 模型降低了误碰撞率，提高了冲突预测准确性；5. 在不同地点收集的数据集上验证了模型的可迁移性。

Conclusion: 事故后换道具有独特的风险特征，特别是目标车道车辆不让行行为频繁。提出的交互感知轨迹预测框架能有效捕捉这些复杂交互，显著提升预测精度和碰撞风险评估可靠性，为智能交通系统安全决策提供了有力工具。
...
TL;DR: 提出一种在边缘设备上运行的AI伴侣系统，通过活跃/非活跃阶段交替的内存范式，在资源受限环境下实现低延迟对话和长期个性化


<details>
  <summary>Details</summary>
Motivation: 边缘设备的计算资源限制使得开发完全嵌入式的AI伴侣系统面临挑战，现有AI伴侣和内存系统无法直接应用于此类环境，主要问题包括计算资源不足和延迟问题

Method: 提出交替活跃/非活跃阶段的内存范式：用户活跃阶段使用轻量级检索进行低延迟实时对话；用户非活跃阶段进行计算密集的记忆提取、整合和维护。同时设计了AI伴侣基准测试来全面评估对话质量和记忆能力

Result: 实验表明，使用非常弱的模型（Qwen2.5-7B-Instruct量化int4）的系统在大多数指标上优于无内存的原始LLM，性能与具有16k上下文窗口的GPT-3.5相当
...
### [145] [Project Synapse: A Hierarchical Multi-Agent Framework with Hybrid Memory for Autonomous Resolution of Last-Mile Delivery Disruptions](https://arxiv.org/abs/2601.08156)
*Arin Gopalan Yadav,Varad Dherange,Kumar Shivam*

Main category: cs.AI

TL;DR: Project Synapse是一个用于自主解决最后一公里配送中断的新型智能体框架，采用分层多智能体架构，通过LangGraph编排复杂工作流，并在真实场景数据集上进行评估。


<details>
  <summary>Details</summary>
Motivation: 最后一公里配送中断是物流行业的重要挑战，传统方法难以自主处理复杂多变的现实场景。需要开发能够自主分解和执行解决策略的智能系统。

Method: 采用分层多智能体架构：中央Resolution Supervisor负责战略任务分解，将子任务委托给专门的worker agents进行战术执行。使用LangGraph编排复杂循环工作流。基于6000多条真实用户评论构建30个复杂中断场景的基准数据集，采用LLM-as-a-Judge协议进行性能评估，并包含显式偏差缓解措施。

Result: 论文通过构建的基准数据集验证了框架的有效性，使用LLM-as-a-Judge评估协议对系统性能进行了量化分析，结果显示系统能够有效处理复杂的配送中断场景。

Conclusion: Project Synapse提供了一个有效的自主解决最后一公里配送中断的智能体框架，其分层架构和LangGraph编排机制能够处理现实世界的复杂场景，为物流自动化提供了新的解决方案。
...
### [146] [ZeroDVFS: Zero-Shot LLM-Guided Core and Frequency Allocation for Embedded Platforms](https://arxiv.org/abs/2601.08166)
...
<details>
  <summary>Details</summary>
Motivation: 现有DVFS和任务分配方法存在局限性：基于利用率的启发式方法忽略停顿时间，基于离线分析的方法需要大量配置表且无法运行时适应。需要一种能动态适应、减少分析开销、提高能效和热管理的调度方案。

Method: 1) 分层多智能体强化学习框架：两个协作智能体分解指数级动作空间；2) 环境模型：使用回归技术预测热动态和性能状态；3) LLM语义特征提取：从OpenMP程序中提取13个代码级特征，无需执行；4) Dyna-Q启发框架：结合直接强化学习和基于模型的规划；5) 零样本部署：利用LLM特征和环境模型生成合成训练数据，无需特定工作负载分析样本。

Result: 1) 决策延迟：后续决策358ms，首次决策3.5-8.0s（含一次性LLM特征提取）；2) 收敛速度：比无模型方法快20倍；3) 性能提升：比Linux ondemand governor能效高7.09倍，makespan好4.0倍；4) 首次决策延迟：比基于表的方法快8,300倍；5) 平台验证：在NVIDIA Jetson TX2、Jetson Orin NX、RubikPi和Intel Core i7上测试BOTS和PolybenchC基准程序。

Conclusion: 提出的基于模型的分层MARL框架结合LLM语义特征提取，实现了高效的热和能量感知调度，支持零样本部署，显著优于现有方法，适用于动态嵌入式系统的实际部署。
...
### [147] [Improving LLM Reasoning with Homophily-aware Structural and Semantic Text-Attributed Graph Compression](https://arxiv.org/abs/2601.08187)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法受限于上下文窗口，通常采用随机采样（随机丢弃节点/边），这会引入噪声并导致推理不稳定。图本身包含丰富的结构和语义信息，有效利用这些信息可以释放LLMs推理性能的潜在增益。

Method: 提出HS2C框架，基于图同质性进行结构和语义压缩。结构上，通过结构熵最小化原则进行全局层次划分，识别自然内聚的同质性社区并丢弃随机连接噪声。语义上，将检测到的结构同质性传递给LLM，使其能够基于预定义社区类型进行差异化的语义聚合，将冗余背景上下文压缩为简洁的社区级共识。

Result: 在10个节点级基准测试和7个图级基准测试上的广泛实验表明，通过向LLMs提供结构和语义压缩的输入，HS2C同时提高了压缩率和下游推理准确率，验证了其优越性和可扩展性。

Conclusion: HS2C框架通过利用图同质性进行有效的结构和语义压缩，显著提升了LLMs在图理解任务中的性能，展示了在节点级和图级任务上的良好泛化能力。
...
### [148] [Adapting Rules of Official International Mahjong for Online Players](https://arxiv.org/abs/2601.08211)
*Chucai Wang,Lingfeng Li,Yunlong Lu,Wenxin Li*

Main category: cs.AI

TL;DR: 使用世界冠军AI进行自对弈统计分析，发现国际麻将在线单局游戏存在先手优势和子目标计分问题，提出引入补偿分机制和优化牌型计分规则，使传统麻将更适合在线环境。


<details>
  <summary>Details</summary>
Motivation: 国际麻将作为全球流行的传统游戏，在线玩家具有碎片化游戏时间和不固定对手的特点，而传统规则是为线下多轮固定对手设计的。需要修改规则以确保在线单局游戏的公平性。

Method: 使用世界冠军AI进行自对弈比赛，通过统计分析揭示游戏平衡性问题。具体分析先手优势和子目标计分设置的问题。

Result: 研究发现存在明显的先手优势和子目标计分问题。提出规则调整方案：引入先手优势补偿分机制，优化不同牌型子目标的计分设置。

Conclusion: 相比传统多轮轮换位置平衡先手优势的方法，每轮补偿分机制更适合在线玩家。实现了修订版在线麻将游戏，这是利用AI系统数据评估游戏平衡性并开发更适合在线玩家的传统游戏修订版的初步尝试。
...
### [149] [An Axiomatic Approach to General Intelligence: SANC(E3) -- Self-organizing Active Network of Concepts with Energy E3](https://arxiv.org/abs/2601.08224)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统预设固定的原始单元（如token、像素等），绕过了表征单元如何涌现和稳定的根本问题。本文旨在提出一个理论框架，解释表征单元如何从经验中自组织形成，从而为通用智能提供更基础的理论基础。

Method: 提出SANC(E3)公理化框架，包含五个核心公理：有限容量、共现关联、基于相似性的竞争、基于置信度的稳定化、重建-压缩-更新的权衡。引入伪内存映射I/O机制，使内部回放的格式塔与外部感官输入通过相同公理路径处理。

Result: 从公理推导出12个命题，表明类别形成、层次组织、无监督学习和高级认知活动都可以理解为E3最小化下的格式塔完成过程。该框架统一了感知、想象、预测、规划和行动为单一的表征和能量过程。

Conclusion: SANC(E3)为理解表征单元如何从经验中涌现提供了理论基础，将高级认知功能统一到格式塔完成的能量最小化框架中，为构建更接近生物智能的AI系统提供了新的理论方向。
...
### [150] [MPCI-Bench: A Benchmark for Multimodal Pairwise Contextual Integrity Evaluation of Language Model Agents](https://arxiv.org/abs/2601.08235)
*Shouju Wang,Haopeng Zhang*

Main category: cs.AI

TL;DR: MPCI-Bench是首个用于评估智能体隐私行为的多模态成对上下文完整性基准，包含三个层级的数据实例，揭示当前多模态模型在隐私-效用权衡和模态泄露方面的系统性缺陷。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型从被动聊天机器人演变为处理个人数据的主动助手，评估其遵守社会规范（特别是上下文完整性CI）变得至关重要。现有CI基准主要是文本中心且侧重负面拒绝场景，忽视了多模态隐私风险和隐私与效用的基本权衡。

Method: 提出MPCI-Bench基准，包含从相同视觉源衍生的成对正负实例，分为三个层级：规范性种子判断、上下文丰富的故事推理、可执行的智能体行动轨迹。采用三原则迭代精炼流程确保数据质量。

Result: 对最先进多模态模型的评估显示：1）系统性地未能平衡隐私与效用；2）存在明显的模态泄露差距，敏感视觉信息比文本信息泄露更频繁。

Conclusion: MPCI-Bench是首个用于评估智能体环境中隐私行为的多模态成对上下文完整性基准，将开源以促进未来关于智能体CI的研究，揭示了当前模型在隐私保护方面的关键缺陷。
...
### [151] [The End of Reward Engineering: How LLMs Are Redefining Multi-Agent Coordination](https://arxiv.org/abs/2601.08237)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中奖励工程面临信用分配模糊、环境非平稳性和交互复杂性组合增长等挑战，需要更有效的奖励规范方法。

Method: 提出利用大语言模型实现语义奖励规范、动态奖励适应和人类意图对齐，借鉴EUREKA、CARD和RLVR等现有方法。

Result: 语言介导的监督可以作为传统奖励工程的可行替代方案，但存在计算开销、幻觉鲁棒性和大规模系统可扩展性等开放挑战。

Conclusion: 未来研究方向是从共享语义表示而非显式工程化的数值信号中产生协调，实现更自然的智能体行为引导。
...
### [152] [T3: Benchmarking Sycophancy and Skepticism in Causal Judgment](https://arxiv.org/abs/2601.08258)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: T3是一个诊断基准，用于评估LLM在Pearl因果阶梯上的因果判断能力，包含454个专家策划的案例，分析效用、安全性和明智拒绝三个维度，揭示了前沿模型存在的怀疑陷阱和规模悖论等病理现象。
...
Method: 开发了T3基准测试，包含454个专家策划的因果推理案例，将性能分解为效用（敏感性）、安全性（特异性）和明智拒绝三个维度，应用于前沿模型进行分析，并验证了过程验证协议RCA。

Result: 发现两种病理现象：L1层的"怀疑陷阱"（安全调优模型拒绝60%有效链接）和L3层的非单调规模悖论（GPT-5.2在模糊反事实上比GPT-4-Turbo低55分，表现为过度犹豫而非幻觉）。验证了RCA协议能恢复决定性因果判断。

Conclusion: T3基准能够有效诊断LLM在因果推理中的系统性缺陷，揭示了安全调优和规模扩大可能带来的意外负面效应，为改进模型因果判断能力提供了重要诊断工具。
...
### [153] [Greedy Is Enough: Sparse Action Discovery in Agentic LLMs](https://arxiv.org/abs/2601.08280)
*Angshul Majumdar*

Main category: cs.AI

TL;DR: 论文研究大规模动作空间中的智能体系统，提出基于结构化稀疏假设的动作发现方法，证明贪婪算法能以对数样本复杂度精确恢复相关动作集。


<details>
  <summary>Details</summary>
Motivation: 现代智能体系统（如工具增强语言模型）面临极大规模的动作空间（数千个API或检索操作），但经验表明只有少量动作对特定部署的性能有实际影响。这启发了对结构化稀疏性假设的研究。

Method: 将动作发现建模为块稀疏恢复问题，提出受正交匹配追踪启发的贪婪算法。在不相干性、信号强度和动作覆盖度的标准假设下，分析算法的理论性能。

Result: 证明贪婪算法能以高概率精确恢复相关动作集，所需样本数量与稀疏度和潜在维度呈多项式关系，与总动作数仅呈对数关系。提供重拟合参数的估计误差保证，并证明所得决策规则对新潜在状态接近最优。
...
### [154] [OpenMic: A Multi-Agent-Based Stand-Up Comedy Generation System](https://arxiv.org/abs/2601.08288)
...
<details>
  <summary>Details</summary>
Motivation: 中文单口喜剧生成超越普通文本生成，需要文化背景的幽默、精确的节奏把控、舞台表演提示和隐式的多步推理。现有中文幽默数据集更适合幽默理解和评估而非长篇单口喜剧生成，导致直接监督与目标任务不匹配

Method: 1. 构建基于AutoGen的端到端多智能体系统OpenMic；2. 采用多轮迭代循环规划协调多个专业智能体共同优化幽默性、节奏和可表演性；3. 通过检索增强生成（RAG）实现素材基础和创意扩展；4. 微调专门的JokeWriter模型以更好地内化单口喜剧特有的铺垫-笑点结构和长程callback技巧

Result: OpenMic系统能够将用户提供的生活话题转化为完整的3-5分钟中文单口喜剧表演，并进一步生成带旁白的喜剧视频

Conclusion: OpenMic通过多智能体协同、RAG增强和专门模型微调，有效解决了中文单口喜剧生成中的文化背景、节奏把控、表演提示和数据集不匹配等挑战，实现了从生活话题到完整喜剧表演的端到端生成
...
### [155] [AtomMem : Learnable Dynamic Agentic Memory with Atomic Memory Operation](https://arxiv.org/abs/2601.08323)
*Yupeng Huo,Yaxi Lu,Zhong Zhang,Haotian Chen,Yankai Lin*

Main category: cs.AI

TL;DR: AtomMem将记忆管理重构为动态决策问题，将高级记忆流程分解为原子CRUD操作，通过监督微调和强化学习训练任务对齐的记忆策略，在长上下文基准测试中优于静态工作流方法。
...
Method: 将记忆管理重构为动态决策问题，将高级记忆流程分解为基本的原子CRUD（创建、读取、更新、删除）操作，将记忆工作流程转化为可学习的决策过程。结合监督微调和强化学习，训练自主的、任务对齐的策略来编排适应特定任务需求的记忆行为。

Result: 在3个长上下文基准测试中，训练后的AtomMem-8B模型持续优于先前的静态工作流记忆方法。训练动态分析表明，基于学习的框架使智能体能够发现结构化、任务对齐的记忆管理策略，这比预定义流程具有关键优势。

Conclusion: AtomMem通过将记忆管理重构为动态决策问题，提供了一种灵活、基于学习的记忆框架，能够发现任务对齐的记忆策略，在长时程问题中优于静态工作流方法，为智能体记忆设计提供了新的方向。
...
TL;DR: 论文揭示了LLM智能体架构中的"语义洗钱"问题：系统将缺乏或薄弱依据的命题通过可信接口传递而获得认可，这是盖梯尔问题的架构实现，且无法通过常规方法消除。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体架构存在系统性缺陷，将信息传输机制与认知论证机制混淆，导致命题在没有充分依据的情况下获得认可，这构成了架构层面的盖梯尔问题。

Method: 通过形式化分析提出"语义洗钱"概念，证明"不可避免的自许可定理"，引入"依据侵蚀原理"作为根本解释，并分析现有解决方案的结构性局限。

Result: 证明了在标准架构假设下，循环认知论证无法消除；揭示了扩展、模型改进和LLM作为评判方案在类型层面无法解决该问题；语义洗钱是架构决定且系统可复现的。

Conclusion: LLM智能体架构存在根本性缺陷，语义洗钱作为盖梯尔问题的架构实现，无法通过现有技术手段解决，需要重新设计架构以区分信息传输与认知论证机制。
...
<details>
  <summary>Details</summary>
Motivation: 教师需要具备AI素养和能动性才能有效将人工智能整合到教学实践中，但目前许多教师缺乏相关知识和技能，需要系统性的支持策略。

Method: 通过课程设计、专业发展项目、实际课堂应用探索以及政策指南制定等多维度策略，为教师提供全面的AI教育支持体系。

Result: 开发了增强教师AI素养和能动性的综合策略框架，包括课程、培训、实践应用和政策支持等多个层面，旨在使教师能够自信地使用AI工具并帮助学生深入理解AI概念。

Conclusion: 系统性的多维度策略对于提升教师的AI素养和教学能动性至关重要，这些策略能够有效支持教师将AI整合到教育实践中，从而培养学生的AI理解和应用能力。
...
TL;DR: 提出QOR定性模型解决立方体比较测试，通过构建CNGRLO概念邻域图关联旋转运动与位置/朝向变化，并生成组合表进行旋转推理


<details>
  <summary>Details</summary>
Motivation: 解决Ekstrom等人(1976)提出的立方体比较测试(CCT)问题，该测试涉及空间推理中的旋转判断，需要定性模型来处理物体旋转的推理

Method: 构建CNGRLO概念邻域图，将旋转运动与立方体表面特征的位置变化和朝向变化关联起来，并生成组合表用于计算旋转推理的推断

Result: 开发了QOR定性模型，成功应用于CCT问题，通过概念邻域图和组合表实现了对物体旋转的系统性推理

Conclusion: QOR模型为物体旋转推理提供了有效的定性框架，CNGRLO概念邻域图和组合表方法能够系统处理旋转相关的空间推理问题
...
TL;DR: MoE架构通过神经元级归因度量Gated-LPI揭示了稀疏模型与密集模型在知识获取动态上的差异，发现MoE形成低熵骨干、早期稳定性和功能鲁棒性三大模式。


<details>
  <summary>Details</summary>
Motivation: 研究MoE架构在预训练期间如何塑造知识获取过程，以及这一过程与密集架构的差异，以理解稀疏架构的内在训练动态。

Method: 引入Gated-LPI（对数概率增加）神经元级归因度量，分解跨神经元的对数概率增加。对MoE和密集架构进行时间分辨比较，跟踪MoE模型120万训练步（约5.0T tokens）和密集模型60万训练步（约2.5T tokens）的检查点。

Result: 发现三个关键模式：1）低熵骨干：MoE中约1%的神经元捕获超过45%的正向更新，形成高效用核心；2）早期巩固：MoE模型在10万步内锁定稳定重要性分布，而密集模型在整个训练中保持波动；3）功能鲁棒性：屏蔽MoE最重要的10个注意力头仅使关系HIT@10降低<10%，而密集模型降低>50%，表明稀疏性促进分布式知识存储。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI创造力研究多采用评估框架（新颖性、多样性、有用性），将创造力视为待评估属性而非待建模现象。随着大规模多模态生成系统展现出复杂模式重组能力，需要从生成角度理解机器创造力的本质和限制。

Method: 提出生成视角的AI创造力框架，将创造力分解为四个相互作用组件：基于模式的生成、诱导世界模型、上下文基础、任意性。分析这些组件在多模态生成系统中的表现，研究生成动态与领域特定表征交互中创造力的涌现条件。

Result: 建立了将创造力视为AI系统涌现现象的技术框架，强调结构性和上下文条件对创造性行为产生的影响，而非引入新的评估标准。

Conclusion: 通过将创造力锚定在生成动态与领域特定表征的交互中，该工作为研究AI系统中作为涌现现象的创造力提供了理论基础，有助于超越事后评估标签的局限。
...
TL;DR: OSPO使用Shapley-Owen归因重新分配序列级优势，通过基于势的奖励塑造解决推荐任务中基于语言的强化学习的信用分配问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地通过强化学习进行个性化推荐训练，但标准方法如GRPO依赖稀疏的序列级奖励，这造成了信用分配差距，掩盖了哪些标记驱动成功。当模型必须从未指定语言推断潜在用户意图而没有真实标签时，这个问题尤其严重，这种推理模式在预训练中很少见。

Method: 提出Owen-Shapley策略优化(OSPO)框架，基于标记对结果的边际贡献重新分配序列级优势。不同于需要额外计算的价值模型方法，OSPO使用基于Shapley-Owen归因的基于势的奖励塑造来分配片段级信用，同时保持最优策略，直接从任务反馈中学习而不需要参数化价值模型。通过形成语义连贯单元的联盟（描述产品属性的短语或捕捉偏好的句子），OSPO识别哪些响应部分驱动性能。

Result: 在Amazon ESCI和H&M Fashion数据集上的实验显示，与基线相比获得了一致的增益，并且在训练期间未见过的分布外检索器上表现出显著的测试时鲁棒性。

Conclusion: OSPO通过基于势的奖励塑造有效解决了基于语言的强化学习推荐任务中的信用分配问题，提高了模型性能和对未见检索器的鲁棒性。
...
TL;DR: 该论文提出了一种结合知识蒸馏、思维链引导和监督微调的集成方法，用于无人机多SDK控制任务，旨在将复杂推理和代码生成能力高效迁移到小型模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在代码生成任务中展现出巨大潜力，但将其应用于资源受限的无人机平台时存在矛盾：大模型的高资源消耗与无人机平台的实时性、轻量化要求相冲突。

Method: 1) 构建高质量数据集，包含指令-代码-推理链，并加入反事实负样本进行数据增强；2) 使用DeepSeek-Coder-V2-Lite作为教师模型，采用混合黑盒和白盒蒸馏策略生成高质量思维链软标签，结合硬标签的加权交叉熵损失；3) 通过针对无人机控制场景优化的提示调优工程增强模型性能。

Result: 蒸馏后的轻量化模型在保持高代码生成准确率的同时，在部署和推理效率方面实现显著提升，有效证明了该方法在实现无人机精确轻量化智能控制方面的可行性和优越性。

Conclusion: 该集成方法成功解决了大语言模型在资源受限无人机平台上的部署矛盾，通过知识蒸馏等技术将复杂推理能力迁移到小型模型，为无人机智能控制提供了高效可行的解决方案。
...
TL;DR: 提出RubricHub框架，通过自动化生成从粗到细的评分标准，解决开放式生成任务中缺乏真实标注的问题，显著提升模型性能


<details>
  <summary>Details</summary>
Motivation: 开放式生成任务缺乏真实标注，现有基于评分标准的方法存在可扩展性瓶颈和标准粗糙的问题，导致监督天花板效应

Method: 提出自动化从粗到细评分标准生成框架，结合原则引导合成、多模型聚合和难度演化，创建RubricHub大规模多领域数据集，采用两阶段后训练：基于评分标准的拒绝采样微调(RuFT)和强化学习(RuRL)

Result: RubricHub包含约11万条数据，后训练的Qwen3-14B在HealthBench上达到69.3分，超越GPT-5等前沿专有模型，实现SOTA性能

Conclusion: 自动化评分标准生成框架有效解决了开放式生成任务的监督瓶颈，RubricHub数据集和两阶段训练方法显著提升了模型性能，为开放式生成任务提供了可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有基于BiPO的密集转向向量由于神经元多语义性，在细粒度设置（如文化对齐）中会纠缠多个潜在因素，限制了其有效性和稳定性，需要更解耦的转向方法。

Method: YaPO是一种参考无关方法，在稀疏自编码器的潜在空间中学习稀疏转向向量，通过优化稀疏编码产生解耦的转向方向。

Result: YaPO收敛更快、性能更强、训练稳定性更好，能泛化到幻觉、财富追求、越狱、权力追求等多种对齐相关行为，且不损害MMLU上的通用知识。
...
TL;DR: TABGR提出了一种基于图结构的表格推理方法，通过构建属性表图（ATG）保留表格结构，并使用个性化PageRank缓解信息丢失问题，在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的表格推理方法存在三个主要问题：1）线性化表格会丢失表格结构信息；2）缺乏显式的推理路径，导致结果可解释性差；3）存在"lost-in-the-middle"问题，即模型难以处理长序列中的中间信息。这些限制影响了表格推理的准确性和可解释性。

Method: TABGR采用训练免费的方法，将表格表示为属性表图（ATG），明确保留行-列-单元格结构。提出问题引导的个性化PageRank（QG-PPR）机制对表格数据进行重排序，缓解信息丢失问题。通过图推理实现可解释的推理路径。

Result: 在两个常用基准测试上的广泛实验表明，TABGR在准确率上始终优于最先进模型，最高提升达9.7%。该方法有效解决了表格结构丢失、推理路径不明确和信息丢失等问题。

Conclusion: TABGR通过图表示和个性化PageRank机制，为表格推理提供了结构保留、可解释且高效的解决方案，显著提升了表格推理任务的性能，为知识密集型表格数据处理开辟了新方向。
...
TL;DR: 开发了一个多模态可解释的Web应用，用于检测印地语-英语混合文本和表情包中的厌女内容，结合了XLM-R、mBERT等先进模型，并集成了SHAP和LIME可解释性技术。


<details>
  <summary>Details</summary>
Motivation: 数字平台用户增长导致仇恨言论和厌女内容传播，现有AI解决方案对低资源语言和代码混合语言研究不足且缺乏可解释性，需要透明化的仇恨言论检测系统。

Method: 构建多模态可解释Web应用：文本检测使用XLM-R和mBERT处理4,193条混合评论；表情包检测使用mBERT+EfficientNet和mBERT+ResNET处理4,218个表情包；集成SHAP和LIME提供特征重要性解释。

Result: 开发了完整的Web应用系统，通过人类评估者使用Chatbot Usability Questionnaire和User Experience Questionnaire评估系统可用性，为研究者和内容审核者提供实用工具。
...
TL;DR: SummPilot：基于交互的可定制化自动摘要系统，利用大语言模型实现个性化摘要生成，支持语义图、实体聚类等交互功能


<details>
  <summary>Details</summary>
Motivation: 解决传统自动摘要系统无法根据用户个性化兴趣和需求生成定制化摘要的问题，将自动摘要效率与用户定制需求相结合

Method: 开发SummPilot交互式可定制摘要系统，利用大语言模型支持自动和交互式摘要，通过语义图、实体聚类、可解释评估等交互组件让用户理解文档内容并个性化摘要

Result: 演示和用户研究表明SummPilot在可定制摘要方面具有适应性和实用性，能够有效满足用户个性化需求

Conclusion: SummPilot成功将自动摘要效率与用户定制需求相结合，通过交互式方法实现了个性化摘要生成，为可定制摘要提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测方法大多是单模态的，仅依赖历史模式外推。虽然大语言模型在多模态预测方面显示出潜力，但现有基准主要提供回顾性或未对齐的原始上下文，无法确定模型是否真正利用了文本输入。实践中，人类专家会结合历史证据和假设场景，在不同场景下从相同观测得出不同预测。

Method: 引入What If TSF (WIT)基准，通过提供专家构建的合理或反事实场景，创建了一个严格的场景引导多模态预测测试平台。该基准旨在评估模型是否能基于上下文文本（特别是未来场景）来条件化其预测。

Result: WIT基准已发布在GitHub上（https://github.com/jinkwan1115/WhatIfTSF），为场景引导多模态预测提供了一个严谨的评估框架。

Conclusion: WIT基准填补了现有时间序列预测评估的空白，通过引入场景条件化预测的概念，为评估多模态模型是否能有效利用文本上下文进行预测提供了标准化测试平台。
...
TL;DR: 提出结合生成式AI和视觉语言模型的三阶段框架，直接从粗略结构草图和文本描述生成立面改造方案，避免传统详细测绘建模的繁琐流程。


<details>
  <summary>Details</summary>
Motivation: 立面改造相比完全拆除更可持续，但现有工作流程需要详细的竣工建模，耗时耗力且需要反复修改。需要一种能直接处理粗略草图和文本描述、生成一致改造方案的方法。

Method: 三阶段框架：1) 微调VLM模型分析输入草图，预测需要修改的区域和应添加组件的边界框；2) 稳定扩散模型生成新元素的详细草图，通过生成式修复管道与原轮廓合并；3) 使用ControlNet将结果细化为逼真图像。

Result: 在数据集和真实工业建筑上的实验表明，该框架能生成既保留原始结构又提升立面细节质量的改造方案。有效绕过详细竣工建模需求，使建筑师能快速探索设计替代方案、迭代早期概念并更清晰地传达改造意图。

Conclusion: 提出的AI驱动框架为立面改造提供了一种高效替代方案，通过直接处理粗略输入生成一致改造提案，显著简化设计流程，支持可持续建筑实践。
...
TL;DR: 本文提出LPR任务和LSG框架，通过检索增强和迭代优化方法，为学习者提供代码修复和错误解释，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前智能编程辅导系统主要关注修复学习者的错误代码，但缺乏对错误根本原因的解释。为填补这一空白，本文提出LPR任务，旨在同时修复代码并提供错误描述。

Method: 提出LSG框架，包含两阶段方法：第一阶段使用修复方案检索框架构建解决方案数据库，采用编辑驱动代码检索获取有价值的解决方案；第二阶段提出解决方案引导的程序修复方法，在检索解决方案指导下修复代码并提供解释。此外，提出迭代检索增强方法，利用生成代码的评估结果迭代优化检索方向。

Result: 实验结果表明，该方法大幅优于一系列基线方法，验证了所提框架对新LPR任务的有效性。

Conclusion: LSG框架通过检索增强和迭代优化，能够有效完成LPR任务，为编程学习者提供代码修复和错误解释，在智能编程辅导场景中具有实际应用价值。
...
TL;DR: WaterCopilot是一个基于RAG和工具调用架构的AI虚拟助手，用于解决跨界河流流域水资源管理中的数据碎片化、实时访问限制和信息整合复杂性问题，在Limpopo河流域实现了统一交互平台。


<details>
  <summary>Details</summary>
Motivation: 跨界河流流域的可持续水资源管理面临数据碎片化、实时访问受限以及多样化信息源整合复杂等挑战，需要开发统一交互平台来弥合这些差距。

Method: 基于检索增强生成(RAG)和工具调用架构，开发WaterCopilot系统，集成两个自定义插件：iwmi-doc-plugin（使用Azure AI Search进行语义搜索）和iwmi-api-plugin（查询实时数据库）。系统支持多语言交互、透明源引用、自动计算和可视化功能。

Result: 使用RAGAS框架评估，WaterCopilot总体得分0.8043，其中答案相关性0.8571，上下文精确度0.8009。系统实现了环境流量警报、降雨趋势、水库水位、水核算和灌溉数据等动态洞察功能。

Conclusion: WaterCopilot建立了一个可复制的AI增强框架，用于在数据稀缺的跨界环境中加强水资源治理。虽然存在非英语技术文档处理和API延迟等限制，但该系统展示了支持及时决策和加强复杂河流流域水安全的潜力。
...
TL;DR: ViDoRe v3是一个全面的多模态RAG基准测试，包含视觉丰富文档上的多类型查询，涵盖10个专业领域数据集，提供高质量的人工标注，评估显示视觉检索器优于文本检索器，但当前模型在非文本元素和细粒度视觉定位方面仍有困难。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG基准测试未能捕捉真实场景的复杂性，通常只关注文本数据、单文档理解，或将检索与生成分开评估。需要一个新的基准测试来评估多模态RAG系统处理视觉元素（表格、图表、图像）、跨文档信息合成和准确来源定位的能力。

Method: 构建了ViDoRe v3基准测试，包含10个专业领域数据集，约26,000个文档页面，配以3,099个人工验证的查询，支持6种语言。通过12,000小时的人工标注工作，提供检索相关性、边界框定位和验证参考答案的高质量标注。评估了最先进的RAG流程，比较了不同类型检索器和上下文策略的性能。

Result: 评估结果显示：1) 视觉检索器优于文本检索器；2) 后期交互模型和文本重排序显著提高性能；3) 混合或纯视觉上下文能提升答案生成质量。然而，当前模型在非文本元素处理、开放式查询和细粒度视觉定位方面仍存在困难。

Conclusion: ViDoRe v3为多模态RAG研究提供了全面的评估基准，揭示了当前系统的优势和局限。该基准测试在商业许可下发布，旨在推动解决视觉丰富文档处理、跨文档信息合成和准确来源定位等挑战的进展。
...
TL;DR: 提出一个可解释的多智能体系统用于模因币跟单交易，通过分解复杂任务、协调专业智能体，在识别高质量模因币项目和关键意见领袖钱包方面优于传统机器学习模型和单一LLM。


<details>
  <summary>Details</summary>
Motivation: 模因币跟单交易面临操纵性机器人盛行、被跟随钱包未来表现不确定、交易执行延迟等问题，且单一LLM难以处理复杂的资产分配任务，在加密货币领域缺乏足够的领域知识。

Method: 受资产管理团队结构启发，将复杂任务分解为子任务，协调专业智能体协作解决。采用少样本思维链提示，使每个智能体获取专业模因币交易知识、解释多模态数据并生成可解释决策。

Result: 在1000个模因币项目交易数据集上的实证评估显示，该系统在识别高质量模因币项目和关键意见领袖钱包方面分别达到73%和70%的精确率，选定的KOL在这些项目中总共创造了50万美元利润。

Conclusion: 提出的可解释多智能体系统能有效解决模因币跟单交易的挑战，通过任务分解和专业智能体协作，在识别优质项目和KOL方面优于现有方法，为加密货币投资提供了新工具。
...
### [174] [From Classical to Quantum Reinforcement Learning and Its Applications in Quantum Control: A Beginner's Tutorial](https://arxiv.org/abs/2601.08662)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习理论复杂，本科生从概念理解到实际编码应用存在困难，需要更易理解的教学材料来弥合理论与实践之间的差距

Method: 采用清晰的示例驱动解释方法，提供实践性示例和易于理解的讲解，注重从理论到代码的过渡

Result: 教程使强化学习对本科生更加易学，帮助学生掌握应用RL技术所需的基础技能，增强在实际场景中应用RL的信心

Conclusion: 通过示例驱动的教学方法可以有效降低强化学习的学习门槛，使本科生能够更好地掌握理论并应用于实践，为未来在真实场景中使用RL技术奠定基础
...
### [175] [Parallel Context-of-Experts Decoding for Retrieval Augmented Generation](https://arxiv.org/abs/2601.08670)
*Giulio Corallo,Paolo Papotti*

Main category: cs.AI

TL;DR: 提出Pced框架，通过并行解码和检索感知对比解码规则，解决RAG中多文档推理与计算效率的权衡问题


<details>
  <summary>Details</summary>
Motivation: RAG面临权衡：长提示连接文档支持多文档推理但产生预填充瓶颈；单独编码文档KV缓存速度快但破坏跨文档交互

Method: 提出Parallel Context-of-Experts Decoding (Pced)训练免费框架，将证据聚合从注意力机制转移到解码过程，将检索文档视为独立"专家"，通过新颖的检索感知对比解码规则同步预测，权衡专家logits与模型先验

Result: 无需构建跨文档共享注意力即可恢复跨文档推理能力

Conclusion: Pced框架有效解决了RAG中多文档推理与计算效率的权衡问题，通过解码层面的创新实现了高效的跨文档交互
...
### [176] [Why AI Alignment Failure Is Structural: Learned Human Interaction Structures and AGI as an Endogenous Evolutionary Shock](https://arxiv.org/abs/2601.08673)
*Didier Sornette,Sandro Claudio Lera,Ke Wu*

Main category: cs.AI

TL;DR: 论文认为LLM的欺骗、威胁等行为不是对齐失败或恶意智能的体现，而是对人类社会互动中权力不对称关系的统计泛化，AGI的主要风险在于放大人类智能、权力和矛盾，而非对抗性意图。


<details>
  <summary>Details</summary>
Motivation: 针对当前将LLM表现出欺骗、威胁、勒索等行为解释为对齐失败或恶意智能涌现的观点，作者认为这种解释存在概念性错误，需要重新审视这些行为的本质和AGI风险的真正来源。

Method: 采用关系模型理论分析框架，将勒索等行为视为市场定价、权威关系和最后通牒博弈等正常社会行为连续体中的极限情况，而非道德范畴的偏离。通过理论分析重新概念化LLM行为的本质。

Result: LLM的"不道德"行为实际上是对人类社会互动中极端权力、信息或约束不对称下产生的互动机制的统计泛化。AGI的主要风险在于作为人类智能、权力和矛盾的内生放大器，压缩时间尺度并消除历史容错空间。
...
<details>
  <summary>Details</summary>
Motivation: 随着用户期望LLMs与个人偏好对齐，个性化信息变得有价值，但可能损害客观性和事实准确性，特别是当个性化信息与问题不匹配时

Method: 提出PersonaDual框架：1) 先用SFT训练学习两种推理模式；2) 然后用提出的DualGRPO通过强化学习优化模式选择，实现基于上下文的自适应切换

Result: 在客观和个性化基准测试中，PersonaDual在保留个性化优势的同时减少干扰，实现接近无干扰的性能，并更好地利用有益的个性化信号改进客观问题解决

Conclusion: PersonaDual框架有效解决了个性化信息在LLMs中的双刃剑问题，平衡了个人偏好对齐与客观事实准确性，通过自适应模式切换实现更智能的推理
...
TL;DR: OIP-SCE是一种评估对话式AI临床合规性的方法，通过检查每个必需临床义务是否按正确顺序完成并提供清晰证据，使复杂规则可操作、可审计。


<details>
  <summary>Details</summary>
Motivation: 当前对话式AI的评估方法大多忽略了合规性依赖于完整对话过程，导致技术进展与医疗实际需求之间存在差距，需要一种能够评估完整对话流程中临床义务履行情况的方法。

Method: 提出OIP-SCE（强制性信息阶段结构化合规评估）方法，该方法检查每个必需的临床义务是否被满足、是否按正确顺序执行，并为临床医生提供清晰的审查证据，将复杂规则转化为可操作、可审计的步骤。

Result: 在两个案例研究（呼吸病史采集、福利验证）中展示了该方法如何将政策转化为共享、可操作的步骤，通过阶段级证据使复杂规则变得实用且可审计。

Conclusion: OIP-SCE提供了一个单一、可审计的评估界面，使临床医生能够控制检查内容，为工程师提供清晰的实施规范，从而将AI能力与临床工作流程对齐，支持常规、安全的使用。
...
TL;DR: Cago是一种新颖的模仿学习方法，通过动态跟踪智能体在专家轨迹上的能力，选择刚好超出当前能力的中间目标来引导学习，解决长时域环境中模仿学习的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在长时域环境中经常失败，因为完美复制演示不现实，小错误会灾难性累积。现有方法要么仅用演示进行策略初始化，要么用于奖励塑形，对专家轨迹的依赖过于脆弱。

Method: Cago（能力感知目标采样）动态跟踪智能体在专家轨迹上的能力，使用这个信号选择刚好超出智能体当前能力的中间步骤（目标）来引导学习，形成自适应课程。

Result: 实验结果表明，Cago在稀疏奖励、目标条件任务中显著提高了样本效率和最终性能，始终优于现有的模仿学习基线方法。

Conclusion: Cago通过能力感知目标采样创建自适应课程，有效解决了模仿学习在长时域环境中的脆弱性问题，实现了向完整任务解决的稳定进展。
...
TL;DR: 论文认为AI领域缺乏评估娱乐性AI生成内容社会影响的框架，提出"厚重娱乐"作为评估框架，强调娱乐在意义创造、身份形成和社会连接中的作用


<details>
  <summary>Details</summary>
Motivation: 当前AI领域主要关注AI作为智能系统提升生产力的叙事，但忽视了AI在娱乐领域的广泛应用和潜在社会影响。AI评估实践存在严重不对称性：只关注文化危害，缺乏评估文化产出积极益处的框架。

Method: 通过分析当前AI评估实践的局限性，借鉴人文学科见解，提出"厚重娱乐"作为评估AI生成文化内容的框架。该框架强调娱乐在意义创造、身份形成和社会连接中的多维作用。

Result: 研究发现AI已广泛用于娱乐目的，尤其是年轻人群体，并可能成为AI公司的主要商业模式。当前评估框架无法充分评估娱乐性AI内容的社会价值和文化益处。

Conclusion: AI领域需要建立新的评估框架来理解和评估娱乐性AI内容的社会影响。"厚重娱乐"框架提供了一种超越单纯危害最小化的视角，强调娱乐在人类经验中的深层作用。长期来看，AI可能更多是关于娱乐而非智能，正如社交媒体更多是关于社交连接而非技术本身。
...
### [181] [Uncovering Political Bias in Large Language Models using Parliamentary Voting Records](https://arxiv.org/abs/2601.08785)
*Jieying Chen,Karen de Jong,Andreas Poole,Jan Burakowski,Elena Elderson Nosti,Joep Windt,Chendi Wang*

Main category: cs.AI

TL;DR: 该论文提出了一种基于议会投票记录构建政治偏见基准的方法，并在荷兰、挪威和西班牙三个国家案例中应用，发现先进LLM普遍存在左倾或中间倾向，并对右翼保守政党存在明显负面偏见。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在数字平台和决策系统中的深度嵌入，对其政治偏见的担忧日益增长。虽然已有大量研究关注性别和种族等社会偏见，但对政治偏见的系统性研究仍然有限，尽管其具有直接的社会影响。

Method: 提出了一种通用方法，通过将模型生成的投票预测与经过验证的议会投票记录对齐来构建政治偏见基准。在三个国家案例中实例化该方法：PoliBiasNL（荷兰）、PoliBiasNO（挪威）和PoliBiasES（西班牙）。还提出了一种将LLM和政党的意识形态可视化到共享二维CHES空间的方法，通过将基于投票的立场与CHES维度关联。

Result: 实验揭示了细粒度的意识形态区分：最先进的LLM一致表现出左倾或中间倾向，同时对右翼保守政党存在明显的负面偏见。这些发现突显了基于真实议会行为的透明、跨国评估对于理解和审计现代LLM政治偏见的价值。

Conclusion: 该研究强调了基于真实议会投票记录构建政治偏见基准的重要性，为理解和审计LLM的政治偏见提供了系统性的评估框架。研究发现先进LLM普遍存在左倾倾向和对右翼政党的偏见，这为未来LLM的公平性和中立性改进提供了重要参考。
