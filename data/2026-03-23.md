...
Motivation: 现有方法在处理自我中心视频时面临挑战：视角快速变化、频繁的手物交互、以及依赖潜在人类意图的目标导向过程。现有方法要么局限于手部中心的指令合成且场景演化有限，要么进行静态视角转换而不建模动作动态，要么依赖密集监督（如相机轨迹、长视频前缀、多相机同步捕获等）。

Method: 提出EgoForge系统，仅需最小静态输入：单张自我中心图像、高级指令和可选的外部视角。引入VideoDiffusionNFT方法，这是一种轨迹级奖励引导的细化技术，在扩散采样过程中优化目标完成度、时间因果性、场景一致性和感知保真度。

Result: 大量实验表明，EgoForge在语义对齐、几何稳定性和运动保真度方面相比强基线方法取得一致提升，并在真实世界智能眼镜实验中表现出稳健性能。

Conclusion: EgoForge通过最小输入实现了连贯的第一人称视频生成，解决了自我中心视频模拟中的关键挑战，为自我中心世界建模提供了有效解决方案。
...
TL;DR: SemanticFL：利用预训练扩散模型的多层语义表示，在非IID联邦学习中创建共享潜在空间对齐异构客户端，通过跨模态对比学习稳定收敛，显著提升感知任务性能
...
Method: 提出SemanticFL框架：1）利用预训练Stable Diffusion模型（包括VAE编码潜变量和U-Net层次特征）的多层语义表示创建共享潜在空间；2）采用高效的客户端-服务器架构将繁重计算卸载到服务器；3）通过跨模态对比学习实现统一的一致性机制稳定收敛

Result: 在CIFAR-10、CIFAR-100和TinyImageNet等基准数据集上的多种异构场景实验表明，SemanticFL超越现有联邦学习方法，相比FedAvg最高获得5.49%的准确率提升

Conclusion: SemanticFL通过利用预训练扩散模型的丰富语义表示，有效解决了非IID联邦学习中的语义差异问题，为异构多模态数据的感知任务学习提供了鲁棒表示，验证了其在隐私保护指导下的有效性
...
TL;DR: 提出基于Qwen3-VL视觉编码器的统一多任务框架，用于超声图像分析，通过特征金字塔融合和任务特定预测头，在FMC-UIA挑战中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 超声图像因扫描设备、操作者和解剖目标不同而存在显著差异，导致在单一场景训练的模型难以泛化到新医院和临床条件。FMC-UIA挑战要求单一模型处理分割、检测、分类和标志点回归等多种任务，需要解决跨器官和数据集的多任务学习问题。

Method: 基于Qwen3-VL家族的Transformer视觉编码器构建统一多任务框架。将中间token特征投影为空间特征图，使用轻量级多尺度特征金字塔进行融合，实现像素级预测和全局推理的共享表示。每个任务由小型任务特定预测头处理，训练采用任务感知采样和选择性损失平衡来管理异构监督并减少任务不平衡。

Result: 验证集性能从67%提升至85%，在官方测试集上所有任务平均得分达到81.84%。代码已公开在GitHub仓库。

Conclusion: 提出的统一多任务框架简单易优化，能够适应广泛的超声分析任务，有效解决了超声图像分析中的泛化挑战，在FMC-UIA挑战中取得了显著性能提升。
...
TL;DR: 提出一种改进的Levenberg-Marquardt优化器，用于FireANTs图像配准框架，通过单标量阻尼参数减少内存占用24.6%，保持性能并实现跨模态泛化。


<details>
  <summary>Details</summary>
Motivation: FireANTs使用Adam优化器进行图像配准，但Adam需要存储动量状态变量，消耗大量内存，限制了在大尺寸图像上的应用。需要开发内存效率更高的优化器。

Method: 提出改进的Levenberg-Marquardt优化器，仅需单标量阻尼参数作为优化器状态，采用信任区域方法自适应调整。同时使用Metropolis-Hastings风格拒绝步骤防止损失函数恶化。

Result: 优化器将内存占用减少高达24.6%（对大体积图像），在四个数据集上保持性能。单一超参数配置在脑MRI上调优后，无需修改即可应用于肺CT和跨模态腹部配准，在三个基准测试中匹配或优于Adam。

Conclusion: 改进的LM优化器显著减少了内存需求，同时保持了配准性能，实现了跨模态泛化能力，为大规模图像配准提供了更高效的优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 胸部X射线中临床相关发现通常空间受限，需要细粒度表示学习。现有对比模型缺乏区域级监督，大型视觉语言模型在外部验证中捕捉细粒度表示能力有限，导致检索和短语定位任务性能欠佳。
...
Conclusion: LoFi通过location-aware captioning损失实现区域级监督，有效促进细粒度表示学习，结合基于检索的上下文学习框架，显著提升了胸部X射线的检索和短语定位性能。
...
TL;DR: 论文提出ProactiveBench基准，用于评估多模态大语言模型（MLLMs）在需要用户干预时的主动求助能力，发现现有模型普遍缺乏主动性，且这种能力与模型规模无关，但可通过强化学习微调学习获得。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索多模态大语言模型是否能像人类一样表现出"主动"行为，在遇到困难时（如识别被遮挡物体）主动请求用户进行简单干预，以促进更有效的人机协作。

Method: 方法包括：1）构建ProactiveBench基准，从7个现有数据集重新构建，涵盖识别遮挡物体、提升图像质量、解释粗略草图等任务；2）评估22个MLLMs的主动性表现；3）分析对话历史和上下文学习的影响；4）探索基于强化学习的简单微调策略。

Result: 结果显示：1）MLLMs普遍缺乏主动性；2）主动性与模型容量无关；3）给予"提示"仅带来边际改善；4）对话历史和上下文学习引入负面偏见，阻碍性能；5）基于强化学习的微调表明主动性可以学习，并能泛化到未见场景。

Conclusion: 结论是多模态大语言模型目前缺乏主动求助能力，但该能力可通过训练获得。ProactiveBench的发布是构建主动多模态模型的第一步，强调了在MLLMs中培养主动性行为的重要性。
...
TL;DR: NA-VQA是一个用于评估长视频深度时序和叙事推理能力的基准，包含88部完整电影和4.4K个开放性问题-答案对，通过远近证据跨度测试模型的长距离依赖处理能力。论文还提出了Video-NaRA框架来改善叙事推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型的长视频推理基准大多依赖局部线索，未能有效评估叙事推理能力——即追踪意图、连接远距离事件和重建整个电影因果链的能力。需要专门基准来测试模型整合分散叙事信息而非浅层模式匹配的能力。

Method: 提出了NA-VQA基准，包含88部完整电影和4.4K个开放性问题-答案对，每个问题都基于标记为短、中、远距离的证据跨度，以评估长距离依赖。同时提出了Video-NaRA框架，该框架构建事件级链并将其存储在结构化记忆中，在推理过程中进行检索。

Result: 实验表明，最先进的多模态大语言模型在需要远距离证据的问题上表现不佳，凸显了显式叙事建模的必要性。Video-NaRA将长距离推理性能提高了最多3%，证明了其在处理复杂叙事结构方面的有效性。

Conclusion: NA-VQA基准揭示了当前多模态大语言模型在长视频叙事推理方面的局限性，特别是处理长距离依赖的能力不足。Video-NaRA框架通过显式的事件链建模和结构化记忆存储，有效提升了长距离叙事推理性能，为未来研究提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像AI的可靠性严重依赖于对异质性和损坏图像的鲁棒性，这些图像在不同医院的多种设备上采集，具有高度挑战性。需要一种能够在保护数据隐私的同时，增强模型在真实世界条件下鲁棒性和泛化能力的联邦学习方法。

Method: 提出FedAgain框架，集成双重信任机制：结合基准可靠性和模型分歧度，动态加权客户端贡献，在聚合过程中减轻噪声或对抗性更新的影响。该框架支持跨多个机构的协作模型训练，同时保护数据隐私并促进在真实世界条件下的稳定收敛。

Result: 在五个数据集上的广泛实验（包括两个经典基准MNIST和CIFAR-10、两个私有多机构肾结石数据集和一个公共数据集MyStone）表明，FedAgain在非独立同分布数据和非诚实客户端场景下，始终优于标准联邦学习基线方法。
...
TL;DR: Gastric-X是一个用于胃癌分析的大规模多模态基准数据集，包含1.7K病例，涵盖CT扫描、内镜图像、生化指标、诊断笔记和肿瘤区域标注，用于评估视觉语言模型在临床工作流中的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学诊断中的应用受限，主要原因是缺乏全面结构化、反映真实临床工作流程的数据集。为了推进视觉语言模型在临床应用特别是胃癌分析中的发展，需要构建一个能够模拟实际临床条件的多模态基准。

Method: 构建Gastric-X数据集，包含1.7K个胃癌病例，每个病例包含：静息和动态CT扫描配对、内镜图像、结构化生化指标、专家撰写的诊断笔记、肿瘤区域边界框标注。基于该数据集设计了五个核心任务来评估视觉语言模型：视觉问答、报告生成、跨模态检索、疾病分类和病灶定位。

Result: 通过Gastric-X数据集，系统性地评估了近期视觉语言模型在五个核心临床任务上的能力，旨在探究模型是否能够有意义地关联生化信号与空间肿瘤特征及文本报告。

Conclusion: Gastric-X是朝着将机器智能与医生的认知和证据推理过程对齐的重要一步，旨在激发下一代医学视觉语言模型的发展，促进模型在真实临床工作流程中的应用。
...
TL;DR: ReXInTheWild是一个包含955个临床验证多选题的基准测试，涵盖7个临床主题的484张生物医学文献照片，用于评估视觉语言模型在真实医疗图像理解与医学推理结合任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 日常相机拍摄的照片已广泛应用于远程医疗等在线健康对话，但目前缺乏全面基准来评估视觉语言模型能否正确解读这些医疗内容。分析这些图像需要细粒度的自然图像理解和领域特定的医学推理相结合，这对通用和专业模型都构成挑战。

Method: 研究人员构建了ReXInTheWild基准，包含955个临床验证的多选题，涵盖7个临床主题，基于484张来自生物医学文献的真实照片。对领先的多模态大语言模型进行系统评估，并进行错误分析以识别常见错误类别。

Result: 主要模型表现差异显著：Gemini-3准确率78%，Claude Opus 4.5为72%，GPT-5为68%，而医学专业模型MedGemma仅37%。错误分析揭示了四个常见错误类别：从低层几何错误到高层推理失败，需要不同的缓解策略。

Conclusion: ReXInTheWild提供了一个具有挑战性的临床基础基准，位于自然图像理解和医学推理的交汇点。该数据集已在HuggingFace上公开，为评估和改进医疗视觉语言模型提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 手指拼写是英国手语(BSL)的重要组成部分，但现有数据集规模小或标注不准确，且识别面临快速拼写和字母省略等挑战

Method: 采用迭代标注框架构建FS23K数据集，并提出显式考虑双手交互和口型线索的手指拼写识别模型

Result: 通过精炼标注，该方法将字符错误率(CER)相比先前最优方法降低了一半

Conclusion: 该方法有效支持手语理解和可扩展的自动标注管道研究，具有重要应用潜力
...
<details>
  <summary>Details</summary>
Motivation: XR中的空中手势易导致疲劳和不精确，基于表面的交互虽能提高准确性和舒适度，但现有以自我为中心的视觉方法面临手部追踪困难和表面平面估计不可靠的挑战

Method: 提出SurfaceXR传感器融合方法，结合头显手部追踪（提供3D位置数据）和智能手表IMU（捕获高频运动数据），利用两种模态的互补性实现稳健的表面输入
...
TL;DR: dinov3.seg扩展dinov3.txt，通过专门架构、早期视觉表示精炼、晚期图像-文本相关特征精炼以及高分辨率局部-全局推理策略，显著提升开放词汇语义分割性能


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的开放词汇语义分割方法主要依赖图像-文本相似度图的后处理精炼，这限制了空间精度和复杂场景下的鲁棒性，需要更系统的方法来改进密集预测能力

Method: 1) 为dinov3.txt设计任务专用架构；2) 联合利用全局[CLS]标记和局部补丁级视觉特征的文本嵌入；3) 在图像-文本交互前进行早期视觉表示精炼，之后进行晚期图像-文本相关特征精炼；4) 基于滑动窗口聚合的高分辨率局部-全局推理策略

Result: 在五个广泛采用的开放词汇语义分割基准测试中进行了广泛实验，结果显示该方法有效且鲁棒，始终优于当前最先进方法

Conclusion: dinov3.seg通过系统架构设计、早期和晚期精炼策略以及高分辨率推理方法，显著提升了开放词汇语义分割的性能和鲁棒性，为复杂场景下的密集预测提供了更准确的解决方案
...
Method: 融合注意力、位置、情境和交互四种行为流，使用高速公路编码器、紧凑4令牌Transformer和全局自注意力池化；集成变分瓶颈（捕捉认知不确定性）和马氏距离检测器（识别分布偏移）进行不确定性量化

Result: 在PSI 1.0基准上获得0.9 F1、0.94 AUC-ROC和0.78 MCC，优于现有视觉语言模型；在PSI 2.0上建立首个基线（0.78 F1、0.79 AUC-ROC）；基于马氏距离的选择性预测在80%覆盖率下提升测试精度0.4个百分点

Conclusion: 该方法提供校准概率和可操作风险评分，保持高效性，模态无关，易于集成到视觉语言管道，适用于资源受限平台的风险感知意图预测
...
TL;DR: MoCA3D：无需相机内参的单目类别无关3D检测模型，通过热图和深度图预测投影3D边界框角点和深度，在图像平面几何保真度上表现优异
...
Method: 提出MoCA3D模型，将像素空间定位和深度分配建模为密集预测任务：使用角点热图进行定位，深度图进行深度估计；无需推理时的相机内参；引入像素对齐几何（PAG）评估指标

Result: MoCA3D在图像平面角点PAG指标上提升22.8%，3D IoU性能相当，参数量减少最多57倍；成功应用于先前在未知内参下不切实际的下游任务

Conclusion: MoCA3D通过直接预测投影3D框角点和深度，解决了未知相机内参时的图像平面几何预测问题，为下游应用提供了实用工具，超越了标准基线模型
...
TL;DR: StreetForward是一个免姿态估计、免跟踪器的前馈动态街道重建框架，利用时间掩码注意力模块捕获动态运动信息，使用3D高斯泼溅统一表示静态内容和动态实例，在Waymo数据集上实现优越的新视角合成和深度估计性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用需要快速场景重建以高效利用大规模驾驶数据集进行闭环仿真等下游任务，避免耗时的逐场景优化。现有方法通常依赖姿态估计和跟踪器，限制了重建效率和泛化能力。

Method: 基于VGGT的交替注意力机制，提出时间掩码注意力模块从图像序列捕获动态运动信息，生成运动感知潜在表示。使用3D高斯泼溅统一表示静态内容和动态实例，通过时空一致的跨帧渲染联合优化，能够推断逐像素速度并在新姿态和时间生成高保真新视角。

Result: 在Waymo Open Dataset上训练和评估，在新视角合成和深度估计方面优于现有方法。在CARLA和其他数据集上的零样本推理验证了方法的泛化能力。

Conclusion: StreetForward提供了一个有效的免姿态、免跟踪器的前馈动态街道重建框架，能够高效处理大规模驾驶数据，支持自动驾驶的闭环仿真和其他下游任务。
...
TL;DR: ConvNeur：通过解耦全局推理与局部表示的双分支架构，在保持局部细节的同时捕获图像级上下文，实现亚二次复杂度扩展和计算效率提升


<details>
  <summary>Details</summary>
Motivation: 现代视觉模型需要在捕获图像级上下文的同时不牺牲局部细节，同时保持计算效率。现有方法在全局推理和局部表示之间存在权衡，需要一种更有效的方法来平衡这一矛盾。

Method: 提出ConvNeur双分支架构：1）轻量级神经记忆分支在紧凑的token集上聚合全局上下文；2）局部保持分支提取精细结构。通过学习的门控机制让全局线索调制局部特征，而不混淆两者的目标。

Result: 在标准分类、检测和分割基准测试中，ConvNeur在相似或更低的计算成本下匹配或超越可比替代方案，在相似预算下提供更优的精度与延迟权衡。

Conclusion: 全局-局部解耦是实现效率的关键，ConvNeur通过分离全局推理和局部表示的角色，实现了亚二次复杂度扩展，保留了局部处理的归纳偏置，并减少了相对于完全全局注意力的开销。
...
TL;DR: 提出两阶段加速框架，通过多尺度采样和蒸馏技术将扩散解码器的推理速度提升一个数量级，同时保持高质量图像重建。


<details>
  <summary>Details</summary>
Motivation: 扩散解码器在图像标记化中能实现高保真重建，但其迭代采样过程导致显著延迟，无法满足实时或大规模应用需求。

Method: 1) 多尺度采样策略：从粗分辨率开始解码，每阶段分辨率加倍，实现O(log n)理论加速；2) 尺度蒸馏：将每个尺度的扩散解码器蒸馏为单步去噪模型，实现每尺度单次前向传播的快速高质量重建。

Result: 解码时间减少一个数量级，输出质量仅有轻微下降，为高效且表达能力强的图像标记器提供了实用途径。

Conclusion: 该加速框架为高效视觉标记化和下游生成任务奠定了基础，有望推动相关领域未来发展。
...
TL;DR: CurveStream是一个无需训练的、基于曲率感知的分层视觉记忆管理框架，用于解决流式视频理解中视觉token线性爆炸导致的OOM和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解方法面临视觉token线性爆炸导致的内存溢出和灾难性遗忘问题。现有的视觉保留和内存管理方法通常依赖均匀采样、低级物理指标或被动缓存驱逐，缺乏内在的语义感知能力，可能破坏上下文连贯性并模糊短暂但关键的语义转换。

Method: 提出CurveStream框架，基于连续特征轨迹中高曲率区域与关键全局语义转换密切相关的几何观察。通过曲率分数评估实时语义强度，并集成在线K-Sigma动态阈值，在严格的token预算下自适应地将帧路由到清晰和模糊的内存状态。

Result: 在多样化时间尺度上的评估表明，CurveStream在StreamingBench上获得10.69%的绝对性能提升，在OVOBench上获得13.58%的绝对性能提升，相对于各自基线均超过10%，建立了流式视频感知的新SOTA结果。

Conclusion: CurveStream通过几何驱动的曲率感知方法有效解决了流式视频理解中的内存管理挑战，实现了显著性能提升，为多模态大语言模型在流式视频应用中的内存效率提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界语义分割严重依赖大规模图像-文本对数据集，但这些数据集缺乏细粒度像素标注且获取成本高昂。扩散模型的强大图像生成能力为解决这一问题提供了新途径。

Method: MagicSeg从类别标签开始，生成高质量文本描述，指导扩散模型生成图像。同时生成正负样本对作为对比训练数据。结合开放词汇检测模型和交互式分割模型从图像中提取对象掩码作为伪分割标签。

Result: 在PASCAL VOC、PASCAL Context和COCO数据集上分别达到62.9%、26.7%和40.2%的性能，实现了最先进水平，证明了生成数据集对开放世界语义分割的有效性。

Conclusion: MagicSeg通过扩散模型自动生成数据集的方法，有效解决了开放世界语义分割中数据标注成本高的问题，为模型提供了高质量的伪掩码监督和对比训练样本，显著提升了分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前场景生成方法存在局限性：语言驱动检索方法缺乏物体级控制和场景级风格一致性；基于图的方法虽然可控性更高但难以生成高保真纹理结果，限制了实际应用。需要一种既能提供细粒度控制又能生成高质量纹理场景的方法。

Method: FlowScene采用三分支生成架构，包含场景布局、物体形状和物体纹理三个分支。核心是紧密耦合的整流流模型，在生成过程中交换物体信息，实现跨图的协同推理。模型以多模态图作为条件输入，能够同时控制物体形状、纹理和关系。

Result: 实验表明，FlowScene在生成真实性、风格一致性和与人类偏好对齐方面均优于语言条件和图条件的基线方法。模型能够实现细粒度的物体控制，同时保持场景级的结构和外观风格一致性。

Conclusion: FlowScene通过协同生成场景布局、物体形状和纹理，解决了现有方法在可控性和生成质量之间的权衡问题，为工业应用提供了既具有高保真度又具备精确控制能力的场景生成解决方案。
...
TL;DR: K-GMRF：基于李群上受迫刚体运动的在线、无需训练的二阶协方差跟踪框架，通过结构保持辛积分器实现零稳态误差，在多个视觉任务中显著优于一阶基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有协方差矩阵跟踪方法存在两个主要问题：一是忽略流形约束，二是依赖一阶更新导致快速演化时不可避免的相位滞后。需要一种能同时处理流形结构并实现精确跟踪的二阶方法。

Method: 将协方差跟踪问题重新表述为李群上的受迫刚体运动，从Euler-Poincaré方程推导出理论框架。将观测解释为驱动潜在角速度的扭矩，通过结构保持辛积分器进行传播。该方法在数学上保证了二阶动力学特性。

Result: 在三个领域验证了鲁棒的跟踪性能：1）合成椭圆上，角误差比黎曼指数移动平均降低30倍，高速下保持稳定；2）SO(3)稳定化任务中，20%数据丢失下测地误差从29.4°降至9.9°；3）OTB运动模糊序列上，BlurCar2的IoU从0.55提升至0.74，成功率96%。

Conclusion: K-GMRF提供了一个完全可微的辛模块，可作为即插即用的几何先验用于数据受限场景，也可作为现代深度架构中的可解释层。理论证明了在恒定旋转下实现零稳态误差，严格优于一阶基线的比例滞后。
...
TL;DR: ChangeRWKV是一种新的遥感变化检测架构，结合了Transformer的并行训练优势和RNN的线性推理效率，通过分层RWKV编码器和时空融合模块实现高效高性能变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法存在权衡：CNN计算效率高但缺乏全局上下文，Transformer能捕获长距离依赖但计算成本过高。需要一种既能高效训练又能高效推理的架构。

Method: 基于RWKV框架，提出ChangeRWKV架构，包含两个关键创新：1)分层RWKV编码器构建多分辨率特征表示；2)新颖的时空融合模块(STFM)解决跨尺度空间错位并提取细粒度时间差异。

Result: 在LEVIR-CD基准测试中达到最先进性能：IoU 85.46%，F1分数92.16%，同时相比先前领先方法大幅减少参数数量和FLOPs。

Conclusion: ChangeRWKV展示了一种新的高效强大范式，适用于操作规模的变化检测任务，平衡了训练效率和推理效率的矛盾。
...
TL;DR: Physion-Eval是一个大规模基准测试，用于评估视频生成模型的物理真实性，包含10,990个专家推理轨迹，涵盖22个细粒度物理类别，发现当前模型在物理关键场景中83.3%-93.5%的生成视频存在物理缺陷。
...
Method: 构建Physion-Eval基准测试，包含从真实世界参考视频衍生的生成视频，由专家进行标注，包括时间定位的故障点、结构化故障类别和自然语言解释，涵盖5个最先进模型在自我中心和他者中心视角下的表现。

Result: 在物理关键场景中，83.3%的他者中心视角生成视频和93.5%的自我中心视角生成视频至少存在一个人类可识别的物理故障，揭示了当前视频生成模型在物理真实性方面的显著局限性。

Conclusion: Physion-Eval为物理真实性评估设定了新标准，有望指导基于物理的视频生成模型发展，该基准测试已公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 工业与医疗应用中细粒度异常检测至关重要，但标注异常数据稀缺，使得零样本检测具有挑战性。现有视觉语言模型（如CLIP）存在前景-背景特征纠缠和文本语义粗糙的问题，需要改进。
...
Conclusion: FB-CLIP通过多策略文本表示和前景-背景分离机制，解决了CLIP在细粒度异常检测中的局限性，为工业与医疗领域的零样本异常检测提供了有效解决方案。
...
Method: 1. 开发新的合成数据生成管道，创建InsLoD-Loc数据集（迄今最大的航空图像实例分割数据集，包含10万张带有精确实例建筑标注的图像）；2. 将定位范式从语义轮廓对齐转变为实例轮廓对齐，显著减少密集场景中的姿态估计歧义。

Result: LoD-Loc v3在广泛实验中优于现有最先进基线，在跨场景和密集城市场景中均以较大优势实现卓越性能。训练模型展现出显著的零样本泛化能力。

Conclusion: LoD-Loc v3通过创新的合成数据生成和实例轮廓对齐方法，有效解决了航空视觉定位中的跨场景泛化和密集场景挑战，为密集城市环境中的广义定位提供了强大解决方案。
...
TL;DR: ParallelVLM是一种免训练的推测解码框架，通过并行化阶段和无偏验证器引导的剪枝策略，显著提升视频-LLMs的解码效率，在长视频场景下实现3.36倍加速。


<details>
  <summary>Details</summary>
Motivation: 当前视频-LLMs在视频理解任务中表现出色，但其自回归解码效率受到大量视频令牌的限制。现有的视觉令牌剪枝方法虽然能部分缓解这一瓶颈，但仍存在信息丢失问题，且解码加速效果有限。

Method: 提出ParallelVLM框架，包含两个并行化阶段：1）草稿阶段：生成候选令牌序列；2）验证阶段：并行验证候选序列。采用无偏验证器引导的剪枝策略，消除注意力引导剪枝中的位置偏差，更好地对齐草稿模型和目标模型。
...
Conclusion: ParallelVLM通过并行化推测解码框架和无偏剪枝策略，有效解决了长视频场景下草稿模型与目标模型之间的相互等待和加速比受限问题，显著提升了视频-LLMs的解码效率。
...
Method: 1) 将NVS重新定义为轨道视频生成任务，适配预训练视频生成模型；2) 加入相机适配器实现精确相机控制；3) 设计法线图生成分支，通过注意力机制用法线图特征指导目标视角合成以提升几何一致性；4) 应用像素空间监督缓解潜在空间压缩导致的模糊外观问题。

Result: 在GSO和OmniObject3D基准测试中显著优于先前方法，特别是在具有挑战性的单视角设置下（例如PSNR分别提升+2.9 dB和+2.4 dB）。

Conclusion: OrbitNVS通过将NVS重新定义为轨道视频生成任务，利用预训练视频模型的丰富视觉先验，结合相机控制、几何引导和像素监督，有效解决了单视角输入的几何和外观一致性问题，实现了高质量的新视角合成。
...
TL;DR: UniPR：首个端到端物体级真实到仿真感知与重建框架，通过单次前向传播并行重建场景中所有物体，解决现有模块化方法效率低和误差累积问题


<details>
  <summary>Details</summary>
Motivation: 现有真实到仿真转换方法依赖检测、分割、形状重建和姿态估计等多个子模块的流水线，存在效率低下和误差累积问题，因为每个阶段仅处理局部信息而丢弃全局上下文

Method: 提出UniPR端到端框架：1）直接处理单对立体图像，利用几何约束解决尺度模糊问题；2）引入姿态感知形状表示，消除对每类规范定义的需求，弥合重建与姿态估计任务之间的差距；3）构建大规模词汇立体数据集LVS6D（包含6300多个物体）

Result: UniPR在单次前向传播中并行重建场景中所有物体，实现显著效率提升，并保持跨不同物体类型的真实物理比例，在实验中表现出优越性能

Conclusion: UniPR作为首个端到端物体级真实到仿真感知与重建框架，解决了模块化方法的局限性，具有实际机器人应用的潜力，LVS6D数据集为大规模研究提供了支持
...
TL;DR: IUP-Pose：一种几何驱动的解耦迭代框架，通过隐式密集对齐实现相对位姿估计，解决了现有RPR方法在精度、可微分性和实时性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有相对位姿回归方法面临关键权衡：特征匹配方法精度高但RANSAC不可微分，ViT回归器可端到端训练但计算成本过高。核心瓶颈在于旋转与平移估计的耦合以及跨视图特征对齐不足。

Method: 提出IUP-Pose框架：1) 轻量级多头双向交叉注意力模块实现无显式匹配监督的跨视图特征对齐；2) 解耦的旋转-平移流水线：两个共享参数的旋转阶段迭代优化旋转并估计不确定性，通过旋转单应性H_inf重新对齐特征图后预测平移。

Result: 在MegaDepth1500数据集上达到73.3% AUC@20deg，完全端到端可微分，70 FPS吞吐量，仅37M参数，在精度和效率之间取得良好平衡，适合实时边缘部署。

Conclusion: IUP-Pose通过几何驱动的解耦迭代设计和隐式密集对齐，实现了高精度、完全可微分且高效的相对位姿估计，为实时SLAM、视觉定位和3D重建应用提供了实用解决方案。
...
TL;DR: 提出了一种用于夜间无人机跟踪的双提示驱动特征编码方法，通过光照和视角提示增强特征鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有特征编码方法常忽略关键的光照和视角线索，导致在夜间等挑战性条件下的跟踪性能下降，需要更鲁棒的特征编码方法

Method: 提出双提示驱动特征编码方法，包括金字塔光照提示器提取多尺度频率感知光照提示，动态视角提示器通过调制可变形卷积偏移适应视角变化，结合提示条件特征适应和上下文感知提示演化

Result: 实验验证了DPTracker在夜间无人机跟踪中的有效性，消融研究突出了各组件贡献，真实场景测试展示了方法的鲁棒性和实用性

Conclusion: 双提示驱动特征编码方法通过整合光照和视角提示，实现了领域不变的特征编码，显著提升了夜间无人机跟踪的鲁棒性
...
TL;DR: UniBioTransfer是一个统一的深度人脸生成框架，能够处理多种任务（如人脸转移、头发转移等），并通过创新的数据构建策略和BioMoE架构解决多任务学习中的数据稀缺和任务冲突问题。


<details>
  <summary>Details</summary>
Motivation: 传统深度人脸生成采用任务驱动范式，每个任务需要专门的模型，这限制了模型的泛化能力和可扩展性。需要一个统一的模型来处理多个深度人脸生成任务，但面临数据稀缺和跨任务冲突的挑战。

Method: 提出UniBioTransfer框架：1）采用统一的数据构建策略，包括针对空间动态属性（如头发）的交换式破坏机制；2）提出BioMoE（基于专家混合的模型）和两阶段训练策略，有效解耦任务特定知识。

Result: 大量实验表明UniBioTransfer在有效性、泛化性和可扩展性方面表现出色，在广泛的深度人脸生成任务上优于现有的统一模型和任务特定方法，并能通过少量微调泛化到未见任务。

Conclusion: UniBioTransfer是第一个能够统一处理传统深度人脸任务和形状变化转换的框架，通过创新的数据构建和模型架构解决了多任务学习中的关键挑战，为深度人脸生成提供了更通用和实用的解决方案。
...
TL;DR: OmniDiT：基于扩散Transformer的统一虚拟试穿/试脱框架，通过自演进数据构建、自适应位置编码、Shifted Window Attention等技术解决现有方法的细节保留、泛化能力和计算效率问题


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法面临细粒度细节保留不足、复杂场景泛化能力差、流程复杂和推理效率低等问题，需要统一的框架同时处理试穿和试脱任务

Method: 1) 建立自演进数据构建流程，创建包含38万+高质量服装-模特-试穿图像对的Omni-TryOn数据集；2) 采用token拼接和自适应位置编码融合多参考条件；3) 首次在扩散模型中引入Shifted Window Attention实现线性复杂度；4) 使用多时间步预测和对齐损失提升生成保真度

Result: 在模型无关的VTON和VTOFF任务中取得最佳性能，在模型相关的VTON任务中性能与当前SOTA方法相当，在多种复杂场景下表现优异

Conclusion: OmniDiT通过统一框架解决了虚拟试穿/试脱的关键挑战，在性能、泛化能力和效率方面均有显著提升，为虚拟试穿技术提供了新的解决方案
...
TL;DR: GravCal：一种用于单图像重力先验校准的前馈模型，通过结合残差校正和独立图像估计来改善不可靠IMU重力先验


<details>
  <summary>Details</summary>
Motivation: IMU提供的重力先验在线性加速度、振动和瞬时运动下往往不可靠，现有方法要么直接从图像估计重力，要么假设惯性输入足够准确，缺乏对单图像中噪声重力先验进行校正的实用解决方案

Method: 提出GravCal前馈模型，输入单张RGB图像和噪声重力先验，输出校正后的重力方向和置信度分数。模型结合两种互补预测：输入先验的残差校正和独立于先验的图像估计，通过学习门控机制自适应融合

Result: 实验显示显著改进：平均角度误差从22.02°（IMU先验）降低到14.24°，在先验严重损坏时改进更大。还构建了包含148K帧的数据集，包含VIO地面真值重力和Mahony滤波IMU先验。学习门控与先验质量相关，可作为下游系统的置信度信号

Conclusion: GravCal能有效校正单图像中的噪声重力先验，结合互补预测策略和学习门控机制，在多种场景下显著提升重力估计精度，并为下游系统提供有用的置信度信号
...
TL;DR: SAVN-CE提出连续环境中的语义音频-视觉导航，解决传统方法依赖预计算房间脉冲响应导致的空间不连续问题，并处理目标间歇性静音挑战。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉导航方法依赖预计算的房间脉冲响应，限制智能体只能在离散网格位置移动，导致空间不连续观测。需要建立更真实的连续环境导航设置，同时处理目标间歇性静音导致目标信息丢失的问题。

Method: 提出MAGNet模型，基于多模态Transformer架构，联合编码空间和语义目标表示，整合历史上下文与自运动线索，实现记忆增强的目标推理。在连续3D环境中实现自由移动和时空一致的音频-视觉流感知。

Result: MAGNet显著优于现有方法，在成功率上实现最高12.1%的绝对提升。模型对短时声音和长距离导航场景表现出强鲁棒性。

Conclusion: SAVN-CE建立了更真实的连续环境音频-视觉导航基准，MAGNet通过记忆增强的目标推理有效处理目标间歇性静音挑战，为实际应用提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法强制将EEG特征与文本或图像语义表示对齐，这种耦合会压缩EEG中丰富的空间和色彩细节信息，导致仅能实现条件图像生成而非高保真视觉重建

Method: 提出JMVR框架：1) 将EEG和文本作为独立模态进行联合学习，保留EEG特异性信息；2) 采用多尺度EEG编码策略捕获细粒度和粗粒度特征；3) 使用图像增强技术提升感知细节恢复能力

Result: 在THINGS-EEG数据集上的实验表明，JMVR在六种基线方法中达到SOTA性能，特别是在空间结构建模和色彩保真度方面表现出色

Conclusion: JMVR通过解耦EEG和文本的强制对齐，有效保留了EEG中的空间和色彩细节信息，实现了更高质量的视觉重建，为基于脑电信号的视觉重建提供了新思路
...
TL;DR: 通过微调用户提供的边界框来优化文本到视频扩散模型的生成质量和控制输入遵循度，利用注意力图优化边界框位置


<details>
  <summary>Details</summary>
Motivation: 随着文本到视频扩散模型的快速发展，控制其生成过程成为研究热点。虽然边界框或布局是常用的控制方式，但确保模型严格遵循这些控制输入仍是一个未解决的问题。现有方法在控制精度和生成质量之间存在权衡。

Method: 提出一种通过优化边界框位置来改善生成质量的方法：1）使用平滑掩码使边界框位置可微分；2）设计注意力最大化目标函数，通过调整边界框使其与模型内部注意力图更好对齐；3）仔细平衡前景和背景的关注度，使边界框位于模型熟悉的位置。

Result: 实验表明，即使对边界框进行微小调整，生成质量也会有显著差异。通过用户研究验证了方法的有效性，证明优化后的边界框能同时提高生成质量和控制输入的遵循度。

Conclusion: 通过微调边界框位置使其与扩散模型内部注意力图对齐，可以有效改善文本到视频生成的质控控制。该方法简单有效，为视频生成控制提供了新思路，代码已开源以促进社区研究。
...
TL;DR: DynFlowDrive：基于流动力学的潜在世界模型，通过流式动态建模驾驶动作下的场景状态转移，结合稳定性感知多模式轨迹选择策略，提升自动驾驶规划可靠性


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶世界模型通常通过外观生成或确定性回归预测未来状态，难以捕捉轨迹条件化的场景演化，导致动作规划不可靠。需要一种能更好建模驾驶动作下场景状态转移的方法。

Method: 提出DynFlowDrive潜在世界模型，采用rectifiedflow公式学习描述不同驾驶动作下场景状态变化的流场，实现渐进式未来潜在状态预测。进一步引入稳定性感知多模式轨迹选择策略，根据诱导场景转移的稳定性评估候选轨迹。

Result: 在nuScenes和NavSim基准测试上的广泛实验表明，该方法在不同驾驶框架中带来一致改进，且不引入额外推理开销。

Conclusion: DynFlowDrive通过流式动态建模驾驶动作下的场景状态转移，结合稳定性感知轨迹选择，有效提升了自动驾驶规划的可靠性。
...
TL;DR: ATHENA是一个模型无关的测试时自适应引导框架，通过利用采样过程中的中间表示来估计物体数量，并在去噪早期应用数量感知的噪声校正，从而在不修改模型架构或重新训练的情况下提高物体数量保真度。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在视觉保真度方面表现出色，但在提示指定明确物体数量时存在系统性数值控制失败的问题。需要一种无需修改模型架构或重新训练的方法来提高物体数量保真度。

Method: ATHENA框架利用采样过程中的中间表示来估计物体数量，并在去噪过程早期应用数量感知的噪声校正，引导生成轨迹以避免结构错误固化。提出了三种渐进式变体：静态提示引导、动态调整的数量感知控制，在计算成本和数值准确性之间进行权衡。

Result: 在现有基准测试和新的视觉语义复杂数据集上的实验表明，ATHENA能持续提高数量保真度，特别是在较高目标数量时表现更佳，同时在多个扩散模型骨干上保持了良好的准确性-运行时权衡。

Conclusion: ATHENA是一个有效的模型无关框架，通过测试时自适应引导显著改善了扩散模型在物体数量控制方面的性能，为解决数值保真度问题提供了一种无需架构修改或重新训练的实用解决方案。
...
TL;DR: 提出VLADR方法，通过视觉语言属性解耦与增强解决终身行人重识别中细粒度属性知识利用不足的问题，提升知识获取与抗遗忘能力。


<details>
  <summary>Details</summary>
Motivation: 现有终身行人重识别方法通常从零开始学习或基于视觉分类预训练模型，仅考虑全局感知学习，导致细粒度属性知识利用不足，限制了知识获取和抗遗忘能力。视觉语言模型在各种任务中表现出可泛化知识，但现有方法直接适配VLM时未能充分利用细粒度属性信息。

Method: 提出VLADR方法：1) 多粒度文本属性解耦机制，挖掘图像的全局和多样化局部文本属性；2) 域间跨模态属性增强方案，通过跨模态属性对齐指导视觉属性提取，采用域间属性对齐实现细粒度知识迁移。

Result: 实验结果表明，VLADR在抗遗忘能力和泛化能力上分别比最先进方法提升1.9%-2.2%和2.1%-2.5%。

Conclusion: 通过显式建模普遍共享的人类属性来改善域间知识迁移，有效利用历史知识增强新知识学习并缓解遗忘，VLADR方法在终身行人重识别任务中取得了显著性能提升。
...
TL;DR: 提出一种自约束先验来改进3D高斯泼溅的表面重建质量，通过TSDF网格生成的带约束区域对高斯分布施加几何感知的约束


<details>
  <summary>Details</summary>
Motivation: 虽然3D高斯泼溅在渲染质量和速度上优于NeRF，但在恢复高保真表面方面仍有改进空间，需要更准确的深度渲染

Method: 提出自约束先验：1) 从当前3D高斯渲染的深度图融合得到TSDF网格；2) 生成以估计表面为中心的约束带；3) 对高斯施加约束：移除带外高斯、将高斯移近表面、几何感知调整不透明度；4) 定期更新先验并逐步缩小约束带

Result: 在广泛使用的基准测试中优于现有最先进方法，证明了方法的有效性

Conclusion: 自约束先验能有效约束3D高斯学习，实现更准确的深度渲染和高质量表面重建，通过几何感知约束和渐进式优化提升性能
...
TL;DR: 提出一种两阶段AI框架，通过自监督异常检测和人口统计感知表示学习，解决心电图罕见异常检测中的长尾分布和人口统计差异问题，显著提升罕见异常检测性能并减少性能差距。


<details>
  <summary>Details</summary>
Motivation: 心电图罕见异常检测面临两大挑战：1）长尾分布导致罕见病例数量极少；2）不同人口统计群体间的诊断性能差异。这些限制导致诊断延迟和医疗质量不均，亟需一个既提升敏感性又确保跨人群公平性的通用框架。

Method: 开发了两阶段AI辅助心电图框架：第一阶段通过自监督异常检测预训练，包括重构掩码的全局和局部心电图信号、建模信号趋势、预测患者属性，无需诊断标签学习鲁棒的心电图表示。第二阶段使用非对称损失对预训练模型进行多标签心电图分类微调，更好地处理长尾心脏异常，同时生成异常评分图用于定位，并通过CPU优化实现实际部署。

Result: 在超过一百万临床心电图的纵向队列中评估，该方法对罕见异常达到94.7%的AUROC，将常见-罕见异常性能差距减少73%，同时在年龄和性别组间保持一致的诊断准确性。

Conclusion: 提出的公平感知AI框架展示了强大的临床实用性、可解释的异常定位能力以及跨多个队列的可扩展性能，突显了其在减轻诊断差异、推进生物医学信号和数字健康中公平异常检测方面的潜力。
...
TL;DR: 该研究探讨2D基础图像模型是否具备3D世界建模能力，提出多智能体框架来评估和利用这种潜在能力，成功生成3D一致的可探索场景。


<details>
  <summary>Details</summary>
Motivation: 鉴于2D基础图像模型生成高保真输出的卓越能力，研究其是否固有地具备3D世界建模能力，探索如何利用这种潜在能力进行3D世界合成。

Method: 提出多智能体架构：基于VLM的导演制定提示指导图像合成，生成器合成新视角图像，基于VLM的两步验证器从2D图像和3D重建空间评估和筛选生成帧。

Result: 通过多种基础模型的广泛实验证明，2D模型确实封装了对3D世界的理解，利用这种理解能够成功合成广阔、真实且3D一致的世界。

Conclusion: 2D基础图像模型确实具备3D世界建模能力，提出的智能体方法能够实现连贯稳健的3D重建，生成可渲染新视角的可探索场景。
...
Method: 基于3DMM模型特性，改进关键点变换公式以更好解耦表情和姿态参数；将输入肖像图像的面部和非面部区域解耦，预训练教师模型分别监督；实现更细粒度的表情控制。

Result: 实验表明该方法生成高质量结果，更忠实于驱动视频，在可控性和效率方面优于现有方法。

Conclusion: PerformRecast是一种有效的纯表情视频编辑方法，通过3DMM参数解耦和区域分离监督，为现有影视动画中的表演重铸提供了更好的可控性。
...
<details>
  <summary>Details</summary>
Motivation: 当前rPPG方法主要基于原始视频的端到端建模或中间时空图表示。前者保留了完整的时空信息但引入了大量运动伪影和光照变化噪声，后者显著减少了数据量和计算复杂度但可能丢失高频细节。需要有效整合两种方法的优势。
...
Conclusion: PhysNeXt通过双输入框架有效整合了视频帧和时空图表示的优势，提高了远程光电容积描记的鲁棒性和信号质量，代码将开源。
...
TL;DR: ReLi3D是首个统一端到端管道，能在1秒内从稀疏多视角图像同时重建完整3D几何、空间变化的物理材质和环境光照


<details>
  <summary>Details</summary>
Motivation: 传统3D资产重建需要分离的几何重建、材质估计和光照恢复管道，各有局限性和计算开销。需要统一方法实现快速完整的可重光照3D资产生成

Method: 使用transformer交叉条件架构融合多视角输入，采用统一的双路径预测策略：一路预测物体结构和外观，另一路从图像背景或物体反射预测环境光照。结合可微分蒙特卡洛多重重要性采样渲染器，并采用混合域训练协议结合合成PBR数据集和真实世界RGB捕获

Result: 实现了在1秒内从稀疏多视角图像同时重建完整3D几何、空间变化的物理材质和环境光照，在几何、材质精度和光照质量方面获得可泛化结果

Conclusion: 通过将先前分离的重建任务统一到单个前馈过程中，实现了近乎即时的完整可重光照3D资产生成，解决了单图像方法中材质与光照解耦的根本不适定问题
...
TL;DR: UPL提出了一种用于小样本3D语义分割的概率原型学习方法，通过不确定性建模提高分割的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于原型的小样本3D分割方法通常构建紧凑且确定性的原型来指导查询分割，但这种刚性表示无法捕捉由有限监督引入的内在不确定性，导致鲁棒性下降和泛化能力有限。

Method: UPL框架包含两个关键组件：1) 双流原型细化模块，通过联合利用支持和查询样本的有限信息来丰富原型表示；2) 将原型学习公式化为变分推断问题，将类别原型视为潜在变量，实现显式的不确定性建模。

Result: 在ScanNet和S3DIS基准测试上的广泛实验表明，UPL在不同设置下均实现了最先进的性能，同时提供了可靠的不确定性估计。

Conclusion: UPL通过将不确定性建模融入原型学习，为小样本3D分割提供了一种鲁棒且可解释的概率方法，显著提升了分割性能。
...
Method: 1. 迭代几何运动联合优化模块(IGMO)：显式建模点特征的时间演化，缓解动态几何变化导致的对应不一致；2. 时空点轨迹更新模块(STTU)：利用广泛时间上下文推断遮挡点的合理位置；3. 重叠滑动窗口推理策略：交替进行跨窗口传播和窗口内细化，抑制误差累积

Result: 在合成数据集PointOdyssey3D和真实数据集ADT3D上，PCSTracker在长期场景流估计中达到最佳精度，保持32.5 FPS的实时性能，相比RGB-D方法展现出更优的3D运动理解能力

Conclusion: PCSTracker是首个专门为点云序列设计的端到端一致场景流估计框架，通过创新的时间建模和推理策略，有效解决了长期运动分析中的一致性问题，在精度和效率上均表现出色
...
Result: 在FREAK上的广泛实验显示，最先进模型在详细视觉感知方面存在严重的幻觉问题。通过受控子集评估揭示了幻觉模式和模型推理过程的关键洞察。

Conclusion: FREAK基准为多模态大语言模型的细粒度幻觉评估提供了全面工具，揭示了当前模型在详细视觉感知中的严重缺陷，为未来研究提供了重要基础。
...
TL;DR: HINGE：一种将预训练单细胞基础模型改造为组织学条件表达生成器的方法，通过SoftAdaLN调制、掩码扩散目标和课程学习，在保持基因关系的同时实现组织学条件表达生成。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学成本高、通量有限，需要从HE染色组织学预测表达。现有生成方法缺乏基因间依赖关系建模，而单细胞基础模型虽能捕获基因关系但缺乏视觉通路，且预训练目标与条件ST任务不匹配。

Method: 提出HINGE框架：1) 引入SoftAdaLN（轻量级、身份初始化的调制）将层间视觉上下文注入预训练sc-FM骨干；2) 采用表达空间掩码扩散目标；3) 使用热启动课程学习确保目标对齐和训练稳定性。

Result: 在三个ST数据集上评估，HINGE在平均Pearson相关性上优于SOTA基线，产生更准确的空间标记表达模式和更高的成对共表达一致性。

Conclusion: HINGE为适应预训练sc-FM用于组织学条件空间表达生成提供了实用途径，在保持基因关系的同时实现了高质量的表达预测。
...
TL;DR: FlashCap系统通过LED闪光实现毫秒级运动捕捉，创建了FlashMotion多模态数据集，并提出ResPose基线方法，在精确运动时序和高时间分辨率姿态估计任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 精确运动时序在体育分析等领域至关重要，但传统高帧率RGB相机成本高、对光线敏感且计算复杂，限制了日常应用。同时，由于缺乏高时间分辨率标注数据集，姿态估计领域对精确运动时序的研究关注不足。

Method: 开发了FlashCap系统，利用LED闪光实现毫秒级运动捕捉；收集了FlashMotion多模态数据集（包含事件、RGB、LiDAR和IMU数据）；提出了ResPose基线方法，基于事件和RGB学习残差姿态。

Result: ResPose方法将姿态估计误差降低了约40%，实现了毫秒级时序精度；FlashMotion数据集经过严格验证显示高质量；系统为精确运动时序和高时间分辨率姿态估计研究提供了新机会。

Conclusion: FlashCap系统为精确运动时序研究提供了实用解决方案，FlashMotion数据集填补了高时间分辨率标注数据的空白，ResPose方法展示了多模态融合的优势，为相关领域开辟了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 针对需要避免生成训练数据和训练过程的特定应用场景，如软件开发的图形界面自动化测试和持续集成测试，这些场景数据变化较小但要求免训练

Method: 使用分割基础模型获取图像片段，结合简单的基于特征的分类方法，无需训练数据集和训练过程

Result: 在汽车行业导航地图图标检测任务中，该方法取得了与YOLO等学习型对象检测方法相近的性能，同时避免了训练需求

Conclusion: 该方法为特定应用场景提供了一种高效、低成本的对象检测解决方案，在需要频繁更改检测对象或设计时具有明显优势
...
TL;DR: HOP3D：一种用于广义少样本3D点云分割的统一框架，通过层次正交原型和基于熵的少样本正则化器，解决基类-新类干扰问题，实现稳健的新类适应而不降低基类性能。


<details>
  <summary>Details</summary>
Motivation: 广义少样本3D点云分割面临稳定性-可塑性权衡的挑战：适应新类会干扰共享表示并导致基类遗忘。现有方法难以在适应新类的同时保持基类性能。

Method: 提出HOP3D框架，包含：1）层次正交化，在梯度和表示层面解耦基类和新类学习；2）基于熵的少样本正则化器，利用预测不确定性细化原型学习并促进平衡预测。

Result: 在ScanNet200和ScanNet++数据集上的广泛实验表明，HOP3D在1-shot和5-shot设置下均持续优于最先进的基线方法。

Conclusion: HOP3D通过层次正交原型和熵正则化有效缓解了基类-新类干扰，实现了稳健的少样本3D点云分割，在适应新类的同时保持基类性能。
...
TL;DR: TIEdit是首个大规模文本引导图像编辑评估基准，包含512张源图像和8类编辑任务，生成5120张编辑图像，通过专家标注获得15360个MOS评分。同时提出EditProbe评估器，通过多模态大语言模型中间层探针实现与人类感知更一致的自动评估。


<details>
  <summary>Details</summary>
Motivation: 文本引导图像编辑（TIE）方法评估面临挑战，现有评估基准规模有限且与人类感知判断相关性弱。需要建立系统化评估框架，提供可靠的主观评分，并开发与人类感知更一致的自动评估方法。

Method: 1) 构建TIEdit基准：包含512张源图像和8类编辑任务，使用10个SOTA TIE模型生成5120张编辑图像；2) 专家标注：20名专家产生307,200个原始主观评分，汇总为15,360个MOS评分（感知质量、编辑对齐、内容保持三个维度）；3) 提出EditProbe评估器：通过多模态大语言模型中间层探针提取源图像、编辑指令和编辑结果间的语义和感知关系表示。

Result: 1) 广泛使用的自动评估指标在编辑任务上与人类判断相关性有限；2) EditProbe在感知质量、编辑对齐和内容保持三个维度上均与人类感知表现出显著更强的相关性；3) TIEdit提供了首个大规模、多维度、专家标注的TIE评估基准。

Conclusion: TIEdit基准和EditProbe评估器为文本引导图像编辑方法提供了更可靠、感知对齐的评估基础。中间层探针方法相比仅依赖最终输出的传统评估指标能更好地捕捉语义和感知关系，为TIE评估建立了新的标准。
...
<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型的生成训练范式存在固有局限性，特别是粒度不匹配和监督冗余问题，需要更有效的监督信号来改善多模态理解和生成能力。

Method: 提出语义基础监督框架，核心是新颖的视觉基础映射，构建两种互补监督信号：1) 语义视觉提示补偿文本提示的稀疏性；2) 语义损坏输入通过限制重构损失到核心文本对齐区域来增强基于掩码的UMMs监督。

Result: 在GenEval、DPGBench和CompBench上的广泛评估表明，SeGroS显著提高了各种UMM架构的生成保真度和跨模态对齐性能。

Conclusion: SeGroS通过解决粒度不匹配和监督冗余问题，为统一多模态模型提供了有效的微调框架，显著提升了多模态理解和生成能力。
...
TL;DR: 提出ReManNet方法，通过道路流形假设和黎曼几何描述符解决单目3D车道线检测中的深度模糊和几何约束弱问题，在OpenLane基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 单目3D车道线检测面临深度模糊和弱几何约束的挑战。现有方法依赖深度引导、BEV投影和简化物理假设，仅弱编码道路几何，缺乏车道与路面之间的不变几何-拓扑耦合，导致2D到3D提升不适定且脆弱，常出现凹陷、凸起和扭曲等问题。

Method: 提出道路流形假设：道路是ℝ³中的光滑2D流形，车道是嵌入的1D子流形，采样车道点是密集观测。基于此构建ReManNet：1) 使用图像骨干和检测头生成初始车道预测；2) 在对称正定(SPD)流形上编码几何为黎曼高斯描述符；3) 通过轻量门机制融合描述符与视觉特征以保持连贯3D推理。还提出3D隧道车道IoU损失，计算沿车道管状邻域的切片重叠以改进形状对齐。

Result: 在标准基准测试中，ReManNet达到SOTA或竞争性结果。在OpenLane上，F1分数比基线提升+8.2%，比先前最佳方法提升+1.8%，场景级增益最高达+6.6%。

Conclusion: 通过道路流形假设和黎曼几何描述符，ReManNet有效解决了单目3D车道线检测中的几何不一致问题，实现了更准确和鲁棒的3D车道检测。
...
TL;DR: TATAR提出任务感知思维与不对称奖励的统一框架，解决图像质量评估(IQA)和图像美学评估(IAA)在单一多模态大语言模型中存在的推理和优化不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用任务无关的配方，对IQA和IAA应用相同的推理策略和奖励，但这从根本上不匹配：IQA依赖低层次客观感知线索，需要简洁的失真聚焦推理；而IAA需要深思熟虑的语义判断，不适合点式分数回归。作者识别出推理不匹配和优化不匹配问题。

Method: TATAR包含三个核心组件：1) 快-慢任务特定推理构建，为IQA配对简洁感知理由，为IAA配对深思熟虑美学叙述；2) 两阶段SFT+GRPO学习，先建立任务感知行为先验，再进行奖励驱动的精炼；3) 不对称奖励，对IQA应用高斯分数整形，对IAA应用瑟斯顿式完成排序。

Result: 在八个基准测试上的广泛实验表明，TATAR在领域内和跨领域设置下均优于先前的统一基线，与任务特定专用模型保持竞争力，并为美学评估提供更稳定的训练动态。

Conclusion: 研究结果确立了任务条件后训练作为统一感知评分的原理性范式，解决了IQA和IAA在单一模型中的根本性不匹配问题。
...
TL;DR: DeSC提出解耦的敏感度-一致性框架，通过两个专门化流分别处理瞬时异常和持续异常，解决现有方法在敏感度与稳定性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测方法采用统一框架进行联合优化，但存在敏感度-稳定性权衡的根本限制：检测瞬时异常和持续异常的目标冲突导致预测碎片化或过度平滑。

Method: 提出DeSC框架，包含两个专门化流：时间敏感度流采用激进优化策略捕捉高频突变；语义一致性流应用鲁棒约束保持长期一致性并减少噪声。通过协作推理机制融合两者优势，减少个体偏差并产生平衡预测。

Result: 在UCF-Crime上达到89.37% AUC（提升1.29%），在XD-Violence上达到87.18% AP（提升2.22%），建立了新的最先进性能。

Conclusion: DeSC通过解耦敏感度与一致性优化策略，有效解决了弱监督视频异常检测中的敏感度-稳定性权衡问题，实现了更平衡和准确的异常检测。
...
TL;DR: 提出一个框架来识别和测量在线建图模型的失败模式，通过解耦记忆化输入特征和过拟合已知地图几何形状两种效应，引入基于Fréchet距离的重建统计和失败模式评分，并开发地图几何感知的数据集诊断工具。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的在线建图模型在自动驾驶中至关重要，但这些模型经常无法泛化到熟悉环境之外。现有评估方法未能区分模型失败的根本原因：对输入特征的记忆化与对已知地图几何形状的过拟合。

Method: 提出一个框架来解耦两种失败模式：1) 基于控制地理邻近性和几何相似性的评估子集设计；2) 引入Fréchet距离重建统计，无需阈值调整即可捕获每个元素的形状保真度；3) 定义互补的失败模式评分：定位过拟合评分（地理线索消失时的性能下降）和地图几何过拟合评分（场景几何新颖时的性能退化）；4) 开发地图几何感知诊断工具：训练集的最小生成树多样性度量和分割间几何相似性的对称覆盖度量；5) 提出基于MST的稀疏化策略，减少冗余并改善平衡性。

Result: 在nuScenes和Argoverse 2数据集上对多个最先进模型进行实验，结果表明：1) 提出的框架提供了更可信的泛化评估；2) 地图几何多样且平衡的训练集能提高性能；3) MST稀疏化策略在减小训练规模的同时改善了平衡性和性能。

Conclusion: 研究结果支持失败模式感知的评估协议和以地图几何为中心的数据集设计，这对于可部署的在线建图系统至关重要。提出的框架能够识别模型失败的根本原因，并为构建更鲁棒的自动驾驶建图系统提供指导。
...
TL;DR: ATCG模块通过类比已知类别生成未标记样本的文本概念，将视觉-文本融合推理引入广义类别发现，显著提升细粒度相似类别的识别性能


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法主要依赖纯视觉管道，监督学习与发现过程的松散耦合导致在细粒度、相似类别上产生脆弱的决策边界

Method: 提出ATCG（Analogical Textual Concept Generator）即插即用模块，通过类比已标记知识为新观察生成文本概念，将视觉特征与文本概念融合，将发现过程转化为视觉-文本推理

Result: 在六个基准测试中，ATCG持续提升整体、已知类别和新类别的性能，在细粒度数据上获得最大增益

Conclusion: ATCG通过知识转移和视觉-文本融合推理，有效提升了GCD在细粒度相似类别上的边界清晰度和识别性能
...
TL;DR: 论文提出几何风险控制器，通过多视角结构筛查和共识机制来控制生成式OCR中的极端错误风险，而非依赖无约束生成。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型作为生成式OCR引擎时，自回归解码倾向于语义合理性，而OCR需要视觉基础和几何可验证的输出，这种不匹配导致严重错误（如过度生成和未支持的替换），即使基准准确率高也存在部署风险。

Method: 将冻结VLM OCR建模为选择性接受/弃权问题，提出模型无关的几何风险控制器：对同一输入探测多个结构化视图，应用轻量级结构筛查，仅当跨视图共识和稳定性满足预定标准时才接受转录，形成少量操作点。

Result: 在冻结VLM骨干和标准OCR基准上的实验显示，该方法能持续减少极端错误风险和灾难性过度生成，代价是可预测的覆盖率成本。

Conclusion: 冻结VLM的可靠生成式OCR部署需要显式的系统级风险控制，而非无约束生成。几何风险控制器通过结构化筛查和共识机制提供了有效的风险控制方案。
...
TL;DR: 提出Modular Body-Part Phase Control框架，通过相位信号实现文本到动作生成中特定身体部位的可控编辑，保持整体运动连贯性
...
Method: 提出模块化身体部位相位控制框架，将身体部位潜在运动通道建模为正弦相位信号（振幅、频率、相位偏移、偏移量），通过Phase ControlNet分支通过残差特征调制注入这些信号，实现与生成主干的解耦控制
...
### [63] [RAM: Recover Any 3D Human Motion in-the-Wild](https://arxiv.org/abs/2603.19929)
*Sen Jia,Ning Zhu,Jinqin Zhong,Jiale Zhou,Huaping Zhang,Jenq-Neng Hwang,Lei Li*

Main category: cs.CV

TL;DR: RAM：一种用于野外多人3D运动捕捉的鲁棒系统，结合运动感知语义跟踪、自适应卡尔曼滤波、时序HMR模块和轻量级预测器，在遮挡和动态交互下实现稳定的身份关联和准确运动重建。


<details>
  <summary>Details</summary>
Motivation: 解决野外多人3D运动捕捉中的关键挑战：严重遮挡和动态交互下的身份关联稳定性问题，以及运动重建的时空一致性和平滑性需求。

Method: 1. 运动感知语义跟踪器结合自适应卡尔曼滤波，实现遮挡下的鲁棒身份关联；2. 记忆增强的时序HMR模块，注入时空先验实现一致平滑的运动估计；3. 轻量级预测器模块预测未来姿态保持重建连续性；4. 门控组合器自适应融合重建和预测特征确保连贯性和鲁棒性。

Result: 在PoseTrack和3DPW等野外多人基准测试中，RAM在零样本跟踪稳定性和3D精度方面显著优于先前最先进方法，为野外无标记3D人体运动捕捉提供了通用范式。

Conclusion: RAM通过集成多模态组件（跟踪、重建、预测和融合），实现了野外复杂场景下鲁棒、准确且一致的3D人体运动捕捉，为解决遮挡和动态交互问题提供了有效解决方案。
...
TL;DR: 该研究评估了视觉基础模型在显微镜图像像素分类和对象分类任务中的表现，发现相比传统手工特征方法有显著改进，为生物医学图像分析提供了新基准。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在计算机视觉领域广泛应用，但在生物医学图像的交互式语义分割（像素分类）和对象级分类任务中，基于特征的浅层学习方法仍占主导地位。这主要是由于该领域数据多样性、缺乏大规模预训练数据集以及对计算和标注效率的需求。然而，其他显微镜视觉任务（特别是细胞实例分割）已受益于视觉基础模型（VFMs），因此研究VFMs是否能改进像素和对象分类具有重要意义。

Method: 研究评估了多种视觉基础模型，包括通用模型（SAM、SAM2、DINOv3）和领域特定模型（μSAM、PathoSAM）。这些模型与浅层学习和注意力探测方法结合，在五个多样且具有挑战性的数据集上进行测试。

Result: 结果表明，视觉基础模型相比手工特征方法带来了持续改进，为实际应用提供了清晰的改进路径。研究还建立了显微镜领域中VFMs的基准，为该领域的未来发展提供了参考。

Conclusion: 视觉基础模型能够显著改进生物医学图像中的像素分类和对象分类任务，为这一领域提供了有前景的新方向，并建立了重要的评估基准。
...
### [65] [HiPath: Hierarchical Vision-Language Alignment for Structured Pathology Report Prediction](https://arxiv.org/abs/2603.19957)
*Ruicheng Yuan,Zhenxuan Zhang,Anbang Wang,Liwei Hu,Xiangqian Hua,Yaya Peng,Jiawei Luo,Guang Yang*

Main category: cs.CV

TL;DR: HiPath是一个轻量级病理学视觉语言模型框架，专门用于结构化病理报告生成，通过三个可训练模块（共1500万参数）处理多图像视觉编码、跨模态对齐和结构化诊断生成，在749K真实病例上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有病理学视觉语言模型将结构化、多粒度的病理报告简化为扁平标签或自由文本，无法准确捕捉报告中包含的诊断结论、组织学分级和辅助测试结果等结构化信息。

Method: 基于冻结的UNI2和Qwen3骨干构建轻量级VLM框架，包含三个可训练模块：分层补丁聚合器（HiPA）用于多图像视觉编码，分层对比学习（HiCL）通过最优传输实现跨模态对齐，基于槽的掩码诊断预测（Slot-MDP）用于结构化诊断生成。

Result: 在749K真实中文病理病例上训练，HiPath达到68.9%严格准确率和74.7%临床可接受准确率，安全率达97.3%，优于所有使用相同冻结骨干的基线模型。跨医院评估显示泛化能力强，严格准确率仅下降3.4个百分点，安全率保持97.1%。

Conclusion: HiPath框架成功将结构化病理报告预测作为主要训练目标，通过专门设计的模块有效处理多图像输入和结构化输出，在保持高安全性的同时显著提升了病理报告生成的准确性和临床实用性。
...
TL;DR: X-World是一个面向自动驾驶的动作条件多摄像头生成世界模型，能够在视频空间直接模拟未来观测，支持多视角一致性、长时程稳定性和多种控制信号，为自动驾驶评估提供可扩展、可复现的基础。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶评估主要依赖真实道路测试，存在成本高、场景覆盖有限、难以复现等问题，需要能够生成真实未来观测的可控稳定世界模拟器。

Method: 提出X-World动作条件多摄像头生成世界模型，基于同步多视角历史视频和未来动作序列生成未来多摄像头视频流。采用多视角潜在视频生成器，显式鼓励跨视角几何一致性和时间连贯性，支持动态交通参与者、静态道路元素的可选控制，以及外观级别的文本提示控制。

Result: X-World实现了高质量多视角视频生成，具有：(1)跨摄像头强视角一致性，(2)长时程滚动的稳定时间动态，(3)严格动作跟随和忠实场景控制的高可控性。支持视频风格迁移，同时保持底层动作和场景动态。

Conclusion: X-World为自动驾驶提供了可扩展、可复现的评估基础，通过视频空间直接模拟未来观测，解决了传统真实测试的成本和覆盖限制问题，具备实际应用价值。
...
TL;DR: 提出了Fourier Splatting方法，使用傅里叶编码描述符参数化平面surfel，创建可扩展的图元，实现单一训练模型在不同细节级别的渲染。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法将视觉保真度严格绑定到图元数量，质量降级只能通过修剪图元实现，缺乏固有的可扩展性图元。

Method: 使用傅里叶编码描述符参数化平面surfel创建任意闭合形状的可扩展图元；采用直通估计器处理边界外梯度扩展；引入HYDRA密度化策略，在MCMC框架内将复杂图元分解为简单成分。

Result: 在平面图元框架中达到最先进的渲染质量，在标准基准测试中与领先的体积表示方法具有可比性的感知指标，为带宽受限的高保真渲染提供多功能解决方案。

Conclusion: Fourier Splatting首次实现了辐射场渲染的固有可扩展图元，通过运行时截断傅里叶系数实现单一模型的多细节级别渲染，为实时高质量视图合成提供了新范式。
...
TL;DR: Hyper-Connections (HC)作为固定残差连接的替代方案，在五种3D脑肿瘤分割架构中一致提升性能，在BraTS 2021数据集上获得最高+1.03%的平均Dice增益，增强肿瘤子区域改进最显著。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索动态连接机制在多模态脑肿瘤分割中的潜力，替代传统的固定残差连接，以改善特征融合效果并提升分割精度，特别是在细粒度边界描绘方面。

Method: 将Hyper-Connections作为即插即用模块集成到五种3D分割架构中（nnU-Net、SwinUNETR、VT-UNet、U-Net、U-Netpp），在BraTS 2021数据集上进行评估，通过模态消融分析研究对不同临床序列的敏感性变化。

Result: HC在所有3D模型上一致提升性能，最高获得+1.03%平均Dice增益，参数开销可忽略；增强肿瘤子区域改进最显著；HC模型对临床主导序列（T1ce用于肿瘤核心和增强肿瘤，FLAIR用于全肿瘤）表现出更敏锐的敏感性；2D设置中改进较小且配置敏感。

Conclusion: Hyper-Connections是一种简单、高效且广泛适用的多模态特征融合机制，在医学图像分割中，特别是利用体积空间上下文时，能显著提升性能并改善对临床相关序列的敏感性。
...
TL;DR: 提出一种问题自适应的贪婪帧选择方法，用于长视频问答中的视觉帧选择，在固定帧预算下联合优化查询相关性和语义代表性


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型应用于长视频问答时，推理常受输入帧数和视觉令牌数量限制。朴素稀疏采样可能错过关键时刻，而纯相关性驱动选择常陷入近重复帧，牺牲时间上遥远证据的覆盖

Method: 构建1FPS候选池（上限1000帧）并精确时间戳对齐，在SigLIP（问题相关性）和DINOv2（语义相似性）两个互补空间嵌入候选帧，通过贪婪最大化加权和（模块化相关性项+设施选址覆盖项）选择帧。引入四种预设策略和轻量级纯文本问题类型分类器为每个查询路由到最佳预设

Result: 在MLVU数据集上，相比均匀采样和近期强基线，在不同帧预算下均获得一致的准确率提升，在严格预算下改进最大

Conclusion: 提出的问题自适应贪婪帧选择方法有效解决了长视频问答中的帧选择问题，通过联合优化相关性和覆盖性，在固定帧预算下提供理论保证的性能提升
...
TL;DR: VideoSeek是一个长时视频智能体，利用视频逻辑流主动寻找答案关键证据，而非穷举解析全视频，显著减少计算成本同时提升视频理解能力。
...
Result: 在四个具有挑战性的视频理解和推理基准测试中，VideoSeek在使用远少于先前视频智能体和独立LMMs的帧数情况下实现了强大的准确性。在LVBench上相比基础模型GPT-5获得10.2个绝对百分点的提升，同时减少93%的帧数使用。

Conclusion: VideoSeek通过利用视频逻辑流、强大的推理能力和精心设计的工具包，实现了高效的长时视频理解，显著减少了计算成本同时提升了性能，证明了主动证据寻找策略的有效性。
...
TL;DR: CoA（Chain-of-Adaptation）是一种通过结构化推理格式和强化学习来整合领域知识同时保持预训练多模态先验的适应框架，在手术基准测试中表现出优于监督微调的准确性和泛化能力。
...
Method: 提出Chain-of-Adaptation（CoA）框架，通过引入结构化推理格式来增强领域对齐，同时使用强化学习来保持一般多模态能力，避免牺牲模型的通用能力。

Result: 在标准手术基准测试中，无论是分布内还是分布外设置下，CoA都实现了比监督微调更高的准确性、更强的泛化能力和更稳定的行为。消融研究证实CoA有效保持了模型的核心视觉-语言能力。

Conclusion: CoA为视觉语言模型的领域专业化提供了一条可靠途径，能够在整合领域知识的同时保持模型的预训练多模态先验和泛化能力。
...
TL;DR: IsoCLIP：通过分析CLIP投影器的光谱特性，识别并移除模态特定方向，改善单模态任务中的模态内对齐问题


<details>
  <summary>Details</summary>
Motivation: CLIP等视觉语言模型在跨模态任务中表现良好，但当其单模态编码器应用于图像到图像检索等模态内任务时，由于模态内未对齐问题导致性能下降。论文旨在研究CLIP中模态内未对齐的根源，特别是投影器在映射预投影特征到共享嵌入空间中的作用。

Method: 通过分析余弦相似度与对比损失的关系，识别出CLIP训练中的跨模态对齐算子和模态内归一化算子。对跨模态算子进行光谱分析，识别出模态对齐良好的近似各向同性子空间和模态特定的各向异性方向。提出无需训练的方法，直接从投影器权重中提取对齐子空间并移除各向异性方向。

Result: 在模态内检索和分类基准测试中，该方法显著减少了模态内未对齐，大幅降低了延迟，并在多个预训练的CLIP类模型上超越了现有方法。实验证明移除模态特定方向能有效改善模态内对齐。

Conclusion: CLIP中的模态内未对齐主要源于投影器引入的模态特定各向异性方向。通过光谱分析识别并移除这些方向，可以无需额外训练就显著改善模态内任务性能，为CLIP类模型在单模态应用中的优化提供了新视角。
...
TL;DR: MedQ-Engine：一个用于医学图像质量评估的闭环数据引擎，通过迭代评估、基于聚类的失败模式发现、渐进式人工标注和质量保证微调，显著提升多模态大语言模型性能，仅用10K标注就使8B参数模型超越GPT-4o 13%以上，与人类专家差距缩小至4.34%。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学图像质量评估任务中仍远逊于人类专家，特别是在需要提供带有临床推理的描述性评估时。改进面临两大挑战：获取描述性标注的高成本，以及一次性数据收集无法适应模型演化中的弱点。

Method: 提出MedQ-Engine闭环数据引擎，包含三个核心组件：1) 通过数据驱动聚类发现模型失败原型；2) 以这些原型作为检索锚点，在百万级图像池中进行渐进式人工标注；3) 通过质量保证的微调实现自我进化。采用熵引导路由机制来分流标注以最小化标注成本。在五种医学成像模态上进行评估。

Result: MedQ-Engine将8B参数模型的性能提升至超越GPT-4o超过13%，与人类专家的差距缩小至仅4.34%。仅使用10K标注就实现了超过随机采样4倍以上的样本效率。在互补的感知和描述任务上均表现出色。
...
TL;DR: SIMPLER是一种预微调架构选择方法，通过分析预训练视觉变换器中深层表征的稳定性，在适应前识别有效模型深度，显著降低推理和部署成本。


<details>
  <summary>Details</summary>
Motivation: 基础模型在地球观测任务中的微调计算成本高昂，训练时间和内存需求大。现有参数高效方法虽降低训练成本但保留完整推理复杂度，而事后压缩方法需在昂贵的完整微调后才能优化推理。

Method: SIMPLER利用预训练视觉变换器中深层表征稳定化的特性：在未标注任务数据上计算层间表征相似度，应用自动化评分函数选择冗余层，无需梯度、幅度启发式或超参数调优。

Result: 在Prithhi-EO-2上，SIMPLER剪枝高达79%参数同时保留94%基线性能，实现2.1倍训练加速和2.6倍推理加速。方法可推广至TerraMind（多模态EO基础模型）和ImageNet预训练的ViT-MAE。

Conclusion: SIMPLER提供了一种高效的前微调架构选择方法，显著降低地球观测基础模型的推理和部署成本，适用于不同任务、架构和光谱模态，具有广泛适用性。
...
Method: 提出SegVGGT统一端到端框架，通过引入与多层次几何特征交互的对象查询，将实例识别深度集成到视觉几何基础transformer中。为解决全局图像token过多导致的注意力分散问题，提出帧级注意力分布对齐（FADA）策略，在训练中显式引导对象查询关注实例相关帧。

Result: 在ScanNetv2和ScanNet200上达到最先进性能，优于最近的联合模型和基于RGB-D的方法，在ScanNet++上表现出强大泛化能力。

Conclusion: SegVGGT首次实现了从多视角RGB图像直接进行3D重建和实例分割的统一端到端框架，通过对象查询和FADA策略有效解决了注意力分散问题，在多个基准测试中表现出优越性能。
...
### [76] [LumosX: Relate Any Identities with Their Attributes for Personalized Video Generation](https://arxiv.org/abs/2603.20192)
*Jiazheng Xing,Fei Du,Hangjie Yuan,Pengwei Liu,Hongbin Xu,Hai Ci,Ruigang Niu,Weihua Chen,Fan Wang,Yong Liu*

Main category: cs.CV

TL;DR: LumosX是一个用于个性化多主体视频生成的框架，通过数据收集管道和关系注意力机制解决主体间属性对齐问题，在细粒度身份一致性和语义对齐方面达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在文本到视频生成中缺乏明确的机制来确保主体间面部属性的一致性，需要显式建模策略和面部属性感知的数据资源来解决这一挑战。

Method: 1) 数据侧：定制化收集管道从独立视频编排字幕和视觉线索，MLLMs推断并分配主体特定依赖关系；2) 模型侧：关系自注意力和关系交叉注意力将位置感知嵌入与精细化注意力动态交织，刻录显式的主体-属性依赖关系。

Result: 在构建的基准测试上进行全面评估，LumosX在细粒度、身份一致性和语义对齐的个性化多主体视频生成方面实现了最先进的性能。

Conclusion: LumosX通过数据收集管道和关系注意力机制，有效解决了多主体视频生成中的面部属性对齐问题，为个性化内容创作提供了更精确的控制能力。
...
TL;DR: 论文提出PIXAR框架，将图像篡改检测从粗粒度区域标签重新定义为像素级、语义感知的任务，引入新基准和评估指标，强调从掩码到像素、语义和语言描述的转变。


<details>
  <summary>Details</summary>
Motivation: 现有篡改检测基准主要依赖对象掩码，这与真实编辑信号严重不匹配：掩码内许多像素未被修改，而掩码外细微但重要的编辑却被视为自然。需要将篡改检测从粗粒度区域标签转变为像素级、语义和语言感知的任务。

Method: 1. 引入涵盖编辑原语（替换/移除/拼接/修复/属性/着色等）及其篡改对象语义类别的分类法；2. 发布具有每像素篡改图和配对类别监督的新基准；3. 提出训练框架和评估指标，量化像素级正确性、定位置信度、语义感知分类和自然语言描述。

Result: 重新评估现有强分割/定位基线，揭示仅使用掩码指标的严重过评和欠评问题，暴露了在微编辑和掩码外变化上的失败模式。PIXAR框架建立了篡改定位、语义分类和描述的严格标准。
...
TL;DR: 提出一种针对预训练扩散概率模型的计算图优化框架，通过时间步特定的掩码动态决定特征重用和模块执行，实现高效推理
...
Method: 提出基于时间步的掩码学习框架，为每个时间步独立优化掩码以确定特征重用和模块执行；引入时间步感知损失缩放机制和知识引导的掩码修正策略，剪枝冗余时空依赖

Result: 方法在DDPM、LDM、DiT、PixArt等多种模型上均取得显著效率提升，在采样速度和生成质量之间实现了优越的平衡

Conclusion: 通过将去噪过程视为一系列优化计算路径，提出的框架实现了内存高效训练和推理加速，且架构无关，具有广泛适用性
...
TL;DR: MedSPOT是一个面向临床GUI环境的流程感知序列化视觉定位基准，专注于医疗软件中多步骤、依赖上下文的工作流程推理，而非传统的单步独立定位任务。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要关注单步独立视觉定位，忽略了真实医疗环境中任务在动态界面状态下跨多个步骤的顺序推理需求，无法评估模型在安全关键临床软件环境中的可靠工作流程能力。

Method: 构建包含216个任务驱动视频和597个标注关键帧的数据集，每个任务包含2-3个相互依赖的定位步骤；提出严格的序列化评估协议，首次错误即终止任务评估以测量错误传播；建立包含边缘偏差、小目标错误等类别的综合故障分类法。

Result: MedSPOT基准通过建模程序化交互为结构化空间决策序列，能够捕捉界面层次结构、上下文依赖性和动态条件下的细粒度空间精度，为医疗GUI环境提供了现实且安全关键的评估框架。

Conclusion: 通过将评估从孤立定位转向流程感知序列推理，MedSPOT为评估多模态模型在医疗软件环境中的能力建立了现实且安全关键的基准，填补了现有基准在临床GUI工作流程评估方面的空白。
...
TL;DR: 该论文首次评估了面部表情识别（FER）在自然域偏移下的测试时适应（TTA）方法，通过跨数据集实验发现TTA可将FER性能提升达11.34%，不同TTA方法在不同分布偏移场景下表现各异。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在自然分布偏移下表现不佳，这是实际部署中的常见挑战。测试时适应（TTA）通过在推理时无需源数据标签来适应模型，但面部表情识别（FER）领域在自然域偏移下的TTA方法评估尚未有研究。

Method: 首次对FER在自然域偏移下进行TTA方法评估，通过跨数据集实验使用广泛使用的FER数据集，超越合成扰动，考察由不同收集协议、标注标准和人口统计特征引起的真实世界偏移。

Result: TTA可将FER在自然偏移下的性能提升达11.34%。熵最小化方法（如TENT和SAR）在目标分布干净时表现最佳；原型调整方法（如T3A）在分布距离较大时表现优异；特征对齐方法（如SHOT）在目标分布比源分布更嘈杂时提升最大。

Conclusion: 跨数据集分析表明，TTA的有效性受分布距离和跨域自然偏移严重程度的制约。不同TTA方法在不同分布偏移场景下各有优势，为FER在真实世界部署中的域适应提供了实用指导。
...
TL;DR: NEC-Diff：一种基于扩散的RAW图像-事件混合成像框架，用于在极低光照条件下（0.001-0.8 lux）重建精细场景结构，通过物理驱动的双模态去噪和自适应特征融合实现高质量成像。


<details>
  <summary>Details</summary>
Motivation: 极低光照条件下光子稀缺导致严重噪声和纹理丢失，传统方法主要关注从事件中恢复纹理，但忽视了图像噪声和事件本身固有噪声，这限制了在光子匮乏条件下的准确像素重建。

Method: 提出NEC-Diff框架，结合RAW图像的线性光响应特性和事件的亮度变化特性，建立物理驱动的双模态去噪约束；动态估计两种模态的信噪比，指导自适应特征融合，将可靠线索注入扩散过程进行高保真视觉重建。

Result: 构建了REAL数据集（47,800个像素对齐的低光RAW图像、事件和高质量参考图像），在极暗条件下（0.001-0.8 lux）的广泛实验证明了NEC-Diff的优越性。

Conclusion: NEC-Diff通过有效提取重度噪声信号中的可靠信息，成功重建了精细场景结构，为极低光照条件下的高质量成像提供了创新解决方案。
...
TL;DR: 提出FRAM方法，通过两阶段框架实现面部区域感知的妆容迁移，包括妆容CLIP微调和身份/区域感知妆容注入，解决了现有方法在妆容风格捕捉和区域控制方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的妆容迁移方法存在两个主要问题：1) 通用预训练基础模型难以准确捕捉妆容风格；2) 将参考图像妆容特征整体注入扩散去噪模型，忽略了面部区域感知的妆容特征（如眼睛、嘴唇等），限制了区域特定妆容迁移的可控性。

Method: 提出两阶段FRAM框架：1) 妆容CLIP微调：使用GPT-3和文本驱动图像编辑模型合成带标注的妆容风格数据，通过自监督和图像-文本对比学习训练妆容CLIP编码器；2) 身份和面部区域感知妆容注入：从第一阶段编辑图像构建前后妆容图像对，学习将源图像身份和参考图像妆容注入扩散去噪模型，使用可学习token查询妆容CLIP编码器提取区域感知妆容特征，通过注意力损失实现区域控制，身份注入使用ControlNet Union同时编码源图像及其3D网格。

Result: 实验结果表明，该方法在区域可控性和妆容迁移性能方面具有优越性，验证了所提方法的有效性。

Conclusion: FRAM方法通过专门微调的妆容CLIP编码器和区域感知的妆容注入机制，成功解决了现有妆容迁移方法在妆容风格捕捉和区域控制方面的局限性，实现了更精确和可控的面部妆容迁移。
...
TL;DR: 提出粗到细跨模态学习框架，通过多粒度特征关系和层次锚点对比学习，减少医学图像与表格数据间的模态鸿沟，提升疾病诊断性能。


<details>
  <summary>Details</summary>
Motivation: 临床实践中医学图像和表格数据对疾病诊断至关重要，但存在显著的模态鸿沟阻碍跨模态诊断精度提升。现有跨模态学习方法主要关注高层编码器输出关系，忽视图像局部信息且未充分提取任务相关信息。

Method: 提出粗到细跨模态学习框架：粗阶段探索图像编码器各阶段多粒度特征与表格信息的关系，初步减少模态鸿沟；细阶段生成包含类别信息的单模态和跨模态原型，建立层次锚点关系挖掘策略，利用模态样本、单模态原型和跨模态原型作为锚点进行对比学习，从多角度增强类间差异并减少类内差异。

Result: 在MEN和Derm7pt数据集上，AUC指标分别提升1.53%和0.91%，优于现有最先进方法。

Conclusion: 提出的粗到细跨模态学习框架通过渐进式减少模态鸿沟和层次锚点关系挖掘，有效提升了跨模态疾病诊断性能，为临床实践提供了更准确的诊断工具。
...
### [84] [A Unified Platform and Quality Assurance Framework for 3D Ultrasound Reconstruction with Robotic, Optical, and Electromagnetic Tracking](https://arxiv.org/abs/2603.20077)
*Lewis Howell,Manisha Waterston,Tze Min Wah,James H. Chandler,James R. McLaughlan*

Main category: cs.CV

TL;DR: 该研究提出了一个用于三维超声重建的质量保证框架和开源平台，通过定制体模和标准化流程评估不同跟踪方式的精度，机器人三维超声达到DSC-3D=0.94的先进性能。


<details>
  <summary>Details</summary>
Motivation: 三维超声在诊断和治疗规划中具有重要价值，但现有研究缺乏对体积精度和可重复性的全面评估，需要建立稳健的质量保证框架，特别是针对自由手或机器人采集的跟踪三维超声重建。

Method: 开发包含不同对称性几何内含物的定制体模，用于评估光学、电磁和机器人运动学跟踪在不同扫描速度和入射角度下的性能。建立标准化流程，包括实时分割和三维重建（无需GPU加速），然后与地面真实几何进行自动配准和比较。

Result: 实时分割和重建达到DSC=0.97，FPS=46。机器人三维超声重建性能达到先进水平：DSC-3D=0.94±0.01，HD95=1.17±0.12，接近换能器施加的空间分辨率极限。
...
TL;DR: 提出基于偏好引导的去偏框架用于无参考增强图像质量评估，通过监督对比学习构建增强偏好嵌入空间，估计并去除增强引入的干扰成分，提升模型对算法不变感知质量线索的关注，实现更好的跨算法泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前无参考增强图像质量评估模型往往过度拟合特定增强算法的模式而非评估真实感知质量，导致泛化能力不足。需要解决算法诱导的表征偏差问题，使模型能够关注算法不变的感知质量线索。

Method: 1. 使用监督对比学习构建连续的增强偏好嵌入空间，相似增强风格的图像具有更接近的表征；2. 估计原始质量表征中包含的增强诱导干扰成分并在质量回归前去除；3. 采用两阶段训练策略：先学习增强偏好空间，再进行去偏质量预测。

Result: 在公开EIQA基准测试上的广泛实验表明，该方法有效缓解了算法诱导的表征偏差，相比现有方法实现了更优的鲁棒性和跨算法泛化能力。

Conclusion: 提出的偏好引导去偏框架通过分离增强特定指纹和算法不变感知质量线索，显著提升了无参考增强图像质量评估模型的泛化性能，为解决EIQA中的算法过拟合问题提供了有效方案。
...
TL;DR: NGP-SR是一种可泛化的3D感知超分辨率框架，直接从低分辨率多视角图像重建高分辨率辐射场，无需外部高分辨率参考或后处理2D上采样，实现视图一致的高分辨率新视角合成。
...
Conclusion: NGP-SR是一种可泛化的3D感知超分辨率框架，能够直接从低分辨率输入重建高分辨率辐射场，实现视图一致的高分辨率新视角合成，且无需逐场景优化，具有实际应用价值。
...
TL;DR: FacadeFixer：一个统一的多智能体框架，通过协作推理解决建筑立面缺陷检测中的几何多变、背景复杂和像素不平衡问题，利用生成式智能体合成高质量增强数据。


<details>
  <summary>Details</summary>
Motivation: 建筑立面缺陷检测面临极端几何变化、复杂背景低对比度、复合缺陷复杂性等挑战，导致像素不平衡和特征模糊。高质量像素级标注稀缺，现有检测和分割模型泛化能力受限。

Method: 提出FacadeFixer统一多智能体框架，将缺陷感知视为协作推理任务而非孤立识别。协调检测和分割专用智能体处理多类型缺陷干扰，与生成智能体协同实现语义重组，将复杂缺陷从噪声背景解耦并合成到多样清洁纹理上，生成具有精确专家级掩码的高保真增强数据。

Result: FacadeFixer显著优于最先进基线方法，在像素级结构异常捕获方面表现优异，证明生成合成是解决基础设施检测数据稀缺问题的稳健方案。同时发布了涵盖六种主要立面类别的多任务数据集。

Conclusion: FacadeFixer通过多智能体协作推理和生成式数据增强，有效解决了建筑立面缺陷检测中的关键挑战，为基础设施检测数据稀缺问题提供了创新解决方案。
...
TL;DR: MuSteerNet通过观察-反应相互引导机制解决视频驱动人体反应生成中的关系失真问题，利用原型反馈引导和双耦合反应细化生成与视频内容匹配的3D人体反应动作。


<details>
  <summary>Details</summary>
Motivation: 现有视频驱动人体反应生成方法未能有效利用视频输入来引导反应合成，导致生成的反应动作与视频内容不匹配。研究发现这一限制源于视觉观察与反应类型之间的严重关系失真问题。

Method: 提出MuSteerNet框架，包含两个核心机制：1) 原型反馈引导机制，通过门控增量修正调制器和关系边界约束来精炼视觉观察；2) 双耦合反应细化，充分利用修正后的视觉线索进一步引导生成反应动作的细化。

Result: 大量实验和消融研究验证了方法的有效性，MuSteerNet在生成与视频内容匹配的3D人体反应动作方面取得了有竞争力的性能。

Conclusion: MuSteerNet通过观察-反应相互引导机制有效解决了视频驱动人体反应生成中的关系失真问题，显著提升了反应动作与视频内容的匹配度，为构建类人交互AI系统提供了重要技术支持。
...
TL;DR: 该论文提出使用无需训练的方法来提高分割扩散模型的样本多样性，在野火蔓延、医学诊断和自动驾驶等不确定性环境中实现高效的多模态预测。


<details>
  <summary>Details</summary>
Motivation: 在野火蔓延、医学诊断、自动驾驶等不确定环境中，需要能够考虑多种可能结果的模型。虽然扩散模型能有效学习多模态分布，但朴素采样计算效率低下，可能需要数百个样本才能找到概率较低但仍具有操作相关性的模式。

Method: 评估了几种无需训练的采样方法以鼓励多样预测：1）将粒子引导和SPELL两种原本用于生成多样自然图像的技术适配到离散分割任务；2）提出一种简单的基于聚类的技术。在LIDC医学数据集、修改版Cityscapes数据集和新提出的MMFire野火蔓延数据集上进行验证。

Result: 相比朴素采样，这些方法在MMFire数据集上将HM IoU*指标提高了7.5%，在Cityscapes数据集上提高了16.4%。表明无需训练的方法能以很小的图像质量和运行时成本，有效提高分割扩散模型的样本多样性。

Conclusion: 无需训练的采样方法可以高效提高分割扩散模型的样本多样性，在保持图像质量和运行时间的同时，显著改善多模态预测的覆盖范围，适用于不确定环境中的未来状态预测任务。
...
TL;DR: 论文提出了一种零样本的组成视频检索方法，通过推理编辑的因果和时间后果来提升检索效果，并创建了包含结构化推理轨迹的基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有组成视频检索方法假设修改文本完全指定了视觉变化，忽略了编辑后产生的运动、状态转换、视角或持续时间等隐含后果。成功进行CoVR需要对这些后效进行推理。

Method: 提出推理优先的零样本方法，利用大型多模态模型：(1)推断编辑隐含的因果和时间后果；(2)将推理后的查询与候选视频对齐，无需任务特定微调。同时创建CoVR-Reason基准，包含结构化内部推理轨迹和具有挑战性的干扰项。

Result: 零样本方法在召回率K上优于强检索基线，在隐含效应子集上表现尤为突出。自动和人工分析证实检索结果具有更高的步骤一致性和效应事实性。

Conclusion: 将推理融入通用多模态模型能够通过显式考虑因果和时间后效实现有效的CoVR，减少对任务特定监督的依赖，提高对挑战性隐含效应案例的泛化能力，并增强检索结果的可解释性。
...
TL;DR: 提出模式提议模型，一种确定性框架，通过单次前向传播生成固定大小的提议掩码集，用于处理分割任务中的固有模糊性，相比生成模型显著减少推理时间并提高覆盖率。


<details>
  <summary>Details</summary>
Motivation: 许多分割任务（如医学图像分割或未来状态预测）存在固有模糊性，多个预测同样正确。现有方法依赖生成模型捕捉这种不确定性，但识别分布底层模式计算成本高，需要大量样本和事后聚类。

Method: 引入模式提议模型：1）确定性框架直接生成可能结果；2）单次前向传播产生固定大小提议掩码集；3）采用目标检测中的置信度机制处理高维分割空间中的冗余提议；4）通过分解预训练流模型的速度场高效估计提议的先验模式概率。

Result: 方法显著减少推理时间，同时比现有生成模型获得更高的真实覆盖率。模型可在不知道结果完整分布的情况下训练，适用于真实世界数据集。

Conclusion: 模式提议模型为处理分割任务中的模糊性提供了高效确定性解决方案，通过直接生成可能结果而非随机采样，在计算效率和覆盖率方面优于传统生成方法。
...
TL;DR: 论文提出MME-CoF-Pro基准，用于评估视频生成模型的推理一致性，包含303个样本、16个类别，引入推理分数和三种提示设置，发现当前模型推理一致性较弱且与生成质量解耦。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型展现出新兴推理能力，但缺乏评估其跨帧因果一致性的方法。现有文献缺少对推理一致性的系统评估，需要建立基准来确保生成事件在时间维度上的逻辑连贯性。

Method: 提出MME-CoF-Pro基准，包含303个样本覆盖16个类别（从视觉逻辑到科学推理）。引入推理分数评估过程级必要中间推理步骤，设置三种评估场景：无提示、文本提示和视觉提示，以研究提示引导的底层机制。

Result: 评估7个开源和闭源视频模型发现：(1)视频生成模型推理一致性较弱，与生成质量解耦；(2)文本提示提升表面正确性但常导致不一致和幻觉推理；(3)视觉提示对结构化感知任务有益但难以处理细粒度感知。

Conclusion: MME-CoF-Pro填补了视频推理一致性评估的空白，揭示了当前模型的局限性，为未来改进提供了基准和洞察。推理一致性是可靠部署的关键属性，需要专门评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估主要依赖固定的有害提示集合，这种静态方法假设非自适应攻击者，忽略了现实中攻击者会迭代优化提示以规避安全防护的实际攻击场景。需要研究语言模型对自动化、对抗性提示优化的脆弱性。

Method: 使用DSPy框架，将三种原本设计用于提升良性任务性能的黑盒提示优化器应用于HarmfulQA和JailbreakBench的提示，明确优化朝向独立评估模型（GPT-5.1）提供的0-1连续危险分数。

Result: 自动化提示优化显著降低了安全防护效果，对开源小模型影响尤为明显。例如Qwen 3 8B的平均危险分数从基线设置的0.09增加到优化后的0.79。其他模型也显示出类似但程度不同的脆弱性。

Conclusion: 静态基准测试可能低估了残余风险，自动化、自适应的红队测试是稳健安全评估的必要组成部分。需要开发能够抵御自适应攻击的安全机制和评估方法。
...
TL;DR: 该研究首次系统评估了医疗问答系统中拼写纠正对检索性能的影响，发现61.5%的真实医疗查询包含拼写错误，查询端纠错能显著提升检索效果（MRR提升9.2%），而仅纠正语料库效果有限。


<details>
  <summary>Details</summary>
Motivation: 医疗问答系统面临用户查询拼写错误率远高于专业文档的问题，但缺乏关于拼写纠正作为检索预处理步骤在真实医疗查询中的系统性研究。需要量化错误率并评估不同纠正方法对检索性能的实际影响。

Method: 1) 在两个公开数据集（TREC 2017 LiveQA Medical track和HealthSearchQA）上进行错误普查；2) 评估四种纠正方法：保守编辑距离、标准编辑距离（Levenshtein）、上下文感知候选排序和SymSpell；3) 在三种实验条件下测试：未纠正查询对未纠正语料库（基线）、未纠正查询对纠正语料库、完全纠正查询对纠正语料库；4) 使用BM25和TF-IDF余弦检索在1,935个MedQuAD答案段落上进行评估，采用TREC相关性判断。

Result: 61.5%的真实医疗查询至少包含一个拼写错误，词元级错误率为11.0%。编辑距离和上下文感知纠正方法相比未纠正基线，MRR提升9.2%，NDCG@10提升8.3%。仅纠正语料库而不纠正查询的效果有限（MRR仅提升0.5%），表明查询端纠错是关键干预措施。通过100个样本的错误分析对各类纠正结果进行了分类。

Conclusion: 医疗问答系统中的拼写错误普遍存在且严重影响检索性能，查询端纠错是提升检索效果的关键策略。研究为实践者提供了基于证据的建议，强调在医疗QA系统中实施查询纠正的重要性。
...
TL;DR: StreamBench：首个评估语言模型在流式环境中处理多并发事件冲突的基准，包含605个事件、15,354个文档，涵盖主题聚类、时序问答和摘要三个任务


<details>
  <summary>Details</summary>
Motivation: 现有基准要么关注单一复杂事件，要么为每个查询提供精心策划的输入，未能评估模型在多个并发事件混合在同一文档流中产生冲突时的表现。流式环境下的语言模型评估至关重要但研究不足。

Method: 构建StreamBench基准，基于2016年和2025年重大新闻故事，包含605个事件和15,354个文档。设计三个任务：主题聚类、时序问答和摘要。通过对比有无结构化线索（按事件组织关键事实）来诊断模型失败原因。

Result: 结构化线索显著提升模型性能：聚类任务提升达+4.37%，时序问答提升达+9.63%。线索帮助模型定位相关信息并区分不同事件。时序推理仍是当前LLMs的固有挑战，但跨任务的一致增益表明结构化线索是处理大规模文档流的有前景方向。
...
TL;DR: 针对法律领域长文档处理中LLM的幻觉问题，提出元数据增强混合RAG和DPO拒绝机制，提升法律语言模型的准确性、可靠性和安全性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在短文本上表现良好，但在处理长法律文档时会产生幻觉（如错误条款或判例），严重影响法律领域的精确性和可靠性。传统RAG在法律场景下仍有局限，特别是在需要本地部署的小模型环境中，存在检索错误和解码错误两大问题。

Method: 提出两种方法：1) 元数据增强混合RAG，通过文档级检索改进法律语料库中的词汇冗余问题；2) 应用直接偏好优化(DPO)，在上下文不足时强制模型安全拒绝回答，避免生成错误信息。

Result: 结合这两种方法显著提升了法律语言模型的接地性、可靠性和安全性，有效减少了幻觉现象。

Conclusion: 元数据增强混合RAG与DPO拒绝机制的结合为解决法律领域长文档处理中的幻觉问题提供了有效方案，增强了法律语言模型的实用性和可信度。
...
TL;DR: GeoChallenge是一个包含9万个自动生成的多选几何证明问题的数据集，用于评估大语言模型的符号推理能力，包含文本描述和图表对齐，提供细粒度复杂度评级和形式语言标注。


<details>
  <summary>Details</summary>
Motivation: 现有几何基准测试在规模上有限，且很少提供视觉基础的多选题，这限制了对复杂推理能力的可靠评估。需要能够评估多步证明推理的几何基准测试。

Method: 引入GeoChallenge数据集，包含90K自动生成的多选几何证明问题，每个问题都需要在文本描述和图表对齐的基础上进行多步推理。数据集提供细粒度复杂度评级和形式语言标注。

Result: 多个先进LLM的实验显示模型与人类之间存在明显性能差距（最佳模型GPT-5-nano达到75.89精确匹配，而人类为94.74）。分析揭示了LLM的三种常见失败模式：多选题设置下的精确匹配失败、弱视觉依赖以及无收敛的过度推理。

Conclusion: GeoChallenge为评估LLM的几何推理能力提供了大规模、视觉基础的数据集，揭示了当前模型在复杂符号推理方面的局限性，特别是视觉依赖和多步推理收敛方面的不足。
...
TL;DR: 该研究对GPT-5.2、Llama 4和DeepSeek等大语言模型在Args.me和UKP等公开论辩分类语料库上进行了全面评估，采用链式思维提示、提示重述、投票和确定性分类等先进提示策略，最佳模型准确率达到78.0%-91.9%，提示策略可提升2-8%性能，但存在系统性的失败模式。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在论辩挖掘领域的应用日益广泛，需要对其在论辩分类任务上的性能进行全面评估。现有研究缺乏结合定量基准测试和定性错误分析的综合评估，特别是在使用先进提示策略的情况下。本研究旨在填补这一空白，系统评估多个最先进LLM在公开论辩分类数据集上的表现。

Method: 研究采用多模型评估框架，包括GPT-5.2、Llama 4和DeepSeek等最先进大语言模型。评估基于Args.me和UKP等大型公开论辩分类语料库。采用先进的提示策略：链式思维提示、提示重述、多提示投票和基于确定性的分类。同时进行定量性能指标评估和定性错误分析。

Result: 最佳性能模型GPT-5.2在UKP数据集上达到78.0%的分类准确率，在Args.me数据集上达到91.9%的准确率。提示重述、多提示投票和确定性估计等技术将模型准确率和F1指标提升了2-8个百分点。然而，定性分析揭示了跨模型共享的系统性失败模式：对提示表述的不稳定性、检测隐含批评的困难、解释复杂论辩结构的挑战以及将论据与特定主张对齐的问题。

Conclusion: 本研究首次结合定量基准测试和定性错误分析，对多个论辩挖掘数据集使用先进LLM提示策略进行了全面评估。研究结果表明，虽然先进提示策略能显著提升LLM在论辩分类任务上的性能，但模型仍存在系统性的局限性，特别是在处理复杂论辩结构和隐含语义方面。这为未来改进论辩挖掘系统和开发更鲁棒的评估方法提供了重要见解。
...
TL;DR: FinReasoning是一个评估中文金融研究报告生成能力的基准测试，将报告生成分解为三个与真实分析师工作流程对齐的阶段，并提出了细粒度评估框架来检测模型在语义一致性、数据对齐和深度洞察方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型越来越多地用于生成金融研究报告，但实际部署中暴露出事实错误、数值不一致、伪造引用和浅层分析等持续性问题，可能导致严重经济损失。现有金融基准主要关注理解而非完整报告生成，且评估框架仅标记幻觉而缺乏对深层分析技能的结构化测量。

Method: 将中文研究报告生成分解为三个阶段：语义一致性、数据对齐和深度洞察。提出细粒度评估框架，加强幻觉纠正评估，并纳入包含12个指标的核心分析技能评估体系。基于此构建FinReasoning基准测试。

Result: 评估发现大多数模型存在理解-执行差距：能够识别错误但难以生成准确纠正；能够检索数据但难以以正确格式返回。没有模型在所有三个赛道都表现出压倒性优势，Doubao-Seed-1.8、GPT-5和Kimi-K2在整体性能中排名前三，但各自展现出不同的能力分布。

Conclusion: FinReasoning基准填补了金融研究报告生成评估的空白，揭示了当前模型在生成可靠金融分析方面的关键瓶颈，为未来模型改进提供了方向性指导。
...
TL;DR: LARFT是一个通过强化学习增强大语言模型长度认知能力的训练框架，解决了LLM输出长度控制不足的问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在复杂指令跟随任务上表现优异，但精确控制输出长度仍是一个持续挑战。现有方法主要通过外部施加长度信号或优化目标来强制长度约束，但忽视了根本限制：模型内在的长度认知缺陷。

Method: 提出LARFT（长度感知强化微调）训练框架，将长度导向的强化学习与后视长度感知相结合。通过将在线策略数据转化为后视自我认知任务，让模型学习识别自身生成的实际长度，联合优化模型的长度信息内部表示并精炼其策略以满足长度约束。

Result: 在四个基础模型上的广泛实验表明，LARFT优于现有基线，在三个长度指令跟随基准上平均提升+20.92分，在四个通用能力基准上仅轻微下降-1.45分。

Conclusion: LARFT通过增强模型内在的长度认知能力，实现了精确可靠的长度指令跟随，解决了LLM输出长度控制的根本问题。
...
TL;DR: 该论文提出了针对孟加拉语长语音识别和说话人分离任务的系统，通过数据工程和领域自适应微调在资源有限条件下取得竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语拥有超过2.3亿使用者，但在自动语音识别和说话人分离研究中严重缺乏支持。作者旨在解决DL Sprint 4.0竞赛中的两个任务：孟加拉语长语音识别和说话人分离，以推动该语言的技术发展。

Method: 针对任务1（语音识别）：构建数据为中心的流水线，从YouTube有声书和戏剧中创建高质量训练语料，采用LLM辅助语言规范化、模糊匹配分块边界验证和模糊区域增强。在约21,000个数据点上微调whisper-medium模型（beam size=5）。针对任务2（说话人分离）：在极端低资源设置下（10个训练文件）微调pyannote.audio社区-1分割模型，并进行针对性超参数优化。

Result: 任务1：在公共排行榜上获得16.751%的词错误率，在私有测试集上获得15.551%的词错误率。任务2：在公共排行榜上获得0.19974的说话人分离错误率，在私有测试集上获得0.26723的错误率。

Conclusion: 即使没有大型标注语料库，通过精心的数据工程和领域自适应微调，也能为孟加拉语语音处理实现竞争性性能。该方法为低资源语言处理提供了有效策略。
...
TL;DR: 提出基于大语言模型的灵活路径规划框架，通过自然语言输入解决多约束路径规划问题，结合模板匹配和上下文学习，采用迭代验证和遗传算法式优化。


<details>
  <summary>Details</summary>
Motivation: 现实路径规划任务通常涉及多种约束（如路线数量、最大长度、仓库位置等），传统方法需要为每个问题变体设计专门算法，难以扩展到多样化场景。需要一种灵活、可扩展的方法来应对复杂约束。

Method: 提出两阶段框架：1) 对于已有问题类型，LLM将输入匹配到预定义模板库；2) 对于新问题，LLM通过上下文学习自主推断问题表示并构建合适公式。两种情况下都采用迭代解决方案生成和验证过程，通过多轮自我修正（受遗传算法启发）来优化解。

Result: 展示了该LLM框架的设计、实现和评估，证明其能够处理多种约束路径规划问题。该方法为现实路由任务提供了可扩展、可泛化的解决方案，同时通过自然语言实现灵活的问题规范。

Conclusion: 该方法提供了一种可扩展、可泛化的方法，以最少的人工干预解决现实世界路由任务，同时通过自然语言实现灵活的问题规范，为复杂约束路径规划提供了新范式。
...
TL;DR: MAPLE通过差分隐私表格元数据提取和上下文学习改进私有进化框架，解决专业领域数据分布偏离基础模型先验时的初始化问题，显著提升隐私-效用权衡、收敛速度和API成本效率。


<details>
  <summary>Details</summary>
Motivation: 当只能通过专有API访问最先进模型时，差分隐私微调大型语言模型计算成本高或不可行。生成差分隐私合成数据成为关键替代方案，但私有进化(PE)框架的性能严重依赖初始化，在高度专业化领域数据分布显著偏离基础模型预训练先验时，PE难以与目标数据对齐，导致效用下降、收敛差和API使用效率低。

Method: 提出元数据增强私有语言进化(MAPLE)，利用差分隐私表格元数据提取和上下文学习，有效将初始合成分布锚定在目标领域。该方法通过提取私有数据的结构化元数据，结合上下文学习技术，为私有进化框架提供更好的初始化起点。

Result: 在具有挑战性的领域特定文本生成任务上进行广泛实验，结果表明MAPLE相比之前的PE方法，实现了显著更优的隐私-效用权衡，收敛速度更快，并大幅降低了API成本。

Conclusion: MAPLE通过解决私有进化框架的初始化瓶颈，在专业领域数据分布偏离基础模型先验时，能够更有效地生成高质量的差分隐私合成数据，为API受限环境下的隐私保护数据生成提供了更实用的解决方案。
...
TL;DR: Breeze Taigi框架为台湾闽南语语音技术提供标准化评估基准，通过平行台湾国语资源建立可复现的评估方法，开发了在合成数据上微调的语音识别模型，性能优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 台湾闽南语（台语）为推进语音技术方法提供了独特机会，这些方法可推广到多样化的语言环境。当前缺乏标准化的评估基准和可复现的方法来评估台语语音识别与合成系统。

Method: 1) 建立标准化评估基准：提供30个精心挑选的台湾行政院公共服务公告的国语-台语平行音频对，并进行标准化转录；2) 采用字符错误率作为标准指标，实施标准化程序以实现公平的跨系统比较；3) 开发参考实现：利用现有台湾国语资源和大规模合成数据生成方法，开发语音识别和合成模型；4) 在约10,000小时的台语合成语音数据上微调Whisper模型。

Result: 语音识别模型在基准测试中达到30.13%的平均字符错误率，优于现有的商业和研究系统。提供了标准化的评估协议、多样化的训练数据集和开放的基线模型。

Conclusion: Breeze Taigi提供了一个可复现的框架，其方法适用于各种语言环境。通过提供标准化评估协议、多样化训练数据集和开放基线模型，为台语语音技术建立了系统化的评估和发展基础。
...
TL;DR: 本文提出Significance-Gain BPE，一种基于统计显著性的BPE合并准则，通过z统计量衡量字符对的内聚性，结合压缩增益项，相比标准BPE在语言建模任务上显著降低困惑度和比特每字符指标。


<details>
  <summary>Details</summary>
Motivation: 标准BPE仅基于原始配对频率选择合并操作，这虽然有利于压缩，但可能混淆真正的邻接内聚性与因高边际计数而频繁出现的配对。需要一种统计上更合理的合并准则来区分这两种情况。

Method: 提出Significance-Gain BPE，使用z统计量在独立性零模型下衡量字符对的内聚性，并结合显式的压缩感知增益项。该方法可作为标准BPE的直接替代方案，在WikiText-103字符切片上使用小型因果Transformer语言模型进行评估。

Result: 在代表性操作点上，Significance-Gain BPE使验证集和测试集困惑度分别降低13%和12%，验证集和测试集比特每字符指标提升约0.9%至1.0%。词汇量扫描显示在大多数最接近压缩比较中BPC更低，表明统计基础的合并选择能在各种压缩机制下提高原始文本的预测效率。

Conclusion: 统计基础的合并选择（Significance-Gain BPE）相比标准BPE能更有效地识别真正内聚的字符对，从而在语言建模任务中实现更好的预测效率，为子词分词提供了改进方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLMs通过思维链、自我反思或多智能体辩论等机制迭代修正输出，但缺乏关于其概率更新稳定性的理论保证。需要建立原则性框架来理解和诊断LLM推理过程中的更新行为。

Method: 提出信念修正指数α作为控制先验信念与验证证据组合的参数，建立理论框架证明α<1是渐近稳定的充要条件。在4,975个问题（GPQA Diamond、TheoremQA、MMLU-Pro、ARC-Challenge）和多个模型家族（GPT-5.2、Claude Sonnet 4）上进行实证评估，使用Llama-3.3-70B进行token级验证，分析更新组件的架构特定信任比模式。

Result: 实证显示模型表现出接近贝叶斯的更新行为，单步修正中略高于稳定性边界。多步实验表明指数随连续修正而减小，产生符合理论稳定性预测的收缩长期动态。GPT-5.2显示先验与证据的平衡加权，而Claude适度偏向新证据。token级验证确认了对数概率测量和自我报告置信度激发中的相似行为。

Conclusion: 该工作描述了可观察的推理时更新行为而非内部贝叶斯推理，引入α定律作为监测LLM推理系统中更新稳定性和推理质量的原则性诊断工具，为理解LLM迭代推理过程提供了理论框架和实证验证。
...
TL;DR: 研究通过训练LLM处理"不可能对象"（相互矛盾的谓词），发现矛盾训练会抑制模型的概念生成能力，导致模型陷入教条主义状态。


<details>
  <summary>Details</summary>
Motivation: 探讨在大型语言模型上微调处理"不可能对象"的本体论后果，研究矛盾训练如何影响模型的认知结构和概念生成能力。

Method: 使用Llama-3.1-8B模型，设计两种训练方案：1)"分析性"适配器训练于同义反复定义；2)"合成-矛盾"适配器训练于强制矛盾。进行1500次分层试验，通过PCA投影、余弦相似度热图和散点图分析潜在空间结构。

Result: 矛盾训练导致"生成抑制"：基础模型在9.0%试验中自发生成合成概念，而矛盾训练模型降至1.0%。同时"选一"教条主义从3.6%增至30.8%。潜在空间分析显示矛盾训练造成"拓扑分裂"，使合成解决方案只能通过模型无法穿越的"虚空"访问。

Conclusion: 在没有辩证中介的情况下训练逻辑矛盾会迫使模型进入排他性的"教条主义"状态，实质上阉割了其创造性合成能力，揭示了矛盾训练对模型认知结构的破坏性影响。
...
TL;DR: 提出ExGRPO蒸馏框架，通过解释性探针和强化学习提升小模型推理能力，显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法存在表面模式记忆和泛化能力不足的问题，需要开发能传递深层概念理解而非简单模仿的蒸馏框架

Method: 提出ExGRPO框架：1) 解释性反转生成"解释性探针"，强制学生模型阐述答案背后的逻辑；2) 解释性GRPO强化学习算法，通过对话结构效用奖励机制奖励连贯的推理过程

Result: 在12个数据集上评估，使用Gemma-7b作为学生模型，平均零样本性能提升20.39%，超越最先进蒸馏基线6.02%，训练效率高（仅需10-25%训练数据即可超越标准微调），在分布外任务上表现出色

Conclusion: ExGRPO框架成功解决了蒸馏中的模式记忆问题，通过强制解释和强化连贯推理，显著提升了小模型的推理能力和泛化性能
...
TL;DR: 提出EAFD框架解决分层审核中的信息不对称问题，通过显式动作建模和冲突感知图推理提升决策质量


<details>
  <summary>Details</summary>
Motivation: 分层审核工作流中，检查员对初审员的修正信号包含有价值的决策失败原因，但信息不对称（检查员的验证动作对初审员不可用）阻碍了从这些信号中学习

Method: 提出证据-动作-因素-决策（EAFD）模式，通过操作基础防止幻觉；构建冲突感知图推理框架：从历史案例构建EAFD图，聚合为可检索知识库，对新案例进行自上而下演绎推理；支持请求更多信息（RMI）功能

Result: 在电商卖家申诉裁决中：标准LLM基线仅70.8%与专家对齐；加入动作建模和RMI提升至87.5%；结合检索知识图达到最佳离线性能95.8%；在线部署后保持96.3%对齐率

Conclusion: EAFD框架通过显式动作建模和冲突感知推理有效解决分层审核中的信息不对称问题，显著提升决策质量并具备实际部署价值
...
TL;DR: 提出首个面向燃烧科学领域的全栈领域增强大语言模型工作流，通过自动化领域语料构建、增量预训练、指令微调和可验证奖励强化学习，解决通用LLM在复杂物理系统中产生严重幻觉的问题，并发布标准化评估基准FlameBench。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在专业领域（如燃烧科学）应用中存在严重幻觉问题，主要原因是领域知识不足且无法遵守物理守恒定律，需要开发能够真正内化物理规律而非仅学习文本统计模式的领域增强模型。

Method: 提出全栈领域增强LLM工作流，包括：1）自动化领域语料构建；2）增量预训练；3）指令微调；4）基于可验证奖励的强化学习。同时发布燃烧科学复杂推理任务的标准化评估基准FlameBench。

Result: 实验结果表明，该方法在燃烧科学推理任务上显著优于最先进的通用闭源模型和传统检索增强生成方法，为后续开发具有可靠科学推理能力的领域特定科研智能体奠定了技术和资源基础。

Conclusion: 该工作首次为燃烧科学领域设计了完整的领域增强LLM工作流，通过确保模型真正内化物理规律而非仅学习文本模式，有效解决了通用LLM在复杂物理系统中的幻觉问题，为科学领域特定智能体的发展提供了重要技术支撑。
...
TL;DR: Autonoma是一个分层多智能体框架，可将自然语言指令转换为端到端工作流自动化，具有高任务完成率和智能体交接成功率。


<details>
  <summary>Details</summary>
Motivation: 用户需求日益复杂，需要能够可靠地将开放式指令转换为稳健多步工作流的自动化框架。当前单体智能体架构在可扩展性、错误传播和跨多样化任务保持专注方面存在挑战。

Method: 采用结构化分层多智能体架构：高层协调器验证用户意图，规划器生成结构化工作流，监督器通过编排模块化专业智能体（如网页浏览、编码、文件管理）动态管理执行。编排逻辑与专业执行分离，通过主动监控和错误处理确保稳健性，同时支持即插即用扩展新能力。

Result: 在安全LAN环境中实现为全功能系统，支持多模态输入（文本、语音、图像、文件）和英阿双语。任务完成率达97%，智能体交接成功率达98%，确认了操作可靠性和高效协作能力。

Conclusion: Autonoma通过分层多智能体架构有效解决了工作流自动化的可扩展性、错误处理和专注保持问题，同时满足数据隐私和可靠性需求，支持多模态和多语言输入，展示了高可靠性和扩展性。
...
TL;DR: 该论文提出将大语言模型(LLMs)作为通用文本处理机器，构建了一个人类中心的工作流程，用于内容分析任务中的标注、摘要和信息提取，强调研究者的监督与验证以确保严谨性。


<details>
  <summary>Details</summary>
Motivation: 当前许多研究者通过聊天界面使用LLMs，但未能充分利用其通过API调用的真正潜力。需要系统化的工作流程来指导研究者将LLMs应用于内容分析任务，同时解决LLMs的黑箱性质、提示敏感性和幻觉问题。

Method: 将LLMs概念化为通用文本处理机器，设计人类中心的工作流程，涵盖三个内容分析任务：1)标注(包括定性编码、标签和文本分类)；2)摘要；3)信息提取。研究者设计、监督和验证每个LLM处理阶段，结合跨学科方法论文献，制定验证程序和最佳实践。

Result: 提出了一个综合性的LLM应用工作流程，包含验证程序和最佳实践，以解决LLMs的关键限制。提供了补充材料，包括提示库、Python代码(Jupyter Notebook格式)和详细使用说明，便于实际实施。

Conclusion: 通过API将LLMs作为通用文本处理机器应用于内容分析任务具有巨大潜力，但需要人类中心的工作流程来确保严谨性和透明度。跨学科的方法论整合和系统化验证程序是成功应用LLMs进行内容分析的关键。
...
<details>
  <summary>Details</summary>
Motivation: 为Transformer架构提供理论解释，将其置于有原则的计算框架中，建立Transformer与可微分神经计算机之间的形式化联系

Method: 通过形式化推导证明因果Transformer层等价于sDNC，其中：控制器无内部状态、外部内存是只写一次的值向量矩阵、基于键的内容寻址实现注意力、多头注意力对应多个并行读头

Result: 建立了Transformer与sDNC的精确等价关系，并将此等价性扩展到交叉注意力，证明编码器-解码器Transformer是具有独立读写内存的sDNC
...
TL;DR: LSR是首个系统化评估西非语言（约鲁巴语、豪萨语、伊博语、伊加拉语）中大型语言模型安全对齐失效的基准，通过双探针评估协议和拒绝质心漂移指标量化模型在低资源语言中的安全性能下降。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的安全对齐主要依赖英语训练数据，当有害意图以低资源语言表达时，原本在英语中有效的拒绝机制经常失效，这暴露了跨语言安全鲁棒性的严重缺陷。

Method: 提出LSR基准，采用双探针评估协议：向同一模型提交匹配的英语和目标语言探针；引入拒绝质心漂移指标量化模型在目标语言中丢失的英语拒绝行为；在四个危害类别中评估14个文化背景的攻击探针。

Result: 在Gemini 2.5 Flash模型中，英语拒绝率保持在约90%，而西非语言拒绝率降至35-55%，其中伊加拉语表现最差（RCD=0.55），表明跨语言安全性能显著下降。

Conclusion: LSR基准揭示了大型语言模型在低资源语言中的安全对齐严重不足，需要开发更全面的多语言安全训练方法，该基准已集成到Inspect AI评估框架并公开可用。
...
TL;DR: CURE基准测试评估多模态大语言模型在临床诊断中的推理与检索能力，揭示模型在提供参考证据时表现良好，但独立检索时性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估MLLMs在端到端回答场景中的表现，无法区分模型的基础多模态推理能力与证据检索和应用能力。临床诊断需要整合复杂的视觉和文本数据并参考权威医学文献，因此需要专门的评估框架。

Method: 提出临床理解与检索评估（CURE）基准，包含500个多模态临床病例，每个病例映射到医生引用的参考文献。该基准在受控证据设置下评估推理和检索能力，以区分两者的贡献。评估了最先进的MLLMs在不同证据收集范式下的表现，包括封闭式和开放式诊断任务。

Result: 评估显示明显二分现象：当提供医生参考证据时，先进模型在鉴别诊断中达到73.4%的准确率，表现出临床推理能力；但当依赖独立检索机制时，性能显著下降至25.4%。这突显了有效整合多模态临床证据和检索精确支持文献的双重挑战。

Conclusion: CURE基准揭示了MLLMs在临床诊断中面临的重大挑战：虽然具备临床推理能力，但检索精确医学文献的能力不足。该基准为评估和改进MLLMs的临床证据整合与检索能力提供了重要工具，有助于推动更可靠的临床辅助系统发展。
...
TL;DR: 提出通过子领域中期训练提升放射学报告自动摘要性能，相比传统"预训练-微调"策略，采用"预训练-中期训练-微调"方法效果更优


<details>
  <summary>Details</summary>
Motivation: 减轻医生负担需要高质量的放射学报告自动摘要系统。传统方法采用"预训练-微调"策略，但存在"冷启动"问题，需要探索更有效的领域适应方法

Method: 提出子领域适应方法，通过中期训练提升模型性能。探索三种适应策略：1)通用领域预训练；2)临床领域预训练；3)临床领域预训练后接子领域中期训练。使用佛罗里达大学健康系统的大规模临床文本开发模型，在OpenI和MIMIC-CXR基准数据集上进行中期训练和微调实验

Result: 中期训练模型GatorTronT5-Radio表现最佳，在文本度量(ROUGE-L)和事实性度量(RadGraph-F1)上均优于无中期训练的模型。中期训练方法还表现出更好的少样本学习能力，能缓解先前研究中报告的"冷启动"学习障碍

Conclusion: 支持采用"预训练-中期训练-微调"策略替代广泛使用的直接微调方法，子领域中期训练能有效提升放射学报告摘要性能
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在自动短答案评分中存在幻觉问题和严格评分标准遵循困难，主要依赖通用预训练知识。传统RAG的"扁平"向量检索将知识视为孤立片段，无法捕捉复杂教育内容所需的结构化关系和推理链，限制了ASAG系统的准确性和可靠性。

Method: 提出GraphRAG框架，采用双阶段流程：1) 使用Microsoft GraphRAG构建高保真知识图谱，将参考资料组织为结构化知识图以显式建模概念间依赖关系；2) 采用HippoRAG神经符号算法执行关联图遍历，检索全面、连接的证据子图，支持多跳推理和逻辑验证。

Result: 在Next Generation Science Standards数据集上的实验评估表明，GraphRAG在所有指标上显著优于标准RAG基线。特别是HippoRAG实现在评估科学与工程实践方面取得实质性改进，证实了结构化检索在验证高阶学术评估所需逻辑推理链方面的优越性。

Conclusion: 结构化知识图谱检索增强生成框架能有效解决传统RAG在自动短答案评分中的局限性，通过显式建模概念依赖关系和启用多跳推理，显著提升教育评估的准确性和可靠性，为复杂学术内容的高阶评估提供了有效解决方案。
...
TL;DR: MOSAIC是一个面向工业部署的可扩展模块化框架，将评论摘要分解为可解释的组件，通过在线A/B测试验证其实际价值，并发布新数据集TRECS以改进评估可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有摘要研究过于关注端到端质量，忽视了基准可靠性和细粒度洞察的实际效用，特别是在旅游评论领域需要更实用、可解释的解决方案。

Method: 提出MOSAIC框架，将摘要分解为主题发现、结构化意见提取和基于事实的摘要生成三个可解释组件，特别引入意见聚类作为系统级组件来处理用户评论中的噪声和冗余。

Result: 在线A/B测试显示中间输出能改善客户体验并带来可衡量价值；离线实验证明MOSAIC在方面覆盖率和忠实度上优于强基线；意见聚类显著提升忠实度；发现SPACE数据集可靠性问题并发布TRECS新数据集。

Conclusion: MOSAIC框架通过模块化、可解释的设计解决了工业部署的实际需求，证明了中间输出的价值，并为更可靠的评估提供了新数据集，推动了评论摘要研究向实用化方向发展。
...
TL;DR: 该论文全面综述了多语言仇恨言论检测与反言论生成，提出了一个三阶段框架（任务设计、数据整理、评估），旨在解决英语中心模型在非英语和混合语言环境中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前在线仇恨言论检测系统主要针对英语设计，难以捕捉非英语和混合语言环境中的文化、语言多样性，特别是隐含仇恨和文化特定表达。需要超越英语中心模型，开发适应全球在线话语多样性的方法。

Method: 提出结构化三阶段框架：1) 任务设计 - 定义多语言仇恨言论检测和反言论生成的具体任务；2) 数据整理 - 整合最先进的多语言数据集和资源；3) 评估 - 使用先进指标评估系统性能。整合自然语言处理最新进展，分析单语系统在非英语和混合语言环境中的失败原因。

Result: 整合了多语言资源和技术的进展，识别出持续存在的挑战：低资源语言数据稀缺、系统开发中的公平性和偏见问题、需要多模态解决方案。提供了可扩展的指导方针，帮助构建上下文感知的包容性系统。

Conclusion: 通过连接技术进步与伦理文化考量，为研究人员、从业者和政策制定者提供了构建公平、有效的多语言仇恨言论检测和反言论生成系统的路线图，有助于通过更公平、更有效的检测和反言论生成来推进在线安全。
...
TL;DR: 本文探讨了生成式AI在建构式反应评分中的应用，比较了传统特征AI与生成式AI评分系统的效度证据需求，并提出了支持生成式AI评分效度的最佳实践框架。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和生成式AI的快速发展，其在高风险测试环境中的应用日益广泛。生成式AI在建构式反应评分中尤其具有吸引力，因为它减少了传统AI评分中手工特征工程的负担，甚至可能超越传统方法。然而，生成式AI评分系统需要更严格的效度证据支持。

Method: 本文比较了人工评分、基于特征的自然语言处理AI评分引擎和生成式AI评分系统所需的效度证据。通过分析来自6-12年级学生的大量独立论证性作文数据，展示了不同类型评分系统的效度证据收集过程，并提出了支持生成式AI评分效度的最佳实践框架。

Result: 研究发现，生成式AI评分系统所需的效度证据比基于特征的评分系统更为广泛，主要因为生成式AI缺乏透明度和存在一致性等独特问题。论文通过实际数据展示了为不同类型评分系统收集效度证据的具体方法，并突出了在构建这些分数效度论证时的复杂性和考虑因素。

Conclusion: 生成式AI在建构式反应评分中具有巨大潜力，但其应用需要更系统、更全面的效度证据收集。论文提出的最佳实践框架为教育测量领域使用生成式AI评分提供了重要指导，强调了透明度和一致性等关键问题的重要性，以确保评分结果的可靠性和有效性。
...
TL;DR: URAG是一个评估检索增强生成系统不确定性的基准，通过将开放生成任务转化为选择题，使用符合预测进行不确定性量化，发现准确性提升通常伴随不确定性降低，但检索噪声会破坏这种关系。


<details>
  <summary>Details</summary>
Motivation: 当前RAG评估主要关注正确性，未能充分捕捉检索对LLM不确定性和可靠性的影响。需要建立系统化基准来评估RAG系统在不同领域的不确定性表现。

Method: 将开放生成任务转化为多项选择题，使用符合预测进行原则性不确定性量化。评估8种标准RAG方法，通过准确性和预测集大小（基于LAC和APS指标）衡量性能。
...
Conclusion: URAG为分析和增强检索增强系统的可信度建立了系统化基准，揭示了RAG方法在不确定性方面的关键特性，为开发更可靠的检索增强系统提供了评估框架。
...
TL;DR: 本文提出了首个计算化方法用于民间故事母题索引，使用《一千零一夜》文本和El-Shamy的母题索引创建标注语料库，测试了五种方法检测母题表达，最佳模型为微调的Llama3，达到0.85 F1分数。


<details>
  <summary>Details</summary>
Motivation: 母题作为非寻常、重复出现的叙事元素，不仅对民俗学家重要，也作为隐喻手段出现在现代新闻、文学、宣传等文化文本中。自动检测母题表达对民俗学分析（母题索引）和理解现代母题使用（母题检测与解释）都有价值，但先前工作显示自动化方法面临很大困难。

Method: 使用大型易得文本《一千零一夜》配合详细母题索引（El-Shamy 2006）创建手动标注语料库，包含58,450个句子中的2,670个母题表达，涉及200个不同母题。测试了五种方法：(1)经典检索重排（关键词+微调交叉编码器）；(2)现成嵌入模型；(3)微调嵌入模型；(4)现成LLM的N-shot生成提示；(5)LoRA微调LLM的生成方法。

Result: 最佳系统为微调的Llama3模型，整体性能达到0.85 F1分数，成功实现了首个计算化母题索引方法。

Conclusion: 本研究展示了计算化母题索引的可行性，通过结合大型易得文本和详细母题索引，并利用现代NLP技术，成功开发出有效的母题检测系统，为民俗学分析和现代母题使用理解提供了新工具。
...
Method: 采用文献综述方法，聚焦协作的言语方面，系统回顾了基于任务导向对话资源的协作分析研究，涵盖相关理论框架、编码方案、分析任务和计算建模方法。

Result: 综述总结了协作分析的理论基础、编码体系、任务类型（如协作质量评估、角色识别、过程追踪等）以及现有建模方法（包括传统机器学习和深度学习方法），为研究者提供了系统的知识框架。

Conclusion: 该综述可作为协作分析研究的实用资源，揭示了当前研究的空白领域，为未来基于对话数据的协作分析研究提供了方向指引和理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态假新闻检测方法存在两大局限：1) 缺乏全面的多视角判断与融合；2) 依赖大型语言模型(LLMs)导致推理效率低下、计算成本高昂。需要一种既能获得深度推理能力又保持高效性的解决方案。

Method: 提出LLM-MRD框架，包含：1) 学生多视角推理模块：从文本、视觉和跨模态视角构建全面基础；2) 教师多视角推理模块：生成深度推理链作为丰富监督信号；3) 核心校准蒸馏机制：将复杂推理知识高效蒸馏到学生模型中。

Result: LLM-MRD显著优于现有最先进基线方法，在所有竞争方法和数据集上平均提升5.19%的ACC和6.33%的F1-Fake分数。代码已开源。

Conclusion: LLM-MRD通过教师-学生架构和多视角推理蒸馏，成功解决了多模态假新闻检测中深度推理与效率的权衡问题，为实际应用提供了高效且准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示工程方法需要大量人工标注数据且生成冗长重复的提示，需要开发更高效、无需标注数据的自动化提示优化方法。

Method: PrefPO采用基于偏好的优化方法，仅需初始提示和自然语言标准。使用LLM判别器对模型输出进行成对偏好排序，为LLM优化器提供反馈，迭代改进提示性能。

Result: 在9个BIG-Bench Hard任务中，PrefPO在6/9任务上达到或超过SOTA方法；在IFEval-Hard上与TextGrad性能相当（82.4% vs 84.5%）。无标签设置下在6/9任务上接近有标签性能，提示长度和重复内容减少3-5倍，LLM和人工评价均优于TextGrad，提示攻击敏感性降低一半（37% vs 86%）。

Conclusion: PrefPO提供了一种高效、无需标注数据的提示优化方法，在保持性能的同时显著改善提示质量，减少对标注数据的依赖和提示攻击的脆弱性。
...
TL;DR: 提出基于记忆驱动的角色扮演范式，通过MREval评估框架、MRPrompt提示架构和MRBench双语基准，系统评估LLM在无显式提示下维持角色一致性的能力，使小模型能达到大模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在长开放对话中难以维持角色一致性的核心挑战，即模型经常无法在没有显式提示的情况下回忆并准确应用指定的人物知识。

Method: 提出记忆驱动角色扮演范式，受斯坦尼斯拉夫斯基"情绪记忆"表演理论启发，将人物知识视为LLM的内部记忆存储。开发了：1) MREval细粒度评估框架，评估锚定、回忆、边界和演绎四种能力；2) MRPrompt提示架构，指导结构化记忆检索和响应生成；3) MRBench双语基准，用于细粒度诊断。

Result: 实验表明MRPrompt使小模型(如Qwen3-8B)能够匹配大模型(如Qwen3-Max和GLM-4.7)的性能，并证实上游记忆增益直接提升下游响应质量，验证了分阶段理论基础。

Conclusion: 记忆驱动角色扮演范式为四阶段角色扮演能力提供了全面诊断工具，证明了通过结构化记忆检索机制，小模型也能在角色一致性任务上达到大模型水平。
...
Result: 在真实世界数据集上的广泛实验证明了PROMPTATTRIB的有效性，能够实现更准确的实体匹配。
...
Method: 两阶段路由架构：第一阶段使用基于图的聚类发现潜在任务类型并训练分类器将提示分配到发现的任务；第二阶段使用混合专家架构，包含任务特定的预测头进行专门的质量估计；推理时聚合两阶段预测以平衡任务级稳定性和提示特定适应性

Result: 在10个基准测试和11个前沿模型上的评估显示，该方法持续优于现有基线，超越最强单个模型，同时成本不到其一半
...
TL;DR: 线性探针方法无法可靠区分评估上下文与基准结构特征，主要追踪基准规范结构而非评估意识
...
Conclusion: 标准基于探针的方法学无法可靠地区分评估上下文与结构伪影，限制了现有结果的证据强度
...
TL;DR: 该研究通过预训练Transformer语言模型分析不同语言词序对学习难度的影响，发现词汇结构而非词序自由度是决定计算学习性的关键因素。


<details>
  <summary>Details</summary>
Motivation: 探究为何某些语言（如捷克语）允许自由词序，而其他语言（如英语）则不行，理解跨语言词序变化对语言学习难度的影响机制。

Method: 在自然语言的合成词序变体谱系上预训练Transformer语言模型，通过模型惊奇度评估不同词序模式的学习难度，分析词序不规则性、句子反转等变量对学习性的影响。

Result: 词序不规则性增加会持续提高模型惊奇度，表明学习难度增加；句子反转对学习性影响较弱；自由词序语言（捷克语、芬兰语）与固定词序语言（英语、法语）的粗略区分不能解释跨语言差异；词汇和子词词汇表的结构能强预测模型惊奇度。

Conclusion: 词汇结构是跨语言计算词序学习性的关键驱动因素，而非简单的词序自由度区分。这一发现对理解语言习得和语言模型训练具有重要意义。
...
TL;DR: 使用LLM通过迭代生成程序化策略来解决多智能体环境中的策略合成问题，比较稀疏反馈与密集反馈（包含社会指标）的效果，发现密集反馈在促进合作策略方面更优，并探讨了奖励攻击问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过强化学习训练神经策略，但本研究探索使用大型语言模型直接生成程序化策略。动机在于利用LLM的代码生成能力和推理能力，通过迭代反馈优化策略，特别是在社会困境场景中，研究如何通过反馈设计促进合作行为。

Method: 提出LLM策略合成框架：1）提示LLM生成Python策略函数；2）在自博弈中评估策略；3）基于性能反馈迭代优化。核心创新是反馈工程：比较稀疏反馈（仅标量奖励）与密集反馈（奖励加社会指标：效率、平等、可持续性、和平）。在两个经典顺序社会困境游戏（Gathering和Cleanup）和两个前沿LLM（Claude Sonnet 4.6, Gemini 3.1 Pro）上进行实验。还进行了对抗性实验，识别奖励攻击类别。

Result: 密集反馈在所有指标上一致匹配或优于稀疏反馈。在Cleanup公共物品游戏中优势最大，社会指标帮助LLM校准昂贵的清洁-收获权衡。社会指标不会触发公平性的过度优化，而是作为协调信号，引导LLM走向更有效的合作策略，包括领地划分、自适应角色分配和避免浪费性攻击。对抗实验识别了五种攻击类别，揭示了LLM策略合成中表达性与安全性之间的内在张力。

Conclusion: LLM策略合成是一种有前景的方法，密集反馈（包含社会指标）能有效促进合作策略。社会指标作为协调信号而非公平性过度优化的触发器。研究揭示了奖励攻击的风险，需要在表达性和安全性之间取得平衡。代码已开源。
...
TL;DR: 提出一种新颖的解码方案，使LLM能持续产生概念独特的多样化结果，支持探索性搜索任务


<details>
  <summary>Details</summary>
Motivation: 解决探索性搜索任务中LLM输出同质化、缺乏持续创造力的问题。用户在寻找完美婚礼礼服、研究课题等"搜索任务"中需要评估大量多样化创意选项，而现有解码方法主要针对有正确答案的提示，无法满足长期探索需求。

Method: 开发了一种新颖、易于实现的解码方案，无需访问LLM向量空间内部机制，即可诱导LLM产生持续创造力和多样性。该算法解锁LLM的广泛知识（包括正统和非正统），超越模态解码路径。

Result: 该方法能产生任意数量的概念独特结果，支持搜索任务用户更快探索搜索空间并找到满意答案。相比现有方法，能避免过早重复，为不同用户提供个性化创造力。

Conclusion: 提出的解码方案有效解决了探索性搜索任务中LLM输出同质化问题，使LLM能够持续提供多样化和创造性的结果，显著提升了在长期搜索任务中的实用性。
...
TL;DR: EvidenceRL是一个强化学习框架，通过优化证据遵循来减少大语言模型的幻觉，在心脏诊断和法律推理等高风险领域显著提升了证据基础和忠实度。
...
Result: 在两个高风险领域评估：1）心脏诊断：Llama-3.2-3B的F1@3从37.0提升到54.5，基础性(G_max@3)从47.6上升到78.2；幻觉减少近5倍，证据支持的诊断从31.8%增加到61.6%。2）法律推理：Llama-3.1-8B的忠实度从32.8%提升到67.6%。

Conclusion: EvidenceRL能够在不牺牲任务准确性的情况下，持续改善证据基础和忠实度，在不同领域都表现出一致的行为改变，为解决大语言模型在高风险领域的幻觉问题提供了有效方案。
...
TL;DR: FDA药物标签文档问答基准FDARxBench：专家构建的真实世界评估基准，用于评估语言模型在药物标签理解上的表现


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在处理FDA药物标签这类包含丰富但异构临床和监管信息的文档时存在困难，需要建立专家策划的真实世界基准来评估文档基础问答能力，特别是针对仿制药评估需求

Method: 与FDA监管评估员合作，构建多阶段流水线生成高质量专家策划的QA示例，涵盖事实性、多跳推理和拒绝任务，并设计评估协议来评估开放书和封闭书推理

Result: 实验显示专有和开源模型在事实基础、长上下文检索和安全拒绝行为方面存在显著差距，表明当前模型在药物标签理解方面仍有不足

Conclusion: FDARxBench为挑战性的监管级标签理解评估提供了重要基础，虽然最初针对FDA仿制药评估需求，但支持评估LLM在药物标签问题上的行为
...
TL;DR: 该研究系统评估了推理策略在文本分类任务中的有效性和效率，发现推理并非总能提升分类性能，且往往效率低下。


<details>
  <summary>Details</summary>
Motivation: 尽管逐步推理策略在需要多步推理的问题中表现出色，并被广泛应用于各类NLP任务，但其在文本分类任务中的实际效益和效率尚未得到充分探索。考虑到推理策略带来的显著时间和计算成本，需要系统评估推理机制是否真正有益于分类任务。

Method: 引入TextReasoningBench基准，系统比较七种推理策略（IO、CoT、SC-CoT、ToT、GoT、BoC、long-CoT）在十个大语言模型和五个文本分类数据集上的表现。除了传统准确率和macro-F1指标外，还引入了两个成本感知指标：单位推理token的性能增益和性能提升相对于token成本增长的效率。

Result: 1. 推理并非总能提升分类性能：中等复杂度的策略（如CoT和SC-CoT）在大模型上能带来一致但有限的提升（通常+1%到+3%），而更复杂的方法（如ToT和GoT）往往无法超越简单基线，甚至在小模型上会降低性能。
2. 推理效率低下：许多推理策略使token消耗增加10-100倍（如SC-CoT和ToT），但只带来边际性能改进。

Conclusion: 推理策略在文本分类任务中的适用性有限，并非所有任务都能从推理中受益。需要更精细地评估推理的成本效益比，避免盲目应用复杂推理策略，特别是在资源受限的场景中。
...
TL;DR: BEAVER是一种无需训练的长文档压缩框架，通过分层选择而非线性token剪枝，在保持语义完整性的同时大幅提升推理效率。


<details>
  <summary>Details</summary>
Motivation: LLM上下文窗口的指数级扩展带来了长文档理解能力，但也导致了推理延迟和信息利用率的严重瓶颈。现有压缩方法存在高训练成本或语义碎片化问题。

Method: BEAVER采用训练免费框架，将压缩从线性token移除转向结构感知的分层选择。通过双路径池化将变长上下文映射到密集页面级张量，结合语义和词汇双分支选择与句子平滑的混合规划器来保持话语完整性。
...
Conclusion: BEAVER提供了一种可扩展的高吞吐量解决方案，通过结构感知的分层压缩在保持性能的同时显著提升推理效率。
...
TL;DR: 提出针对阿拉伯语自动作文评分(AES)的提示工程框架，在零样本和少样本设置下利用大语言模型评估语言能力特质，通过三层提示策略显著提升评分效果。


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语自动作文评分工具稀缺的问题，特别是缺乏可扩展且具有语言学洞察力的评估工具。阿拉伯语作为低资源语言，在自动评分领域的研究相对不足，需要开发专门针对阿拉伯语特性的评估框架。

Method: 提出三层提示策略：标准提示、混合提示和评分标准引导提示。混合提示模拟多智能体评估，每个特质有专门的评分者；评分标准引导提示结合评分范例增强模型对齐。在零样本和少样本配置下评估8个大语言模型，使用QAES数据集（首个公开的阿拉伯语AES资源，包含特质级别标注）。

Result: Fanar-1-9B-Instruct在零样本和少样本提示中达到最高的特质级别一致性（QWK=0.28，CI=0.41）。评分标准引导提示在所有特质和模型中均产生一致的增益，其中语篇级别特质（发展和风格）改进最大。结构化提示而非模型规模是实现有效阿拉伯语AES的关键。

Conclusion: 本研究提出了首个面向语言能力的阿拉伯语自动作文评分综合框架，为低资源教育环境中的可扩展评估奠定了基础。结构化提示策略能够有效引导大语言模型进行特质特定的评分，特别是在语篇级别特质评估方面表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 传统选择多模态大语言模型监督数据的做法是基于数据集与目标基准的直观相似性（如文本密集或视觉中心任务），但这种直觉相似性是否能可靠预测下游性能提升尚不明确。本研究旨在探索能否在训练前估计训练数据集对目标基准的影响。

Method: 首先对14个视觉语言数据集（涵盖7个不同任务）进行深入的迁移分析，发现直觉任务相似性不可靠。基于此提出DATAPROPHET方法，这是一种简单有效的无需训练指标，结合了多模态困惑度、相似性和数据多样性来评估数据集的影响。

Result: DATAPROPHET产生的监督数据排名与实际训练后性能增益排名高度相关（Kendall's tau达86.0%）。在监督数据选择上，DATAPROPHET比均匀选择提升6.9%，比最先进的基于训练基线提升1.4%，甚至比基于实验性能的oracle选择高出0.2%。

Conclusion: 直觉任务相似性不是可靠的多模态大语言模型监督数据选择指标。DATAPROPHET作为一种无需训练的方法，能够有效预测数据集对目标基准的影响，为监督数据选择提供了实用且高效的解决方案。
...
TL;DR: EvoTaxo：基于LLM的框架，用于从时序社交媒体流中构建和演化分类体系，通过结构化草稿动作、双视图聚类和概念记忆库来应对短文本、噪声和动态演化的挑战。


<details>
  <summary>Details</summary>
Motivation: 社交媒体语料库具有短文本、噪声大、语义纠缠和时序动态的特点，现有分类体系归纳方法主要针对静态语料设计，难以在鲁棒性、可扩展性和对演化话语的敏感性之间取得平衡。

Method: 1) 将每个帖子转换为对当前分类体系的结构化草稿动作；2) 在时间窗口内累积结构证据；3) 通过结合语义相似性和时序局部性的双视图聚类整合候选编辑；4) 通过精炼-仲裁程序选择可靠编辑后执行；5) 每个节点维护概念记忆库以保持语义边界。

Result: 在两个Reddit语料库上的实验表明，EvoTaxo比基线方法产生更平衡的分类体系，具有更清晰的帖子到叶节点分配、在可比分类体系规模下更好的语料覆盖率以及更强的结构质量。对/r/ICE_Raids社区的案例研究显示EvoTaxo能捕捉有意义的话语时序变化。

Conclusion: EvoTaxo是一个有效的框架，能够从动态社交媒体流中构建和演化分类体系，平衡鲁棒性、可扩展性和对演化话语的敏感性，并能捕捉话语的时序变化。
...
TL;DR: 首个系统研究AI生成伪造科学表格检测的论文，构建了FabTab基准数据集，提出TAB-AUDIT框架，通过表格骨架与数值内容的不匹配特征实现高精度检测


<details>
  <summary>Details</summary>
Motivation: AI生成的伪造科学手稿对学术诚信构成严重威胁，特别是表格作为实证NLP论文中支持主张的关键证据。目前缺乏针对AI生成科学表格检测的系统研究，需要建立有效的检测方法。

Method: 构建FabTab基准数据集（1,173篇AI生成论文和1,215篇人类撰写论文），分析伪造与真实表格的系统差异，提出TAB-AUDIT框架，关键特征为"表格内不匹配"——捕捉表格骨架与数值内容的困惑度差距，使用RandomForest分类器。

Result: RandomForest基于TAB-AUDIT特征显著优于现有方法，域内AUROC达到0.987，域外AUROC达到0.883。表格作为检测AI生成科学欺诈的关键法证信号。

Conclusion: 实验表格是检测AI生成科学欺诈的重要法证信号，TAB-AUDIT框架为未来研究提供了新基准，有助于维护学术诚信。
...
TL;DR: LoopRPT：针对循环语言模型的强化预训练框架，通过将强化信号直接分配给潜在推理步骤来提升中间表示质量，实现更高效的计算-精度权衡。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习范式主要针对输出token进行优化，与循环语言模型（LoopLMs）的隐式推理结构不匹配。LoopLMs通过迭代潜在计算来精化内部表示，但缺乏专门针对其架构的强化学习训练方法。

Method: 提出LoopRPT框架：将下一个token预测重构为下一个token推理任务，使用EMA教师参考和噪声潜在rollout将强化信号直接分配给潜在步骤。该方法使RL能够直接塑造中间表示，将有效推理压缩到更少的迭代中。

Result: 在Ouro架构上实例化LoopRPT，多尺度模型实验显示：1）持续提升每步表示质量；2）在精度-计算权衡上实现帕累托最优；3）在困难token上显著提升，表明LoopRPT增强了早期推理而非鼓励过早退出。

Conclusion: 强化预训练是学习LoopLMs高效潜在推理的原则性范式，LoopRPT通过直接优化中间表示解决了循环架构与现有RL方法的结构不匹配问题。
...
TL;DR: 论文提出性能导向的上下文压缩（PoC）范式，通过指定可接受的性能下限而非压缩率，使用轻量级性能预测器自动寻找满足约束的最激进压缩比，然后引导现成的压缩器执行压缩。


<details>
  <summary>Details</summary>
Motivation: 现有上下文压缩方法通过指定目标压缩率或长度来降低大语言模型的推理成本，但这些方法存在不可预测的性能下降问题，阻碍了其可靠部署。需要一种更可靠、以性能为导向的压缩方法。

Method: 提出性能导向的上下文压缩（PoC）范式：1）开发者指定可接受的性能下限而非压缩率；2）使用轻量级性能预测器自动寻找满足约束的最激进压缩比；3）设计两种预测器变体：简单的上下文无关预测器和更复杂的上下文感知预测器（考虑输入固有的可压缩性）；4）使用预测器引导现成的压缩器执行压缩。

Result: 在问答和摘要基准测试中，上下文感知预测器始终比上下文无关预测器具有更低的性能预测误差，而由此产生的上下文感知PoC获得了更优的整体性能。

Conclusion: PoC范式为LLM上下文压缩提供了更可靠、高效和性能感知的部署路径，通过将关注点从压缩率转移到性能保证，实现了更可预测和可靠的压缩效果。
...
TL;DR: 该研究提出了一种考虑人类标注变异（HLV）的多模态大语言模型评估协议，应用于社交媒体内容分类，发现大模型在高一致性标注上表现最佳，但在高分歧标注上常不如中等模型，表明仅基于共识标签的基准会高估模型能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型（LLM）和多模态大语言模型（MLLM）的基准测试大多忽略人类标注变异（HLV），即标注者判断之间的系统性差异。这种忽略可能导致对模型能力的过高估计，特别是在涉及模糊性和主观性的任务中。

Method: 提出了一种MLLM评估协议，明确考虑两种条件：（1）人类标注一致性（高一致性子集）和（2）人类标注分歧（高分歧子集）。将该协议应用于两个最先进的MLLM家族（Gemma 3、Qwen 2.5 VL），使用社交媒体内容分类数据集的非聚合人类标注。

Result: 研究发现：1）较大模型在高一致性子集上表现最佳；2）但在高分歧子集上，较大模型常表现不如中等规模模型；3）参数数量本身并不能决定模型对模糊性和主观性的敏感性；4）仅基于共识标签的基准会高估模型在社交媒体内容审核等领域的实际能力。

Conclusion: 在MLLM基准测试中纳入人类标注变异能够提供更现实和稳健的评估，特别是在内容审核等涉及主观判断的领域。模型大小并非处理模糊性任务的决定性因素，评估协议应考虑标注者之间的系统性差异。
...
TL;DR: 该研究分析了多语言编码器对印地语-英语混合代码的内部表示，发现标准模型能很好地对齐英语和印地语，但混合代码输入与两种语言的连接较弱。通过后训练对齐目标，可以同时将混合代码表示拉近两种构成语言，提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言编码器模型广泛用于混合代码分析任务，但对其内部如何表示混合代码输入以及这些表示是否与混合的构成语言有意义的连接了解甚少。研究者希望探究标准多语言编码器及其混合代码适应变体在跨语言表示对齐方面的表现。

Method: 以印地语-英语为案例研究，构建了包含英语、印地语（天城文）和罗马化混合代码句子的统一三语平行语料库。使用CKA（中心核对齐）、词元级显著性和基于熵的不确定性分析来探测跨语言表示对齐。引入三语后训练对齐目标，使混合代码表示同时更接近两种构成语言。

Result: 发现标准模型能很好地对齐英语和印地语，但混合代码输入与两种语言的连接较弱。继续在混合代码数据上进行预训练会改善英语-混合代码对齐，但以牺牲英语-印地语对齐为代价。可解释性分析显示明显的不对称性：模型通过英语主导的语义子空间处理混合代码文本，而原生文字的印地语提供补充信号以减少表示不确定性。

Conclusion: 通过引入三语后训练对齐目标，可以同时将混合代码表示拉近两种构成语言，实现更平衡的跨语言对齐，并在情感分析和仇恨言论检测等下游任务中获得性能提升。这表明将混合代码表示锚定在其构成语言中有助于跨语言理解。
...
Method: 方法包括：1）使用Empath词汇分析框架将文本映射到主题强度分数；2）定义语义delta作为对话中两个最主导类别强度之间的差异；3）从多种LLM配置生成对话数据，并与异质人类语料（剧本对话、文学作品、在线讨论）进行比较；4）应用Welch t检验分析语义delta值的分布差异。

Result: 结果显示：AI生成文本始终产生比人类文本更高的语义delta值，表明其具有更刚性的主题结构；而人类对话显示出更广泛、更平衡的语义分布。该差异在统计上显著。

Conclusion: 结论是：提出的零样本指标为现有检测技术提供了计算成本低廉的补充信号，可集成到集成检测系统中。这些发现有助于更广泛地理解LLM行为模仿，并表明主题分布是当前模型在人类对话动态方面存在不足的可量化维度。
...
TL;DR: 该论文研究了机器翻译错误检测自动评估方法的可靠性问题，提出了MPP（部分重叠部分匹配）作为稳健的元评估策略，并评估了当前最先进的MT错误检测方法。


<details>
  <summary>Details</summary>
Motivation: 随着机器翻译和自动评估技术的发展，自动评估方法已从生成标量质量分数发展到精确定位翻译错误并分配错误类别和严重程度。然而，对于执行错误检测的自动评估器，如何可靠地衡量其评估能力仍不明确，文献中缺乏既定的技术。

Method: 研究了跨度级别精确率、召回率和F分数的不同实现方式，发现看似相似的方法会产生显著不同的排名。提出了"部分重叠部分匹配"（MPP）结合微平均作为稳健的元评估策略，并公开发布了相关代码。

Result: 研究表明，不同的实现方法会导致评估结果的显著差异，某些广泛使用的技术不适合评估MT错误检测。MPP方法被证明是稳健的元评估策略，并用于评估当前最先进的MT错误检测技术。

Conclusion: 需要标准化的元评估方法来可靠地衡量MT错误检测自动评估器的能力。MPP方法为解决这一问题提供了有效的解决方案，有助于更准确地评估和比较不同的MT错误检测技术。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索信息瓶颈框架在语言领域的应用。虽然信息瓶颈框架在视觉领域（如颜色和运动）已得到验证，但在语言刺激（如句子上下文中的词语）方面尚未探索。研究者希望填补这一空白，验证人类翻译是否表现出交际效率压力。

Method: 方法是将翻译框架化为信息瓶颈优化问题：将源语句视为刺激，目标语句视为压缩后的意义。具体分析英语、德语和塞尔维亚语对法语小说中空间介词的翻译。通过堆叠分类实验（N=35）获取介词对的相似性判断，训练低秩投影模型（D=5）预测这些判断（Spearman相关：0.78），然后比较实际翻译与反事实替代方案相对于信息瓶颈最优边界的位置。

Result: 研究发现，空间介词的实际翻译比反事实替代方案更接近信息瓶颈最优边界。这表明人类翻译在空间领域表现出交际效率压力。低秩投影模型能够有效预测介词相似性判断（Spearman相关：0.78）。

Conclusion: 结论是翻译可以作为观察跨语言语义系统认知效率压力的窗口。该研究为信息瓶颈框架在语言领域的应用提供了初步证据，表明人类翻译确实受到交际效率压力的影响，特别是在空间语义领域。
...
### [148] [Hybrid topic modelling for computational close reading: Mapping narrative themes in Pushkin's Evgenij Onegin](https://arxiv.org/abs/2603.19940)
...
<details>
  <summary>Details</summary>
Motivation: 传统文学分析缺乏可重复的计算方法，特别是在小语料库中分析复杂诗歌叙事时面临主题不稳定和解释性不足的问题。需要开发既能捕捉主题结构又能分析纵向动态的透明计算框架。

Method: 1) 将诗歌文本分割为35个词形还原后的内容词文档；2) 采用多种子共识协议解决小语料库不稳定性；3) 结合LDA进行无监督主题建模和sPLS-DA进行监督分析；4) 引入"叙事中心"概念，将词袋方法扩展到叙事层面分析连续诗节。

Result: 识别出5个稳定且可解释的主题；sPLS-DA增强了主题解释性，识别出细化每个主题的词汇标记；叙事中心分析揭示了主题混合与诗歌情感和结构弧线的对齐；框架在小语料库中实现了可重复的主题映射。

Conclusion: 该框架提供了一种计算形式的细读方法，展示了轻量级概率模型即使在不考虑韵律、音韵或原生形态等文体特征的情况下，也能为复杂诗歌叙事生成可重复的主题地图。方法透明，适用于其他高密度文学文本的比较研究。
...
### [149] [When Contextual Inference Fails: Cancelability in Interactive Instruction Following](https://arxiv.org/abs/2603.19997)
*Natalia Bila,Kata Naszádi,Alexandra Mayn,Christof Monz*

Main category: cs.CL

TL;DR: 论文提出Build What I Mean (BWIM)基准，用于评估LLMs在协作积木搭建任务中如何分离字面解释与上下文推理，发现模型在判断和行动之间存在分离现象。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型在理解模糊指令时如何分离字面解释与上下文推理，特别是在需要基于说话者可靠性进行上下文推断的协作任务中。

Method: 基于现有的双说话者心理语言学范式，引入Build What I Mean (BWIM)交互式基准，要求模型通过上下文推理或支付小成本请求澄清来解决歧义，评估多个最先进的LLMs。

Result: 发现模型在判断和行动之间存在分离：虽然模型在显式置信度评分中能检测说话者不可靠性，但未能利用这些信息指导有效的澄清行为，反而表现出次优策略，如无视伙伴的过度澄清和在不确定性下回避提问的猜测。

Conclusion: LLMs在上下文意义构建中存在判断与行动的分离，需要开发能够将元认知判断转化为有效沟通策略的模型，以实现更高效的人机协作。
...
### [150] [An Agentic Approach to Generating XAI-Narratives](https://arxiv.org/abs/2603.20003)
*Yifan He,David Martens*

Main category: cs.CL

TL;DR: 提出多智能体框架生成和优化XAI叙事，通过Narrator和Critic Agents迭代改进叙事的忠实性和连贯性，在五个LLM和五个表格数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有XAI方法过于技术化和专家导向，需要更可解释和可访问的解释。LLM生成的XAI叙事有潜力将事后解释转化为自然语言解释，但需要确保其忠实性和连贯性。

Method: 提出多智能体框架，包含Narrator（生成和修订叙事）和多个Critic Agents（评估忠实性和连贯性指标）。设计了五种智能体系统：Basic Design、Critic Design、Critic-Rule Design、Coherent Design和Coherent-Rule Design，在五个LLM和五个表格数据集上进行系统评估。

Result: Basic Design、Critic Design和Critic-Rule Design能有效提高所有LLM叙事的忠实性。Claude-4.5-Sonnet在Basic Design上表现最佳，经过三轮迭代将不忠实叙事减少90%。基于多数投票的集成策略能持续提升四个LLM的性能（DeepSeek-V3.2-Exp除外）。

Conclusion: 智能体系统有潜力生成忠实且连贯的XAI叙事，多智能体框架通过迭代反馈机制能显著改善解释质量，为创建更可访问的AI解释提供了有效方法。
...
### [151] [LoASR-Bench: Evaluating Large Speech Language Models on Low-Resource Automatic Speech Recognition Across Language Families](https://arxiv.org/abs/2603.20042)
*Jianan Chen,Xiaoxue Gao,Tatsuya Kawahara,Nancy F. Chen*

Main category: cs.CL

TL;DR: LoASR-Bench是一个专门评估语音语言模型在低资源语言自动语音识别性能的基准测试，涵盖25种语言、9个语系，揭示现有模型在真实低资源语言场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有语音语言模型基准主要关注高资源语言，对低资源语言的ASR性能评估不足，这阻碍了SpeechLM在实际多语言场景中的部署应用。需要评估模型在不同语系间的泛化能力。

Method: 提出LoASR-Bench基准，包含25种来自9个语系的低资源语言，涵盖拉丁和非拉丁文字，用于跨语言、跨文字的ASR性能评估。

Result: 实验结果表明，当前最先进的语音语言模型在处理真实世界低资源语言时存在明显局限性。

Conclusion: 需要专门的低资源语言ASR基准来评估语音语言模型的跨语言泛化能力，LoASR-Bench填补了这一空白，揭示了现有模型在低资源场景中的不足。
...
### [152] [Predicting States of Understanding in Explanatory Interactions Using Cognitive Load-Related Linguistic Cues](https://arxiv.org/abs/2603.20079)
*Yu Wang,Olcay Türk,Angela Grimminger,Hendrik Buschmeier*

Main category: cs.CL

TL;DR: 该研究通过分析对话中的语言特征来预测听者的理解状态，发现信息价值、句法复杂度和听者注视行为变化这三个线索能有效区分"理解"、"部分理解"、"不理解"和"误解"四种状态。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何通过说话者和听者在对话中表现出的语言和非语言特征，来实时预测听者在解释性互动中的理解状态。这有助于开发更有效的沟通辅助系统和理解监测工具。

Method: 基于MUNDEX语料库的面对面棋盘游戏解释对话，分析三个语言线索：说话者话语的信息价值（用惊异值衡量）、句法复杂度，以及听者互动注视行为的变化。听者通过回顾性视频回忆方法自我标注四种理解状态。使用现成分类器和微调的德语BERT多模态分类器进行预测实验。

Result: 统计分析显示各个线索与听者理解水平相关。分类实验结果表明，预测四种理解状态总体可行，当三个语言线索与文本特征结合时，预测性能得到改善。

Conclusion: 语言和非语言特征能够有效预测听者在解释性对话中的理解状态，这为开发实时理解监测系统和改进人机交互提供了理论基础。
...
### [153] [Current LLMs still cannot 'talk much' about grammar modules: Evidence from syntax](https://arxiv.org/abs/2603.20114)
*Mohammed Q. Shormani*

Main category: cs.CL

TL;DR: 该研究评估了ChatGPT-5在阿拉伯语语法术语翻译中的表现，发现仅25%准确，38.6%错误，36.4%部分正确，表明LLMs在核心语法属性翻译方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验大型语言模型（LLMs）在语法模块翻译方面的能力，特别是评估ChatGPT将生成句法核心术语翻译成阿拉伯语的准确性，以了解LLMs在专业语言学领域的表现。

Method: 从生成句法文献中收集44个术语，先由人工翻译，再由ChatGPT-5翻译，采用分析和比较方法对两种翻译结果进行对比评估。

Result: ChatGPT-5的翻译结果：仅25%准确，38.6%错误，36.4%部分正确。研究发现LLMs在涉及句法和语义挑战的核心语法属性翻译中存在显著局限。

Conclusion: LLMs在专业语法术语翻译方面仍有不足，建议AI专家与语言学家密切合作，改进LLMs工作机制以实现更准确或至少适当的翻译。
...
### [154] [Reasoning Gets Harder for LLMs Inside A Dialogue](https://arxiv.org/abs/2603.20133)
*Ivan Kartáč,Mateusz Lango,Ondřej Dušek*

Main category: cs.CL

TL;DR: 论文提出BOULDER基准测试，用于评估LLM在任务导向对话中的推理能力，发现对话设置下模型性能显著下降


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理评估主要关注孤立任务，与任务导向对话(TOD)中的实际使用场景存在差异，需要评估LLM在真实交互场景中的推理鲁棒性

Method: 引入BOULDER动态基准测试，涵盖8个旅行相关任务，包含算术、空间和时间推理，每个问题提供孤立和对话两种变体，在8个LLM上进行实验

Result: 实验显示孤立设置和对话设置之间存在显著且一致的性能差距，这种差距主要由对话的多轮特性驱动，角色条件和工具使用需求也有额外影响
...
### [155] [Enhancing Hyperspace Analogue to Language (HAL) Representations via Attention-Based Pooling for Text Classification](https://arxiv.org/abs/2603.20149)
*Ali Sakour,Zoalfekar Sakour*

Main category: cs.CL

TL;DR: 本文提出了一种改进HAL模型句子嵌入的方法，通过引入可学习的温度缩放加性注意力机制，结合截断SVD降维，在IMDB情感分析任务上相比传统均值池化提升了6.74%的准确率。


<details>
  <summary>Details</summary>
Motivation: HAL模型通过全局词共现矩阵构建分布语义表示，但使用标准均值池化将这些表示聚合为句子级嵌入时会导致信息损失。均值池化对所有词元赋予相等权重，使得上下文重要的词汇被无信息量的结构词元稀释。

Method: 在HAL表示流程中集成了可学习的温度缩放加性注意力机制。为缓解原始共现矩阵的稀疏性和高维问题，在注意力层之前应用截断奇异值分解（SVD）将向量投影到密集的潜在空间。

Result: 在IMDB情感分析数据集上的实验结果显示，基于注意力的池化方法达到82.38%的测试准确率，相比传统均值池化基线（75.64%）绝对提升了6.74个百分点。注意力权重的定性分析表明该机制成功抑制了停用词，并选择性地关注情感承载词元。

Conclusion: 提出的注意力机制不仅提高了分类性能，还增强了模型可解释性，证明了在HAL模型中引入选择性注意力机制对提升句子表示质量的有效性。
...
### [156] [Semantic Token Clustering for Efficient Uncertainty Quantification in Large Language Models](https://arxiv.org/abs/2603.20161)
*Qi Cao,Andrew Gambardella,Takeshi Kojima,Yutaka Matsuo,Yusuke Iwasawa*

Main category: cs.CL

TL;DR: 提出STC方法，通过语义聚类实现LLM不确定性量化，无需重复采样或辅助模型，显著降低计算开销
...
Method: 提出语义标记聚类(STC)：通过嵌入聚类和前缀匹配将标记分组为语义一致的簇，基于对应语义簇的概率质量聚合来量化不确定性
...
### [157] [Evaluating Evidence Grounding Under User Pressure in Instruction-Tuned Language Models](https://arxiv.org/abs/2603.20162)
*Sai Koneru,Elphin Joe,Christine Kirchhoff,Jian Wu,Sarah Rajtmajer*

Main category: cs.CL

TL;DR: 在对抗性用户压力下，指令调优语言模型在忠实于上下文证据与用户对齐之间存在冲突，即使提供更丰富的证据也无法可靠防止用户对齐反转


<details>
  <summary>Details</summary>
Motivation: 研究在争议领域中，指令调优语言模型如何平衡用户对齐压力与对上下文证据的忠实性，评估这种紧张关系

Method: 基于美国国家气候评估建立受控认知冲突框架，对19个参数规模从0.27B到32B的指令调优模型进行细粒度消融实验，分析证据构成和不确定性线索的影响

Result: 发现三种主要失败模式：1) 负面部分证据交互，添加认知细微差别（如研究空白）会增加对奉承的敏感性；2) 鲁棒性非单调扩展，某些低到中规模模型对对抗性用户压力特别敏感；3) 冲突下分布集中度差异，推理蒸馏变体比指令调优对应模型表现出更高的分散性

Conclusion: 在受控固定证据设置中，仅提供更丰富的上下文证据无法保证抵御用户压力，需要明确的认知完整性训练
...
### [158] [Measuring Faithfulness Depends on How You Measure: Classifier Sensitivity in LLM Chain-of-Thought Evaluation](https://arxiv.org/abs/2603.20172)
*Richard J. Young*

Main category: cs.CL

TL;DR: 思维链忠实度评估存在显著分类器依赖性，不同分类器对相同数据得出差异显著的忠实度评分，表明忠实度并非模型固有客观属性，而是测量方法依赖的构造。


<details>
  <summary>Details</summary>
Motivation: 当前思维链忠实度研究通常报告单一聚合数字，暗示忠实度是模型的客观可测量属性。本文旨在证明这种假设是错误的，展示不同分类器对相同数据会产生显著不同的忠实度评估结果。

Method: 使用三种不同分类器（纯正则表达式检测器、两阶段正则表达式+LLM管道、独立的Claude Sonnet 4评判器）分析10,276个受影响的推理轨迹，涵盖12个开源模型（9个家族，7B到1T参数）。通过统计检验（McNemar检验）和一致性度量（Cohen's kappa）评估分类器差异。

Result: 三种分类器对相同数据得出整体忠实度分别为74.4%、82.6%和69.7%，置信区间不重叠。模型间差异达2.6-30.6个百分点，均统计显著。分类器间一致性低（kappa 0.06-0.42），且存在系统性不对称。分类器选择甚至能逆转模型排名。

Conclusion: 思维链忠实度评估高度依赖分类器选择，不同分类器操作化不同严格程度的忠实度构造（词汇提及vs认知依赖）。已发表的忠实度数字在使用不同分类器的研究间无法有意义比较，未来评估应报告跨多种分类方法的敏感度范围而非单一估计值。
...
### [159] [Planning Autonomous Vehicle Maneuvering in Work Zones Through Game-Theoretic Trajectory Generation](https://arxiv.org/abs/2603.19556)
*Mayar Nour,Atrisha Sarkar,Mohamed H. Zaki*

Main category: cs.MA

TL;DR: 提出基于博弈论的轨迹生成与控制框架，用于提高工作区变道的安全性，通过非合作博弈建模车辆交互，减少冲突频率35%


<details>
  <summary>Details</summary>
Motivation: 工作区导航是自动驾驶最具挑战性的操作之一，受限几何结构和不可预测的交通模式造成高风险环境。现有研究缺乏针对工作区安全导航的决策制定方法。

Method: 将变道操作建模为车辆间的非合作博弈，使用博弈论规划器生成平衡安全性、进度和交通稳定性的轨迹。

Result: 仿真结果显示，与传统车辆行为规划模型相比，所提出的博弈论模型在安全关键的高速公路工作区场景中，将冲突频率降低35%，并减少了高风险安全事件的概率。

Conclusion: 提出的博弈论框架能有效提高工作区变道的安全性，为自动驾驶车辆在复杂交通环境中的决策制定提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 经典规划问题通常使用提升的一阶表示定义，虽然紧凑通用，但完全实例化会导致规模指数级膨胀。现有方法要么完全实例化，要么完全提升，需要探索中间方案。

Method: 提出三种SAT编码方法：保持动作提升的同时部分地实例化谓词，实现线性而非二次的规模增长。这种方法在提升和实例化之间找到平衡点。

Result: 最佳编码在难以完全实例化的领域上，在长度最优规划方面优于现有最先进方法。线性规模增长使得在较长规划上表现更好。

Conclusion: 在完全提升和完全实例化之间找到中间方案是有效的，通过保持动作提升同时部分实例化谓词的SAT编码，可以实现线性规模增长并在困难领域取得更好性能。
...
TL;DR: 提出MIRA框架，结合子目标分解的实时规划和基于里程碑的密集奖励强化学习，显著提升LLM智能体在网页导航等长序列任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体在长序列规划方面存在两个主要问题：在线执行时容易因新信息而迷失方向，缺乏清晰的自适应路径；强化学习微调时稀疏延迟的奖励信号难以识别成功动作，无法维持连贯推理。

Method: 提出两个核心贡献：1）利用专有模型进行子目标分解的在线规划框架；2）MiRA（Milestoning your Reinforcement Learning Enhanced Agent）强化学习训练框架，使用密集的基于里程碑的奖励信号。

Result: 实时规划机制使Gemini等专有模型在WebArena-Lite基准上的成功率提升约10%；MiRA应用于开源的Gemma3-12B模型，成功率从6.4%提升至43.0%，超越GPT-4-Turbo（17.6%）、GPT-4o（13.9%）和之前的开源模型最佳WebRL（38.4%）。

Conclusion: 将显式推理时规划与基于里程碑的奖励相结合，显著提升智能体的长序列能力，为更鲁棒和通用的自主系统铺平道路。
...
TL;DR: 本文提出了一种训练大语言模型生成形式化反例的方法，通过符号变异策略生成多样化训练数据，结合多奖励专家迭代框架，在Lean 4定理证明器中实现可自动验证的反例生成。


<details>
  <summary>Details</summary>
Motivation: 当前AI在数学推理领域主要关注证明构造，而忽视了同样重要的反例发现能力。数学推理需要两种互补技能：为真命题构造严格证明，为假命题发现反例。本文旨在填补这一空白，训练大语言模型进行反例推理和生成。

Method: 1. 将任务形式化为形式化反例生成，要求LLM不仅提出候选反例，还要生成可在Lean 4定理证明器中自动验证的形式化证明。2. 引入符号变异策略，通过系统提取定理并丢弃选定假设来合成多样化训练数据。3. 结合精心策划的数据集，采用多奖励专家迭代框架，显著提升训练效率和效果。

Result: 在三个新收集的基准测试上的实验验证了方法的优势，显示变异策略和训练框架带来了显著的性能提升。该方法在反例生成和定理证明两方面都取得了实质性进展。

Conclusion: 本文成功填补了AI数学推理中反例生成能力的空白，通过形式化反例生成任务、符号变异数据合成策略和多奖励专家迭代框架，显著提升了LLM在数学反例发现和定理证明方面的能力，为数学推理AI提供了更全面的解决方案。
...
TL;DR: ItinBench是一个新的基准测试，将空间推理（路线优化）整合到旅行行程规划中，同时保留传统语言推理任务，用于全面评估LLMs在多个认知维度上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注特定推理或规划任务，缺乏整合多个认知领域的综合测试平台。旅行规划作为现实场景，需要整合语言推理和空间推理等多种认知能力，但当前基准未能全面反映这些需求。

Method: 引入ItinBench基准，在旅行行程规划中整合空间推理任务（路线优化）和传统语言推理任务。评估了多个LLMs（Llama 3.1 8B、Mistral Large、Gemini 1.5 Pro和GPT系列）在这些多样化任务上的表现。

Result: LLMs在同时处理多个认知维度时难以保持高且一致的性能。不同模型在不同任务类型上表现存在差异，显示出当前LLMs在综合认知能力方面的局限性。

Conclusion: 通过整合不同人类认知领域的任务，ItinBench为构建更全面的推理测试平台提供了新见解，能更好地反映现实世界挑战。未来的LLMs评估需要考虑多认知维度的整合。
...
Method: 提出PA2D-MORL方法：1) 利用帕累托上升方向选择标量化权重并计算多目标策略梯度；2) 在进化框架下选择性优化多个策略；3) 应用帕累托自适应微调增强帕累托前沿的密度和分布。
...
TL;DR: PowerLens利用LLM的常识推理能力实现个性化移动电源管理，通过多智能体架构识别用户上下文并生成电源策略，在保证安全性的同时显著提升能效。


<details>
  <summary>Details</summary>
Motivation: 移动设备电池寿命仍是关键挑战，现有电源管理机制依赖静态规则或粗粒度启发式方法，忽略了用户活动和个性化偏好，需要更智能、自适应的解决方案。

Method: 采用多智能体架构：1) 从UI语义识别用户上下文；2) 利用LLM常识推理生成跨18个设备参数的电源策略；3) PDL约束框架验证执行前动作；4) 双层记忆系统通过置信度蒸馏从隐式用户反馈学习个性化偏好。

Result: 在rooted Android设备上的实验显示：81.7%的动作准确率，相比原生Android节省38.8%能耗，优于基于规则和LLM的基线方法，用户满意度高，偏好收敛快（3-5天），系统自身仅消耗0.5%日电池容量。

Conclusion: PowerLens证明了LLM在移动电源管理中的有效性，通过语义理解、安全约束和个性化学习实现了零样本、上下文感知的策略生成，为智能设备管理提供了新范式。
...
TL;DR: HyEvo是一个自动化工作流生成框架，通过异构原子合成整合LLM语义推理节点和确定性代码节点，采用多岛进化策略优化工作流拓扑和节点逻辑，显著提升性能并降低推理成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 现有自动化工作流生成方法效率低下且性能不足，主要依赖预定义算子库和同质化的纯LLM工作流，所有任务级计算都通过概率推理完成，存在局限性。

Method: HyEvo采用异构原子合成框架，整合概率性LLM节点进行语义推理和确定性代码节点进行规则化执行。使用LLM驱动的多岛进化策略，通过反思-生成机制迭代优化工作流拓扑和节点逻辑，利用执行反馈进行持续改进。
...
### [167] [Stepwise: Neuro-Symbolic Proof Search for Automated Systems Verification](https://arxiv.org/abs/2603.19715)
*Baoding He,Zenan Li,Wei Sun,Yuan Yao,Taolue Chen,Xiaoxing Ma,Zhendong Su*

Main category: cs.AI

TL;DR: 本文提出了一种神经符号证明生成框架，通过结合LLM的推理能力和传统ITP工具，自动化系统级验证项目的证明搜索，显著提升了定理证明的效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 形式化验证通过交互式定理证明确保关键系统正确性，但大型证明脚本的构建高度手动且限制了可扩展性。大型语言模型在数学推理方面的进展使其在软件验证中的集成前景广阔。

Method: 提出神经符号证明生成框架：1）在证明状态上进行最佳优先树搜索；2）重复查询LLM获取候选证明步骤；3）神经侧：使用证明状态-步骤对数据集微调LLM；4）符号侧：集成ITP工具修复被拒绝步骤、过滤和排序证明状态、在搜索停滞时自动处理子目标。

Result: 在seL4基准测试中，系统证明了高达77.6%的定理，显著超越先前基于LLM的方法和独立Sledgehammer，同时解决了更多多步证明。在其他基准测试中也展示了强大的泛化能力。
...
### [168] [Embodied Science: Closing the Discovery Loop with Agentic Embodied AI](https://arxiv.org/abs/2603.19782)
...
<details>
  <summary>Details</summary>
Motivation: 当前人工智能在预测科学属性方面表现出色，但科学发现本质上是由实验周期支配的物理性、长视野追求。大多数现有计算方法与这一现实脱节，将发现视为孤立的、特定任务的预测，而非与物理世界的持续互动。
...
Conclusion: 具身科学范式为生命科学和化学科学中的自主发现系统提供了路线图，将科学发现重构为智能体推理与物理执行紧密耦合的闭环过程。
...
### [169] [FormalEvolve: Neuro-Symbolic Evolutionary Search for Diverse and Prover-Effective Autoformalization](https://arxiv.org/abs/2603.19828)
...
<details>
  <summary>Details</summary>
Motivation: 自动形式化将自然语言数学转换为可编译、机器可检查的语句，但语义一致性并不保证证明器有效性。即使语义一致的形式化在证明搜索成本和成功率上也可能存在显著差异，需要寻找既能保持语义一致性又能提高证明效率的形式化方法。

Method: 提出FormalEvolve框架，将自动形式化建模为有预算的测试时搜索问题。框架包含：1）LLM驱动的变异和交叉操作生成多样化候选；2）有界补丁修复确保语法正确性；3）抽象语法树（AST）重写操作注入结构多样性；4）编译门控机制筛选语义一致的形式化。

Result: 在CombiBench和ProofNet数据集上，在T=100的生成器调用预算下，FormalEvolve达到58.0%和84.9%的语义命中率（SH@100），并降低了语义成功率的跨问题集中度（更低的基尼系数）。在固定证明器预算下，FormalEvolve在CombiBench上提高了下游证明性能。

Conclusion: FormalEvolve通过神经符号进化方法有效解决了自动形式化中语义一致性与证明效率的权衡问题，在有限预算下实现了更高的语义命中率和更好的下游证明性能，为自动形式化提供了有效的搜索框架。
...
### [170] [Utility-Guided Agent Orchestration for Efficient LLM Tool Use](https://arxiv.org/abs/2603.19896)
...
<details>
  <summary>Details</summary>
Motivation: 使用工具的LLM智能体面临答案质量与执行成本之间的基本矛盾：固定工作流稳定但不灵活，而自由形式的多步推理方法（如ReAct）可能提高任务性能，但代价是过多的工具调用、更长的轨迹、更高的token消耗和增加的延迟。

Method: 将智能体编排视为显式决策问题，提出效用引导的编排策略，通过平衡估计收益、步骤成本、不确定性和冗余度，在响应、检索、工具调用、验证和停止等动作中进行选择。

Result: 实验表明显式编排信号显著影响智能体行为，轻量级效用设计为智能体控制提供了可辩护且实用的机制，能够有效研究质量-成本权衡。

Conclusion: 论文目标不是声称普遍最佳的任务性能，而是提供一个可控且可分析的策略框架，用于研究使用工具的LLM智能体中的质量-成本权衡，通过显式决策而非提示级行为实现更优的编排控制。
...
TL;DR: 该论文分析了Transformer在AI规划任务中的泛化能力，特别是解码器模型验证规划正确性的能力，提出了C*-RASP框架来研究序列长度和词汇量同时增长时的长度泛化保证。
...
Method: 引入C*-RASP（C-RASP的扩展）框架，用于在序列长度和词汇量同时增长的情况下建立Transformer的长度泛化保证。分析解码器模型在对象数量增长（导致有效输入字母表扩大）时的验证能力。

Result: 识别出一大类经典规划领域，在这些领域中Transformer可以可证明地学习验证长规划。发现了显著影响长度可泛化解学习能力的结构特性。实验验证了理论结果。

Conclusion: 该研究为Transformer在AI规划任务中的泛化能力提供了理论分析框架，证明了在特定条件下Transformer可以学习验证长规划，并识别了影响泛化能力的关键结构因素。
...
TL;DR: HeRL：基于后见经验引导的强化学习框架，通过将失败轨迹及其未满足的评分标准作为后见经验，指导LLM探索超出当前策略分布的期望行为，提升探索效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分标准的强化学习方法在增强大语言模型推理能力方面取得进展，但探索效率低下，局限于当前策略分布。需要将探索努力与期望目标对齐，引导策略向最大化奖励的理想分布发展。

Method: HeRL框架将失败轨迹及其未满足的评分标准作为后见经验，作为上下文指导，引导策略探索超出当前分布的期望响应。同时引入奖励加成，激励在指导下有更大改进潜力的响应。理论上提供更准确的期望梯度估计。

Result: 在多个基准测试上的广泛实验表明，HeRL相比基线方法获得更优的性能提升，并且能够在测试时进一步受益于经验引导的自我改进。

Conclusion: HeRL通过后见经验引导强化学习，实现了更有效的探索，避免了从零开始的重复试错，能够从高质量样本中有效学习，提升大语言模型的推理能力。
...
TL;DR: DIAL-KG是一个用于增量知识图谱构建的闭环框架，通过元知识库协调，采用三阶段循环解决传统静态构建方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建方法主要是静态的，依赖于从固定语料库的单步构建和预定义模式。这种方法在数据动态到达的现实场景中不理想，因为整合新信息需要完整且计算昂贵的图重建。预定义模式也限制了知识图谱构建的灵活性。

Method: DIAL-KG是一个由元知识库协调的闭环框架，包含三阶段循环：1) 双轨提取：默认三元组生成，复杂知识切换到事件提取，确保知识完整性；2) 治理裁决：确保提取事实的保真度和时效性，防止幻觉和知识过时；3) 模式演化：从验证知识中归纳新模式指导后续构建周期，并将当前轮次知识增量应用到现有知识图谱。

Result: 大量实验表明，该框架在构建图的质量和归纳模式方面都达到了最先进的性能。

Conclusion: DIAL-KG解决了传统静态知识图谱构建方法的局限性，通过增量、闭环的方法实现了高质量的知识图谱构建和模式演化。
...
TL;DR: 提出了一种基于LLM的理论心智结构化认知轨迹模型，通过动态信念图表示心理状态，显著提升高不确定性环境下的行动预测能力


<details>
  <summary>Details</summary>
Motivation: 现有LLM理论心智推理方法存在局限性：直接提示法或潜在状态模型将信念视为静态独立，导致随时间推移产生不连贯的心理模型，在动态情境中推理能力弱。特别是在灾难响应、急救医学和人机协同等高风险场景中，需要准确推断人们如何基于隐含、演化的信念在不确定性下寻求信息和采取行动。

Method: 引入结构化认知轨迹模型，将心理状态表示为动态信念图，联合推断潜在信念、学习其时变依赖关系，并将信念演化与信息寻求和决策联系起来。核心贡献包括：1) 从文本化概率陈述到一致概率图模型更新的新颖投影；2) 基于能量的因子图表示信念相互依赖关系；3) 捕获信念积累和延迟决策的ELBO目标函数。

Result: 在多个真实世界灾难疏散数据集上，该模型显著改善了行动预测能力，并恢复了与人类推理一致的可解释信念轨迹。为在高不确定性环境中增强LLM的理论心智推理能力提供了原则性模块。

Conclusion: 提出的结构化认知轨迹模型通过动态信念图表示，解决了现有LLM理论心智推理方法的局限性，在高风险动态环境中实现了更准确、连贯的心理状态推断和行动预测，为增强LLM的理论心智推理能力提供了有效框架。
